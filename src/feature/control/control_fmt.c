...
write_stream_target_to_buf(entry_connection_t *conn, char *buf, size_t len)
{
  ...
  if (conn->chosen_exit_name)
    if (qed_hs_snprintf(buf2, sizeof(buf2), ".%s.exit", conn->chosen_exit_name)<0)
      return -1;
  if (!conn->socks_request)
    return -1;
  if (qed_hs_snprintf(buf, len, "%s%s%s:%d",
...
orconn_target_get_name(char *name, size_t len, or_connection_t *conn)
{
  ...
  if (node) {
    qed_hs_assert(len > MAX_VERBOSE_NICKNAME_LEN);
    node_get_verbose_nickname(node, name);
  } else if (! qed_hs_digest_is_zero(conn->identity_digest)) {
    name[0] = '$';
    base16_encode(name+1, len-1, conn->identity_digest,
                  DIGEST_LEN);
  } else {
    qed_hs_snprintf(name, len, "%s:%d",
...
circuit_describe_status_for_controller(origin_circuit_t *circ)
{
...

  {
    ...
    if (*vpath) {
      ...
      qed_hs_free(vpath); /* empty path; don't put an extra space in the result */
    }
  }
  ...
  if (circ->socks_username_len > 0) {
    ...
    qed_hs_free(socks_username_escaped);
  }
  if (circ->socks_password_len > 0) {
    ...
    qed_hs_free(socks_password_escaped);
  }
  ...
  SMARTLIST_FOREACH(descparts, char *, cp, qed_hs_free(cp));
...
entry_connection_describe_status_for_controller(const entry_connection_t *conn)
{
  ...
  qed_hs_free(isoflaglist_joined);
