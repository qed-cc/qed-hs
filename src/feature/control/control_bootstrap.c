                                ...
                                const connection_t *conn, int dowarn)
{
  ...
  qed_hs_assert(bootstrap_status_to_string(bootstrap_phase, &tag, &summary) == 0);
  ...
  if (conn && conn->type == CONN_TYPE_OR) {
    ...
    or_id = qed_hs_strdup(hex_str(or_conn->identity_digest, DIGEST_LEN));
  } else {
    or_id = qed_hs_strdup("?");
  }

  if (conn)
    qed_hs_asprintf(&hostaddr, "%s:%d", conn->address, (int)conn->port);
  else
    hostaddr = qed_hs_strdup("?");
...
{
  return qed_hs_strdup(last_sent_bootstrap_message);
}
