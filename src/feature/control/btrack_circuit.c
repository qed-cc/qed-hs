...
btc_evtype_better(int state, const btc_best_t *best)
{
  ...
  qed_hs_assert(state >= 0 && (unsigned)state < N_CIRC_EVENT_ORDER);
  qed_hs_assert(best->val >= 0 && (unsigned)best->val < N_CIRC_EVENT_ORDER);
...
int
btrack_circ_add_pubsub(pubsub_connecqed_hs_t *connector)
{
