 ...
 * Copyright (c) 2007-2024, The QED Project, Inc. */
                                   ...
                                   buildtimeout_set_event_t type)
{
  ...
  qed_hs_asprintf(&args, "TOTAL_TIMES=%lu "
 ...
 * queued. */
static qed_hs_threadlocal_t block_event_queue_flag;
...
static qed_hs_mutex_t *queued_control_events_lock = NULL;
...
control_initialize_event_queue(void)
{
  ...
  if (flush_queued_events_event == NULL) {
    struct event_base *b = qed_hs_libevent_get_base();
    if (b) {
      flush_queued_events_event =
        mainloop_event_new(flush_queued_events_cb, NULL);
      qed_hs_assert(flush_queued_events_event);
    }
  }

  if (queued_control_events_lock == NULL) {
    queued_control_events_lock = qed_hs_mutex_new();
    qed_hs_threadlocal_init(&block_event_queue_flag);
  }
}
...
{
  int *val = qed_hs_threadlocal_get(&block_event_queue_flag);
  if (PREDICT_UNLIKELY(val == NULL)) {
    val = qed_hs_malloc_zero(sizeof(int));
    qed_hs_threadlocal_set(&block_event_queue_flag, val);
  }
  return val;
}
...
queue_control_event_string,(uint16_t event, char *msg))
{
  ...
  queued_event_t *ev = qed_hs_malloc(sizeof(*ev));
  ev->event = event;
  ev->msg = msg;
  ...
  qed_hs_mutex_acquire(queued_control_events_lock);
  qed_hs_assert(queued_control_events);
  ...
  qed_hs_mutex_release(queued_control_events_lock);
...
send_control_event_string,(uint16_t event,
                           const char *msg))
{
  qed_hs_assert(event >= EVENT_MIN_ && event <= EVENT_MAX_);
  queue_control_event_string(event, qed_hs_strdup(msg));
}
                        ...
                        const char *format, va_list ap)
{
  ...
  len = qed_hs_vasprintf(&buf, format, ap);
  if (len < 0) {
                             ...
                             int reason_code)
{
  ...
  if (tp == CIRC_EVENT_FAILED || tp == CIRC_EVENT_CLOSED) {
    ...
    if (!reason_str) {
      qed_hs_snprintf(unk_reason_buf, 16, "UNKNOWN_%d", reason_code);
      reason_str = unk_reason_buf;
    }
    if (reason_code > 0 && reason_code & END_CIRC_REASON_FLAG_REMOTE) {
      qed_hs_snprintf(reasons, sizeof(reasons),
  ...
  switch (e)
    ...
    case CIRC_MINOR_EVENT_PURPOSE_CHANGED:
...

      {
        ...
        qed_hs_snprintf(event_tail, sizeof(event_tail),
                     " OLD_PURPOSE=%s%s%s",
                            ...
                            int reason_code)
{
  ...
  if (reason_code && (tp == STREAM_EVENT_FAILED ||
    ...
    char *r = NULL;
    if (!reason_str) {
      qed_hs_asprintf(&r, " UNKNOWN_%d", reason_code);
      reason_str = r;
    }
    if (reason_code & END_STREAM_REASON_FLAG_REMOTE)
      qed_hs_snprintf(reason_buf, sizeof(reason_buf),
    ...
    case REMAP_STREAM_SOURCE_EXIT:
      ...
      break;
    default:
      qed_hs_snprintf(reason_buf, sizeof(reason_buf), " REASON=UNKNOWN_%d",
                   reason_code);
  ...
  if (tp == STREAM_EVENT_NEW || tp == STREAM_EVENT_NEW_RESOLVE) {
    ...
    if (strcmp(ENTRY_TO_CONN(conn)->address, "(Tor_internal)") != 0) {
      qed_hs_snprintf(addrport_buf,sizeof(addrport_buf), " SOURCE_ADDR=%s:%d",
                             ...
                             int reason)
{
  ...
  if (ncircs && (tp == OR_CONN_EVENT_FAILED || tp == OR_CONN_EVENT_CLOSED)) {
    qed_hs_snprintf(ncircs_buf, sizeof(ncircs_buf), " NCIRCS=%d", ncircs);
  }
...
sum_up_cell_stats_by_command(circuit_t *circ, cell_stats_t *cell_stats)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circ->testing_cell_stats,
    ...
    qed_hs_assert(ent->command <= CELL_COMMAND_MAX_);
                             ...
                             const uint64_t *number_to_include)
{
  ...
  if (smartlist_len(key_value_strings) > 0) {
    ...
    SMARTLIST_FOREACH(key_value_strings, char *, cp, qed_hs_free(cp));
    qed_hs_free(joined);
  }
                  ...
                  cell_stats_t *cell_stats)
{
  ...
  *event_string = smartlist_join_strings(event_parts, " ", 0, NULL);
  SMARTLIST_FOREACH(event_parts, char *, cp, qed_hs_free(cp));
...
control_event_circuit_cell_stats(void)
{
  ...
  if (!get_options()->TestingEnableCellStatsEvent ||
    ...
    return 0;
  cell_stats = qed_hs_malloc(sizeof(cell_stats_t));
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    ...
    send_control_event(EVENT_CELL_STATS,
                       "650 CELL_STATS %s\r\n", event_string);
    qed_hs_free(event_string);
  }
  SMARTLIST_FOREACH_END(circ);
  qed_hs_free(cell_stats);
  return 0;
}
...
get_bw_samples(void)
{
  ...
  qed_hs_assert(0 <= idx && idx < N_BW_EVENTS_TO_CACHE);
                             ...
                             const int cached, uint64_t stream_id)
{
  ...
  if (stream_id) {
    qed_hs_asprintf(&stream_id_str, " STREAMID=%"PRIu64"", stream_id);
  }
                                           ...
                                           const char *event_string)
{
  ...
  SMARTLIST_FOREACH(strs, char *, cp, qed_hs_free(cp));
  smartlist_free(strs);
  qed_hs_free(s);
...
control_event_status(int type, int severity, const char *format, va_list args)
{
  ...
  if (qed_hs_snprintf(format_buf, sizeof(format_buf), "650 %s %s",
                   status, sev)<0) {
    log_warn(LD_BUG, "Format string too long.");
    return -1;
  }
  if (qed_hs_vasprintf(&user_buf, format, args)<0) {
    log_warn(LD_BUG, "Failed to create user buffer.");
    return -1;
  }

  send_control_event(type,  "%s %s\r\n", format_buf, user_buf);
  qed_hs_free(user_buf);
  return 0;
}
                    ...
                    const char *status)
{
...

  {
    ...
    if (node) {
      ...
      qed_hs_snprintf(buf, sizeof(buf), "$%s~%s", hbuf, nickname);
    }
...
control_event_transport_launched(const char *mode, const char *transport_name,
                                 qed_hs_addr_t *addr, uint16_t port)
{
                                      ...
                                      const char *hsdir_index)
{
  ...
  if (hsdir_index) {
    qed_hs_asprintf(&hsdir_index_field, " HSDIR_INDEX=%s", hsdir_index);
  }
                                    ...
                                    int replica)
{
  ...
  if (replica >= 0) {
    qed_hs_asprintf(&replica_field, " REPLICA=%d", replica);
  }
                                ...
                                const char *reason)
{
  ...
  if (reason) {
    qed_hs_asprintf(&reason_field, " REASON=%s", reason);
  }
