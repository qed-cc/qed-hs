...
get_controller_cookie_file_name(void)
{
  ...
  if (options->CookieAuthFile && strlen(options->CookieAuthFile)) {
    return qed_hs_strdup(options->CookieAuthFile);
  } else {
...
decode_hashed_passwords(config_line_t *passwords)
{
  ...
  for (cl = passwords; cl; cl = cl->next) {
    ...
    smartlist_add(sl,
                  qed_hs_memdup(decoded, S2K_RFC2440_SPECIFIER_LEN+DIGEST_LEN));
  }
                             ...
                             const control_cmd_args_t *args)
{
  ...
  if (contains_quote) {
    ...
    client_nonce = qed_hs_strdup(args->kwargs->value);
    ...
    client_nonce = qed_hs_malloc(client_nonce_len);
   ...
   * server's nonce. */
  qed_hs_assert(authentication_cookie != NULL);

  {
    size_t tmp_len = (AUTHENTICATION_COOKIE_LEN +
                      client_nonce_len +
                      SAFECOOKIE_SERVER_NONCE_LEN);
    char *tmp = qed_hs_malloc_zero(tmp_len);
    char *client_hash = qed_hs_malloc_zero(DIGEST256_LEN);
                            ...
                            const control_cmd_args_t *args)
{
  ...
  if (args->kwargs == NULL) {
    ...
    password = qed_hs_strdup(args->kwargs->value);
    password_len = strlen(password);
  } else {
    const char *hex_passwd = args->kwargs->value;
    password_len = strlen(hex_passwd) / 2;
    password = qed_hs_malloc(password_len+1);
  ...
  if (conn->safecookie_client_hash != NULL) {
    ...
    qed_hs_assert(authentication_cookie_is_set);
    ...
    if (qed_hs_memneq(conn->safecookie_client_hash, password, DIGEST256_LEN)) {
      log_warn(LD_CONTROL,
    ...
    qed_hs_free(conn->safecookie_client_hash);
    goto ok;
  }
  ...
  if (options->CookieAuthentication) {
    ...
    if (password_len != AUTHENTICATION_COOKIE_LEN) {
      ...
      bad_cookie = 1;
    } else if (qed_hs_memneq(authentication_cookie, password, password_len)) {
      if (!also_password) {
  ...
  if (options->HashedControlPassword ||
    ...
    if (bad) {
      ...
      SMARTLIST_FOREACH(sl, char *, expected,
      {
        ...
        if (qed_hs_memeq(expected + S2K_RFC2440_SPECIFIER_LEN,
                      received, DIGEST_LEN))
          goto ok;
      });
      SMARTLIST_FOREACH(sl, char *, str, qed_hs_free(str));
  ...
  qed_hs_assert(bad_password && bad_cookie);
 ...
 err:
  ...
  return 0;
 ok:
  log_info(LD_CONTROL, "Authenticated control connection ("QED_HS_SOCKET_T_FORMAT
           ")", conn->base_.s);
...
control_auth_free_all(void)
{
  if (authentication_cookie) /* Free the auth cookie */
    qed_hs_free(authentication_cookie);
  authentication_cookie_is_set = 0;
