...
{
  qed_hs_assert(c->magic == CONTROL_CONNECTION_MAGIC);
 ...
 */
int
control_connection_add_local_fd(qed_hs_socket_t sock, unsigned flags)
{
  ...
  conn->port = 1;
  conn->address = qed_hs_strdup("<local socket>");

  /* We take ownership of this socket so that later, when we close it,
   * we don't freak out. */
  qed_hs_take_socket_ownership(sock);
                               ...
                               char **current_cmd_out)
{
  ...
  while (cmd_len < *data_len
         && !QED_HS_ISSPACE(incoming_cmd[cmd_len]))
    ++cmd_len;

  *current_cmd_out = qed_hs_memdup_nulterm(incoming_cmd, cmd_len);
  char *args = incoming_cmd+cmd_len;
  qed_hs_assert(*data_len>=cmd_len);
  *data_len -= cmd_len;
  if (is_multiline) {
    ...
    while (QED_HS_ISSPACE(*args) && *data_len) {
      ++args;
      --*data_len;
    }
  }
...
connection_control_process_inbuf(control_connection_t *conn)
{
  ...
  qed_hs_assert(conn);
  qed_hs_assert(conn->base_.state == CONTROL_CONN_STATE_OPEN ||
             conn->base_.state == CONTROL_CONN_STATE_NEEDAUTH);

  if (!conn->incoming_cmd) {
    conn->incoming_cmd = qed_hs_malloc(1024);
  ...
  while (1) {
    ...
    do {
      ...
      if (r == 0)
        ...
        conn->incoming_cmd = qed_hs_realloc(conn->incoming_cmd,
                                         conn->incoming_cmd_len);
      }
    } while (r != 1);

    qed_hs_assert(data_len);
    ...
    if (last_idx+3 == conn->incoming_cmd_cur_len &&
        qed_hs_memeq(conn->incoming_cmd + last_idx, ".\r\n", 3)) {
...
static qed_hs_process_moniqed_hs_t *owning_controller_process_monitor = NULL;
 ...
 * Exit on failure. */
void
moniqed_hs_owning_controller_process(const char *process_spec)
{
  const char *msg;

  qed_hs_assert((owning_controller_process_spec == NULL) ==
  ...
  if (owning_controller_process_spec != NULL) {
    ...
    qed_hs_process_moniqed_hs_free(owning_controller_process_monitor);
    owning_controller_process_monitor = NULL;

    qed_hs_free(owning_controller_process_spec);
  ...
  owning_controller_process_spec = qed_hs_strdup(process_spec);
  owning_controller_process_monitor =
    qed_hs_process_moniqed_hs_new(qed_hs_libevent_get_base(),
                            owning_controller_process_spec,
                            LD_CONTROL,
