                                    ...
                                    control_connection_t *conn)
{
 ...
 err:
  SMARTLIST_FOREACH(key_args, char *, c, qed_hs_free(c));
                                     ...
                                     const control_cmd_args_t *args)
{
  ...
  creds = qed_hs_malloc_zero(sizeof(hs_client_service_authorization_t));
  ...
  for (const config_line_t *line = args->kwargs; line; line = line->next) {
    ...
    if (!strcasecmp(line->key, "ClientName")) {
      ...
      creds->client_name = qed_hs_strdup(line->value);
    }
  }
 ...
 err:
  ...
  SMARTLIST_FOREACH(flags, char *, s, qed_hs_free(s));
                                       ...
                                       hs_client_service_authorization_t *cred)
{
 ...
 err:
  SMARTLIST_FOREACH(control_line, char *, cp, qed_hs_free(cp));
                                      ...
                                      const control_cmd_args_t *args)
{
  ...
  while (!digest256map_iter_done(itr)) {
    ...
    qed_hs_assert_nonfatal(encoding_str);
 ...
 err:
  SMARTLIST_FOREACH(creds_str_list, char *, cp, qed_hs_free(cp));
