...
control_cmd_args_free_(control_cmd_args_t *args)
{
  ...
  if (args->args) {
    SMARTLIST_FOREACH(args->args, char *, c, qed_hs_free(c));
    smartlist_free(args->args);
  }
  config_free_lines(args->kwargs);
  qed_hs_free(args->cmddata);
                          ...
                          char **error_out)
{
  if (result->kwargs == NULL) {
    qed_hs_asprintf(error_out, "Cannot parse keyword argument(s)");
    return -1;
  }
  ...
  for (line = result->kwargs; line; line = line->next) {
    if (! string_array_contains_keyword(syntax->allowed_keywords,
                                        line->key)) {
      qed_hs_asprintf(error_out, "Unrecognized keyword argument %s",
                   escaped(line->key));
      return -1;
    }
  }
...
{
  *error_out = NULL;
  control_cmd_args_t *result = qed_hs_malloc_zero(sizeof(control_cmd_args_t));
  const char *cmdline;
  char *cmdline_alloc = NULL;
  qed_hs_assert(syntax->max_args < INT_MAX || syntax->max_args == UINT_MAX);

  result->command = command;

  if (syntax->store_raw_body) {
    qed_hs_assert(body[body_len] == 0);
  ...
  if (syntax->want_cmddata) {
    if (! eol || (eol+1) == body+body_len) {
      *error_out = qed_hs_strdup("Empty body");
      goto err;
    }
    cmdline_alloc = qed_hs_memdup_nulterm(body, eol-body);
    ...
    if (eol && (eol+1) != body+body_len) {
      *error_out = qed_hs_strdup("Unexpected body");
      goto err;
    }
    cmdline = body;
  }
  ...
  if (n_args < syntax->min_args) {
    qed_hs_asprintf(error_out, "Need at least %u argument(s)",
                 syntax->min_args);
    goto err;
  } else if (n_args > syntax->max_args && ! syntax->accept_keywords) {
    qed_hs_asprintf(error_out, "Cannot accept more than %u argument(s)",
                 syntax->max_args);
    goto err;
  }
    ...
    if (!option_is_recognized(q)) {
      ...
      while (answer) {
        ...
        control_reply_add_one_kv(answers, 250, KV_RAW, answer->key,
                                 answer->value);
        next = answer->next;
        qed_hs_free(answer->key);
        qed_hs_free(answer->value);
        qed_hs_free(answer);
        answer = next;
      }
    }
                      ...
                      const control_cmd_args_t *args)
{
  ...
  qed_hs_assert(smartlist_len(args->args) == 1);
...
get_circ(const char *id)
{
  uint32_t n_id;
  int ok;
  n_id = (uint32_t) qed_hs_parse_ulong(id, 10, 0, UINT32_MAX, &ok, NULL);
  if (!ok)
    return NULL;
...
get_stream(const char *id)
{
  ...
  int ok;
  connection_t *conn;
  n_id = qed_hs_parse_uint64(id, 10, 0, UINT64_MAX, &ok, NULL);
  if (!ok)
    return NULL;
                          ...
                          const control_cmd_args_t *args)
{
  ...
  SMARTLIST_FOREACH(reply, char *, cp, qed_hs_free(cp));
  ...
  */
  if (kwargs) {
    ...
    if (!strcmp(arg1->value, "")) {
      ...
      qed_hs_asprintf(&path_str_alloc, "%s=%s", arg1->key, arg1->value);
  ...
  SMARTLIST_FOREACH(nodes, const node_t *, node,
  {
    ...
    if (!info) {
      qed_hs_assert_nonfatal(first_node);
      log_warn(LD_CONTROL,
  ...
  SMARTLIST_FOREACH(router_nicknames, char *, n, qed_hs_free(n));
  smartlist_free(router_nicknames);
  smartlist_free(nodes);
  qed_hs_free(path_str_alloc);
  return 0;
}
                            ...
                            const control_cmd_args_t *args)
{
  ...
  if (!(ap_conn = get_stream(stream_id))) {
    ...
    if (hoparg) {
      hop = (int) qed_hs_parse_ulong(hoparg->value, 10, 0, INT_MAX,
                                  &hop_line_ok, NULL);
                              ...
                              const control_cmd_args_t *cmd_args)
{
  ...
  if (!(ap_conn = get_stream(smartlist_get(args, 0)))
    ...
    if (smartlist_len(args) > 2) { /* they included a port too */
      new_port = (uint16_t) qed_hs_parse_ulong(smartlist_get(args, 2),
                                            10, 1, 65535, &ok, NULL);
    }
    if (!ok) {
      ...
      new_addr = qed_hs_strdup(smartlist_get(args, 1));
    }
  }
                           ...
                           const control_cmd_args_t *cmd_args)
{
  ...
  qed_hs_assert(smartlist_len(args) >= 2);

  if (!(ap_conn = get_stream(smartlist_get(args, 0))))
    control_printf_endreply(conn, 552, "Unknown stream \"%s\"",
                            (char*)smartlist_get(args, 0));
  else {
    reason = (uint8_t) qed_hs_parse_ulong(smartlist_get(args,1), 10, 0, 255,
                                       &ok, NULL);
    if (!ok) {
                            ...
                            const control_cmd_args_t *cmd_args)
{
  ...
  SMARTLIST_FOREACH(args, const char *, arg, {
      int ok;
      qed_hs_parse_long(arg, 10, 0, LONG_MAX, &ok, NULL);
      if (!ok) {
        bad_arg = arg;
        break;
      }
                         ...
                         const control_cmd_args_t *args)
{
  ...
  for (arg = args->kwargs; arg; arg = arg->next) {
    if (!strcasecmp(arg->key, "Port")) {
      ...
      /* "MaxStreams=[0..65535]". */
      int ok = 0;
      max_streams = (int)qed_hs_parse_long(arg->value, 10, 0, 65535, &ok, NULL);
      if (!ok) {
      ...
      int ok = 0;
      pow_defenses_enabled = (int)qed_hs_parse_long(arg->value, 10,
                                                 0, 1, &ok, NULL);
      if (!ok) {
      ...
      int ok = 0;
      pow_queue_rate = (uint32_t)qed_hs_parse_ulong(arg->value, 10,
      ...
      int ok = 0;
      pow_queue_burst = (uint32_t)qed_hs_parse_ulong(arg->value, 10,
                                                  0, UINT32_MAX, &ok, NULL);
      if (!ok) {
      ...
      smartlist_add(auth_clients_v3_str, qed_hs_strdup(arg->value));
    } else {
      qed_hs_assert_nonfatal_unreached();
      goto out;
    }
                        ...
                        control_connection_t *conn)
{
  ...
  if (!strcasecmp(key_type_ed25519_v3, key_type)) {
    ...
    ed25519_secret_key_t *sk = qed_hs_malloc_zero(sizeof(*sk));
    ...
    *hs_version = HS_VERSION_THREE;
    ...
    if (!strcasecmp(key_type_ed25519_v3, key_blob) ||
      ...
      if (!discard_pk) {
        ...
        key_new_blob = qed_hs_malloc_zero(len);
                         ...
                         const control_cmd_args_t *cmd_args)
{
  ...
  qed_hs_assert(smartlist_len(args) == 1);
...
{
  (void)args;
  char *command = qed_hs_strdup(conn->current_cmd);
  qed_hs_strupper(command);
                       ...
                       char *args)
{
  qed_hs_assert(conn);
  qed_hs_assert(args);
  qed_hs_assert(args[cmd_data_len] == '\0');
...
control_cmd_free_all(void)
{
  if (detached_onion_services) { /* Free the detached onion services */
    SMARTLIST_FOREACH(detached_onion_services, char *, cp, qed_hs_free(cp));
