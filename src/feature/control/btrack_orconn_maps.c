...
bto_update(bt_orconn_t *bto, const bt_orconn_t *key)
{
  ...
  qed_hs_assert(!bto->gid || !key->gid || bto->gid == key->gid);
  if (!bto->gid && key->gid) {
  ...
  qed_hs_assert(!bto->chan || !key->chan || bto->chan == key->chan);
...
{
  struct bt_orconn_t *bto = qed_hs_malloc(sizeof(*bto));
...
bto_init_maps(void)
{
  bto_gid_map = qed_hs_malloc(sizeof(*bto_gid_map));
  HT_INIT(bto_gid_ht, bto_gid_map);
  bto_chan_map = qed_hs_malloc(sizeof(*bto_chan_map));
