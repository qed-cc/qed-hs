                    ...
                    char **answer, const char **errmsg)
{
  ...
  if (!strcmp(question, "version")) {
    ...
    *answer = qed_hs_strdup("VERBOSE_NAMES EXTENDED_EVENTS");
    ...
    *answer = qed_hs_addr_to_str_dup(&addr);
    qed_hs_assert_nonfatal(*answer);
    ...
    *answer = qed_hs_addr_to_str_dup(&addr);
    ...
    qed_hs_asprintf(answer, "%"PRIu64, (get_bytes_read()));
  } else if (!strcmp(question, "traffic/written")) {
    qed_hs_asprintf(answer, "%"PRIu64, (get_bytes_written()));
  } else if (!strcmp(question, "uptime")) {
    long uptime_secs = get_uptime();
    qed_hs_asprintf(answer, "%ld", uptime_secs);
    ...
    qed_hs_asprintf(answer, "%d", myPid);
  } else if (!strcmp(question, "process/uid")) {
#ifdef _WIN32
      *answer = qed_hs_strdup("-1");
#else
      int myUid = geteuid();
      qed_hs_asprintf(answer, "%d", myUid);
...
#else
      int myUid = geteuid();
      const struct passwd *myPwEntry = qed_hs_getpwuid(myUid);

      if (myPwEntry) {
        *answer = qed_hs_strdup(myPwEntry->pw_name);
      } else {
        *answer = qed_hs_strdup("");
      }
#endif /* defined(_WIN32) */
    ...
    qed_hs_asprintf(answer, "%d", max_fds);
  } else if (!strcmp(question, "limits/max-mem-in-queues")) {
    qed_hs_asprintf(answer, "%"PRIu64,
    ...
    *answer = qed_hs_malloc(HEX_DIGEST_LEN+1);
                                ...
                                const signed_descripqed_hs_t *ei)
{
  ...
  outp = out = qed_hs_malloc(ri_len+ei_len+1);
  if (!(router_sig = qed_hs_memstr(ri_body, ri_len, "\nrouter-signature")))
    goto bail;
  ++router_sig;
  ...
  for (i=0; i < 2; ++i) {
    const char *kwd = i ? "\nwrite-history " : "\nread-history ";
    const char *cp, *eol;
    if (!(cp = qed_hs_memstr(ei_body, ei_len, kwd)))
      continue;
    ++cp;
    if (!(eol = memchr(cp, '\n', ei_len - (cp-ei_body))))
      continue;
  ...
  *outp++ = '\0';
  qed_hs_assert(outp-out < (int)(ri_len+ei_len+1));
                         ...
                         char **answer, const char **errmsg)
{
  ...
  SMARTLIST_FOREACH_BEGIN(get_connection_array(), connection_t *, conn) {
    ...
    if (getsockname(conn->s, (struct sockaddr *)&ss, &ss_len) < 0) {
      ...
      char *tmp = qed_hs_sockaddr_to_str((struct sockaddr *)&ss);
  ...
  *answer = smartlist_join_strings(res, " ", 0, NULL);
  ...
  SMARTLIST_FOREACH(res, char *, cp, qed_hs_free(cp));
  smartlist_free(res);
  return 0;
}
                                 ...
                                 const char** errmsg)
{
  ...
  qed_hs_mmap_t *mapped = networkstatus_map_cached_consensus(flavor_name);
  if (mapped) {
    *answer = qed_hs_memdup_nulterm(mapped->data, mapped->size);
    qed_hs_munmap_file(mapped);
  }
  if (!*answer) { /* Maybe it's in the cache? */
    if (we_want_to_fetch_flavor(get_options(), flavor)) {
      ...
      if (consensus) {
        *answer = qed_hs_strdup(consensus->dir);
      }
    }
  }
 ...
 *   - "/qed-hs/server/fp/FP" where FP is a plus-separated sequence of
 *     hex identity digests; or
 *   - "/qed-hs/server/d/D" where D is a plus-separated sequence
                        ...
                        const char **msg)
{
  *msg = NULL;
  ...
  if (!strcmp(key, "/qed-hs/server/all")) {
    routerlist_t *rl = router_get_routerlist();
    SMARTLIST_FOREACH(rl->routers, routerinfo_t *, r,
                      smartlist_add(descs_out, &(r->cache_info)));
  } else if (!strcmp(key, "/qed-hs/server/authority")) {
    const routerinfo_t *ri = router_get_my_routerinfo();
    if (ri)
      smartlist_add(descs_out, (void*) &(ri->cache_info));
  } else if (!strcmpstart(key, "/qed-hs/server/d/")) {
    smartlist_t *digests = smartlist_new();
    key += strlen("/qed-hs/server/d/");
    ...
    SMARTLIST_FOREACH(digests, char *, d, qed_hs_free(d));
    smartlist_free(digests);
  } else if (!strcmpstart(key, "/qed-hs/server/fp/")) {
    smartlist_t *digests = smartlist_new();
    time_t cutoff = time(NULL) - ROUTER_MAX_AGE_TO_PUBLISH;
    key += strlen("/qed-hs/server/fp/");
    dir_split_resource_into_fingerprints(key, digests, NULL,
                                         DSR_HEX|DSR_SORT_UNIQ);
  ...
  if (!strcmpstart(question, "desc/id/")) {
    ...
    *answer = smartlist_join_strings(sl, "", 0, NULL);
    ...
    if (addr_type == ONION_V3_HOSTNAME) {
      ...
      qed_hs_assert(addr_type == ONION_V3_HOSTNAME);
    ...
    SMARTLIST_FOREACH_BEGIN(nodes, node_t *, n) {
      if (n->md && n->md->body) {
        char *copy = qed_hs_strndup(n->md->body, n->md->bodylen);
        smartlist_add(microdescs, copy);
      }
    } SMARTLIST_FOREACH_END(n);

    *answer = smartlist_join_strings(microdescs, "", 0, NULL);
    SMARTLIST_FOREACH(microdescs, char *, md, qed_hs_free(md));
    ...
    if (node) md = node->md;
    if (md && md->body) {
      *answer = qed_hs_strndup(md->body, md->bodylen);
    }
    ...
    int res;
    char *cp;
    qed_hs_asprintf(&url, "/qed-hs/%s", question+4);
                   ...
                   const char **errmsg)
{
  ...
  qed_hs_assert(control_conn != NULL);
  qed_hs_assert(question != NULL);
  qed_hs_assert(answer != NULL);
  qed_hs_assert(errmsg != NULL);
  ...
  *errmsg = NULL;
  ...
  if (dl_to_emit) {
    ...
    *answer = digest_list_to_string(digest_list);
    SMARTLIST_FOREACH(digest_list, void *, s, qed_hs_free(s));
                      ...
                      const char **errmsg)
{
  ...
  if (!strcmp(question, "circuit-status")) {
    ...
    SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ_) {
      ...
      qed_hs_free(circdesc);
    }
    SMARTLIST_FOREACH_END(circ_);
    *answer = smartlist_join_strings(status, "\r\n", 0, NULL);
    SMARTLIST_FOREACH(status, char *, cp, qed_hs_free(cp));
    ...
    *answer = smartlist_join_strings(mappings, "\r\n", 0, NULL);
    SMARTLIST_FOREACH(mappings, char *, cp, qed_hs_free(cp));
    ...
    if (!strcmp(question, "status/circuit-established")) {
      *answer = qed_hs_strdup(have_completed_a_circuit() ? "1" : "0");
    } else if (!strcmp(question, "status/enough-dir-info")) {
      *answer = qed_hs_strdup(router_have_minimum_dir_info() ? "1" : "0");
       ...
       * control-spec.txt. */
      *answer = qed_hs_strdup(directories_have_accepted_server_descriptor()
                           ? "1" : "0");
      ...
      *answer = control_event_boot_last_msg();
      ...
      if (!strcmp(question, "status/version/recommended")) {
        *answer = qed_hs_strdup(recommended);
        return 0;
      }
      if (!strcmp(question, "status/version/current")) {
        switch (status)
          {
          case VS_RECOMMENDED: *answer = qed_hs_strdup("recommended"); break;
          case VS_OLD: *answer = qed_hs_strdup("obsolete"); break;
          case VS_NEW: *answer = qed_hs_strdup("new"); break;
          case VS_NEW_IN_SERIES: *answer = qed_hs_strdup("new in series"); break;
          case VS_UNRECOMMENDED: *answer = qed_hs_strdup("unrecommended"); break;
          case VS_EMPTY: *answer = qed_hs_strdup("none recommended"); break;
          case VS_UNKNOWN: *answer = qed_hs_strdup("unknown"); break;
          default: qed_hs_fragile_assert();
          }
      }
      ...
      *answer = bridge_stats;
      ...
      if ((result = router_build_fresh_descriptor(&r, &e)) < 0) {
        switch (result) {
          case QED_HS_ROUTERINFO_ERROR_NO_EXT_ADDR:
            *errmsg = "Cannot get relay address while generating descriptor";
            break;
          case QED_HS_ROUTERINFO_ERROR_DIGEST_FAILED:
            *errmsg = "Key digest failed";
            break;
          case QED_HS_ROUTERINFO_ERROR_CANNOT_GENERATE:
                      ...
                      const char **errmsg)
{
  ...
  if (strcmp(question, "network-liveness") == 0) {
    if (get_cached_network_liveness()) {
      *answer = qed_hs_strdup("up");
    } else {
      *answer = qed_hs_strdup("down");
    }
  }
                       ...
                       const char **errmsg)
{
  ...
  qed_hs_asprintf(answer, "%d", result);
...
list_getinfo_options(void)
{
  ...
  SMARTLIST_FOREACH(lines, char *, cp, qed_hs_free(cp));
                      ...
                      const char **err_out)
{
  ...
  *answer = NULL; /* unrecognized key by default */
  ...
  for (i = 0; getinfo_items[i].varname; ++i) {
    ...
    if (match) {
      qed_hs_assert(getinfo_items[i].fn);
