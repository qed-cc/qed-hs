...
write_escaped_data(const char *data, size_t len, char **out)
{
  ...
  *outp = '\0'; /* NUL-terminate just in case. */
  qed_hs_assert(outp >= *out);
  qed_hs_assert((size_t)(outp - *out) <= sz_out);
  return outp - *out;
}
                      ...
                      const char *fmt, va_list ap)
{
  ...
  len = qed_hs_vasprintf(&buf, fmt, ap);
  if (len < 0) {
    log_err(LD_BUG, "Unable to format string for controller.");
    qed_hs_assert(0);
  }
                         ...
                         const control_reply_line_t *line, bool lastone)
{
  ...
  if (strpbrk(kvline->value, "\r\n") != NULL) {
    ...
    qed_hs_assert(kvline->next == NULL);
...
{
  control_reply_line_t *line = qed_hs_malloc_zero(sizeof(*line));
...
control_reply_add_printf(smartlist_t *reply, int code, const char *fmt, ...)
{
  ...
  va_start(ap, fmt);
  (void)qed_hs_vasprintf(&buf, fmt, ap);
  va_end(ap);
