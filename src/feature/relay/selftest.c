...
static void inform_testing_reachability(const qed_hs_addr_t *addr,
                                        uint16_t port);
...
router_orport_seems_reachable(const or_options_t *options,
                              int family)
{
  qed_hs_assert_nonfatal(family == AF_INET || family == AF_INET6 || family == 0);
...
extend_info_from_router(const routerinfo_t *r, int family)
{
   ...
   * ReachableAddresses. */
  qed_hs_assert_nonfatal(router_or_conn_should_skip_reachable_address_check(
                                                           get_options(), 0));
                                     ...
                                     int orport_reachable)
{
  ...
  if (ei) {
    ...
    const qed_hs_addr_port_t *ap = extend_info_get_orport(ei, family);
    if (BUG(!ap)) {
...
static void
inform_testing_reachability(const qed_hs_addr_t *addr, uint16_t port)
{
  ...
  char addr_buf[QED_HS_ADDRPORT_BUF_LEN];
  strlcpy(addr_buf, fmt_addrport(addr, port), sizeof(addr_buf));

  const char *afname = fmt_af_family(qed_hs_addr_family(addr));
...
{
  return options->PublishServerDescripqed_hs_ != NO_DIRINFO &&
...
router_orport_found_reachable(int family)
{
  ...
  if (!*can_reach_ptr && me) {
    ...
    char *address = qed_hs_strdup(fmt_addrport_ap(&ap));
