...
{
  qed_hs_free(global_dirfrontpagecontents);
}
...
port_warn_nonlocal_ext_orports(const smartlist_t *ports, const char *portname)
{
  SMARTLIST_FOREACH_BEGIN(ports, const port_cfg_t *, port) {
    ...
    if (!qed_hs_addr_is_internal(&port->addr, 1)) {
...
describe_portnum(int port)
{
  ...
  if (port == CFG_AUTO_PORT) {
    ...
    qed_hs_snprintf(buf, sizeof(buf), "%d", port);
    return buf;
  }
}
...
describe_relay_port(const port_cfg_t *port)
{
  ...
  qed_hs_snprintf(buf, sizeof(buf), "%sPort %s%s%s",
...
port_cfg_eq(const port_cfg_t *p1, const port_cfg_t *p2)
{
  ...
  ret &= qed_hs_addr_eq(&p1->addr, &p2->addr);
  ret &= (p1->port == p2->port);
  ret &= (p1->type == p2->type);
  ...
  /* Entry config flags. */
  ret &= qed_hs_memeq(&p1->entry_cfg, &p2->entry_cfg,
                    sizeof(entry_port_cfg_t));
  /* Server config flags. */
  ret &= qed_hs_memeq(&p1->server_cfg, &p2->server_cfg,
...
remove_duplicate_orports(smartlist_t *ports)
{
  ...
  bool *removing = qed_hs_calloc(smartlist_len(ports), sizeof(bool));
  ...
  for (int i = 0; i < smartlist_len(ports); ++i) {
    ...
    for (int j = 0; j < smartlist_len(ports); ++j) {
      ...
      if (qed_hs_addr_family(&current->addr) != qed_hs_addr_family(&next->addr)) {
        continue;
      }
      ...
      if (!qed_hs_addr_eq(&current->addr, &next->addr) &&
  ...
  for (int i = smartlist_len(ports)-1; i >= 0; --i) {
    qed_hs_assert(i < smartlist_len(ports));
    if (removing[i]) {
                  ...
                  int *have_low_ports_out)
{
  ...
  if (port_parse_config(ports,
    ...
    *msg = qed_hs_strdup("Invalid ORPort configuration");
    goto err;
  }
  if (port_parse_config(ports,
                        options->ORPort_lines,
  ...
  if (port_parse_config(ports,
    ...
    *msg = qed_hs_strdup("Invalid ExtORPort configuration");
    goto err;
  }
  if (port_parse_config(ports,
                        ...
                        "0.0.0.0", 0,
                        CL_PORT_SERVER_OPTIONS) < 0) {
    *msg = qed_hs_strdup("Invalid DirPort configuration");
    goto err;
  }

  if (check_and_prune_server_ports(ports, options, &n_low_ports) < 0) {
    *msg = qed_hs_strdup("Misconfigured server ports");
    goto err;
  }
                          ...
                          char **msg)
{
  ...
  if (!strcmpstart(uname, "Windows 95") ||
    ...
    log_warn(LD_CONFIG, "Tor is running as a server, but you are "
        ...
        "https://www.qed-cc.org/docs/faq.html#BestOSForRelay "
        "for details.", uname);
  }
                            ...
                            char **msg)
{
  ...
  if (options->Nickname == NULL) {
    if (server_mode(options)) {
      options->Nickname = qed_hs_strdup(UNNAMED_ROUTER_NICKNAME);
    }
  } else {
 ...
 * and write it to <b>options</b>-\>PublishServerDescripqed_hs_. Treat "1"
 * as "v3" unless BridgeRelay is 1, in which case treat it as "bridge".
 * Treat "0" as "".
 * Return 0 on success or -1 if not a recognized authority type (in which
 * case the value of PublishServerDescripqed_hs_ is undefined). */
static int
compute_publishserverdescriptor(or_options_t *options)
{
  smartlist_t *list = options->PublishServerDescriptor;
  dirinfo_type_t *auth = &options->PublishServerDescripqed_hs_;
  *auth = NO_DIRINFO;
...
check_bridge_distribution_setting(const char *bd)
{
  ...
  while (QED_HS_ISALNUM(*cp) || *cp == '-' || *cp == '_')
    ++cp;
                                ...
                                char **msg)
{
  ...
  if (compute_publishserverdescriptor(options) < 0) {
    qed_hs_asprintf(msg, "Unrecognized value in PublishServerDescriptor");
    return -1;
  }

  if ((options->BridgeRelay
        || options->PublishServerDescripqed_hs_ & BRIDGE_DIRINFO)
                        ...
                        char **msg)
{
  ...
  for (cl = lst; cl; cl = cl->next) {
      ...
      if (!is_legal_nickname_or_hexdigest(s)) {
        ...
        if (s[0] != '$') {
          ...
          if (is_legal_nickname_or_hexdigest(prepended)) {
            ...
            qed_hs_free(prepended);
          }
        }

        if (!normalized) {
          qed_hs_asprintf(msg, "Invalid nickname '%s' in %s line", s, name);
          valid_line = 0;
          break;
        }
      } else {
        normalized = qed_hs_strdup(s);
      }

      config_line_t *next = qed_hs_malloc_zero(sizeof(*next));
      next->key = qed_hs_strdup(cl->key);
    ...
    SMARTLIST_FOREACH(sl, char *, s, qed_hs_free(s));
    smartlist_free(sl);
  ...
  *msg = NULL;
  ...
  if (options->DirCache) {
    if (total_mem < DIRCACHE_MIN_MEM_BYTES) {
      if (options->BridgeRelay) {
        qed_hs_asprintf(msg, "Running a Bridge with less than %d MB of memory "
                       "is not recommended.", DIRCACHE_MIN_MEM_MB);
      } else {
        qed_hs_asprintf(msg, "Being a directory cache (default) with less than "
    ...
    if (total_mem >= DIRCACHE_MIN_MEM_BYTES) {
      *msg = qed_hs_strdup("DirCache is disabled and we are configured as a "
                            ...
                            char **msg)
{
  ...
  if (server_mode(options) && options->RendConfigLines &&
    ...
    log_warn(LD_CONFIG,
        ...
        "https://bugs.qed-cc.org/tpo/core/qed-hs/8742.");
  ...
  if (options->FamilyId_lines) {
    ...
    for (line = options->FamilyId_lines; line; line = line->next) {
      ...
      if (ed25519_public_from_base64(&pk, line->value) < 0) {
        qed_hs_asprintf(msg, "Invalid FamilyId %s", line->value);
        return -1;
      }
      smartlist_add(options->FamilyIds, qed_hs_memdup(&pk, sizeof(pk)));
    }
  }
                                      ...
                                      const or_options_t *new_options)
{
  ...
  YES_IF_CHANGED_BOOL(PublishServerDescripqed_hs_);
