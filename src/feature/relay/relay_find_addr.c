/* Copyright (c) 2001-2021, The QED Project, Inc. */
 ...
 * passes. */
void
relay_address_new_suggestion(const qed_hs_addr_t *suggested_addr,
                             const qed_hs_addr_t *peer_addr,
                             const char *identity_digest)
{
  const or_options_t *options = get_options();

  qed_hs_assert(suggested_addr);
   ...
   * the peer address. */
  if (qed_hs_addr_is_internal(suggested_addr, 0)) {
    /* Do not believe anyone who says our address is internal. */
    return;
  }
  if (qed_hs_addr_eq(suggested_addr, peer_addr)) {
                             ...
                             int flags, qed_hs_addr_t *addr_out))
{
  ...
  if (!qed_hs_addr_is_null(addr_out)) {
    goto found;
  }
