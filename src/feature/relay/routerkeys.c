...
static qed_hs_cert_t *signing_key_cert = NULL;
static qed_hs_cert_t *link_cert_cert = NULL;
static qed_hs_cert_t *auth_key_cert = NULL;
...
load_ed_keys(const or_options_t *options, time_t now)
{
  ...
  const qed_hs_cert_t *check_signing_cert = NULL;
  qed_hs_cert_t *sign_cert = NULL;
  qed_hs_cert_t *auth_cert = NULL;
  int signing_key_changed = 0;

  // It is later than 1972, since otherwise there would be no C compilers.
  // (Try to diagnose #22466.)
  qed_hs_assert_nonfatal(now >= 2 * 365 * 86400);
...
#define EXPIRES_SOON(cert, interval)            \
  ...
  if (use_signing) {
    ...
    if (! master_signing_key) {
      ...
      signing_key_changed = 1;
               ...
               ! qed_hs_memeq(use_signing->seckey.seckey,
...

  {
    ...
    if (options->master_key_fname) {
      fname = qed_hs_strdup(options->master_key_fname);
    ...
    if (!id) {
      if (need_new_signing_key) {
        ...
        qed_hs_assert(check_signing_cert);
        id = qed_hs_malloc_zero(sizeof(*id));
  ...
  if (sign_cert) {
    ...
    if (! ed25519_pubkey_eq(&sign_cert->signing_key, &id->pubkey))
      FAIL("The signing cert we have was not signed with the master key "
           "we loaded!");
    if (qed_hs_cert_checksig(sign_cert, &id->pubkey, 0) < 0) {
      log_warn(LD_OR, "The signing cert we loaded was not signed "
               "correctly: %s!",
               qed_hs_cert_describe_signature_status(sign_cert));
      goto err;
    }
  }
  ...
  if (want_new_signing_key && sign_signing_key_with_id) {
    ...
    qed_hs_assert(sign_cert->signing_key_included);
    qed_hs_assert(ed25519_pubkey_eq(&sign_cert->signing_key, &id->pubkey));
    qed_hs_assert(ed25519_pubkey_eq(&sign_cert->signed_key, &sign->pubkey));
  } else if (want_new_signing_key) {
  ...
  if (server_mode(options) &&
    ...
    ssize_t crosscert_len = qed_hs_make_rsa_ed25519_crosscert(&id->pubkey,
                                                   get_server_identity_key(),
                                                   expiration,
                                                   &crosscert);
    qed_hs_free(rsa_ed_crosscert);
...
generate_ed_link_cert(const or_options_t *options, time_t now,
                      int force)
{
  const qed_hs_x509_cert_t *link_ = NULL, *id = NULL;
  qed_hs_cert_t *link_cert = NULL;

  if (qed_hs_tls_get_my_certs(1, &link_, &id) < 0 || link_ == NULL) {
    if (!server_mode(options)) {
  ...
  const common_digests_t *digests = qed_hs_x509_cert_get_cert_digests(link_);
  ...
  link_cert = qed_hs_cert_create_raw(get_master_signing_keypair(),
...
#define MAKECERT(cert, signing, signed_, type, flags)            \
  cert = qed_hs_cert_create_ed25519(signing,                        \
                         type,                                   \
                         &signed_->pubkey,                       \
                         time(NULL), 86400,                      \
                         flags);                                 \
  ...
  rsa_ed_crosscert_len = qed_hs_make_rsa_ed25519_crosscert(
 ...
 */
static void
log_ed_cert_expiration(const qed_hs_cert_t *cert,
                       const char *description,
                       const char *fname,
  ...
  if (BUG(!cert)) { /* If the specified key hasn't been loaded */
    ...
    switch (time_format) {
      ...
      case KEY_EXPIRATION_FORMAT_TIMESTAMP:
        qed_hs_snprintf(expiration, sizeof(expiration), "%"PRId64,
...
{
  const qed_hs_cert_t *signing_key;
  char *fn = NULL;
  int failed = 0;
  ...
  qed_hs_assert(server_identity_key_is_set());
...
MOCK_IMPL(const struct qed_hs_cert_st *,
...
{
  smartlist_t *files = qed_hs_listdir(keydir);
 ...
 err:
  ...
  if (files) {
    SMARTLIST_FOREACH(files, char *, cp, qed_hs_free(cp));
                         ...
                         const char *keydir)
{
  ...
  SMARTLIST_FOREACH_BEGIN(key_files, const char *, fn) {
    kp_tmp = qed_hs_malloc_zero(sizeof(*kp_tmp));
  ...
  if (key_files) {
    SMARTLIST_FOREACH(key_files, char *, cp, qed_hs_free(cp));
...
{
  int r = -1;
  ed25519_keypair_t *kp = qed_hs_malloc_zero(sizeof(ed25519_keypair_t));
...
#define FAMILY_INFO_URL \
  "https://community.qed-cc.org/relay/setup/post-install/family-ids/"
      ...
      int *sign_out)
{
  ...
  cert = qed_hs_cert_create_ed25519(&ed_onion_key, CERT_TYPE_ONION_ID,
                             ...
                             int *len_out)
{
  ...
  *len_out = r;
  ...
  return qed_hs_memdup(signature, r);
}
...
routerkeys_free_all(void)
{
  ...
  qed_hs_cert_free(signing_key_cert);
  qed_hs_cert_free(link_cert_cert);
  qed_hs_cert_free(auth_key_cert);
  qed_hs_free(rsa_ed_crosscert);

  if (cached_family_key_file_list) {
    SMARTLIST_FOREACH(cached_family_key_file_list, char *, cp, qed_hs_free(cp));
