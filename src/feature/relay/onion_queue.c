...
typedef struct onion_queue_t {
  QED_HS_TAILQ_ENTRY(onion_queue_t) next;
  or_circuit_t *circ;
  uint16_t queue_idx;
...
QED_HS_TAILQ_HEAD(onion_queue_head_t, onion_queue_t);
...
{ QED_HS_TAILQ_HEAD_INITIALIZER(ol_list[0]), /* tap */
  QED_HS_TAILQ_HEAD_INITIALIZER(ol_list[1]), /* fast */
  QED_HS_TAILQ_HEAD_INITIALIZER(ol_list[2]), /* ntor */
};
...
onion_pending_add(or_circuit_t *circ, create_cell_t *onionskin)
{
  ...
  tmp = qed_hs_malloc_zero(sizeof(onion_queue_t));
  tmp->circ = circ;
...
#define WARN_TOO_MANY_CIRC_CREATIONS_INTERVAL (60)
  ...
  QED_HS_TAILQ_INSERT_TAIL(&ol_list[queue_idx], tmp, next);

  /* cull elderly requests. */
  while (1) {
    onion_queue_t *head = QED_HS_TAILQ_FIRST(&ol_list[queue_idx]);
...
onion_next_task(create_cell_t **onionskin_out)
{
  ...
  onion_queue_t *head = QED_HS_TAILQ_FIRST(&ol_list[handshake_to_choose]);

  if (!head)
    return NULL; /* no onions pending, we're done */

  qed_hs_assert(head->circ);
  qed_hs_assert(head->queue_idx <= MAX_QUEUE_IDX);
//  qed_hs_assert(head->circ->p_chan); /* make sure it's still valid */
...
onion_queue_entry_remove(onion_queue_t *victim)
{
  ...
  QED_HS_TAILQ_REMOVE(&ol_list[victim->queue_idx], victim, next);
  ...
  qed_hs_free(victim->onionskin);
  qed_hs_free(victim);
}

/** Remove all circuits from the pending list.  Called from qed_hs_free_all. */
void
clear_pending_onions(void)
{
  ...
  for (i=0; i<=MAX_QUEUE_IDX; i++) {
    for (victim = QED_HS_TAILQ_FIRST(&ol_list[i]); victim; victim = next) {
      next = QED_HS_TAILQ_NEXT(victim,next);
      onion_queue_entry_remove(victim);
    }
    qed_hs_assert(QED_HS_TAILQ_EMPTY(&ol_list[i]));
  }
