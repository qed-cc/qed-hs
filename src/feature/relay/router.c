 ...
 */
static qed_hs_mutex_t *key_lock=NULL;
 ...
 * used by qed-hs-gencert to sign new signing keys and make new key
 * certificates. */
...
routerinfo_err_to_string(int err)
{
  switch (err) {
    ...
    case QED_HS_ROUTERINFO_ERROR_CANNOT_PARSE:
      return "Cannot parse descriptor";
    case QED_HS_ROUTERINFO_ERROR_NOT_A_SERVER:
    ...
    case QED_HS_ROUTERINFO_ERROR_DESC_REBUILDING:
      return "Descriptor still rebuilding - not ready yet";
    case QED_HS_ROUTERINFO_ERROR_INTERNAL_BUG:
...
routerinfo_err_is_transient(int err)
{
   ...
   * https://bugs.qed-cc.org/tpo/core/qed-hs/27034.
   */
  return err != QED_HS_ROUTERINFO_ERROR_NOT_A_SERVER;
}
...
construct_ntor_key_map(void)
{
  ...
  if (!fast_mem_is_zero((const char *)cur_pk, CURVE25519_PUBKEY_LEN)) {
    dimap_add_entry(&m, cur_pk,
                    qed_hs_memdup(&curve25519_onion_key,
                               sizeof(curve25519_keypair_t)));
  }
  if (!fast_mem_is_zero((const char*)last_pk, CURVE25519_PUBKEY_LEN) &&
      qed_hs_memneq(cur_pk, last_pk, CURVE25519_PUBKEY_LEN)) {
    dimap_add_entry(&m, last_pk,
                    qed_hs_memdup(&last_curve25519_onion_key,
                               sizeof(curve25519_keypair_t)));
  }
...
assert_identity_keys_ok(void)
{
  if (1)
    return;
  qed_hs_assert(client_identitykey);
  if (public_server_mode(get_options())) {
    /* assert that we have set the client and server keys to be equal */
    qed_hs_assert(server_identitykey);
    qed_hs_assert(crypto_pk_eq_keys(client_identitykey, server_identitykey));
  } else {
...
{
  qed_hs_assert(server_identitykey);
  qed_hs_assert(server_mode(get_options()) ||
    ...
    case FN_FILE:
      {
        ...
        if (!tag_in || strcmp(tag_in, tag)) {
          qed_hs_log(severity, LD_GENERAL,"Unexpected tag %s on private key.",
              escaped(tag_in));
          qed_hs_free(tag_in);
          goto error;
        }
...
v3_authority_check_key_expiry(void)
{
  ...
  if (time_left <= 0) {
    qed_hs_log(badness, LD_DIR, "Your v3 authority certificate has expired."
            " Generate a new one NOW.");
  } else if (time_left <= 24*60*60) {
    qed_hs_log(badness, LD_DIR, "Your v3 authority certificate expires in %d "
...
router_initialize_tls_context(void)
{
  ...
  if (public_server_mode(options))
    flags |= QED_HS_TLS_CTX_IS_PUBLIC_SERVER;
  ...
  return qed_hs_tls_context_init(flags,
...
router_announce_bridge_status_page(void)
{
  ...
  log_notice(LD_GENERAL, "You can check the status of your bridge relay at "
                         "https://bridges.qed-cc.org/status?id=%s",
                         fingerprint);
#endif
}
...
router_write_fingerprint(int hashed, int ed25519_identity)
{
  ...
  qed_hs_asprintf(&fingerprint_line, "%s %s\n", options->Nickname, fingerprint);
...
init_keys(void)
{
  ...
  if (authdir_mode_v3(options)) {
    if (init_v3_authority_keys()<0) {
      log_err(LD_GENERAL, "We're configured as a V3 authority, but we "
              ...
              "Use qed-hs-gencert to generate them. Dying.");
      return -1;
    }
  ...
  if (v3_digest_set && (ds->type & V3_DIRINFO) &&
      qed_hs_memneq(v3_digest, ds->v3_identity_digest, DIGEST_LEN)) {
                                 ...
                                 qed_hs_addr_port_t *ipv6_ap_out)
{
  ...
  qed_hs_assert(ipv6_ap_out);

  /* If there is no valid IPv6 ORPort, return a null address and port. */
  qed_hs_addr_make_null(&ipv6_ap_out->addr, AF_INET6);
  ipv6_ap_out->port = 0;

  const qed_hs_addr_t *addr = portconf_get_first_advertised_addr(
  ...
  if (qed_hs_addr_is_internal(addr, 0) && default_auth) {
    log_warn(LD_CONFIG,
  ...
  qed_hs_addr_copy(&ipv6_ap_out->addr, addr);
...
routerconf_has_ipv6_orport(const or_options_t *options)
{
  ...
  return qed_hs_addr_port_is_valid(addr, port, 1);
}
...
router_upload_dir_desc_to_dirservers(int force)
{
  ...
  dirinfo_type_t auth = get_options()->PublishServerDescripqed_hs_;
 ...
 */
int
router_compare_to_my_exit_policy(const qed_hs_addr_t *addr, uint16_t port)
{
  ...
  if ((qed_hs_addr_family(addr) == AF_INET ||
    ...
    return compare_qed_hs_addr_to_addr_policy(addr, port,
    ...
    return get_options()->IPv6Exit &&
      ...
      compare_qed_hs_addr_to_short_policy(addr, port,
...
{
  return (server_identitykey &&
          qed_hs_memeq(server_identitykey_digest, digest, DIGEST_LEN));
}
 ...
 **/
bool
router_addr_is_my_published_addr(const qed_hs_addr_t *addr)
{
  IF_BUG_ONCE(!addr)
    return false;
  ...
  case AF_INET:
    return qed_hs_addr_eq(addr, &me->ipv4_addr);
  case AF_INET6:
    return qed_hs_addr_eq(addr, &me->ipv6_addr);
  default:
    return false;
  }
}
 ...
 * appropriate QED_HS_ROUTERINFO_ERROR_* value on failure.
 */
MOCK_IMPL(const routerinfo_t *,
router_get_my_routerinfo_with_err,(int *err))
{
  if (!server_mode(get_options())) {
    if (err)
      *err = QED_HS_ROUTERINFO_ERROR_NOT_A_SERVER;
  ...
  if (!desc_routerinfo) {
    if (err)
      *err = QED_HS_ROUTERINFO_ERROR_DESC_REBUILDING;
                                                 ...
                                                 int listener_type)
{
  ...
  qed_hs_assert(addr);
  qed_hs_assert(listener_type == CONN_TYPE_OR_LISTENER ||
             listener_type == CONN_TYPE_DIR_LISTENER);

  family = qed_hs_addr_family(addr);
  ...
  if (port_cfg != 0 &&
    ...
    const qed_hs_addr_t *port_addr =
      portconf_get_first_advertised_addr(listener_type, family);
    /* If we're building a descriptor with no advertised address,
     * something is terribly wrong. */
    qed_hs_assert(port_addr);

    char port_addr_str[QED_HS_ADDR_BUF_LEN];
    char desc_addr_str[QED_HS_ADDR_BUF_LEN];

    qed_hs_addr_to_str(port_addr_str, port_addr, QED_HS_ADDR_BUF_LEN, 0);
    qed_hs_addr_to_str(desc_addr_str, addr, QED_HS_ADDR_BUF_LEN, 0);
...
get_my_declared_family(const or_options_t *options)
{
  ...
  for (family = options->MyFamily; family; family = family->next) {
    ...
    if (!member) {
      ...
      char *fp = qed_hs_malloc(HEX_DIGEST_LEN+2);
      fp[0] = '$';
  ...
  nodefamily_t *nf = nodefamily_from_members(declared_family,
                                     ...
                                     NF_WARN_MALFORMED,
                                     NULL);
  SMARTLIST_FOREACH(declared_family, char *, s, qed_hs_free(s));
...
router_build_fresh_unsigned_routerinfo,(routerinfo_t **ri_out))
{
  ...
  const or_options_t *options = get_options();
  int result = QED_HS_ROUTERINFO_ERROR_INTERNAL_BUG;

  if (BUG(!ri_out)) {
    result = QED_HS_ROUTERINFO_ERROR_INTERNAL_BUG;
    goto err;
  }
  ...
  if (!have_v4) {
    ...
    result = QED_HS_ROUTERINFO_ERROR_NO_EXT_ADDR;
    goto err;
  }
  ...
  ri->nickname = qed_hs_strdup(options->Nickname);
  ...
  ri->onion_curve25519_pkey =
    qed_hs_memdup(&get_current_curve25519_keypair()->pubkey,
               sizeof(curve25519_public_key_t));

  ri->identity_pkey = crypto_pk_dup_key(get_server_identity_key());
  if (BUG(crypto_pk_get_digest(ri->identity_pkey,
                           ri->cache_info.identity_digest) < 0)) {
    result = QED_HS_ROUTERINFO_ERROR_DIGEST_FAILED;
    goto err;
  }
  ...
  ri->platform = qed_hs_strdup(platform);

  ri->protocol_list = qed_hs_strdup(protover_get_supported_protocols());
...
router_build_fresh_unsigned_extrainfo(const routerinfo_t *ri)
{
  ...
  ei = qed_hs_malloc_zero(sizeof(extrainfo_t));
...
#define FORCE_REGENERATE_DESCRIPQED_HS_INTERVAL (18*60*60)
...
#define FAST_RETRY_DESCRIPQED_HS_INTERVAL (90*60)
...
mark_my_descriptor_dirty_if_too_old(time_t now)
{
  ...
  const time_t slow_cutoff = now - FORCE_REGENERATE_DESCRIPQED_HS_INTERVAL;
  const time_t fast_cutoff = now - FAST_RETRY_DESCRIPQED_HS_INTERVAL;
...
mark_my_descriptor_dirty(const char *reason)
{
  ...
  if (server_mode(options) && options->PublishServerDescripqed_hs_) {
...
log_addr_has_changed(int severity,
                     const qed_hs_addr_t *prev,
                     const qed_hs_addr_t *cur,
                     const char *source)
{
  char addrbuf_prev[QED_HS_ADDR_BUF_LEN];
  char addrbuf_cur[QED_HS_ADDR_BUF_LEN];

  if (BUG(!server_mode(get_options())))
    return;

  if (qed_hs_addr_to_str(addrbuf_prev, prev, sizeof(addrbuf_prev), 1) == NULL)
    strlcpy(addrbuf_prev, "???", QED_HS_ADDR_BUF_LEN);
  if (qed_hs_addr_to_str(addrbuf_cur, cur, sizeof(addrbuf_cur), 1) == NULL)
    strlcpy(addrbuf_cur, "???", QED_HS_ADDR_BUF_LEN);

  if (!qed_hs_addr_is_null(prev))
...
check_descriptor_ipaddress_changed(time_t now)
{
  ...
  for (size_t i = 0; i < ARRAY_LENGTH(families); i++) {
    qed_hs_addr_t current;
    const qed_hs_addr_t *previous;
    ...
    if (!qed_hs_addr_eq(previous, &current)) {
      char *source;
      qed_hs_asprintf(&source, "METHOD=%s%s%s",
...
{
  qed_hs_snprintf(platform, len, "Tor %s on %s",
                             ...
                             const ed25519_keypair_t *signing_keypair)
{
  ...
  if (emit_ed_sigs) {
    ...
    qed_hs_asprintf(&ed_cert_line, "identity-ed25519\n"
                 "-----BEGIN ED25519 CERT-----\n"
                 "%s"
                 "-----END ED25519 CERT-----\n"
  ...
  if (tap_key && rsa_pubkey && router->cache_info.signing_key_cert &&
    ...
    qed_hs_asprintf(&rsa_tap_cc_line,
                 "onion-key-crosscert\n"
                 "-----BEGIN CROSSCERT-----\n"
                 "%s"
                 "-----END CROSSCERT-----\n", buf);
  ...
  if (router->declared_family && smartlist_len(router->declared_family)) {
    char *family = smartlist_join_strings(router->declared_family,
                                          " ", 0, NULL);
    qed_hs_asprintf(&family_line, "family %s\n", family);
    qed_hs_free(family);
  } else {
    family_line = qed_hs_strdup("");
  }

  if (!qed_hs_digest_is_zero(router->cache_info.extra_info_digest)) {
    char extra_info_digest[HEX_DIGEST_LEN+1];
    base16_encode(extra_info_digest, sizeof(extra_info_digest),
                  router->cache_info.extra_info_digest, DIGEST_LEN);
    if (!qed_hs_digest256_is_zero(router->cache_info.extra_info_digest256)) {
      char d256_64[BASE64_DIGEST256_LEN+1];
      digest256_to_base64(d256_64, router->cache_info.extra_info_digest256);
      qed_hs_asprintf(&extra_info_line, "extra-info-digest %s %s\n",
                   extra_info_digest, d256_64);
    } else {
      qed_hs_asprintf(&extra_info_line, "extra-info-digest %s\n",
                   extra_info_digest);
    }
  }

  if (!omit_ipv6_on_publish && router->ipv6_orport &&
      qed_hs_addr_family(&router->ipv6_addr) == AF_INET6) {
    char addr[QED_HS_ADDR_BUF_LEN];
    const char *a;
    a = qed_hs_addr_to_str(addr, &router->ipv6_addr, sizeof(addr), 1);
    if (a) {
      qed_hs_asprintf(&extra_or_address,
  ...
  if (router->protocol_list) {
    qed_hs_asprintf(&proto_line, "proto %s\n", router->protocol_list);
  } else {
    proto_line = qed_hs_strdup("");
  }

  address = qed_hs_addr_to_str_dup(&router->ipv4_addr);
  if (!address)
    goto err;
...
#define FAMILY_CERT_LIFETIME (30*86400)
    qed_hs_cert_t *family_cert = qed_hs_cert_create_ed25519(
          k_family_id,
  ...
  if (options->BridgeRelay) {
    ...
    if (options->BridgeDistribution && strlen(options->BridgeDistribution)) {
      bd = qed_hs_strdup(options->BridgeDistribution);
    } else {
      bd = qed_hs_strdup("any");
    }
 ...
 err:
  qed_hs_free(output); /* sets output to NULL */
 done:
  if (chunks) {
  ...
  qed_hs_free(address);
  qed_hs_free(family_line);
  qed_hs_free(onion_pkey);
  qed_hs_free(identity_pkey);
...
{
  if (warned_family) {
    SMARTLIST_FOREACH(warned_family, char *, cp, qed_hs_free(cp));
                          ...
                          size_t *onion_pkey_len_out)
{
  ...
  qed_hs_assert(pk);
  qed_hs_assert(onion_pkey_out);
  qed_hs_assert(onion_pkey_len_out);
  ...
  *onion_pkey_out = qed_hs_memdup(buf, len);
