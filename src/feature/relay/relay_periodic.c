 ...
 * address has changed. */
#define CHECK_DESCRIPQED_HS_INTERVAL (60)
  ...
  return CHECK_DESCRIPQED_HS_INTERVAL;
}
...
reachability_warnings_callback(time_t now, const or_options_t *options)
{
  ...
  if (server_mode(options) &&
    ...
    if (me && !(v4_ok && v6_ok)) {
      ...
      char *address4 = qed_hs_addr_to_str_dup(&me->ipv4_addr);
      char *address6 = qed_hs_addr_to_str_dup(&me->ipv6_addr);
      if (address4 || address6) {
        char *where4=NULL, *where6=NULL;
        if (!v4_ok)
          qed_hs_asprintf(&where4, "%s:%d", address4, me->ipv4_orport);
        if (!v6_ok)
          qed_hs_asprintf(&where6, "[%s]:%d", address6, me->ipv6_orport);
      ...
      qed_hs_free(address4);
      qed_hs_free(address6);
    }
  }
 ...
 * descriptor immediately, rather than after up to CHECK_DESCRIPQED_HS_INTERVAL
 * seconds.
 */
void
