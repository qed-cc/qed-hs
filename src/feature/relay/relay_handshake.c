...
add_certs_cell_cert_helper(certs_cell_t *certs_cell,
                           ...
                           size_t cert_len)
{
  qed_hs_assert(cert_len <= UINT16_MAX);
              ...
              const qed_hs_x509_cert_t *cert)
{
  ...
  size_t cert_len;
  qed_hs_x509_cert_get_der(cert, &cert_encoded, &cert_len);
...
{
  const qed_hs_x509_cert_t *global_link_cert = NULL, *id_cert = NULL;
  qed_hs_x509_cert_t *own_link_cert = NULL;
  var_cell_t *cell;

  certs_cell_t *certs_cell = NULL;

  qed_hs_assert(conn->base_.state == OR_CONN_STATE_OR_HANDSHAKING_V3);
  ...
  if (qed_hs_tls_get_my_certs(conn_in_server_mode,
                           &global_link_cert, &id_cert) < 0)
    return -1;

  if (conn_in_server_mode) {
    own_link_cert = qed_hs_tls_get_own_cert(conn->tls);
  }
  qed_hs_assert(id_cert);

  certs_cell = certs_cell_new();

  /* Start adding certs.  First the link cert or auth1024 cert. */
  if (conn_in_server_mode) {
    qed_hs_assert_nonfatal(own_link_cert);
    add_x509_cert(certs_cell,
                  OR_CERT_TYPE_TLS_LINK, own_link_cert);
  } else {
    qed_hs_assert(global_link_cert);
    add_x509_cert(certs_cell,
  ...
  if (conn_in_server_mode) {
    qed_hs_assert_nonfatal(conn->handshake_state->own_link_cert ||
...
connection_or_send_auth_challenge_cell(or_connection_t *conn)
{
  ...
  qed_hs_assert(sizeof(ac->challenge) == 32);
                                      ...
                                      int server)
{
...

  {
    ...
    if (qed_hs_tls_get_my_certs(server, NULL, &id_cert))
      goto err;
    my_digests = qed_hs_x509_cert_get_id_digests(id_cert);
    their_digests =
      qed_hs_x509_cert_get_id_digests(conn->handshake_state->certs->id_cert);
    qed_hs_assert(my_digests);
    qed_hs_assert(their_digests);
...

  {
    ...
    if (server) {
      cert = qed_hs_tls_get_own_cert(conn->tls);
    } else {
      cert = qed_hs_tls_get_peer_cert(conn->tls);
    }
    if (!cert) {
    ...
    memcpy(auth->scert,
           qed_hs_x509_cert_get_cert_digests(cert)->d[DIGEST_SHA256], 32);
  ...
  {
    char label[128];
    qed_hs_snprintf(label, sizeof(label),
                 "EXPORTER FOR TOR TLS CLIENT BINDING %s", authtype_str);
    int r = qed_hs_tls_export_key_material(conn->tls, auth->tlssecrets,
  ...
  qed_hs_assert(len + AUTH_CELL_HEADER_LEN <= result->payload_len);
