...
circuit_extend_add_ed25519_helper(struct extend_cell_t *ec)
{
  ...
  if (qed_hs_digest_is_zero((const char*)ec->node_id)) {
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
...
circuit_extend_add_ipv4_helper(struct extend_cell_t *ec)
{
  ...
  const node_t *node = node_get_by_id((const char *) ec->node_id);
  if (node) {
    qed_hs_addr_port_t node_ipv4;
    node_get_prim_orport(node, &node_ipv4);
    if (qed_hs_addr_is_null(&ec->orport_ipv4.addr) &&
        !qed_hs_addr_is_null(&node_ipv4.addr)) {
      qed_hs_addr_copy(&ec->orport_ipv4.addr, &node_ipv4.addr);
...
circuit_extend_add_ipv6_helper(struct extend_cell_t *ec)
{
  ...
  const node_t *node = node_get_by_id((const char *) ec->node_id);
  if (node) {
    qed_hs_addr_port_t node_ipv6;
    node_get_pref_ipv6_orport(node, &node_ipv6);
    if (qed_hs_addr_is_null(&ec->orport_ipv6.addr) &&
        !qed_hs_addr_is_null(&node_ipv6.addr)) {
      qed_hs_addr_copy(&ec->orport_ipv6.addr, &node_ipv6.addr);
...
/* Check if the address and port in the qed_hs_addr_port_t <b>ap</b> are valid,
 ...
 */
static bool
circuit_extend_addr_port_is_valid(const struct qed_hs_addr_port_t *ap,
                                  ...
                                  int log_level)
{
  ...
  if (qed_hs_addr_is_internal(&ap->addr, 0) &&
                                  ...
                                  const struct circuit_t *circ)
{
  ...
  if (qed_hs_memeq(ec->node_id, p_chan->identity_digest, DIGEST_LEN)) {
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
...
circuit_choose_ip_ap_for_extend(const qed_hs_addr_port_t *ipv4_ap,
                                const qed_hs_addr_port_t *ipv6_ap)
{
                                   ...
                                   int should_launch)
{
  ...
  const qed_hs_addr_port_t *chosen_ap = circuit_choose_ip_ap_for_extend(
  ...
  circ->n_chan_create_cell = qed_hs_memdup(&ec->create_cell,
                 ...
                 const uint8_t *rend_circ_nonce)
{
  ...
  qed_hs_assert(keys_len == CPATH_KEY_MATERIAL_LEN);
  ...
  if ((!channel_is_local(circ->p_chan)
    ...
    const qed_hs_addr_t *my_supposed_addr = &circ->p_chan->addr_according_to_peer;
    if (router_addr_is_my_published_addr(my_supposed_addr)) {
      ...
      int family = qed_hs_addr_family(my_supposed_addr);
      if (channel_get_addr_if_possible(circ->p_chan, &remote_addr) &&
          qed_hs_addr_family(&remote_addr) == family) {
