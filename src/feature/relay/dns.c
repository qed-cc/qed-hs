...
HT_GENERATE2(cache_map, cached_resolve_t, node, cached_resolve_hash,
             cached_resolves_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
...
evdns_log_cb(int warn, const char *msg)
{
  ...
  if (!strcmpstart(msg, "Nameserver ") && (cp=strstr(msg, " has failed: "))) {
    char *ns = qed_hs_strndup(msg+11, cp-(msg+11));
  ...
  qed_hs_log(severity, LD_EXIT, "eventdns: %s", msg);
}
...
dns_reset(void)
{
  ...
  if (! server_mode(options)) {
    ...
    if (!the_evdns_base) {
      if (!(the_evdns_base = evdns_base_new(qed_hs_libevent_get_base(), 0))) {
...
free_cached_resolve_(cached_resolve_t *r)
{
  ...
  if (r->res_status_hostname == RES_STATUS_DONE_OK)
    qed_hs_free(r->result_ptr.hostname);
  r->magic = 0xFF00FF00;
  qed_hs_free(r);
}
...
cached_resolve_add_answer(cached_resolve_t *resolve,
                          int query_type,
                          int dns_result,
                          const qed_hs_addr_t *answer_addr,
                          ...
                          uint32_t ttl)
{
  if (query_type == DNS_PTR) {
    ...
    if (dns_result == DNS_ERR_NONE && answer_hostname) {
      resolve->result_ptr.hostname = qed_hs_strdup(answer_hostname);
    ...
    if (dns_result == DNS_ERR_NONE && answer_addr &&
        qed_hs_addr_family(answer_addr) == AF_INET) {
      resolve->result_ipv4.addr_ipv4 = qed_hs_addr_to_ipv4h(answer_addr);
    ...
    if (dns_result == DNS_ERR_NONE && answer_addr &&
        qed_hs_addr_family(answer_addr) == AF_INET6) {
      memcpy(&resolve->result_ipv6.addr_ipv6,
             qed_hs_addr_to_in6(answer_addr),
...
{
  qed_hs_assert(resolve && resolve->expire == 0);
...
purge_expired_resolves(time_t now)
{
  ...
  while (smartlist_len(cached_resolve_pqueue)) {
    ...
    if (resolve->state == CACHE_STATE_PENDING) {
      ...
      qed_hs_assert(!resolve->pending_connections);
    } else {
      qed_hs_assert(resolve->state == CACHE_STATE_DONE);
      qed_hs_assert(!resolve->pending_connections);
    }
    ...
    if (resolve->pending_connections) {
      ...
      while (resolve->pending_connections) {
        ...
        qed_hs_assert(!SOCKET_OK(pend->conn->base_.s));
        pendconn = pend->conn;
                 ...
                 cached_resolve_t **resolve_out))
{
  ...
  *made_connection_pending_out = 0;
  ...
  if (address_is_invalid_destination(exitconn->base_.address, 0)) {
    qed_hs_log(LOG_PROTOCOL_WARN, LD_EXIT,
  ...
  qed_hs_strlower(exitconn->base_.address);

  /* Check whether this is a reverse lookup.  If it's malformed, or it's a
   * .in-addr.arpa address but this isn't a resolve request, kill the
   * connection.
   */
  if ((r = qed_hs_addr_parse_PTR_name(&addr, exitconn->base_.address,
                                              AF_UNSPEC, 0)) != 0) {
    if (r == 1) {
      is_reverse = 1;
      if (qed_hs_addr_is_internal(&addr, 0)) /* internal address? */
        return -1;
    }
  ...
  if (resolve && resolve->expire > now) { /* already there */
    switch (resolve->state) {
      case CACHE_STATE_PENDING:
        ...
        pending_connection = qed_hs_malloc_zero(
                                      sizeof(pending_connection_t));
        pending_connection->conn = exitconn;
        pending_connection->next = resolve->pending_connections;
        resolve->pending_connections = pending_connection;
        *made_connection_pending_out = 1;
        log_debug(LD_EXIT,"Connection (fd "QED_HS_SOCKET_T_FORMAT") waiting "
                  "for pending DNS resolve of %s", exitconn->base_.s,
                  escaped_safe_str(exitconn->base_.address));
        return 0;
      case CACHE_STATE_CACHED:
        log_debug(LD_EXIT,"Connection (fd "QED_HS_SOCKET_T_FORMAT") found "
                  "cached answer for %s",
                  exitconn->base_.s,
                  escaped_safe_str(resolve->address));
  ...
  qed_hs_assert(!resolve);
  /* not there, need to add it */
  resolve = qed_hs_malloc_zero(sizeof(cached_resolve_t));
  ...
  pending_connection = qed_hs_malloc_zero(sizeof(pending_connection_t));
  pending_connection->conn = exitconn;
  resolve->pending_connections = pending_connection;
  *made_connection_pending_out = 1;
                                ...
                                char **hostname_out))
{
  ...
  qed_hs_assert(exitconn);
  qed_hs_assert(resolve);

  if (exitconn->is_reverse_dns_lookup) {
    exitconn->address_ttl = resolve->ttl_hostname;
    if (resolve->res_status_hostname == RES_STATUS_DONE_OK) {
      *hostname_out = qed_hs_strdup(resolve->result_ptr.hostname);
      return 1;
    } else {
      return -1;
    }
  ...
  if (ipv4_ok && ipv6_ok && is_resolve) {
    ...
    qed_hs_addr_t a4, a6;
    qed_hs_addr_from_ipv4h(&a4, resolve->result_ipv4.addr_ipv4);
    qed_hs_addr_from_in6(&a6, &resolve->result_ipv6.addr_ipv6);
  ...
  if (answer_with_ipv4) {
    if (resolve->res_status_ipv4 == RES_STATUS_DONE_OK) {
      qed_hs_addr_from_ipv4h(&exitconn->base_.addr,
    ...
    if (resolve->res_status_ipv6 == RES_STATUS_DONE_OK) {
      qed_hs_addr_from_in6(&exitconn->base_.addr,
...
connection_dns_remove(edge_connection_t *conn)
{
  ...
  qed_hs_assert(conn->base_.type == CONN_TYPE_EXIT);
  qed_hs_assert(conn->base_.state == EXIT_CONN_STATE_RESOLVING);
  ...
  if (pend->conn == conn) {
    resolve->pending_connections = pend->next;
    qed_hs_free(pend);
    log_debug(LD_EXIT, "First connection (fd "QED_HS_SOCKET_T_FORMAT") no "
    ...
    for ( ; pend->next; pend = pend->next) {
      if (pend->next->conn == conn) {
        ...
        qed_hs_free(victim);
        log_debug(LD_EXIT,
                  "Connection (fd "QED_HS_SOCKET_T_FORMAT") no longer waiting "
                  "for resolve of %s",
                  conn->base_.s, escaped_safe_str(conn->base_.address));
        return; /* more are pending */
      }
    }
    log_warn(LD_BUG, "Connection (fd "QED_HS_SOCKET_T_FORMAT") was not waiting "
...
dns_cancel_pending_resolve,(const char *address))
{
  ...
  while (resolve->pending_connections) {
    ...
    qed_hs_assert(!SOCKET_OK(pendconn->base_.s));
    if (!pendconn->base_.marked_for_close) {
  ...
  if (resolve->state != CACHE_STATE_PENDING) {
    ...
    if (!is_test_addr)
      log_notice(LD_EXIT,
                 ...
                 escaped_safe_str(address));
    qed_hs_assert(resolve->pending_connections == NULL);
    return;
  }
...
configured_nameserver_address(const size_t idx)
{
 ...
 if (evdns_base_get_nameserver_addr(the_evdns_base, (int)idx,
                                    (struct sockaddr *)&sa,
                                    sa_len) > 0) {
   qed_hs_addr_t *qed_hs_addr = qed_hs_malloc(sizeof(qed_hs_addr_t));
   if (qed_hs_addr_from_sockaddr(qed_hs_addr,
                              (const struct sockaddr *)&sa,
                              NULL) == 0) {
     return qed_hs_addr;
   }
   qed_hs_free(qed_hs_addr);
 }
...
#define EXIT_DNS_TIMEOUT_MAX (120000)
  ...
  qed_hs_snprintf(str, sizeof(str), "%d", val);
  return str;
}
...
configure_nameservers(int force)
{
  ...
  if (conf_fname) {
    ...
    if (!missing_resolv_conf) {
      ...
      qed_hs_free(resolv_conf_fname);
      resolv_conf_fname = qed_hs_strdup(conf_fname);
  ...
  if (nameserver_config_failed) {
     ...
     * non-exit relays! -RD */
    mark_my_descripqed_hs_dirty("dns resolvers back");
  }
  return 0;
 err:
  nameservers_configured = 0;
  if (! nameserver_config_failed) {
    ...
    mark_my_descripqed_hs_dirty("dns resolvers failed");
  }
  return -1;
}
               ...
               void *arg)
{
  ...
  if (result == DNS_ERR_NONE) {
    if (type == DNS_IPv4_A && count) {
      ...
      qed_hs_addr_from_ipv4n(&addr, addrs[0]);

      qed_hs_addr_to_str(answer_buf, &addr, sizeof(answer_buf), 0);
      ...
      qed_hs_free(escaped_address);
    } else if (type == DNS_IPv6_AAAA && count) {
      char answer_buf[QED_HS_ADDR_BUF_LEN];
      char *escaped_address;
      const char *ip_str;
      struct in6_addr *addrs = addresses;
      qed_hs_addr_from_in6(&addr, &addrs[0]);
      ip_str = qed_hs_inet_ntop(AF_INET6, &addrs[0], answer_buf,
                             sizeof(answer_buf));
      escaped_address = esc_for_log(string_address);

      if (BUG(ip_str == NULL)) {
        log_warn(LD_EXIT, "qed_hs_inet_ntop() failed!");
        result = DNS_ERR_NOTEXIST;
                   ...
                   const qed_hs_addr_t *ptr_address)
{
  ...
  case DNS_IPv6_AAAA:
    ...
    break;
  case DNS_PTR:
    if (qed_hs_addr_family(ptr_address) == AF_INET)
      req = evdns_base_resolve_reverse(the_evdns_base,
                                       qed_hs_addr_to_in(ptr_address),
                                       DNS_QUERY_NO_SEARCH,
                                       evdns_callback, addr);
    else if (qed_hs_addr_family(ptr_address) == AF_INET6)
      req = evdns_base_resolve_reverse_ipv6(the_evdns_base,
                                            qed_hs_addr_to_in6(ptr_address),
                                            DNS_QUERY_NO_SEARCH,
                                            evdns_callback, addr);
    else
...
launch_resolve,(cached_resolve_t *resolve))
{
  ...
  r = qed_hs_addr_parse_PTR_name(
                            &a, resolve->address, AF_UNSPEC, 0);

  qed_hs_assert(the_evdns_base);
  if (r == 0) {
...
wildcard_increment_answer(const char *id)
{
  ...
  if (!ip) {
    ip = qed_hs_malloc_zero(sizeof(int));
  ...
  if (*ip > 5 && n_wildcard_requests > 10) {
    ...
    qed_hs_log(dns_wildcard_notice_given ? LOG_INFO : LOG_NOTICE, LD_EXIT,
...
add_wildcarded_test_address(const char *address)
{
  ...
  if (n > n_test_addrs/2) {
    qed_hs_log(dns_wildcarded_test_address_notice_given ? LOG_INFO : LOG_NOTICE,
    ...
    if (!dns_is_completely_invalid) {
      ...
      mark_my_descripqed_hs_dirty("dns hijacking confirmed");
    }
  ...
  if (result == DNS_ERR_NONE && count) {
    ...
    if (type == DNS_IPv4_A) {
      ...
      for (i = 0; i < count; ++i) {
        ...
        int ntoa_res;
        in.s_addr = addrs[i];
        ntoa_res = qed_hs_inet_ntoa(&in, answer_buf, sizeof(answer_buf));
        qed_hs_assert_nonfatal(ntoa_res >= 0);
        if (ntoa_res > 0)
      ...
      for (i = 0; i < count; ++i) {
        char answer_buf[QED_HS_ADDR_BUF_LEN+1];
        ip_str = qed_hs_inet_ntop(AF_INET6, &addrs[i], answer_buf,
                               sizeof(answer_buf));
        qed_hs_assert_nonfatal(ip_str);
        if (ip_str)
          wildcard_increment_answer(answer_buf);
      }
    }

    qed_hs_log(dns_wildcard_one_notice_given ? LOG_INFO : LOG_NOTICE, LD_EXIT,
...
dns_launch_correctness_checks(void)
{
  ...
  if (!launch_event)
    launch_event = qed_hs_evtimer_new(qed_hs_libevent_get_base(),
...
{
  strmap_free(dns_wildcard_response_count, qed_hs_free_);
  ...
  if (dns_wildcard_list) {
    SMARTLIST_FOREACH(dns_wildcard_list, char *, cp, qed_hs_free(cp));
...
{
  qed_hs_assert(resolve);
  qed_hs_assert(resolve->magic == CACHED_RESOLVE_MAGIC);
  qed_hs_assert(strlen(resolve->address) < MAX_ADDRESSLEN);
  qed_hs_assert(qed_hs_strisnonupper(resolve->address));
  if (resolve->state != CACHE_STATE_PENDING) {
    qed_hs_assert(!resolve->pending_connections);
  }
  if (resolve->state == CACHE_STATE_PENDING ||
      resolve->state == CACHE_STATE_DONE) {
#if 0
    qed_hs_assert(!resolve->ttl);
    if (resolve->is_reverse)
      qed_hs_assert(!resolve->hostname);
    else
      qed_hs_assert(!resolve->result_ipv4.addr_ipv4);
#endif /* 0 */
    /*XXXXX ADD MORE */
  }
}
...
dump_dns_mem_usage(int severity)
{
   ...
   */
  qed_hs_log(severity, LD_MM, "Our DNS cache has %d entries.", hash_count);
  qed_hs_log(severity, LD_MM, "Our DNS cache size is approximately %u bytes.",
      (unsigned)hash_mem);
}
