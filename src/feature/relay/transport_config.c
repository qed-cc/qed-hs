                                        ...
                                        const char *transport)
{
  ...
  addrport = qed_hs_strdup(smartlist_get(items, 1));
  ...
  /* Validate addrport */
  if (qed_hs_addr_port_parse(LOG_WARN, addrport, &addr, &port, -1)<0) {
                                        ...
                                        const char *transport)
{
 ...
 err:
  SMARTLIST_FOREACH(pt_options, char*, s, qed_hs_free(s));
                                  ...
                                  char **msg)
{
  ...
  for (cl = options->ServerTransportOptions; cl; cl = cl->next) {
    ...
    SMARTLIST_FOREACH(options_sl, char *, cp, qed_hs_free(cp));
