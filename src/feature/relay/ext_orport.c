...
ext_or_cmd_new(uint16_t len)
{
  ...
  ext_or_cmd_t *cmd = qed_hs_malloc(size);
  cmd->len = len;
  return cmd;
}
...
{
  qed_hs_assert(conn->base_.type == CONN_TYPE_EXT_OR);
...
get_ext_or_auth_cookie_file_name(void)
{
  ...
  if (options->ExtORPortCookieAuthFile &&
    ...
    return qed_hs_strdup(options->ExtORPortCookieAuthFile);
  } else {
                         ...
                         char **reply_out, size_t *reply_len_out)
{
  ...
  { /* set up macs */
    ...
    char *hmac_s_msg = qed_hs_malloc_zero(hmac_s_msg_len);
    char *hmac_c_msg = qed_hs_malloc_zero(hmac_c_msg_len);
    char *correct_client_hash = qed_hs_malloc_zero(EXT_OR_PORT_AUTH_HASH_LEN);
    ...
    *client_hash_out = correct_client_hash;
    ...
    qed_hs_free(hmac_s_msg);
    qed_hs_free(hmac_c_msg);
  }
  ...
  { /* write reply: (server_hash, server_nonce) */
    ...
    reply = qed_hs_malloc_zero(reply_len);
...
connection_ext_or_auth_handle_client_hash(connection_t *conn)
{
  ...
  if (qed_hs_memneq(TO_OR_CONN(conn)->ext_or_auth_correct_client_hash,
                                      ...
                                      const char *payload, uint16_t len)
{
  ...
  addr_str = qed_hs_memdup_nulterm(payload, len);

  res = qed_hs_addr_port_split(LOG_INFO, addr_str, &address_part, &port);
  qed_hs_free(addr_str);
  if (res<0)
    return -1;
  if (port == 0) {
  ...
  res = qed_hs_addr_parse(&addr, address_part);
  qed_hs_free(address_part);
  if (res<0)
    return -1;

  { /* do some logging */
    char *old_address = qed_hs_addr_to_str_dup(&conn->addr);
    char *new_address = qed_hs_addr_to_str_dup(&addr);
  ...
  conn->address = qed_hs_addr_to_str_dup(&addr);
                                       ...
                                       const char *payload, uint16_t len)
{
  ...
  if (conn->ext_or_transport)
    qed_hs_free(conn->ext_or_transport);
  ...
  while (1) {
    ...
    if (command->cmd == EXT_OR_CMD_TB_DONE) {
      ...
      if (!or_conn->ext_or_transport) {
         ...
         * trigraph. */
        or_conn->ext_or_transport = qed_hs_strdup("<?" "?>");
      }
...
ext_orport_free_all(void)
{
  if (ext_or_auth_cookie) /* Free the auth cookie */
    qed_hs_free(ext_or_auth_cookie);
}
