                    ...
                    guard_selection_type_t type)
{
  ...
  gs = qed_hs_malloc_zero(sizeof(*gs));
  gs->name = qed_hs_strdup(name);
  gs->type = type;
...
{
  qed_hs_assert(! curr_guard_context);
  if (!guard_contexts) {
    guard_contexts = smartlist_new();
  }
  ...
  const char *name = choose_guard_selection(
                             ...
                             NULL,
                             &type);
  qed_hs_assert(name); // "name" can only be NULL if we had an old name.
  qed_hs_assert(type != GS_TYPE_INFER);
...
update_guard_selection_choice(const or_options_t *options)
{
  ...
  qed_hs_assert(new_guard_context);
  qed_hs_assert(new_guard_context != curr_guard_context);
...
{
  qed_hs_assert(gs);
  qed_hs_assert(rsa_id);
  SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, guard) {
    if (qed_hs_memeq(guard->identity, rsa_id, DIGEST_LEN))
      return guard;
  } SMARTLIST_FOREACH_END(guard);
  return NULL;
}
                             ...
                             const bridge_info_t *bridge)
{
  ...
  const qed_hs_addr_port_t *addrport = bridge_get_addr_port(bridge);
  entry_guard_t *guard;
  if (BUG(!addrport))
    return NULL; // LCOV_EXCL_LINE
  guard = get_sampled_guard_by_bridge_addr(gs, addrport);
  if (! guard || (id && qed_hs_memneq(id, guard->identity, DIGEST_LEN)))
    return NULL;
  else
    return guard;
}
...
get_bridge_info_for_guard(const entry_guard_t *guard)
{
  ...
  if (! qed_hs_digest_is_zero(guard->identity)) {
                               ...
                               const qed_hs_addr_port_t *bridge_addrport)
{
  ...
  entry_guard_t *guard = qed_hs_malloc_zero(sizeof(entry_guard_t));

  /* persistent fields */
  guard->is_persistent = (rsa_id_digest != NULL);
  guard->selection_name = qed_hs_strdup(gs->name);
  if (rsa_id_digest)
  ...
  qed_hs_free(guard->sampled_by_version);
  guard->sampled_by_version = qed_hs_strdup(VERSION);
  ...
  guard->is_reachable = GUARD_REACHABLE_MAYBE;
  if (bridge_addrport)
    guard->bridge_addr = qed_hs_memdup(bridge_addrport, sizeof(*bridge_addrport));
                                 ...
                                 const qed_hs_addr_port_t *addrport)
{
  ...
  SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, g) {
    if (g->bridge_addr && qed_hs_addr_port_eq(addrport, g->bridge_addr))
      return g;
 ...
 */
void
entry_guard_learned_bridge_identity(const qed_hs_addr_port_t *addrport,
                                    const uint8_t *rsa_id_digest)
{
  ...
  if (qed_hs_digest_is_zero(g->identity)) {
    memcpy(g->identity, rsa_id_digest, DIGEST_LEN);
    make_persistent = 1;
  } else if (qed_hs_memeq(g->identity, rsa_id_digest, DIGEST_LEN)) {
...
sampled_guards_update_consensus_presence(guard_selection_t *gs)
{
  ...
  SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, guard) {
    ...
    if (is_listed && ! guard->currently_listed) {
      ...
      qed_hs_assert(! is_listed && ! guard->currently_listed);
...
guard_create_exit_restriction(const uint8_t *exit_id)
{
  ...
  rst = qed_hs_malloc_zero(sizeof(entry_guard_restriction_t));
...
guard_obeys_exit_restriction(const entry_guard_t *guard,
                             const entry_guard_restriction_t *rst)
{
  qed_hs_assert(rst->type == RST_EXIT_NODE ||
          rst->type == RST_EXCL_LIST);
  ...
  return qed_hs_memneq(guard->identity, rst->exclude_id, DIGEST_LEN);
}
                         ...
                         entry_guard_restriction_t *rst))
{
  ...
  result = qed_hs_malloc_zero(sizeof(circuit_guard_state_t));
...
{
  qed_hs_assert(gs);
  qed_hs_assert(chosen_node_out);
  qed_hs_assert(guard_state_out);
                               ...
                               origin_circuit_t *b)
{
  ...
  qed_hs_assert(state_a);
  qed_hs_assert(state_b);
...
{
  qed_hs_assert(gs);
  qed_hs_assert(all_circuits_in);
  qed_hs_assert(newly_complete_out);
  ...
  */
  ...
  qed_hs_assert_nonfatal(n_succeeded >= 1);
...
entry_guard_encode_for_state(entry_guard_t *guard, int dense_sampled_idx)
{
  ...
  /* Make a copy of the pathbias object, since we will want to update
     some of them */
  guard_pathbias_t *pb = qed_hs_memdup(&guard->pb, sizeof(*pb));
...
#undef PB_FIELD
  ...
  SMARTLIST_FOREACH(result, char *, cp, qed_hs_free(cp));
    ...
    *extra, strmap_t *vals)
{
    ...
    SMARTLIST_FOREACH_BEGIN(entries, char *, entry) {
      ...
      char *key = qed_hs_strndup(entry, eq-entry);
...
#undef FIELD
  ...
  if (bridge_addr) {
    qed_hs_addr_port_t res;
    memset(&res, 0, sizeof(res));
    int r = qed_hs_addr_port_parse(LOG_WARN, bridge_addr,
                                &res.addr, &res.port, -1);
    if (r == 0)
      guard->bridge_addr = qed_hs_memdup(&res, sizeof(res));
  ...
  if (confirmed_idx) {
    int ok=1;
    long idx = qed_hs_parse_long(confirmed_idx, 10, 0, INT_MAX, &ok, NULL);
    if (! ok) {
  ...
  if (sampled_idx) {
    int ok = 1;
    long idx = qed_hs_parse_long(sampled_idx, 10, 0, INT_MAX, &ok, NULL);
    if (!ok) {
...
#define PB_FIELD(field)                                                 \
  do {                                                                  \
    if (pb_ ## field) {                                                 \
      int ok = 1;                                                       \
      double r = qed_hs_parse_double(pb_ ## field, 0.0, 1e9, &ok, NULL);   \
      if (! ok) {                                                       \
...
#undef PB_FIELD
 ...
 err:
  ...
  qed_hs_free(sampled_on);
  qed_hs_free(sampled_by);
  qed_hs_free(unlisted_since);
  qed_hs_free(listed);
  qed_hs_free(confirmed_on);
  qed_hs_free(confirmed_idx);
  qed_hs_free(sampled_idx);
  qed_hs_free(bridge_addr);
  qed_hs_free(pb_use_attempts);
  qed_hs_free(pb_use_successes);
  qed_hs_free(pb_circ_attempts);
  qed_hs_free(pb_circ_successes);
  qed_hs_free(pb_successful_circuits_closed);
  qed_hs_free(pb_collapsed_circuits);
  qed_hs_free(pb_unusable_circuits);
  qed_hs_free(pb_timeouts);

  SMARTLIST_FOREACH(extra, char *, cp, qed_hs_free(cp));
  ...
  SMARTLIST_FOREACH_BEGIN(guard_contexts, guard_selection_t *, gs) {
    ...
    SMARTLIST_FOREACH_BEGIN(gs->sampled_entry_guards, entry_guard_t *, guard) {
      if (guard->is_persistent == 0)
        continue;
      *nextline = qed_hs_malloc_zero(sizeof(config_line_t));
      (*nextline)->key = qed_hs_strdup("Guard");
...
entry_guards_load_guards_from_state(or_state_t *state, int set)
{
  ...
  for ( ; line != NULL; line = line->next) {
    ...
    qed_hs_assert(guard->selection_name);
...
entry_guard_free_(entry_guard_t *e)
{
  ...
  qed_hs_free(e->sampled_by_version);
  qed_hs_free(e->extra_state_fields);
  qed_hs_free(e->selection_name);
  qed_hs_free(e->bridge_addr);
  qed_hs_free(e);
}
...
entry_guards_parse_state(or_state_t *state, int set, char **msg)
{
  ...
  if (r1 < 0) {
    if (msg && *msg == NULL) {
      *msg = qed_hs_strdup("parsing error");
    }
    return -1;
  }
  return 0;
                                  ...
                                  uint32_t guardfraction_percentage)
{
  ...
  qed_hs_assert(guardfraction_percentage <= 100);
  guardfraction_fraction = guardfraction_percentage / 100.0;

  long guard_bw = qed_hs_lround(guardfraction_fraction * orig_bandwidth);
  qed_hs_assert(guard_bw <= INT_MAX);
                    ...
                    circuit_guard_state_t **guard_state_out)
{
  ...
  if (CIRCUIT_IS_CONFLUX(TO_CIRCUIT(circ)) && state
    ...
    if (state && (exit_id = build_state_get_exit_rsa_id(state))) {
      ...
      smartlist_add(rst->excluded, qed_hs_memdup(exit_id, DIGEST_LEN));
    }
...
remove_all_entry_guards_for_guard_selection(guard_selection_t *gs)
{
  ...
  qed_hs_assert(gs != NULL);
  char *old_name = qed_hs_strdup(gs->name);
                                        ...
                                        int num_present, int num_usable)
{
  ...
  qed_hs_asprintf(&ret_str, "We're missing descriptors for %d/%d of our "
...
vanguards_lite_is_enabled(void)
{
  ...
  /* else it's enabled */
  qed_hs_assert_nonfatal(options->VanguardsLiteEnabled == 1);
...
maintain_layer2_guards(void)
{
  ...
  for (int i = 0; i < new_guards_needed_n; i++) {
    ...
    layer2_guard_t *layer2_guard = qed_hs_malloc_zero(sizeof(layer2_guard_t));
