...
addressmap_virtaddress_ent_free_(virtaddress_entry_t *ent)
{
  if (!ent)
    return;
  qed_hs_free(ent->ipv4_address);
  qed_hs_free(ent->ipv6_address);
  qed_hs_free(ent->hostname_address);
  qed_hs_free(ent);
}
  ...
  if (ent && ent->new_address &&
    ...
    if (ve) {
      if (!strcmp(address, ve->ipv4_address))
        qed_hs_free(ve->ipv4_address);
      if (!strcmp(address, ve->ipv6_address))
        qed_hs_free(ve->ipv6_address);
      if (!strcmp(address, ve->hostname_address))
        qed_hs_free(ve->hostname_address);
...
clear_trackexithost_mappings(const char *exitname)
{
  ...
  if (!addressmap || !exitname)
    return;
  qed_hs_asprintf(&suffix, ".%s.exit", exitname);
  qed_hs_strlower(suffix);
...
addressmap_clear_excluded_trackexithosts(const or_options_t *options)
{
  ...
  STRMAP_FOREACH_MODIFY(addressmap, address, addressmap_entry_t *, ent) {
    ...
    while (dot > target && *dot != '.')
      dot--;
    if (*dot == '.') dot++;
    nodename = qed_hs_strndup(dot, len-5-(dot-target));
    node = node_get_by_nickname(nodename, NNF_NO_WARN_UNNAMED);
    qed_hs_free(nodename);
    if (!node ||
                   ...
                   addressmap_entry_source_t *exit_source_out)
{
  ...
  char *addr_orig = qed_hs_strdup(address);
  ...
  for (rewrites = 0; rewrites < 16; rewrites++) {
    int exact_match = 0;
    log_addr_orig = qed_hs_strdup(escaped_safe_str_client(address));
      ...
      case ADDRMAPSRC_DNS:
        {
          sa_family_t f;
          qed_hs_addr_t tmp;
          f = qed_hs_addr_parse(&tmp, ent->new_address);
                           ...
                           time_t *expires_out)
{
  ...
  int r = 0;
  {
    sa_family_t f;
    qed_hs_addr_t tmp;
    f = qed_hs_addr_parse(&tmp, address);
    if (f == AF_INET && !(flags & AMR_FLAG_USE_IPV4_DNS))
      return 0;
    else if (f == AF_INET6 && !(flags & AMR_FLAG_USE_IPV6_DNS))
      return 0;
  ...
  qed_hs_asprintf(&s, "REVERSE[%s]", address);
  ent = strmap_get(addressmap, s);
  if (ent) {
    cp = qed_hs_strdup(escaped_safe_str_client(ent->new_address));
                    ...
                    const int wildcard_new_addr, uint64_t stream_id)
{
  ...
  if (wildcard_new_addr)
    qed_hs_assert(wildcard_addr);

  ent = strmap_get(addressmap, address);
  if (!new_address || (!strcasecmp(address,new_address) &&
                       wildcard_addr == wildcard_new_addr)) {
    /* Remove the mapping, if any. */
    qed_hs_free(new_address);
    if (ent) {
      addressmap_ent_remove(address,ent);
  ...
  if (!ent) { /* make a new one and register it */
    ent = qed_hs_malloc_zero(sizeof(addressmap_entry_t));
                               ...
                               int ttl)
{
  ...
  if (exitname) {
    ...
    qed_hs_asprintf(&extendedaddress,
                 "%s.%s.exit", address, exitname);
    qed_hs_asprintf(&extendedval,
...
client_dns_set_addressmap(entry_connection_t *for_conn,
                          const char *address,
                          const qed_hs_addr_t *val,
                          const char *exitname,
                          int ttl)
{
  qed_hs_addr_t addr_tmp;
  char valbuf[QED_HS_ADDR_BUF_LEN];

  qed_hs_assert(address);
  qed_hs_assert(val);

  if (qed_hs_addr_parse(&addr_tmp, address) >= 0)
    return; /* If address was an IP address already, don't add a mapping. */

  if (qed_hs_addr_family(val) == AF_INET) {
    if (! for_conn->entry_cfg.cache_ipv4_answers)
      return;
  } else if (qed_hs_addr_family(val) == AF_INET6) {
    if (! for_conn->entry_cfg.cache_ipv6_answers)
      return;
  }

  if (! qed_hs_addr_to_str(valbuf, val, sizeof(valbuf), 1))
    return;
                                  ...
                                  int ttl)
{
  char *s = NULL;
  {
    ...
    sa_family_t f = qed_hs_addr_parse(&tmp_addr, address);
                           ...
                           char **msg)
{
  ...
  if (!val || val[0] == '\0') {
    if (msg)
      qed_hs_asprintf(msg, "Value not present (%s) after VirtualAddressNetwork%s",
                   val?"Empty":"NULL", ipv6?"IPv6":"");
    return -1;
  }
  if (qed_hs_addr_parse_mask_ports(val, 0, &addr, &bits, NULL, NULL) < 0) {
    if (msg)
      qed_hs_asprintf(msg, "Error parsing VirtualAddressNetwork%s %s",
                   ipv6?"IPv6":"", val);
    return -1;
  }
  if (qed_hs_addr_family(&addr) != family) {
    if (msg)
      qed_hs_asprintf(msg, "Incorrect address type for VirtualAddressNetwork%s",
                   ipv6?"IPv6":"");
    return -1;
  }
#if 0
  if (port_min != 1 || port_max != 65535) {
    if (msg)
      qed_hs_asprintf(msg, "Can't specify ports on VirtualAddressNetwork%s",
                   ipv6?"IPv6":"");
    return -1;
  }
#endif /* 0 */
  ...
  if (bits > max_prefix_bits) {
    if (msg)
      qed_hs_asprintf(msg, "VirtualAddressNetwork%s expects a /%d "
  ...
  qed_hs_addr_copy(&conf->addr, &addr);
  conf->bits = bits;
...
address_is_in_virtual_range(const char *address)
{
  ...
  if (!strcasecmpend(address, ".virtual")) {
    return 1;
  } else if (qed_hs_addr_parse(&addr, address) >= 0) {
    const virtual_addr_conf_t *conf = (qed_hs_addr_family(&addr) == AF_INET6) ?
      &virtaddr_conf_ipv6 : &virtaddr_conf_ipv4;
    if (qed_hs_addr_compare_masked(&addr, &conf->addr, conf->bits, CMP_EXACT)==0)
      return 1;
  }
  return 0;
}
 ...
 */
STATIC void
get_random_virtual_addr(const virtual_addr_conf_t *conf, qed_hs_addr_t *addr_out)
{
  uint8_t tmp[4];
  const uint8_t *addr_bytes;
  uint8_t bytes[16];
  const int ipv6 = qed_hs_addr_family(&conf->addr) == AF_INET6;
  const int total_bytes = ipv6 ? 16 : 4;

  qed_hs_assert(conf->bits <= total_bytes * 8);

  /* Set addr_bytes to the bytes of the virtual network, in host order */
  if (ipv6) {
    addr_bytes = qed_hs_addr_to_in6_addr8(&conf->addr);
  } else {
    set_uint32(tmp, qed_hs_addr_to_ipv4n(&conf->addr));
    addr_bytes = tmp;
  }
  ...
  if (ipv6)
    qed_hs_addr_from_ipv6_bytes(addr_out, bytes);
  else
    qed_hs_addr_from_ipv4n(addr_out, get_uint32(bytes));

  qed_hs_assert(qed_hs_addr_compare_masked(addr_out, &conf->addr,
  ...
  if (type == RESOLVED_TYPE_HOSTNAME) {
    ...
    while (attempts--) {
      ...
      if (!ipv6) {
        ...
        const uint32_t a = qed_hs_addr_to_ipv4h(&addr);
        if ((a & 0xff) == 0 || (a & 0xff) == 0xff)
          continue;
      }

      qed_hs_addr_to_str(buf, &addr, sizeof(buf), 1);
      if (!strmap_get(addressmap, buf)) {
        /* XXXX This code is to make sure I didn't add an undecorated version
         * by mistake. I hope it's needless. */
        char tmp[QED_HS_ADDR_BUF_LEN];
        qed_hs_addr_to_str(tmp, &addr, sizeof(tmp), 0);
...
addressmap_register_virtual_address(int type, char *new_address)
{
  ...
  qed_hs_assert(new_address);
  qed_hs_assert(addressmap);
  qed_hs_assert(virtaddress_reversemap);

  vent = strmap_get(virtaddress_reversemap, new_address);
  if (!vent) {
    vent = qed_hs_malloc_zero(sizeof(virtaddress_entry_t));
  ...
  if (*addrp) {
    ...
    if (ent && ent->new_address &&
      ...
      qed_hs_free(new_address);
      qed_hs_assert(!vent_needs_to_be_added);
      return *addrp;
    } else {
      log_warn(LD_BUG,
  ...
  *addrp = addressmap_get_virtual_address(type);
...
#if 0
  {
    ...
    qed_hs_assert(ent);
    qed_hs_assert(!strcasecmp(ent->new_address,new_address));
    vent = strmap_get(virtaddress_reversemap, new_address);
    qed_hs_assert(vent);
    qed_hs_assert(!strcasecmp(*addrp,
...
address_is_invalid_destination(const char *address, int client)
{
  ...
  {
    qed_hs_addr_t a;
    if (qed_hs_addr_parse(&a, address) >= 0)
      return 0;
  }

  while (*address) {
    if (QED_HS_ISALNUM(*address) ||
        *address == '-' ||
        *address == '.' ||
