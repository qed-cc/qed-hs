...
#ifndef QED_HS_TRANSPORTS_H
#define QED_HS_TRANSPORTS_H
...
char *pt_get_extra_info_descripqed_hs_string(void);
...
char *pt_get_socks_args_for_proxy_addrport(const qed_hs_addr_t *addr,
                                            uint16_t port);

char *qed_hs_escape_str_for_pt_args(const char *string,
...
typedef struct {
  ...
  char *proxy_uri;  /* the outgoing proxy in QED_HS_PT_PROXY URI format */
  unsigned int proxy_supported : 1; /* the proxy honors QED_HS_PT_PROXY */
...
STATIC transport_t *transport_new(const qed_hs_addr_t *addr, uint16_t port,
                                  const char *name, int socks_ver,
...
STATIC const qed_hs_addr_t *managed_proxy_outbound_address(const or_options_t *,
                                                        sa_family_t);
...
#endif /* !defined(QED_HS_TRANSPORTS_H) */
