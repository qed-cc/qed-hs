...
pathbias_send_usable_probe(circuit_t *circ)
{
  ...
  probe_nonce = qed_hs_dup_ip(ocirc->pathbias_probe_nonce);
  ...
  qed_hs_snprintf(payload,RELAY_PAYLOAD_SIZE_MAX, "%s:25", probe_nonce);
...
pathbias_check_probe_response(circuit_t *circ, const relay_msg_t *msg)
{
  ...
  qed_hs_assert(msg);
  qed_hs_assert(ocirc);
  qed_hs_assert(circ->purpose == CIRCUIT_PURPOSE_PATH_BIAS_TESTING);
      ...
      if (pathbias_get_dropguards(options)) {
        if (!pb->path_bias_disabled) {
          log_warn(LD_CIRC,
                 ...
                 entry_guard_describe(guard),
                 qed_hs_lround(pathbias_get_use_success_count(guard)),
                 qed_hs_lround(pb->use_attempts),
                 qed_hs_lround(pathbias_get_close_success_count(guard)),
                 qed_hs_lround(pb->circ_attempts),
                 qed_hs_lround(pb->circ_successes),
                 qed_hs_lround(pb->unusable_circuits),
                 qed_hs_lround(pb->collapsed_circuits),
                 qed_hs_lround(pb->timeouts),
                 qed_hs_lround(get_circuit_build_close_time_ms()/1000));
