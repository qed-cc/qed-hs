...
struct bridge_info_t {
  ...
  qed_hs_addr_port_t addrport_configured;
...
bridge_free_(bridge_info_t *bridge)
{
  ...
  qed_hs_free(bridge->transport_name);
  if (bridge->socks_args) {
    SMARTLIST_FOREACH(bridge->socks_args, char*, s, qed_hs_free(s));
...
{
  qed_hs_assert(bridge);
  if (qed_hs_digest_is_zero(bridge->identity))
    return NULL;
  else
 ...
 * qed_hs_addr_port_t's in <b>orports</b>, return that bridge.  Else return
 * NULL. */
                                        ...
                                        const smartlist_t *orports)
{
  ...
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge)
    {
      if (qed_hs_digest_is_zero(bridge->identity)) {
        SMARTLIST_FOREACH_BEGIN(orports, qed_hs_addr_port_t *, ap)
          {
            if (qed_hs_addr_compare(&bridge->addr, &ap->addr, CMP_EXACT) == 0 &&
                bridge->port == ap->port)
              return bridge;
          }
        SMARTLIST_FOREACH_END(ap);
      }
      if (digest && qed_hs_memeq(bridge->identity, digest, DIGEST_LEN))
        return bridge;
    }
...
bridge_info_t *
get_configured_bridge_by_addr_port_digest(const qed_hs_addr_t *addr,
                                          uint16_t port,
                                          const char *digest)
{
  if (!bridge_list)
  ...
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge)
    {
      if ((qed_hs_digest_is_zero(bridge->identity) || digest == NULL) &&
          !qed_hs_addr_compare(&bridge->addr, addr, CMP_EXACT) &&
          bridge->port == port)
        return bridge;
      if (digest && qed_hs_memeq(bridge->identity, digest, DIGEST_LEN))
        return bridge;
 ...
 */
bridge_info_t *
get_configured_bridge_by_exact_addr_port_digest(const qed_hs_addr_t *addr,
                                                uint16_t port,
                                                const char *digest)
{
  if (!bridge_list)
    return NULL;
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge) {
    if (!qed_hs_addr_compare(&bridge->addr, addr, CMP_EXACT) &&
        bridge->port == port) {

      if (digest && qed_hs_memeq(bridge->identity, digest, DIGEST_LEN))
        return bridge;
      else if (!digest || qed_hs_digest_is_zero(bridge->identity))
        return bridge;
    }
 ...
 * address/port matches only. */
int
addr_is_a_configured_bridge(const qed_hs_addr_t *addr,
                            uint16_t port,
                            const char *digest)
{
  qed_hs_assert(addr);
...
extend_info_is_a_configured_bridge(const extend_info_t *ei)
{
  ...
  const qed_hs_addr_port_t *ap1 = NULL, *ap2 = NULL;
  if (! qed_hs_addr_is_null(&ei->orports[0].addr))
    ap1 = &ei->orports[0];
  if (! qed_hs_addr_is_null(&ei->orports[1].addr))
    ap2 = &ei->orports[1];
...
get_configured_bridge_by_routerinfo(const routerinfo_t *ri)
{
  ...
  bi = get_configured_bridge_by_orports_digest(ri->cache_info.identity_digest,
                                               orports);
  SMARTLIST_FOREACH(orports, qed_hs_addr_port_t *, p, qed_hs_free(p));
 ...
 */
static int
bridge_exists_with_addr_and_port(const qed_hs_addr_t *addr,
                                 const uint16_t port,
                                 const char *digest)
{
  if (!qed_hs_addr_port_is_valid(addr, port, 0))
    return 0;
 ...
 */
void
learned_router_identity(const qed_hs_addr_t *addr, uint16_t port,
                        const char *digest,
                        const ed25519_public_key_t *ed_id)
{
  ...
  if (bridge && qed_hs_digest_is_zero(bridge->identity)) {
  ...
  if (learned) {
    ...
    if (transport_name)
      qed_hs_asprintf(&transport_info, " (with transport '%s')", transport_name);
...
{
  if (digest)
    return qed_hs_memeq(digest, bridge->identity, DIGEST_LEN);
  else
    return qed_hs_digest_is_zero(bridge->identity);
}
 ...
 */
STATIC void
bridge_resolve_conflicts(const qed_hs_addr_t *addr, uint16_t port,
                         const char *digest, const char *transport_name)
{
  ...
  */
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge) {
    ...
    if (qed_hs_addr_eq(&bridge->addr, addr) && (bridge->port == port)) {
      ...
      if (!bridge_has_digest(bridge, digest) ||
        ...
        qed_hs_asprintf(&bridge_description_new, "%s:%s:%s",
                     fmt_addrport(addr, port),
                     digest ? hex_str(digest, DIGEST_LEN) : "",
                     transport_name ? transport_name : "");
        qed_hs_asprintf(&bridge_description_old, "%s:%s:%s",
                     fmt_addrport(&bridge->addr, bridge->port),
                     qed_hs_digest_is_zero(bridge->identity) ?
        ...
        qed_hs_free(bridge_description_new);
        qed_hs_free(bridge_description_old);
      }
    }
  } SMARTLIST_FOREACH_END(bridge);
}
...
bridge_add_from_config(bridge_line_t *bridge_line)
{
  ...
  { /* Log the bridge we are about to register: */
    log_debug(LD_GENERAL, "Registering bridge at %s (transport: %s) (%s)",
              ...
              qed_hs_digest_is_zero(bridge_line->digest) ?
  ...
  b = qed_hs_malloc_zero(sizeof(bridge_info_t));
  qed_hs_addr_copy(&b->addrport_configured.addr, &bridge_line->addr);
  b->addrport_configured.port = bridge_line->port;
  qed_hs_addr_copy(&b->addr, &bridge_line->addr);
  ...
  qed_hs_free(bridge_line); /* Deallocate bridge_line now. */
...
find_bridge_by_digest(const char *digest)
{
  ...
  SMARTLIST_FOREACH(bridge_list, bridge_info_t *, bridge,
    {
      if (qed_hs_memeq(bridge->identity, digest, DIGEST_LEN))
        return bridge;
    });
  return NULL;
}
 ...
 *  NULL. */
const char *
find_transport_name_by_bridge_addrport(const qed_hs_addr_t *addr, uint16_t port)
{
  if (!bridge_list)
    return NULL;

  SMARTLIST_FOREACH_BEGIN(bridge_list, const bridge_info_t *, bridge) {
    if (qed_hs_addr_eq(&bridge->addr, addr) &&
        (bridge->port == port))
      return bridge->transport_name;
  } SMARTLIST_FOREACH_END(bridge);
 ...
 */
int
get_transport_by_bridge_addrport(const qed_hs_addr_t *addr, uint16_t port,
...
const smartlist_t *
get_socks_args_by_bridge_addrport(const qed_hs_addr_t *addr, uint16_t port)
{
...
launch_direct_bridge_descripqed_hs_fetch(bridge_info_t *bridge)
{
  ...
  qed_hs_addr_port_t bridge_addrport;
  memcpy(&bridge_addrport.addr, &bridge->addr, sizeof(qed_hs_addr_t));
  ...
  SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, bridge)
    {
      ...
      can_use_bridge_authority = !qed_hs_digest_is_zero(bridge->identity) &&
                                 num_bridge_auths;
      ask_bridge_directly = !can_use_bridge_authority ||
                            !options->UpdateBridgesFromAuthority;
      log_debug(LD_DIR, "ask_bridge_directly=%d (%d, %d, %d)",
                ask_bridge_directly, qed_hs_digest_is_zero(bridge->identity),
...
rewrite_node_address_for_bridge(const bridge_info_t *bridge, node_t *node)
{
  ...
  if (node->ri) {
    ...
    if ((!qed_hs_addr_compare(&bridge->addr, &ri->ipv4_addr, CMP_EXACT) &&
         bridge->port == ri->ipv4_orport) ||
        (!qed_hs_addr_compare(&bridge->addr, &ri->ipv6_addr, CMP_EXACT) &&
         bridge->port == ri->ipv6_orport)) {
      /* they match, so no need to do anything */
    } else {
      if (qed_hs_addr_family(&bridge->addr) == AF_INET) {
        qed_hs_addr_copy(&ri->ipv4_addr, &bridge->addr);
        ri->ipv4_orport = bridge->port;
        log_info(LD_DIR,
                 "Adjusted bridge routerinfo for '%s' to match configured "
                 "address %s:%d.",
                 ri->nickname, fmt_addr(&ri->ipv4_addr), ri->ipv4_orport);
      } else if (qed_hs_addr_family(&bridge->addr) == AF_INET6) {
        qed_hs_addr_copy(&ri->ipv6_addr, &bridge->addr);
        ...
        log_err(LD_BUG, "Address family not supported: %d.",
                qed_hs_addr_family(&bridge->addr));
        return;
      }
    }
    ...
    if (options->ClientPreferIPv6ORPort == -1) {
      ...
      node->ipv6_preferred = (qed_hs_addr_family(&bridge->addr) == AF_INET6 &&
                              !qed_hs_addr_is_null(&node->ri->ipv6_addr));
    } else {
    ...
    if (!qed_hs_addr_is_null(&ri->ipv6_addr)) {
  ...
  if (node->rs) {
    ...
    if ((!qed_hs_addr_compare(&bridge->addr, &rs->ipv4_addr, CMP_EXACT) &&
        bridge->port == rs->ipv4_orport) ||
       (!qed_hs_addr_compare(&bridge->addr, &rs->ipv6_addr, CMP_EXACT) &&
        bridge->port == rs->ipv6_orport)) {
      /* they match, so no need to do anything */
    } else {
      if (qed_hs_addr_family(&bridge->addr) == AF_INET) {
        qed_hs_addr_copy(&rs->ipv4_addr, &bridge->addr);
      ...
      /* set IPv6 preferences even if there is no ri */
        ...
        qed_hs_addr_copy(&rs->ipv6_addr, &bridge->addr);
    ...
    if (options->ClientPreferIPv6ORPort == -1) {
      /* Mark which address to use based on which bridge_t we got. */
      node->ipv6_preferred = (qed_hs_addr_family(&bridge->addr) == AF_INET6 &&
                              !qed_hs_addr_is_null(&node->rs->ipv6_addr));
    } else {
      /* Mark which address to use based on user preference */
      node->ipv6_preferred = (reachable_addr_prefer_ipv6_orport(options) &&
                              !qed_hs_addr_is_null(&node->rs->ipv6_addr));
    ...
    /* XXXipv6 we lack support for falling back to another address for
    the same relay, warn the user */
    if (!qed_hs_addr_is_null(&rs->ipv6_addr)) {
      qed_hs_addr_port_t ap;
      node_get_pref_orport(node, &ap);
      log_notice(LD_CONFIG,
                 "Bridge '%s' has both an IPv4 and an IPv6 address.  "
...
{
  qed_hs_assert(ri);
  qed_hs_assert(ri->purpose == ROUTER_PURPOSE_BRIDGE);
...
get_bridge_dl_status_by_id, (const char *digest))
{
  ...
  if (digest && get_options()->UseBridges && bridge_list) {
    SMARTLIST_FOREACH_BEGIN(bridge_list, bridge_info_t *, b) {
      if (qed_hs_memeq(digest, b->identity, DIGEST_LEN)) {
        dl = &(b->fetch_status);
        break;
      }
    } SMARTLIST_FOREACH_END(b);
