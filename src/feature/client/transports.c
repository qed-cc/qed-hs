...
STATIC transport_t *
transport_new(const qed_hs_addr_t *addr, uint16_t port,
              const char *name, int socks_ver,
              const char *extra_info_args)
{
  transport_t *t = qed_hs_malloc_zero(sizeof(transport_t));

  qed_hs_addr_copy(&t->addr, addr);
  t->port = port;
  t->name = qed_hs_strdup(name);
  t->socks_version = socks_ver;
  if (extra_info_args)
    t->extra_info_args = qed_hs_strdup(extra_info_args);
...
transport_free_(transport_t *transport)
{
  ...
  qed_hs_free(transport->name);
  qed_hs_free(transport->extra_info_args);
  qed_hs_free(transport);
}
...
transport_copy(const transport_t *transport)
{
  ...
  qed_hs_assert(transport);

  new_transport = qed_hs_malloc_zero(sizeof(transport_t));

  new_transport->socks_version = transport->socks_version;
  new_transport->name = qed_hs_strdup(transport->name);
  qed_hs_addr_copy(&new_transport->addr, &transport->addr);
...
transport_resolve_conflicts(const transport_t *t)
{
  ...
  if (t_tmp) { /* same name */
    if (qed_hs_addr_eq(&t->addr, &t_tmp->addr) && (t->port == t_tmp->port)) {
      /* same name *and* addrport */
      t_tmp->marked_for_removal = 0;
      return 1;
    } else { /* same name but different addrport */
      char *new_transport_addrport =
        qed_hs_strdup(fmt_addrport(&t->addr, t->port));
      if (t_tmp->marked_for_removal) { /* marked for removal */
        ...
        qed_hs_free(new_transport_addrport);
...
MOCK_IMPL(int,
transport_add_from_config, (const qed_hs_addr_t *addr, uint16_t port,
                            const char *name, int socks_ver))
{
...
assert_unconfigured_count_ok(void)
{
  int n_completed = 0;
  if (!managed_proxy_list) {
    qed_hs_assert(unconfigured_proxies_n == 0);
    return;
  }
  ...
  qed_hs_assert(n_completed + unconfigured_proxies_n ==
...
{
  qed_hs_assert(mp->transports_to_launch);
...
proxy_needs_restart(const managed_proxy_t *mp)
{
  ...
  qed_hs_assert(smartlist_len(mp->transports_to_launch) > 0);
  qed_hs_assert(mp->conf_state == PT_PROTO_COMPLETED);
...
launch_managed_proxy(managed_proxy_t *mp)
{
  ...
  /* Cleanup our env. */
  SMARTLIST_FOREACH(env, char *, x, qed_hs_free(x));
  smartlist_free(env);
...
pt_configure_remaining_proxies(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(tmp,  managed_proxy_t *, mp) {
    qed_hs_assert(mp->conf_state != PT_PROTO_BROKEN &&
...
configure_proxy(managed_proxy_t *mp)
{
  ...
  qed_hs_assert(mp->conf_state != PT_PROTO_INFANT);
  qed_hs_assert(mp->process);
                      ...
                      int also_terminate_process)
{
  ...
  SMARTLIST_FOREACH(mp->transports_to_launch, char *, t, qed_hs_free(t));
  ...
  qed_hs_free(mp->version);
  qed_hs_free(mp->implementation);
...
/** Convert the tor proxy options to a URI suitable for QED_HS_PT_PROXY.
...
get_pt_proxy_uri(void)
{
  ...
  /* XXX: Currently TCPProxy is not supported in QED_HS_PT_PROXY because
   * there isn't a standard URI scheme for some proxy protocols, such as
   * haproxy. */
  if (options->Socks4Proxy || options->Socks5Proxy || options->HTTPSProxy) {
    char addr[QED_HS_ADDR_BUF_LEN+1];

    if (options->Socks4Proxy) {
      qed_hs_addr_to_str(addr, &options->Socks4ProxyAddr, sizeof(addr), 1);
      qed_hs_asprintf(&uri, "socks4a://%s:%d", addr, options->Socks4ProxyPort);
    } else if (options->Socks5Proxy) {
      qed_hs_addr_to_str(addr, &options->Socks5ProxyAddr, sizeof(addr), 1);
      if (!options->Socks5ProxyUsername && !options->Socks5ProxyPassword) {
        qed_hs_asprintf(&uri, "socks5://%s:%d", addr, options->Socks5ProxyPort);
      } else {
        qed_hs_asprintf(&uri, "socks5://%s:%s@%s:%d",
                     options->Socks5ProxyUsername,
                     options->Socks5ProxyPassword,
                     addr, options->Socks5ProxyPort);
      }
    } else if (options->HTTPSProxy) {
      qed_hs_addr_to_str(addr, &options->HTTPSProxyAddr, sizeof(addr), 1);
      if (!options->HTTPSProxyAuthenticator) {
        qed_hs_asprintf(&uri, "http://%s:%d", addr, options->HTTPSProxyPort);
      } else {
        qed_hs_asprintf(&uri, "http://%s@%s:%d", options->HTTPSProxyAuthenticator,
...
handle_proxy_line(const char *line, managed_proxy_t *mp)
{
  ...
  if (!strcmpstart(line, PROTO_ENV_ERROR)) {
    ...
    qed_hs_assert(mp->conf_protocol != 0);
     ...
     * See https://gitlab.qed-cc.org/tpo/core/qed-hs/-/issues/7362
     * */
                         ...
                         int is_smethod)
{
  ...
  qed_hs_assert(!strcmp(smartlist_get(items, item_index),method_str));
  ++item_index;
  ...
  ++item_index;
  if (qed_hs_addr_port_split(LOG_WARN, addrport, &address, &port)<0) {
  ...
  if (qed_hs_addr_parse(&qed_hs_addr, address) < 0) {
  ...
  transport = transport_new(&qed_hs_addr, port, transport_name,
...
parse_log_line(const char *line, managed_proxy_t *mp)
{
  ...
  qed_hs_log(log_severity, LD_PT, "Managed proxy \"%s\": %s",
                      ...
                      managed_proxy_t *mp)
{
  ...
  if (! strcasecmp(message_type->value, "version")) {
    ...
    qed_hs_free(mp->version);
    mp->version = qed_hs_strdup(version->value);

    qed_hs_free(mp->implementation);
    mp->implementation = qed_hs_strdup(implementation->value);
 ...
 * QED_HS_PT_SERVER_TRANSPORT_OPTIONS while configuring the server
...
get_transport_options_for_server_proxy(const managed_proxy_t *mp)
{
  ...
  SMARTLIST_FOREACH_BEGIN(mp->transports_to_launch, const char *, transport) {
    ...
    SMARTLIST_FOREACH_BEGIN(options_tmp_sl, const char *, options) {
      char *escaped_opts = qed_hs_escape_str_for_pt_args(options, ":;\\");
      smartlist_add_asprintf(string_sl, "%s:%s",
                             transport, escaped_opts);
      qed_hs_free(escaped_opts);
    } SMARTLIST_FOREACH_END(options);

    SMARTLIST_FOREACH(options_tmp_sl, char *, c, qed_hs_free(c));
  ...
  SMARTLIST_FOREACH(string_sl, char *, t, qed_hs_free(t));
  smartlist_free(string_sl);

  return options_string;
}

/** Return the string that tor should place in QED_HS_PT_SERVER_BINDADDR
...
get_bindaddr_for_server_proxy(const managed_proxy_t *mp)
{
  ...
  SMARTLIST_FOREACH_BEGIN(mp->transports_to_launch, char *, t) {
    ...
    qed_hs_free(bindaddr_tmp);
  } SMARTLIST_FOREACH_END(t);

  bindaddr_result = smartlist_join_strings(string_tmp, ",", 0, NULL);

  SMARTLIST_FOREACH(string_tmp, char *, t, qed_hs_free(t));
...
create_managed_proxy_environment(const managed_proxy_t *mp)
{
...

  {
    ...
    smartlist_add_asprintf(envs, "QED_HS_PT_STATE_LOCATION=%s", state_tmp);
    qed_hs_free(state_tmp);
  }

  smartlist_add_strdup(envs, "QED_HS_PT_MANAGED_TRANSPORT_VER=1");

  {
    ...
    smartlist_add_asprintf(envs,
                           mp->is_server ?
                           "QED_HS_PT_SERVER_TRANSPORTS=%s" :
                           "QED_HS_PT_CLIENT_TRANSPORTS=%s",
                           transports_to_launch);

    qed_hs_free(transports_to_launch);
  }
  ...
  if (mp->is_server) {
    {
      ...
      if (orport_tmp) {
        smartlist_add_asprintf(envs, "QED_HS_PT_ORPORT=%s", orport_tmp);
        qed_hs_free(orport_tmp);
      }
    }

    {
      char *bindaddr_tmp = get_bindaddr_for_server_proxy(mp);
      smartlist_add_asprintf(envs, "QED_HS_PT_SERVER_BINDADDR=%s", bindaddr_tmp);
      qed_hs_free(bindaddr_tmp);
    }

    {
      ...
      if (server_transport_options) {
        smartlist_add_asprintf(envs, "QED_HS_PT_SERVER_TRANSPORT_OPTIONS=%s",
                               server_transport_options);
        qed_hs_free(server_transport_options);
      }
    }
    ...
    if (options->ExtORPort_lines) {
      ...
      if (ext_or_addrport_tmp) {
        smartlist_add_asprintf(envs, "QED_HS_PT_EXTENDED_SERVER_PORT=%s",
                               ext_or_addrport_tmp);
      }
      if (cookie_file_loc) {
        smartlist_add_asprintf(envs, "QED_HS_PT_AUTH_COOKIE_FILE=%s",
                               cookie_file_loc);
      }

      qed_hs_free(ext_or_addrport_tmp);
      qed_hs_free(cookie_file_loc);

    } else {
      smartlist_add_asprintf(envs, "QED_HS_PT_EXTENDED_SERVER_PORT=");
    }
  } else {
    /* If ClientTransportPlugin has a HTTPS/SOCKS proxy configured, set the
     * QED_HS_PT_PROXY line.
     */

    if (mp->proxy_uri) {
      smartlist_add_asprintf(envs, "QED_HS_PT_PROXY=%s", mp->proxy_uri);
    }
  }
   ...
   */
  smartlist_add_asprintf(envs, "QED_HS_PT_EXIT_ON_STDIN_CLOSE=1");

  /* Specify which IPv4 and IPv6 addresses the PT should make its outgoing
   * connections from. See: https://bugs.qed-cc.org/5304 for more
   * information about this. */
  {
    /* Set QED_HS_PT_OUTBOUND_BIND_ADDRESS_V4. */
    const qed_hs_addr_t *ipv4_addr = managed_proxy_outbound_address(options,
                                                                 AF_INET);

    /* managed_proxy_outbound_address() only returns a non-NULL value if
     * qed_hs_addr_is_null() was false, which means we don't have to check that
     * here. */
    if (ipv4_addr) {
      char *ipv4_addr_str = qed_hs_addr_to_str_dup(ipv4_addr);
      smartlist_add_asprintf(envs,
                             "QED_HS_PT_OUTBOUND_BIND_ADDRESS_V4=%s",
                             ipv4_addr_str);
      qed_hs_free(ipv4_addr_str);
    }

    /* Set QED_HS_PT_OUTBOUND_BIND_ADDRESS_V6. */
    const qed_hs_addr_t *ipv6_addr = managed_proxy_outbound_address(options,
                                                                 AF_INET6);
    if (ipv6_addr) {
      char *ipv6_addr_str = qed_hs_addr_to_str_dup(ipv6_addr);
      smartlist_add_asprintf(envs,
                             "QED_HS_PT_OUTBOUND_BIND_ADDRESS_V6=[%s]",
                             ipv6_addr_str);
      qed_hs_free(ipv6_addr_str);
    }
  }
...
{
  managed_proxy_t *mp = qed_hs_malloc_zero(sizeof(managed_proxy_t));
  managed_proxy_set_state(mp, PT_PROTO_INFANT);
  mp->is_server = is_server;
  mp->argv = proxy_argv;
...
pt_get_extra_info_descripqed_hs_string(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(managed_proxy_list,  const managed_proxy_t *, mp) {
    ...
    SMARTLIST_FOREACH_BEGIN(mp->transports, const transport_t *, t) {
      ...
      if (t->extra_info_args)
        qed_hs_asprintf(&transport_args, " %s", t->extra_info_args);
    ...
    /* Set transport-info line. */
    {
      ...
      if (mp->version) {
        qed_hs_asprintf(&version, " version=%s", mp->version);
      }
      if (mp->implementation) {
        qed_hs_asprintf(&impl, " implementation=%s", mp->implementation);
      }
  ...
  SMARTLIST_FOREACH(string_chunks, char *, s, qed_hs_free(s));
  smartlist_free(string_chunks);
...
pt_stringify_socks_args(const smartlist_t *socks_args)
{
  ...
  qed_hs_assert(socks_args);
  qed_hs_assert(smartlist_len(socks_args) > 0);

  sl_tmp = smartlist_new();

  SMARTLIST_FOREACH_BEGIN(socks_args, const char *, s) {
    /* Escape ';' and '\'. */
    escaped_string = qed_hs_escape_str_for_pt_args(s, ";\\");
    if (!escaped_string)
      goto done;
 ...
 *  and it's the responsibility of the caller to free it after use. */
char *
pt_get_socks_args_for_proxy_addrport(const qed_hs_addr_t *addr, uint16_t port)
{
...
char *
qed_hs_escape_str_for_pt_args(const char *string, const char *chars_to_escape)
{
  char *new_string = NULL;
  char *new_cp = NULL;
  size_t length, new_length;

  qed_hs_assert(string);
  ...
  new_string = new_cp = qed_hs_malloc(new_length);
...
managed_proxy_exit_callback(process_t *process, process_exit_code_t exit_code)
{
  ...
  if (mp) {
    ...
    qed_hs_assert(mp->process == process);
    mp->process = NULL;
...
managed_proxy_outbound_address(const or_options_t *options, sa_family_t family)
{
  ...
  const qed_hs_addr_t *address = NULL;
  int family_index;
  ...
  if (! qed_hs_addr_is_null(address))
    return address;
...
managed_proxy_set_state(managed_proxy_t *mp, enum pt_proto_state new_state)
{
  ...
  qed_hs_log(LOG_INFO, LD_PT, "Managed proxy \"%s\" changed state: %s -> %s",
          mp->argv[0],
