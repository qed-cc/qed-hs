...
evdns_server_callback(struct evdns_server_request *req, void *data_)
{
  ...
  if (qed_hs_addr_from_sockaddr(&qed_hs_addr, sa, &port)<0) {
    log_warn(LD_APP, "Requesting address wasn't recognized.");
    evdns_server_request_respond(req, DNS_ERR_SERVERFAILED);
    return;
  }

  if (!socks_policy_permits_address(&qed_hs_addr)) {
  ...
  qed_hs_addr_copy(&TO_CONN(conn)->addr, &qed_hs_addr);
  TO_CONN(conn)->port = port;
  TO_CONN(conn)->address = qed_hs_addr_to_str_dup(&qed_hs_addr);
  ...
  if (q->type == EVDNS_TYPE_A || q->type == EVDNS_TYPE_AAAA ||
    ...
    qed_hs_assert(q->type == EVDNS_TYPE_PTR);
  ...
  * automapped), and then attached to a circuit. */
  ...
  q_name = qed_hs_strdup(q->name); /* q could be freed in rewrite_and_attach */
                       ...
                       control_connection_t *control_conn)
{
  ...
  qed_hs_addr_copy(&TO_CONN(conn)->addr, &control_conn->base_.addr);
#ifdef AF_UNIX
  /*
   * The control connection can be AF_UNIX and if so qed_hs_addr_to_str_dup will
   * unhelpfully say "<unknown address type>"; say "(Tor_internal)"
   * instead.
   */
  if (control_conn->base_.socket_family == AF_UNIX) {
    TO_CONN(conn)->port = 0;
    TO_CONN(conn)->address = qed_hs_strdup("(Tor_internal)");
  } else {
    TO_CONN(conn)->port = control_conn->base_.port;
    TO_CONN(conn)->address = qed_hs_addr_to_str_dup(&control_conn->base_.addr);
  }
  ...
  entry_conn->original_dest_address = qed_hs_strdup(name);
  ...
  * automapped), and then attached to a circuit. */
  ...
  q_name = qed_hs_strdup(name); /* q could be freed in rewrite_and_attach */
  connection_ap_rewrite_and_attach_if_allowed(entry_conn, NULL, NULL);
  /* Now, the connection is marked if it was bad. */
                 ...
                 int ttl)
{
  ...
  if (answer_type == RESOLVED_TYPE_IPV6) {
    ...
    char *ans = qed_hs_strndup(answer, answer_len);
...
dnsserv_configure_listener(connection_t *conn)
{
  ...
  qed_hs_assert(conn->type == CONN_TYPE_AP_DNS_LISTENER);

  listener_conn = TO_LISTENER_CONN(conn);
  listener_conn->dns_server_port =
    qed_hs_evdns_add_server_port(conn->s, 0, evdns_server_callback,
                              listener_conn);
}
