...
#ifndef QED_HS_BRIDGES_H
#define QED_HS_BRIDGES_H
...
const qed_hs_addr_port_t * bridge_get_addr_port(const bridge_info_t *bridge);
...
int addr_is_a_configured_bridge(const qed_hs_addr_t *addr, uint16_t port,
                                const char *digest);
int extend_info_is_a_configured_bridge(const extend_info_t *ei);
int routerinfo_is_a_configured_bridge(const routerinfo_t *ri);
int node_is_a_configured_bridge(const node_t *node);
void learned_router_identity(const qed_hs_addr_t *addr, uint16_t port,
                             const char *digest,
...
const smartlist_t *get_socks_args_by_bridge_addrport(const qed_hs_addr_t *addr,
                                                     uint16_t port);

int any_bridges_dont_support_microdescriptors(void);

const char *find_transport_name_by_bridge_addrport(const qed_hs_addr_t *addr,
                                                   uint16_t port);
struct transport_t;
int get_transport_by_bridge_addrport(const qed_hs_addr_t *addr, uint16_t port,
...
STATIC void bridge_resolve_conflicts(const qed_hs_addr_t *addr,
                                     uint16_t port,
                                     const char *digest,
                                     const char *transport_name);
#endif /* defined(QED_HS_BRIDGES_PRIVATE) */

#endif /* !defined(QED_HS_BRIDGES_H) */
