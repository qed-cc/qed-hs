...
router_get_orport(const routerinfo_t *router,
                  qed_hs_addr_port_t *ap_out,
                  int family)
{
  qed_hs_assert(ap_out != NULL);
  if (family == AF_INET) {
    qed_hs_addr_copy(&ap_out->addr, &router->ipv4_addr);
    ap_out->port = router->ipv4_orport;
    return 0;
  } else if (family == AF_INET6) {
    /* IPv6 addresses are optional, so check if it is valid. */
    if (qed_hs_addr_port_is_valid(&router->ipv6_addr, router->ipv6_orport, 0)) {
      qed_hs_addr_copy(&ap_out->addr, &router->ipv6_addr);
      ap_out->port = router->ipv6_orport;
      return 0;
    } else {
      qed_hs_addr_port_make_null_ap(ap_out, AF_INET6);
      return -1;
    }
  } else {
    /* Unsupported address family */
    qed_hs_assert_nonfatal_unreached();
    qed_hs_addr_port_make_null_ap(ap_out, AF_UNSPEC);
    return -1;
  }
}

int
router_has_orport(const routerinfo_t *router, const qed_hs_addr_port_t *orport)
{
  return
    (qed_hs_addr_eq(&orport->addr, &router->ipv4_addr) &&
     orport->port == router->ipv4_orport) ||
    (qed_hs_addr_eq(&orport->addr, &router->ipv6_addr) &&
     orport->port == router->ipv6_orport);
}

/** Return a smartlist of qed_hs_addr_port_t's with all the OR ports of
...
routerinfo_get_ed25519_id(const routerinfo_t *ri)
{
  ...
  const qed_hs_cert_t *cert = ri->cache_info.signing_key_cert;
