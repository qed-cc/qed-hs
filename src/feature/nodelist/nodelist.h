...
#ifndef QED_HS_NODELIST_H
#define QED_HS_NODELIST_H

struct ed25519_public_key_t;
struct curve25519_public_key_t;

#define node_assert_ok(n) STMT_BEGIN {                          \
    qed_hs_assert((n)->ri || (n)->rs);                             \
  } STMT_END
...
int nodelist_probably_contains_address(const qed_hs_addr_t *addr);
bool nodelist_reentry_contains(const qed_hs_addr_t *addr, uint16_t port);
void nodelist_add_addr_to_address_set(const qed_hs_addr_t *addr,
...
void node_get_prim_orport(const node_t *node, qed_hs_addr_port_t *ap_out);
void node_get_pref_orport(const node_t *node, qed_hs_addr_port_t *ap_out);
void node_get_pref_ipv6_orport(const node_t *node, qed_hs_addr_port_t *ap_out);
int node_ipv6_dir_preferred(const node_t *node);
void node_get_prim_dirport(const node_t *node, qed_hs_addr_port_t *ap_out);
void node_get_pref_dirport(const node_t *node, qed_hs_addr_port_t *ap_out);
void node_get_pref_ipv6_dirport(const node_t *node, qed_hs_addr_port_t *ap_out);
...
void node_get_addr(const node_t *node, qed_hs_addr_t *addr_out);
...
int router_exit_policy_all_nodes_reject(const qed_hs_addr_t *addr, uint16_t port,
                                        int need_uptime);
void router_set_status(const char *digest, int up);
int router_addrs_in_same_network(const qed_hs_addr_t *a1,
...
#endif /* !defined(QED_HS_NODELIST_H) */
