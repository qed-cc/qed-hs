...
{
  size_t rv = 0;
  bool has_ipv6 = ipv6_addr && !qed_hs_addr_is_null(ipv6_addr);
  ...
  if (!rsa_id_digest) {
    ...
    qed_hs_assert_nonfatal(rv < NODE_DESC_BUF_LEN);
    return buf;
  }
  ...
  if (ipv4_addr) {
    ...
    char addr_str[QED_HS_ADDR_BUF_LEN];
    memset(addr_str, 0, sizeof(addr_str));

    str_rv = qed_hs_addr_to_str(addr_str, ipv4_addr, sizeof(addr_str), 0);
    if (str_rv) {
  ...
  if (has_ipv6) {
    ...
    str_rv = qed_hs_addr_to_str(addr_str, ipv6_addr, sizeof(addr_str), 1);
    if (str_rv) {
      rv = strlcat(buf, addr_str, NODE_DESC_BUF_LEN);
      qed_hs_assert_nonfatal(rv < NODE_DESC_BUF_LEN);
    }
...
node_describe(const node_t *node)
{
  ...
  const qed_hs_addr_t *ipv6_addr = NULL, *ipv4_addr = NULL;
  ...
  if (node->rs) {
    ...
    if (node->md && qed_hs_addr_is_null(ipv6_addr)) {
...
extend_info_describe(const extend_info_t *ei)
{
  ...
  const qed_hs_addr_port_t *ap4 = extend_info_get_orport(ei, AF_INET);
  const qed_hs_addr_port_t *ap6 = extend_info_get_orport(ei, AF_INET6);
  const qed_hs_addr_t *addr4 = ap4 ? &ap4->addr : NULL;
  const qed_hs_addr_t *addr6 = ap6 ? &ap6->addr : NULL;
...
router_get_verbose_nickname(char *buf, const routerinfo_t *router)
{
  ...
  if (!router) {
    ...
    qed_hs_assert_nonfatal(rv < MAX_VERBOSE_NICKNAME_LEN+1);
    return;
  }
