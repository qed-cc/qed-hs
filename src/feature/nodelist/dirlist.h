...
#ifndef QED_HS_DIRLIST_H
#define QED_HS_DIRLIST_H
...
const qed_hs_addr_port_t *trusted_dir_server_get_dirport(const dir_server_t *ds,
                                                  auth_dirport_usage_t usage,
                                                  int addr_family);
const qed_hs_addr_port_t *trusted_dir_server_get_dirport_exact(
                                                  const dir_server_t *ds,
                                                  auth_dirport_usage_t usage,
                                                  int addr_family);

bool router_addr_is_trusted_dir_type(const qed_hs_addr_t *addr,
                                     dirinfo_type_t type);
...
dir_server_t *fallback_dir_server_new(const qed_hs_addr_t *addr,
                                      uint16_t dir_port, uint16_t or_port,
                                      const qed_hs_addr_port_t *addrport_ipv6,
...
#endif /* !defined(QED_HS_DIRLIST_H) */
