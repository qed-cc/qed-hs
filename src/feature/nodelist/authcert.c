...
get_cert_list(const char *id_digest)
{
  ...
  if (!cl) {
    cl = qed_hs_malloc_zero(sizeof(cert_list_t));
...
cert_list_free_(cert_list_t *cl)
{
  ...
  dsmap_free(cl->dl_status_map, qed_hs_free_);
  qed_hs_free(cl);
}
                                    ...
                                    int flush, const char *source_dir)
{
  ...
  for (s = contents; *s; s = eos) {
    ...
    if (ds && cert->cache_info.published_on > ds->addr_current_at) {
       ...
       * address. */
      if (!qed_hs_addr_is_null(&cert->ipv4_addr) && cert->ipv4_dirport &&
          (!qed_hs_addr_eq(&ds->ipv4_addr, &cert->ipv4_addr) ||
        ...
        qed_hs_addr_copy(&ds->ipv4_addr, &cert->ipv4_addr);
        ds->ipv4_dirport = cert->ipv4_dirport;
      }
...
trusted_dirs_flush_certs_to_disk(void)
{
  ...
  qed_hs_free(filename);
  SMARTLIST_FOREACH(chunks, sized_chunk_t *, c, qed_hs_free(c));
...
authority_cert_get_by_sk_digest(const char *sk_digest)
{
  ...
  if ((c = get_my_v3_authority_cert()) &&
      qed_hs_memeq(c->signing_key_digest, sk_digest, DIGEST_LEN))
    return c;
  ...
  DIGESTMAP_FOREACH(trusted_dir_certs, key, cert_list_t *, cl) {
    SMARTLIST_FOREACH(cl->certs, authority_cert_t *, cert,
    {
      if (qed_hs_memeq(cert->signing_key_digest, sk_digest, DIGEST_LEN))
        return cert;
    });
  } DIGESTMAP_FOREACH_END;
  return NULL;
                              ...
                              const char *dir_hint)
{
    ...
    SMARTLIST_FOREACH_BEGIN(status->voters, networkstatus_voter_info_t *,
      ...
      SMARTLIST_FOREACH_BEGIN(voter->sigs, document_signature_t *, sig) {
        ...
        if (download_status_is_ready_by_sk_in_cl(
          ...
          fp_tmp = qed_hs_malloc(sizeof(*fp_tmp));
  ...
  if (smartlist_len(missing_id_digests) > 0) {
    ...
    SMARTLIST_FOREACH_BEGIN(missing_id_digests, const char *, d) {
      ...
      if (need_plus) {
        qed_hs_asprintf(&fp, "+%s", id_digest_str);
      } else {
        /* No need for qed_hs_asprintf() in this case; first one gets no '+' */
        fp = qed_hs_strdup(id_digest_str);
        need_plus = 1;
      }
    ...
    SMARTLIST_FOREACH(fps, char *, cp, qed_hs_free(cp));
    smartlist_free(fps);
  }
  ...
  if (smartlist_len(missing_cert_digests) > 0) {
    ...
    SMARTLIST_FOREACH_BEGIN(missing_cert_digests, const fp_pair_t *, d) {
      ...
      /* Now qed_hs_asprintf() */
      if (need_plus) {
        qed_hs_asprintf(&fp_pair, "+%s-%s", id_digest_str, sk_digest_str);
      } else {
    ...
    SMARTLIST_FOREACH(fp_pairs, char *, p, qed_hs_free(p));
    smartlist_free(fp_pairs);
  }

  smartlist_free(missing_id_digests);
  SMARTLIST_FOREACH(missing_cert_digests, fp_pair_t *, p, qed_hs_free(p));
