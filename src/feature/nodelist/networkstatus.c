                               ...
                               int unverified_consensus))
{
  ...
  if (flav == FLAV_NS) {
    qed_hs_snprintf(buf, sizeof(buf), "%s-consensus", prefix);
  } else {
    qed_hs_snprintf(buf, sizeof(buf), "%s-%s-consensus", prefix, flavorname);
  }
                                        ...
                                        int unverified_consensus)
{
  ...
  qed_hs_mmap_t *result = qed_hs_mmap_file(filename);
...
vote_routerstatus_free_(vote_routerstatus_t *rs)
{
  ...
  if (!rs)
    return;
  qed_hs_free(rs->version);
  qed_hs_free(rs->protocols);
  qed_hs_free(rs->status.exitsummary);
  for (h = rs->microdesc; h; h = next) {
    qed_hs_free(h->microdesc_hash_line);
    next = h->next;
    qed_hs_free(h);
  }
  qed_hs_free(rs);
...
{
  document_signature_t *r = qed_hs_memdup(sig, sizeof(document_signature_t));
  if (r->signature)
    r->signature = qed_hs_memdup(sig->signature, sig->signature_len);
  return r;
}
...
networkstatus_vote_free_(networkstatus_t *ns)
{
  ...
  qed_hs_free(ns->client_versions);
  qed_hs_free(ns->server_versions);
  qed_hs_free(ns->recommended_client_protocols);
  qed_hs_free(ns->recommended_relay_protocols);
  qed_hs_free(ns->required_client_protocols);
  qed_hs_free(ns->required_relay_protocols);

  if (ns->known_flags) {
    SMARTLIST_FOREACH(ns->known_flags, char *, c, qed_hs_free(c));
    smartlist_free(ns->known_flags);
  }
  if (ns->weight_params) {
    SMARTLIST_FOREACH(ns->weight_params, char *, c, qed_hs_free(c));
    smartlist_free(ns->weight_params);
  }
  if (ns->net_params) {
    SMARTLIST_FOREACH(ns->net_params, char *, c, qed_hs_free(c));
    smartlist_free(ns->net_params);
  }
  if (ns->supported_methods) {
    SMARTLIST_FOREACH(ns->supported_methods, char *, c, qed_hs_free(c));
    smartlist_free(ns->supported_methods);
  }
  if (ns->package_lines) {
    SMARTLIST_FOREACH(ns->package_lines, char *, c, qed_hs_free(c));
    smartlist_free(ns->package_lines);
  }
  if (ns->voters) {
    SMARTLIST_FOREACH_BEGIN(ns->voters, networkstatus_voter_info_t *, voter) {
      qed_hs_free(voter->nickname);
      qed_hs_free(voter->address);
      qed_hs_free(voter->contact);
      if (voter->sigs) {
  ...
  if (ns->bw_file_headers) {
    SMARTLIST_FOREACH(ns->bw_file_headers, char *, c, qed_hs_free(c));
  ...
  qed_hs_free(ns->sr_info.previous_srv);
  qed_hs_free(ns->sr_info.current_srv);
                                       ...
                                       const authority_cert_t *cert)
{
  ...
  if (crypto_pk_get_digest(cert->signing_key, key_digest)<0)
    return -1;
  if (qed_hs_memneq(sig->signing_key_digest, key_digest, DIGEST_LEN) ||
      qed_hs_memneq(sig->identity_digest, cert->cache_info.identity_digest,
                 DIGEST_LEN))
    return -1;
  ...
  signed_digest = qed_hs_malloc(signed_digest_len);
  if (crypto_pk_public_checksig(cert->signing_key,
                                signed_digest,
                                signed_digest_len,
                                sig->signature,
                                sig->signature_len) < dlen ||
      qed_hs_memneq(signed_digest, consensus->digests.d[sig->alg], dlen)) {
    log_warn(LD_DIR, "Got a bad signature on a networkstatus vote");
    sig->bad_signature = 1;
  } else {
    sig->good_signature = 1;
  }
  qed_hs_free(signed_digest);
  return 0;
}
                                        ...
                                        int warn)
{
  ...
  qed_hs_assert(consensus->type == NS_TYPE_CONSENSUS);
  ...
  SMARTLIST_FOREACH_BEGIN(consensus->voters, networkstatus_voter_info_t *,
    ...
    SMARTLIST_FOREACH_BEGIN(voter->sigs, document_signature_t *, sig) {
      if (!sig->good_signature && !sig->bad_signature &&
        ...
        qed_hs_assert(qed_hs_memeq(sig->identity_digest, voter->identity_digest,
                           DIGEST_LEN));
  ...
  if (warn >= 0) {
    SMARTLIST_FOREACH(unrecognized, networkstatus_voter_info_t *, voter,
      {
        qed_hs_log(severity, LD_DIR, "Consensus includes unrecognized authority "
    ...
    SMARTLIST_FOREACH(need_certs_from, networkstatus_voter_info_t *, voter,
      {
        qed_hs_log(severity, LD_DIR, "Looks like we need to download a new "
    ...
    SMARTLIST_FOREACH(missing_authorities, dir_server_t *, ds,
      {
        qed_hs_log(severity, LD_DIR, "Consensus does not include configured "
      ...
      });
    {
      ...
      qed_hs_log(severity, LD_DIR, "%s", joined);
      qed_hs_free(joined);
...
compare_digest_to_routerstatus_entry(const void *_key, const void **_member)
{
  ...
  return qed_hs_memcmp(key, rs->identity_digest, DIGEST_LEN);
}
                                          ...
                                          const void **_member)
{
  ...
  return qed_hs_memcmp(key, vrs->status.identity_digest, DIGEST_LEN);
}
...
router_get_descripqed_hs_digests_in_consensus(networkstatus_t *consensus)
{
  ...
  for (i = digestmap_iter_init(consensus->desc_digest_map);
    ...
    digest_tmp = qed_hs_malloc(DIGEST_LEN);
                                  ...
                                  download_status_t *dls)
{
  ...
  qed_hs_assert(flavor >= 0 && flavor < N_CONSENSUS_FLAVORS);
...
update_consensus_networkstatus_downloads(time_t now)
{
  ...
  for (i=0; i < N_CONSENSUS_FLAVORS; ++i) {
    ...
    if (we_are_bootstrapping && use_multi_conn
      ...
      qed_hs_assert(consensus_dl_status[i].schedule == DL_SCHED_CONSENSUS);
                                      ...
                                      download_want_authority_t want_authority)
{
  ...
  qed_hs_assert(dls->schedule == DL_SCHED_CONSENSUS);
...
networkstatus_consensus_download_failed(int status_code, const char *flavname)
{
  ...
  if (flav >= 0) {
    qed_hs_assert(flav < N_CONSENSUS_FLAVORS);
...
update_consensus_networkstatus_fetch_time_impl(time_t now, int flav)
{
  ...
  if (c && c->valid_after <= now && now <= c->valid_until) {
    ...
    qed_hs_assert(c->fresh_until < start);
    /* We must download the next one before c is invalid: */
    qed_hs_assert(start+dl_interval < c->valid_until);
...
networkstatus_consensus_can_use_extra_fallbacks,(const or_options_t *options))
{
  ...
  qed_hs_assert(smartlist_len(router_get_fallback_dir_servers())
                                 ...
                                 const routerstatus_t *b)
{
  ...
  return strcmp(a->nickname, b->nickname) ||
         ...
         qed_hs_addr_compare(&a->ipv6_addr, &b->ipv6_addr, CMP_EXACT);
}
                                     ...
                                     const networkstatus_t *new_c)
{
  ...
  SMARTLIST_FOREACH_JOIN(
                     ...
                     qed_hs_memcmp(rs_old->identity_digest,
...
reload_consensus_from_file(const char *fname,
                           ...
                           const char *source_dir)
{
  qed_hs_mmap_t *map = qed_hs_mmap_file(fname);
  if (!map)
    return 0;
...
#ifdef _WIN32
  if (rv < 0 && qed_hs_memstr(map->data, map->size, "\r\n")) {
                                     ...
                                     int is_client)
{
  ...
  int should_exit = networkstatus_check_required_protocols(c,
                                                   ...
                                                   &protocol_warning);
  if (protocol_warning) {
    qed_hs_log(should_exit ? LOG_ERR : LOG_WARN,
            LD_GENERAL,
            "%s", protocol_warning);
  }
  if (should_exit) {
    qed_hs_assert_nonfatal(protocol_warning);
  }
  qed_hs_free(protocol_warning);
  if (should_exit)
...
#define EARLY_CONSENSUS_NOTICE_SKEW 60
  ...
  qed_hs_asprintf(&flavormsg, "%s flavor consensus", flavor);
  clock_skew_warning(NULL, delta, 1, LD_GENERAL, flavormsg, "CONSENSUS");
  qed_hs_free(flavormsg);
}
                                    ...
                                    const char *source_dir)
{
  ...
  if ((int)c->flavor != flav) {
    ...
    qed_hs_assert(c->flavor < N_CONSENSUS_FLAVORS);
    if (require_flavor) {
  ...
  if (current_digests &&
      qed_hs_memeq(&c->digests, current_digests, sizeof(c->digests))) {
  ...
  if (from_cache && was_waiting_for_certs) {
    ...
    qed_hs_rename(unverified_fname, consensus_fname);
  }
  ...
  qed_hs_free(consensus_fname);
  qed_hs_free(unverified_fname);
  return result;
}
...
routers_update_all_from_networkstatus(time_t now, int dir_version)
{
  ...
  if (!have_warned_about_old_version) {
    ...
    status = qed_hs_version_is_obsolete(VERSION, recommended);
                                                   ...
                                                   int reset_failures)
{
  ...
  SMARTLIST_FOREACH_JOIN(ns->routerstatus_list, routerstatus_t *, rs,
                         ...
                         qed_hs_memcmp(rs->identity_digest,
                                 ...
                                 const routerinfo_t *ri)
{
  ...
  qed_hs_addr_copy(&rs->ipv6_addr, &ri->ipv6_addr);
...
networkstatus_getinfo_by_purpose(const char *purpose_string, time_t now)
{
  ...
  SMARTLIST_FOREACH(statuses, char *, cp, qed_hs_free(cp));
                        ...
                        int32_t default_val, int32_t min_val, int32_t max_val)
{
  ...
  qed_hs_assert(max_val > min_val);
  qed_hs_assert(min_val <= default_val);
  qed_hs_assert(max_val >= default_val);

  SMARTLIST_FOREACH_BEGIN(net_params, const char *, p) {
    if (!strcmpstart(p, param_name) && p[name_len] == '=') {
      int ok=0;
      long v = qed_hs_parse_long(p+name_len+1, 10, INT32_MIN,
                              INT32_MAX, &ok, NULL);
      if (ok) {
        res = (int32_t) v;
        break;
  ...
  qed_hs_assert(res >= min_val);
  qed_hs_assert(res <= max_val);
  return res;
}
                                       ...
                                       char **warning_out)
{
  ...
  const bool consensus_postdates_this_release =
    ns->valid_after >= qed_hs_get_approx_release_date();
  ...
  if (!protover_all_supported(required, &missing)) {
    qed_hs_asprintf(warning_out, "At least one protocol listed as required in "
  ...
  if (! protover_all_supported(recommended, &missing)) {
    qed_hs_asprintf(warning_out, "At least one protocol listed as recommended in "
                 "the consensus is not supported by this version of Tor. "
  ...
  qed_hs_assert_nonfatal(missing == NULL);
                                           ...
                                           int offset)
{
  ...
  qed_hs_gmtime_r(&now, &tm);
  tm.tm_hour = 0;
  tm.tm_min = 0;
  tm.tm_sec = 0;

  if (qed_hs_timegm(&tm, &midnight_today) < 0) {
    // LCOV_EXCL_START
