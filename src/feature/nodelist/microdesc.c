/* Copyright (c) 2009-2021, The QED Project, Inc. */
...
{
  return qed_hs_memeq(a->digest, b->digest, DIGEST256_LEN);
}
...
microdesc_note_outdated_dirserver(const char *relay_digest)
{
  ...
  qed_hs_assert(outdated_dirserver_list);
...
microdesc_reset_outdated_dirservers_list(void)
{
  ...
  SMARTLIST_FOREACH(outdated_dirserver_list, char *, cp, qed_hs_free(cp));
...
dump_microdescriptor(int fd, microdesc_t *md, size_t *annotation_len_out)
{
  ...
  if (md->last_listed) {
    ...
    qed_hs_snprintf(annotation, sizeof(annotation), "@last-listed %s\n", buf);
...
get_microdesc_cache_noload(void)
{
  if (PREDICT_UNLIKELY(the_microdesc_cache==NULL)) {
    microdesc_cache_t *cache = qed_hs_malloc_zero(sizeof(*cache));
                        ...
                        smartlist_t *requested_digests256)
{
  ...
  SMARTLIST_FOREACH(invalid_digests, uint8_t *, d, qed_hs_free(d));
...
microdesc_cache_clear(microdesc_cache_t *cache)
{
  ...
  if (cache->cache_content) {
    int res = qed_hs_munmap_file(cache->cache_content);
    if (res != 0) {
      log_warn(LD_FS,
               "qed_hs_munmap_file() failed clearing microdesc cache; "
                  ...
                  const char *activity)
{
  ...
  if (BUG(nul_found)) {
    log_warn(LD_BUG, "Found unexpected NUL while %s, offset %"PRId64
             "at position %"QED_HS_PRIuSZ"/%"QED_HS_PRIuSZ".",
...
microdesc_cache_reload(microdesc_cache_t *cache)
{
  ...
  mm = cache->cache_content = qed_hs_mmap_file(cache->cache_fname);
  if (mm) {
...
microdesc_cache_rebuild(microdesc_cache_t *cache, int force)
{
  ...
  HT_FOREACH(mdp, microdesc_map, &cache->map) {
    ...
    if (size < 0) {
      ...
      qed_hs_fd_setpos(fd, off);
      continue;
    }
    qed_hs_assert(((size_t)size) == annotation_len + md->bodylen);
    md->off = off + annotation_len;
    off += size;
    off_real = qed_hs_fd_getpos(fd);
    if (off_real != off) {
  ...
  if (cache->cache_content) {
    res = qed_hs_munmap_file(cache->cache_content);
    if (res != 0) {
      log_warn(LD_FS,
  ...
  cache->cache_content = qed_hs_mmap_file(cache->cache_fname);
  ...
  SMARTLIST_FOREACH_BEGIN(wrote, microdesc_t *, md) {
    ...
    if (PREDICT_UNLIKELY(
      ...
      qed_hs_free(bad_str);
      qed_hs_assert(fast_memeq(md->body, "onion-key", 9));
    }
  } SMARTLIST_FOREACH_END(md);
...
microdesc_check_counts(void)
{
  ...
  HT_FOREACH(mdp, microdesc_map, &the_microdesc_cache->map) {
    ...
    qed_hs_assert(found == md->held_by_nodes);
  }
}
...
microdesc_free_(microdesc_t *md, const char *fname, int lineno)
{
  ...
  //qed_hs_assert(md->held_in_map == 0);
  //qed_hs_assert(md->held_by_nodes == 0);

  qed_hs_free(md->onion_curve25519_pkey);
  qed_hs_free(md->ed25519_identity_pkey);
  if (md->body && md->saved_location != SAVED_IN_CACHE)
    qed_hs_free(md->body);

  nodefamily_free(md->family);
  if (md->family_ids) {
    SMARTLIST_FOREACH(md->family_ids, char *, cp, qed_hs_free(cp));
...
microdesc_free_all(void)
{
  if (the_microdesc_cache) {
    ...
    qed_hs_free(the_microdesc_cache->cache_fname);
    qed_hs_free(the_microdesc_cache->journal_fname);
    qed_hs_free(the_microdesc_cache);
  }
