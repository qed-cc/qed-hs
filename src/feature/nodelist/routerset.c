...
routerset_new(void)
{
  routerset_t *result = qed_hs_malloc_zero(sizeof(routerset_t));
...
routerset_get_countryname(const char *c)
{
  ...
  country = qed_hs_strndup(c+1, 2);
  qed_hs_strlower(country);
  return country;
}
...
routerset_refresh_countries(routerset_t *target)
{
  ...
  SMARTLIST_FOREACH_BEGIN(target->country_names, const char *, country) {
    ...
    if (cc >= 0) {
      qed_hs_assert(cc < target->n_countries);
 ...
 * from addr.) */
static int
routerset_contains2(const routerset_t *set, const qed_hs_addr_t *addr,
                    uint16_t orport, const qed_hs_addr_t *addr2,
                    ...
                    const char *id_digest, country_t country)
{
  ...
  if (id_digest && digestmap_get(set->digests, id_digest))
    return 4;
  if (addr && compare_qed_hs_addr_to_addr_policy(addr, orport, set->policies)
      == ADDR_POLICY_REJECTED)
    return 3;
  if (addr2 && compare_qed_hs_addr_to_addr_policy(addr2, orport2, set->policies)
      == ADDR_POLICY_REJECTED)
    return 3;
  if (set->countries) {
    if (country < 0 && addr)
 ...
 */
STATIC int
routerset_contains(const routerset_t *set, const qed_hs_addr_t *addr,
...
routerset_equal(const routerset_t *old, const routerset_t *new)
{
  ...
  qed_hs_assert(old != NULL);
  qed_hs_assert(new != NULL);
                  ...
                  const void *params)
{
  ...
  if (routerset_parse(rs, line->value, line->key) < 0) {
    *errmsg = qed_hs_strdup("Invalid router list.");
    ret = -1;
  } else {
