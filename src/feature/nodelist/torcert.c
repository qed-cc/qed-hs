 ...
 * certificates are represented with qed_hs_x509_cert_t, and implemented in
...
/** As qed_hs_cert_create(), but accept an arbitrary signed_key_type as the
 * subject key -- not just an ed25519 key.
 */
qed_hs_cert_t *
qed_hs_cert_create_raw(const ed25519_keypair_t *signing_key,
                      uint8_t cert_type,
                      ...
                      uint32_t flags)
{
  qed_hs_cert_t *torcert = NULL;

  ed25519_cert_t *cert = ed25519_cert_new();
  qed_hs_assert(cert); // Unlike Tor's, Trunnel's "new" functions can return NULL.
  ...
  qed_hs_assert(alloc_len > 0);
  uint8_t *encoded = qed_hs_malloc(alloc_len);
  const ssize_t real_len = ed25519_cert_encode(encoded, alloc_len, cert);
  if (real_len < 0)
    goto err;
  qed_hs_assert(real_len == alloc_len);
  qed_hs_assert(real_len > ED25519_SIG_LEN);
  uint8_t *sig = encoded + (real_len - ED25519_SIG_LEN);
  qed_hs_assert(fast_mem_is_zero((char*)sig, ED25519_SIG_LEN));
  ...
  torcert = qed_hs_cert_parse(encoded, real_len);
  if (! torcert) {
    /* LCOV_EXCL_START */
  ...
  if (qed_hs_cert_checksig(torcert, &signing_key->pubkey, now) < 0) {
    /* LCOV_EXCL_START */
    log_warn(LD_BUG, "Generated a certificate whose signature we can't "
             "check: %s", qed_hs_cert_describe_signature_status(torcert));
    goto err;
    /* LCOV_EXCL_STOP */
  }
 ...
 */
qed_hs_cert_t *
qed_hs_cert_create_ed25519(const ed25519_keypair_t *signing_key,
                uint8_t cert_type,
                const ed25519_public_key_t *signed_key,
                time_t now, time_t lifetime,
                uint32_t flags)
{
  return qed_hs_cert_create_raw(signing_key, cert_type,
...
void
qed_hs_cert_free_(qed_hs_cert_t *cert)
{
  if (! cert)
    return;
  ...
  qed_hs_free(cert->encoded);

  memwipe(cert, 0, sizeof(qed_hs_cert_t));
  qed_hs_free(cert);
}

/** Parse a certificate encoded with <b>len</b> bytes in <b>encoded</b>. */
qed_hs_cert_t *
qed_hs_cert_parse(const uint8_t *encoded, const size_t len)
{
  ...
  cert = qed_hs_malloc_zero(sizeof(qed_hs_cert_t));
  cert->encoded = qed_hs_memdup(encoded, len);
 ...
 */
int
qed_hs_cert_get_checkable_sig(ed25519_checkable_t *checkable_out,
                           const qed_hs_cert_t *cert,
                           ...
                           time_t *expiration_out)
{
  ...
  qed_hs_assert(cert->encoded_len > ED25519_SIG_LEN);
 ...
 */
int
qed_hs_cert_checksig(qed_hs_cert_t *cert,
                  const ed25519_public_key_t *pubkey, time_t now)
{
  ...
  if (qed_hs_cert_get_checkable_sig(&checkable, cert, pubkey, &expires) < 0)
    return -1;
 ...
 *
 * Will always be "unchecked" unless qed_hs_cert_checksig has been called.
 */
const char *
qed_hs_cert_describe_signature_status(const qed_hs_cert_t *cert)
{
...
MOCK_IMPL(qed_hs_cert_t *,
qed_hs_cert_dup,(const qed_hs_cert_t *cert))
{
  qed_hs_cert_t *newcert = qed_hs_memdup(cert, sizeof(qed_hs_cert_t));
  if (cert->encoded)
    newcert->encoded = qed_hs_memdup(cert->encoded, cert->encoded_len);
  return newcert;
}

/** Return true iff cert1 and cert2 are the same cert. */
int
qed_hs_cert_eq(const qed_hs_cert_t *cert1, const qed_hs_cert_t *cert2)
{
  qed_hs_assert(cert1);
  qed_hs_assert(cert2);
  return cert1->encoded_len == cert2->encoded_len &&
    qed_hs_memeq(cert1->encoded, cert2->encoded, cert1->encoded_len);
}

/** Return true iff cert1 and cert2 are the same cert, or if they are both
 * NULL. */
int
qed_hs_cert_opt_eq(const qed_hs_cert_t *cert1, const qed_hs_cert_t *cert2)
{
  if (cert1 == NULL && cert2 == NULL)
    return 1;
  if (!cert1 || !cert2)
    return 0;
  return qed_hs_cert_eq(cert1, cert2);
}
...
ssize_t
qed_hs_make_rsa_ed25519_crosscert(const ed25519_public_key_t *ed_key,
                               ...
                               uint8_t **cert)
{
  ...
  qed_hs_assert_nonfatal(expires >= 15 * 365 * 86400);
  ...
  qed_hs_assert(alloc_sz > 0);
  res = qed_hs_malloc_zero(alloc_sz);
  ssize_t sz = rsa_ed_crosscert_encode(res, alloc_sz, cc);
  qed_hs_assert(sz > 0 && sz <= alloc_sz);
  ...
  qed_hs_assert(siglen > 0 && siglen <= (int)crypto_pk_keysize(rsa_key));
  qed_hs_assert(siglen <= UINT8_MAX);
  cc->sig_len = siglen;
...
#define ERR(code, s)                                            \
  ...
  if (qed_hs_memneq(rsa_ed_crosscert_getarray_ed_key(cc),
                 master_key->pubkey,
                 ED25519_PUBKEY_LEN)) {
  ...
  qed_hs_assert(eos >= crosscert);
  qed_hs_assert((size_t)(eos - crosscert) <= crosscert_len);
  qed_hs_assert(siglen == rsa_ed_crosscert_getlen_sig(cc));
  ...
  if (qed_hs_memneq(digest, signed_, DIGEST256_LEN)) {
...
{
  return qed_hs_malloc_zero(sizeof(or_handshake_certs_t));
}
...
or_handshake_certs_free_(or_handshake_certs_t *certs)
{
  ...
  qed_hs_x509_cert_free(certs->auth_cert);
  qed_hs_x509_cert_free(certs->link_cert);
  qed_hs_x509_cert_free(certs->id_cert);

  qed_hs_cert_free(certs->ed_id_sign);
  qed_hs_cert_free(certs->ed_sign_link);
  qed_hs_cert_free(certs->ed_sign_auth);
  qed_hs_free(certs->ed_rsa_crosscert);
...
or_handshake_certs_rsa_ok(int severity,
                          ...
                          time_t now)
{
  qed_hs_x509_cert_t *link_cert = certs->link_cert;
  qed_hs_x509_cert_t *auth_cert = certs->auth_cert;
  qed_hs_x509_cert_t *id_cert = certs->id_cert;

  if (certs->started_here) {
    if (! (id_cert && link_cert))
      ERR("The certs we wanted (ID, Link) were missing");
    if (! qed_hs_tls_cert_matches_key(tls, link_cert))
      ERR("The link certificate didn't match the TLS public key");
    if (! qed_hs_tls_cert_is_valid(severity, link_cert, id_cert, now, 0))
      ERR("The link certificate was not valid");
    if (! qed_hs_tls_cert_is_valid(severity, id_cert, id_cert, now, 1))
      ERR("The ID certificate was not valid");
  } else {
    if (! (id_cert && auth_cert))
      ERR("The certs we wanted (ID, Auth) were missing");
    if (! qed_hs_tls_cert_is_valid(LOG_PROTOCOL_WARN, auth_cert, id_cert, now, 1))
      ERR("The authentication certificate was not valid");
    if (! qed_hs_tls_cert_is_valid(LOG_PROTOCOL_WARN, id_cert, id_cert, now, 1))
      ERR("The ID certificate was not valid");
  }
...
#define ADDCERT(cert, pk)                                               \
  do {                                                                  \
    qed_hs_assert(n_checkable < ARRAY_LENGTH(check));                      \
    if (qed_hs_cert_get_checkable_sig(&check[n_checkable++], cert, pk,     \
                                   &expiration) < 0)                    \
  ...
  if (certs->started_here) {
      ...
      ERR("No Ed25519 link key");
    {
      ...
      qed_hs_x509_cert_t *peer_cert = qed_hs_tls_get_peer_cert(tls);
      if (BUG(!peer_cert)) {
      ...
      const common_digests_t *peer_cert_digests =
        qed_hs_x509_cert_get_cert_digests(peer_cert);
      int okay = qed_hs_memeq(peer_cert_digests->d[DIGEST_SHA256],
                           certs->ed_sign_link->signed_key.pubkey,
                           DIGEST256_LEN);
      qed_hs_x509_cert_free(peer_cert);
      if (!okay)
  ...
  qed_hs_x509_cert_t *rsa_id_cert = certs->id_cert;
  if (!rsa_id_cert) {
    ERR("Missing legacy RSA ID certificate");
  }
  if (! qed_hs_tls_cert_is_valid(severity, rsa_id_cert, rsa_id_cert, now, 1)) {
    ERR("The legacy RSA ID certificate was not valid");
  }
  if (! certs->ed_rsa_crosscert) {
    ERR("Missing RSA->Ed25519 crosscert");
  }
  crypto_pk_t *rsa_id_key = qed_hs_tls_cert_get_key(rsa_id_cert);
  if (!rsa_id_key) {
...
{
  uint8_t *cc = qed_hs_malloc(crypto_pk_keysize(onion_pkey));
  int cc_len =
  ...
  if (qed_hs_memneq(cc, rsa_id_digest, DIGEST_LEN) ||
      qed_hs_memneq(cc + DIGEST_LEN, master_id_pkey->pubkey,
                 ED25519_PUBKEY_LEN)) {
...
{
  qed_hs_assert(ed_id_out);
  qed_hs_assert(rsa_id_out);
  ...
  *rsa_id_out = NULL;
  ...
  if (certs->ed_id_sign) {
    if (or_handshake_certs_ed25519_ok(severity, certs, tls, now)) {
      qed_hs_assert(certs->ed_id_sign);
      qed_hs_assert(certs->id_cert);

      *ed_id_out = &certs->ed_id_sign->signing_key;
      *rsa_id_out = qed_hs_x509_cert_get_id_digests(certs->id_cert);
...
int
qed_hs_cert_encode_ed22519(const qed_hs_cert_t *cert, char **cert_str_out)
{
  int ret = -1;
