...
nodefamily_alloc(int n_members)
{
  ...
  nodefamily_t *nf = qed_hs_malloc_zero(alloc_len);
...
HT_GENERATE2(nodefamily_map, nodefamily_t, ht_ent, nodefamily_hash,
             node_family_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
                        ...
                        unsigned flags)
{
  ...
  SMARTLIST_FOREACH(sl, char *, cp, qed_hs_free(cp));
  smartlist_free(sl);
  SMARTLIST_FOREACH(result_members, char *, cp, qed_hs_free(cp));
  smartlist_free(result_members);
  qed_hs_free(formatted);
                        ...
                        smartlist_t *unrecognized_out)
{
  ...
  SMARTLIST_FOREACH_BEGIN(members, const char *, cp) {
    ...
    if (is_legal_nickname(cp)) {
      ...
      qed_hs_assert(strlen(cp) < DIGEST_LEN); // guaranteed by is_legal_nickname
      memcpy(ptr+1, cp, strlen(cp));
      qed_hs_strlower((char*) ptr+1);
      bad_element = false;
