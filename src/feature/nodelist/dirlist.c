...
{
  qed_hs_assert(dir);
  qed_hs_assert(dir->is_authority);
  ...
  nodelist_add_addr_to_address_set(&dir->ipv4_addr, dir->ipv4_orport,
                                   dir->ipv4_dirport);
  if (!qed_hs_addr_is_null(&dir->ipv6_addr)) {
...
router_get_trusteddirserver_by_digest(const char *digest)
{
  ...
  SMARTLIST_FOREACH(trusted_dir_servers, dir_server_t *, ds,
     {
       if (qed_hs_memeq(ds->digest, digest, DIGEST_LEN))
         return ds;
     });
...
trusteddirserver_get_by_v3_auth_digest, (const char *digest))
{
  ...
  SMARTLIST_FOREACH(trusted_dir_servers, dir_server_t *, ds,
     {
       if (qed_hs_memeq(ds->v3_identity_digest, digest, DIGEST_LEN) &&
           (ds->type & V3_DIRINFO))
         return ds;
     });
        ...
        (const char *digest, dirinfo_type_t type))
{
  ...
  SMARTLIST_FOREACH(trusted_dir_servers, dir_server_t *, ent,
    if (qed_hs_memeq(digest, ent->digest, DIGEST_LEN)) {
 ...
 */
bool
router_addr_is_trusted_dir_type(const qed_hs_addr_t *addr, dirinfo_type_t type)
{
  int family = qed_hs_addr_family(addr);
  ...
  SMARTLIST_FOREACH_BEGIN(trusted_dir_servers, dir_server_t *, ent) {
    ...
    if ((family == AF_INET && qed_hs_addr_eq(addr, &ent->ipv4_addr)) ||
        (family == AF_INET6 && qed_hs_addr_eq(addr, &ent->ipv6_addr))) {
      return true;
    }
  } SMARTLIST_FOREACH_END(ent);
...
dir_server_new(int is_authority,
               const char *nickname,
               const qed_hs_addr_t *ipv4_addr,
               const char *hostname,
               uint16_t ipv4_dirport, uint16_t ipv4_orport,
               const qed_hs_addr_port_t *addrport_ipv6,
               ...
               double weight)
{
  ...
  if (!hostname)
    hostname_ = qed_hs_addr_to_str_dup(ipv4_addr);
  else
    hostname_ = qed_hs_strdup(hostname);

  ent = qed_hs_malloc_zero(sizeof(dir_server_t));
  ent->nickname = nickname ? qed_hs_strdup(nickname) : NULL;
  ent->address = hostname_;
  qed_hs_addr_copy(&ent->ipv4_addr, ipv4_addr);
  ...
  ent->type = type;
  ent->weight = weight;
  if (addrport_ipv6 && qed_hs_addr_port_is_valid_ap(addrport_ipv6, 0)) {
    if (qed_hs_addr_family(&addrport_ipv6->addr) != AF_INET6) {
      log_warn(LD_BUG, "Hey, I got a non-ipv6 addr as addrport_ipv6.");
      qed_hs_addr_make_unspec(&ent->ipv6_addr);
    } else {
      qed_hs_addr_copy(&ent->ipv6_addr, &addrport_ipv6->addr);
  ...
  if (nickname)
    qed_hs_asprintf(&ent->description, "directory server \"%s\" at %s:%" PRIu16,
                 nickname, hostname_, ipv4_dirport);
  else
    qed_hs_asprintf(&ent->description, "directory server at %s:%" PRIu16,
                 hostname_, ipv4_dirport);

  qed_hs_addr_copy(&ent->fake_status.ipv4_addr, &ent->ipv4_addr);
  qed_hs_addr_copy(&ent->fake_status.ipv6_addr, &ent->ipv6_addr);
...
trusted_dir_server_new(const char *nickname, const char *address,
                       uint16_t ipv4_dirport, uint16_t ipv4_orport,
                       const qed_hs_addr_port_t *ipv6_addrport,
                       const char *digest, const char *v3_auth_digest,
                       dirinfo_type_t type, double weight)
{
  ...
  if (!address) { /* The address is us; we should guess. */
    ...
    if (!hostname)
      hostname = qed_hs_addr_to_str_dup(&ipv4_addr);

    if (!hostname)
      return NULL;
  } else {
    if (qed_hs_addr_lookup(address, AF_INET, &ipv4_addr)) {
      log_warn(LD_CONFIG,
               "Unable to lookup address for directory server at '%s'",
               address);
      return NULL;
    }
    hostname = qed_hs_strdup(address);
  }
  ...
  if (ipv4_dirport) {
    qed_hs_addr_port_t p;
    memset(&p, 0, sizeof(p));
    qed_hs_addr_copy(&p.addr, &ipv4_addr);
    p.port = ipv4_dirport;
...
trusted_dir_server_add_dirport(dir_server_t *ds,
                               auth_dirport_usage_t usage,
                               const qed_hs_addr_port_t *dirport)
{
  qed_hs_assert(ds);
  qed_hs_assert(dirport);
  ...
  auth_dirport_t *port = qed_hs_malloc_zero(sizeof(auth_dirport_t));
  port->usage = usage;
  qed_hs_addr_port_copy(&port->dirport, dirport);
...
{
  qed_hs_assert(ds);
  qed_hs_assert_nonfatal(addr_family == AF_INET || addr_family == AF_INET6);
  if (ds->auth_dirports == NULL)
    return NULL;

  SMARTLIST_FOREACH_BEGIN(ds->auth_dirports, const auth_dirport_t *, port) {
    if (port->usage == usage &&
        qed_hs_addr_family(&port->dirport.addr) == addr_family) {
      return &port->dirport;
    }
 ...
 * <b>id_digest</b> */
dir_server_t *
fallback_dir_server_new(const qed_hs_addr_t *ipv4_addr,
                        uint16_t ipv4_dirport, uint16_t ipv4_orport,
                        const qed_hs_addr_port_t *addrport_ipv6,
...
dir_server_free_(dir_server_t *ds)
{
  ...
  if (ds->auth_dirports) {
    SMARTLIST_FOREACH(ds->auth_dirports, auth_dirport_t *, p, qed_hs_free(p));
    smartlist_free(ds->auth_dirports);
  }
  qed_hs_free(ds->nickname);
  qed_hs_free(ds->description);
  qed_hs_free(ds->address);
  qed_hs_free(ds);
}
