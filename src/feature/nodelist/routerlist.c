...
router_rebuild_store(int flags, desc_store_t *store)
{
  ...
  if (store->mmap) {
    int res = qed_hs_munmap_file(store->mmap);
    store->mmap = NULL;
    if (res != 0) {
  ...
  errno = 0;
  store->mmap = qed_hs_mmap_file(fname);
  if (! store->mmap) {
    if (errno == ERANGE) {
      /* empty store.*/
  ...
  qed_hs_free(fname);
  qed_hs_free(fname_tmp);
  if (chunk_list) {
    SMARTLIST_FOREACH(chunk_list, sized_chunk_t *, c, qed_hs_free(c));
...
{
  return qed_hs_addr_eq(&r1->ipv4_addr, &r2->ipv4_addr) &&
    r1->ipv4_orport == r2->ipv4_orport &&
    qed_hs_addr_eq(&r1->ipv6_addr, &r2->ipv6_addr) &&
                            ...
                            const char *nickname)
{
  ...
  return qed_hs_memeq(digest, identity_digest, DIGEST_LEN);
}
                                ...
                                int with_annotations)
{
  ...
  qed_hs_assert(len > 32);
  if (desc->saved_location == SAVED_IN_CACHE && routerlist) {
    desc_store_t *store = desc_get_store(router_get_routerlist(), desc);
    if (store && store->mmap) {
      qed_hs_assert(desc->saved_offset + len <= store->mmap->size);
...
router_get_routerlist(void)
{
  if (PREDICT_UNLIKELY(!routerlist)) {
    routerlist = qed_hs_malloc_zero(sizeof(routerlist_t));
...
routerinfo_free_(routerinfo_t *router)
{
  ...
  qed_hs_free(router->contact_info);
  if (router->tap_onion_pkey)
    qed_hs_free(router->tap_onion_pkey);
  qed_hs_free(router->onion_curve25519_pkey);
  if (router->identity_pkey)
    crypto_pk_free(router->identity_pkey);
  qed_hs_cert_free(router->cache_info.signing_key_cert);
  if (router->declared_family) {
    SMARTLIST_FOREACH(router->declared_family, char *, s, qed_hs_free(s));
    smartlist_free(router->declared_family);
  }
  if (router->family_ids) {
    SMARTLIST_FOREACH(router->family_ids, char *, cp, qed_hs_free(cp));
...
routerlist_free_(routerlist_t *rl)
{
  ...
  if (rl->desc_store.mmap) {
    int res = qed_hs_munmap_file(rl->desc_store.mmap);
    if (res != 0) {
      log_warn(LD_FS, "Failed to munmap routerlist->desc_store.mmap");
    }
  }
  if (rl->extrainfo_store.mmap) {
    int res = qed_hs_munmap_file(rl->extrainfo_store.mmap);
    if (res != 0) {
...
routerlist_find_elt_(smartlist_t *sl, void *ri, int idx)
{
  if (idx < 0) {
    ...
    qed_hs_assert(idx < smartlist_len(sl));
    qed_hs_assert(smartlist_get(sl, idx) == ri);
  };
  return idx;
}
...
routerlist_insert(routerlist_t *rl, routerinfo_t *ri)
{
  ...
  qed_hs_assert(ri->cache_info.routerlist_index == -1);
...
routerlist_remove(routerlist_t *rl, routerinfo_t *ri, int make_old, time_t now)
{
  ...
  qed_hs_assert(0 <= idx && idx < smartlist_len(rl->routers));
  qed_hs_assert(smartlist_get(rl->routers, idx) == ri);
...
routerlist_remove_old(routerlist_t *rl, signed_descriptor_t *sd, int idx)
{
  ...
  qed_hs_assert(0 <= idx && idx < smartlist_len(rl->old_routers));
   ...
   * can get a backtrace. */
  qed_hs_assert(smartlist_get(rl->old_routers, idx) == sd);
  qed_hs_assert(idx == sd->routerlist_index);
                   ...
                   routerinfo_t *ri_new)
{
  ...
  {
    const routerinfo_t *ri_generated = router_get_my_routerinfo();
    qed_hs_assert(ri_generated != ri_new);
  }
  qed_hs_assert(ri_old != ri_new);
  qed_hs_assert(ri_new->cache_info.routerlist_index == -1);

  idx = ri_old->cache_info.routerlist_index;
  qed_hs_assert(0 <= idx && idx < smartlist_len(rl->routers));
  qed_hs_assert(smartlist_get(rl->routers, idx) == ri_old);

  {
    routerinfo_t *ri_old_tmp=NULL;
    nodelist_set_routerinfo(ri_new, &ri_old_tmp);
    qed_hs_assert(ri_old == ri_old_tmp);
  }
  ...
  if (idx >= 0) {
    ...
    qed_hs_assert( routerlist_find_elt_(rl->routers, ri_old, -1) == -1 );
  } else {
    log_warn(LD_BUG, "Appending entry from routerlist_replace.");
    routerlist_insert(rl, ri_new);
    return;
  }
  if (qed_hs_memneq(ri_old->cache_info.identity_digest,
  ...
  if (should_cache_old_descriptors() &&
    ...
    if (!same_descriptors) {
      ...
      if (!qed_hs_digest_is_zero(ri_old->cache_info.extra_info_digest)) {
...
routerlist_free_all(void)
{
  ...
  dirlist_free_all();
  if (warned_nicknames) {
    SMARTLIST_FOREACH(warned_nicknames, char *, cp, qed_hs_free(cp));
                                             ...
                                             digestset_t *retain)
{
  ...
  lifespans = qed_hs_calloc(n, sizeof(struct duration_idx_t));
  rmv = qed_hs_calloc(n, sizeof(uint8_t));
  must_keep = qed_hs_calloc(n, sizeof(uint8_t));
  ...
  for (i = lo; i <= hi; ++i) {
    ...
    if (i < hi) {
      ...
      qed_hs_assert(r->published_on <= r_next->published_on);
  ...
  qed_hs_free(must_keep);
  qed_hs_free(rmv);
  qed_hs_free(lifespans);
}
...
routerlist_remove_old_routers(void)
{
  ...
  */
  ...
  for (i = smartlist_len(routerlist->old_routers)-1; i >= 0; --i) {
    ...
    if (qed_hs_memneq(cur_id, r->identity_digest, DIGEST_LEN)) {
                          ...
                          const char **msg)
{
  ...
  *msg = NULL;
  ...
  qed_hs_snprintf(annotation_buf, sizeof(annotation_buf),
               "@source controller\n"
                       ...
                       int purpose, const char *prefix)
{
  ...
  qed_hs_assert(result || result256);
                               ...
                               int lo, int hi, int pds_flags))
{
  ...
  for (; lo < hi; ++lo) {
    cp = qed_hs_malloc(enc_digest_len);
    if (b64_256) {
  ...
  qed_hs_asprintf(&resource, "d/%s.z", cp);

  SMARTLIST_FOREACH(tmp, char *, cp1, qed_hs_free(cp1));
  smartlist_free(tmp);
  qed_hs_free(cp);
                            ...
                            const routerstatus_t *source, time_t now)
{
  ...
  qed_hs_assert(fetch_microdesc || purpose == DIR_PURPOSE_FETCH_SERVERDESC);
...
router_tap_onion_keys_eq(const routerinfo_t *r1, const routerinfo_t *r2)
{
  ...
  if ((r1->tap_onion_pkey == NULL) && (r2->tap_onion_pkey == NULL)) {
    ...
    return qed_hs_memeq(r1->tap_onion_pkey, r2->tap_onion_pkey,
...
router_differences_are_cosmetic(const routerinfo_t *r1, const routerinfo_t *r2)
{
  ...
  if (!qed_hs_addr_eq(&r1->ipv4_addr, &r2->ipv4_addr) ||
                                       ...
                                       const char **msg)
{
  ...
  digest256_matches = qed_hs_memeq(ei->digest256,
   ...
   * by the same router. */
  if (qed_hs_memneq(sd->identity_digest,
                 ei->cache_info.identity_digest,
                 DIGEST_LEN)) {
  ...
  if (! qed_hs_cert_opt_eq(sd->signing_key_cert,
...
esc_router_info(const routerinfo_t *router)
{
  ...
  qed_hs_asprintf(&info, "Contact %s, Platform %s", esc_contact, esc_platform);
  qed_hs_free(esc_contact);
  qed_hs_free(esc_platform);
