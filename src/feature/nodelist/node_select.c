...
router_pick_directory_server(dirinfo_type_t type, int flags)
{
  ...
  if (busy) {
    ...
    qed_hs_assert((flags & (PDS_NO_EXISTING_SERVERDESC_FETCH|
...
router_picked_poor_directory_log(const routerstatus_t *rs)
{
  ...
  if (!rs) {
    ...
    char *ipv4_str = qed_hs_addr_to_str_dup(&rs->ipv4_addr);
 ...
 * Also returns 0 if addr is NULL, qed_hs_addr_is_null(addr), or dir_port is 0.
 */
STATIC int
router_is_already_dir_fetching(const qed_hs_addr_port_t *ap, int serverdesc,
                               int microdesc)
{
  if (!ap || qed_hs_addr_is_null(&ap->addr) || !ap->port) {
    return 0;
  }
 ...
 */
static int
router_is_already_dir_fetching_(const qed_hs_addr_t *ipv4_addr,
                                const qed_hs_addr_t *ipv6_addr,
                                uint16_t dir_port,
                                int serverdesc,
                                int microdesc)
{
  qed_hs_addr_port_t ipv4_dir_ap, ipv6_dir_ap;

  /* Assume IPv6 DirPort is the same as IPv4 DirPort */
  qed_hs_addr_copy(&ipv4_dir_ap.addr, ipv4_addr);
  ipv4_dir_ap.port = dir_port;
  qed_hs_addr_copy(&ipv6_dir_ap.addr, ipv6_addr);
                            ...
                            uint64_t *total_out)
{
  ...
  for (i = 0; i < n_entries; ++i)
    entries_out[i] = qed_hs_llround(entries_in[i] * scale_factor);
                                           ...
                                           bandwidth_weight_rule_t rule)
{
  ...
  bandwidths_u64 = qed_hs_calloc(smartlist_len(sl), sizeof(uint64_t));
...

  {
    int idx = choose_array_element_by_weight(bandwidths_u64,
                                             smartlist_len(sl));
    qed_hs_free(bandwidths_dbl);
    qed_hs_free(bandwidths_u64);
                            ...
                            double *total_bandwidth_out)
{
  ...
  qed_hs_assert(sl);
  qed_hs_assert(bandwidths_out);

  /* Can't choose exit and guard at same time */
  qed_hs_assert(rule == NO_WEIGHTING ||
  ...
  *bandwidths_out = NULL;
  ...
  bandwidths = qed_hs_calloc(smartlist_len(sl), sizeof(double));
...
dirserver_choose_by_weight(const smartlist_t *servers, double authority_weight)
{
  ...
  weights_dbl = qed_hs_calloc(n, sizeof(double));
  weights_u64 = qed_hs_calloc(n, sizeof(uint64_t));
  for (i = 0; i < n; ++i) {
  ...
  qed_hs_free(weights_dbl);
  qed_hs_free(weights_u64);
