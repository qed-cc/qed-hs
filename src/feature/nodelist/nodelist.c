...
{
  return qed_hs_memeq(node1->identity, node2->identity, DIGEST_LEN);
}

HT_PROTOTYPE(nodelist_map, node_t, ht_ent, node_id_hash, node_id_eq);
HT_GENERATE2(nodelist_map, node_t, ht_ent, node_id_hash, node_id_eq,
             0.6, qed_hs_reallocarray_, qed_hs_free_);
...
HT_GENERATE2(nodelist_ed_map, node_t, ed_ht_ent, node_ed_id_hash,
             node_ed_id_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
...
init_nodelist(void)
{
  if (PREDICT_UNLIKELY(the_nodelist == NULL)) {
    the_nodelist = qed_hs_malloc_zero(sizeof(nodelist_t));
...
node_get_or_create(const char *identity_digest)
{
  ...
  node = qed_hs_malloc_zero(sizeof(node_t));
...
{
  char *s;
  char *olddesc = qed_hs_strdup(node_describe(old));

  qed_hs_asprintf(&s, "Reused ed25519_id %s: old %s new %s", ed_id,
...
node_set_hsdir_index(node_t *node, const networkstatus_t *ns)
{
  ...
  qed_hs_free(fetch_srv);
  qed_hs_free(store_first_srv);
  qed_hs_free(store_second_srv);
  return;
}
...
static char *
build_addr_port_item(const qed_hs_addr_t *addr, const uint16_t port)
{
...
void
nodelist_add_addr_to_address_set(const qed_hs_addr_t *addr,
                                 uint16_t or_port, uint16_t dir_port)
{
  if (BUG(!addr) || qed_hs_addr_is_null(addr) ||
      (!qed_hs_addr_is_v4(addr) && !qed_hs_addr_is_v6(addr)) ||
...
int
nodelist_probably_contains_address(const qed_hs_addr_t *addr)
{
  if (BUG(!addr))
    return 0;
...
bool
nodelist_reentry_contains(const qed_hs_addr_t *addr, uint16_t port)
{
...
nodelist_set_consensus(const networkstatus_t *ns)
{
  ...
  SMARTLIST_FOREACH_BEGIN(ns->routerstatus_list, routerstatus_t *, rs) {
    ...
    /* Set node's flags based on rs's flags. */
    {
      ...
      if (reachable_addr_prefer_ipv6_orport(options) &&
          (qed_hs_addr_is_null(&rs->ipv6_addr) == 0 ||
           (node->md && qed_hs_addr_is_null(&node->md->ipv6_addr) == 0)))
  ...
  if (! authdir) {
    SMARTLIST_FOREACH_BEGIN(the_nodelist->nodes, node_t *, node) {
       ...
       * it, maybe.*/
      if (!node->rs) {
        qed_hs_assert(node->ri); /* if it had only an md, or nothing, purge
...
nodelist_drop_node(node_t *node, int remove_from_ht)
{
  ...
  qed_hs_assert(node == smartlist_get(the_nodelist->nodes, idx));
...
node_free_(node_t *node)
{
  ...
  if (node->md)
    node->md->held_by_nodes--;
  qed_hs_assert(node->nodelist_idx == -1);
  qed_hs_free(node);
}
...
nodelist_assert_ok(void)
{
  ...
  if (rl) {
    SMARTLIST_FOREACH_BEGIN(rl->routers, routerinfo_t *, ri) {
      ...
      qed_hs_assert(node && node->ri == ri);
      qed_hs_assert(fast_memeq(ri->cache_info.identity_digest,
                             node->identity, DIGEST_LEN));
      qed_hs_assert(! digestmap_get(dm, node->identity));
  ...
  if (ns) {
    SMARTLIST_FOREACH_BEGIN(ns->routerstatus_list, routerstatus_t *, rs) {
      ...
      qed_hs_assert(node && node->rs == rs);
      qed_hs_assert(fast_memeq(rs->identity_digest, node->identity, DIGEST_LEN));
      digestmap_set(dm, node->identity, (void*)node);
      if (ns->flavor == FLAV_MICRODESC) {
   ...
   * well-formed. */
  SMARTLIST_FOREACH_BEGIN(the_nodelist->nodes, node_t *, node) {
    qed_hs_assert(digestmap_get(dm, node->identity) != NULL);
    qed_hs_assert(node_sl_idx == node->nodelist_idx);
  } SMARTLIST_FOREACH_END(node);
  ...
  SMARTLIST_FOREACH_BEGIN(the_nodelist->nodes, node_t *, node) {
    if (!ed25519_public_key_is_zero(&node->ed25519_id)) {
      qed_hs_assert(node == node_get_by_ed25519_id(&node->ed25519_id));
    }
  } SMARTLIST_FOREACH_END(node);
  ...
  qed_hs_assert((long)smartlist_len(the_nodelist->nodes) ==
...
node_get_link_specifier_smartlist,(const node_t *node, bool direct_conn))
{
   ...
   * address (or both). */
  if (BUG(!qed_hs_addr_is_v4(&ap.addr)) ||
      BUG(!qed_hs_addr_port_is_valid_ap(&ap, 0))) {
    return lspecs;
  }

  ls = link_specifier_new();
  link_specifier_set_ls_type(ls, LS_IPV4);
  link_specifier_set_un_ipv4_addr(ls, qed_hs_addr_to_ipv4h(&ap.addr));
  ...
  if (node_has_ipv6_orport(node)) {
    ...
    const uint8_t *in6_addr = qed_hs_addr_to_in6_addr8(&ap.addr);
 ...
 * qed_hs_addr_port_t containing "addr" and port_field to sl.
...
#define SL_ADD_NEW_AP(r, addr_field, port_field, sl, valid)             \
  STMT_BEGIN                                                            \
    if (qed_hs_addr_port_is_valid(&(r)->addr_field, (r)->port_field, 0)) { \
      valid = 1;                                                        \
      qed_hs_addr_port_t *ap = qed_hs_addr_port_new(&(r)->addr_field,         \
                                              (r)->port_field);         \
      smartlist_add((sl), ap);                                          \
    }                                                                   \
  STMT_END

/** Return list of qed_hs_addr_port_t with all OR ports (in the sense IP
 * addr + TCP port) for <b>node</b>.  Caller must free all elements
 * using qed_hs_free() and free the list using smartlist_free().
 *
...
/** Wrapper around node_get_prim_orport for backward
    compatibility.  */
void
node_get_addr(const node_t *node, qed_hs_addr_t *addr_out)
{
  qed_hs_addr_port_t ap;
  node_get_prim_orport(node, &ap);
  qed_hs_addr_copy(addr_out, &ap.addr);
}

/** Return the IPv4 address for <b>node</b>, or NULL if none found. */
static const qed_hs_addr_t *
node_get_prim_addr_ipv4(const node_t *node)
{
   ...
   * ORPort or DirPort.
   */
  if (node->ri && qed_hs_addr_is_valid(&node->ri->ipv4_addr, 0)) {
    return &node->ri->ipv4_addr;
  } else if (node->rs && qed_hs_addr_is_valid(&node->rs->ipv4_addr, 0)) {
...
{
  const qed_hs_addr_t *ipv4_addr = node_get_prim_addr_ipv4(node);

  if (ipv4_addr) {
    qed_hs_addr_to_str(buf, ipv4_addr, len, 0);
  } else if (len > 0) {
    buf[0] = '\0';
  }
}
...
node_has_ipv6_addr(const node_t *node)
{
   ...
   * ORPort or DirPort.
   */
  if (node->ri && qed_hs_addr_is_valid(&node->ri->ipv6_addr, 0))
    return 1;
  if (node->rs && qed_hs_addr_is_valid(&node->rs->ipv6_addr, 0))
    return 1;
  if (node->md && qed_hs_addr_is_valid(&node->md->ipv6_addr, 0))
    return 1;
...
{
  qed_hs_addr_port_t ipv6_orport;
  node_get_pref_ipv6_orport(node, &ipv6_orport);
  return qed_hs_addr_port_is_valid_ap(&ipv6_orport, 0);
}
...
{
  qed_hs_addr_port_t ipv6_dirport;
  node_get_pref_ipv6_dirport(node, &ipv6_dirport);
  return qed_hs_addr_port_is_valid_ap(&ipv6_dirport, 0);
}
...
node_ipv6_or_preferred(const node_t *node)
{
  ...
  if (!reachable_addr_use_ipv6(options)) {
    return 0;
             ...
             !qed_hs_addr_port_is_valid_ap(&ipv4_addr, 0)) {
...
#define RETURN_IPV4_AP(r, port_field, ap_out)                               \
  STMT_BEGIN                                                                \
    if (r && qed_hs_addr_port_is_valid(&(r)->ipv4_addr, (r)->port_field, 0)) { \
      qed_hs_addr_copy(&(ap_out)->addr, &(r)->ipv4_addr);                      \
 ...
 * into *<b>ap_out</b>. */
void
node_get_prim_orport(const node_t *node, qed_hs_addr_port_t *ap_out)
{
  node_assert_ok(node);
  qed_hs_assert(ap_out);

  /* Clear the address, as a safety precaution if calling functions ignore the
   * return value */
  qed_hs_addr_make_null(&ap_out->addr, AF_INET);
  ap_out->port = 0;
...
void
node_get_pref_orport(const node_t *node, qed_hs_addr_port_t *ap_out)
{
  qed_hs_assert(ap_out);
 ...
 * <b>node</b> into *<b>ap_out</b>. */
void
node_get_pref_ipv6_orport(const node_t *node, qed_hs_addr_port_t *ap_out)
{
  node_assert_ok(node);
  qed_hs_assert(ap_out);
  ...
  if (node->ri && qed_hs_addr_port_is_valid(&node->ri->ipv6_addr,
                                         node->ri->ipv6_orport, 0)) {
    qed_hs_addr_copy(&ap_out->addr, &node->ri->ipv6_addr);
    ap_out->port = node->ri->ipv6_orport;
  } else if (node->rs && qed_hs_addr_port_is_valid(&node->rs->ipv6_addr,
                                                 node->rs->ipv6_orport, 0)) {
    qed_hs_addr_copy(&ap_out->addr, &node->rs->ipv6_addr);
    ap_out->port = node->rs->ipv6_orport;
  } else if (node->md && qed_hs_addr_port_is_valid(&node->md->ipv6_addr,
                                                 node->md->ipv6_orport, 0)) {
    qed_hs_addr_copy(&ap_out->addr, &node->md->ipv6_addr);
    ap_out->port = node->md->ipv6_orport;
  } else {
    qed_hs_addr_make_null(&ap_out->addr, AF_INET6);
    ap_out->port = 0;
  }
}
...
node_ipv6_dir_preferred(const node_t *node)
{
  ...
  if (!reachable_addr_use_ipv6(options)) {
    return 0;
  } else if (!qed_hs_addr_port_is_valid_ap(&ipv4_addr, 0)
 ...
 * into *<b>ap_out</b>. */
void
node_get_prim_dirport(const node_t *node, qed_hs_addr_port_t *ap_out)
{
  node_assert_ok(node);
  qed_hs_assert(ap_out);
 ...
 * <b>node</b> into *<b>ap_out</b>.  */
void
node_get_pref_dirport(const node_t *node, qed_hs_addr_port_t *ap_out)
{
  qed_hs_assert(ap_out);
 ...
 * <b>node</b> into *<b>ap_out</b>. */
void
node_get_pref_ipv6_dirport(const node_t *node, qed_hs_addr_port_t *ap_out)
{
  node_assert_ok(node);
  qed_hs_assert(ap_out);
...
{
  const qed_hs_addr_t *ipv4_addr = NULL;
...
int
router_addrs_in_same_network(const qed_hs_addr_t *a1,
                             const qed_hs_addr_t *a2)
{
  if (qed_hs_addr_is_null(a1) || qed_hs_addr_is_null(a2))
    return 0;

  switch (qed_hs_addr_family(a1)) {
    case AF_INET:
      return 0 == qed_hs_addr_compare_masked(a1, a2, 16, CMP_SEMANTIC);
    case AF_INET6:
      return 0 == qed_hs_addr_compare_masked(a1, a2, 32, CMP_SEMANTIC);
    default:
...
nodes_in_same_family(const node_t *node1, const node_t *node2)
{
  ...
  if (options->EnforceDistinctSubnets) {
    ...
    qed_hs_addr_port_t ap6_1, ap6_2;
...
nodelist_add_node_and_family(smartlist_t *sl, const node_t *node)
{
  ...
  if (options->EnforceDistinctSubnets) {
    qed_hs_addr_t node_addr;
    qed_hs_addr_port_t node_ap6;
    node_get_addr(node, &node_addr);
    node_get_pref_ipv6_orport(node, &node_ap6);

    SMARTLIST_FOREACH_BEGIN(all_nodes, const node_t *, node2) {
      qed_hs_addr_t a;
      qed_hs_addr_port_t ap6;
...
{/*XXXX MOVE*/
  ...
  if (!qed_hs_inet_aton(address, &in))
    return NULL; /* it's not an IP already */
  qed_hs_addr_from_in(&ipv4_addr, &in);

  SMARTLIST_FOREACH(nodelist_get_list(), const node_t *, node, {
    if (qed_hs_addr_eq(node_get_prim_addr_ipv4(node), &ipv4_addr) &&
        node->is_running &&
        compare_qed_hs_addr_to_node_policy(&ipv4_addr, port, node) ==
...
int
router_exit_policy_all_nodes_reject(const qed_hs_addr_t *addr, uint16_t port,
                                    int need_uptime)
{
  ...
  SMARTLIST_FOREACH_BEGIN(nodelist_get_list(), const node_t *, node) {
    if (node->is_running &&
      ...
      r = compare_qed_hs_addr_to_node_policy(addr, port, node);
...
router_set_status(const char *digest, int up)
{
  ...
  SMARTLIST_FOREACH(router_get_fallback_dir_servers(),
                    dir_server_t *, d,
                    if (qed_hs_memeq(d->digest, digest, DIGEST_LEN))
                      d->is_running = up);
 ...
 * If <b>exit_only</b> & USABLE_DESCRIPQED_HS_EXIT_POLICY, only consider nodes
 * present if they have an exit policy that accepts at least one port.
 * If <b>exit_only</b> & USABLE_DESCRIPQED_HS_EXIT_FLAG, only consider nodes
  ...
  *num_present = 0, *num_usable = 0;
  ...
  SMARTLIST_FOREACH_BEGIN(consensus->routerstatus_list, routerstatus_t *, rs)
    {
       ...
       if ((exit_only & USABLE_DESCRIPQED_HS_EXIT_FLAG) && ! rs->is_exit)
         continue;
       ...
       if (client_would_use_router(rs, now)) {
         ...
         if (present) {
           ...
           if ((exit_only & USABLE_DESCRIPQED_HS_EXIT_POLICY) &&
  ...
  log_debug(LD_DIR, "%d usable, %d present (%s%s%s%s%s).",
            *num_usable, *num_present,
            md ? "microdesc" : "desc",
            (exit_only & USABLE_DESCRIPQED_HS_EXIT_POLICY_AND_FLAG) ?
              " exit"     : "s",
            (exit_only & USABLE_DESCRIPQED_HS_EXIT_POLICY) ?
              " policies" : "" ,
            (exit_only == USABLE_DESCRIPQED_HS_EXIT_POLICY_AND_FLAG) ?
              " and" : "" ,
            (exit_only & USABLE_DESCRIPQED_HS_EXIT_FLAG) ?
              " flags"    : "" );
}
                             ...
                             char **status_out)
{
  ...
  if (options->EntryNodes) {
    count_usable_descriptors(&np, &nu, guards, consensus, now,
                             options->EntryNodes, USABLE_DESCRIPQED_HS_ALL);
    log_debug(LD_NET,
              "%s: %d present, %d usable",
              "guard",
              np,
  ...
  count_usable_descriptors(&np, &nu, exits, consensus, now,
                           NULL, USABLE_DESCRIPQED_HS_EXIT_POLICY_AND_FLAG);
  log_debug(LD_NET,
            "%s: %d present, %d usable",
            "exits",
            np,
  ...
  if (options->ExitNodes) {
    ...
    count_usable_descriptors(&np, &nu, myexits, consensus, now,
                             options->ExitNodes,
                             USABLE_DESCRIPQED_HS_EXIT_POLICY_AND_FLAG);
    log_debug(LD_NET,
              "%s: %d present, %d usable",
              "myexits",
              np,
    ...
    count_usable_descriptors(&np, &nu, myexits_unflagged,
                             consensus, now,
                             options->ExitNodes,
                             USABLE_DESCRIPQED_HS_EXIT_POLICY);
    log_debug(LD_NET,
              "%s: %d present, %d usable",
...
update_router_have_minimum_dir_info(void)
{
  ...
  /* Check fraction of available paths */
  {
    ...
    if (paths < get_frac_paths_needed_for_circs(options,consensus)) {
      qed_hs_snprintf(dir_info_status, sizeof(dir_info_status),
  ...
  if (res && !have_min_dir_info) {
    ...
    qed_hs_log(be_loud_when_things_work_again ? LOG_NOTICE : LOG_INFO, LD_DIR,
