...
{
  hs_ident_circuit_t *ident = qed_hs_malloc_zero(sizeof(*ident));
...
{
  hs_ident_dir_conn_t *ident = qed_hs_malloc_zero(sizeof(*ident));
  memcpy(ident, src, sizeof(*ident));
  return ident;
}
...
{
  hs_ident_edge_conn_t *ident = qed_hs_malloc_zero(sizeof(*ident));
  ed25519_pubkey_copy(&ident->identity_pk, identity_pk);
  return ident;
}
