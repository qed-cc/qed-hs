...
set_unix_port(edge_connection_t *conn, hs_port_config_t *p)
{
  ...
  qed_hs_addr_make_unspec(&conn->base_.addr);
  conn->base_.port = 1;
  conn->base_.address = qed_hs_strdup(p->unix_addr);
  return 0;
}
...
compare_digest_to_fetch_hsdir_index(const void *_key, const void **_member)
{
  ...
  return qed_hs_memcmp(key, node->hsdir_index.fetch, DIGEST256_LEN);
}
                                          ...
                                          const void **_member)
{
  const char *key = _key;
  const node_t *node = *_member;
  return qed_hs_memcmp(key, node->hsdir_index.store_first, DIGEST256_LEN);
}
                                          ...
                                          const void **_member)
{
  const char *key = _key;
  const node_t *node = *_member;
  return qed_hs_memcmp(key, node->hsdir_index.store_second, DIGEST256_LEN);
}
...
compare_node_fetch_hsdir_index(const void **a, const void **b)
{
  ...
  return qed_hs_memcmp(node1->hsdir_index.fetch,
...
compare_node_store_first_hsdir_index(const void **a, const void **b)
{
  const node_t *node1= *a;
  const node_t *node2 = *b;
  return qed_hs_memcmp(node1->hsdir_index.store_first,
...
compare_node_store_second_hsdir_index(const void **a, const void **b)
{
  const node_t *node1= *a;
  const node_t *node2 = *b;
  return qed_hs_memcmp(node1->hsdir_index.store_second,
...
hs_path_from_filename(const char *directory, const char *filename)
{
  ...
  qed_hs_assert(directory);
  qed_hs_assert(filename);

  qed_hs_asprintf(&file_path, "%s%s%s", directory, PATH_SEPARATOR, filename);
  return file_path;
}
...
get_time_period_length(void)
{
  ...
  if (get_options()->TestingTorNetwork) {
    ...
    qed_hs_assert_nonfatal(run_duration > 60);
  ...
  qed_hs_assert(time_period_length > 0);
...
hs_get_time_period_num(time_t now)
{
  ...
  qed_hs_assert(minutes_since_epoch > time_period_rotation_offset);
...
compute_disaster_srv(uint64_t time_period_num, uint8_t *srv_out)
{
  ...
  /* Setup INT_8(period_length) | INT_8(period_num) */
  {
    ...
    size_t offset = 0;
    set_uint64(period_stuff, qed_hs_htonll(time_period_length));
    offset += sizeof(uint64_t);
    set_uint64(period_stuff+offset, qed_hs_htonll(time_period_num));
    offset += sizeof(uint64_t);
    qed_hs_assert(offset == sizeof(period_stuff));
                        ...
                        uint8_t *param_out)
{
  ...
  qed_hs_assert(pubkey);
  qed_hs_assert(param_out);
  ...
  set_uint64(nonce + offset, qed_hs_htonll(period_num));
  offset += sizeof(uint64_t);
  set_uint64(nonce + offset, qed_hs_htonll(period_length));
  offset += sizeof(uint64_t);
  qed_hs_assert(offset == HS_KEYBLIND_NONCE_LEN);
                  ...
                  uint8_t *checksum_out)
{
  ...
  qed_hs_assert(offset == HS_SERVICE_ADDR_CHECKSUM_INPUT_LEN);
                 ...
                 uint8_t version, char *addr_out)
{
  ...
  qed_hs_assert(offset == HS_SERVICE_ADDR_LEN);
}
...
hs_set_conn_addr_port(const smartlist_t *ports, edge_connection_t *conn)
{
  ...
  if (chosen_port) {
    ...
    if (!(chosen_port->is_unix_addr)) {
      ...
      qed_hs_addr_copy(&TO_CONN(conn)->addr, &chosen_port->real_addr);
...
{
  if (!socket_path)
    return qed_hs_malloc_zero(sizeof(hs_port_config_t) + 1);

  const size_t pathlen = strlen(socket_path) + 1;
  hs_port_config_t *conf =
    qed_hs_malloc_zero(sizeof(hs_port_config_t) + pathlen);
                               ...
                               char **err_msg_out)
{
  ...
  if (smartlist_len(sl) < 1 || BUG(smartlist_len(sl) > 2)) {
    err_msg = qed_hs_strdup("Bad syntax in hidden service port configuration.");
    goto err;
  }
  virtport = (int)qed_hs_parse_long(smartlist_get(sl,0), 10, 1, 65535, NULL,NULL);
  if (!virtport) {
    qed_hs_asprintf(&err_msg, "Missing or invalid port %s in hidden service "
  ...
  if (smartlist_len(sl) == 1) {
    ...
    realport = virtport;
    qed_hs_addr_from_ipv4h(&addr, 0x7F000001u); /* 127.0.0.1 */
  } else {
    int ret;
    ...
    if (ret < 0) {
      qed_hs_asprintf(&err_msg, "Couldn't process address <%s> from hidden "
                   "service configuration", addrport_element);
      goto err;
    }

    if (rest && strlen(rest)) {
      err_msg = qed_hs_strdup("HiddenServicePort parse error: invalid port "
                           "mapping");
      goto err;
    }
    ...
    if (is_unix) {
      ...
      if (qed_hs_addr_port_lookup(addrport, &addr, &p)<0) {
        err_msg = qed_hs_strdup("Unparseable address in hidden service port "
                             "configuration.");
        goto err;
      }
      ...
      realport = (int)qed_hs_parse_long(addrport, 10, 1, 65535, NULL, NULL);
      if (!realport) {
        qed_hs_asprintf(&err_msg, "Unparseable or out-of-range port %s in "
                     "hidden service port configuration.",
                     escaped(addrport));
        goto err;
      }
      qed_hs_addr_from_ipv4h(&addr, 0x7F000001u); /* Default to 127.0.0.1 */
    }
  }
  ...
  if (!is_unix_addr) {
    ...
    qed_hs_addr_copy(&result->real_addr, &addr);
...
hs_address_is_valid(const char *address)
{
  ...
  if (qed_hs_memcmp(checksum, target_checksum, sizeof(checksum))) {
                 ...
                 char *addr_out)
{
  ...
  qed_hs_assert(hs_address_is_valid(addr_out));
}
                        ...
                        ed25519_public_key_t *blinded_pk_out)
{
  ...
  qed_hs_assert(pk);
  qed_hs_assert(blinded_pk_out);
  qed_hs_assert(!fast_mem_is_zero((char *) pk, ED25519_PUBKEY_LEN));
                         ...
                         ed25519_keypair_t *blinded_kp_out)
{
  ...
  qed_hs_assert(kp);
  qed_hs_assert(blinded_kp_out);
  /* Extra safety. A zeroed key is bad. */
  qed_hs_assert(!fast_mem_is_zero((char *) &kp->pubkey, ED25519_PUBKEY_LEN));
  qed_hs_assert(!fast_mem_is_zero((char *) &kp->seckey, ED25519_SECKEY_LEN));
                  ...
                  uint64_t period_num, uint8_t *hs_index_out)
{
  ...
  /* Now setup INT_8(replicanum) | INT_8(period_length) | INT_8(period_num) */
  {
    ...
    size_t offset = 0;
    set_uint64(buf, qed_hs_htonll(replica));
    offset += sizeof(uint64_t);
    set_uint64(buf+offset, qed_hs_htonll(period_length));
    offset += sizeof(uint64_t);
    set_uint64(buf+offset, qed_hs_htonll(period_num));
    offset += sizeof(uint64_t);
    qed_hs_assert(offset == sizeof(buf));
                     ...
                     uint8_t *hsdir_index_out)
{
  ...
  qed_hs_assert(identity_pk);
  qed_hs_assert(srv_value);
  qed_hs_assert(hsdir_index_out);
...

  {
    ...
    char period_stuff[sizeof(uint64_t)*2];
    size_t offset = 0;
    set_uint64(period_stuff, qed_hs_htonll(period_num));
    offset += sizeof(uint64_t);
    set_uint64(period_stuff+offset, qed_hs_htonll(time_period_length));
    offset += sizeof(uint64_t);
    qed_hs_assert(offset == sizeof(period_stuff));
...
{
  uint8_t *sr_value = qed_hs_malloc_zero(DIGEST256_LEN);
...
{
  qed_hs_assert(node_supports_v3_hsdir(node));
                          ...
                          int for_fetching, smartlist_t *responsible_dirs)
{
  ...
  qed_hs_assert(blinded_pk);
  qed_hs_assert(responsible_dirs);
                                ...
                                time_t now, int set)
{
  ...
  qed_hs_asprintf(&hsdir_desc_comb_id, "%s%s", hsdir_id_base32, req_key_str);

  if (set) {
    time_t *oldptr;
    last_request_ptr = qed_hs_malloc_zero(sizeof(time_t));
  ...
  qed_hs_free(hsdir_desc_comb_id);
...
hs_purge_hid_serv_from_last_hid_serv_requests(const char *req_key_str)
{
  ...
  for (iter = strmap_iter_init(last_hid_serv_requests);
    ...
    if (qed_hs_memeq(key + REND_DESC_ID_V2_LEN_BASE32, req_key_str,
                  strlen(req_key_str))) {
      iter = strmap_iter_next_rmv(last_hid_serv_requests, iter);
      qed_hs_free(val);
    } else {
...
hs_purge_last_hid_serv_requests(void)
{
 /* Don't create the table if it doesn't exist yet (and it may very
  ...
  if (old_last_hid_serv_requests != NULL) {
    ...
    strmap_free(old_last_hid_serv_requests, qed_hs_free_);
  }
}
                               ...
                               int direct_conn)
{
  ...
  SMARTLIST_FOREACH_BEGIN(lspecs, const link_specifier_t *, ls) {
    ...
    case LS_IPV4:
      ...
      qed_hs_addr_from_ipv4h(&ap.addr,
  ...
  if (!qed_hs_addr_port_is_valid_ap(&ap, 0)) {
...
/** Initialize the entire HS subsystem. This is called in qed_hs_init() before any
...
link_specifier_dup(const link_specifier_t *src)
{
  ...
  buf = qed_hs_malloc_zero(encoded_len_alloc);
