                       ...
                       unsigned int cell_type, const void *cell)
{
  ...
  default:
    ...
    qed_hs_assert_unreached(); /* LCOV_EXCL_LINE */
  }
  qed_hs_assert(key_array);
  qed_hs_assert(auth_key_len == sizeof(auth_key_out->pubkey));
                            ...
                            size_t circuit_key_material_len)
{
  ...
  /* Verify the MAC */
  {
    ...
    crypto_mac_sha3_256(mac, sizeof(mac),
                        ...
                        msg, auth_msg_len);
    if (qed_hs_memneq(mac, cell->handshake_mac, sizeof(mac))) {
      log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
...
hs_intro_send_intro_established_cell,(or_circuit_t *circ))
{
  ...
  encoded_cell = qed_hs_malloc_zero(encoded_len);
  result_len = trn_cell_intro_established_encode(encoded_cell, encoded_len,
                                                cell);
  qed_hs_assert(encoded_len == result_len);
...
{
  qed_hs_assert(circ);
  qed_hs_assert(log_cell_type_str);
