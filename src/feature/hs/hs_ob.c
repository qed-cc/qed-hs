...
ob_option_parse(hs_service_config_t *config, const ob_options_t *opts)
{
  ...
  for (line = opts->MasterOnionAddress; line; line = line->next) {
    ...
    ed25519_public_key_t *pubkey = qed_hs_malloc_zero(sizeof(*pubkey));
                       ...
                       hs_subcredential_t **subcredentials_out)
{
  ...
  qed_hs_assert(subcredentials_out);
  ...
  subcreds =
    qed_hs_calloc((num_steps * num_pkeys) + 2, sizeof(hs_subcredential_t));
