                                    ...
                                    uint64_t *next_lowest)
{
  ...
  log_info(LD_REND, "Cleaning HS cache for downloaded target of %" PRIu64
                    ". Maximum bytes to removed: %" QED_HS_PRIuSZ,
                       ...
                       const char **desc_out)
{
  ...
  qed_hs_assert(hs_desc_is_supported_version(version));
                      ...
                      hs_desc_decode_status_t *decode_status_out)
{
  ...
  client_desc->encoded_desc = qed_hs_strdup(desc_str);
...
{
  hs_cache_intro_state_t *state = qed_hs_malloc_zero(sizeof(*state));
...
{
  hs_cache_client_intro_state_t *cache = qed_hs_malloc_zero(sizeof(*cache));
                                ...
                                hs_cache_intro_state_t **entry)
{
  ...
  qed_hs_assert_nonfatal(!ed25519_public_key_is_zero(service_pk));
  qed_hs_assert_nonfatal(!ed25519_public_key_is_zero(auth_key));
                             ...
                             hs_cache_intro_state_t **state)
{
  ...
  qed_hs_assert_nonfatal(old_entry == NULL);
  qed_hs_free(old_entry);
...
hs_cache_lookup_encoded_as_client(const ed25519_public_key_t *key)
{
  ...
  if (cached_desc) {
    qed_hs_assert(cached_desc->encoded_desc);
...
hs_cache_init(void)
{
  ...
  qed_hs_assert(!hs_cache_v3_dir);
  hs_cache_v3_dir = digest256map_new();

  qed_hs_assert(!hs_cache_v3_client);
  hs_cache_v3_client = digest256map_new();

  qed_hs_assert(!hs_cache_client_intro_state);
