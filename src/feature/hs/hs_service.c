...
#define FOR_EACH_DESCRIPQED_HS_END } STMT_END ;
#endif /* !defined(COCCI) */
...
HT_GENERATE2(hs_service_ht, hs_service_t, hs_service_node,
             ...
             0.6, qed_hs_reallocarray, qed_hs_free_);
...
register_service(hs_service_ht *map, hs_service_t *service)
{
  qed_hs_assert(map);
  qed_hs_assert(service);
  qed_hs_assert(!ed25519_public_key_is_zero(&service->keys.identity_pk));
...
{
  service->state.pow_state = qed_hs_malloc_zero(sizeof(hs_pow_service_state_t));
...
service_clear_config(hs_service_config_t *config)
{
  ...
  qed_hs_free(config->directory_path);
  if (config->ports) {
...
service_intro_point_new(const node_t *node)
{
  ...
  ip = qed_hs_malloc_zero(sizeof(*ip));
...
service_intro_point_add(digest256map_t *map, hs_service_intro_point_t *ip)
{
  ...
  qed_hs_assert_nonfatal(!old_ip_entry);
}
...
{
  qed_hs_assert(src_service);
  qed_hs_assert(dst_service);
...
register_all_services(void)
{
  ...
  qed_hs_assert(hs_service_staging_list);

  /* Allocate a new map that will replace the current one. */
  new_service_map = qed_hs_malloc_zero(sizeof(*new_service_map));
...
load_service_keys(hs_service_t *service)
{
  ...
  qed_hs_assert(service->config.version <= UINT8_MAX);
...
parse_authorized_client_key(const char *key_str, int severity)
{
  ...
  client = qed_hs_malloc_zero(sizeof(hs_service_authorized_client_t));
...
parse_authorized_client(const char *client_key_str)
{
 ...
 err:
  ...
  qed_hs_assert(fields);
  SMARTLIST_FOREACH(fields, char *, s, qed_hs_free(s));
...
load_client_keys(hs_service_t *service)
{
  ...
  file_list = qed_hs_listdir(client_keys_dir_path);
  if (file_list == NULL) {
  ...
  SMARTLIST_FOREACH_BEGIN(file_list, const char *, filename) {
    ...
    if (!client_key_str) {
      ...
      qed_hs_free(client_key_file_path);
      continue;
    }
    qed_hs_free(client_key_file_path);
  ...
  if (file_list) {
    SMARTLIST_FOREACH(file_list, char *, s, qed_hs_free(s));
    smartlist_free(file_list);
  }
  qed_hs_free(client_key_str);
  qed_hs_free(client_key_file_path);
  qed_hs_free(client_keys_dir_path);
  return ret;
}
...
service_descripqed_hs_free_(hs_service_descripqed_hs_t *desc)
{
  ...
  digestmap_free(desc->intro_points.failed_id, qed_hs_free_);
  if (desc->previous_hsdirs) {
    SMARTLIST_FOREACH(desc->previous_hsdirs, char *, s, qed_hs_free(s));
...
service_authorized_client_dup(const hs_service_authorized_client_t *client)
{
  ...
  client_dup = qed_hs_malloc_zero(sizeof(hs_service_authorized_client_t));
...
{
  qed_hs_assert(client1);
  qed_hs_assert(client2);

  /* Currently, the public key is the only component of
   * hs_service_authorized_client_t. */
  return qed_hs_memcmp(client1->client_pk.public_key,
                                       ...
                                       const hs_service_config_t *config2)
{
  ...
  qed_hs_assert(config1);
  qed_hs_assert(config2);
  qed_hs_assert(config1->clients);
  qed_hs_assert(config2->clients);
                       ...
                       time_t now, hs_desc_intro_point_t *desc_ip)
{
  ...
  if (ip->base.is_only_legacy) {
    ...
    ssize_t cert_len = qed_hs_make_rsa_ed25519_crosscert(
                                    &signing_kp->pubkey,
                                    desc_ip->legacy.key,
  ...
  /* Encryption key and its cross certificate. */
  {
    ...
    ed25519_public_key_from_curve25519_public_key(&ed25519_pubkey,
                                                  ...
                                                  0);
    desc_ip->enc_key_cert = qed_hs_cert_create_ed25519(signing_kp,
...
build_desc_signing_key_cert(hs_service_descripqed_hs_t *desc, time_t now)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(desc->desc);

  /* Ease our life a bit. */
  plaintext = &desc->desc->plaintext_data;

  /* Get rid of what we have right now. */
  qed_hs_cert_free(plaintext->signing_key_cert);

  /* Fresh certificate for the signing key. */
  plaintext->signing_key_cert =
    qed_hs_cert_create_ed25519(&desc->blinded_kp, CERT_TYPE_SIGNING_HS_DESC,
   ...
   * where the call came from and the qed_hs_cert_create_ed25519() will log the
   * error. */
  qed_hs_assert_nonfatal(plaintext->signing_key_cert);
}
                                  ...
                                  hs_service_descripqed_hs_t *desc)
{
  ...
  if (is_client_auth_enabled(service)) {
    SMARTLIST_FOREACH_BEGIN(config->clients,
      ...
      desc_client = qed_hs_malloc_zero(sizeof(hs_desc_authorized_client_t));
                             ...
                             hs_service_descripqed_hs_t *desc)
{
  ...
  qed_hs_assert(service);
  qed_hs_assert(desc);
  qed_hs_assert(!fast_mem_is_zero((char *) &desc->blinded_kp,
                              sizeof(desc->blinded_kp)));
  qed_hs_assert(!fast_mem_is_zero((char *) &desc->signing_kp,
                        ...
                        hs_service_descripqed_hs_t *desc)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(!fast_mem_is_zero((char *) &service->keys.identity_pk,
             ED25519_PUBKEY_LEN));
  ...
  qed_hs_assert_nonfatal(!desc->ope_cipher);
...
build_descriptors_for_new_service(hs_service_t *service, time_t now)
{
  ...
  qed_hs_assert(!service->desc_current);
  qed_hs_assert(!service->desc_next);
  ...
  FOR_EACH_SERVICE_BEGIN(service) {
    ...
    FOR_EACH_DESCRIPQED_HS_BEGIN(service, desc) {
      if (!service->config.has_pow_defenses_enabled &&
        ...
        qed_hs_free(desc->desc->encrypted_data.pow_params);
    ...
    FOR_EACH_DESCRIPQED_HS_BEGIN(service, desc) {
      ...
      if (!encrypted->pow_params) {
        ...
        encrypted->pow_params = qed_hs_malloc_zero(sizeof(hs_pow_desc_params_t));
      }
...
get_max_intro_circ_per_period(const hs_service_t *service)
{
  ...
  qed_hs_assert(service);
  qed_hs_assert(service->config.num_intro_points <=
                           ...
                           hs_service_descripqed_hs_t *desc, const node_t *hsdir)
{
  ...
  /* Logging so we know where it was sent. */
  {
    ...
    char *blinded_pubkey_log_str =
      qed_hs_strdup(hex_str((char*)&desc->blinded_kp.pubkey.pubkey, 32));
    ...
    qed_hs_free(blinded_pubkey_log_str);
                                ...
                                bool is_current)
{
  ...
  qed_hs_assert_nonfatal(now >= srv_start);
  ...
  qed_hs_assert(hs_desc->ope_cipher);
  ...
  qed_hs_assert_nonfatal(rev_counter < CRYPTO_OPE_ERROR);
                              ...
                              const hs_service_descripqed_hs_t *desc, time_t now)
{
  ...
  if (desc->missing_intro_points && num_intro_points == 0) {
    msg = qed_hs_strdup("Missing intro points");
  ...
  if (count_ip_established != num_intro_points) {
    qed_hs_asprintf(&msg, "Intro circuits aren't yet all established (%d/%d).",
  ...
  if (desc->next_upload_time > now) {
    qed_hs_asprintf(&msg, "Next upload time is %ld, it is now %ld.",
  ...
  if (!networkstatus_get_reasonably_live_consensus(now,
                                            usable_consensus_flavor())) {
    msg = qed_hs_strdup("No reasonably live consensus");
    log_cant_upload_desc(service, desc, msg,
  ...
  if (!router_have_minimum_dir_info()) {
    msg = qed_hs_strdup("Not enough directory information");
    log_cant_upload_desc(service, desc, msg,
...
service_rendezvous_circ_has_opened(origin_circuit_t *circ)
{
  ...
  qed_hs_assert(circ);
  qed_hs_assert(circ->cpath);
  /* Getting here means this is a v3 rendezvous circuit. */
  qed_hs_assert(circ->hs_ident);
  qed_hs_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_S_CONNECT_REND);
                                 ...
                                 size_t payload_len)
{
  ...
  qed_hs_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_S_ESTABLISH_INTRO);
                          ...
                          size_t payload_len)
{
  ...
  qed_hs_assert(circ);
  qed_hs_assert(payload);
  qed_hs_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_S_INTRO);
...
service_add_fnames_to_list(const hs_service_t *service, smartlist_t *list)
{
  ...
  qed_hs_snprintf(fname, sizeof(fname), "%s_secret_key", fname_keyfile_prefix);
  smartlist_add(list, hs_path_from_filename(s_dir, fname));
  qed_hs_snprintf(fname, sizeof(fname), "%s_public_key", fname_keyfile_prefix);
  smartlist_add(list, hs_path_from_filename(s_dir, fname));
}
...
{
  qed_hs_assert(hs_service_non_anonymous_mode_consistent(options));
...
{
  qed_hs_assert(circ);
  qed_hs_assert(CIRCUIT_IS_ORIGIN(circ));
                              ...
                              const routerstatus_t *hsdir_rs)
{
  ...
  qed_hs_assert(blinded_pk);
  qed_hs_assert(hsdir_rs);
   ...
   * with the version number: "/qed-hs/hs/<version>/publish". */
  qed_hs_snprintf(version_str, sizeof(version_str), "%u", version);
                         ...
                         smartlist_t *auth_clients_v3, char **address_out)
{
  ...
  *address_out = qed_hs_strdup(service->onion_address);
  ret = RSAE_OKAY;
  goto end;
                              ...
                              edge_connection_t *conn)
{
  ...
  qed_hs_assert(circ);
  qed_hs_assert(conn);
  qed_hs_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_S_REND_JOINED);
  qed_hs_assert(circ->hs_ident);
...
{
  qed_hs_assert(file_list);
  qed_hs_assert(dir_list);
...
hs_service_dump_stats(int severity)
{
  ...
  FOR_EACH_SERVICE_BEGIN(hs) {
    ...
    qed_hs_log(severity, LD_GENERAL, "Service configured in %s:",
            service_escaped_dir(hs));
    FOR_EACH_DESCRIPQED_HS_BEGIN(hs, desc) {
      ...
      DIGEST256MAP_FOREACH(desc->intro_points.map, key,
        ...
        if (!intro_node) {
          qed_hs_log(severity, LD_GENERAL, "  Couldn't find intro point, "
                  "skipping");
          continue;
        }
        ...
        if (!circ) {
          qed_hs_log(severity, LD_GENERAL, "  Intro point at %s: no circuit",
                  nickname);
          continue;
        }
        qed_hs_log(severity, LD_GENERAL, "  Intro point %s: circuit is %s",
...
{
  hs_service_t *service = qed_hs_malloc_zero(sizeof(hs_service_t));
...
hs_service_free_(hs_service_t *service)
{
  ...
  if (service->state.ob_subcreds) {
    qed_hs_free(service->state.ob_subcreds);
  }
...
hs_service_init(void)
{
  ...
  qed_hs_assert(!hs_service_map);
  qed_hs_assert(!hs_service_staging_list);

  hs_service_map = qed_hs_malloc_zero(sizeof(struct hs_service_ht));
