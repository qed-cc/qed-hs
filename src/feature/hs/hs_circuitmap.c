                            ...
                            const circuit_t *second_circuit)
{
  ...
  qed_hs_assert(first_circuit);
  qed_hs_assert(second_circuit);
  ...
  return qed_hs_memeq(first_token->token,
                   second_token->token,
...
{
  qed_hs_assert(circuit->hs_token);
             ...
             const uint8_t *token)
{
  ...
  hs_token_t *hs_token = qed_hs_malloc_zero(sizeof(hs_token_t));
  hs_token->type = type;
  hs_token->token_len = token_len;
  hs_token->token = qed_hs_memdup(token, token_len);
...
hs_token_free_(hs_token_t *hs_token)
{
  ...
  qed_hs_free(hs_token->token);
  qed_hs_free(hs_token);
}
...
{
  qed_hs_assert(the_hs_circuitmap);
                                 ...
                                 uint8_t wanted_circ_purpose)
{
  circuit_t *circ;
  qed_hs_assert(token);
  qed_hs_assert(CIRCUIT_PURPOSE_IS_ORIGIN(wanted_circ_purpose));
                                             ...
                                             const uint8_t *cookie)
{
  ...
  { /* Basic circ purpose sanity checking */
    qed_hs_assert_nonfatal(circ->purpose == CIRCUIT_PURPOSE_C_ESTABLISH_REND);
  }
...
hs_circuitmap_init(void)
{
  ...
  the_hs_circuitmap = qed_hs_malloc_zero(sizeof(struct hs_circuitmap_ht));
