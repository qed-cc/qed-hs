                               ...
                               const routerstatus_t *hsdir)
{
  ...
  qed_hs_assert(hsdir);
  qed_hs_assert(onion_identity_pk);
                               ...
                               const hs_descriptor_t *desc)
{
  ...
  qed_hs_assert(ident);
  qed_hs_assert(desc);
  qed_hs_assert_nonfatal(!ed25519_public_key_is_zero(&ident->intro_auth_pk));
...
{
  qed_hs_assert(circ);
  qed_hs_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_C_INTRODUCING);
...
{
  qed_hs_assert(circ);
  qed_hs_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_C_ESTABLISH_REND);
                     ...
                     size_t payload_len)
{
  ...
  qed_hs_assert(circ->build_state->chosen_exit);
  assert_circ_anonymity_ok(circ, get_options());
                               ...
                               const char *dir)
{
  ...
  qed_hs_asprintf(&fname, "%s.auth_private", onion_address);
                              ...
                              const hs_client_service_authorization_t *creds)
{
  ...
  qed_hs_assert(creds->flags & CLIENT_AUTH_FLAG_IS_PERMANENT);
  ...
  qed_hs_asprintf(&file_contents, "%s:descriptor:x25519:%s",
 ...
 err:
  qed_hs_free(file_contents);
  qed_hs_free(full_fname);
...
remove_client_auth_creds_file(const char *filename)
{
  ...
  creds_file_path = hs_path_from_filename(options->ClientOnionAuthDir,
                                          filename);
  if (qed_hs_unlink(creds_file_path) != 0) {
                                 ...
                                 const hs_client_service_authorization_t *cred)
{
  ...
  qed_hs_assert(cred->flags & CLIENT_AUTH_FLAG_IS_PERMANENT);
  ...
  file_list = qed_hs_listdir(options->ClientOnionAuthDir);
  if (file_list == NULL) {
...
hs_client_circuit_cleanup_on_free(const circuit_t *circ)
{
  ...
  qed_hs_assert(orig_circ->hs_ident);
  ...
  case CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT:
    log_info(LD_REND, "Failed v3 intro circ for service %s to intro point %s "
        ...
        failure);
    qed_hs_assert_nonfatal(!ed25519_public_key_is_zero(intro_pk));
                            ...
                            hs_descriptor_t **desc)
{
  ...
  qed_hs_cert_t *cert = (*desc)->plaintext_data.signing_key_cert;
  if (qed_hs_cert_checksig(cert,
...
parse_auth_file_content(const char *client_key_str)
{
  ...
  auth = qed_hs_malloc_zero(sizeof(hs_client_service_authorization_t));
