                           ...
                           const hs_service_t *service)
{
  ...
  qed_hs_assert(service);
  qed_hs_assert(service->config.version <= HS_VERSION_MAX);
                       ...
                       hs_service_t *service)
{
  ...
  qed_hs_assert(hs_opts->HiddenServiceDir);
  config->directory_path = qed_hs_strdup(hs_opts->HiddenServiceDir);
  ...
  if (hs_opts->HiddenServiceVersion == -1) {
    ...
    log_warn(LD_CONFIG, "Onion services version 2 are obsolete. Please see "
                        "https://blog.qed-cc.org/v2-deprecation-timeline "
               ...
               smartlist_t *service_list)
{
  ...
  qed_hs_assert_nonfatal(msg == NULL);
