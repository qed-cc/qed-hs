                      ...
                      uint8_t *mac_out, size_t mac_out_len)
{
  ...
  qed_hs_assert(encoded_cell);
  qed_hs_assert(encrypted);
  qed_hs_assert(mac_key);
  qed_hs_assert(mac_out);
  qed_hs_assert(mac_out_len >= DIGEST256_LEN);
  ...
  qed_hs_assert(mac_msg_len <= sizeof(mac_msg));
  ...
  qed_hs_assert(offset == mac_msg_len);
                            ...
                            curve25519_public_key_t *client_pk)
{
  ...
  qed_hs_assert(n_subcredentials > 0);
                   ...
                   size_t encrypted_section_len)
{
  ...
  qed_hs_assert(cipher);

  /* This is symmetric encryption so can't be bigger than the encrypted
   * section length. */
  decrypted = qed_hs_malloc_zero(encrypted_section_len);
...
{
  qed_hs_assert(cell);
  qed_hs_assert(lspecs);
  qed_hs_assert(smartlist_len(lspecs) > 0);
  qed_hs_assert(smartlist_len(lspecs) <= UINT8_MAX);
                              ...
                              const hs_cell_introduce1_data_t *data)
{
  ...
  qed_hs_assert(encoded_cell_len > 0);
  ...
  encrypted_len = sizeof(data->client_kp->pubkey) + encoded_enc_cell_len +
                  sizeof(mac);
  qed_hs_assert(encrypted_len < RELAY_PAYLOAD_SIZE_MAX);
  ...
  offset += sizeof(mac);
  qed_hs_assert(offset == (size_t) encrypted_len);
                              ...
                              trn_extension_t *extensions)
{
  ...
  qed_hs_assert(pow_solution);
  qed_hs_assert(extensions);
  ...
  qed_hs_assert(ret == (ssize_t)pow_ext_encoded_len);
                                ...
                                uint8_t param_type, uint64_t param_value)
{
  ...
  qed_hs_assert(param_type == TRUNNEL_DOS_PARAM_TYPE_INTRO2_RATE_PER_SEC ||
                                    ...
                                    trn_extension_t *extensions)
{
  ...
  qed_hs_assert(ret == (ssize_t) dos_ext_encoded_len);
                              ...
                              uint8_t *cell_out)
{
  ...
  /* Calculate HANDSHAKE_AUTH field (MAC). */
  {
    ...
    /* Sanity check. */
    qed_hs_assert(tmp_cell_enc_len > tmp_cell_mac_offset);
                                   ...
                                   size_t encrypted_section_len)
{
  ...
  intro_keys_result = qed_hs_malloc_zero(sizeof(*intro_keys_result));
  for (unsigned i = 0; i < data->n_subcredentials; ++i) {
    ...
    bool equal = qed_hs_memeq(mac, encrypted_section + mac_offset, sizeof(mac));
  ...
  if (safe_mem_is_zero(intro_keys_result, sizeof(*intro_keys_result))) {
    ...
    qed_hs_free(intro_keys_result); /* sets intro_keys_result to NULL */
  }
                          ...
                          uint8_t *cell_out)
{
  ...
  qed_hs_assert(rendezvous_cookie);
  qed_hs_assert(rendezvous_handshake_info);
  qed_hs_assert(cell_out);
                          ...
                          uint8_t *handshake_info, size_t handshake_info_len)
{
   ...
   * our code flow. */
  qed_hs_assert(trn_cell_rendezvous2_getlen_handshake_info(cell) ==
             handshake_info_len);
