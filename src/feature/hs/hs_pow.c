/* Copyright (c) 2017-2020, The QED Project, Inc. */
...
{
  size_t offset = 0;
  uint8_t *challenge = qed_hs_malloc_zero(HS_POW_CHALLENGE_LEN);

  CTASSERT(HS_POW_ID_LEN == sizeof *blinded_id);
  qed_hs_assert_nonfatal(!ed25519_public_key_is_zero(blinded_id));
  ...
  qed_hs_assert(HS_POW_NONCE_OFFSET == offset);
  memcpy(challenge + offset, nonce, HS_POW_NONCE_LEN);
  offset += HS_POW_NONCE_LEN;
  set_uint32(challenge + offset, qed_hs_htonl(effort));
  offset += HS_POW_EFFORT_LEN;
  qed_hs_assert(HS_POW_CHALLENGE_LEN == offset);
                         ...
                         const uint32_t effort)
{
  ...
  uint64_t RE = qed_hs_htonl(get_uint32(hash_result)) * (uint64_t) effort;
...
hs_pow_equix_option_flags(int CompiledProofOfWorkHash)
{
  if (CompiledProofOfWorkHash == 0) {
    ...
    qed_hs_assert_nonfatal(CompiledProofOfWorkHash == -1);
             ...
             hs_pow_solution_t *pow_solution_out)
{
  ...
  qed_hs_assert(pow_inputs);
  qed_hs_assert(pow_solution_out);
              ...
              const hs_pow_solution_t *pow_solution)
{
  ...
  qed_hs_assert(pow_state);
  qed_hs_assert(pow_solution);
  qed_hs_assert(service_blinded_id);
  qed_hs_assert_nonfatal(!ed25519_public_key_is_zero(service_blinded_id));
  ...
  entry = qed_hs_malloc_zero(sizeof(nonce_cache_entry_t));
...
hs_pow_free_service_state(hs_pow_service_state_t *state)
{
  ...
  qed_hs_assert(smartlist_len(state->rend_request_pqueue) == 0);
...
pow_worker_threadfn(void *state_, void *work_)
{
  ...
  job->pow_solution_out = qed_hs_malloc_zero(sizeof(hs_pow_solution_t));

  if (hs_pow_solve(&job->pow_inputs, job->pow_solution_out)) {
    qed_hs_free(job->pow_solution_out);
...
{
  qed_hs_assert(in_main_thread());
  qed_hs_assert(rend_circ_cookie);
  qed_hs_assert(pow_inputs);
  qed_hs_assert_nonfatal(
    !ed25519_public_key_is_zero(&pow_inputs->service_blinded_id));

  pow_worker_job_t *job = qed_hs_malloc_zero(sizeof(*job));
