...
#include "feature/nodelist/torcert.h" /* qed_hs_cert_encode_ed22519() */
          ...
          uint8_t *mac_out, size_t mac_len)
{
  ...
  const uint64_t mac_len_netorder = qed_hs_htonll(mac_key_len);
  const uint64_t salt_len_netorder = qed_hs_htonll(salt_len);

  qed_hs_assert(mac_key);
  qed_hs_assert(salt);
  qed_hs_assert(encrypted);
  qed_hs_assert(mac_out);
                   ...
                   uint8_t **secret_input_out)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(secret_data);
  qed_hs_assert(secret_input_out);

  secret_input = qed_hs_malloc_zero(secret_input_len);
  ...
  set_uint64(secret_input + offset,
             qed_hs_htonll(desc->plaintext_data.revision_counter));
  offset += sizeof(uint64_t);
  qed_hs_assert(secret_input_len == offset);
                        ...
                        int is_superencrypted_layer)
{
  ...
  qed_hs_assert((offset + mac_len) == sizeof(kdf_key));
...
encode_link_specifiers(const smartlist_t *specs)
{
  ...
  qed_hs_assert(smartlist_len(specs) > 0);
  qed_hs_assert(smartlist_len(specs) <= UINT8_MAX);
...
encode_enc_key(const hs_desc_intro_point_t *ip)
{
  ...
  if (qed_hs_cert_encode_ed22519(ip->enc_key_cert, &encoded_cert) < 0) {
    goto done;
  }
  qed_hs_asprintf(&encoded,
               "%s ntor %s\n"
...
encode_onion_key(const hs_desc_intro_point_t *ip)
{
  ...
  qed_hs_asprintf(&encoded, "%s ntor %s", str_ip_onion_key, key_b64);
                   ...
                   const hs_desc_intro_point_t *ip)
{
  ...
  /* Onion key encoding. */
  {
    ...
    qed_hs_free(encoded_onion_key);
  }

  /* Authentication key encoding. */
  {
    char *encoded_cert;
    if (qed_hs_cert_encode_ed22519(ip->auth_key_cert, &encoded_cert) < 0) {
      goto err;
    }
 ...
 err:
  SMARTLIST_FOREACH(lines, char *, l, qed_hs_free(l));
...
compute_padded_plaintext_length(size_t plaintext_len)
{
  ...
  qed_hs_assert(plaintext_len <= (SIZE_T_CEILING - padding_block_length));
  ...
  qed_hs_assert(!(plaintext_padded_len % padding_block_length));
                        ...
                        uint8_t **padded_out)
{
  ...
  qed_hs_assert(plaintext);
  qed_hs_assert(padded_out);

  /* Allocate the final length including padding. */
  padded_len = compute_padded_plaintext_length(plaintext_len);
  qed_hs_assert(padded_len >= plaintext_len);
  padded = qed_hs_malloc_zero(padded_len);
                ...
                int is_superencrypted_layer)
{
  ...
  if (is_superencrypted_layer) {
    ...
    qed_hs_assert(!(encrypted_len % HS_DESC_SUPERENC_PLAINTEXT_PAD_MULTIPLE));
  } else { /* No padding required for inner layers */
    padded_plaintext = qed_hs_memdup(plaintext, plaintext_len);
  ...
  encrypted = qed_hs_malloc_zero(encrypted_len);
  /* This can't fail. */
                        ...
                        char **encrypted_out, int is_superencrypted_layer)
{
  ...
  final_blob = qed_hs_malloc_zero(final_blob_len);
  ...
  qed_hs_free(encrypted);
  /* Extra precaution. */
  qed_hs_assert(offset == final_blob_len);
...
#define ASSERT_AND_BASE64(field) STMT_BEGIN                        \
  qed_hs_assert(!fast_mem_is_zero((char *) client->field,              \
                              sizeof(client->field)));             \
  ...
  qed_hs_asprintf(&auth_client_str, "%s %s %s %s", str_desc_auth_client,
...
get_all_auth_client_lines(const hs_descripqed_hs_t *desc)
{
  ...
  qed_hs_assert(smartlist_len(desc->superencrypted_data.clients) != 0);
  qed_hs_assert(smartlist_len(desc->superencrypted_data.clients)
  ...
  auth_client_lines_str = smartlist_join_strings(auth_client_lines,
                                                 "\n", 1, NULL);
  /* Cleanup the mess */
  SMARTLIST_FOREACH(auth_client_lines, char *, a, qed_hs_free(a));
...
get_inner_encrypted_layer_plaintext(const hs_descripqed_hs_t *desc)
{
  ...
  /* Build the start of the section prior to the introduction points. */
  {
    ...
    if (desc->encrypted_data.test_extra_plaintext) {
      smartlist_add(lines,
                    qed_hs_strdup(desc->encrypted_data.test_extra_plaintext));
    }
#endif
    ...
    if (desc->encrypted_data.pow_params) {
      ...
      char *seed_b64 = qed_hs_malloc_zero(seed_b64_len);
      ...
      qed_hs_assert((size_t) ret == (seed_b64_len - 1));
                                    ...
                                    const char *layer2_b64_ciphertext)
{
  ...
  {  /* Print ephemeral x25519 key */
    ...
    qed_hs_assert(!fast_mem_is_zero((char *) ephemeral_pubkey->public_key,
  ...
  {  /* Create auth-client lines. */
    ...
    qed_hs_assert(auth_client_lines);
  ...
  SMARTLIST_FOREACH(lines, char *, a, qed_hs_free(a));
  smartlist_free(lines);
                  ...
                  uint8_t **secret_data_out)
{
  ...
  if (descripqed_hs_cookie) {
    ...
    secret_data = qed_hs_malloc(secret_data_len);
                           ...
                           char **encrypted_blob_out)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(encrypted_blob_out);
 ...
 err:
  ...
  qed_hs_free(secret_data);
  qed_hs_free(layer1_str);
  qed_hs_free(layer2_str);
  qed_hs_free(layer2_b64_ciphertext);
               ...
               char **encoded_out)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(signing_kp);
  qed_hs_assert(encoded_out);
  qed_hs_assert(desc->plaintext_data.version == 3);
  ...
  /* Build the non-encrypted values. */
  {
    ...
    if (qed_hs_cert_encode_ed22519(desc->plaintext_data.signing_key_cert,
                                &encoded_cert) < 0) {
                   ...
                   hs_desc_authorized_client_t *client)
{
  ...
  qed_hs_assert(tok);
  qed_hs_assert(tok->n_args >= 3);
  qed_hs_assert(client);
...
decode_link_specifiers(const char *encoded)
{
  ...
  decoded = qed_hs_malloc(encoded_len);
...
decode_create2_list(hs_desc_encrypted_data_t *desc, const char *list)
{
  ...
  SMARTLIST_FOREACH_BEGIN(tokens, char *, s) {
    int ok;
    unsigned long type = qed_hs_parse_ulong(s, 10, 1, UINT16_MAX, &ok, NULL);
    if (!ok) {
  ...
  SMARTLIST_FOREACH(tokens, char *, s, qed_hs_free(s));
...
STATIC int
cert_is_valid(qed_hs_cert_t *cert, uint8_t type, const char *log_obj_type)
{
  qed_hs_assert(log_obj_type);
  ...
  if (qed_hs_cert_checksig(cert, &cert->signing_key, approx_time()) < 0) {
    if (cert->cert_expired) {
      ...
      log_fn(LOG_PROTOCOL_WARN, LD_REND, "Invalid signature for %s: %s (%s)",
             log_obj_type, qed_hs_cert_describe_signature_status(cert),
             expiration_str);
    } else {
...
static int
cert_parse_and_validate(qed_hs_cert_t **cert_out, const char *data,
                        size_t data_len, unsigned int cert_type_wanted,
                        const char *err_msg)
{
  qed_hs_cert_t *cert;

  qed_hs_assert(cert_out);
  qed_hs_assert(data);
  qed_hs_assert(err_msg);

  /* Parse certificate. */
  cert = qed_hs_cert_parse((const uint8_t *) data, data_len);
  if (!cert) {
                             ...
                             uint8_t **keys_out)
{
  ...
  qed_hs_assert(sk);
  qed_hs_assert(pk);
  qed_hs_assert(keys_out);

  keystream = qed_hs_malloc_zero(keystream_len);
                          ...
                          uint8_t **descripqed_hs_cookie_out)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(client);
  qed_hs_assert(client_auth_sk);
  qed_hs_assert(!fast_mem_is_zero(
        (char *) &desc->superencrypted_data.auth_ephemeral_pubkey,
        sizeof(desc->superencrypted_data.auth_ephemeral_pubkey)));
  qed_hs_assert(!fast_mem_is_zero((char *) desc->subcredential.subcred,
                               DIGEST256_LEN));
  ...
  if (qed_hs_memneq(client->client_id, keystream, HS_DESC_CLIENT_ID_LEN)) {
    goto done;
  }
                    ...
                    char **decrypted_out))
{
  ...
  qed_hs_assert(decrypted_out);
  qed_hs_assert(desc);
  qed_hs_assert(encrypted_blob);
  ...
  qed_hs_assert(encrypted_len > 0); /* guaranteed by the check above */
  ...
  if (!qed_hs_memeq(our_mac, desc_mac, sizeof(our_mac))) {
...

  {
    ...
    decrypted = qed_hs_malloc_zero(encrypted_len + 1);
...
desc_decrypt_superencrypted,(const hs_descripqed_hs_t *desc,char **decrypted_out))
{
  ...
  qed_hs_assert(superencrypted_plaintext);
                        ...
                        char **decrypted_out))
{
  ...
  qed_hs_assert(encrypted_plaintext);
                        ...
                        const hs_descripqed_hs_t *desc)
{
  ...
  ip->legacy.cert.encoded = qed_hs_memdup(tok->object_body, tok->object_size);
...
decode_introduction_point(const hs_descripqed_hs_t *desc, const char *start)
{
  ...
  if (qed_hs_cert_checksig(ip->auth_key_cert,
                        &desc->plaintext_data.signing_pubkey, 0) < 0) {
    log_warn(LD_REND, "Invalid authentication key signature: %s",
             qed_hs_cert_describe_signature_status(ip->auth_key_cert));
    goto err;
  }
  ...
  if (qed_hs_cert_checksig(ip->enc_key_cert,
                        &desc->plaintext_data.signing_pubkey, 0) < 0) {
    log_warn(LD_REND, "Invalid encryption key signature: %s",
             qed_hs_cert_describe_signature_status(ip->enc_key_cert));
    goto err;
  }
                    ...
                    const char *data)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(desc_enc);
  qed_hs_assert(data);
  qed_hs_assert(desc_enc->intro_points);
  ...
  SMARTLIST_FOREACH(chunked_desc, char *, a, qed_hs_free(a));
  smartlist_free(chunked_desc);
  SMARTLIST_FOREACH(intro_points, char *, a, qed_hs_free(a));
                  ...
                  const char *encoded_desc, size_t encoded_len)
{
  ...
  qed_hs_assert(b64_sig);
  qed_hs_assert(signing_pubkey);
  qed_hs_assert(encoded_desc);
  ...
  sig_start = qed_hs_memstr(encoded_desc, encoded_len, "\n" str_signature " ");
                  ...
                  hs_pow_desc_params_t **pow_params_out)
{
  ...
  *pow_params_out = NULL;
  ...
  SMARTLIST_FOREACH_BEGIN(toks, const directory_token_t *, tok) {
    ...
    hs_pow_desc_params_t *pow_params = qed_hs_malloc_zero(sizeof(*pow_params));
    *pow_params_out = pow_params;
    ...
    unsigned long effort =
      qed_hs_parse_ulong(tok->args[2], 10, 0, UINT32_MAX, &ok, NULL);
    if (!ok) {
  ...
  if (ret < 0 && *pow_params_out) {
    qed_hs_free(*pow_params_out); // sets it to NULL
  }
                         ...
                         const char *encoded_desc, size_t encoded_len)
{
  ...
  qed_hs_assert(desc->version >= 3);

  /* Descriptor lifetime parsing. */
  tok = find_by_keyword(tokens, R3_DESC_LIFETIME);
  qed_hs_assert(tok->n_args == 1);
  desc->lifetime_sec = (uint32_t) qed_hs_parse_ulong(tok->args[0], 10, 0,
                                                  UINT32_MAX, &ok, NULL);
  if (!ok) {
  ...
  qed_hs_assert(tok->n_args == 1);
  desc->revision_counter = qed_hs_parse_uint64(tok->args[0], 10, 0,
                                            UINT64_MAX, &ok, NULL);
  if (!ok) {
   ...
   * latter if needed. */
  desc->superencrypted_blob = qed_hs_memdup(tok->object_body, tok->object_size);
                              ...
                              desc_superencrypted_out)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(desc_superencrypted_out);
  ...
  SMARTLIST_FOREACH_BEGIN(tokens, const directory_token_t *, token) {
    if (token->tp == R3_DESC_AUTH_CLIENT) {
      qed_hs_assert(token->n_args >= 3);

      hs_desc_authorized_client_t *client =
        qed_hs_malloc_zero(sizeof(hs_desc_authorized_client_t));
   ...
   * latter if needed. */
  qed_hs_assert(tok->object_size <= INT_MAX);
  superencrypted->encrypted_blob = qed_hs_memdup(tok->object_body,
                                              tok->object_size);
 ...
 err:
  qed_hs_assert(ret < HS_DESC_DECODE_OK);
                         ...
                         hs_desc_encrypted_data_t *desc_encrypted_out)
{
  ...
  qed_hs_assert(desc);
  qed_hs_assert(desc_encrypted_out);
  ...
  if (tok) {
    ...
    qed_hs_asprintf(&desc_encrypted_out->flow_control_pv, "FlowCtrl=%s",
                 tok->args[0]);
    uint8_t sendme_inc =
      (uint8_t) qed_hs_parse_uint64(tok->args[1], 10, 0, UINT8_MAX, &ok, NULL);
                         ...
                         hs_desc_encrypted_data_t *desc_encrypted)
{
  ...
  qed_hs_assert(desc->superencrypted_data.encrypted_blob);
  ...
  qed_hs_assert(ARRAY_LENGTH(decode_encrypted_handlers) >= version);
  qed_hs_assert(decode_encrypted_handlers[version]);
                              ...
                              desc_superencrypted)
{
  ...
  /* Ease our life a bit. */
  version = desc->plaintext_data.version;
  qed_hs_assert(desc_superencrypted);
  ...
  qed_hs_assert(desc->plaintext_data.superencrypted_blob);
  /* Let's make sure we have a supported version as well. By correctly parsing
   * the plaintext, this should not fail. */
  if (BUG(!hs_desc_is_supported_version(version))) {
    goto err;
  ...
  /* Extra precaution. Having no handler for the supported version should
   * never happened else we forgot to add it but we bumped the version. */
  qed_hs_assert(ARRAY_LENGTH(decode_superencrypted_handlers) >= version);
  qed_hs_assert(decode_superencrypted_handlers[version]);
                         ...
                         hs_desc_plaintext_data_t *plaintext)
{
  ...
  qed_hs_assert(tok->n_args == 1);
  plaintext->version = (uint32_t) qed_hs_parse_ulong(tok->args[0], 10, 0,
                                                  UINT32_MAX, &ok, NULL);
  if (!ok) {
  ...
  /* Extra precaution. Having no handler for the supported version should
   * never happened else we forgot to add it but we bumped the version. */
  qed_hs_assert(ARRAY_LENGTH(decode_plaintext_handlers) >= plaintext->version);
  qed_hs_assert(decode_plaintext_handlers[plaintext->version]);
                           ...
                           char **encoded_out))
{
  ...
  /* Extra precaution. Having no handler for the supported version should
   * never happened else we forgot to add it but we bumped the version. */
  qed_hs_assert(ARRAY_LENGTH(encode_handlers) >= version);
...
hs_desc_plaintext_data_free_contents(hs_desc_plaintext_data_t *desc)
{
  ...
  if (desc->superencrypted_blob) {
    qed_hs_free(desc->superencrypted_blob);
  }
  qed_hs_cert_free(desc->signing_key_cert);
...
hs_desc_superencrypted_data_free_contents(hs_desc_superencrypted_data_t *desc)
{
  ...
  if (desc->encrypted_blob) {
    qed_hs_free(desc->encrypted_blob);
  }
  if (desc->clients) {
...
hs_desc_encrypted_data_free_contents(hs_desc_encrypted_data_t *desc)
{
  ...
  if (desc->intro_auth_types) {
    SMARTLIST_FOREACH(desc->intro_auth_types, char *, a, qed_hs_free(a));
  ...
  qed_hs_free(desc->flow_control_pv);
  qed_hs_free(desc->pow_params);
  memwipe(desc, 0, sizeof(*desc));
}
...
{
  hs_desc_intro_point_t *ip = qed_hs_malloc_zero(sizeof(*ip));
...
hs_desc_intro_point_free_(hs_desc_intro_point_t *ip)
{
  ...
  qed_hs_cert_free(ip->auth_key_cert);
  qed_hs_cert_free(ip->enc_key_cert);
  crypto_pk_free(ip->legacy.key);
  qed_hs_free(ip->legacy.cert.encoded);
  qed_hs_free(ip);
}
...
hs_desc_build_fake_authorized_client(void)
{
  hs_desc_authorized_client_t *client_auth =
    qed_hs_malloc_zero(sizeof(*client_auth));
