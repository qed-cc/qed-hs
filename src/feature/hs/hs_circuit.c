                  ...
                  int is_service_side)
{
  ...
  /* Setup the cpath */
  cpath = qed_hs_malloc_zero(sizeof(crypt_path_t));
                                       ...
                                       const hs_service_descriptor_t *desc)
{
  ...
  DIGEST256MAP_FOREACH(desc->intro_points.map, key,
    ...
    qed_hs_assert(circ->purpose == CIRCUIT_PURPOSE_S_ESTABLISH_INTRO ||
               circ->purpose == CIRCUIT_PURPOSE_S_INTRO);
    /* Having a circuit not for the requested service is really bad. */
    qed_hs_assert(ed25519_pubkey_eq(&service->keys.identity_pk,
                             ...
                             const hs_ntor_rend_cell_keys_t *keys)
{
  ...
  memcpy(handshake_info + CURVE25519_PUBKEY_LEN, keys->rend_cell_auth_mac,
         DIGEST256_LEN);
  qed_hs_assert(sizeof(ident->rendezvous_handshake_info) ==
                                 ...
                                 time_t now))
{
  ...
  qed_hs_assert(service);
  qed_hs_assert(ip_auth_pubkey);
  qed_hs_assert(ip_enc_key_kp);
  qed_hs_assert(rdv_data);
                      ...
                      hs_cell_introduce1_data_t *intro1_data)
{
  ...
  qed_hs_assert(ip);
  qed_hs_assert(rp_node);
  qed_hs_assert(subcredential);
  qed_hs_assert(intro1_data);
...
{
  qed_hs_assert(pow_state->rend_request_pqueue);
  qed_hs_assert(smartlist_len(pow_state->rend_request_pqueue));
                     ...
                     hs_cell_introduce2_data_t *data, time_t now)
{
  ...
  req = qed_hs_malloc_zero(sizeof(pending_rend_t));
                           ...
                           bool direct_conn)
{
   ...
   * direct connection. */
  qed_hs_assert_nonfatal(ip->circuit_retries > 0);
                                 ...
                                 origin_circuit_t *circ)
{
  ...
  if (num_intro_circ > num_needed_circ) {
    ...
    log_info(LD_CIRC | LD_REND, "Introduction circuit just opened but we "
             ...
             safe_str_client(service->onion_address));
    qed_hs_assert(circ->build_state->is_internal);
...
hs_circ_is_rend_sent_in_intro1(const origin_circuit_t *circ)
{
  ...
  qed_hs_assert(TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_C_REND_READY);
