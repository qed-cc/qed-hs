...
static bool
metrics_request_allowed(const qed_hs_addr_t *peer_addr)
{
  qed_hs_assert(peer_addr);
...
write_metrics_http_response(const size_t data_len, connection_t *conn)
{
  ...
  buf_add_printf(buf, "Content-Length: %" QED_HS_PRIuSZ "\r\n", data_len);
  buf_add_string(buf, "\r\n");
...
metrics_get_output(const metrics_format_t fmt)
{
  ...
  for (unsigned i = 0; i < n_qed_hs_subsystems; ++i) {
    const smartlist_t *stores;
    const subsys_fns_t *sys = qed_hs_subsystems[i];
...
metrics_connection_process_inbuf(connection_t *conn)
{
  ...
  qed_hs_assert(conn);
  qed_hs_assert(conn->type == CONN_TYPE_METRICS);
                    ...
                    char **err_msg_out)
{
  ...
  if (num_elems < 1) {
    *err_msg_out = qed_hs_strdup("MetricsPort is missing port.");
    goto end;
  }
  ...
  if (num_elems >= 2) {
    ...
    if (!strcasecmp(fmt_str, "prometheus")) {
      ...
      qed_hs_asprintf(err_msg_out, "MetricsPort unknown format: %s", fmt_str);
      goto end;
    }
  }
  ...
  cfg->port = (int) qed_hs_parse_long(addrport_str, 10, 0, 65535, &ok, NULL);
  if (ok) {
    qed_hs_addr_parse(&cfg->addr, "127.0.0.1");
  } else {
    /* We probably have a host:port situation */
    if (qed_hs_addr_port_lookup(addrport_str, &cfg->addr,
                             (uint16_t *) &cfg->port) < 0) {
      *err_msg_out = qed_hs_strdup("MetricsPort address/port failed to parse or "
                                "resolve.");
      goto end;
    }
  }
