...
bw_array_new(void)
{
  bw_array_t *b;
  time_t start;
  b = qed_hs_malloc_zero(sizeof(bw_array_t));
  start = time(NULL);
...
bwhist_fill_bandwidth_history(char *buf, size_t len, const bw_array_t *b)
{
  ...
  for (n=0; n<b->num_maxes_set; ++n,++i) {
    ...
    if (n==(b->num_maxes_set-1))
      qed_hs_snprintf(cp, len-(cp-buf), "%"PRIu64, (total));
    else
      qed_hs_snprintf(cp, len-(cp-buf), "%"PRIu64",", (total));
    cp += strlen(cp);
  }
                                     ...
                                     int *s_interval)
{
  ...
  if (*s_values) {
    SMARTLIST_FOREACH(*s_values, char *, val, qed_hs_free(val));
    smartlist_free(*s_values);
  }
  if (*s_maxima) {
    SMARTLIST_FOREACH(*s_maxima, char *, val, qed_hs_free(val));
                                   ...
                                   const int s_interval)
{
  ...
  if (s_values && s_begins >= now - NUM_SECS_BW_SUM_INTERVAL*NUM_TOTALS) {
    ...
    SMARTLIST_FOREACH_BEGIN(s_values, const char *, cp) {
        ...
        v = qed_hs_parse_uint64(cp, 10, 0, UINT64_MAX, &ok, NULL);
        if (have_maxima) {
          maxstr = smartlist_get(s_maxima, cp_sl_idx);
          mv = qed_hs_parse_uint64(maxstr, 10, 0, UINT64_MAX, &ok_m, NULL);
...
bwhist_load_state(or_state_t *state, char **err)
{
  ...
  qed_hs_assert(read_array && write_array);
  qed_hs_assert(read_array_ipv6 && write_array_ipv6);
  qed_hs_assert(dir_read_array && dir_write_array);
...
#undef LOAD
  if (!all_ok) {
    *err = qed_hs_strdup("Parsing of bandwidth history values failed");
