...
typedef struct or_history_t {
   ...
   * successfully. */
  qed_hs_addr_t last_reached_addr;
...
typedef struct {
  ...
  uint64_t stats_n_error_qed_hs_timeout;   /* 67 */
...
rep_hist_get_n_dns_error(int type, uint8_t error)
{
  ...
  case DNS_ERR_TIMEOUT:
    return dns_stats->stats_n_error_qed_hs_timeout;
 ...
 *
 * See: https://gitlab.qed-cc.org/tpo/core/qed-hs/-/issues/40490 */
void
rep_hist_note_dns_error(int type, uint8_t error)
{
  ...
  case DNS_ERR_UNKNOWN:
    ...
    break;
  case DNS_ERR_TIMEOUT:
    dns_stats->stats_n_error_qed_hs_timeout++;
    break;
  case DNS_ERR_SHUTDOWN:
...
rep_hist_get_overload_general_line(void)
{
  ...
  if (overload_happened_recently(overload_stats.overload_general_time, 72)) {
    ...
    qed_hs_asprintf(&result, "overload-general %d %s\n",
...
get_or_history(const char* id)
{
  ...
  if (!hist) {
    hist = qed_hs_malloc_zero(sizeof(or_history_t));
    rephist_total_alloc += sizeof(or_history_t);
    rephist_total_num++;
    hist->since = hist->changed = time(NULL);
    qed_hs_addr_make_unspec(&hist->last_reached_addr);
...
void
rep_hist_note_router_reachable(const char *id, const qed_hs_addr_t *at_addr,
                               const uint16_t at_port, time_t when)
{
  ...
  qed_hs_assert(hist);
  qed_hs_assert((!at_addr && !at_port) || (at_addr && at_port));

  addr_changed = at_addr && !qed_hs_addr_is_null(&hist->last_reached_addr) &&
    qed_hs_addr_compare(at_addr, &hist->last_reached_addr, CMP_EXACT) != 0;
  ...
  if (at_addr)
    qed_hs_addr_copy(&hist->last_reached_addr, at_addr);
  if (at_port)
...
rep_hist_dump_stats(time_t now, int severity)
{
  ...
  qed_hs_log(severity, LD_HIST, "--------------- Dumping history information:");
...
parse_possibly_bad_iso_time(const char *s, time_t *time_out)
{
  ...
  b[4] = '\0';
  year = (int)qed_hs_parse_long(b, 10, 0, INT_MAX, NULL, NULL);
  if (year < 1970) {
    *time_out = 0;
    ++n_bogus_times;
    return 0;
...
rep_hist_load_mtbf_data(time_t now)
{
...

  {
    ...
    if (smartlist_len(lines)>4) {
      ...
      if (!strcmpstart(firstline, "format "))
        format = qed_hs_parse_long(firstline+strlen("format "),
  ...
  for (; i < smartlist_len(lines); ++i) {
    ...
    if (format == 1) {
      n = qed_hs_sscanf(line, "%40s %ld %lf S=%10s %8s",
      ...
      if (mtbf_idx >= 0) {
        ...
        n = qed_hs_sscanf(mtbfline, "+MTBF %lu %lf S=%10s %8s",
      ...
      if (wfu_idx >= 0) {
        ...
        n = qed_hs_sscanf(wfuline, "+WFU %lu %lu S=%10s %8s",
...
rep_hist_exit_stats_init(time_t now)
{
  ...
  exit_bytes_read = qed_hs_calloc(EXIT_STATS_NUM_PORTS, sizeof(uint64_t));
  exit_bytes_written = qed_hs_calloc(EXIT_STATS_NUM_PORTS, sizeof(uint64_t));
  exit_streams = qed_hs_calloc(EXIT_STATS_NUM_PORTS, sizeof(uint32_t));
}
...
rep_hist_exit_stats_term(void)
{
  ...
  qed_hs_free(exit_bytes_read);
  qed_hs_free(exit_bytes_written);
  qed_hs_free(exit_streams);
}
...
rep_hist_format_exit_stats(time_t now)
{
  ...
  qed_hs_assert(now >= start_of_exit_stats_interval);
  ...
  SMARTLIST_FOREACH(written_strings, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(read_strings, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(streams_strings, char *, cp, qed_hs_free(cp));
  ...
  qed_hs_asprintf(&result, "exit-stats-end %s (%d s)\n"
               ...
               read_string,
               streams_string);
  qed_hs_free(written_string);
  qed_hs_free(read_string);
  qed_hs_free(streams_string);
  return result;
}
...
{
  qed_hs_assert(type <= CONN_TYPE_MAX_);
    ...
    double mean_time_cells_in_queue, uint32_t processed_cells)
{
  ...
  if (!start_of_buffer_stats_interval)
    return; /* Not initialized. */
  stats = qed_hs_malloc_zero(sizeof(circ_buffer_stats_t));
...
#define SHARES 10
  ...
  qed_hs_assert(now >= start_of_buffer_stats_interval);
  ...
  time_in_queue_string = smartlist_join_strings(time_in_queue_strings,
                                                ",", 0, NULL);
  SMARTLIST_FOREACH(processed_cells_strings, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(queued_cells_strings, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(time_in_queue_strings, char *, cp, qed_hs_free(cp));
  ...
  /* Put everything together. */
  format_iso_time(t, now);
  qed_hs_asprintf(&result, "cell-stats-end %s (%d s)\n"
  ...
  qed_hs_free(processed_cells_string);
  qed_hs_free(queued_cells_string);
  qed_hs_free(time_in_queue_string);
  return result;
#undef SHARES
}
...
rep_hist_format_desc_stats(time_t now)
{
  ...
  if (size > 0) {
    vals = qed_hs_calloc(size, sizeof(int));
...
{
  hs_v2_stats_t *new_hs_v2_stats = qed_hs_malloc_zero(sizeof(hs_v2_stats_t));
...
hs_v2_stats_free_(hs_v2_stats_t *victim_hs_v2_stats)
{
  ...
  qed_hs_free(victim_hs_v2_stats);
}
...
{
  hs_v3_stats_t *new_hs_v3_stats = qed_hs_malloc_zero(sizeof(hs_v3_stats_t));
...
hs_v3_stats_free_(hs_v3_stats_t *victim_hs_v3_stats)
{
  ...
  qed_hs_free(victim_hs_v3_stats);
}
...
rep_hist_format_hs_stats(time_t now, bool is_v3)
{
  ...
  format_iso_time(t, now);
  qed_hs_asprintf(&hs_stats_string, "%s %s (%u s)\n"
...
rep_hist_get_padding_count_lines(void)
{
  ...
  qed_hs_asprintf(&result, "padding-counts %s (%d s)"
                        " bin-size=%"PRIu64
                        " write-drop=%"PRIu64
                        " write-pad=%"PRIu64
                        " write-total=%"PRIu64
...
rep_hist_log_link_protocol_counts(void)
{
  ...
  for (int i = 1; i <= MAX_LINK_PROTO; i++) {
     char *line = NULL;
     qed_hs_asprintf(&line, "initiated %"PRIu64" and received "
