...
increment_v3_ns_request(country_t country)
{
  ...
  if ((size_t)country >= n_v3_ns_requests_len) {
    ...
    n_v3_ns_requests = qed_hs_reallocarray(n_v3_ns_requests, new_len,
                                        sizeof(uint32_t));
...
{
  unsigned h = (unsigned) qed_hs_addr_hash(&a->addr);
...
clientmap_entries_eq(const clientmap_entry_t *a, const clientmap_entry_t *b)
{
  ...
  return !qed_hs_addr_compare(&a->addr, &b->addr, CMP_EXACT) &&
         a->action == b->action;
}
...
HT_GENERATE2(clientmap, clientmap_entry_t, node, clientmap_entry_hash,
             clientmap_entries_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
...
clientmap_entry_free_(clientmap_entry_t *ent)
{
  ...
  qed_hs_free(ent->transport_name);
  qed_hs_free(ent);
}
...
clientmap_entry_new(geoip_client_action_t action, const qed_hs_addr_t *addr,
                    const char *transport_name)
{
  clientmap_entry_t *entry;

  qed_hs_assert(action == GEOIP_CLIENT_CONNECT ||
             action == GEOIP_CLIENT_NETWORKSTATUS);
  qed_hs_assert(addr);

  entry = qed_hs_malloc_zero(sizeof(clientmap_entry_t));
  entry->action = action;
  qed_hs_addr_copy(&entry->addr, addr);
  if (transport_name) {
    entry->transport_name = qed_hs_strdup(transport_name);
  }
 ...
 * be NULL. */
clientmap_entry_t *
geoip_lookup_client(const qed_hs_addr_t *addr, const char *transport_name,
                    geoip_client_action_t action)
{
  ...
  qed_hs_addr_copy(&lookup.addr, addr);
...
geoip_client_cache_handle_oom(time_t now, size_t min_remove_bytes)
{
  ...
  qed_hs_assert(min_remove_bytes != 0);
...
geoip_note_ns_response(geoip_ns_response_t response)
{
  ...
  qed_hs_assert(response < GEOIP_NS_RESPONSE_NUM);
...
HT_GENERATE2(dirreqmap, dirreq_map_entry_t, node, dirreq_map_ent_hash,
             dirreq_map_ent_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
               ...
               uint64_t dirreq_id)
{
  ...
  qed_hs_assert(entry->type == type);
  qed_hs_assert(entry->dirreq_id == dirreq_id);
...
geoip_get_transport_history(void)
{
  ...
  SMARTLIST_FOREACH(transports_used, char *, s, qed_hs_free(s));
...
#define MIN_DIR_REQ_RESPONSES 16
  if (complete >= MIN_DIR_REQ_RESPONSES) {
    ...
    dltimes = qed_hs_calloc(complete, sizeof(uint32_t));
                         ...
                         char **country_str, char **ipver_str)
{
  ...
  counts = qed_hs_calloc(n_countries, sizeof(unsigned));
  HT_FOREACH(cm_ent, clientmap, &client_history) {
    ...
    qed_hs_assert(0 <= country && country < n_countries);
    ++counts[country];
    ++total;
    switch (qed_hs_addr_family(&(*cm_ent)->addr)) {
    case AF_INET:
      ipv4_count++;
      break;
    case AF_INET6:
  ...
  if (ipver_str) {
    ...
    *ipver_str = smartlist_join_strings(chunks, ",", 0, NULL);
    SMARTLIST_FOREACH(chunks, char *, c, qed_hs_free(c));
  ...
  for (i = 0; i < n_countries; ++i) {
    ...
    if (c >= MIN_IPS_TO_NOTE_COUNTRY) {
      ...
      ent = qed_hs_malloc(sizeof(c_hist_t));
  ...
  SMARTLIST_FOREACH(entries, c_hist_t *, c, qed_hs_free(c));
...
geoip_get_request_history(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(geoip_get_countries(), const geoip_country_t *, c) {
      ...
      if (!tot)
        continue;
      ent = qed_hs_malloc_zero(sizeof(c_hist_t));
  ...
  SMARTLIST_FOREACH(strings, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(entries, c_hist_t *, ent, qed_hs_free(ent));
...
geoip_format_dirreq_stats(time_t now)
{
  ...
  qed_hs_assert(now >= start_of_dirreq_stats_interval);
...
#undef RESPONSE_GRANULARITY
  ...
  qed_hs_asprintf(&result, "dirreq-stats-end %s (%d s)\n"
              "dirreq-v3-ips %s\n"
              "dirreq-v3-reqs %s\n"
  ...
  qed_hs_free(v3_ips_string);
  qed_hs_free(v3_reqs_string);
  qed_hs_free(v3_direct_dl_string);
  qed_hs_free(v3_tunneled_dl_string);
...
geoip_format_bridge_stats(time_t now)
{
  ...
  qed_hs_free(country_data);
  qed_hs_free(ipver_data);
  qed_hs_free(transport_data);
...
geoip_bridge_stats_write(time_t now)
{
  ...
  qed_hs_free(bridge_stats_extrainfo);
...
geoip_format_entry_stats(time_t now)
{
  ...
  qed_hs_assert(now >= start_of_entry_stats_interval);
