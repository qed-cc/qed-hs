 ...
 * nul-terminate it. If the url doesn't start with "/qed-hs/", rewrite it
 * so it does. Return 0.
 * Otherwise, return -1.
 */
STATIC int
parse_http_url(const char *headers, char **url)
{
  ...
  if (strcmpstart(*url, "/qed-hs/")) {
    char *new_url = NULL;
    qed_hs_asprintf(&new_url, "/tor%s%s",
                 *url[0] == '/' ? "" : "/",
                 *url);
    qed_hs_free(*url);
    *url = new_url;
...
parse_accept_encoding_header(const char *h)
{
  ...
  SMARTLIST_FOREACH_BEGIN(methods, const char *, m) {
    ...
    if (method != UNKNOWN_METHOD) {
      qed_hs_assert(((unsigned)method) < 8*sizeof(unsigned));
      result |= (1u << method);
    }
                       ...
                       const char *want_url)
{
  ...
  SMARTLIST_FOREACH(want_authorities, char *, d, qed_hs_free(d));
  smartlist_free(want_authorities);
  SMARTLIST_FOREACH(voters, char *, cp, qed_hs_free(cp));
  smartlist_free(voters);
...
static const url_table_ent_t url_table[] = {
  { "/qed-hs/", 0, handle_get_frontpage },
  { "/qed-hs/status-vote/current/consensus", 1, handle_get_current_consensus },
  { "/qed-hs/status-vote/current/", 1, handle_get_status_vote },
  { "/qed-hs/status-vote/next/bandwidth", 0, handle_get_next_bandwidth },
  { "/qed-hs/status-vote/next/", 1, handle_get_status_vote },
  { "/qed-hs/micro/d/", 1, handle_get_microdesc },
                              ...
                              const char *req_body, size_t req_body_len))
{
  ...
  if ((header = http_get_header(headers, "If-Modified-Since: "))) {
    ...
    if (parse_http_time(header, &tm) == 0) {
      if (qed_hs_timegm(&tm, &if_modified_since)<0) {
        if_modified_since = 0;
      } else {
  ...
  compression_methods_supported &= qed_hs_compress_get_supported_method_bitmask();
...
/** Helper function for GET / or GET /qed-hs/
 */
static int
  ...
  *digests_out = smartlist_new();
  ...
  SMARTLIST_FOREACH_BEGIN(hex_digests, const char *, hex) {
    ...
    if (!parse_one_diff_hash(digest, hex, "X-Or-Diff-From-Consensus header",
                             "ignoring")) {
      smartlist_add(*digests_out, qed_hs_memdup(digest, sizeof(digest)));
    }
  } SMARTLIST_FOREACH_END(hex);
  SMARTLIST_FOREACH(hex_digests, char *, cp, qed_hs_free(cp));
                                    ...
                                    const smartlist_t *digests)
{
  ...
  SMARTLIST_FOREACH_BEGIN(digests, const uint8_t *, digest) {
    if (qed_hs_memeq(ns->digest_sha3_as_signed, digest, DIGEST256_LEN))
      return 1;
  } SMARTLIST_FOREACH_END(digest);
...
parsed_consensus_request_clear(parsed_consensus_request_t *req)
{
  if (!req)
    return;
  qed_hs_free(req->flavor);
  if (req->diff_from_digests) {
    SMARTLIST_FOREACH(req->diff_from_digests, uint8_t *, d, qed_hs_free(d));
                        ...
                        const get_handler_args_t *args)
{
  ...
  const char CONSENSUS_URL_PREFIX[] = "/qed-hs/status-vote/current/consensus/";
  const char CONSENSUS_FLAVORED_PREFIX[] =
    "/qed-hs/status-vote/current/consensus-";
  ...
  if (!strcmpstart(url, CONSENSUS_FLAVORED_PREFIX)) {
    ...
    if (cp) {
      after_flavor = cp+1;
      out->flavor = qed_hs_strndup(f, cp-f);
    } else {
      out->flavor = qed_hs_strdup(f);
    }
  ...
  if (after_flavor && !strcmpstart(after_flavor, DIFF_COMPONENT)) {
    ...
    if (cp) {
      diff_hash_in_url = qed_hs_strndup(after_flavor, cp-after_flavor);
      out->want_fps = cp+1;
    } else {
      diff_hash_in_url = qed_hs_strdup(after_flavor);
      out->want_fps = NULL;
    }
  } else {
  ...
  if (diff_hash_in_url) {
    ...
    int ok = !parse_one_diff_hash(diff_from, diff_hash_in_url, "URL",
                                  "rejecting");
    qed_hs_free(diff_hash_in_url);
    if (ok) {
      smartlist_add(out->diff_from_digests,
                    qed_hs_memdup(diff_from, DIGEST256_LEN));
    } else {
      return -1;
    }
  } else {
...
/** Helper function for GET /qed-hs/status-vote/current/consensus
 */
static int
                             ...
                             const get_handler_args_t *args)
{
  ...
  qed_hs_addr_t addr;
  if (qed_hs_addr_parse(&addr, (TO_CONN(conn))->address) >= 0) {
  ...
  if (compress_method == NO_METHOD && smartlist_len(conn->spool))
    conn->compress_state = qed_hs_compress_new(0, compression_used,
                                            HIGH_COMPRESSION);

  /* Prime the connection with some data. */
  const int initial_flush_result = connection_dirserv_flushed_some(conn);
  qed_hs_assert_nonfatal(initial_flush_result == 0);
  goto done;
...
/** Helper function for GET /qed-hs/status-vote/{current,next}/...
 */
static int
handle_get_status_vote(dir_connection_t *conn, const get_handler_args_t *args)
{
  const char *url = args->url;
  {
    ...
    qed_hs_assert_nonfatal(smartlist_len(items) == 0 ||
    ...
    if (smartlist_len(items)) {
      if (compress_method != NO_METHOD) {
        conn->compress_state = qed_hs_compress_new(1, compress_method,
...
/** Helper function for GET /qed-hs/micro/d/...
 */
static int
handle_get_microdesc(dir_connection_t *conn, const get_handler_args_t *args)
{
  ...
  int clear_spool = 1;
  {
    ...
    dir_split_resource_into_spoolable(url+strlen("/qed-hs/micro/d/"),
                                      DIR_SPOOL_MICRODESC,
                                      conn->spool, NULL,
...
/** Helper function for GET /qed-hs/{server,extra}/...
 */
static int
handle_get_descriptor(dir_connection_t *conn, const get_handler_args_t *args)
{
  ...
  int clear_spool = 1;
  if (!strcmpstart(url,"/qed-hs/server/") ||
      (!options->BridgeAuthoritativeDir &&
       !options->BridgeRelay && !strcmpstart(url,"/qed-hs/extra/"))) {
    int res;
    const char *msg = NULL;
    int cache_lifetime = 0;
    int is_extra = !strcmpstart(url,"/qed-hs/extra/");
    url += is_extra ? strlen("/qed-hs/extra/") : strlen("/qed-hs/server/");
...
/** Helper function for GET /qed-hs/keys/...
 */
static int
handle_get_keys(dir_connection_t *conn, const get_handler_args_t *args)
{
  ...
  const time_t if_modified_since = args->if_modified_since;
  {
    ...
    ssize_t len = -1;
    if (!strcmp(url, "/qed-hs/keys/all")) {
      authority_cert_get_all(certs);
    } else if (!strcmp(url, "/qed-hs/keys/authority")) {
      authority_cert_t *cert = get_my_v3_authority_cert();
      if (cert)
        smartlist_add(certs, cert);
    } else if (!strcmpstart(url, "/qed-hs/keys/fp/")) {
      smartlist_t *fps = smartlist_new();
      dir_split_resource_into_fingerprints(url+strlen("/qed-hs/keys/fp/"),
                                           fps, NULL,
      ...
      smartlist_free(fps);
    } else if (!strcmpstart(url, "/qed-hs/keys/sk/")) {
      smartlist_t *fps = smartlist_new();
      dir_split_resource_into_fingerprints(url+strlen("/qed-hs/keys/sk/"),
                                           fps, NULL,
                                           DSR_HEX|DSR_SORT_UNIQ);
      SMARTLIST_FOREACH(fps, char *, d, {
      ...
      smartlist_free(fps);
    } else if (!strcmpstart(url, "/qed-hs/keys/fp-sk/")) {
      smartlist_t *fp_sks = smartlist_new();
      dir_split_resource_into_fingerprint_pairs(url+strlen("/qed-hs/keys/fp-sk/"),
                                                fp_sks);
                            ...
                            const get_handler_args_t *args)
{
   ...
   * pubkey. */
  qed_hs_assert(!strcmpstart(url, "/qed-hs/hs/3/"));
  pubkey_str = url + strlen("/qed-hs/hs/3/");
...
/** Helper function for GET /qed-hs/networkstatus-bridges
 */
static int
...
/** Helper function for GET robots.txt or /qed-hs/robots.txt */
static int
                           ...
                           const char **end_pos)
{
  ...
  qed_hs_assert(url);
  qed_hs_assert(prefix);
  qed_hs_assert(end_pos);
  ...
  version = qed_hs_parse_long(start, 10, 0, INT_MAX, &ok, &end);
  if (!ok) {
    goto err;
  }
...
handle_post_hs_descriptor(const char *url, const char *body)
{
  ...
  version = parse_hs_version_from_post(url, "/qed-hs/hs/", &end_pos);
  if (version < 0) {
    goto err;
  }
                               ...
                               const char *body, size_t body_len))
{
  ...
  if (authdir_mode(options) &&
      !strcmp(url,"/qed-hs/")) { /* server descriptor post */
    const char *msg = "[None]";
  ...
  if (authdir_mode_v3(options) &&
      !strcmp(url,"/qed-hs/post/vote")) { /* v3 networkstatus vote */
    const char *msg = "OK";
    int status;
  ...
  if (authdir_mode_v3(options) &&
      !strcmp(url,"/qed-hs/post/consensus-signature")) { /* sigs on consensus. */
    const char *msg = NULL;
...
http_set_address_origin(const char *headers, connection_t *conn)
{
  ...
  if (fwd) {
    qed_hs_addr_t toraddr;
    if (qed_hs_addr_parse(&toraddr,fwd) == -1 ||
        qed_hs_addr_is_internal(&toraddr,0)) {
      log_debug(LD_DIR, "Ignoring local/internal IP %s", escaped(fwd));
      qed_hs_free(fwd);
      return;
    }

    qed_hs_free(conn->address);
    conn->address = qed_hs_strdup(fwd);
    qed_hs_free(fwd);
  }
}
...
directory_handle_command(dir_connection_t *conn)
{
  ...
  qed_hs_free(headers); qed_hs_free(body);
  return r;
}
