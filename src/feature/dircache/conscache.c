...
consensus_cache_open(const char *subdir, int max_entries)
{
  ...
  consensus_cache_t *cache = qed_hs_malloc_zero(sizeof(consensus_cache_t));
                    ...
                    size_t datalen)
{
  ...
  consensus_cache_entry_t *ent =
    qed_hs_malloc_zero(sizeof(consensus_cache_entry_t));
  ent->magic = CCE_MAGIC;
  ent->fname = fname;
...
{
  SMARTLIST_FOREACH_BEGIN(cache->entries, consensus_cache_entry_t *, ent) {
    qed_hs_assert_nonfatal(ent->in_cache == cache);
...
consensus_cache_get_n_filenames_available(consensus_cache_t *cache)
{
  ...
  if (used > max)
    return 0;
#else
  qed_hs_assert_nonfatal(max >= used);
#endif /* defined(MUST_UNMAP_TO_UNLINK) */
  return max - used;
}
...
consensus_cache_delete_pending(consensus_cache_t *cache, int force)
{
  SMARTLIST_FOREACH_BEGIN(cache->entries, consensus_cache_entry_t *, ent) {
    ...
    ent->in_cache = NULL;
    char *fname = qed_hs_strdup(ent->fname); /* save a copy */
    consensus_cache_entry_decref(ent);
...
consensus_cache_rescan(consensus_cache_t *cache)
{
  ...
  SMARTLIST_FOREACH_BEGIN(fnames, const char *, fname) {
    ...
    map = storage_dir_map_labeled(cache->dir, fname,
                                  &labels, &body, &bodylen);
    if (! map) {
      /* The ERANGE error might come from qed_hs_mmap_file() -- it means the file
    ...
    consensus_cache_entry_t *ent =
      qed_hs_malloc_zero(sizeof(consensus_cache_entry_t));
    ent->magic = CCE_MAGIC;
    ent->fname = qed_hs_strdup(fname);
    ent->labels = labels;
    ent->refcnt = 1;
    ent->in_cache = cache;
    ent->unused_since = TIME_MAX;
    smartlist_add(cache->entries, ent);
    qed_hs_munmap_file(map); /* don't actually need to keep this around */
