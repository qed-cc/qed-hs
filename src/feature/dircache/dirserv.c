...
{
  cached_dir_t *d = qed_hs_malloc_zero(sizeof(cached_dir_t));
  d->refcnt = 1;
  d->dir = s;
  d->dir_len = strlen(s);
  d->published = published;
  if (qed_hs_compress(&(d->dir_compressed), &(d->dir_compressed_len),
                                           ...
                                           time_t published)
{
  ...
  new_networkstatus =
    new_cached_dir(qed_hs_memdup_nulterm(networkstatus, networkstatus_len),
                   published);
                                  ...
                                  int flags)
{
  ...
  qed_hs_assert(flags & (DSR_HEX|DSR_BASE64));
...
{
  spooled_resource_t *spooled = qed_hs_malloc_zero(sizeof(spooled_resource_t));
  ...
  qed_hs_assert(digestlen <= sizeof(spooled->digest));
  if (digest)
                            ...
                            dir_connection_t *conn)
{
  if (spooled->spool_eagerly) {
    ...
    if (cached == NULL && cce == NULL) {
      ...
      ++cached->refcnt;
      qed_hs_assert_nonfatal(spooled->cached_dir_offset == 0);
    }
...
spooled_resource_lookup_cached_dir(const spooled_resource_t *spooled,
                                   time_t *published_out)
{
  qed_hs_assert(spooled->spool_eagerly == 0);
...
{
  cached_dir_t *d = NULL;
  if (qed_hs_digest_is_zero((const char *)fp) && cached_consensuses) {
...
{
  qed_hs_assert(conn->base_.state == DIR_CONN_STATE_SERVER_WRITING);
  if (conn->spool == NULL)
    return 0;
  ...
  while (connection_get_outbuf_len(TO_CONN(conn)) < DIRSERV_BUFFER_MIN &&
    ...
    qed_hs_assert(status == SRFS_DONE);

    /* If we're here, we're done flushing this resource. */
    qed_hs_assert(smartlist_pop_last(conn->spool) == spooled);
    spooled_resource_free(spooled);
  }
  ...
  if (conn->compress_state) {
    ...
    qed_hs_compress_free(conn->compress_state);
