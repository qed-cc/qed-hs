...
{
  cdm_diff_t *ent;
  ent = qed_hs_malloc_zero(sizeof(cdm_diff_t));
  ent->flavor = flav;
                                   ...
                                   const uint8_t *target_sha3)
{
  ...
  for (u = 0; u < n_diff_compression_methods(); ++u) {
    ...
    if (ent) {
      qed_hs_assert_nonfatal(ent->cdm_diff_status != CDM_DIFF_PRESENT);
      result = 1;
      continue;
    }
...
{
  if (handle == NULL) {
    qed_hs_assert_nonfatal(status != CDM_DIFF_PRESENT);
  }
  ...
  qed_hs_assert_nonfatal(ent->cdm_diff_status == CDM_DIFF_IN_PROGRESS);
...
cdm_cache_init(void)
{
  ...
  qed_hs_assert(cons_diff_cache == NULL);
                                  ...
                                  const networkstatus_t *as_parsed)
{
   ...
   * of catching errors. */
  char *ctmp = qed_hs_memdup(consensus, len);
...
consdiffmgr_find_consensus(struct consensus_cache_entry_t **entry_out,
                           ...
                           compress_method_t method)
{
  qed_hs_assert(entry_out);
  qed_hs_assert((int)flavor < N_CONSENSUS_FLAVORS);
...
consdiffmgr_rescan_flavor_(consensus_flavor_t flavor)
{
  ...
  qed_hs_assert(smartlist_len(matches));
...
{
  cdm_cache_dirty = 1;
  qed_hs_assert(consdiffmgr_rescan_ev);
                  ...
                  const config_line_t *labels_in)
{
  ...
  for (i = 0; i < n_methods; ++i) {
    ...
    char *result;
    size_t sz;
    if (0 == qed_hs_compress(&result, &sz, (const char*)input, len, method)) {
  ...
  *owned_out = NULL;
  ...
  if (method == NO_METHOD) {
    ...
    *outlen = bodylen;
    ...
    rv = qed_hs_uncompress(owned_out, outlen, (const char *)body, bodylen,
                        method, 1, LOG_WARN);
    *out = *owned_out;
  }
  return rv;
...
consensus_diff_worker_threadfn(void *state_, void *work_)
{
  ...
  char *consensus_diff;
  {
    ...
    qed_hs_assert(diff_from_nt);
    qed_hs_assert(diff_to_nt);
  ...
  qed_hs_assert(compress_diffs_with[0] == NO_METHOD);
...
consensus_diff_worker_replyfn(void *work_)
{
  ...
  qed_hs_snprintf(description, sizeof(description),
  ...
  for (u = 0; u < ARRAY_LENGTH(handles); ++u) {
    ...
    if (cache) {
      ...
      qed_hs_assert_nonfatal(h != NULL || this_status == CDM_DIFF_ERROR);
                               ...
                               consensus_cache_entry_t *diff_to)
{
  ...
  consensus_diff_worker_job_t *job = qed_hs_malloc_zero(sizeof(*job));
...
consensus_compress_worker_replyfn(void *work_)
{
  ...
  qed_hs_assert((int)f < N_CONSENSUS_FLAVORS);
  for (u = 0; u < ARRAY_LENGTH(handles); ++u) {
    if (handles[u] == NULL)
      continue;
...
consensus_queue_compression_work(const char *consensus,
                                 ...
                                 const networkstatus_t *as_parsed)
{
  qed_hs_assert(consensus);
  qed_hs_assert(as_parsed);

  consensus_compress_worker_job_t *job = qed_hs_malloc_zero(sizeof(*job));
  job->consensus = qed_hs_memdup_nulterm(consensus, consensus_len);
  ...
  if (as_parsed->voters) {
    ...
    qed_hs_free(signers);
    SMARTLIST_FOREACH(hexvoters, char *, cp, qed_hs_free(cp));
