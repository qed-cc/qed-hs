...
srv_to_control_string(const sr_srv_t *srv)
{
  ...
  qed_hs_asprintf(&srv_str, "%s", srv_hash_encoded);
  return srv_str;
}
...
sr_get_current(const networkstatus_t *ns)
{
  ...
  qed_hs_assert_nonfatal(consensus);
...
sr_parse_srv(const smartlist_t *args)
{
  ...
  num_reveals = qed_hs_parse_uint64(smartlist_get(args, 0),
