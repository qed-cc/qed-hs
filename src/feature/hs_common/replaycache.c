...
replaycache_free_(replaycache_t *r)
{
  ...
  if (r->digests_seen) digest256map_free(r->digests_seen, qed_hs_free_);
    ...
    time_t *elapsed)
{
  ...
  if (access_time != NULL) {
    ...
    access_time = qed_hs_malloc(sizeof(*access_time));
    *access_time = present;
