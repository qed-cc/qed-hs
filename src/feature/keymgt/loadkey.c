                   ...
                   bool *created_out)
{
  ...
  if (!(prkey = crypto_pk_new())) {
    qed_hs_log(severity, LD_GENERAL,"Error constructing key");
    goto error;
  }
  ...
  switch (file_status(fname)) {
    case FN_DIR:
    case FN_ERROR:
      qed_hs_log(severity, LD_FS,"Can't read key from \"%s\"", fname);
      goto error;
      ...
      if (generate) {
        if (!have_lockfile()) {
          if (try_locking(get_options(), 0)<0) {
            ...
            qed_hs_log(severity, LD_FS, "Another Tor process has locked \"%s\". "
        ...
        if (crypto_pk_generate_key(prkey)) {
          qed_hs_log(severity, LD_GENERAL,"Error generating onion key");
          goto error;
        }
        if (! crypto_pk_is_valid_private_key(prkey)) {
          qed_hs_log(severity, LD_GENERAL,"Generated key seems invalid");
          goto error;
        }
        ...
        if (crypto_pk_write_private_key_to_filename(prkey, fname)) {
          qed_hs_log(severity, LD_FS,
              "Couldn't write generated key to \"%s\".", fname);
          goto error;
        }
      } else {
        qed_hs_log(severity, LD_GENERAL, "No key found in \"%s\"", fname);
        goto error;
      }
      return prkey;
    case FN_FILE:
      if (crypto_pk_read_private_key_from_filename(prkey, fname)) {
        qed_hs_log(severity, LD_GENERAL,"Error loading private key.");
        goto error;
      }
      return prkey;
    default:
           ...
           int twice, const or_options_t *options)
{
  ...
  if (twice) {
    ...
    if (p2len < sizeof(msg))
      p2len = sizeof(msg);
    prompt2 = qed_hs_malloc(p2len);
    memset(prompt2, ' ', p2len);
    memcpy(prompt2 + p2len - sizeof(msg), msg, sizeof(msg));

    buf2 = qed_hs_malloc_zero(buflen);
  }

  while (1) {
    length = qed_hs_getpass(prompt, buf, buflen);
    if (length < 0)
      goto done_reading;
    ...
    ssize_t length2 = qed_hs_getpass(prompt2, buf2, buflen);

    if (length != length2 || qed_hs_memneq(buf, buf2, length)) {
...
ed_key_init_from_file(const char *fname, uint32_t flags,
                      ...
                      time_t lifetime,
                      uint8_t cert_type,
                      struct qed_hs_cert_st **cert_out,
                      const or_options_t *options)
{
  ...
  qed_hs_assert((flags & (INIT_ED_KEY_NO_REPAIR|INIT_ED_KEY_NEEDCERT)) !=
                      (INIT_ED_KEY_NO_REPAIR|INIT_ED_KEY_NEEDCERT));

  char tag[8];
  qed_hs_snprintf(tag, sizeof(tag), "type%d", (int)cert_type);

  qed_hs_cert_t *cert = NULL;
  char *got_tag = NULL;
  ed25519_keypair_t *keypair = qed_hs_malloc_zero(sizeof(ed25519_keypair_t));

  if (explicit_fname) {
    secret_fname = qed_hs_strdup(fname);
    encrypted_secret_fname = qed_hs_strdup(fname);
  } else {
    qed_hs_asprintf(&secret_fname, "%s_secret_key", fname);
    qed_hs_asprintf(&encrypted_secret_fname, "%s_secret_key_encrypted", fname);
  }
  qed_hs_asprintf(&public_fname, "%s_public_key", fname);
  qed_hs_asprintf(&cert_fname, "%s_cert", fname);
  ...
  if (load_secret) {
    ...
    if (rv == 0) {
      ...
      qed_hs_assert(got_tag);
    } else {
      if (errno != ENOENT && norepair) {
        qed_hs_log(severity, LD_OR, "Unable to read %s: %s", secret_fname,
                strerror(errno));
        goto err;
      }
    }
  ...
  if (!have_secret && try_to_load && encrypt_key) {
    ...
    if (r > 0) {
      ...
      qed_hs_free(got_tag); /* convince coverity we aren't leaking */
      got_tag = qed_hs_strdup(tag);
      loaded_secret_fname = encrypted_secret_fname;
    } else if (errno != ENOENT && norepair) {
      qed_hs_log(severity, LD_OR, "Unable to read %s: %s",
  ...
  if (have_secret) {
    if (strcmp(got_tag, tag)) {
      qed_hs_log(severity, LD_OR, "%s has wrong tag", loaded_secret_fname);
      goto err;
    }
    /* Derive the public key */
    if (ed25519_public_key_generate(&keypair->pubkey, &keypair->seckey)<0) {
      qed_hs_log(severity, LD_OR, "%s can't produce a public key",
              loaded_secret_fname);
      goto err;
    }
  }
  ...
  if (try_to_load && (!have_secret || split)) {
    ...
    if (!found_public && errno != ENOENT && norepair) {
      qed_hs_log(severity, LD_OR, "Unable to read %s: %s", public_fname,
              strerror(errno));
      goto err;
    }
    if (found_public && strcmp(got_tag, tag)) {
      qed_hs_log(severity, LD_OR, "%s has wrong tag", public_fname);
      goto err;
    }
    if (found_public) {
      if (have_secret) {
        ...
        if (! ed25519_pubkey_eq(&keypair->pubkey, &pubkey_tmp)) {
          qed_hs_log(severity, LD_OR, "%s does not match %s!  If you are trying "
      ...
      if (have_secret) {
        if (ed25519_pubkey_write_to_file(&keypair->pubkey, public_fname, tag)
            < 0) {
          qed_hs_log(severity, LD_OR, "Couldn't repair %s", public_fname);
          goto err;
        } else {
          qed_hs_log(LOG_NOTICE, LD_OR,
  ...
  if (!have_secret && found_public &&
    ...
    if (have_encrypted_secret_file) {
      qed_hs_log(severity, LD_OR, "We needed to load a secret key from %s, "
              "but it was encrypted. Try 'tor --keygen' instead, so you "
              "can enter the passphrase.",
              secret_fname);
    } else if (offline_secret) {
      qed_hs_log(severity, LD_OR, "We wanted to load a secret key from %s, "
  ...
  if (!have_secret && !found_public && !(flags & INIT_ED_KEY_CREATE)) {
    if (split) {
      qed_hs_log(severity, LD_OR, "No key found in %s or %s.",
              secret_fname, public_fname);
    } else {
      qed_hs_log(severity, LD_OR, "No key found in %s.", secret_fname);
    }
    goto err;
  }
  ...
  if (!have_secret && !found_public && have_encrypted_secret_file) {
    qed_hs_assert(!encrypt_key);
    qed_hs_log(severity, LD_OR, "Found an encrypted secret key, "
  ...
  if (!have_secret && !found_public) {
    ...
    keypair = ed_key_new(signing_key, flags, now, lifetime,
                         cert_type, &cert);
    if (!keypair) {
      qed_hs_log(severity, LD_OR, "Couldn't create keypair");
      goto err;
    }
  ...
  if (created_sk || (have_secret && options != NULL &&
    ...
    if (write_secret_key(&keypair->seckey,
      ...
      qed_hs_log(severity, LD_OR, "Couldn't write keys or cert to file.");
      goto err;
    }
    goto done;
  }
  ...
  if (cert_body_len >= 0 && !strcmp(got_tag, tag))
    cert = qed_hs_cert_parse(certbuf, cert_body_len);

  /* If we got it, check it to the extent we can. */
  int bad_cert = 0;

  if (! cert) {
    qed_hs_log(severity, LD_OR, "Cert was unparseable");
    bad_cert = 1;
  } else if (!qed_hs_memeq(cert->signed_key.pubkey, keypair->pubkey.pubkey,
                        ED25519_PUBKEY_LEN)) {
    qed_hs_log(severity, LD_OR, "Cert was for wrong key");
    bad_cert = 1;
  } else if (signing_key &&
             qed_hs_cert_checksig(cert, &signing_key->pubkey, now) < 0) {
    qed_hs_log(severity, LD_OR, "Can't check certificate: %s",
            qed_hs_cert_describe_signature_status(cert));
    bad_cert = 1;
  } else if (cert->cert_expired) {
    qed_hs_log(severity, LD_OR, "Certificate is expired");
    bad_cert = 1;
  } else if (signing_key && cert->signing_key_included &&
             ! ed25519_pubkey_eq(&signing_key->pubkey, &cert->signing_key)) {
    qed_hs_log(severity, LD_OR, "Certificate signed by unexpected key!");
    bad_cert = 1;
  }
  ...
  if (!signing_key || !(flags & INIT_ED_KEY_CREATE)) {
    qed_hs_log(severity, LD_OR, "Without signing key, can't create certificate");
    goto err;
  }
  ...
  cert = qed_hs_cert_create_ed25519(signing_key, cert_type,
                         &keypair->pubkey,
                         now, lifetime,
                         cert_flags);

  if (! cert) {
    qed_hs_log(severity, LD_OR, "Couldn't create certificate");
    goto err;
  }
  ...
  if (crypto_write_tagged_contents_to_file(cert_fname, "ed25519v1-cert",
                             tag, cert->encoded, cert->encoded_len) < 0) {
    qed_hs_log(severity, LD_OR, "Couldn't write cert to disk.");
    goto err;
  }
 ...
 cleanup:
  qed_hs_free(encrypted_secret_fname);
  qed_hs_free(secret_fname);
  qed_hs_free(public_fname);
  qed_hs_free(cert_fname);
  qed_hs_free(got_tag);
           ...
           struct qed_hs_cert_st **cert_out)
{
  ...
  qed_hs_assert(signing_key);
  qed_hs_assert(cert_out);
  uint32_t cert_flags = 0;
  if (flags & INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT)
    cert_flags |= CERT_FLAG_INCLUDE_SIGNING_KEY;
  qed_hs_cert_t *cert = qed_hs_cert_create_ed25519(signing_key, cert_type,
                                     &keypair->pubkey,
                                     now, lifetime,
                                     cert_flags);
  if (! cert)
