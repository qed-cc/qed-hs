                              ...
                              smartlist_t *invalid_digests_out)
{
  ...
  while (1) {
    ...
    end = qed_hs_memstr(*s, eos-*s, "\nrouter-signature");
    if (end)
      end = qed_hs_memstr(end, eos-end, "\n-----END SIGNATURE-----\n");
    if (end)
      end += strlen("\n-----END SIGNATURE-----\n");
    ...
    if (! elt && ! dl_again && have_raw_digest && invalid_digests_out) {
      smartlist_add(invalid_digests_out, qed_hs_memdup(raw_digest, DIGEST_LEN));
    }
    if (!elt) {
      *s = end;
      continue;
...
{
  int ret = 0;
  qed_hs_assert(list != NULL);
  qed_hs_assert(addr_out != NULL);
  qed_hs_assert(port_out != NULL);
  ...
  SMARTLIST_FOREACH_BEGIN(list, directory_token_t *, t) {
    ...
    if (qed_hs_addr_parse_mask_ports(t->args[0], 0,
                                  &a, &bits, &port_min,
                                  &port_max) == AF_INET6 &&
        bits == 128 &&
        port_min == port_max) {
                               ...
                               int *can_dl_again_out)
{
  ...
  qed_hs_assert(!allow_annotations || !prepend_annotations);
  ...
  qed_hs_assert(tok->n_args >= 5);

  router = qed_hs_malloc_zero(sizeof(routerinfo_t));
  ...
  if (!qed_hs_inet_aton(tok->args[1], &in)) {
    log_warn(LD_DIR,"Router address is not an IP address.");
    goto err;
  }
  qed_hs_addr_from_in(&router->ipv4_addr, &in);

  router->ipv4_orport =
    (uint16_t) qed_hs_parse_long(tok->args[2],10,0,65535,&ok,NULL);
  if (!ok) {
    log_warn(LD_DIR,"Invalid OR port %s", escaped(tok->args[2]));
    goto err;
  }
  router->ipv4_dirport =
    (uint16_t) qed_hs_parse_long(tok->args[4],10,0,65535,&ok,NULL);
  if (!ok) {
  ...
  router->bandwidthrate = (int)
    qed_hs_parse_long(tok->args[0],10,1,INT_MAX,&ok,NULL);
  ...
  router->bandwidthcapacity = (int)
    qed_hs_parse_long(tok->args[2],10,0,INT_MAX,&ok,NULL);
  if (!ok) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_UPTIME))) {
    ...
    router->uptime = qed_hs_parse_long(tok->args[0],10,0,LONG_MAX,&ok,NULL);
    if (!ok) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_HIBERNATING))) {
    ...
    router->is_hibernating
      = (qed_hs_parse_long(tok->args[0],10,0,LONG_MAX,NULL,NULL) != 0);
  }
  ...
  if (tok) {
    ...
    router->tap_onion_pkey = qed_hs_memdup(tok->object_body, tok->object_size);
  ...
  if ((tok = find_opt_by_keyword(tokens, K_ONION_KEY_NTOR))) {
    ...
    router->onion_curve25519_pkey =
      qed_hs_memdup(&k, sizeof(curve25519_public_key_t));
  }
...

  {
...

    {
      ...
      if (master_key_tok) {
        ...
        qed_hs_assert(master_key_tok->n_args >= 1);
      ...
      if (qed_hs_cert_get_checkable_sig(&check[0], cert, NULL, &expires) < 0) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_FINGERPRINT))) {
    ...
    if (qed_hs_memneq(d,router->cache_info.identity_digest, DIGEST_LEN)) {
...

  {
    ...
    if ((tok = find_opt_by_keyword(tokens, K_PLATFORM))) {
      router->platform = qed_hs_strdup(tok->args[0]);
      version = tok->args[0];
    }

    if ((tok = find_opt_by_keyword(tokens, K_PROTO))) {
      router->protocol_list = qed_hs_strdup(tok->args[0]);
  ...
  if ((tok = find_opt_by_keyword(tokens, K_CONTACT))) {
    router->contact_info = qed_hs_strdup(tok->args[0]);
  }
  ...
  if (!router->platform) {
    router->platform = qed_hs_strdup("<unknown>");
  }
  goto done;
                            ...
                            int *can_dl_again_out)
{
...

  {
    ...
    if (ed_sig_tok) {
      ...
      uint8_t d256[DIGEST256_LEN];
      const char *signed_start, *signed_end;
      qed_hs_cert_t *cert = qed_hs_cert_parse(
                       (const uint8_t*)ed_cert_tok->object_body,
                       ed_cert_tok->object_size);
      if (! cert) {
        log_warn(LD_DIR, "Couldn't parse ed25519 cert");
      ...
      int check_ok[2];
      if (qed_hs_cert_get_checkable_sig(&check[0], cert, NULL, NULL) < 0) {
        log_err(LD_BUG, "Couldn't create 'checkable' for cert.");
        goto err;
      }
  ...
  if (key) {
    ...
    extrainfo->pending_sig = qed_hs_memdup(tok->object_body,
                                        tok->object_size);
...
router_add_exit_policy(routerinfo_t *router, directory_token_t *tok)
{
  ...
  if (((tok->tp == K_ACCEPT6 || tok->tp == K_REJECT6) &&
       qed_hs_addr_family(&newe->addr) == AF_INET)
      ||
      ((tok->tp == K_ACCEPT || tok->tp == K_REJECT) &&
       qed_hs_addr_family(&newe->addr) == AF_INET6)) {
                      ...
                      time_t *family_expiration_out)
{
  ...
  cert = qed_hs_cert_parse(cert_body, cert_body_size);
   ...
   * separately.
   */
  if (qed_hs_cert_checksig(cert, NULL, 0) < 0) {
  ...
  *family_expiration_out = valid_until;
  ...
  qed_hs_asprintf(family_id_out, "ed25519:%s", buf);
                   ...
                   time_t *family_expiration_out)
{
  ...
  *family_ids_out = smartlist_new();
  ...
  return 0;
 err:
  SMARTLIST_FOREACH(*family_ids_out, char *, cp, qed_hs_free(cp));
