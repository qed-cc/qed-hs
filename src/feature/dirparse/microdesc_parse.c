                       ...
                       saved_location_t where)
{
  ...
  const char *cp = qed_hs_memstr(s, start_of_next_microdesc-s, "onion-key");
  ...
  if (copy_body)
    md->body = qed_hs_memdup_nulterm(cp, md->bodylen);
  else
    md->body = (char*)cp;
  md->off = cp - start;
  ...
  if (id_lines) {
    SMARTLIST_FOREACH_BEGIN(id_lines, directory_token_t *, t) {
      ...
      if (!strcmp(t->args[0], "ed25519")) {
        ...
        md->ed25519_identity_pkey = qed_hs_memdup(&k, sizeof(k));
      }
                             ...
                             smartlist_t *invalid_digests_out)
{
  ...
  while (s < eos) {
   bool okay = false;
    ...
    md = qed_hs_malloc_zero(sizeof(microdesc_t));
  ...
  next:
    if (! okay && invalid_digests_out) {
      smartlist_add(invalid_digests_out,
                    qed_hs_memdup(md_digest, DIGEST256_LEN));
    }
    microdesc_free(md);
    md = NULL;
