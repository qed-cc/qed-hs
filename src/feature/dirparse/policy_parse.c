                                           ...
                                           int *malformed_list))
{
   ...
   * We don't need to multiply QED_HS_ADDR_BUF_LEN by 2, as there is only one
  ...
  *malformed_list = 0;
  ...
  if ((tok->tp == K_ACCEPT6 || tok->tp == K_REJECT6) &&
       qed_hs_addr_family(&r->addr) != AF_INET6) {
...
router_parse_addr_policy(directory_token_t *tok, unsigned fmt_flags)
{
  ...
  qed_hs_assert(tok->tp == K_REJECT || tok->tp == K_REJECT6 ||
  ...
  if (qed_hs_addr_parse_mask_ports(arg, fmt_flags, &newe.addr, &newe.maskbits,
