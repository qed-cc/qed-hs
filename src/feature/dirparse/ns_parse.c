...
find_start_of_next_routerstatus(const char *s, const char *s_eos)
{
  ...
  if ((eos = qed_hs_memstr(s, s_eos - s, "\nr ")))
    ++eos;
  else
    eos = s_eos;

  footer = qed_hs_memstr(s, eos-s, "\ndirectory-footer");
  sig = qed_hs_memstr(s, eos-s, "\ndirectory-signature");
                                 ...
                                 routerstatus_t *rs)
{
  ...
  qed_hs_assert(bool_eq(vote, vote_rs));
  ...
  guardfraction = (uint32_t)qed_hs_parse_ulong(end_of_header+1,
                                            10, 0, 100, &ok, NULL);
  if (!ok) {
                                     ...
                                     consensus_flavor_t flav)
{
  ...
  if (!consensus_method)
    flav = FLAV_NS;
  qed_hs_assert(flav == FLAV_NS || flav == FLAV_MICRODESC);
  ...
  qed_hs_assert(tok->n_args >= 7); /* guaranteed by GE(7) in K_R setup */
  if (flav == FLAV_NS) {
    if (tok->n_args < 8) {
  ...
  if (vote_rs) {
    ...
    rs = qed_hs_malloc_zero(sizeof(routerstatus_t));
  }
  ...
  time_t published_on;
  if (qed_hs_snprintf(timebuf, sizeof(timebuf), "%s %s",
  ...
  if (qed_hs_inet_aton(tok->args[5+offset], &in) == 0) {
    log_warn(LD_DIR, "Error parsing router address in network-status %s",
             escaped(tok->args[5+offset]));
    goto err;
  }
  qed_hs_addr_from_in(&rs->ipv4_addr, &in);

  rs->ipv4_orport = (uint16_t) qed_hs_parse_long(tok->args[6+offset],
                                              10,0,65535,NULL,NULL);
  rs->ipv4_dirport = (uint16_t) qed_hs_parse_long(tok->args[7+offset],
                                               10,0,65535,NULL,NULL);
  ...
  }
  {
    ...
    if ((tok = find_opt_by_keyword(tokens, K_V))) {
      ...
      version = tok->args[0];
      if (vote_rs) {
        vote_rs->version = qed_hs_strdup(tok->args[0]);
      }
    }
  ...
  if ((tok = find_opt_by_keyword(tokens, K_W))) {
    ...
    for (i=0; i < tok->n_args; ++i) {
      if (!strcmpstart(tok->args[i], "Bandwidth=")) {
        int ok;
        rs->bandwidth_kb =
          (uint32_t)qed_hs_parse_ulong(strchr(tok->args[i], '=')+1,
                                    10, 0, UINT32_MAX,
                                    &ok, NULL);
        if (!ok) {
  ...
  if ((tok = find_opt_by_keyword(tokens, K_P))) {
     ...
     */
    rs->exitsummary = qed_hs_strdup(tok->args[0]);
    ...
    SMARTLIST_FOREACH_BEGIN(tokens, directory_token_t *, t) {
      if (t->tp == K_M && t->n_args) {
        vote_microdesc_hash_t *line =
          qed_hs_malloc(sizeof(vote_microdesc_hash_t));
        line->next = vote_rs->microdesc;
        line->microdesc_hash_line = qed_hs_strdup(t->args[0]);
        vote_rs->microdesc = line;
      }
      if (t->tp == K_ID) {
        qed_hs_assert(t->n_args >= 2);
      ...
      if (t->tp == K_PROTO) {
        qed_hs_assert(t->n_args == 1);
        vote_rs->protocols = qed_hs_strdup(t->args[0]);
      }
...
extract_shared_random_srvs(networkstatus_t *ns, smartlist_t *tokens)
{
  ...
  qed_hs_assert(ns->type == NS_TYPE_VOTE || ns->type == NS_TYPE_CONSENSUS);
...
dup_protocols_string(smartlist_t *tokens, bool *error, directory_keyword kw)
{
  ...
  if (protover_list_is_invalid(tok->args[0]))
    *error = true;
  return qed_hs_strdup(tok->args[0]);
}
                                     ...
                                     networkstatus_type_t ns_type)
{
  ...
  ns = qed_hs_malloc_zero(sizeof(networkstatus_t));
  ...
  if (ns_type != NS_TYPE_CONSENSUS) {
    ...
    if (!(cert = qed_hs_memstr(s, end_of_header - s,
  ...
  if (ns->type == NS_TYPE_VOTE || ns->type == NS_TYPE_OPINION) {
    ...
    if (tok) {
      int num_ok;
      ns->consensus_method = (int)qed_hs_parse_long(tok->args[0], 10, 1, INT_MAX,
                                                 &num_ok, NULL);
      if (!num_ok)
        goto err;
    } else {
  ...
  qed_hs_assert(tok->n_args >= 2);
  {
    int ok;
    ns->vote_seconds =
      (int) qed_hs_parse_long(tok->args[0], 10, 0, INT_MAX, &ok, NULL);
    if (!ok)
      goto err;
    ns->dist_seconds =
      (int) qed_hs_parse_long(tok->args[1], 10, 0, INT_MAX, &ok, NULL);
    if (!ok)
      goto err;
  }
  if (ns->valid_after +
  ...
  if ((tok = find_opt_by_keyword(tokens, K_CLIENT_VERSIONS))) {
    ns->client_versions = qed_hs_strdup(tok->args[0]);
  }
  if ((tok = find_opt_by_keyword(tokens, K_SERVER_VERSIONS))) {
    ns->server_versions = qed_hs_strdup(tok->args[0]);
  }
  ...
  if (tok) {
    ...
    for (i = 0; i < tok->n_args; ++i) {
      ...
      qed_hs_free(last_kwd);
      last_kwd = qed_hs_strndup(tok->args[i], eq_pos);
  ...
  SMARTLIST_FOREACH_BEGIN(tokens, directory_token_t *, _tok) {
    ...
    if (tok->tp == K_DIR_SOURCE) {
      ...
      voter->nickname = qed_hs_strdup(tok->args[0]);
      ...
      if (ns->type != NS_TYPE_CONSENSUS &&
          qed_hs_memneq(ns->cert->cache_info.identity_digest,
      ...
      voter->address = qed_hs_strdup(tok->args[2]);
      if (!qed_hs_inet_aton(tok->args[3], &in)) {
        log_warn(LD_DIR, "Error decoding IP address %s in network-status.",
                 escaped(tok->args[3]));
        goto err;
      }
      qed_hs_addr_from_in(&voter->ipv4_addr, &in);
      int ok;
      voter->ipv4_dirport = (uint16_t)
        qed_hs_parse_long(tok->args[4], 10, 0, 65535, &ok, NULL);
      if (!ok)
        goto err;
      voter->ipv4_orport = (uint16_t)
        qed_hs_parse_long(tok->args[5], 10, 0, 65535, &ok, NULL);
      if (!ok)
        goto err;
      ...
      voter->contact = qed_hs_strdup(tok->args[0]);
    } else if (tok->tp == K_VOTE_DIGEST) {
      qed_hs_assert(ns->type == NS_TYPE_CONSENSUS);
      qed_hs_assert(tok->n_args >= 1);
      if (!voter || ! qed_hs_digest_is_zero(voter->vote_digest)) {
  ...
  while (eos - s >= 2 && fast_memeq(s, "r ", 2)) {
    if (ns->type != NS_TYPE_CONSENSUS) {
      vote_routerstatus_t *rs = qed_hs_malloc_zero(sizeof(vote_routerstatus_t));
  ...
  if ((end_of_footer = qed_hs_memstr(s, eos-s, "\nnetwork-status-version ")))
    ++end_of_footer;
  else
    end_of_footer = eos;
  ...
  SMARTLIST_FOREACH_BEGIN(footer_tokens, directory_token_t *, _tok) {
    ...
    sig = qed_hs_malloc_zero(sizeof(document_signature_t));
    ...
    if (ns->type != NS_TYPE_CONSENSUS) {
      if (qed_hs_memneq(declared_identity, ns->cert->cache_info.identity_digest,
                 DIGEST_LEN)) {
    ...
    if (ns->type != NS_TYPE_CONSENSUS) {
      ...
      sig->signature = qed_hs_memdup(tok->object_body, tok->object_size);
