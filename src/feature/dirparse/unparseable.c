                             ...
                             size_t len)
{
  ...
  qed_hs_assert(filename != NULL);
  qed_hs_assert(digest_sha256 != NULL);

  if (descs_dumped == NULL) {
    /* We better have no length, then */
    qed_hs_assert(len_descs_dumped == 0);
    /* Make a smartlist */
  ...
  while (len > max_len - len_descs_dumped &&
    ...
    if (strcmp(tmp->filename, filename) != 0) {
      ...
      qed_hs_unlink(tmp->filename);
      qed_hs_assert(len_descs_dumped >= tmp->len);
...
dump_desc_fifo_bump_hash(const uint8_t *digest_sha256)
{
  ...
  qed_hs_assert(digest_sha256);

  if (descs_dumped) {
    /* Find a match if one exists */
    SMARTLIST_FOREACH_BEGIN(descs_dumped, dumped_desc_t *, ent) {
      if (ent &&
          qed_hs_memeq(ent->digest_sha256, digest_sha256, DIGEST256_LEN)) {
        /*
...
dump_desc_populate_one_file, (const char *dirname, const char *f))
{
  ...
  /* Sanity-check args */
  qed_hs_assert(dirname != NULL);
  qed_hs_assert(f != NULL);

  /* Form the full path */
  qed_hs_asprintf(&path, "%s" PATH_SEPARATOR "%s", dirname, f);
  ...
  /* Compare the digests */
  if (qed_hs_memneq(digest, content_digest, DIGEST256_LEN)) {
    /* No match */
    log_info(LD_DIR,
  ...
  ent = qed_hs_malloc_zero(sizeof(dumped_desc_t));
  ent->filename = path;
...
dump_desc_populate_fifo_from_directory(const char *dirname)
{
  ...
  SMARTLIST_FOREACH(files, char *, f, qed_hs_free(f));
...
dump_desc,(const char *desc, const char *type))
{
   ...
   */
  qed_hs_asprintf(&debugfile_base,
  ...
  qed_hs_free(debugfile_base);
  qed_hs_free(debugfile);
