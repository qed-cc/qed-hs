                   ...
                   directory_token_t *tok, obj_syntax o_syn)
{
  ...
  switch (o_syn) {
    case NO_OBJ:
      ...
      if (tok->object_body) {
        qed_hs_snprintf(ebuf, sizeof(ebuf), "Unexpected object for %s", kwd);
        RET_ERR(ebuf);
      }
      if (tok->key) {
        qed_hs_snprintf(ebuf, sizeof(ebuf), "Unexpected public key for %s", kwd);
        RET_ERR(ebuf);
      }
      break;
    case NEED_OBJ:
      ...
      if (!tok->object_body) {
        qed_hs_snprintf(ebuf, sizeof(ebuf), "Missing object for %s", kwd);
        RET_ERR(ebuf);
      }
      break;
    case OPT_KEY_1024:
    ...
    case NEED_KEY_1024: /* There must be a 1024-bit public key. */
      if (tok->key && crypto_pk_num_bits(tok->key) != PK_BYTES*8) {
        qed_hs_snprintf(ebuf, sizeof(ebuf), "Wrong size on key for %s: %d bits",
    ...
    case NEED_KEY: /* There must be some kind of key. */
      if (!tok->key) {
        qed_hs_snprintf(ebuf, sizeof(ebuf), "Missing public key for %s", kwd);
        RET_ERR(ebuf);
      }

      if (crypto_pk_key_is_private(tok->key)) {
        qed_hs_snprintf(ebuf, sizeof(ebuf),
...
#define MAX_LINE_LENGTH (128*1024)
      ...
      *s = eat_whitespace_eos_no_nl(next, eol);
      ...
      if (table[i].concat_args) {
        ...
        if (get_token_arguments(area, tok, *s, eol)<0) {
          qed_hs_snprintf(ebuf, sizeof(ebuf),"Far too many arguments to %s", kwd);
          RET_ERR(ebuf);
        }
        *s = eol;
      }
      if (tok->n_args < table[i].min_args) {
        qed_hs_snprintf(ebuf, sizeof(ebuf), "Too few arguments to %s", kwd);
        RET_ERR(ebuf);
      } else if (tok->n_args > table[i].max_args) {
        qed_hs_snprintf(ebuf, sizeof(ebuf), "Too many arguments to %s", kwd);
        RET_ERR(ebuf);
      }
      break;
    }
  ...
  *s = eol+1;    /* Set *s to possible start of object data (could be eos) */
  ...
  next = qed_hs_memstr(*s, eos-*s, "-----END ");
  if (!next) {
    RET_ERR("Malformed object: missing object end line");
  }
  qed_hs_assert(eos >= next);
  eol = memchr(next, '\n', eos-next);
  ...
  if ((size_t)(eol-next) != 9+obname_len+5 ||
    ...
    qed_hs_snprintf(ebuf, sizeof(ebuf), "Malformed object: mismatched end tag %s",
             tok->object_type);
