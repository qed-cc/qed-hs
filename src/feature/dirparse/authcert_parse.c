...
#define MAX_CERT_SIZE (128*1024)
  ...
  eos = qed_hs_memstr(s, end_of_s - s, "\ndir-key-certification");
  if (! eos) {
    log_warn(LD_DIR, "No signature found on key certificate");
    return NULL;
  }
  eos = qed_hs_memstr(eos, end_of_s - eos, "\n-----END SIGNATURE-----\n");
  if (! eos) {
  ...
  if (qed_hs_memneq(cert->cache_info.identity_digest, fp_declared, DIGEST_LEN)) {
  ...
  if (tok) {
    ...
    char *address = NULL;
    qed_hs_assert(tok->n_args);
    /* XXX++ use some qed_hs_addr parse function below instead. -RD */
    if (qed_hs_addr_port_split(LOG_WARN, tok->args[0], &address,
                            &cert->ipv4_dirport) < 0 ||
        qed_hs_inet_aton(address, &in) == 0) {
      log_warn(LD_DIR, "Couldn't parse dir-address in certificate");
      qed_hs_free(address);
      goto err;
    }
    qed_hs_addr_from_in(&cert->ipv4_addr, &in);
    qed_hs_free(address);
  }
