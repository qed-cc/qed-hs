...
hibernate_state_to_string(hibernate_state_t state)
{
  ...
  switch (state) {
    ...
    default:
      ...
      qed_hs_snprintf(buf, sizeof(buf), "unknown [%d]", state);
      return buf;
  }
}
...
accounting_parse_options(const or_options_t *options, int validate_only)
{
  ...
  switch (unit) {
  case UNIT_WEEK:
    d = qed_hs_parse_long(smartlist_get(items,1), 10, 1, 7, &ok, NULL);
    if (!ok) {
    ...
    break;
  case UNIT_MONTH:
    d = qed_hs_parse_long(smartlist_get(items,1), 10, 1, 28, &ok, NULL);
    if (!ok) {
  ...
  h = qed_hs_parse_long(s, 10, 0, 23, &ok, &cp);
  if (!ok) {
  ...
  m = qed_hs_parse_long(cp+1, 10, 0, 59, &ok, &cp);
  if (!ok) {
  ...
  SMARTLIST_FOREACH(items, char *, item, qed_hs_free(item));
...
hibernate_begin(hibernate_state_t new_state, time_t now)
{
  ...
  if (new_state == HIBERNATE_STATE_EXITING) {
    ...
    sd_notifyf(0, "EXTEND_TIMEOUT_USEC=%" PRIu64,
            ((uint64_t)(options->ShutdownWaitLength) + 30) * QED_HS_USEC_PER_SEC);
#endif /* defined(HAVE_SYSTEMD) */
...
{
  qed_hs_assert(hibernate_state == HIBERNATE_STATE_LOWBANDWIDTH ||
...
consider_hibernation(time_t now)
{
  ...
  if (hibernate_state == HIBERNATE_STATE_DORMANT) {
    ...
    qed_hs_assert(hibernate_end_time);
                          ...
                          const char **errmsg)
{
  ...
  if (!strcmp(question, "accounting/enabled")) {
    *answer = qed_hs_strdup(accounting_is_enabled(get_options()) ? "1" : "0");
  } else if (!strcmp(question, "accounting/hibernating")) {
    *answer = qed_hs_strdup(hibernate_state_to_string(hibernate_state));
    qed_hs_strlower(*answer);
  } else if (!strcmp(question, "accounting/bytes")) {
      qed_hs_asprintf(answer, "%"PRIu64" %"PRIu64,
                 (n_bytes_read_in_interval),
                 (n_bytes_written_in_interval));
    ...
    *answer = qed_hs_malloc(ISO_TIME_LEN+1);
