...
directories_have_accepted_server_descriptor(void)
{
  ...
  SMARTLIST_FOREACH(servers, dir_server_t *, d, {
    if ((d->type & options->PublishServerDescripqed_hs_) &&
...
directory_get_from_all_authorities(uint8_t dir_purpose,
                                   ...
                                   const char *resource)
{
  qed_hs_assert(dir_purpose == DIR_PURPOSE_FETCH_STATUS_VOTE ||
...
directory_choose_address_routerstatus(const routerstatus_t *status,
                                      dir_indirection_t indirection,
                                      qed_hs_addr_port_t *use_or_ap,
                                      qed_hs_addr_port_t *use_dir_ap)
{
  qed_hs_assert(status != NULL);
  qed_hs_assert(use_or_ap != NULL);
  qed_hs_assert(use_dir_ap != NULL);
  ...
  qed_hs_addr_make_null(&use_or_ap->addr, AF_UNSPEC);
  use_or_ap->port = 0;
  qed_hs_addr_make_null(&use_dir_ap->addr, AF_UNSPEC);
  use_dir_ap->port = 0;

  /* ORPort connections */
  if (indirection == DIRIND_ANONYMOUS) {
    if (!qed_hs_addr_is_null(&status->ipv4_addr)) {
      ...
      qed_hs_addr_copy(&use_or_ap->addr, &status->ipv4_addr);
    ...
    reachable_addr_choose_from_rs(status, FIREWALL_OR_CONNECTION, 0,
                                       use_or_ap);
    have_or = qed_hs_addr_port_is_valid_ap(use_or_ap, 0);
  }
  ...
  if (indirection == DIRIND_DIRECT_CONN ||
    ...
    reachable_addr_choose_from_rs(status, FIREWALL_DIR_CONNECTION, 0,
                                       use_dir_ap);
    have_dir = qed_hs_addr_port_is_valid_ap(use_dir_ap, 0);
  }
  ...
  if (!have_or && !have_dir) {
    ...
    char *ipv6_str = qed_hs_addr_to_str_dup(&status->ipv6_addr);
...
connection_dir_download_routerdesc_failed(dir_connection_t *conn)
{
   ...
   * every 10 or 60 seconds (FOO_DESCRIPQED_HS_RETRY_INTERVAL) in main.c. */
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_SERVERDESC ||
             conn->base_.purpose == DIR_PURPOSE_FETCH_EXTRAINFO ||
...
connection_dir_bridge_routerdesc_failed(dir_connection_t *conn)
{
  ...
  qed_hs_assert(conn->base_.purpose != DIR_PURPOSE_FETCH_EXTRAINFO);
...
connection_dir_download_cert_failed(dir_connection_t *conn, int status)
{
  ...
  smartlist_t *failed;
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_CERTIFICATE);
...
directory_command_should_use_begindir(const or_options_t *options,
                                      ...
                                      const char **reason)
{
  const qed_hs_addr_t *or_addr = &req->or_addr_port.addr;
  //const qed_hs_addr_t *dir_addr = &req->dir_addr_port.addr;
...
{
  qed_hs_assert(dir_purpose >= DIR_PURPOSE_MIN_);
  qed_hs_assert(dir_purpose <= DIR_PURPOSE_MAX_);
  qed_hs_assert(dir_purpose != DIR_PURPOSE_SERVER);
  qed_hs_assert(dir_purpose != DIR_PURPOSE_HAS_FETCHED_HSDESC);

  directory_request_t *result = qed_hs_malloc_zero(sizeof(*result));
  qed_hs_addr_make_null(&result->or_addr_port.addr, AF_INET);
  result->or_addr_port.port = 0;
  qed_hs_addr_make_null(&result->dir_addr_port.addr, AF_INET);
...
directory_request_set_router_purpose(directory_request_t *req,
                                     uint8_t router_purpose)
{
  qed_hs_assert(router_purpose == ROUTER_PURPOSE_GENERAL ||
...
directory_request_set_payload(directory_request_t *req,
                              ...
                              size_t payload_len)
{
  qed_hs_assert(DIR_PURPOSE_IS_UPLOAD(req->dir_purpose));
...
{
  if (ident) {
    qed_hs_assert(req->dir_purpose == DIR_PURPOSE_UPLOAD_HSDESC);
  }
  req->hs_ident = ident;
}
/**
...
{
  if (ident) {
    qed_hs_assert(req->dir_purpose == DIR_PURPOSE_FETCH_HSDESC);
  }
  req->hs_ident = ident;
}
...
directory_request_dir_contact_info_specified(const directory_request_t *req)
{
  ...
  return (req->or_addr_port.port ||
          ...
          ! qed_hs_digest_is_zero(req->digest));
}
...

{
  ...
  const or_options_t *options = get_options();
  const node_t *node;
  qed_hs_addr_port_t use_or_ap, use_dir_ap;
  ...
  if (req->indirection == DIRIND_DIRECT_CONN && status->is_authority) {
    ...
    if (ds) {
      const qed_hs_addr_port_t *v4 = NULL;
      ...
      if (v4) {
        ...
        qed_hs_addr_port_copy(&use_dir_ap, v4);
      }
    }
  }
...
{
  qed_hs_assert(request);
  if (request->routerstatus) {
    qed_hs_assert_nonfatal(
  ...
  const qed_hs_addr_port_t *or_addr_port = &request->or_addr_port;
  const qed_hs_addr_port_t *dir_addr_port = &request->dir_addr_port;
  ...
  qed_hs_assert(or_addr_port->port || dir_addr_port->port);
  ...
  qed_hs_addr_t addr;
  qed_hs_addr_copy(&addr, &(use_begindir ? or_addr_port : dir_addr_port)->addr);
  ...
  if (purpose_needs_anonymity(dir_purpose, router_purpose, resource)) {
    qed_hs_assert(anonymized_connection ||
  ...
  if (!port || qed_hs_addr_is_null(&addr)) {
    static int logged_backtrace = 0;
    log_warn(LD_DIR,
  ...
  conn = dir_connection_new(qed_hs_addr_family(&addr));

  /* set up conn so it's got all the data we need to remember */
  qed_hs_addr_copy(&conn->base_.addr, &addr);
  conn->base_.port = port;
  conn->base_.address = qed_hs_addr_to_str_dup(&addr);
  ...
  if (!anonymized_connection && !use_begindir) {
    ...
    if (options->HTTPProxy) {
      qed_hs_addr_copy(&addr, &options->HTTPProxyAddr);
...
directory_get_consensus_url(const char *resource)
{
...

  {
    ...
    SMARTLIST_FOREACH_BEGIN(router_get_trusted_dir_servers(),
        ...
        hex = qed_hs_malloc(2*CONDITIONAL_CONSENSUS_FPR_LEN+1);
    ...
    qed_hs_asprintf(&url, "/qed-hs/status-vote/current/consensus%s%s/%s.z",
                 hyphen, flavor, authority_id_list);

    SMARTLIST_FOREACH(authority_digests, char *, cp, qed_hs_free(cp));
    smartlist_free(authority_digests);
    qed_hs_free(authority_id_list);
  }
  return url;
}
...
copy_ipv6_address(char* destination, const char* source, size_t len,
                  int decorate) {
  qed_hs_assert(destination);
  qed_hs_assert(source);

  if (decorate && source[0] != '[') {
    qed_hs_snprintf(destination, len, "[%s]", source);
  } else {
                       ...
                       const directory_request_t *req)
{
  ...
  qed_hs_assert(conn);
  qed_hs_assert(conn->base_.type == CONN_TYPE_DIR);

  qed_hs_free(conn->requested_resource);
  if (resource)
    conn->requested_resource = qed_hs_strdup(resource);
  ...
  if (conn->base_.port == 80) {
    ...
    qed_hs_snprintf(hoststring, sizeof(hoststring), "%s:%d",
  ...
  if (direct && get_options()->HTTPProxy) {
    ...
    qed_hs_snprintf(proxystring, sizeof(proxystring),"http://%s", hoststring);
    if (authenticator) {
  ...
  switch (purpose) {
    ...
    case DIR_PURPOSE_FETCH_CERTIFICATE:
      ...
      qed_hs_assert(!payload);
      httpcommand = "GET";
      qed_hs_asprintf(&url, "/qed-hs/keys/%s", resource);
      break;
    case DIR_PURPOSE_FETCH_STATUS_VOTE:
      qed_hs_assert(resource);
      qed_hs_assert(!payload);
      httpcommand = "GET";
      qed_hs_asprintf(&url, "/qed-hs/status-vote/next/%s.z", resource);
      break;
    case DIR_PURPOSE_FETCH_DETACHED_SIGNATURES:
      qed_hs_assert(!resource);
      qed_hs_assert(!payload);
      httpcommand = "GET";
      url = qed_hs_strdup("/qed-hs/status-vote/next/consensus-signatures.z");
      break;
    case DIR_PURPOSE_FETCH_SERVERDESC:
      qed_hs_assert(resource);
      httpcommand = "GET";
      qed_hs_asprintf(&url, "/qed-hs/server/%s", resource);
      break;
    case DIR_PURPOSE_FETCH_EXTRAINFO:
      qed_hs_assert(resource);
      httpcommand = "GET";
      qed_hs_asprintf(&url, "/qed-hs/extra/%s", resource);
      break;
    case DIR_PURPOSE_FETCH_MICRODESC:
      qed_hs_assert(resource);
      httpcommand = "GET";
    ...
    case DIR_PURPOSE_UPLOAD_VOTE:
      ...
      qed_hs_assert(payload);
      httpcommand = "POST";
      url = qed_hs_strdup("/qed-hs/post/vote");
      break;
    case DIR_PURPOSE_UPLOAD_SIGNATURES:
      qed_hs_assert(!resource);
      qed_hs_assert(payload);
      httpcommand = "POST";
      url = qed_hs_strdup("/qed-hs/post/consensus-signature");
      break;
    case DIR_PURPOSE_FETCH_HSDESC:
      qed_hs_assert(resource);
      qed_hs_assert(strlen(resource) <= ED25519_BASE64_LEN);
      qed_hs_assert(!payload);
      httpcommand = "GET";
      qed_hs_asprintf(&url, "/qed-hs/hs/3/%s", resource);
      break;
    case DIR_PURPOSE_UPLOAD_HSDESC:
      qed_hs_assert(resource);
      qed_hs_assert(payload);
      httpcommand = "POST";
      qed_hs_asprintf(&url, "/qed-hs/hs/%s/publish", resource);
      break;
    default:
      qed_hs_assert(0);
      return;
  ...
  qed_hs_snprintf(request, sizeof(request), "%s %s", httpcommand, proxystring);
...

  {
    ...
    qed_hs_snprintf(request, sizeof(request), " HTTP/1.0\r\nHost: %s\r\n%s\r\n",
                 hoststring, header);
    qed_hs_free(header);
  }
  ...
  SMARTLIST_FOREACH(headers, char *, h, qed_hs_free(h));
  smartlist_free(headers);

  log_debug(LD_DIR,
            "Sent request to directory server %s "
            "(purpose: %d, request size: %"QED_HS_PRIuSZ", "
            "payload size: %"QED_HS_PRIuSZ")",
            connection_describe_peer(TO_CONN(conn)),
            conn->base_.purpose,
            (total_request_len),
...
body_is_plausible(const char *body, size_t len, int purpose)
{
  ...
  if (!strcmpstart(body,"router") ||
    ...
    return 1;
  for (i=0;i<32;++i) {
    if (!QED_HS_ISPRINT(body[i]) && !QED_HS_ISSPACE(body[i]))
      return 0;
  }
                        ...
                        const char *source)
{
  ...
  if (qed_hs_snprintf(buf, sizeof(buf),
                   "@downloaded-at %s\n"
                   "@source %s\n"
                                    ...
                                    int anonymized_connection)
{
   ...
   * qed_hs_compress_supports_method() also returns true for NO_METHOD.
  ...
  if (qed_hs_compress_supports_method(compression)) {
    qed_hs_uncompress(&new_body, &new_len, body, body_len, compression,
  ...
  if (qed_hs_compress_supports_method(guessed) &&
      compression != guessed) {
    qed_hs_uncompress(&new_body, &new_len, body, body_len, guessed,
...
dirclient_dump_total_dls(void)
{
  ...
  for (int bootstrapped = 0; bootstrapped < 2; ++bootstrapped) {
    ...
    if (smartlist_len(lines) > 0) {
      ...
      qed_hs_free(log_line);

      SMARTLIST_FOREACH(lines, char *, s, qed_hs_free(s));
    }
...
connection_dir_client_reached_eof(dir_connection_t *conn)
{
  ...
  log_debug(LD_DIR, "Downloaded %"QED_HS_PRIuSZ" bytes on connection of purpose "
             "%s; bootstrap %d%%",
             received_bytes,
             dir_conn_purpose_to_string(conn->base_.purpose),
  ...
  qed_hs_log(LOG_DEBUG, LD_DIR,
            "Received response on %s: %d %s "
            "(purpose: %d, response size: %"QED_HS_PRIuSZ
  ...
  if (conn->dirconn_direct) {
    ...
    if (guess) {
      qed_hs_addr_t addr;
      if (qed_hs_addr_parse(&addr, guess) < 0) {
...
{
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_CONSENSUS);
  ...
  if (looks_like_a_consensus_diff(body, body_len)) {
    ...
    qed_hs_mmap_t *mapped_consensus = NULL;
    ...
    if (!consensus_body) {
      ...
      qed_hs_munmap_file(mapped_consensus);
                                  ...
                                  const response_handler_args_t *args)
{
  ...
  if (status_code != 200) {
    log_warn(LD_DIR,
             "Received http status code %d (%s) from server "
             "%s while fetching \"/qed-hs/keys/%s\".",
             status_code, escaped(reason),
             connection_describe_peer(TO_CONN(conn)),
             conn->requested_resource);
...
handle_response_fetch_status_vote(dir_connection_t *conn,
                                  const response_handler_args_t *args)
{
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_STATUS_VOTE);
  const int status_code = args->status_code;
  const char *reason = args->reason;
  const char *body = args->body;
  const size_t body_len = args->body_len;
  ...
  if (status_code != 200) {
    log_warn(LD_DIR,
             "Received http status code %d (%s) from server "
             "%s while fetching \"/qed-hs/status-vote/next/%s.z\".",
             status_code, escaped(reason),
             connection_describe_peer(TO_CONN(conn)),
             conn->requested_resource);
    return -1;
...
handle_response_fetch_detached_signatures(dir_connection_t *conn,
                                          const response_handler_args_t *args)
{
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_DETACHED_SIGNATURES);
  const int status_code = args->status_code;
  const char *reason = args->reason;
  const char *body = args->body;
  const size_t body_len = args->body_len;
  ...
  if (status_code != 200) {
    log_warn(LD_DIR,
        ...
        "\"/qed-hs/status-vote/next/consensus-signatures.z\".",
        status_code, escaped(reason),
        connection_describe_peer(TO_CONN(conn)));
    return -1;
  }
                           ...
                           const response_handler_args_t *args)
{
  ...
  if (status_code != 200) {
    ...
    log_fn(dir_okay ? LOG_INFO : LOG_WARN, LD_DIR,
           ...
           "while fetching \"/qed-hs/server/%s\". I'll try again soon.",
           status_code, escaped(reason),
           connection_describe_peer(TO_CONN(conn)),
           conn->requested_resource);
    if (!which) {
...
handle_response_fetch_microdesc(dir_connection_t *conn,
                                const response_handler_args_t *args)
{
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_MICRODESC);
  const int status_code = args->status_code;
  const char *reason = args->reason;
  const char *body = args->body;
  const size_t body_len = args->body_len;
  ...
  log_info(LD_DIR,"Received answer to microdescriptor request (status %d, "
           ...
           connection_describe_peer(TO_CONN(conn)));
  qed_hs_assert(conn->requested_resource &&
             !strcmpstart(conn->requested_resource, "d/"));
  qed_hs_assert_nonfatal(!fast_mem_is_zero(conn->identity_digest, DIGEST_LEN));
  which = smartlist_new();
  ...
  if (status_code != 200) {
    log_info(LD_DIR, "Received status code %d (%s) from server "
             "%s while fetching \"/qed-hs/micro/%s\".  I'll try again "
             "soon.",
             status_code, escaped(reason),
             connection_describe_peer(TO_CONN(conn)),
             conn->requested_resource);
...
handle_response_upload_dir(dir_connection_t *conn,
                           const response_handler_args_t *args)
{
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_UPLOAD_DIR);
  const int status_code = args->status_code;
  const char *reason = args->reason;
...
handle_response_upload_vote(dir_connection_t *conn,
                            const response_handler_args_t *args)
{
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_UPLOAD_VOTE);
  const int status_code = args->status_code;
  const char *reason = args->reason;
...
handle_response_upload_signatures(dir_connection_t *conn,
                                  const response_handler_args_t *args)
{
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_UPLOAD_SIGNATURES);
  const int status_code = args->status_code;
  const char *reason = args->reason;
                              ...
                              const response_handler_args_t *args)
{
  ...
  qed_hs_assert(conn);
  qed_hs_assert(conn->base_.purpose == DIR_PURPOSE_UPLOAD_HSDESC);
...
allowed_anonymous_connection_compression_method(compress_method_t method)
{
  ...
  for (u = 0; u < ARRAY_LENGTH(client_meth_allowed_anonymous_compression);
    ...
    if (! qed_hs_compress_supports_method(allowed_method))
      continue;
