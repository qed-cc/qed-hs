...
dirserv_clear_measured_bw_cache(void)
{
  if (mbw_cache) {
    ...
    digestmap_free(mbw_cache, qed_hs_free_);
    mbw_cache = NULL;
  }
}
...
dirserv_read_measured_bandwidths(const char *from_file,
                                 ...
                                 uint8_t *digest_out)
{
  FILE *fp = qed_hs_fopen_cloexec(from_file, "r");
  int applied_lines = 0;
  ...
  file_time = (time_t)qed_hs_parse_ulong(line, 10, 0, ULONG_MAX, &ok, NULL);
  if (!ok) {
 ...
 err:
  if (line) {
    // we need to raw_free this buffer because we got it from qed_hs_getdelim()
    raw_free(line);
  }
  if (fp)
    fclose(fp);
...
{
  char *line = qed_hs_strdup(orig_line);
  char *cp = line;
  int got_bw = 0;
  int got_node_id = 0;
  ...
  cp = qed_hs_strtok_r(cp, " \t", &strtok_state);
  ...
  do {
    ...
    if (strcmpstart(cp, "vote=0") == 0) {
      ...
      out->bw_kb = qed_hs_parse_long(cp, 10, 0, LONG_MAX, &parse_ok, &endptr);
      if (!parse_ok || (*endptr && !QED_HS_ISSPACE(*endptr))) {
  ...
  } while ((cp = qed_hs_strtok_r(NULL, " \t", &strtok_state)));
