...
state_free_(sr_state_t *state)
{
  ...
  qed_hs_free(state->fname);
  digestmap_free(state->commits, commit_free_);
  qed_hs_free(state->current_srv);
  qed_hs_free(state->previous_srv);
  qed_hs_free(state);
}
...
{
  sr_state_t *new_state = qed_hs_malloc_zero(sizeof(*new_state));
  /* If file name is not provided, use default. */
  if (fname == NULL) {
    fname = default_fname;
  }
  new_state->fname = qed_hs_strdup(fname);
...
disk_state_new(time_t now)
{
  ...
  new_state->Version = SR_PROTO_VERSION;
  new_state->TorVersion = qed_hs_strdup(get_version());
...
disk_state_parse_srv(const char *value, sr_srv_t *dst)
{
 ...
 error:
  SMARTLIST_FOREACH(args, char *, s, qed_hs_free(s));
  smartlist_free(args);
  return ret;
}
                           ...
                           const sr_disk_state_t *disk_state)
{
  ...
  for (line = disk_state->SharedRandValues; line; line = line->next) {
    ...
    srv = qed_hs_malloc_zero(sizeof(*srv));
...
disk_state_put_commit_line(const sr_commit_t *commit, config_line_t *line)
{
  ...
  if (!fast_mem_is_zero(commit->encoded_reveal,
    ...
    qed_hs_asprintf(&reveal_str, " %s", commit->encoded_reveal);
  }
  qed_hs_asprintf(&line->value, "%u %s %s %s%s",
               SR_PROTO_VERSION,
...
disk_state_put_srv_line(const sr_srv_t *srv, config_line_t *line)
{
  ...
  qed_hs_asprintf(&line->value, "%" PRIu64 " %s", srv->num_reveals, encoded);
}
...
disk_state_reset(void)
{
  ...
  qed_hs_free(sr_disk_state->TorVersion);
  ...
  sr_disk_state->TorVersion = qed_hs_strdup(get_version());
}
...
disk_state_update(void)
{
  ...
  if (sr_state->previous_srv != NULL) {
    *next = line = qed_hs_malloc_zero(sizeof(config_line_t));
    line->key = qed_hs_strdup(dstate_prev_srv_key);
    disk_state_put_srv_line(sr_state->previous_srv, line);
    /* Go to the next shared random value. */
    next = &(line->next);
  }
  if (sr_state->current_srv != NULL) {
    *next = line = qed_hs_malloc_zero(sizeof(*line));
    line->key = qed_hs_strdup(dstate_cur_srv_key);
  ...
  DIGESTMAP_FOREACH(sr_state->commits, key, sr_commit_t *, commit) {
    *next = line = qed_hs_malloc_zero(sizeof(*line));
    line->key = qed_hs_strdup(dstate_commit_key);
...
state_query_del_(sr_state_object_t obj_type, void *data)
{
  ...
  switch (obj_type) {
  case SR_STATE_OBJ_PREVSRV:
    qed_hs_free(sr_state->previous_srv);
    break;
  case SR_STATE_OBJ_CURSRV:
    qed_hs_free(sr_state->current_srv);
    break;
  case SR_STATE_OBJ_COMMIT:
  case SR_STATE_OBJ_COMMITS:
  case SR_STATE_OBJ_PHASE:
...
sr_state_init(int save_to_disk, int read_from_disk)
{
  ...
  qed_hs_assert(sr_disk_state == NULL);
  qed_hs_assert(sr_state == NULL);
