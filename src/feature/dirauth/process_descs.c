...
dirserv_get_status_impl(const char *id_digest,
                        ...
                        const char *nickname, const qed_hs_addr_t *ipv4_addr,
...
{
  authdir_config_t *list = qed_hs_malloc_zero(sizeof(authdir_config_t));
                           ...
                           rtr_flags_t add_status)
{
  ...
  if (!status) {
    status = qed_hs_malloc_zero(sizeof(rtr_flags_t));
...
dirserv_load_fingerprint_file(void)
{
  ...
  for (list=front; list; list=list->next) {
    ...
    qed_hs_strstrip(fingerprint, " "); /* remove spaces */
 ...
 */
STATIC bool
dirserv_rejects_qed_hs_version(const char *platform,
                            const char **msg)
{
  if (!platform)
    return false;
  ...
  if (!qed_hs_version_as_new_as(platform,
                        ...
                        const char **msg, int severity)
{
  ...
  if (platform) {
    qed_hs_version_t ver_tmp;
    if (qed_hs_version_parse_platform(platform, &ver_tmp, 1) < 0) {
      if (msg) {
  ...
  if (platform && dirserv_rejects_qed_hs_version(platform, msg)) {
    return RTR_REJECT;
  }
  ...
  if (result & RTR_REJECT) {
    if (msg)
      *msg = "Fingerprint and/or ed25519 identity is marked rejected -- if "
             ...
             "bad-relays@lists.qed-cc.org mentioning your fingerprint(s)?";
    return RTR_REJECT;
  ...
  if (!authdir_policy_permits_address(ipv4_addr, ipv4_orport)) {
    ...
    if (msg)
      *msg = "Suspicious relay address range -- if you think this is a "
             ...
             "send an email to bad-relays@lists.qed-cc.org mentioning "
...
dirserv_free_fingerprint_list(void)
{
  ...
  strmap_free(fingerprint_list->fp_by_name, qed_hs_free_);
  digestmap_free(fingerprint_list->status_by_digest, qed_hs_free_);
  digest256map_free(fingerprint_list->status_by_digest256, qed_hs_free_);
  qed_hs_free(fingerprint_list);
}
...
dirserv_router_has_valid_address(routerinfo_t *ri)
{
  ...
  if (qed_hs_addr_is_null(&ri->ipv4_addr) ||
      qed_hs_addr_is_internal(&ri->ipv4_addr, 0)) {
    log_info(LD_DIRSERV,
  ...
  if (!qed_hs_addr_is_null(&ri->ipv6_addr) &&
      qed_hs_addr_is_internal(&ri->ipv6_addr, 0)) {
    log_info(LD_DIRSERV,
                                 ...
                                 const char **msg)
{
  ...
  if (qed_hs_snprintf(annotation_buf, sizeof(annotation_buf),
                   "@uploaded-at %s\n"
                   "@source %s\n"
  ...
  if (!router_parse_list_from_string(&s, s+desclen, list, SAVED_NOWHERE, 0, 0,
    ...
    SMARTLIST_FOREACH(list, routerinfo_t *, ri, {
        msg_out = NULL;
        qed_hs_assert(ri->purpose == purpose);
