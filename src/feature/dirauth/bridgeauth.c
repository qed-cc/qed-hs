...
bridgeauth_dump_bridge_status_to_file(time_t now)
{
  ...
  if (me && crypto_pk_get_fingerprint(me->identity_pkey,
                                      fingerprint, 0) >= 0) {
    qed_hs_asprintf(&fingerprint_line, "fingerprint %s\n", fingerprint);
  } else {
  ...
  qed_hs_asprintf(&published_thresholds_and_status,
               "published %s\nflag-thresholds %s\n%s%s",
  ...
  qed_hs_free(thresholds);
  qed_hs_free(published_thresholds_and_status);
  qed_hs_free(fname);
  qed_hs_free(status);
  qed_hs_free(fingerprint_line);
}
