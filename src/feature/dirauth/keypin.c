...
{
  return qed_hs_memeq(a->rsa_id, b->rsa_id, sizeof(a->rsa_id));
}
...
{
  return qed_hs_memeq(a->ed25519_key, b->ed25519_key, sizeof(a->ed25519_key));
}
...
HT_GENERATE2(rsamap, keypin_ent_st, rsamap_node, keypin_ent_hash_rsa,
             keypin_ents_eq_rsa, 0.6, qed_hs_reallocarray, qed_hs_free_);
...
HT_GENERATE2(edmap, keypin_ent_st, edmap_node, keypin_ent_hash_ed,
             keypin_ents_eq_ed, 0.6, qed_hs_reallocarray, qed_hs_free_);
                          ...
                          const int replace)
{
  ...
  if (ent) {
    qed_hs_assert(fast_memeq(ent->rsa_id, rsa_id_digest, sizeof(ent->rsa_id)));
    if (qed_hs_memeq(ent->ed25519_key, ed25519_id_key,sizeof(ent->ed25519_key))) {
  ...
  if (! replace) {
    ...
    if (ent) {
      ...
      qed_hs_assert(fast_memeq(ent->ed25519_key, ed25519_id_key,
                            sizeof(ent->ed25519_key)));
      qed_hs_assert(fast_memneq(ent->rsa_id, rsa_id_digest, sizeof(ent->rsa_id)));
  ...
  ent = qed_hs_memdup(&search, sizeof(search));
...
#define O_SYNC 0
#endif
  int fd = qed_hs_open_cloexec(fname, O_WRONLY|O_CREAT|O_BINARY|O_SYNC, 0600);
  if (fd < 0)
    goto err;
  ...
  qed_hs_snprintf(buf, sizeof(buf), "@opened-at %s\n", tbuf);
...
keypin_load_journal_impl(const char *data, size_t size)
{
  ...
  qed_hs_log(severity, LD_DIRSERV,
...
keypin_parse_journal_line(const char *cp)
{
  ...
  keypin_ent_t *ent = qed_hs_malloc_zero(sizeof(keypin_ent_t));
