                      ...
                      smartlist_t *vote_routerstatuses)
{
  ...
  qed_hs_assert(vote_routerstatuses);
                                    ...
                                    char **err_msg)
{
  ...
  if (smartlist_len(sl) < 3) {
    qed_hs_asprintf(err_msg, "bad line '%s'", guard_line);
    goto done;
  }
  ...
  if (strlen(inputs_tmp) != HEX_DIGEST_LEN ||
    ...
    qed_hs_asprintf(err_msg, "bad digest '%s'", inputs_tmp);
    goto done;
  }
  ...
  guardfraction =
    (uint32_t) qed_hs_parse_long(inputs_tmp, 10, 0, 100, &num_ok, NULL);
  if (!num_ok) {
    qed_hs_asprintf(err_msg, "wrong percentage '%s'", inputs_tmp);
    goto done;
  }
                                     ...
                                     char **err_msg)
{
  ...
  if (smartlist_len(sl) < 2) {
    qed_hs_asprintf(err_msg, "incomplete line '%s'", inputs_line);
    goto done;
  }

  inputs_tmp = smartlist_get(sl, 0);
  *total_consensuses =
    (int) qed_hs_parse_long(inputs_tmp, 10, 0, INT_MAX, &num_ok, NULL);
  if (!num_ok) {
    qed_hs_asprintf(err_msg, "unparseable consensus '%s'", inputs_tmp);
    goto done;
  }
  ...
  *total_days =
    (int) qed_hs_parse_long(inputs_tmp, 10, 0, INT_MAX, &num_ok, NULL);
  if (!num_ok) {
    qed_hs_asprintf(err_msg, "unparseable days '%s'", inputs_tmp);
    goto done;
  }
                                      ...
                                      smartlist_t *vote_routerstatuses)
{
  ...
  for (line = front; line; line=line->next) {
    ...
    if (!strcmp(line->key, GUARDFRACTION_VERSION)) {
      ...
      version =
        (unsigned int) qed_hs_parse_long(line->value,
