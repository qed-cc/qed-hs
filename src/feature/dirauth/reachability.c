 ...
 */
void
dirserv_orconn_tls_done(const qed_hs_addr_t *addr,
                        uint16_t or_port,
                        const char *digest_rcvd,
                        const ed25519_public_key_t *ed_id_rcvd)
{
  node_t *node = NULL;
  qed_hs_addr_port_t orport;
  routerinfo_t *ri = NULL;
  time_t now = time(NULL);
  qed_hs_assert(addr);
  qed_hs_assert(digest_rcvd);
  ...
  if (dirauth_get_options()->AuthDirTestEd25519LinkKeys &&
    ...
    qed_hs_assert(!ed25519_public_key_is_zero(expected_id));
...
dirserv_single_reachability_test(time_t now, routerinfo_t *router)
{
  ...
  if (dirauth_get_options()->AuthDirHasIPv6Connectivity == 1 &&
    ...
    log_info(LD_OR, "Testing reachability of %s at %s:%u.",
             router->nickname,
             qed_hs_addr_to_str(addrstr, &router->ipv6_addr, sizeof(addrstr), 1),
             router->ipv6_orport);
