...
networkstatus_parse_detached_signatures(const char *s, const char *eos)
{
  ...
  ns_detached_signatures_t *sigs =
    qed_hs_malloc_zero(sizeof(ns_detached_signatures_t));
  ...
  SMARTLIST_FOREACH_BEGIN(tokens, directory_token_t *, _tok) {
    ...
    SMARTLIST_FOREACH(siglist, document_signature_t *, dsig, {
      if (dsig->alg == alg &&
          qed_hs_memeq(id_digest, dsig->identity_digest, DIGEST_LEN) &&
          qed_hs_memeq(sk_digest, dsig->signing_key_digest, DIGEST_LEN)) {
        is_duplicate = 1;
      }
    });
    if (is_duplicate) {
...
ns_detached_signatures_free_(ns_detached_signatures_t *s)
{
  ...
  if (s->signatures) {
    ...
    strmap_free(s->digests, qed_hs_free_);
  }
