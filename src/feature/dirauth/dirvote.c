...
{
  authority_cert_t *out = qed_hs_malloc(sizeof(authority_cert_t));
  qed_hs_assert(cert);
...
format_protocols_lines_for_vote(const networkstatus_t *v3_ns)
{
  ...
  char *result = NULL;
  qed_hs_asprintf(&result, "%s%s%s%s",
               recommended_relay_protocols_line,
               recommended_client_protocols_line,
               required_relay_protocols_line,
               required_client_protocols_line);

  qed_hs_free(recommended_relay_protocols_line);
  qed_hs_free(recommended_client_protocols_line);
  qed_hs_free(required_relay_protocols_line);
  qed_hs_free(required_client_protocols_line);
                          ...
                          networkstatus_t *v3_ns)
{
  ...
  qed_hs_assert(private_signing_key);
  qed_hs_assert(v3_ns->type == NS_TYPE_VOTE || v3_ns->type == NS_TYPE_OPINION);
...

  {
     ...
     * parseable.
     */
    if (!qed_hs_digest256_is_zero((const char *)v3_ns->bw_file_digest256)) {
      /* Encode the digest. */
      ...
      qed_hs_asprintf(&digest_algo_b64_digest_bw_file, "%s=%s",
                   crypto_digest_algorithm_get_name(DIGEST_ALG_BW_FILE),
                   b64_digest_bw_file);
      /* No need for qed_hs_strdup(""), format_line_if_present does it. */
      bw_file_digest = format_line_if_present(
          "bandwidth-file-digest", digest_algo_b64_digest_bw_file);
      qed_hs_free(digest_algo_b64_digest_bw_file);
    }
    ...
    qed_hs_free(params);
    qed_hs_free(flags);
    qed_hs_free(flag_thresholds);
    qed_hs_free(methods);
    qed_hs_free(shared_random_vote_str);
    qed_hs_free(bw_headers_line);
    qed_hs_free(bw_file_digest);

    if (ip_str[0] == '\0')
      goto err;

    if (!qed_hs_digest_is_zero(voter->legacy_id_digest)) {
 ...
 err:
  ...
  qed_hs_free(client_versions_line);
  qed_hs_free(server_versions_line);
  qed_hs_free(protocols_lines);
                                      ...
                                      digest_algorithm_t alg)
{
  ...
  qed_hs_snprintf(mstr, sizeof(mstr), "%d", method);
  mlen = strlen(mstr);
  qed_hs_snprintf(dstr, sizeof(dstr), " %s=",
...
{
  qed_hs_assert(vote);
  qed_hs_assert(vote->type == NS_TYPE_VOTE);
  qed_hs_assert(vote->voters);
  qed_hs_assert(smartlist_len(vote->voters) == 1);
...
#define CMP_FIELD(utype, itype, field) do {                             \
  ...
  if ((r = qed_hs_addr_compare(&a->status.ipv4_addr, &b->status.ipv4_addr,
                            CMP_EXACT))) {
    return r;
  }
...
{
  const qed_hs_addr_port_t *a = *_a, *b = *_b;
  int r;

  if ((r = qed_hs_addr_compare(&a->addr, &b->addr, CMP_EXACT)))
    return r;
...
compute_routerstatus_consensus(smartlist_t *votes, int consensus_method,
                               ...
                               qed_hs_addr_port_t *best_alt_orport_out)
{
  ...
  if (best_alt_orport_out) {
    ...
    const qed_hs_addr_port_t *most_alt_orport = NULL;

    SMARTLIST_FOREACH_BEGIN(votes, vote_routerstatus_t *, rs) {
      qed_hs_assert(rs);
      if (compare_vote_rs(most, rs) == 0 &&
          !qed_hs_addr_is_null(&rs->status.ipv6_addr)
          && rs->status.ipv6_orport) {
        smartlist_add(alt_orports, qed_hs_addr_port_new(&rs->status.ipv6_addr,
                                                     rs->status.ipv6_orport));
      }
    } SMARTLIST_FOREACH_END(rs);
    ...
    most_alt_orport = smartlist_get_most_frequent(alt_orports,
                                                  compare_orports_);
    if (most_alt_orport) {
      memcpy(best_alt_orport_out, most_alt_orport, sizeof(qed_hs_addr_port_t));
      log_debug(LD_DIR, "\"a\" line winner for %s is %s",
                most->status.nickname,
                fmt_addrport(&most_alt_orport->addr, most_alt_orport->port));
    }

    SMARTLIST_FOREACH(alt_orports, qed_hs_addr_port_t *, ap, qed_hs_free(ap));
  ...
  if (microdesc_digest256_out) {
    ...
    SMARTLIST_FOREACH_BEGIN(votes, vote_routerstatus_t *, rs) {
        ...
        if (!vote_routerstatus_find_microdesc_hash(d, rs, consensus_method,
                                                   DIGEST_SHA256))
          smartlist_add(digests, qed_hs_memdup(d, sizeof(d)));
    } SMARTLIST_FOREACH_END(rs);
    ...
    SMARTLIST_FOREACH(digests, char *, cp, qed_hs_free(cp));
...
cmp_int_strings_(const void **_a, const void **_b)
{
  ...
  int ai = (int)qed_hs_parse_long(a, 10, 1, INT_MAX, NULL, NULL);
  int bi = (int)qed_hs_parse_long(b, 10, 1, INT_MAX, NULL, NULL);
  if (ai<bi) {
    return -1;
  } else if (ai==bi) {
...
compute_consensus_method(smartlist_t *votes)
{
  ...
  {
    qed_hs_assert(vote->supported_methods);
  ...
  if (n_ok) {
    ...
    result = (int)qed_hs_parse_long(best, 10, 1, INT_MAX, NULL, NULL);
  } else {
    result = 1;
  }
  smartlist_free(tmp);
                                     ...
                                     int32_t default_val)
{
  ...
  SMARTLIST_FOREACH_BEGIN(param_list, const char *, k_v_pair) {
    if (!strcmpstart(k_v_pair, keyword) && k_v_pair[strlen(keyword)] == '=') {
      ...
      int ok;
      value = (int32_t)
        qed_hs_parse_long(integer_str, 10, INT32_MIN, INT32_MAX, &ok, NULL);
      if (BUG(!ok))
        return default_val;
      ++n_found;
    }
  ...
  if (n_found == 1) {
    ...
    qed_hs_assert_nonfatal(n_found == 0);
    return default_val;
  }
}
...
dirvote_compute_params(smartlist_t *votes, int method, int total_authorities)
{
  ...
  vals = qed_hs_calloc(n_votes, sizeof(int));
  ...
  SMARTLIST_FOREACH_BEGIN(param_list, const char *, param) {
    ...
    int ok=0;
    eq = strchr(param, '=');
    qed_hs_assert(i<n_votes); /* Make sure we prevented vote-stuffing. */
    vals[i++] = (int32_t)
      qed_hs_parse_long(eq+1, 10, INT32_MIN, INT32_MAX, &ok, NULL);
    qed_hs_assert(ok); /* Already checked these when parsing. */
    ...
    if (!next_param || strncmp(next_param, param, cur_param_len)) {
      ...
      if (i > total_authorities/2 ||
        ...
        char *out_string = qed_hs_malloc(64+cur_param_len);
        memcpy(out_string, param, cur_param_len);
        qed_hs_snprintf(out_string+cur_param_len,64, "%ld", (long)median);
                                     ...
                                     int64_t T, int64_t weight_scale)
{
  ...
  qed_hs_assert(0 < weight_scale && weight_scale <= INT32_MAX);
                                ...
                                consensus_flavor_t flavor)
{
  ...
  char *packages = NULL;
  int added_weights = 0;
  dircollaqed_hs_t *collator = NULL;
  smartlist_t *param_list = NULL;

  qed_hs_assert(flavor == FLAV_NS || flavor == FLAV_MICRODESC);
  qed_hs_assert(total_authorities >= smartlist_len(votes));
  qed_hs_assert(total_authorities > 0);
   ...
   * routers we might need to talk about. */
  {
    ...
    time_t *va_times = qed_hs_calloc(n_votes, sizeof(time_t));
    time_t *fu_times = qed_hs_calloc(n_votes, sizeof(time_t));
    time_t *vu_times = qed_hs_calloc(n_votes, sizeof(time_t));
    int *votesec_list = qed_hs_calloc(n_votes, sizeof(int));
    int *distsec_list = qed_hs_calloc(n_votes, sizeof(int));
    ...
    SMARTLIST_FOREACH_BEGIN(votes, networkstatus_t *, v) {
      qed_hs_assert(v->type == NS_TYPE_VOTE);
    ...
    qed_hs_assert(valid_after +
               (get_options()->TestingTorNetwork ?
                MIN_VOTE_INTERVAL_TESTING : MIN_VOTE_INTERVAL) <= fresh_until);
    qed_hs_assert(fresh_until +
               (get_options()->TestingTorNetwork ?
                MIN_VOTE_INTERVAL_TESTING : MIN_VOTE_INTERVAL) <= valid_until);
    qed_hs_assert(vote_seconds >= MIN_VOTE_SECONDS);
    qed_hs_assert(dist_seconds >= MIN_DIST_SECONDS);
    ...
    if (consensus_method < MIN_METHOD_TO_OMIT_PACKAGE_FINGERPRINTS)
      packages = qed_hs_strdup("");
    else
      packages = compute_consensus_package_lines(votes);

    SMARTLIST_FOREACH(combined_server_versions, char *, cp, qed_hs_free(cp));
    SMARTLIST_FOREACH(combined_client_versions, char *, cp, qed_hs_free(cp));
    ...
    qed_hs_free(va_times);
    qed_hs_free(fu_times);
    qed_hs_free(vu_times);
    qed_hs_free(votesec_list);
    qed_hs_free(distsec_list);
  }
  ...
  /* Add the authority sections. */
  {
    ...
    SMARTLIST_FOREACH_BEGIN(votes, networkstatus_t *, v) {
      dir_src_ent_t *e = qed_hs_malloc_zero(sizeof(dir_src_ent_t));
      e->v = v;
      e->digest = get_voter(v)->identity_digest;
      e->is_legacy = 0;
      smartlist_add(dir_sources, e);
      if (!qed_hs_digest_is_zero(get_voter(v)->legacy_id_digest)) {
        dir_src_ent_t *e_legacy = qed_hs_malloc_zero(sizeof(dir_src_ent_t));
        e_legacy->v = v;
    ...
    SMARTLIST_FOREACH(dir_sources, dir_src_ent_t *, e, qed_hs_free(e));
  ...
  /* Add the actual router entries. */
  {
    ...
    uint32_t *bandwidths_kb = qed_hs_calloc(smartlist_len(votes),
                                         sizeof(uint32_t));
    uint32_t *measured_bws_kb = qed_hs_calloc(smartlist_len(votes),
                                           sizeof(uint32_t));
    uint32_t *measured_guardfraction = qed_hs_calloc(smartlist_len(votes),
                                                  sizeof(uint32_t));
    int num_bandwidths;
    int num_mbws;
    ...
    size = qed_hs_calloc(smartlist_len(votes), sizeof(int));
    n_voter_flags = qed_hs_calloc(smartlist_len(votes), sizeof(int));
    n_flag_voters = qed_hs_calloc(smartlist_len(flags), sizeof(int));
    flag_map = qed_hs_calloc(smartlist_len(votes), sizeof(int *));
    named_flag = qed_hs_calloc(smartlist_len(votes), sizeof(int));
    unnamed_flag = qed_hs_calloc(smartlist_len(votes), sizeof(int));
     ...
     * them.*/
    SMARTLIST_FOREACH_BEGIN(votes, networkstatus_t *, v) {
      flag_map[v_sl_idx] = qed_hs_calloc(smartlist_len(v->known_flags),
                                      sizeof(int));
    ...
    collator = dircollaqed_hs_new(smartlist_len(votes), total_authorities);
    SMARTLIST_FOREACH_BEGIN(votes, networkstatus_t *, v) {
      dircollaqed_hs_add_vote(collator, v);
    } SMARTLIST_FOREACH_END(v);

    dircollaqed_hs_collate(collator, consensus_method);

    /* Now go through all the votes */
    flag_counts = qed_hs_calloc(smartlist_len(flags), sizeof(int));
    const int num_routers = dircollaqed_hs_n_routers(collator);
    for (i = 0; i < num_routers; ++i) {
      vote_routerstatus_t **vrs_lst =
        dircollaqed_hs_get_votes_for_router(collator, i);
      ...
      qed_hs_addr_port_t alt_orport = {QED_HS_ADDR_NULL, 0};
      ...
      for (int voter_idx = 0; voter_idx < smartlist_len(votes); ++voter_idx) {
        ...
        if (rs->ed25519_reflects_consensus) {
          ++ed_consensus;
          if (ed_consensus_val) {
            qed_hs_assert(fast_memeq(ed_consensus_val, rs->ed25519_id,
                                  ED25519_PUBKEY_LEN));
          } else {
      ...
      qed_hs_addr_copy(&rs_out.ipv6_addr, &alt_orport.addr);
      ...
      */
      ...
      if (is_guard && num_guardfraction_inputs > 2) {
        ...
        qed_hs_assert(rs_out.guardfraction_percentage <= 100);
      }
       ...
       */
      {
        ...
        SMARTLIST_FOREACH_BEGIN(matching_descs, vote_routerstatus_t *, vsr) {
           ...
           * proper descriptor */
          qed_hs_assert(fast_memeq(rs_out.identity_digest,
      ...
      if (flavor == FLAV_MICRODESC &&
          qed_hs_digest256_is_zero(microdesc_digest)) {
      ...
      if (rs_out.has_bandwidth) {
        ...
        if (rs_out.has_guardfraction) {
          qed_hs_asprintf(&guardfraction_str,
    ...
    qed_hs_free(size);
    qed_hs_free(n_voter_flags);
    qed_hs_free(n_flag_voters);
    for (i = 0; i < smartlist_len(votes); ++i)
      qed_hs_free(flag_map[i]);
    qed_hs_free(flag_map);
    qed_hs_free(flag_counts);
    qed_hs_free(named_flag);
    qed_hs_free(unnamed_flag);
    ...
    qed_hs_free(bandwidths_kb);
    qed_hs_free(measured_bws_kb);
    qed_hs_free(measured_guardfraction);
  }
  ...
  dircollaqed_hs_free(collator);
  qed_hs_free(client_versions);
  qed_hs_free(server_versions);
  qed_hs_free(packages);
  SMARTLIST_FOREACH(flags, char *, cp, qed_hs_free(cp));
  smartlist_free(flags);
  SMARTLIST_FOREACH(chunks, char *, cp, qed_hs_free(cp));
  smartlist_free(chunks);
  SMARTLIST_FOREACH(param_list, char *, cp, qed_hs_free(cp));
  smartlist_free(param_list);
...
compute_consensus_package_lines(smartlist_t *votes)
{
  ...
  SMARTLIST_FOREACH_BEGIN(votes, networkstatus_t *, v) {
    ...
    SMARTLIST_FOREACH_BEGIN(v->package_lines, const char *, line) {
      ...
      char *key = qed_hs_strndup(line, cp - line);

      const char **status = strmap_get(package_status, key);
      if (!status) {
        status = qed_hs_calloc(n_votes, sizeof(const char *));
  ...
  SMARTLIST_FOREACH(result_list, char *, cp, qed_hs_free(cp));
  smartlist_free(result_list);
  smartlist_free(entries);
  strmap_free(package_status, qed_hs_free_);
                                      ...
                                      const char **msg_out)
{
  ...
  smartlist_t *siglist;
  qed_hs_assert(sigs);
  qed_hs_assert(target);
  qed_hs_assert(target->type == NS_TYPE_CONSENSUS);
  ...
  SMARTLIST_FOREACH_BEGIN(siglist, document_signature_t *, sig) {
    ...
    if (sig->good_signature || !old_sig || old_sig->bad_signature) {
      log_info(LD_DIR, "Adding signature from %s with %s", voter_identity,
               algorithm);
      qed_hs_log(severity, LD_DIR, "Added a signature for %s from %s.",
                                ...
                                int for_detached_signatures)
{
  ...
  SMARTLIST_FOREACH(elements, char *, cp, qed_hs_free(cp));
...
networkstatus_get_detached_signatures(smartlist_t *consensuses)
{
  ...
  qed_hs_assert(consensuses);

  SMARTLIST_FOREACH(consensuses, networkstatus_t *, ns, {
      qed_hs_assert(ns);
      qed_hs_assert(ns->type == NS_TYPE_CONSENSUS);
...
dirvote_act(const or_options_t *options, time_t now)
{
  if (!authdir_mode_v3(options))
    return TIME_MAX;
  qed_hs_assert_nonfatal(voting_schedule.voting_starts);
...
dirvote_fetch_missing_votes(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(router_get_trusted_dir_servers(),
      ...
      if (!dirvote_get_vote(ds->v3_identity_digest,
        ...
        char *cp = qed_hs_malloc(HEX_DIGEST_LEN+1);
  ...
  directory_get_from_all_authorities(DIR_PURPOSE_FETCH_STATUS_VOTE,
                                     0, resource);
  qed_hs_free(resource);
  SMARTLIST_FOREACH(missing_fps, char *, cp, qed_hs_free(cp));
  smartlist_free(missing_fps);
}
...
dirvote_clear_votes(int all_votes)
{
  ...
  qed_hs_free(pending_consensus_signatures);
...
list_v3_auth_ids(void)
{
  ...
  SMARTLIST_FOREACH(router_get_trusted_dir_servers(),
                    dir_server_t *, ds,
    if ((ds->type & V3_DIRINFO) &&
        !qed_hs_digest_is_zero(ds->v3_identity_digest))
      smartlist_add(known_v3_keys,
                    qed_hs_strdup(hex_str(ds->v3_identity_digest, DIGEST_LEN))));
  keys = smartlist_join_strings(known_v3_keys, ", ", 0, NULL);
  SMARTLIST_FOREACH(known_v3_keys, char *, cp, qed_hs_free(cp));
                 ...
                 const char **msg_out, int *status_out)
{
  ...
  int any_failed = 0;
  qed_hs_assert(vote_body);
  qed_hs_assert(msg_out);
  qed_hs_assert(status_out);
 ...
 again:
  ...
  SMARTLIST_FOREACH_BEGIN(pending_vote_list, pending_vote_t *, v) {
      if (fast_memeq(v->vote->cert->cache_info.identity_digest,
        ...
        if (fast_memeq(vi_old->vote_digest, vi->vote_digest, DIGEST_LEN)) {
          ...
          cached_dir_decref(v->vote_body);
          networkstatus_vote_free(v->vote);
          v->vote_body = new_cached_dir(qed_hs_strndup(vote_body,
  ...
  pending_vote = qed_hs_malloc_zero(sizeof(pending_vote_t));
  pending_vote->vote_body = new_cached_dir(qed_hs_strndup(vote_body,
                                                       end_of_vote-vote_body),
                                           vote->published);
...
write_v3_votes_to_disk(const smartlist_t *pending_votes)
{
  ...
  qed_hs_free(votefile);
  SMARTLIST_FOREACH(votestrings, sized_chunk_t *, c, qed_hs_free(c));
...
dirvote_compute_consensuses(void)
{
  ...
  qed_hs_assert(pending_vote_list);
  SMARTLIST_FOREACH(pending_vote_list, pending_vote_t *, v, {
...

  {
    ...
    for (flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
      ...
      {
        char *filename;
        qed_hs_asprintf(&filename, "my-consensus-%s", flavor_name);
                       ...
                       const char **msg_out)
{
  ...
  qed_hs_assert(pc->consensus);
  qed_hs_assert(pc->body);
  qed_hs_assert(pending_consensus_signatures);
  ...
  *msg_out = NULL;
  ...
  if (r >= 1) {
    ...
    pc->body = qed_hs_realloc(pc->body, new_consensus_len);
                       ...
                       const char **msg_out)
{
  ...
  ns_detached_signatures_t *sigs;
  qed_hs_assert(detached_signatures_body);
  qed_hs_assert(msg_out);
  qed_hs_assert(pending_consensus_signatures);
...
{
  qed_hs_assert(((int)flav) >= 0 && (int)flav < N_CONSENSUS_FLAVORS);
                                   ...
                                   int consensus_method_high)
{
  ...
  char *microdesc_consensus_methods =
    make_consensus_method_list(consensus_method_low,
                               ...
                               ",");
  qed_hs_assert(microdesc_consensus_methods);

  digest256_to_base64(d64, md->digest);

  if (qed_hs_snprintf(out_buf, out_buf_len, "m %s sha256=%s\n",
 ...
 out:
  qed_hs_free(microdesc_consensus_methods);
  return ret;
}
                                        ...
                                        smartlist_t *microdescriptors_out)
{
  ...
  for (cmr = microdesc_consensus_methods;
    ...
    if (md) {
      microdesc_vote_line_t *e =
        qed_hs_malloc_zero(sizeof(microdesc_vote_line_t));
      e->md = md;
      e->low = cmr->low;
      e->high = cmr->high;
      e->next = entries;
  ...
  while ((ep = entries)) {
    ...
    if (dirvote_format_microdesc_vote_line(buf, sizeof(buf), ep->md,
                                           ep->low, ep->high) >= 0) {
      h = qed_hs_malloc_zero(sizeof(vote_microdesc_hash_t));
      h->microdesc_hash_line = qed_hs_strdup(buf);
      h->next = result;
      result = h;
...
extract_shared_random_commits(networkstatus_t *ns, const smartlist_t *tokens)
{
  ...
  qed_hs_assert(ns->type == NS_TYPE_VOTE);
...
/* The given url is the /qed-hs/status-vote GET directory request. Populates the
                               ...
                               smartlist_t *dir_items)
{
  ...
  if (!strcmp(url, "consensus")) {
    const char *item;
    qed_hs_assert(!current); /* we handle current consensus specially above,
...
compare_routerinfo_addrs_by_family(const routerinfo_t *a,
                                   ...
                                   int family)
{
  const qed_hs_addr_t *addr1 = (family==AF_INET) ? &a->ipv4_addr : &a->ipv6_addr;
  const qed_hs_addr_t *addr2 = (family==AF_INET) ? &b->ipv4_addr : &b->ipv6_addr;
  return qed_hs_addr_compare(addr1, addr2, CMP_EXACT);
}
...
get_all_possible_sybil(const smartlist_t *routers)
{
  ...
  SMARTLIST_FOREACH_BEGIN(routers, routerinfo_t *, ri) {
    ...
    if (qed_hs_addr_family(&(ri->ipv6_addr)) == AF_INET6) {
      smartlist_add(routers_ipv6, ri);
    }
    // If the router has an IPv4 address
    if (qed_hs_addr_family(&(ri->ipv4_addr)) == AF_INET) {
...
version_from_platform(const char *platform)
{
  if (platform && !strcmpstart(platform, "Tor ")) {
    ...
    if (eos) {
      return qed_hs_strndup(platform, eos-platform);
    }
  }
  return NULL;
}
...
format_recommended_version_list(const config_line_t *ln, int warn)
{
  ...
  if (warn) {
    SMARTLIST_FOREACH_BEGIN(versions, const char *, v) {
      ...
      if (qed_hs_version_parse(v, &ver) < 0) {
  ...
  SMARTLIST_FOREACH(versions,char *,s,qed_hs_free(s));
  smartlist_free(versions);
  return result;
}
                                        ...
                                        authority_cert_t *cert)
{
  ...
  if (!hostname || !strchr(hostname, '.')) {
    qed_hs_free(hostname);
    hostname = qed_hs_addr_to_str_dup(&addr);
  }
  ...
  SMARTLIST_FOREACH_BEGIN(routers, routerinfo_t *, ri) {
    ...
    if (ri->cache_info.published_on >= cutoff) {
      ...
      if (ri->protocol_list) {
        vrs->protocols = qed_hs_strdup(ri->protocol_list);
      } else {
        vrs->protocols = qed_hs_strdup(
  ...
  v3_out = qed_hs_malloc_zero(sizeof(networkstatus_t));
  ...
  qed_hs_assert(v3_out->vote_seconds > 0);
  qed_hs_assert(v3_out->dist_seconds > 0);
  qed_hs_assert(timing.n_intervals_valid > 0);

  v3_out->client_versions = client_versions;
  v3_out->server_versions = server_versions;

  v3_out->recommended_relay_protocols =
    qed_hs_strdup(protover_get_recommended_relay_protocols());
  v3_out->recommended_client_protocols =
    qed_hs_strdup(protover_get_recommended_client_protocols());
  v3_out->required_client_protocols =
    qed_hs_strdup(protover_get_required_client_protocols());
  v3_out->required_relay_protocols =
    qed_hs_strdup(protover_get_required_relay_protocols());

  /* We are not allowed to vote to require anything we don't have. */
  qed_hs_assert(protover_all_supported(v3_out->required_relay_protocols, NULL));
  qed_hs_assert(protover_all_supported(v3_out->required_client_protocols, NULL));

  /* We should not recommend anything we don't have. */
  qed_hs_assert_nonfatal(protover_all_supported(
  ...
  voter = qed_hs_malloc_zero(sizeof(networkstatus_voter_info_t));
  voter->nickname = qed_hs_strdup(options->Nickname);
  memcpy(voter->identity_digest, identity_digest, DIGEST_LEN);
  voter->sigs = smartlist_new();
  voter->address = hostname;
  qed_hs_addr_copy(&voter->ipv4_addr, &addr);
  voter->ipv4_dirport = routerconf_find_dir_port(options, 0);
  voter->ipv4_orport = routerconf_find_or_port(options, AF_INET);
  voter->contact = qed_hs_strdup(contact);
