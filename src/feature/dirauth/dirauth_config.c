...
dirauth_options_validate(const void *arg, char **msg)
{
  ...
  qed_hs_version_t minimal_accepted_server_version, recommended_version;
  ...
  if (qed_hs_version_parse(options->MinimalAcceptedServerVersion,
        &minimal_accepted_server_version) != 0) {
    REJECT("Invalid MinimalAcceptedServerVersion");
  }

  qed_hs_assertf(qed_hs_version_parse(get_short_version(),
        &recommended_version) == 0,
      "We failed to parse our own version");
  if (qed_hs_version_compare(&recommended_version,
        &minimal_accepted_server_version) < 0) {
  ...
  qed_hs_free(recommended_versions);
  ...
  SMARTLIST_FOREACH_BEGIN(version_sl, const char *, version) {
    if (version[0] != '\0' && qed_hs_version_parse(version,
          &recommended_version) != 0) {
