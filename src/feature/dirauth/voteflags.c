...
dirserv_compute_performance_thresholds(digestmap_t *omit_as_sybil)
{
  ...
  uptimes = qed_hs_calloc(smartlist_len(nodelist), sizeof(uint32_t));
  /* Bandwidth for every active router. */
  bandwidths_kb = qed_hs_calloc(smartlist_len(nodelist), sizeof(uint32_t));
  /* Bandwidth for every active non-exit router. */
  bandwidths_excluding_exits_kb =
    qed_hs_calloc(smartlist_len(nodelist), sizeof(uint32_t));
  /* Weighted mean time between failure for each active router. */
  mtbfs = qed_hs_calloc(smartlist_len(nodelist), sizeof(double));
  /* Time-known for each active router. */
  tks = qed_hs_calloc(smartlist_len(nodelist), sizeof(long));
  /* Weighted fractional uptime for each active router. */
  wfus = qed_hs_calloc(smartlist_len(nodelist), sizeof(double));
...
#define ABSOLUTE_MIN_VALUE_FOR_FAST_FLAG 4
  ...
  qed_hs_free(bandwidths_excluding_exits_kb);
  qed_hs_free(tks);
  qed_hs_free(wfus);
}
 ...
 * https://bugs.qed-cc.org/tpo/anti-censorship/rdsys/102 */
int
  ...
  if (router_is_me(router)) {
    ...
    answer = (now < node->last_reachable + REACHABLE_TIMEOUT &&
              (dirauth_options->AuthDirHasIPv6Connectivity != 1 ||
               qed_hs_addr_is_null(&router->ipv6_addr) ||
                         ...
                         time_t now)
{
  ...
  return options->AuthDirHasIPv6Connectivity == 1 &&
    !qed_hs_addr_is_null(&ri->ipv6_addr) &&
                                         ...
                                         int listmiddleonly)
{
  ...
  if (! should_publish_node_ipv6(node, ri, now)) {
    ...
    qed_hs_addr_make_null(&rs->ipv6_addr, AF_INET6);
    rs->ipv6_orport = 0;
  }
...
dirserv_set_routerstatus_testing(routerstatus_t *rs)
{
  ...
  qed_hs_assert(get_options()->TestingTorNetwork);
