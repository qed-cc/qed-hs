...
static void dircollaqed_hs_collate_by_ed25519(dircollaqed_hs_t *dc);
...
{
  return qed_hs_malloc_zero(offsetof(ddmap_entry_t, vrs_lst) +
...
HT_GENERATE2(double_digest_map, ddmap_entry_t, node, ddmap_entry_hash,
             ddmap_entry_eq, 0.6, qed_hs_reallocarray, qed_hs_free_);
...
static void
dircollaqed_hs_add_routerstatus(dircollaqed_hs_t *dc,
                             int vote_num,
                             ...
                             vote_routerstatus_t *vrs)
{
  ...
  if (NULL == vrs_lst) {
    vrs_lst = qed_hs_calloc(dc->n_votes, sizeof(vote_routerstatus_t *));
    digestmap_set(dc->by_rsa_sha1, id, vrs_lst);
  }
  qed_hs_assert(vrs_lst[vote_num] == NULL);
...
dircollaqed_hs_t *
dircollaqed_hs_new(int n_votes, int n_authorities)
{
  dircollaqed_hs_t *dc = qed_hs_malloc_zero(sizeof(dircollaqed_hs_t));

  qed_hs_assert(n_votes <= n_authorities);
...
void
dircollaqed_hs_free_(dircollaqed_hs_t *dc)
{
  if (!dc)
    return;
  ...
  digestmap_free(dc->by_rsa_sha1, qed_hs_free_);
 ...
 * only be called before dircollaqed_hs_collate().
 */
void
dircollaqed_hs_add_vote(dircollaqed_hs_t *dc, networkstatus_t *v)
{
  qed_hs_assert(v->type == NS_TYPE_VOTE);
  qed_hs_assert(dc->next_vote_num < dc->n_votes);
  qed_hs_assert(!dc->is_collated);

  const int votenum = dc->next_vote_num++;

  SMARTLIST_FOREACH_BEGIN(v->routerstatus_list, vote_routerstatus_t *, vrs) {
    dircollaqed_hs_add_routerstatus(dc, votenum, v, vrs);
 ...
 * dircollaqed_hs_n_routers() and dircollaqed_hs_get_votes_for_router(). */
void
dircollaqed_hs_collate(dircollaqed_hs_t *dc, int consensus_method)
{
  ...
  dircollaqed_hs_collate_by_ed25519(dc);
 ...
 */
static void
dircollaqed_hs_collate_by_ed25519(dircollaqed_hs_t *dc)
{
 ...
 * only be called after dircollaqed_hs_collate. */
int
dircollaqed_hs_n_routers(dircollaqed_hs_t *dc)
{
  qed_hs_assert(dc->is_collated);
 ...
 *
 * The maximum value for <b>idx</b> is dircollaqed_hs_n_routers().
 *
 * This function may only be called after dircollaqed_hs_collate. */
vote_routerstatus_t **
dircollaqed_hs_get_votes_for_router(dircollaqed_hs_t *dc, int idx)
{
  qed_hs_assert(dc->is_collated);
  qed_hs_assert(idx < smartlist_len(dc->all_rsa_sha1_lst));
