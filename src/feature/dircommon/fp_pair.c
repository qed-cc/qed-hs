...
{
  return qed_hs_memeq(&(a->key), &(b->key), sizeof(fp_pair_t));
}
...
{
  qed_hs_assert(sizeof(a->key) == DIGEST_LEN*2);
...
fp_pair_map_new(void)
{
  ...
  result = qed_hs_malloc(sizeof(fp_pair_map_t));
...
fp_pair_map_set(fp_pair_map_t *map, const fp_pair_t *key, void *val)
{
  ...
  if (resolve) {
    ...
    resolve = qed_hs_malloc_zero(sizeof(fp_pair_map_entry_t));
...
{
  qed_hs_assert(!fp_pair_map_impl_HT_REP_IS_BAD_(&(map->head)));
}
