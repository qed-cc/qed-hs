...
{
  qed_hs_assert(c->magic == DIR_CONNECTION_MAGIC);
...
authdir_type_to_string(dirinfo_type_t auth)
{
  ...
  if (smartlist_len(lst)) {
    ...
    result = qed_hs_strdup("[Not an authority]");
  }
  smartlist_free(lst);
  return result;
}
...
parse_http_command(const char *headers, char **command_out, char **url_out)
{
   ...
   * should be HTTP/1.X\r\n). Assumes we're supporting 1.0? */
  {
    ...
    if (2 != qed_hs_sscanf(e, "HTTP/1.%u%c", &minor_ver, &ch)) {
      return -1;
    }
    if (ch != '\r')
      return -1;
  ...
  *url_out = qed_hs_memdup_nulterm(start, s-start);
  *command_out = qed_hs_memdup_nulterm(command, end_of_command - command);
  return 0;
}
...
http_get_header(const char *headers, const char *which)
{
  ...
  while (cp) {
    if (!strcasecmpstart(cp, which)) {
      ...
      cp += strlen(which);
      if ((eos = strchr(cp,'\r')))
        return qed_hs_strndup(cp, eos-cp);
      else
        return qed_hs_strdup(cp);
    }
    cp = strchr(cp, '\n');
    if (cp)
      ++cp;
                    ...
                    compress_method_t *compression, char **reason)
{
  ...
  qed_hs_assert(headers);
  qed_hs_assert(code);

  while (QED_HS_ISSPACE(*headers)) headers++; /* tolerate leading whitespace */

  if (qed_hs_sscanf(headers, "HTTP/1.%u %u", &n1, &n2) < 2 ||
      (n1 != 0 && n1 != 1) ||
      (n2 < 100 || n2 >= 600)) {
  ...
  *code = n2;
  ...
  if (reason) {
    ...
    qed_hs_assert(smartlist_len(parsed_headers));
    smartlist_split_string(status_line_elements,
                           smartlist_get(parsed_headers, 0),
                           " ", SPLIT_SKIP_SPACE|SPLIT_IGNORE_BLANK, 3);
    qed_hs_assert(smartlist_len(status_line_elements) <= 3);
    ...
    SMARTLIST_FOREACH(status_line_elements, char *, cp, qed_hs_free(cp));
  ...
  SMARTLIST_FOREACH(parsed_headers, char *, s, qed_hs_free(s));
...
connection_dir_finished_connecting(dir_connection_t *conn)
{
  qed_hs_assert(conn);
  qed_hs_assert(conn->base_.type == CONN_TYPE_DIR);
  qed_hs_assert(conn->base_.state == DIR_CONN_STATE_CONNECTING);
                                          ...
                                          smartlist_t *pairs_out)
{
  ...
  SMARTLIST_FOREACH_BEGIN(pairs_tmp, char *, cp) {
    if (strlen(cp) != HEX_DIGEST_LEN*2+1) {
      ...
      if (base16_decode(pair.first, DIGEST_LEN,
        ...
        smartlist_add(pairs_result, qed_hs_memdup(&pair, sizeof(pair)));
      }
    }
    qed_hs_free(cp);
  ...
  smartlist_uniq(pairs_result, compare_pairs_, qed_hs_free_);
                                     ...
                                     int flags)
{
  ...
  qed_hs_assert(!(decode_hex && decode_base64));
  qed_hs_assert(fp_out);
  ...
  if (decode_hex || decode_base64) {
    ...
    for (i = 0; i < smartlist_len(fp_tmp); ++i) {
      ...
      d = qed_hs_malloc_zero(digest_len);
      if (decode_hex ?
