...
smartlist_slice(const smartlist_t *list, int start, int end)
{
  ...
  qed_hs_assert(start >= 0);
  qed_hs_assert(start <= list_len);
  if (end == -1) {
    end = list_len;
  }
  qed_hs_assert(start <= end);

  smartlist_slice_t *slice = qed_hs_malloc(sizeof(smartlist_slice_t));
  slice->list = list;
  slice->offset = start;
  slice->len = end - start;
  return slice;
            ...
            int direction)
{
  ...
  int *result = qed_hs_malloc_zero(a_size);
  /* Copy of the lcs lengths from the last iteration. */
  int *prev = qed_hs_malloc(a_size);

  qed_hs_assert(direction == 1 || direction == -1);
...
{
  int toskip = -1;
  qed_hs_assert(slice1->len == 0 || slice1->len == 1);
                        ...
                        const smartlist_slice_t *slice2)
{
  ...
  qed_hs_free(lens_top);
  qed_hs_free(lens_bot);
...
get_id_hash(const cdline_t *line, cdline_t *hash_out)
{
  ...
  qed_hs_assert(hash_end >= hash);
  qed_hs_assert((size_t)(hash_end - hash) <= UINT32_MAX);
...
typedef struct router_id_iteraqed_hs_t {
  cdline_t last_hash;
  cdline_t hash;
} router_id_iteraqed_hs_t;

#ifndef COCCI
/**
 * Initializer for a router_id_iteraqed_hs_t.
 */
#define ROUTER_ID_ITERAQED_HS_INIT { { NULL, 0 }, { NULL, 0 } }
#endif /* !defined(COCCI) */
                     ...
                     smartlist_t *cons)
{
  ...
  if (dirsig_idx >= 0) {
    ...
    qed_hs_snprintf(buf, sizeof(buf), "%d,$d", dirsig_idx+1);
            ...
            memarea_t *area)
{
  ...
  router_id_iteraqed_hs_t iter1 = ROUTER_ID_ITERAQED_HS_INIT;
  router_id_iteraqed_hs_t iter2 = ROUTER_ID_ITERAQED_HS_INIT;
...
#define MAX_LINE_COUNT (10000)
    ...
    calc_changes(cons1_sl, cons2_sl, changed1, changed2);
    qed_hs_free(cons1_sl);
    qed_hs_free(cons2_sl);
    start1 = i1, start2 = i2;
  }
  ...
  while (i1 >= 0 || i2 >= 0) {
    ...
    if (added == 0) {
      if (deleted == 1) {
        qed_hs_snprintf(buf, sizeof(buf), "%id", start1x+1);
        smartlist_add_linecpy(result, area, buf);
      } else {
        qed_hs_snprintf(buf, sizeof(buf), "%i,%id", start1x+1, start1x+deleted);
        smartlist_add_linecpy(result, area, buf);
      }
    } else {
      int i;
      if (deleted == 0) {
        qed_hs_snprintf(buf, sizeof(buf), "%ia", start1x);
        smartlist_add_linecpy(result, area, buf);
      } else if (deleted == 1) {
        qed_hs_snprintf(buf, sizeof(buf), "%ic", start1x+1);
        smartlist_add_linecpy(result, area, buf);
      } else {
        qed_hs_snprintf(buf, sizeof(buf), "%i,%ic", start1x+1, start1x+deleted);
        smartlist_add_linecpy(result, area, buf);
      }
...
get_linenum(const char **s, int *num_out)
{
  ...
  *num_out = (int) qed_hs_parse_long(*s, 10, 0, INT32_MAX, &ok, &next);
  if (ok && next) {
    *s = next;
    return 0;
  } else {
                  ...
                  memarea_t *area)
{
  ...
  char buf[160];
  smartlist_t *result = smartlist_new();
  qed_hs_snprintf(buf, sizeof(buf), "%s", ns_diff_version);
  smartlist_add_linecpy(result, area, buf);
  qed_hs_snprintf(buf, sizeof(buf), "%s %s %s", hash_token,
                     ...
                     char *digest2_out)
{
  ...
  hash_words = smartlist_new();
  {
    ...
    char *h = qed_hs_memdup_nulterm(line2->s, line2->len);
  ...
  SMARTLIST_FOREACH(hash_words, char *, cp, qed_hs_free(cp));
                    ...
                    const consensus_digest_t *digests1)
{
 ...
 error_cleanup:
  qed_hs_free(cons2_str); /* Sets it to NULL */
 ...
 *
 * Unlike qed_hs_split_lines, this function avoids ambiguity on its
