...
do_hup(void)
{
  ...
  if (options->ReloadTorrcOnSIGHUP) {
    ...
    if (set_options((or_options_t*)options, &msg) < 0) {
      if (!msg)
        msg = qed_hs_strdup("Unknown error");
...
process_signal(int sig)
{
  switch (sig)
    ...
    case SIGTERM:
      ...
      qed_hs_shutdown_event_loop_and_exit(0);
      break;
    case SIGINT:
...
dumpmemusage(int severity)
{
  ...
  qed_hs_log(severity, LD_GENERAL, "In rephist: %"PRIu64" used by %d Tors.",
...
dumpstats(int severity)
{
  ...
  qed_hs_log(severity, LD_GENERAL, "Dumping stats:");
  ...
  SMARTLIST_FOREACH_BEGIN(get_connection_array(), connection_t *, conn) {
    ...
    if (!connection_is_listener(conn)) {
      ...
      if (conn->type == CONN_TYPE_OR) {
        ...
        if (or_conn->tls) {
          if (qed_hs_tls_get_buffer_sizes(or_conn->tls, &rbuf_cap, &rbuf_len,
                                       &wbuf_cap, &wbuf_len) == 0) {
            qed_hs_log(severity, LD_GENERAL,
  ...
  if (stats_n_data_cells_packaged)
    qed_hs_log(severity,LD_NET,"Average packaged cell fullness: %2.3f%%",
        100*(((double)stats_n_data_bytes_packaged) /
             ((double)stats_n_data_cells_packaged*RELAY_PAYLOAD_SIZE_MAX)) );
  if (stats_n_data_cells_received)
    qed_hs_log(severity,LD_NET,"Average delivered cell fullness: %2.3f%%",
  ...
  qed_hs_log(severity, LD_NET, "--------------- Dumping memory information:");
  ...
  for (i = 0; signal_handlers[i].signal_value >= 0; ++i) {
    ...
    if (enabled && signal_handlers[i].try_to_register) {
      signal_handlers[i].signal_event =
        qed_hs_evsignal_new(qed_hs_libevent_get_base(),
      ...
      signal_handlers[i].signal_event =
        qed_hs_event_new(qed_hs_libevent_get_base(), -1,
 ...
 */
int
qed_hs_init(int argc, char *argv[])
{
  char progname[256];
  ...
  qed_hs_init_connection_lists();
  /* Have the log set up with our application name. */
  qed_hs_snprintf(progname, sizeof(progname), "Tor %s", get_version());
 ...
 /* give it somewhere to log to initially */
...

  {
    ...
    log_notice(LD_GENERAL, "Tor can't help you if you use it wrong! "
               ...
               "https://support.qed-cc.org/faq/staying-anonymous/");
    ...
    qed_hs_compress_log_init_warnings();
  }
 ...
 * the lockfile. */
static qed_hs_lockfile_t *lockfile = NULL;
...
try_locking(const or_options_t *options, int err_if_locked)
{
  if (lockfile)
    ...
    qed_hs_lockfile_t *lf = qed_hs_lockfile_lock(fname, 0, &already_locked);
    qed_hs_free(fname);
    if (!lf) {
...
{
  if (lockfile) {
    qed_hs_lockfile_unlock(lockfile);
    lockfile = NULL;
  }
}
 ...
 */
void
qed_hs_remove_file(const char *filename)
{
  if (filename && qed_hs_unlink(filename) != 0 && errno != ENOENT) {
...
do_keygen_family(const char *fname_base)
{
  ...
  qed_hs_asprintf(&fname_key, "%s.secret_family_key", fname_base);
  qed_hs_asprintf(&fname_id, "%s.public_family_id", fname_base);

  if (create_family_id_key(fname_key, &pk) < 0)
    goto done;
  qed_hs_asprintf(&id_contents, "%s\n", ed25519_fmt(&pk));
  ...
  qed_hs_free(fname_key);
  qed_hs_free(fname_id);
  qed_hs_free(id_contents);
  return r;
}
...
#define OPEN(name)                              \
  sandbox_cfg_allow_open_filename(&cfg, qed_hs_strdup(name))

#define OPENDIR(dir)                            \
  sandbox_cfg_allow_opendir_dirname(&cfg, qed_hs_strdup(dir))
...
#define OPEN_KEYDIR_SUFFIX(name, suffix) do {    \
...
#ifdef HAVE_MODULE_RELAY
  {
    ...
    SMARTLIST_FOREACH(family_id_files, char *, cp, qed_hs_free(cp));
  ...
  if (options->ServerDNSResolvConfFile)
    sandbox_cfg_allow_open_filename(&cfg,
                                qed_hs_strdup(options->ServerDNSResolvConfFile));
  else
    sandbox_cfg_allow_open_filename(&cfg, qed_hs_strdup("/etc/resolv.conf"));

  const char *torrc_defaults_fname = get_torrc_fname(1);
  if (torrc_defaults_fname) {
    sandbox_cfg_allow_open_filename(&cfg, qed_hs_strdup(torrc_defaults_fname));
  }
  const char *torrc_fname = get_torrc_fname(0);
  if (torrc_fname) {
    sandbox_cfg_allow_open_filename(&cfg, qed_hs_strdup(torrc_fname));
    // allow torrc backup and torrc.tmp to make SAVECONF work
    char *torrc_bck = NULL;
    qed_hs_asprintf(&torrc_bck, CONFIG_BACKUP_PATTERN, torrc_fname);
    sandbox_cfg_allow_rename(&cfg, qed_hs_strdup(torrc_fname), torrc_bck);
    char *torrc_tmp = NULL;
    qed_hs_asprintf(&torrc_tmp, "%s.tmp", torrc_fname);
    sandbox_cfg_allow_rename(&cfg, torrc_tmp, qed_hs_strdup(torrc_fname));
    sandbox_cfg_allow_open_filename(&cfg, qed_hs_strdup(torrc_tmp));
    // we need to stat the existing backup file
    sandbox_cfg_allow_stat_filename(&cfg, qed_hs_strdup(torrc_bck));
  }
...
#define STAT_KEY_DIRECTORY() \
  sandbox_cfg_allow_stat_filename(&cfg, qed_hs_strdup(options->KeyDirectory))
...

  {
    ...
    qed_hs_log_get_logfile_names(files);
...

  {
    ...
    SMARTLIST_FOREACH(files, char *, file_name, {
      char *tmp_name = NULL;
      qed_hs_asprintf(&tmp_name, "%s.tmp", file_name);
      sandbox_cfg_allow_rename(&cfg,
                               qed_hs_strdup(tmp_name), qed_hs_strdup(file_name));
      /* steals references */
      sandbox_cfg_allow_open_filename(&cfg, file_name);
  ...
  SMARTLIST_FOREACH_BEGIN(get_configured_ports(), port_cfg_t *, port) {
    ...
    char *dirname = qed_hs_strdup(port->unix_addr);
    if (get_parent_directory(dirname) == 0) {
      OPENDIR(dirname);
    }
    qed_hs_free(dirname);
    sandbox_cfg_allow_chmod_filename(&cfg, qed_hs_strdup(port->unix_addr));
    sandbox_cfg_allow_chown_filename(&cfg, qed_hs_strdup(port->unix_addr));
  } SMARTLIST_FOREACH_END(port);

  if (options->DirPortFrontPage) {
    sandbox_cfg_allow_open_filename(&cfg,
                                    qed_hs_strdup(options->DirPortFrontPage));
  }
...
pubsub_install(void)
{
    ...
    qed_hs_assert(r == 0);
    r = qed_hs_mainloop_connect_pubsub(builder); // consumes builder
    qed_hs_assert(r == 0);
}
...
pubsub_connect(void)
{
  if (get_options()->command == CMD_RUN_TOR) {
    qed_hs_mainloop_connect_pubsub_events();
    /* XXXX For each pubsub channel, its delivery strategy should be set at
     * this XXXX point, using qed_hs_mainloop_set_delivery_strategy().
     */
    qed_hs_mainloop_set_delivery_strategy("orconn", DELIV_IMMEDIATE);
    qed_hs_mainloop_set_delivery_strategy("ocirc", DELIV_IMMEDIATE);
  }
}

/* Main entry point for the Tor process.  Called from qed_hs_main(), and by
 * anybody embedding Tor. */
int
qed_hs_run_main(const qed_hs_main_configuration_t *qed_hs_cfg)
{
  int result = 0;

#ifdef EVENT_SET_MEM_FUNCTIONS_IMPLEMENTED
  event_set_mem_functions(qed_hs_malloc_, qed_hs_realloc_, qed_hs_free_);
#endif
  ...
  int argc = qed_hs_cfg->argc + qed_hs_cfg->argc_owned;
  char **argv = qed_hs_calloc(argc, sizeof(char*));
  memcpy(argv, qed_hs_cfg->argv, qed_hs_cfg->argc*sizeof(char*));
  if (qed_hs_cfg->argc_owned)
    memcpy(argv + qed_hs_cfg->argc, qed_hs_cfg->argv_owned,
           qed_hs_cfg->argc_owned*sizeof(char*));
  ...
  {
    int init_rv = qed_hs_init(argc, argv);
    if (init_rv) {
      qed_hs_free_all(0);
  ...
  if (get_options()->Sandbox && get_options()->command == CMD_RUN_TOR) {
    ...
    qed_hs_make_getaddrinfo_cache_active();
