...
qed_hs_cleanup(void)
{
  ...
  if (options->command == CMD_RUN_TOR) {
    ...
    qed_hs_remove_file(options->PidFile);
    /* Remove control port file */
    qed_hs_remove_file(options->ControlPortWriteToFile);
    /* Remove cookie authentication file */
    {
      char *cookie_fname = get_controller_cookie_file_name();
      qed_hs_remove_file(cookie_fname);
      qed_hs_free(cookie_fname);
    }
  ...
  qed_hs_free_all(0); /* We could move qed_hs_free_all back into the ifdef below
