  ...
  switch (request) {
    case SERVICE_CONTROL_STOP:
        case SERVICE_CONTROL_SHUTDOWN:
          ...
          qed_hs_libevent_exit_loop_after_delay(qed_hs_libevent_get_base(),
                                             &exit_now);
          return;
  }
  service_fns.SetServiceStatus_fn(hStatus, &service_status);
 ...
 * manager. This calls qed_hs_init() and starts the main event loop. If
 * qed_hs_init() fails, the service will be stopped and exit code set to
...
nt_service_body(int argc, char **argv)
{
  ...
  pubsub_install();
  r = qed_hs_init(backup_argc, backup_argv);
...
nt_service_main(void)
{
  ...
  if (!service_fns.StartServiceCtrlDispatcherA_fn(table)) {
    ...
    if (result == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {
      if (qed_hs_init(backup_argc, backup_argv))
        return;
      pubsub_connect();
...
{
  TCHAR qed_hs_exe[MAX_PATH+1];
  char qed_hs_exe_ascii[MAX_PATH*2+1];
  ...
  *using_default_torrc = 1;
  ...
  if (0 == GetModuleFileName(NULL, qed_hs_exe, MAX_PATH))
    return NULL;
...
#ifdef UNICODE
  wcstombs(qed_hs_exe_ascii, qed_hs_exe, sizeof(qed_hs_exe_ascii));
  qed_hs_exe_ascii[sizeof(qed_hs_exe_ascii)-1] = '\0';
#else
  strlcpy(qed_hs_exe_ascii, qed_hs_exe, sizeof(qed_hs_exe_ascii));
#endif /* defined(UNICODE) */

  /* Allocate a string for the NT service command line and */
  /* Format the service command */
  if (options) {
    qed_hs_asprintf(&command, "\"%s\" --nt-service \"%s\"",
                 qed_hs_exe_ascii, options);
  } else { /* ! options */
    qed_hs_asprintf(&command, "\"%s\" --nt-service", qed_hs_exe_ascii);
  }
