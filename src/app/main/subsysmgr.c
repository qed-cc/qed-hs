...
/**
 * True iff we have checked qed_hs_subsystems for consistency.
 **/
...
check_and_setup(void)
{
  ...
  raw_assert(ARRAY_LENGTH(sys_status) >= n_qed_hs_subsystems);
                           ...
                           int target_level)
{
  for (unsigned i = 0; i < n_qed_hs_subsystems; ++i) {
    ...
    if (sys->add_pubsub) {
      ...
      pubsub_connecqed_hs_t *connector;
      connector = pubsub_connecqed_hs_for_subsystem(builder, sysid);
      r = sys->add_pubsub(connector);
      pubsub_connecqed_hs_free(connector);
    }
    if (r < 0) {
...
subsystems_shutdown_downto(int target_level)
{
  ...
  for (int i = (int)n_qed_hs_subsystems - 1; i >= 0; --i) {
    const subsys_fns_t *sys = qed_hs_subsystems[i];
    if (!sys->supported)
      continue;
    if (sys->level <= target_level)
