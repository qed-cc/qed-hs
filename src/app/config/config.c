...
#if defined(HAVE_MODULE_RELAY) || defined(QED_HS_UNIT_TESTS)
  /* The unit tests expect the ClientOnly default to be 0. */
  V(ClientOnly,                  BOOL,     "0"),
#else
  /* We must be a Client if the relay module is disabled. */
  V(ClientOnly,                  BOOL,     "1"),
#endif /* defined(HAVE_MODULE_RELAY) || defined(QED_HS_UNIT_TESTS) */
...
static const struct {
  const char *k;
  const char *v;
} testing_qed_hs_network_defaults[] = {
#ifndef COCCI
#include "testnet.inc"
#endif
  { NULL, NULL }
...
{
  qed_hs_assert(global_options);
  qed_hs_assert_nonfatal(! in_option_validation);
...
get_options_defaults(void)
{
  ...
  if (testing_network_configured) {
    for (i = 0; testing_qed_hs_network_defaults[i].k; ++i) {
      config_line_append(next,
                         testing_qed_hs_network_defaults[i].k,
                         testing_qed_hs_network_defaults[i].v);
      next = &(*next)->next;
    }
  }
...
set_options(or_options_t *new_val, char **msg)
{
  ...
  if (subsystems_set_options(get_options_mgr(), new_val) < 0 ||
    ...
    if (! qed_hs_event_loop_shutdown_is_pending()) {
      log_err(LD_BUG,
              "Acting on config options left us in a broken state. Dying.");
      qed_hs_shutdown_event_loop_and_exit(1);
    }
    global_options = old_options;
    return -1;
  }
...
options_clear_cb(const config_mgr_t *mgr, void *opts)
{
  ...
  if (options->SchedulerTypes_) {
    SMARTLIST_FOREACH(options->SchedulerTypes_, int *, i, qed_hs_free(i));
    smartlist_free(options->SchedulerTypes_);
  }
  if (options->FilesOpenedByIncludes) {
    SMARTLIST_FOREACH(options->FilesOpenedByIncludes, char *, f, qed_hs_free(f));
    smartlist_free(options->FilesOpenedByIncludes);
  }
  qed_hs_free(options->DataDirectory);
  qed_hs_free(options->CacheDirectory);
  qed_hs_free(options->FamilyKeyDirectory);
  qed_hs_free(options->KeyDirectory);
  qed_hs_free(options->BridgePassword_AuthDigest_);
  qed_hs_free(options->command_arg);
  qed_hs_free(options->master_key_fname);
  config_free_lines(options->MyFamily);
  if (options->FamilyIds) {
    SMARTLIST_FOREACH(options->FamilyIds,
                      ed25519_public_key_t *, k, qed_hs_free(k));
...
config_free_all(void)
{
  ...
  qed_hs_free(torrc_fname);
  qed_hs_free(torrc_defaults_fname);
                            ...
                            const or_options_t *old_options)
{
   ...
   * the unit tests. */
  qed_hs_assert(!(options->DirAuthorities &&
               (options->AlternateDirAuthority
...
options_act_once_on_startup(char **msg_out)
{
  ...
  if (options->DisableAllSwap) {
    if (qed_hs_mlockall() == -1) {
      *msg_out = qed_hs_strdup("DisableAllSwap failure. Do you have proper "
                        "permissions?");
      return -1;
    }
  }
...
options_switch_id(char **msg_out)
{
  ...
  if (options->User) {
    qed_hs_assert(have_low_ports != -1);
    ...
    if (switch_id(options->User, switch_id_flags) != 0) {
      /* No need to roll back, since you can't change the value. */
      *msg_out = qed_hs_strdup("Problem with User value. See logs for details.");
      return -1;
    }
  }
...
options_start_listener_transaction(const or_options_t *old_options,
                                   char **msg_out)
{
  listener_transaction_t *xn = qed_hs_malloc_zero(sizeof(listener_transaction_t));
  ...
  if (! sandbox_is_active()) {
    if (set_max_file_descriptors((unsigned)options->ConnLimit,
      ...
      *msg_out = qed_hs_strdup("Problem with ConnLimit value. "
  ...
  if (parse_ports(options, 0, msg_out, &n_ports, NULL)) {
    if (!*msg_out)
      *msg_out = qed_hs_strdup("Unexpected problem parsing port config");
    goto rollback;
  }
  ...
  if (!we_are_hibernating()) {
    if (retry_all_listeners(xn->new_listeners,
      ...
      *msg_out = qed_hs_strdup("Failed to bind one of the listener ports.");
      goto rollback;
    }
  }
  ...
  if (options->TransPort_set &&
    ...
    if (get_pf_socket() < 0) {
      *msg_out = qed_hs_strdup("Unable to open /dev/pf for transparent proxy.");
      goto rollback;
    }
  }
                              ...
                              char **msg_out)
{
  ...
  log_transaction_t *xn = qed_hs_malloc_zero(sizeof(log_transaction_t));
  ...
  xn->logs_marked = true;
  /* Configure the qed_hs_log(s) */
  if (options_init_logs(old_options, options, 0)<0) {
    *msg_out = qed_hs_strdup("Failed to init Log options. See logs for details.");
...
options_commit_log_transaction(log_transaction_t *xn)
{
  ...
  if (xn->logs_marked) {
    log_severity_list_t *severity =
      qed_hs_malloc_zero(sizeof(log_severity_list_t));
    close_temp_logs();
    add_callback_log(severity, control_event_logmsg);
    logs_set_pending_callback_callback(control_event_logmsg_pending);
    control_adjust_event_log_severity();
    qed_hs_free(severity);
    qed_hs_log_update_sigsafe_err_fds();
  }
...
options_act,(const or_options_t *old_options))
{
  ...
  if (options->NoExec || options->Sandbox) {
    qed_hs_disable_spawning_background_processes();
  }
  ...
  /* disable ptrace and later, other basic debugging techniques */
  {
    ...
    if (options->DisableDebuggerAttachment && !disabled_debugger_attach &&
        running_tor) {
      int ok = qed_hs_disable_debugger_attach();
  ...
  if (running_tor && !old_options &&
    ...
    qed_hs_socket_t ctrl_sock = (qed_hs_socket_t)options->OwningControllerFD;
  ...
  moniqed_hs_owning_controller_process(options->OwningControllerProcess);
...
{
  parsed_cmdline_t *result = qed_hs_malloc_zero(sizeof(parsed_cmdline_t));
  ...
  while (i < argc) {
    ...
    if (want_arg == ARGUMENT_NECESSARY && is_last) {
      ...
      arg = (want_arg != ARGUMENT_NONE) ? qed_hs_strdup(argv[i+1]) :
                                              qed_hs_strdup("");
    }

    param = qed_hs_malloc_zero(sizeof(config_line_t));
    param->key = is_cmdline ? qed_hs_strdup(argv[i]) :
                 qed_hs_strdup(config_expand_abbrev(get_options_mgr(), s, 1, 1));
    param->value = arg;
...
"Copyright (c) 2007-2021, The QED Project, Inc.\n\n"
"tor -f <torrc> [args]\n"
"See man page for options, or https://www.qed-cc.org/ for "
"documentation.\n");
}
...
print_library_versions(void)
{
  ...
  printf("Libevent\t\t%-15s\t\t%s\n",
                    qed_hs_libevent_get_header_version_str(),
                    qed_hs_libevent_get_version_str());
...
#endif
  if (qed_hs_compress_supports_method(ZLIB_METHOD)) {
    printf("Zlib    \t\t%-15s\t\t%s\n",
                      qed_hs_compress_version_str(ZLIB_METHOD),
                      qed_hs_compress_header_version_str(ZLIB_METHOD));
  }
  if (qed_hs_compress_supports_method(LZMA_METHOD)) {
    printf("Liblzma \t\t%-15s\t\t%s\n",
                      qed_hs_compress_version_str(LZMA_METHOD),
                      qed_hs_compress_header_version_str(LZMA_METHOD));
  }
  if (qed_hs_compress_supports_method(ZSTD_METHOD)) {
    printf("Libzstd \t\t%-15s\t\t%s\n",
                      qed_hs_compress_version_str(ZSTD_METHOD),
                      qed_hs_compress_header_version_str(ZSTD_METHOD));
  }
  if (qed_hs_libc_get_name()) {
    printf("%-7s \t\t%-15s\t\t%s\n",
           qed_hs_libc_get_name(),
           qed_hs_libc_get_header_version_str(),
           qed_hs_libc_get_version_str());
  }
  //TODO: Hex versions?
}
...
static int
handle_cmdline_no_passphrase(qed_hs_cmdline_mode_t command)
{
...
static int
handle_cmdline_format(qed_hs_cmdline_mode_t command, const char *value)
{
...
static int
handle_cmdline_newpass(qed_hs_cmdline_mode_t command)
{
  if (command == CMD_KEYGEN) {
...
static int
handle_cmdline_passphrase_fd(qed_hs_cmdline_mode_t command, const char *value)
{
  if (get_options()->keygen_force_passphrase == FORCE_PASSPHRASE_OFF) {
    ...
    int ok = 1;
    long fd = qed_hs_parse_long(value, 10, 0, INT_MAX, &ok, NULL);
    if (fd < 0 || ok == 0) {
...
static int
handle_cmdline_master_key(qed_hs_cmdline_mode_t command, const char *value)
{
  if (command != CMD_KEYGEN) {
    log_err(LD_CONFIG, "--master-key without --keygen!");
    return -1;
  } else {
    get_options_mutable()->master_key_fname = qed_hs_strdup(value);
    return 0;
  }
}
...
validate_ports_csv(smartlist_t *sl, const char *name, char **msg)
{
  ...
  SMARTLIST_FOREACH(sl, const char *, cp,
  {
    i = atoi(cp);
    if (i < 1 || i > 65535) {
      qed_hs_asprintf(msg, "Port '%s' out of range in %s", cp, name);
      return -1;
    }
  });
  return 0;
...
config_ensure_bandwidth_cap(uint64_t *value, const char *desc, char **msg)
{
  ...
  if (*value > ROUTER_MAX_DECLARED_BANDWIDTH) {
    qed_hs_asprintf(msg, "%s (%"PRIu64") must be at most %d",
                 desc, (*value),
                         ...
                         char **msg_out)
{
 ...
 err:
  in_option_validation = 0;
  qed_hs_assert(new_options == NULL || rv != SETOPT_OK);
 ...
 *
 * On error, qed_hs_strdup an error explanation into *<b>msg</b>.
 */
int
...
#define REJECT(arg) \
  STMT_BEGIN *msg = qed_hs_strdup(arg); return -1; STMT_END
...
options_validate_scheduler(or_options_t *options, char **msg)
{
  ...
  if (options->SchedulerTypes_) {
    SMARTLIST_FOREACH(options->SchedulerTypes_, int *, iptr, qed_hs_free(iptr));
    smartlist_free(options->SchedulerTypes_);
  }
  ...
  SMARTLIST_FOREACH_BEGIN(options->Schedulers, const char *, type) {
    ...
    if (!strcasecmp("KISTLite", type)) {
      sched_type = qed_hs_malloc_zero(sizeof(int));
      ...
      *sched_type = SCHEDULER_VANILLA;
      ...
      qed_hs_asprintf(msg, "Unknown type %s in option Schedulers. "
  ...
  if (options->KISTSchedRunInterval > KIST_SCHED_RUN_INTERVAL_MAX) {
    qed_hs_asprintf(msg, "KISTSchedRunInterval must not be more than %d (ms)",
...
options_validate_cb(const void *old_options_, void *options_, char **msg)
{
  ...
  *msg = NULL;
  ...
  if (options->ControlSocket || options->ControlSocketsGroupWritable) {
    *msg = qed_hs_strdup("Unix domain sockets (ControlSocket) not supported "
                      "on this OS/with this build.");
    return -1;
  }
#else /* defined(HAVE_SYS_UN_H) */
  if (options->ControlSocketsGroupWritable && !options->ControlSocket) {
    *msg = qed_hs_strdup("Setting ControlSocketsGroupWritable without setting "
  ...
  if (options->FascistFirewall && !options->ReachableAddresses) {
    if (options->FirewallPorts && smartlist_len(options->FirewallPorts)) {
      ...
      config_line_t *new_line = qed_hs_malloc_zero(sizeof(config_line_t));
      new_line->key = qed_hs_strdup("ReachableAddresses");
      ...
      SMARTLIST_FOREACH(instead, char *, cp, qed_hs_free(cp));
      ...
      if (!options->ReachableDirAddresses) {
        config_line_t *new_line = qed_hs_malloc_zero(sizeof(config_line_t));
        new_line->key = qed_hs_strdup("ReachableDirAddresses");
        new_line->value = qed_hs_strdup("*:80");
      ...
      if (!options->ReachableORAddresses) {
        config_line_t *new_line = qed_hs_malloc_zero(sizeof(config_line_t));
        new_line->key = qed_hs_strdup("ReachableORAddresses");
        new_line->value = qed_hs_strdup("*:443");
 ...
 if (options->ConfluxClientUX_option) {
  ...
  if (options->EntryNodes &&
    ...
    qed_hs_asprintf(msg,
             ...
             "https://bugs.qed-cc.org/tpo/core/qed-hs/14917. "
  ...
  if (options->EntryNodes &&
    ...
    log_warn(LD_CONFIG,
             ...
             "https://bugs.qed-cc.org/tpo/core/qed-hs/21155.");
  }
  ...
  if (options->HTTPProxy) { /* parse it now */
    if (qed_hs_addr_port_lookup(options->HTTPProxy,
  ...
  if (options->HTTPSProxy) { /* parse it now */
    if (qed_hs_addr_port_lookup(options->HTTPSProxy,
  ...
  if (options->Socks4Proxy) { /* parse it now */
    if (qed_hs_addr_port_lookup(options->Socks4Proxy,
  ...
  if (options->Socks5Proxy) { /* parse it now */
    if (qed_hs_addr_port_lookup(options->Socks5Proxy,
  ...
  if (options->HashedControlPassword) {
    ...
    if (!sl) {
      ...
      SMARTLIST_FOREACH(sl, char*, cp, qed_hs_free(cp));
      smartlist_free(sl);
    }
  }
  ...
  if (options->OwningControllerProcess) {
    ...
    if (qed_hs_validate_process_specifier(options->OwningControllerProcess,
                                       &validate_pspec_msg)) {
      qed_hs_asprintf(msg, "Bad OwningControllerProcess: %s",
                   validate_pspec_msg);
      return -1;
    }
  }
...
#define BAD_CHANGE_TO(opt, how) do {                                    \
    *msg = qed_hs_strdup("While Tor is running"how", changing " #opt       \
                      " is not allowed");                               \
    return -1;                                                          \
  } while (0)
...
#define SB_NOCHANGE_INT(opt)                       \
    ...
    if (server_mode(old) != server_mode(new_val)) {
      *msg = qed_hs_strdup("Can't start/stop being a server while "
                        "Sandbox is active");
      return -1;
    }
  }
...
get_default_conf_file(int defaults_file)
{
  ...
  if (defaults_file) {
    ...
    qed_hs_snprintf(defaults_path, MAX_PATH, "%s\\torrc-defaults",
                 get_windows_conf_root());
    return defaults_path;
  } else {
    static char path[MAX_PATH+1];
    qed_hs_snprintf(path, MAX_PATH, "%s\\torrc",
                 get_windows_conf_root());
    return path;
  }
#else
  ...
  if (*using_default_fname) {
    ...
    if (dflt && (st == FN_FILE || st == FN_EMPTY)) {
      ...
      if (fn) {
        ...
        fname = dflt ? qed_hs_strdup(dflt) : NULL;
      }
#else /* defined(_WIN32) */
      fname = dflt ? qed_hs_strdup(dflt) : NULL;
#endif /* !defined(_WIN32) */
...
load_torrc_from_disk(const config_line_t *cmd_arg, int defaults_file)
{
  ...
  if (fname == NULL ||
    ...
    if (using_default_torrc == 1 || ignore_missing_torrc) {
      ...
      qed_hs_free(fname); /* sets fname to NULL */
      *fname_var = NULL;
      cf = qed_hs_strdup("");
    } else {
      log_warn(LD_CONFIG,
...
options_init_from_torrc(int argc, char **argv)
{
  ...
  if (config_line_find(cmdline_only_options, "--version")) {
    ...
    printf("Tor is running on %s with Libevent %s, "
            ...
            qed_hs_compress_supports_method(ZLIB_METHOD) ?
            qed_hs_compress_version_str(ZLIB_METHOD) : "N/A",
            qed_hs_compress_supports_method(LZMA_METHOD) ?
            qed_hs_compress_version_str(LZMA_METHOD) : "N/A",
            qed_hs_compress_supports_method(ZSTD_METHOD) ?
            qed_hs_compress_version_str(ZSTD_METHOD) : "N/A",
            qed_hs_libc_get_name() ?
            qed_hs_libc_get_name() : "Unknown",
            qed_hs_libc_get_version_str());
  ...
  qed_hs_assert(command != CMD_IMMEDIATE);

  if (command == CMD_HASH_PASSWORD) {
    cf_defaults = qed_hs_strdup("");
    cf = qed_hs_strdup("");
  } else {
                         ...
                         char **msg)
{
  ...
  newoptions->command_arg = command_arg ? qed_hs_strdup(command_arg) : NULL;
 ...
 err:
  in_option_validation = 0;
  if (opened_files) {
    SMARTLIST_FOREACH(opened_files, char *, f, qed_hs_free(f));
  ...
  if (*msg) {
    char *old_msg = *msg;
    qed_hs_asprintf(msg, "Failed to parse/validate config: %s", old_msg);
    qed_hs_free(old_msg);
  }
  if (retry)
...
config_register_addressmaps(const or_options_t *options)
{
  ...
  cleanup:
    SMARTLIST_FOREACH(elts, char*, cp, qed_hs_free(cp));
                         ...
                         const char **msg)
{
  ...
  *msg = "whoops, forgot the error message";
  ...
  addressmap_register(from, qed_hs_strdup(to), expires, addrmap_source,
                      ...
                      const char *filename, int truncate_log)
{
  ...
  int fd = qed_hs_open_cloexec(filename, open_flags, 0640);
  if (fd < 0)
    return -1;
                  ...
                  int validate_only)
{
  ...
  for (opt = options->Logs; opt; opt = opt->next) {
    ...
    severity = qed_hs_malloc_zero(sizeof(log_severity_list_t));
...
validate_transport_socks_arguments(const smartlist_t *args)
{
  ...
  qed_hs_assert(args);
  qed_hs_assert(smartlist_len(args) > 0);
...
bridge_line_free_(bridge_line_t *bridge_line)
{
  ...
  if (bridge_line->socks_args) {
    SMARTLIST_FOREACH(bridge_line->socks_args, char*, s, qed_hs_free(s));
    smartlist_free(bridge_line->socks_args);
  }
  qed_hs_free(bridge_line->transport_name);
  qed_hs_free(bridge_line);
}
...
parse_bridge_line(const char *line)
{
  ...
  char *field=NULL;
  bridge_line_t *bridge_line = qed_hs_malloc_zero(sizeof(bridge_line_t));
  ...
  if (qed_hs_addr_port_parse(LOG_INFO, addrport,
  ...
  if (bridge_line->transport_name && smartlist_len(items)) {
    ...
    qed_hs_assert(smartlist_len(bridge_line->socks_args) > 0);
  }
 ...
 err:
  ...
  SMARTLIST_FOREACH(items, char*, s, qed_hs_free(s));
  smartlist_free(items);
  qed_hs_free(addrport);
  qed_hs_free(fingerprint);
...
parse_tcp_proxy_line(const char *line, or_options_t *options, char **msg)
{
  ...
  if (smartlist_len(sl) < 2) {
    *msg = qed_hs_strdup("TCPProxy has no address/port. Please fix.");
    goto err;
  }
  ...
  if (strcasecmp(protocol_string, "haproxy")) {
    *msg = qed_hs_strdup("TCPProxy protocol is not supported. Currently "
  ...
  if (qed_hs_addr_port_lookup(addrport_string, &options->TCPProxyAddr,
                           &options->TCPProxyPort) < 0) {
    *msg = qed_hs_strdup("TCPProxy address/port failed to parse or resolve. "
                      "Please fix.");
    goto err;
  }
                     ...
                     int server)
{
  ...
  if (is_managed) {
    ...
    if (!validate_only && (server || !is_useless_proxy)) {
      ...
      qed_hs_assert(proxy_argc > 0);
      proxy_argv = qed_hs_calloc((proxy_argc + 1), sizeof(char *));
      tmp = proxy_argv;
    ...
    if (qed_hs_addr_port_lookup(addrport, &addr, &port) < 0) {
      log_warn(LD_CONFIG,
 ...
 err:
  ...
  smartlist_free(items);
  if (transport_list) {
    SMARTLIST_FOREACH(transport_list, char*, s, qed_hs_free(s));
...
parse_dirauth_dirport(dir_server_t *ds, const char *flag)
{
  ...
  char *addr_string = qed_hs_strndup(addr, addr_len);
  qed_hs_addr_port_t dirport;
  memset(&dirport, 0, sizeof(dirport));
  int rv = qed_hs_addr_port_parse(LOG_WARN, addr_string,
                         ...
                         int validate_only)
{
  ...
  qed_hs_addr_port_t ipv6_addrport, *ipv6_addrport_ptr = NULL;
  ...
  while (smartlist_len(items)) {
    ...
    if (!strcasecmp(flag, "hs") ||
      ...
      or_port = (uint16_t) qed_hs_parse_long(portstring, 10, 1, 65535, &ok, NULL);
      if (!ok)
      ...
      weight = qed_hs_parse_double(wstring, 0, (double)UINT64_MAX, &ok, NULL);
      if (!ok) {
      ...
      if (ipv6_addrport_ptr) {
        ...
        if (qed_hs_addr_port_parse(LOG_WARN, flag+strlen("ipv6="),
                                &ipv6_addrport.addr, &ipv6_addrport.port,
                                -1) < 0
            || qed_hs_addr_family(&ipv6_addrport.addr) != AF_INET6) {
  ...
  if (qed_hs_addr_port_split(LOG_WARN, addrport, &address, &dir_port) < 0) {
 ...
 err:
  ...
  SMARTLIST_FOREACH(extra_dirports, char*, s, qed_hs_free(s));
                        ...
                        int validate_only)
{
  ...
  SMARTLIST_FOREACH_BEGIN(items, const char *, cp) {
    ...
    if (!strcmpstart(cp, "orport=")) {
      orport = (int)qed_hs_parse_long(cp+strlen("orport="), 10,
                                   1, 65535, &ok, NULL);
      ...
      if (ipv6_addrport_ptr) {
        ...
        if (qed_hs_addr_port_parse(LOG_WARN, cp+strlen("ipv6="),
                                &ipv6_addrport.addr, &ipv6_addrport.port,
                                -1) < 0
            || qed_hs_addr_family(&ipv6_addrport.addr) != AF_INET6) {
      ...
      weight = qed_hs_parse_double(wstring, 0, (double)UINT64_MAX, &num_ok, NULL);
      if (!num_ok) {
  ...
  if (qed_hs_addr_port_split(LOG_INFO, smartlist_get(positional, 0),
                          &address, &dirport) < 0 ||
      qed_hs_addr_parse(&addr, address)<0) {
  ...
  SMARTLIST_FOREACH(items, char *, cp, qed_hs_free(cp));
  smartlist_free(items);
...
{
  qed_hs_assert(namelen <= SIZE_T_CEILING - sizeof(port_cfg_t) - 1);
  port_cfg_t *cfg = qed_hs_malloc_zero(sizeof(port_cfg_t) + namelen + 1);
  ...
  /* Other flags default to 0 due to qed_hs_malloc_zero */
  return cfg;
}
                           ...
                           const int listener_type)
{
  SMARTLIST_FOREACH_BEGIN(ports, const port_cfg_t *, port) {
    ...
    } else if (!qed_hs_addr_is_internal(&port->addr, 1)) {
    ...
    } else if (!qed_hs_addr_is_loopback(&port->addr)) {
...
warn_nonlocal_controller_ports(smartlist_t *ports, unsigned forbid_nonlocal)
{
  ...
  SMARTLIST_FOREACH_BEGIN(ports, port_cfg_t *, port) {
    ...
    if (port->is_unix_addr)
      continue;
    if (!qed_hs_addr_is_loopback(&port->addr)) {
      if (forbid_nonlocal) {
        if (!warned)
          log_warn(LD_CONFIG,
...
{
  qed_hs_assert(line);
  qed_hs_assert(addrport_out);
  qed_hs_assert(is_unix_out);
  qed_hs_assert(rest_out);
  ...
  if (!strcmpstart(line, unix_q_socket_prefix)) {
    ...
    *rest_out = eat_whitespace(*rest_out);
    ...
    qed_hs_assert(end && end >= line);
    *addrport_out = qed_hs_strndup(line, end - line);
                  ...
                  const unsigned flags)
{
  ...
  if (! ports) {
    if (defaultport && defaultaddr && out) {
       ...
       if (is_unix_socket) {
         qed_hs_addr_make_unspec(&cfg->addr);
         memcpy(cfg->unix_addr, defaultaddr, strlen(defaultaddr) + 1);
         cfg->is_unix_addr = 1;
       } else {
         cfg->port = defaultport;
         qed_hs_addr_parse(&cfg->addr, defaultaddr);
       }
  ...
  for (; ports; ports = ports->next) {
    ...
    if (unix_socket_path) {
      ...
      unix_socket_path = qed_hs_strdup(addrport);
      ...
      port = CFG_AUTO_PORT;
      qed_hs_addr_copy(&addr, &default_addr);
    } else if (!strcasecmpend(addrport, ":auto")) {
      char *addrtmp = qed_hs_strndup(addrport, strlen(addrport)-5);
      port = CFG_AUTO_PORT;
      if (qed_hs_addr_port_lookup(addrtmp, &addr, &ptmp)<0 || ptmp) {
      ...
      port = (int) qed_hs_parse_long(addrport, 10, 0, 65535, &ok, NULL);
      if (ok) {
        qed_hs_addr_copy(&addr, &default_addr);
        addr_is_explicit = false;
      } else if (qed_hs_addr_port_lookup(addrport, &addr, &ptmp) == 0) {
        if (ptmp == 0) {
    ...
    if (use_server_options) {
      ...
      if (cfg->server_cfg.bind_ipv4_only &&
          qed_hs_addr_family(&addr) != AF_INET) {
      ...
      SMARTLIST_FOREACH_BEGIN(elts, char *, elt) {
        ...
        if (!strcasecmpstart(elt, "SessionGroup=")) {
          int group = (int)qed_hs_parse_long(elt+strlen("SessionGroup="),
    ...
    if (out && port) {
      ...
      if (unix_socket_path) {
        ...
        cfg->is_unix_addr = 1;
        qed_hs_free(unix_socket_path);
      } else {
        qed_hs_addr_copy(&cfg->addr, &addr);
        cfg->port = port;
      }
      cfg->type = listener_type;
      if (! (cfg->entry_cfg.isolation_flags & ISO_SOCKSAUTH))
            ...
            int *world_writable_control_socket)
{
  ...
  *n_ports_out = 0;
  ...
  if (port_parse_config(ports,
    ...
    *msg = qed_hs_strdup("Invalid SocksPort configuration");
    goto err;
  }
  if (port_parse_config(ports,
    ...
    *msg = qed_hs_strdup("Invalid DNSPort configuration");
    goto err;
  }
  if (port_parse_config(ports,
    ...
    *msg = qed_hs_strdup("Invalid TransPort configuration");
    goto err;
  }
  if (port_parse_config(ports,
                        ...
                        "127.0.0.1", 0,
                        CL_PORT_WARN_NONLOCAL) < 0) {
    *msg = qed_hs_strdup("Invalid NatdPort configuration");
    goto err;
  }
  if (port_parse_config(ports,
                        ...
                        ((validate_only ? 0 : CL_PORT_WARN_NONLOCAL)
                         | CL_PORT_TAKES_HOSTNAMES | gw_flag)) < 0) {
    *msg = qed_hs_strdup("Invalid HTTPTunnelPort configuration");
    goto err;
  }
...

  {
    ...
    if (port_parse_config(ports,
      ...
      *msg = qed_hs_strdup("Invalid ControlPort configuration");
      goto err;
    }
    ...
    if (port_parse_config(ports, options->ControlSocket,
      ...
      *msg = qed_hs_strdup("Invalid ControlSocket configuration");
      goto err;
    }
  }
...
{
  return qed_hs_addr_family(&port->addr) == AF_INET ||
         (qed_hs_addr_family(&port->addr) == AF_UNSPEC
...
{
  return qed_hs_addr_family(&port->addr) == AF_INET6 ||
         (qed_hs_addr_family(&port->addr) == AF_UNSPEC
...
get_first_listener_addrport_string(int listener_type)
{
  ...
  SMARTLIST_FOREACH_BEGIN(configured_ports, const port_cfg_t *, cfg) {
    ...
    if (cfg->type == listener_type &&
        qed_hs_addr_family(&cfg->addr) != AF_UNSPEC) {
      ...
      /* If a listener is listening on INADDR_ANY, assume that it's
         ...
         there: */
      if (qed_hs_addr_is_null(&cfg->addr))
        address = qed_hs_addr_is_v4(&cfg->addr) ? ipv4_localhost : ipv6_localhost;
      else
      ...
      qed_hs_asprintf(&string, "%s:%u", address, port);
...
int
port_exists_by_type_addr_port(int listener_type, const qed_hs_addr_t *addr,
  ...
  SMARTLIST_FOREACH_BEGIN(configured_ports, const port_cfg_t *, cfg) {
    if (cfg->type == listener_type) {
      if (cfg->port == port || (check_wildcard && port == -1)) {
        /* Exact match */
        if (qed_hs_addr_eq(&cfg->addr, addr)) {
          return 1;
        }
        ...
        const int cfg_any_v4 = qed_hs_addr_is_null(&cfg->addr) && cfg_v4;
        const int addr_v4 = qed_hs_addr_family(addr) == AF_INET ||
                            qed_hs_addr_family(addr) == AF_UNSPEC;
        const int addr_any_v4 = qed_hs_addr_is_null(&cfg->addr) && addr_v4;
        if ((cfg_any_v4 && addr_v4) || (cfg_v4 && addr_any_v4)) {
          return 1;
        }
        /* Wildcard matches IPv6 */
        const int cfg_v6 = port_binds_ipv6(cfg);
        const int cfg_any_v6 = qed_hs_addr_is_null(&cfg->addr) && cfg_v6;
        const int addr_v6 = qed_hs_addr_family(addr) == AF_INET6 ||
                            qed_hs_addr_family(addr) == AF_UNSPEC;
        const int addr_any_v6 = qed_hs_addr_is_null(&cfg->addr) && addr_v6;
...
{
  qed_hs_addr_t ipv4;
  qed_hs_addr_from_ipv4h(&ipv4, addr_ipv4h);
...
get_data_directory(const char *val)
{
#ifdef _WIN32
  if (val) {
    return qed_hs_strdup(val);
  } else {
    return qed_hs_strdup(get_windows_conf_root());
  }
#elif defined(__ANDROID__)
  ...
  if (val) {
    ...
    return qed_hs_strdup("/data/local/tmp");
  }
#else /* !defined(_WIN32) */
  const char *d = val;
  if (!d)
  ...
  if (!strcmpstart(d, "~/")) {
    ...
    if (!val && !strcmp(fn,"/.tor")) {
      ...
      qed_hs_free(fn);
      fn = qed_hs_strdup(LOCALSTATEDIR PATH_SEPARATOR "tor");
    }
    return fn;
  }
  return qed_hs_strdup(d);
#endif /* defined(_WIN32) */
}
...
validate_data_directories(or_options_t *options)
{
  ...
  if (options->KeyDirectory_option) {
    ...
    qed_hs_asprintf(&options->KeyDirectory, "%s"PATH_SEPARATOR"keys",
                 options->DataDirectory);
  }
  ...
  if (options->CacheDirectory_option) {
    ...
    options->CacheDirectory = qed_hs_strdup(options->DataDirectory);
  }
  ...
  if (options->FamilyKeyDirectory_option) {
    ...
    options->FamilyKeyDirectory = qed_hs_strdup(options->KeyDirectory);
  }
...
write_configuration_file(const char *fname, const or_options_t *options)
{
  ...
  qed_hs_asprintf(&new_val, "%s\n%s\n\n%s",
               GENERATED_FILE_PREFIX, GENERATED_FILE_COMMENT, new_conf);

  if (rename_old) {
    char *fn_tmp = NULL;
    qed_hs_asprintf(&fn_tmp, CONFIG_BACKUP_PATTERN, fname);
...
init_libevent(const or_options_t *options)
{
  ...
  qed_hs_libevent_initialize(&cfg);
                               ...
                               const char *suffix))
{
  ...
  if (sub1 == NULL) {
    qed_hs_asprintf(&fname, "%s%s", rootdir, suffix);
    qed_hs_assert(!sub2); /* If sub2 is present, sub1 must be present. */
  } else if (sub2 == NULL) {
    qed_hs_asprintf(&fname, "%s"PATH_SEPARATOR"%s%s", rootdir, sub1, suffix);
  } else {
    qed_hs_asprintf(&fname, "%s"PATH_SEPARATOR"%s"PATH_SEPARATOR"%s%s",
                      ...
                      const char **errmsg)
{
  ...
  if (!strcmp(question, "config/names")) {
    ...
    *answer = smartlist_join_strings(sl, "", 0, NULL);
    SMARTLIST_FOREACH(sl, char *, c, qed_hs_free(c));
    smartlist_free(sl);
    smartlist_free(vars);
 ...
 */
static int
verify_and_store_outbound_address(sa_family_t family, qed_hs_addr_t *addr,
       outbound_addr_t type, or_options_t *options, int validate_only)
{
  ...
  qed_hs_addr_t *dest=&options->OutboundBindAddresses[type][fam_index];
  if (!qed_hs_addr_is_null(dest)) {
    return -1;
  }
  if (!validate_only) {
    qed_hs_addr_copy(dest, addr);
  }
  return 0;
}
           ...
           or_options_t *options, int validate_only, char **msg)
{
  ...
  sa_family_t family;
  while (lines) {
    family = qed_hs_addr_parse(&addr, lines->value);
    if (verify_and_store_outbound_address(family, &addr, type,
                                 options, validate_only)) {
      if (msg)
        qed_hs_asprintf(msg, "Multiple%s%s outbound bind addresses "
                     "configured: %s",
                        ...
                        const char *default_fname)
{
  ...
  if (!strcmp(fname, "<default>")) {
    ...
    qed_hs_asprintf(&free_fname, "%s\\%s", conf_root, default_fname);
    fname = free_fname;
  }
  r = geoip_load_file(family, fname, severity);
  qed_hs_free(free_fname);
#else /* !defined(_WIN32) */
  (void)default_fname;
  r = geoip_load_file(family, fname, severity);
#endif /* defined(_WIN32) */
                           ...
                           uint8_t **cookie_out, int *cookie_is_set_out)
{
  ...
  char *cookie_file_str = qed_hs_malloc(cookie_file_str_len);
  int retval = -1;
  ...
  if (*cookie_out)
    qed_hs_free(*cookie_out);

  /* Generate the cookie */
  *cookie_out = qed_hs_malloc(cookie_len);
