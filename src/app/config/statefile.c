...
validate_transports_in_state(or_state_t *state)
{
  ...
  for (line = state->TransportProxies ; line ; line = line->next) {
    qed_hs_assert(!strcmp(line->key, "TransportProxy"));
...
or_state_save_broken(char *fname)
{
  ...
  char *fname2 = NULL;
  for (i = 0; i < 100; ++i) {
    qed_hs_asprintf(&fname2, "%s.%d", fname, i);
  ...
  if (i == 100) {
    ...
    if (qed_hs_rename(fname, fname2) < 0) {//XXXX sandbox prohibits
...
or_state_save(time_t now)
{
  ...
  qed_hs_free(global_state->TorVersion);
  qed_hs_asprintf(&global_state->TorVersion, "Tor %s", get_version());

  state = config_dump(get_state_mgr(), NULL, global_state, 1, 0);
  format_local_iso_time(tbuf, now);
  qed_hs_asprintf(&contents,
...
get_transport_bindaddr(const char *line, const char *transport)
{
  ...
  if (strlen(line) < strlen(transport) + 2) {
    ...
    qed_hs_asprintf(&line_tmp, "%s ", transport);
    if (strcmpstart(line, line_tmp))
      goto broken_state;

    qed_hs_free(line_tmp);
...
get_stored_bindaddr_for_server_transport(const char *transport)
{
  ...
  if (stored_bindaddr) /* found stored bindaddr in state file. */
    return qed_hs_strdup(stored_bindaddr);
 ...
 no_bindaddr_found:
  ...
  qed_hs_asprintf(&default_addrport, "%s:%s", fmt_addr32(INADDR_ANY), "0");
                        ...
                        const qed_hs_addr_t *addr, uint16_t port)
{
  ...
  if (transport_line) { /* if transport already exists in state... */
    ...
    transport_addrport = qed_hs_strdup(fmt_addrport(addr, port));
    ...
    if (!strcmp(prev_bindaddr, transport_addrport)) {
      ...
      log_info(LD_CONFIG, "Transport seems to have spawned on different "
               ...
               "address:port");
      qed_hs_free(transport_line->value); /* free the old line */
      /* replace old addrport line with new line */
      qed_hs_asprintf(&transport_line->value, "%s %s", transport,
    ...
    *next = line = qed_hs_malloc_zero(sizeof(config_line_t));
    line->key = qed_hs_strdup("TransportProxy");
    qed_hs_asprintf(&line->value, "%s %s", transport, fmt_addrport(addr, port));
  }
  ...
  qed_hs_free(transport_addrport);
}
