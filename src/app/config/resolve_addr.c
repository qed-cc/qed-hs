...
static qed_hs_addr_t last_resolved_addrs[] =
  { QED_HS_ADDR_NULL, QED_HS_ADDR_NULL, QED_HS_ADDR_NULL };
...
static qed_hs_addr_t last_suggested_addrs[] =
  { QED_HS_ADDR_NULL, QED_HS_ADDR_NULL, QED_HS_ADDR_NULL };
 ...
 * qed_hs_addr_is_null() to confirm). */
void
resolved_addr_get_suggested(int family, qed_hs_addr_t *addr_out)
{
  qed_hs_addr_copy(addr_out, &last_suggested_addrs[af_to_idx(family)]);
}

/** Set the last suggested address into our cache. This is called when we get
 * a new NETINFO cell from a trusted source. */
void
resolved_addr_set_suggested(const qed_hs_addr_t *addr)
{
  if (BUG(qed_hs_addr_family(addr) != AF_INET &&
  ...
  const int idx = af_to_idx(qed_hs_addr_family(addr));
  if (qed_hs_addr_is_null(&last_resolved_addrs[idx]) &&
      !qed_hs_addr_eq(&last_suggested_addrs[idx], addr)) {
    log_notice(LD_CONFIG, "External address seen and suggested by a "
                          "directory authority: %s", fmt_addr(addr));
  }
  qed_hs_addr_copy(&last_suggested_addrs[idx], addr);
}
 ...
 * qed_hs_addr_is_null()). */
void
resolved_addr_get_last(int family, qed_hs_addr_t *addr_out)
{
  qed_hs_addr_copy(addr_out, &last_resolved_addrs[af_to_idx(family)]);
}
...
{
  qed_hs_addr_make_null(&last_resolved_addrs[af_to_idx(family)], family);
}
 ...
 */
static int
address_can_be_used(const qed_hs_addr_t *addr, const or_options_t *options,
                    int warn_severity, const bool explicit_ip)
{
  ...
  if (options->PublishServerDescripqed_hs_ == NO_DIRINFO &&
      (options->AssumeReachable ||
       (qed_hs_addr_family(addr) == AF_INET6 && options->AssumeReachableIPv6))) {
    goto allow;
  }
...
get_address_from_config(const or_options_t *options, int warn_severity,
                        ...
                        char **hostname_out, qed_hs_addr_t *addr_out)
{
  int ret;
  ...
  qed_hs_assert(method_out);
  qed_hs_assert(hostname_out);
  ...
  *method_out = RESOLVED_ADDR_NONE;
  ...
  for (const config_line_t *cfg = options->Address; cfg != NULL;
    ...
    af = qed_hs_addr_parse(&addr, cfg->value);
    if (af == family) {
     ...
     * do a DNS lookup. */
    if (!qed_hs_addr_lookup(cfg->value, family, &addr)) {
      qed_hs_addr_copy(addr_out, &addr);
      *method_out = RESOLVED_ADDR_RESOLVED;
      if (*hostname_out) {
        qed_hs_free(*hostname_out);
      }
      *hostname_out = qed_hs_strdup(cfg->value);
      explicit_ip = false;
      num_valid_addr++;
      continue;
    } else {
                          ...
                          char **hostname_out, qed_hs_addr_t *addr_out)
{
  ...
  *method_out = RESOLVED_ADDR_NONE;
  ...
  if (qed_hs_gethostname(hostname, sizeof(hostname)) < 0) {
    log_fn(warn_severity, LD_NET, "Error obtaining local hostname");
    /* Unable to obtain the local hostname is a fatal error. */
    return FN_RET_BAIL;
  }
  if (qed_hs_addr_lookup(hostname, family, addr_out)) {
  ...
  *hostname_out = qed_hs_strdup(hostname);
 ...
 */
void
resolved_addr_set_last(const qed_hs_addr_t *addr,
                       ...
                       const char *hostname_used)
{
  ...
  int idx = af_to_idx(qed_hs_addr_family(addr));
  if (idx == IDX_NULL) {
  ...
  if (qed_hs_addr_eq(last_resolved, addr)) {
    return;
  }
  ...
  qed_hs_addr_copy(last_resolved, addr);
...
find_my_address(const or_options_t *options, int family, int warn_severity,
                qed_hs_addr_t *addr_out, resolved_addr_method_t *method_out,
                char **hostname_out)
{
  resolved_addr_method_t method_used = RESOLVED_ADDR_NONE;
  char *hostname_used = NULL;
  qed_hs_addr_t my_addr;
 ...
 found:
  ...
  if (hostname_out) {
    *hostname_out = hostname_used;
    ...
    qed_hs_free(hostname_used);
  }

  qed_hs_addr_copy(addr_out, &my_addr);
  return true;
}
 ...
 */
MOCK_IMPL(bool,
is_local_to_resolve_addr, (const qed_hs_addr_t *addr))
{
  const int family = qed_hs_addr_family(addr);
  const qed_hs_addr_t *last_resolved_addr =
  ...
  case AF_INET:
     ...
     * qed_hs_addr_is_internal. */
    return qed_hs_addr_compare_masked(addr, last_resolved_addr, 24,
                                   CMP_SEMANTIC) == 0;
  case AF_INET6:
    ...
    return qed_hs_addr_compare_masked(addr, last_resolved_addr, 48,
                                   CMP_SEMANTIC) == 0;
    break;
  default:
...
{
  qed_hs_addr_make_unspec(&last_suggested_addrs[af_to_idx(family)]);
}
