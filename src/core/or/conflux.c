...
conflux_decide_circ_minrtt(const conflux_t *cfx)
{
  ...
  qed_hs_assert(CONFLUX_NUM_LEGS(cfx));
...
conflux_queue_cmp(const void *a, const void *b)
{
  ...
  qed_hs_assert(cell_a);
  qed_hs_assert(cell_b);
...
circuit_ccontrol(const circuit_t *circ)
{
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    qed_hs_assert(CONST_TO_ORIGIN_CIRCUIT(circ)->cpath);
    qed_hs_assert(CONST_TO_ORIGIN_CIRCUIT(circ)->cpath->prev);
                          ...
                          crypt_path_t *layer_hint, const relay_msg_t *msg)
{
  ...
  if (leg->last_seq_recv == cfx->last_seq_delivered + 1) {
    ...
    conflux_msg_t *c_msg = qed_hs_malloc_zero(sizeof(conflux_msg_t));
