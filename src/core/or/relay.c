                           ...
                           cell_direction_t cell_direction)
{
  ...
  qed_hs_assert(cell);
  qed_hs_assert(circ);
  qed_hs_assert(cell_direction == CELL_DIRECTION_OUT ||
  ...
  if (!chan) {
    ...
    if (! CIRCUIT_IS_ORIGIN(circ) &&
      ...
      qed_hs_assert(circ->purpose == CIRCUIT_PURPOSE_REND_ESTABLISHED);
      qed_hs_assert(splice_->base_.purpose == CIRCUIT_PURPOSE_REND_ESTABLISHED);
                           ...
                           const char *filename, int lineno))
{
  ...
  if (cell_direction == CELL_DIRECTION_OUT) {
    ...
    ocirc->n_written_circ_bw = qed_hs_add_u32_nowrap(ocirc->n_written_circ_bw,
                                                  CELL_PAYLOAD_SIZE);
...
relay_command_to_string(uint8_t command)
{
  ...
  switch (command) {
    ...
    default:
      qed_hs_snprintf(buf, sizeof(buf), "Unrecognized relay command %u",
                   (unsigned)command);
      return buf;
  }
}
      ...
      case END_STREAM_REASON_EXITPOLICY: {
        ...
        if (msg->length >= 5) {
          ...
          if (msg->length == 5 || msg->length == 9) {
            qed_hs_addr_from_ipv4n(&addr, get_uint32(msg->body + 1));
            if (msg->length == 9)
              ttl = (int)ntohl(get_uint32(msg->body + 5));
          } else if (msg->length == 17 || msg->length == 21) {
            qed_hs_addr_from_ipv6_bytes(&addr, msg->body + 1);
            if (msg->length == 21)
          ...
          {
            char new_addr[QED_HS_ADDR_BUF_LEN];
            qed_hs_addr_to_str(new_addr, &addr, sizeof(new_addr), 1);
        ...
        if (conn->chosen_exit_optional ||
          ...
          qed_hs_free(conn->chosen_exit_name); /* clears it */
        }
                                           ...
                                           const qed_hs_addr_t *addr)
{
  ...
  if (node) {
    qed_hs_addr_t tmp;
    int asked_for_family = qed_hs_addr_parse(&tmp, conn->socks_request->address);
    if (family == AF_UNSPEC) {
      make_reject_all = 1;
...
static void
remap_event_helper(entry_connection_t *conn, const qed_hs_addr_t *new_addr)
{
  qed_hs_addr_to_str(conn->socks_request->address, new_addr,
                  sizeof(conn->socks_request->address),
                  1);
 ...
 * is set to -1). */
STATIC int
connected_cell_parse(const relay_msg_t *msg, qed_hs_addr_t *addr_out,
                     int *ttl_out)
{
  uint32_t bytes;
  ...
  *ttl_out = -1;
  ...
  if (bytes != 0) {
    /* v4 address */
    qed_hs_addr_from_ipv4h(addr_out, bytes);
    if (msg->length >= 8) {
    ...
    if (get_uint8(payload + 4) != 6)
      return -1;
    qed_hs_addr_from_ipv6_bytes(addr_out, (payload + 5));
                    ...
                    int *errcode_out)
{
  ...
  qed_hs_assert(msg);
  qed_hs_assert(addresses_out);
  qed_hs_assert(errcode_out);
  ...
  *errcode_out = 0;
  ...
  while (remaining) {
    ...
    if (answer_type == RESOLVED_TYPE_IPV4) {
      ...
      addr = qed_hs_malloc_zero(sizeof(*addr));
      qed_hs_addr_from_ipv4n(&addr->addr, get_uint32(cp));
      cp += 4;
      ...
      if (answer_len != 16)
        goto err;
      addr = qed_hs_malloc_zero(sizeof(*addr));
      qed_hs_addr_from_ipv6_bytes(&addr->addr, cp);
      cp += 16;
      addr->ttl = ntohl(get_uint32(cp));
      cp += 4;
      smartlist_add(addrs, addr);
      ...
      addr = qed_hs_malloc_zero(sizeof(*addr));
      addr->hostname = qed_hs_memdup_nulterm(cp, answer_len);
      cp += answer_len;
      addr->ttl = ntohl(get_uint32(cp));
      cp += 4;
      smartlist_add(addrs, addr);
    ...
    qed_hs_assert(((ssize_t)remaining) >= (cp - cp_orig));
                                                ...
                                                smartlist_t *results)
{
  ...
  if (error_code) {
    qed_hs_assert(error_code == RESOLVED_TYPE_ERROR ||
  ...
  SMARTLIST_FOREACH_BEGIN(results, address_ttl_t *, addr) {
    ...
    } else if (qed_hs_addr_family(&addr->addr) == AF_INET) {
      if (!addr_ipv4 && conn->entry_cfg.ipv4_traffic) {
        addr_ipv4 = addr;
      }
    } else if (qed_hs_addr_family(&addr->addr) == AF_INET6) {
                                      ...
                                      const relay_msg_t *msg)
{
  ...
  qed_hs_assert(SOCKS_COMMAND_IS_RESOLVE(entry_conn->socks_request->command));
  ...
  if (get_options()->ClientDNSRejectInternalAddresses) {
    ...
    SMARTLIST_FOREACH_BEGIN(resolved_addresses, address_ttl_t *, addr) {
      if (addr->hostname == NULL && qed_hs_addr_is_internal(&addr->addr, 0)) {
    ...
    edge_connection_t *conn, crypt_path_t *layer_hint)
{
  ...
  if (conn->base_.type == CONN_TYPE_AP &&
    ...
    if (qed_hs_addr_family(&addr) != AF_UNSPEC) {
    ...
    qed_hs_assert(entry_conn->socks_request != NULL);
...
/* XXX add to this log_fn the exit node's nickname? */
      log_info(domain,QED_HS_SOCKET_T_FORMAT": end cell (%s) for stream %d. "
               "Removing stream.",
               conn->base_.s,
               stream_end_reason_to_string(reason),
               conn->stream_id);
...
#define RELAY_CELL_PADDING_GAP 4
  ...
  qed_hs_assert_nonfatal(package_length > 0);
  ...
  if (on_circuit->send_randomness_after_n_cells == 0) {
    ...
    qed_hs_assert_nonfatal(on_circuit->have_sent_sufficiently_random_cell);
                                  ...
                                  int *max_cells)
{
 ...
 repeat_connection_edge_package_raw_inbuf:
  ...
  log_debug(domain,QED_HS_SOCKET_T_FORMAT": Packaging %d bytes (%d waiting).",
            conn->base_.s,
...
{
  ++total_cells_allocated;
  return qed_hs_malloc_zero(sizeof(packed_cell_t));
}
...
{
  QED_HS_SIMPLEQ_INSERT_TAIL(&queue->head, cell, next);
  ++queue->n;
}
...
cell_queue_init(cell_queue_t *queue)
{
  ...
  QED_HS_SIMPLEQ_INIT(&queue->head);
}
...
{
  packed_cell_t *cell;
  while ((cell = QED_HS_SIMPLEQ_FIRST(&queue->head))) {
    QED_HS_SIMPLEQ_REMOVE_HEAD(&queue->head, next);
    packed_cell_free_unchecked(cell);
  }
  QED_HS_SIMPLEQ_INIT(&queue->head);
  queue->n = 0;
...
{
  packed_cell_t *cell = QED_HS_SIMPLEQ_FIRST(&queue->head);
  if (!cell)
    return NULL;
  QED_HS_SIMPLEQ_REMOVE_HEAD(&queue->head, next);
  --queue->n;
...
{
  destroy_cell_t *cell = QED_HS_SIMPLEQ_FIRST(&queue->head);
  if (!cell)
    return NULL;
  QED_HS_SIMPLEQ_REMOVE_HEAD(&queue->head, next);
  --queue->n;
...
destroy_cell_queue_append(destroy_cell_queue_t *queue,
                          ...
                          uint8_t reason)
{
  destroy_cell_t *cell = qed_hs_malloc_zero(sizeof(destroy_cell_t));
  cell->circid = circid;
  cell->reason = reason;
...
cell_queues_check_size(void)
{
  ...
  alloc += qed_hs_compress_get_total_allocation();
                        ...
                        const char *file, int lineno)
{
  ...
  qed_hs_assert(circuitmux_attached_circuit_direction(cmux, circ) == direction);
...
channel_flush_from_first_active_circuit, (channel_t *chan, int max))
{
  ...
  while (n_flushed < max) {
    ...
    if (destroy_queue) {
       ...
       * nonempty... */
      qed_hs_assert(destroy_queue->n > 0);
    ...
    if (circ->n_chan == chan) {
      ...
      or_circ = TO_OR_CIRCUIT(circ);
      qed_hs_assert(or_circ->p_chan == chan);
    ...
    if (get_options()->CellStatistics ||
      ...
      if (get_options()->TestingEnableCellStatsEvent) {
        ...
        testing_cell_stats_entry_t *ent =
          qed_hs_malloc_zero(sizeof(testing_cell_stats_entry_t));
        ent->command = command;
...
int
append_address_to_payload(uint8_t *payload_out, const qed_hs_addr_t *addr)
{
  uint32_t a;
  ...
  case AF_INET:
    ...
    payload_out[1] = 4;
    a = qed_hs_addr_to_ipv4n(addr);
    memcpy(payload_out+2, &a, 4);
    return 6;
  case AF_INET6:
    payload_out[0] = RESOLVED_TYPE_IPV6;
    payload_out[1] = 16;
    memcpy(payload_out+2, qed_hs_addr_to_in6_addr8(addr), 16);
    return 18;
  case AF_UNSPEC:
  default:
    return -1;
...
const uint8_t *
decode_address_from_payload(qed_hs_addr_t *addr_out, const uint8_t *payload,
                            int payload_len)
{
  if (payload_len < 2)
    return NULL;
  ...
  case RESOLVED_TYPE_IPV4:
    if (payload[1] != 4)
      return NULL;
    qed_hs_addr_from_ipv4n(addr_out, get_uint32(payload+2));
    break;
  case RESOLVED_TYPE_IPV6:
    if (payload[1] != 16)
      return NULL;
    qed_hs_addr_from_ipv6_bytes(addr_out, (payload+2));
    break;
  default:
    qed_hs_addr_make_unspec(addr_out);
    break;
...
circuit_clear_cell_queue(circuit_t *circ, channel_t *chan)
{
  ...
  if (circ->n_chan == chan) {
    ...
    qed_hs_assert(orcirc->p_chan == chan);
    queue = &orcirc->p_chan_cells;
