...
congestion_control_enabled(void)
{
  ...
  qed_hs_assert_nonfatal_once(in_main_thread());
...
{
  congestion_control_t *cc = qed_hs_malloc_zero(sizeof(congestion_control_t));
...
congestion_control_free_(congestion_control_t *cc)
{
  ...
  SMARTLIST_FOREACH(cc->sendme_pending_timestamps, uint64_t *, t, qed_hs_free(t));
...
{
  uint64_t *timestamp_ptr = qed_hs_malloc(sizeof(uint64_t));
                                      ...
                                      uint64_t curr_rtt_usec)
{
  ...
  if (!cc->ewma_rtt_usec) {
     ...
     qed_hs_assert_nonfatal(cc->cwnd <= cwnd_max);
  ...
  if (ret) {
    qed_hs_trace(TR_SUBSYS(cc), TR_EV(bdp_update), circ, cc, curr_rtt_usec);
  }
  return ret;
}
                                   ...
                                   circuit_t *circ)
{
  ...
  qed_hs_assert_nonfatal_once(cc->cc_alg == CC_ALG_VEGAS);
...
congestion_control_build_ext_request(uint8_t **msg_out, size_t *msg_len_out)
{
  ...
  request = qed_hs_malloc_zero(request_len);
                                      ...
                                      uint8_t **msg_out, size_t *msg_len_out)
{
  ...
  qed_hs_assert(our_params);
  qed_hs_assert(circ_params);
  qed_hs_assert(msg_out);
  qed_hs_assert(msg_len_out);
