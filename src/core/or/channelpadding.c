...
#define QED_HS_MSEC_PER_SEC 1000
#define QED_HS_USEC_PER_MSEC 1000

/**
 * How often do we get called by the connection housekeeping (ie: once
 * per second) */
#define QED_HS_HOUSEKEEPING_CALLBACK_MSEC 1000
/**
...
#define QED_HS_HOUSEKEEPING_CALLBACK_SLACK_MSEC 100
...
channelpadding_send_disable_command(channel_t *chan)
{
  ...
  qed_hs_assert(chan);
  qed_hs_assert(BASE_CHAN_TO_TLS(chan)->conn->link_proto >=
...
/**
 * qed_hs_timer callback function for us to send padding on an idle channel.
 *
 ...
 */
static void
channelpadding_send_padding_callback(qed_hs_timer_t *timer, void *args,
                                     const struct monotime_t *when)
{
  ...
  if (chan && CHANNEL_CAN_HANDLE_CELLS(chan)) {
    ...
    qed_hs_assert(TO_CONN(BASE_CHAN_TO_TLS(chan)->conn)->magic ==
               OR_CONNECTION_MAGIC);
...
channelpadding_schedule_padding(channel_t *chan, int in_ms)
{
  ...
  qed_hs_assert(!chan->pending_padding_callback);
  ...
  timeout.tv_sec = in_ms/QED_HS_MSEC_PER_SEC;
  timeout.tv_usec = (in_ms%QED_HS_USEC_PER_MSEC)*QED_HS_USEC_PER_MSEC;
...
channelpadding_compute_time_until_pad_for_netflow(channel_t *chan)
{
  ...
  /* If the timeout will expire before the next time we're called (1000ms
     ...
     then.
   */
  if (ms_till_pad < (QED_HS_HOUSEKEEPING_CALLBACK_MSEC +
                       QED_HS_HOUSEKEEPING_CALLBACK_SLACK_MSEC)) {
     ...
     * See https://bugs.qed-cc.org/22212 and
     * https://bugs.qed-cc.org/16585. This is a systemic problem
