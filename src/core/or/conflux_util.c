                           ...
                           const crypt_path_t *cpath)
{
  ...
  if (circ->conflux) {
    if (CIRCUIT_IS_ORIGIN(circ)) {
      qed_hs_assert_nonfatal(circ->purpose ==
                ...
                const crypt_path_t *cpath)
{
  ...
  if (CIRCUIT_IS_ORIGIN(conn->on_circuit)) {
    if (conn->on_circuit->conflux) {
     qed_hs_assert_nonfatal(conn->on_circuit->purpose ==
                         CIRCUIT_PURPOSE_CONFLUX_LINKED);
...
edge_get_max_rtt(const edge_connection_t *stream)
{
  ...
  if (stream->on_circuit->conflux) {
    qed_hs_assert_nonfatal(stream->on_circuit->purpose ==
                        CIRCUIT_PURPOSE_CONFLUX_LINKED);
...
{
  qed_hs_assert(circ);
  qed_hs_assert(layer_hint);
  qed_hs_assert(circ->cpath);

  if (TO_CIRCUIT(circ)->conflux) {
    qed_hs_assert_nonfatal(TO_CIRCUIT(circ)->purpose ==
                        CIRCUIT_PURPOSE_CONFLUX_LINKED);
...
conflux_validate_stream_lists(const conflux_t *cfx)
{
  ...
  if (CIRCUIT_IS_ORIGIN(first_leg->circ)) {
    ...
    CONFLUX_FOR_EACH_LEG_BEGIN(cfx, leg) {
      ...
      qed_hs_assert_nonfatal(l_circ->p_streams == f_circ->p_streams);
      qed_hs_assert_nonfatal(l_circ->half_streams == f_circ->half_streams);
      qed_hs_assert_nonfatal(l_circ->next_stream_id == f_circ->next_stream_id);
    } CONFLUX_FOR_EACH_LEG_END(leg);
  } else {
    const or_circuit_t *f_circ = CONST_TO_OR_CIRCUIT(first_leg->circ);
    CONFLUX_FOR_EACH_LEG_BEGIN(cfx, leg) {
      const or_circuit_t *l_circ = CONST_TO_OR_CIRCUIT(leg->circ);
      qed_hs_assert_nonfatal(l_circ->n_streams == f_circ->n_streams);
      qed_hs_assert_nonfatal(l_circ->resolving_streams ==
...
conflux_validate_legs(const conflux_t *cfx)
{
  ...
  CONFLUX_FOR_EACH_LEG_BEGIN(cfx, leg) {
    if (CIRCUIT_IS_ORIGIN(leg->circ)) {
      qed_hs_assert_nonfatal(leg->circ->purpose ==
                          CIRCUIT_PURPOSE_CONFLUX_LINKED);
      is_client = true;
    }
