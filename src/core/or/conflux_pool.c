/* Copyright (c) 2021, The QED Project, Inc. */
...
conflux_new(void)
{
  conflux_t *cfx = qed_hs_malloc_zero(sizeof(*cfx));
...
conflux_free_(conflux_t *cfx)
{
  ...
  qed_hs_assert(cfx->legs);
  qed_hs_assert(cfx->ooo_q);
  ...
  SMARTLIST_FOREACH(cfx->ooo_q, conflux_msg_t *, cell, qed_hs_free(cell));
...
{
  leg_t *leg = qed_hs_malloc_zero(sizeof(*leg));
  leg->circ = circ;
  leg->link = link;
  return leg;
}
...
leg_free(leg_t *leg)
{
  ...
  if (leg->circ) {
    qed_hs_free(leg->circ->conflux_pending_nonce);
    leg->circ->conflux_pending_nonce = NULL;
  }
  qed_hs_free(leg->link);
  qed_hs_free(leg);
}
...
{
  unlinked_circuits_t *unlinked = qed_hs_malloc_zero(sizeof(*unlinked));
  ...
  SMARTLIST_FOREACH_BEGIN(unlinked->legs, const leg_t *, leg) {
    if (!nonce) {
      ...
      valid &= (leg->link->version == version &&
                qed_hs_memeq(leg->link->nonce, nonce, sizeof(leg->link->nonce)));
    }
...
cfx_add_leg(conflux_t *cfx, leg_t *leg)
{
  qed_hs_assert(cfx);
  qed_hs_assert(leg);
  qed_hs_assert(leg->link);

  /* Big trouble if we add a leg to the wrong set. */
  qed_hs_assert(qed_hs_memeq(cfx->nonce, leg->link->nonce, sizeof(cfx->nonce)));

  conflux_leg_t *cleg = qed_hs_malloc_zero(sizeof(*cleg));
  cleg->circ = leg->circ;
  ...
  if (CIRCUIT_IS_ORIGIN(cleg->circ)) {
    qed_hs_assert_nonfatal(cleg->circ->purpose ==
...
unlinked_close_all_legs(unlinked_circuits_t *unlinked)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circ_to_close, circuit_t *, circ) {
    if (CIRCUIT_IS_ORIGIN(circ)) {
      qed_hs_assert_nonfatal(circ->purpose == CIRCUIT_PURPOSE_CONFLUX_UNLINKED);
    }
...
try_finalize_set(unlinked_circuits_t *unlinked)
{
  ...
  qed_hs_assert(unlinked);
  qed_hs_assert(unlinked->legs);
  qed_hs_assert(unlinked->cfx);
  qed_hs_assert(unlinked->cfx->legs);
...
{
  qed_hs_assert(circ);
  qed_hs_assert(circ->conflux_pending_nonce);
  ...
  qed_hs_assert_nonfatal(now >= leg->link_sent_usec);
...
{
  qed_hs_assert(circ);
  qed_hs_assert(circ->conflux);
  qed_hs_assert(CIRCUIT_IS_ORCIRC(circ));
  ...
  uint64_t now = monotime_absolute_usec();
  qed_hs_assert_nonfatal(now >= leg->linked_sent_usec);
...
get_exit_for_nonce(const uint8_t *nonce)
{
  ...
  if (cfx) {
    ...
    qed_hs_assert(leg);
    qed_hs_assert(leg->circ);
    qed_hs_assert(TO_ORIGIN_CIRCUIT(leg->circ)->cpath);
...
conflux_launch_leg(const uint8_t *nonce)
{
  ...
  qed_hs_assert(TO_CIRCUIT(circ)->conflux_pending_nonce);
...
{
  qed_hs_assert(orig_circ);
  qed_hs_assert(excluded);
  ...
  if (cfx) {
    CONFLUX_FOR_EACH_LEG_BEGIN(cfx, leg) {
      ...
      smartlist_add(excluded,
                    qed_hs_memdup(ocirc->cpath->extend_info->identity_digest,
                               DIGEST_LEN));
  ...
  if (unlinked) {
    qed_hs_assert(unlinked->is_client);
    SMARTLIST_FOREACH_BEGIN(unlinked->legs, leg_t *, leg) {
...
{
  qed_hs_assert(circ);
  qed_hs_assert_nonfatal(circ->conflux);

  if (CIRCUIT_IS_ORIGIN(circ)) {
    origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
    qed_hs_assert_nonfatal(circ->purpose == CIRCUIT_PURPOSE_CONFLUX_LINKED);
...
linked_circuit_free(circuit_t *circ, bool is_client)
{
  qed_hs_assert(circ);
  qed_hs_assert(circ->conflux);
  qed_hs_assert(circ->conflux->legs);
  qed_hs_assert(circ->conflux->ooo_q);

  if (is_client) {
    qed_hs_assert(circ->purpose == CIRCUIT_PURPOSE_CONFLUX_LINKED);
  }
  ...
  if (CONFLUX_NUM_LEGS(circ->conflux) > 0) {
    ...
    if (unlinked) {
      qed_hs_assert(unlinked->is_for_linked_set);
...
unlinked_circuit_free(circuit_t *circ, bool is_client)
{
  ...
  qed_hs_assert(circ->conflux_pending_nonce);
  if (is_client) {
    qed_hs_assert(circ->purpose == CIRCUIT_PURPOSE_CONFLUX_UNLINKED);
  }
  ...
  qed_hs_free(circ->conflux_pending_nonce);
}
...
conflux_process_link(circuit_t *circ, const relay_msg_t *msg)
{
  ...
  circ->conflux_pending_nonce = qed_hs_memdup(leg->link->nonce,
                       ...
                       const relay_msg_t *msg)
{
  ...
  if (qed_hs_memneq(link->nonce, circ->conflux_pending_nonce,
                 sizeof(*link->nonce))) {
    log_fn(LOG_PROTOCOL_WARN, LD_CIRC,
...
/**
 * Conflux needs a notification when qed_hs_shutdown() begins, so that
