...
#define QED_HS_CONGESTION_CONTROL_FLOW_PRIVATE
...
flow_control_decide_xoff(edge_connection_t *stream)
{
  ...
  if (total_buffered > buffer_limit_xoff) {
    if (!stream->xoff_sent) {
      log_info(LD_EDGE, "Sending XOFF: %"QED_HS_PRIuSZ" %d",
                 total_buffered, buffer_limit_xoff);
      qed_hs_trace(TR_SUBSYS(cc), TR_EV(flow_decide_xoff_sending), stream);
...
flow_control_decide_xon(edge_connection_t *stream, size_t n_written)
{
  ...
  qed_hs_trace(TR_SUBSYS(cc), TR_EV(flow_decide_xon), stream, n_written);
  ...
  if (!is_monotime_clock_reliable()) {
    ...
    if (!stream->drain_start_usec && total_buffered > 0) {
      log_debug(LD_EDGE, "Began edge buffering: %d %d %"QED_HS_PRIuSZ,
                 stream->ewma_rate_last_sent,
                 stream->ewma_drain_rate,
                 total_buffered);
      qed_hs_trace(TR_SUBSYS(cc), TR_EV(flow_decide_xon_drain_start),
                stream);
      ...
      if (drain_rate) {
        stream->ewma_drain_rate =
            (uint32_t)n_count_ewma(drain_rate,
                                   stream->ewma_drain_rate,
                                   xon_ewma_cnt);
        log_debug(LD_EDGE, "Updating drain rate: %d %d %"QED_HS_PRIuSZ,
                   drain_rate,
                   stream->ewma_drain_rate,
                   total_buffered);
        qed_hs_trace(TR_SUBSYS(cc), TR_EV(flow_decide_xon_drain_update),
                  stream, drain_rate);
  ...
  if (!stream->xoff_sent) {
    if (stream_drain_rate_changed(stream)) {
       ...
       * advisory XON */
      log_info(LD_EDGE, "Sending rate-change XON: %d %d %"QED_HS_PRIuSZ,
                 stream->ewma_rate_last_sent,
                 stream->ewma_drain_rate,
                 total_buffered);
      qed_hs_trace(TR_SUBSYS(cc), TR_EV(flow_decide_xon_rate_change), stream);
    ...
    log_info(LD_EDGE, "Sending XON: %d %d %"QED_HS_PRIuSZ,
               stream->ewma_rate_last_sent,
               stream->ewma_drain_rate,
               total_buffered);
    qed_hs_trace(TR_SUBSYS(cc), TR_EV(flow_decide_xon_partial_drain), stream);
    circuit_send_stream_xon(stream);
  }
  ...
  if (total_buffered == 0) {
    ...
    if (stream->drained_bytes >= xon_rate_bytes &&
      ...
      log_debug(LD_EDGE,
                 ...
                 stream->ewma_rate_last_sent,
                 stream->ewma_drain_rate);
      qed_hs_trace(TR_SUBSYS(cc), TR_EV(flow_decide_xon_drain_doubled), stream);
