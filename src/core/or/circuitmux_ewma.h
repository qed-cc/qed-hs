...
#ifndef QED_HS_CIRCUITMUX_EWMA_H
#define QED_HS_CIRCUITMUX_EWMA_H
...
TO_EWMA_POL_DATA(circuitmux_policy_data_t *pol)
{
  if (!pol) return NULL;
    ...
    qed_hs_assertf(pol->magic == EWMA_POL_DATA_MAGIC,
...
TO_EWMA_POL_CIRC_DATA(circuitmux_policy_circ_data_t *pol)
{
  if (!pol) return NULL;
    ...
    qed_hs_assertf(pol->magic == EWMA_POL_CIRC_DATA_MAGIC,
                "Mismatch: %"PRIu32" != %"PRIu32,
...
#endif /* !defined(QED_HS_CIRCUITMUX_EWMA_H) */
