...
extend_info_new(const char *nickname,
                ...
                bool for_exit_use)
{
  extend_info_t *info = qed_hs_malloc_zero(sizeof(extend_info_t));
  if (rsa_id_digest)
...
{
  for (int i = 0; i < EXTEND_INFO_MAX_ADDRS; ++i) {
    if (qed_hs_addr_is_unspec(&ei->orports[i].addr)) {
      qed_hs_addr_copy(&ei->orports[i].addr, addr);
                      ...
                      bool for_exit)
{
  ...
  valid_addr = qed_hs_addr_port_is_valid_ap(&ap, 0);
...
extend_info_dup(extend_info_t *info)
{
  ...
  qed_hs_assert(info);
  newinfo = qed_hs_malloc(sizeof(extend_info_t));
...
int
extend_info_addr_is_allowed(const qed_hs_addr_t *addr)
{
  qed_hs_assert(addr);

  /* Check if we have a private address and if we can extend to it. */
  if ((qed_hs_addr_is_internal(addr, 0) || qed_hs_addr_is_multicast(addr)) &&
                       ...
                       const qed_hs_addr_t *addr, uint16_t port)
{
  ...
  for (int i = 0; i < EXTEND_INFO_MAX_ADDRS; ++i) {
    const qed_hs_addr_port_t *ei_ap = &ei->orports[i];
    if (qed_hs_addr_eq(&ei_ap->addr, addr) && ei_ap->port == port)
      return true;
  }
  return false;
}
...
extend_info_get_orport(const extend_info_t *ei, int family)
{
  for (int i = 0; i < EXTEND_INFO_MAX_ADDRS; ++i) {
    if (qed_hs_addr_is_unspec(&ei->orports[i].addr))
      continue;
    if (qed_hs_addr_family(&ei->orports[i].addr) == family)
      return &ei->orports[i];
  }
  return NULL;
}
...
extend_info_pick_orport(const extend_info_t *ei)
{
  ...
  const qed_hs_addr_port_t *usable[EXTEND_INFO_MAX_ADDRS];
  int n_usable = 0;
  for (int i = 0; i < EXTEND_INFO_MAX_ADDRS; ++i) {
    const qed_hs_addr_port_t *a = &ei->orports[i];
    const int family = qed_hs_addr_family(&a->addr);
...
extend_info_any_orport_addr_is_internal(const extend_info_t *ei)
{
  ...
  for (int i = 0; i < EXTEND_INFO_MAX_ADDRS; ++i) {
    if (! qed_hs_addr_is_unspec(&ei->orports[i].addr) &&
        qed_hs_addr_is_internal(&ei->orports[i].addr, 0))
      return true;
  }
  return false;
}
