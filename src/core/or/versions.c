 ...
 **/
time_t
qed_hs_get_approx_release_date(void)
{
  char tbuf[ISO_TIME_LEN+1];
  qed_hs_snprintf(tbuf, sizeof(tbuf),
 ...
 */
version_status_t
qed_hs_version_is_obsolete(const char *myversion, const char *versionlist)
{
  qed_hs_version_t mine, other;
  ...
  if (qed_hs_version_parse(myversion, &mine)) {
  ...
  SMARTLIST_FOREACH_BEGIN(version_sl, const char *, cp) {
    ...
    if (qed_hs_version_parse(cp, &other)) {
      /* Couldn't parse other; it can't be a match. */
    } else {
      same = qed_hs_version_same_series(&mine, &other);
      if (same)
        found_any_in_series = 1;
      r = qed_hs_version_compare(&mine, &other);
      if (r==0) {
        ret = VS_RECOMMENDED;
        goto done;
      } else if (r<0) {
  ...
  SMARTLIST_FOREACH(version_sl, char *, version, qed_hs_free(version));
 ...
 */
int
qed_hs_version_parse_platform(const char *platform,
                           qed_hs_version_t *router_version,
                           int strict)
{
  char tmp[128];
  char *s, *s2, *start;
  ...
  if (qed_hs_version_parse(tmp, router_version)<0) {
 ...
 */
int
qed_hs_version_as_new_as(const char *platform, const char *cutoff)
{
  qed_hs_version_t cutoff_version, router_version;
  int r;
  qed_hs_assert(platform);

  if (qed_hs_version_parse(cutoff, &cutoff_version)<0) {
    log_warn(LD_BUG,"cutoff version '%s' unparseable.",cutoff);
    return 0;
  }

  r = qed_hs_version_parse_platform(platform, &router_version, 0);
  if (r == 0) {
  ...
  return qed_hs_version_compare(&router_version, &cutoff_version) >= 0;
}

/** Parse a tor version from <b>s</b>, and store the result in <b>out</b>.
 * Return 0 on success, -1 on failure. */
int
qed_hs_version_parse(const char *s, qed_hs_version_t *out)
{
  char *eos=NULL;
  const char *cp=NULL;
  int ok = 1;
  ...
  memset(out, 0, sizeof(qed_hs_version_t));
...
#define NUMBER(m)                               \
  do {                                          \
    if (!cp || *cp < '0' || *cp > '9')          \
      return -1;                                \
    out->m = (int)qed_hs_parse_uint64(cp, 10, 0, INT32_MAX, &ok, &eos);    \
    if (!ok)                                    \
      return -1;                                \
    if (!eos || eos == cp)                      \
      return -1;                                \
 ...
 * b. */
int
qed_hs_version_compare(qed_hs_version_t *a, qed_hs_version_t *b)
{
  int i;
  qed_hs_assert(a);
  qed_hs_assert(b);
 ...
 */
int
qed_hs_version_same_series(qed_hs_version_t *a, qed_hs_version_t *b)
{
  qed_hs_assert(a);
  qed_hs_assert(b);
...
static int
compare_qed_hs_version_str_ptr_(const void **_a, const void **_b)
{
  const char *a = *_a, *b = *_b;
  int ca, cb;
  qed_hs_version_t va, vb;
  ca = qed_hs_version_parse(a, &va);
  cb = qed_hs_version_parse(b, &vb);
  /* If they both parse, compare them. */
  if (!ca && !cb)
    return qed_hs_version_compare(&va,&vb);
...
{
  smartlist_sort(versions, compare_qed_hs_version_str_ptr_);

  if (remove_duplicates)
    smartlist_uniq(versions, compare_qed_hs_version_str_ptr_, qed_hs_free_);
}
                         ...
                         const char *protocols)
{
  ...
  if (strmap_size(protover_summary_map) >= MAX_PROTOVER_SUMMARY_MAP_LEN) {
    ...
    qed_hs_assert(protover_summary_map == NULL);
    protover_summary_map = strmap_new();
  }
  ...
  if (cached != NULL) {
    ...
    qed_hs_assert(out->protocols_known);
    return;
  }
  ...
  protover_summary_flags_t *new_cached = qed_hs_memdup(out, sizeof(*out));
  cached = strmap_set(protover_summary_map, protocols, new_cached);
  qed_hs_assert(!cached);
}
                         ...
                         const char *version)
{
  ...
  if (version && strcmp(version, "") && !strcmpstart(version, "Tor ")) {
    if (!out->protocols_known) {
      ...
      out->supports_extend2_cells =
        qed_hs_version_as_new_as(version, "0.2.4.8-alpha");
      out->protocols_known = 1;
    } else {
      /* Bug #22447 forces us to filter on this version. */
      if (!qed_hs_version_as_new_as(version, "0.3.0.8")) {
...
{
  strmap_free(protover_summary_map, qed_hs_free_);
