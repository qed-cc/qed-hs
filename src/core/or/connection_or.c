...
{
  qed_hs_assert(c->magic == OR_CONNECTION_MAGIC);
                                  ...
                                  const ed25519_public_key_t *ed_id)
{
  ...
  const int rsa_id_was_set = ! qed_hs_digest_is_zero(conn->identity_digest);
  ...
  const int rsa_changed =
    qed_hs_memneq(conn->identity_digest, rsa_digest, DIGEST_LEN);
  ...
  if (qed_hs_digest_is_zero(rsa_digest) && !new_ed_id_is_set)
    return;
                                    ...
                                    char *buf, size_t buflen)
{
  ...
  qed_hs_assert(conn->type == CONN_TYPE_OR || conn->type == CONN_TYPE_EXT_OR);

  conn_state = conn_state_to_string(conn->type, conn->state);
  qed_hs_tls_get_state_description(orconn->tls, tls_state, sizeof(tls_state));

  qed_hs_snprintf(buf, buflen, "%s with SSL state %s", conn_state, tls_state);
}
...
connection_or_report_broken_states(int severity, int domain)
{
  ...
  STRMAP_FOREACH(broken_connection_counts, state, void *, countptr) {
    broken_state_count_t *c = qed_hs_malloc(sizeof(broken_state_count_t));
  ...
  qed_hs_log(severity, domain, "%d connections have failed%s", total,
  ...
  SMARTLIST_FOREACH_BEGIN(items, const broken_state_count_t *, c) {
    if (c_sl_idx > MAX_REASONS_TO_REPORT)
      break;
    qed_hs_log(severity, domain,
        " %d connections died in state %s", (int)c->count, c->state);
  } SMARTLIST_FOREACH_END(c);

  SMARTLIST_FOREACH(items, broken_state_count_t *, c, qed_hs_free(c));
...
connection_or_event_status(or_connection_t *conn, or_conn_status_event_t tp,
                           int reason)
{
  orconn_status_msg_t *msg = qed_hs_malloc(sizeof(*msg));
...
{
  orconn_state_msg_t *msg = qed_hs_malloc(sizeof(*msg));
  ...
  if (conn->is_pt) {
    ...
    qed_hs_assert_nonfatal(conn->proxy_type != PROXY_NONE);
...
var_cell_new(uint16_t payload_len)
{
  ...
  var_cell_t *cell = qed_hs_malloc_zero(size);
...
connection_or_process_inbuf(or_connection_t *conn)
{
  ...
  switch (conn->base_.state) {
    case OR_CONN_STATE_PROXY_HANDSHAKING:
      ...
      if (ret == 1) {
        qed_hs_assert(TO_CONN(conn)->proxy_state == PROXY_CONNECTED);
  ...
  switch (conn->base_.state) {
    case OR_CONN_STATE_PROXY_HANDSHAKING:
      ...
      if (conn->proxy_type == PROXY_HAPROXY) {
        qed_hs_assert(TO_CONN(conn)->proxy_state == PROXY_HAPROXY_WAIT_FOR_FLUSH);
...
connection_or_finished_connecting(or_connection_t *or_conn)
{
  ...
  qed_hs_assert(or_conn);
  conn = TO_CONN(or_conn);
  qed_hs_assert(conn->state == OR_CONN_STATE_CONNECTING);
...
connection_or_about_to_close(or_connection_t *or_conn)
{
      ...
      if (conn->state >= OR_CONN_STATE_TLS_HANDSHAKING) {
        ...
        if (!authdir_mode_tests_reachability(options)) {
          ...
          if (reason == END_OR_CONN_REASON_TLS_ERROR && or_conn->tls) {
            warning = qed_hs_tls_get_last_error_msg(or_conn->tls);
          }
          if (warning == NULL) {
  ...
  } else if (!qed_hs_digest_is_zero(or_conn->identity_digest)) {
    connection_or_event_status(or_conn, OR_CONN_EVENT_CLOSED,
                tls_error_to_orconn_end_reason(or_conn->tls_error));
  } else {
                                     ...
                                     int started_here)
{
  ...
  if (! conn->base_.address) {
    conn->base_.address = qed_hs_strdup(fmt_addr(addr));
  }
...
connection_or_check_canonicity(or_connection_t *conn, int started_here)
{
  ...
  if (r) {
    qed_hs_addr_port_t node_ipv4_ap;
    qed_hs_addr_port_t node_ipv6_ap;
    node_get_prim_orport(r, &node_ipv4_ap);
    node_get_pref_ipv6_orport(r, &node_ipv6_ap);
    if (qed_hs_addr_eq(&conn->base_.addr, &node_ipv4_ap.addr) ||
        qed_hs_addr_eq(&conn->base_.addr, &node_ipv6_ap.addr)) {
      connection_or_set_canonical(conn, 1);
    }
    /* Choose the correct canonical address and port. */
    qed_hs_addr_port_t *node_ap;
    ...
    /* Remember the canonical addr/port so our log messages will make
       sense. */
    qed_hs_addr_port_copy(&conn->canonical_orport, node_ap);
    qed_hs_free(conn->nickname);
    conn->nickname = qed_hs_strdup(node_get_nickname(r));
  } else {
    qed_hs_free(conn->nickname);
    conn->nickname = qed_hs_malloc(HEX_DIGEST_LEN+2);
    conn->nickname[0] = '$';
                                  ...
                                  int force)
{
  ...
  if (force ||
    ...
    log_info(LD_OR,
             ...
             "(fd "QED_HS_SOCKET_T_FORMAT", %d secs old).",
  ...
  SMARTLIST_FOREACH_BEGIN(group, or_connection_t *, or_conn) {
    ...
    if (n_canonical && !or_conn->is_canonical) {
      ...
      log_info(LD_OR,
               ...
               "(fd "QED_HS_SOCKET_T_FORMAT", %d secs old).  It is not "
  ...
  SMARTLIST_FOREACH_BEGIN(group, or_connection_t *, or_conn) {
    ...
    if (or_conn != best &&
      ...
      if (best->is_canonical) {
        ...
        connection_or_mark_bad_for_new_circs(or_conn);
      } else if (qed_hs_addr_eq(&TO_CONN(or_conn)->addr,
                             &TO_CONN(best)->addr)) {
        log_info(LD_OR,
                 "Marking %s unsuitable for new circuits: "
                 "(fd "QED_HS_SOCKET_T_FORMAT", %d secs old).  We have a better "
                 "one with the "
                 "same address (fd "QED_HS_SOCKET_T_FORMAT"; %d secs old).",
                 connection_describe(TO_CONN(or_conn)),
                 or_conn->base_.s,
                 (int)(now - or_conn->base_.timestamp_created),
                 best->base_.s, (int)(now - best->base_.timestamp_created));
...
or_connect_failure_ht_hash(const or_connect_failure_entry_t *entry)
{
  ...
  switch (qed_hs_addr_family(&entry->addr)) {
  case AF_INET:
    addr_size = 4;
...
static void
or_connect_failure_init(const char *identity_digest, const qed_hs_addr_t *addr,
                        uint16_t port, or_connect_failure_entry_t *ocf)
{
  ...
  if (addr) {
    qed_hs_addr_copy(&ocf->addr, addr);
  }
  ocf->port = port;
}
...
{
  or_connect_failure_entry_t *ocf = qed_hs_malloc_zero(sizeof(*ocf));
...
connection_or_connect, (const qed_hs_addr_t *_addr, uint16_t port,
                        const char *id_digest,
                        const ed25519_public_key_t *ed_id,
                        channel_tls_t *chan))
{
  ...
  qed_hs_assert(_addr);
  qed_hs_assert(id_digest);
  qed_hs_addr_copy(&addr, _addr);
  ...
  conn = or_connection_new(CONN_TYPE_OR, qed_hs_addr_family(&addr));
  ...
  if (r == 0) {
    ...
    if (proxy_type != PROXY_NONE) {
      qed_hs_addr_copy(&addr, &proxy_addr);
      port = proxy_port;
...
connection_tls_start_handshake,(or_connection_t *conn, int receiving))
{
  ...
  qed_hs_assert(!conn->tls);
  conn->tls = qed_hs_tls_new(conn->base_.s, receiving);
  if (!conn->tls) {
    log_warn(LD_BUG,"qed_hs_tls_new failed. Closing.");
    return -1;
  }
  qed_hs_tls_set_logged_address(conn->tls,
                             connection_describe_peer(TO_CONN(conn)));

  connection_start_reading(TO_CONN(conn));
  log_debug(LD_HANDSHAKE,"starting TLS handshake on fd "QED_HS_SOCKET_T_FORMAT,
            conn->base_.s);
...
connection_tls_continue_handshake(or_connection_t *conn)
{
  ...
  qed_hs_assert(conn->base_.state == OR_CONN_STATE_TLS_HANDSHAKING);
  // log_notice(LD_OR, "Continue handshake with %p", conn->tls);
  result = qed_hs_tls_handshake(conn->tls);
  ...
  switch (result) {
    ...
    case QED_HS_TLS_DONE:
      {
        if (!qed_hs_tls_is_server(conn->tls)) {
          qed_hs_assert(conn->base_.state == OR_CONN_STATE_TLS_HANDSHAKING);
    ...
    case QED_HS_TLS_WANTWRITE:
      ...
      return 0;
    case QED_HS_TLS_WANTREAD: /* handshaking conns are *always* reading */
                                     ...
                                     const ed25519_public_key_t *ed_peer_id)
{
  ...
  const int expected_rsa_key =
    ! qed_hs_digest_is_zero(conn->identity_digest);
  ...
  const int rsa_mismatch = expected_rsa_key &&
    qed_hs_memneq(rsa_peer_id, conn->identity_digest, DIGEST_LEN);
...
{
  qed_hs_assert(connection_or_nonopen_was_started_here(conn));
...
connection_init_or_handshake_state(or_connection_t *conn, int started_here)
{
  ...
  s = conn->handshake_state = qed_hs_malloc_zero(sizeof(or_handshake_state_t));
  s->started_here = started_here ? 1 : 0;
  s->digest_sent_data = 1;
  s->digest_received_data = 1;
  if (! started_here && get_current_link_cert_cert()) {
    s->own_link_cert = qed_hs_cert_dup(get_current_link_cert_cert());
  }
...
or_handshake_state_free_(or_handshake_state_t *state)
{
  ...
  qed_hs_cert_free(state->own_link_cert);
...
connection_or_process_cells_from_inbuf(or_connection_t *conn)
{
  ...
  while (1) {
    log_debug(LD_OR,
              QED_HS_SOCKET_T_FORMAT": starting, inbuf_datalen %d "
              "(%d pending in tls object).",
              conn->base_.s,(int)connection_get_inbuf_len(TO_CONN(conn)),
              qed_hs_tls_get_pending_bytes(conn->tls));
...
connection_or_send_versions(or_connection_t *conn, int v3_plus)
{
  ...
  qed_hs_assert(conn->handshake_state &&
...
netinfo_addr_from_qed_hs_addr(const qed_hs_addr_t *qed_hs_addr)
{
  sa_family_t addr_family = qed_hs_addr_family(qed_hs_addr);
  ...
  if (addr_family == AF_INET) {
    ...
    netinfo_addr_set_addr_ipv4(netinfo_addr, qed_hs_addr_to_ipv4h(qed_hs_addr));
    ...
    const uint8_t *in6_addr = qed_hs_addr_to_in6_addr8(qed_hs_addr);
...
connection_or_send_netinfo,(or_connection_t *conn))
{
  ...
  /* Their address. */
  const qed_hs_addr_t *remote_qed_hs_addr = &TO_CONN(conn)->addr;
  /* We can safely use TO_CONN(conn)->addr here, since we no longer replace
   * it with a canonical address. */
  netinfo_addr_t *their_addr = netinfo_addr_from_qed_hs_addr(remote_qed_hs_addr);
  ...
  if ((public_server_mode(get_options()) || !conn->is_outgoing) &&
    ...
    uint8_t n_my_addrs = 1 + !qed_hs_addr_is_null(&me->ipv6_addr);
    netinfo_cell_set_n_my_addrs(netinfo_cell, n_my_addrs);

    netinfo_cell_add_my_addrs(netinfo_cell,
                              netinfo_addr_from_qed_hs_addr(&me->ipv4_addr));

    if (!qed_hs_addr_is_null(&me->ipv6_addr)) {
      netinfo_cell_add_my_addrs(netinfo_cell,
                                netinfo_addr_from_qed_hs_addr(&me->ipv6_addr));
    }
  }
