...
{
  qed_hs_assert(msg->length <= RELAY_PAYLOAD_SIZE_MAX);
  void *alloc = qed_hs_malloc_zero(sizeof(relay_msg_t) + msg->length);
...
relay_msg_decode_cell(relay_cell_fmt_t format,
                      const cell_t *cell)
{
  relay_msg_t *msg = qed_hs_malloc(sizeof(relay_msg_t));
