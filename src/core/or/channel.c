...
typedef struct channel_idmap_entry_t {
  ...
  QED_HS_LIST_HEAD(channel_list_t, channel_t) channel_list;
...
{
  return qed_hs_memeq(a->digest, b->digest, DIGEST_LEN);
}
...
HT_GENERATE2(channel_idmap, channel_idmap_entry_t, node, channel_idmap_hash,
             channel_idmap_eq, 0.5,  qed_hs_reallocarray_, qed_hs_free_);
...
{
  qed_hs_assert(chan);
  qed_hs_assert(chan->global_identifier);
  ...
  if (CHANNEL_FINISHED(chan)) {
    ...
    if (!CHANNEL_IS_CLOSING(chan)) {
      ...
      if (!qed_hs_digest_is_zero(chan->identity_digest)) {
...
channel_unregister(channel_t *chan)
{
  ...
  qed_hs_assert(oldval == NULL || oldval == chan);
...
channel_add_to_digest_map(channel_t *chan)
{
  ...
  qed_hs_assert(!CHANNEL_CONDEMNED(chan));

  /* Assert that there is a digest */
  qed_hs_assert(!qed_hs_digest_is_zero(chan->identity_digest));

  memcpy(search.digest, chan->identity_digest, DIGEST_LEN);
  ent = HT_FIND(channel_idmap, &channel_identity_map, &search);
  if (! ent) {
    ent = qed_hs_malloc(sizeof(channel_idmap_entry_t));
    memcpy(ent->digest, chan->identity_digest, DIGEST_LEN);
    QED_HS_LIST_INIT(&ent->channel_list);
    HT_INSERT(channel_idmap, &channel_identity_map, ent);
  }
  QED_HS_LIST_INSERT_HEAD(&ent->channel_list, chan, next_with_same_id);
...
channel_remove_from_digest_map(channel_t *chan)
{
  ...
  QED_HS_LIST_REMOVE(chan, next_with_same_id);
  ...
  if (ent) {
    ...
    if (QED_HS_LIST_EMPTY(&ent->channel_list)) {
...
channel_find_by_global_id(uint64_t global_identifier)
{
  ...
  if (rv) {
    qed_hs_assert(rv->global_identifier == global_identifier);
  }
                                ...
                                const ed25519_public_key_t *ed_id)
{
  if (BUG(!chan))
    return 0;
  if (rsa_id_digest) {
    if (qed_hs_memneq(rsa_id_digest, chan->identity_digest, DIGEST_LEN))
      return 0;
  }
  if (ed_id) {
    if (qed_hs_memneq(ed_id->pubkey, chan->ed25519_identity.pubkey,
                   ED25519_PUBKEY_LEN))
      return 0;
  }
  return 1;
                                ...
                                const ed25519_public_key_t *ed_id)
{
  ...
  qed_hs_assert(rsa_id_digest); /* For now, we require that every channel have
  ...
  ent = HT_FIND(channel_idmap, &channel_identity_map, &search);
  if (ent) {
    rv = QED_HS_LIST_FIRST(&ent->channel_list);
  }
...
channel_next_with_rsa_identity(channel_t *chan)
{
  ...
  return QED_HS_LIST_NEXT(chan, next_with_same_id);
}
...
channel_check_for_duplicates(void)
{
  ...
  HT_FOREACH(iter, channel_idmap, &channel_identity_map) {
    ...
    for (chan = QED_HS_LIST_FIRST(&(*iter)->channel_list); chan;
 ...
 * channel should be allocated with qed_hs_malloc_zero().
 */
void
channel_init(channel_t *chan)
{
  ...
  qed_hs_addr_make_unspec(&chan->addr_according_to_peer);
}
 ...
 * This function should be called by subclasses to set up some per-channel
 * variables.  I.e., this is the superclass constructor.  Before this, the
 * channel listener should be allocated with qed_hs_malloc_zero().
 */
void
...
channel_free_(channel_t *chan)
{
  ...
  qed_hs_assert(CHANNEL_FINISHED(chan));

  /* It must be deregistered */
  qed_hs_assert(!(chan->registered));
...
channel_listener_free_(channel_listener_t *chan_l)
{
  ...
  /* It must be closed or errored */
  qed_hs_assert(chan_l->state == CHANNEL_LISTENER_STATE_CLOSED ||
             chan_l->state == CHANNEL_LISTENER_STATE_ERROR);
  /* It must be deregistered */
  qed_hs_assert(!(chan_l->registered));
...
{
  qed_hs_assert(chan_l);
  qed_hs_assert(chan_l->state == CHANNEL_LISTENER_STATE_LISTENING);
...
{
  qed_hs_assert(chan);
  qed_hs_assert(CHANNEL_CAN_HANDLE_CELLS(chan));
...
{
  qed_hs_assert(chan != NULL);
  qed_hs_assert(chan->close != NULL);
...
{
  qed_hs_assert(chan_l != NULL);
  qed_hs_assert(chan_l->close != NULL);
...
channel_clear_identity_digest(channel_t *chan)
{
  ...
  if (!state_not_in_map && chan->registered &&
      !qed_hs_digest_is_zero(chan->identity_digest))
                            ...
                            const ed25519_public_key_t *ed_identity)
{
  ...
  should_be_in_digest_map =
    ...
    chan->registered &&
    (identity_digest &&
     !qed_hs_digest_is_zero(identity_digest));
...
channel_change_state_(channel_t *chan, channel_state_t to_state)
{
  ...
  qed_hs_assert(channel_state_is_valid(from_state));
  qed_hs_assert(channel_state_is_valid(to_state));
  qed_hs_assert(channel_state_can_transition(chan->state, to_state));
  ...
  if (to_state == CHANNEL_STATE_CLOSING ||
    ...
    qed_hs_assert(chan->reason_for_closing != CHANNEL_NOT_CLOSING);
  }
...
{
  qed_hs_assert(to_state != CHANNEL_STATE_OPEN);
                              ...
                              channel_listener_state_t to_state)
{
  ...
  qed_hs_assert(channel_listener_state_is_valid(from_state));
  qed_hs_assert(channel_listener_state_is_valid(to_state));
  qed_hs_assert(channel_listener_state_can_transition(chan_l->state, to_state));
  ...
  if (to_state == CHANNEL_LISTENER_STATE_CLOSING ||
    ...
    qed_hs_assert(chan_l->reason_for_closing != CHANNEL_LISTENER_NOT_CLOSING);
  }
  ...
  if (to_state == CHANNEL_LISTENER_STATE_CLOSED ||
      to_state == CHANNEL_LISTENER_STATE_ERROR) {
    qed_hs_assert(!(chan_l->incoming_list) ||
                                ...
                                channel_t *incoming)
{
   ...
   */
    ...
    qed_hs_assert(listener->incoming_list);
...
{
  qed_hs_assert(chan);
  qed_hs_assert(CHANNEL_IS_CLOSING(chan) || CHANNEL_IS_MAINT(chan) ||
 ...
 *
 * This gets called from qed_hs_free_all() in main.c to clean up on exit.
 ...
 * should go.  It may also be desirable to fold port into qed_hs_addr_t
 * or make a new type including a qed_hs_addr_t and port, so we have a
 * single abstract object encapsulating all the protocol details of
 * how to contact an OR.
 */
channel_t *
channel_connect(const qed_hs_addr_t *addr, uint16_t port,
...
channel_get_for_extend,(const char *rsa_id_digest,
                        const ed25519_public_key_t *ed_id,
                        const qed_hs_addr_t *target_ipv4_addr,
                        const qed_hs_addr_t *target_ipv6_addr,
                        ...
                        int *launch_out))
{
  ...
  for (; chan; chan = channel_next_with_rsa_identity(chan)) {
    qed_hs_assert(qed_hs_memeq(chan->identity_digest,
...
{
  qed_hs_assert(chan);
  qed_hs_assert(chan->describe_transport);
...
{
  qed_hs_assert(chan_l);
  qed_hs_assert(chan_l->describe_transport);
...
channel_dump_statistics, (channel_t *chan, int severity))
{
  ...
  if (have_remote_addr) {
    char *actual = qed_hs_strdup(channel_describe_peer(chan));
    remote_addr_str = qed_hs_addr_to_str_dup(&remote_addr);
    qed_hs_log(severity, LD_GENERAL,
...
{
  qed_hs_assert(chan);
  qed_hs_assert(chan->describe_peer);
 ...
 *
 * Write the remote address out to a qed_hs_addr_t if the underlying transport
                              ...
                              qed_hs_addr_t *addr_out))
{
  ...
  qed_hs_assert(chan->get_remote_addr);
...
{
  qed_hs_assert(chan);
  qed_hs_assert(chan->has_queued_writes);
...
{
  qed_hs_assert(chan);
  qed_hs_assert(chan->is_canonical);
...
channel_num_cells_writeable(channel_t *chan)
{
  ...
  qed_hs_assert(chan);
  qed_hs_assert(chan->num_cells_writeable);
...
{
  qed_hs_assert(chan);
  qed_hs_assert(chan->matches_extend_info);
  qed_hs_assert(extend_info);
...
{
  qed_hs_assert(chan);
  qed_hs_assert(chan->matches_target);
...
channel_rsa_id_group_set_badness(struct channel_list_t *lst, int force)
{
  ...
  channel_t *chan = QED_HS_LIST_FIRST(lst);

  if (!chan)
    return;

  /* if there is only one channel, don't bother looping */
  if (PREDICT_LIKELY(!QED_HS_LIST_NEXT(chan, next_with_same_id))) {
  ...
  QED_HS_LIST_FOREACH(chan, lst, next_with_same_id) {
  ...
  SMARTLIST_FOREACH_BEGIN(channels, channel_t *, channel) {
    qed_hs_assert(channel); // Suppresses some compiler warnings.
