...
orconn_state_fmt(msg_aux_data_t u)
{
  ...
  qed_hs_asprintf(&s, "<gid=%"PRIu64" chan=%"PRIu64" proxy_type=%d state=%d>",
...
orconn_status_fmt(msg_aux_data_t u)
{
  ...
  qed_hs_asprintf(&s, "<gid=%"PRIu64" status=%d reason=%d>",
...
int
orconn_add_pubsub(struct pubsub_connecqed_hs_t *connector)
{
