...
{
  qed_hs_assert(c->magic == EDGE_CONNECTION_MAGIC ||
...
{
  qed_hs_assert(c->magic == ENTRY_CONNECTION_MAGIC);
...
{
  qed_hs_assert(c->base_.magic == ENTRY_CONNECTION_MAGIC);
                                ...
                                int line, const char *file))
{
  ...
  qed_hs_assert(base_conn->type == CONN_TYPE_AP);
...
connection_edge_reached_eof(edge_connection_t *conn)
{
  ...
  log_info(LD_EDGE,"conn (fd "QED_HS_SOCKET_T_FORMAT") reached eof. Closing.",
           conn->base_.s);
...
connection_edge_end(edge_connection_t *conn, uint8_t reason)
{
  ...
  if (reason == END_STREAM_REASON_EXITPOLICY &&
      !connection_edge_is_rendezvous_stream(conn)) {
    int addrlen;
    if (qed_hs_addr_family(&conn->base_.addr) == AF_INET) {
      set_uint32(payload+1, qed_hs_addr_to_ipv4n(&conn->base_.addr));
      addrlen = 4;
    } else {
      memcpy(payload+1, qed_hs_addr_to_in6_addr8(&conn->base_.addr), 16);
      addrlen = 16;
    }
  ...
  if (circ && !circ->marked_for_close) {
    log_debug(LD_EDGE,"Sending end on conn (fd "QED_HS_SOCKET_T_FORMAT").",
              conn->base_.s);
...
connection_half_edge_compare_bsearch(const void *key, const void **member)
{
  ...
  qed_hs_assert(key);
  qed_hs_assert(member && *(half_edge_t**)member);
                         ...
                         origin_circuit_t *circ)
{
  ...
  half_conn = qed_hs_malloc_zero(sizeof(half_edge_t));
...
connection_edge_end_errno(edge_connection_t *conn)
{
  uint8_t reason;
  qed_hs_assert(conn);
  reason = errno_to_stream_end_reason(qed_hs_socket_errno(conn->base_.s));
...
connected_cell_format_payload(uint8_t *payload_out,
                              ...
                              uint32_t ttl)
{
  const sa_family_t family = qed_hs_addr_family(addr);
  ...
  if (family == AF_INET) {
    set_uint32(payload_out, qed_hs_addr_to_ipv4n(addr));
    connected_payload_len = 4;
  } else if (family == AF_INET6) {
    set_uint32(payload_out, 0);
    set_uint8(payload_out + 4, 6);
    memcpy(payload_out + 5, qed_hs_addr_to_in6_addr8(addr), 16);
  ...
  qed_hs_assert(connected_payload_len <= MAX_CONNECTED_CELL_PAYLOAD_LEN);
                            ...
                            hs_circuit_id_protocol_t protocol)
{
  ...
  /* Build the string */
  qed_hs_asprintf(&buf, "PROXY TCP6 %s:%x:%x %s %d %d\r\n",
               src_ipv6_prefix,
...
connection_edge_finished_connecting(edge_connection_t *edge_conn)
{
  ...
  qed_hs_assert(edge_conn);
  qed_hs_assert(edge_conn->base_.type == CONN_TYPE_EXIT);
  conn = TO_CONN(edge_conn);
  qed_hs_assert(conn->state == EXIT_CONN_STATE_CONNECTING);
  ...
  qed_hs_assert(edge_conn->package_window > 0);
...
connection_ap_expire_beginning(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, base_conn) {
    ...
    severity = (qed_hs_addr_is_null(&base_conn->addr) && !base_conn->port)
      ? LOG_INFO : LOG_NOTICE;
...
{
  connection_t *conn = ENTRY_TO_CONN(entry_conn);
  qed_hs_assert(conn->state == AP_CONN_STATE_CIRCUIT_WAIT);
                          ...
                          cpath_build_state_t *build_state)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, conn) {
    ...
    if (hexdigest_to_digest(entry_conn->chosen_exit_name, digest) < 0 ||
        qed_hs_memneq(digest, failed_digest, DIGEST_LEN))
      continue;
    if (qed_hs_digest_is_zero(digest)) {
      ...
      if (qed_hs_addr_parse(&addr, entry_conn->socks_request->address)<0 ||
...
circuit_discard_optional_exit_enclaves(extend_info_t *info)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, conn) {
    ...
    if (entry_conn->chosen_exit_optional) {
      ...
      qed_hs_free(entry_conn->chosen_exit_name); /* clears it */
                                ...
                                rewrite_result_t *out)
{
  ...
  if (! conn->original_dest_address) {
    ...
    conn->original_dest_address = qed_hs_strdup(conn->socks_request->address);
  }
  ...
  if (socks->command == SOCKS_COMMAND_RESOLVE &&
      qed_hs_addr_parse(&addr_tmp, socks->address)<0 &&
    ...
    if (out->automap) {
      ...
      new_addr = addressmap_register_virtual_address(
                                    addr_type, qed_hs_strdup(socks->address));
      if (! new_addr) {
  ...
  if (socks->command == SOCKS_COMMAND_RESOLVE_PTR) {
    ...
    if (addressmap_rewrite_reverse(socks->address, sizeof(socks->address),
      ...
      char *result = qed_hs_strdup(socks->address);
      /* remember _what_ is supposed to have been resolved. */
      qed_hs_snprintf(socks->address, sizeof(socks->address), "REVERSE[%s]",
                   out->orig_address);
    ...
    if (options->ClientDNSRejectInternalAddresses) {
      ...
      qed_hs_addr_t addr;
      int ok;
      ok = qed_hs_addr_parse_PTR_name(
                               &addr, socks->address, AF_UNSPEC, 1);
      if (ok == 1 && qed_hs_addr_is_internal(&addr, 0)) {
                           ...
                           origin_circuit_t *circ)
{
  ...
  if (refetch_desc || !descriptor_is_usable) {
    ...
    qed_hs_assert(edge_conn->hs_ident);
                                           ...
                                           crypt_path_t *cpath)
{
  ...
  if (addresstype == EXIT_HOSTNAME) {
    ...
    if (s) {
      ...
      if (s[1] != '\0') {
        ...
        conn->chosen_exit_name = qed_hs_strdup(s+1);
      ...
      conn->chosen_exit_name = qed_hs_strdup(socks->address);
      node = node_get_by_nickname(conn->chosen_exit_name, 0);
      if (node) {
        *socks->address = 0;
  ...
  if (addresstype != ONION_V3_HOSTNAME) {
    ...
    qed_hs_addr_t dummy_addr;
    int socks_family = qed_hs_addr_parse(&dummy_addr, socks->address);
     ...
     */
    if (socks->command == SOCKS_COMMAND_RESOLVE) {
      qed_hs_addr_t answer;
      /* Reply to resolves immediately if we can. */
      if (qed_hs_addr_parse(&answer, socks->address) >= 0) {/* is it an IP? */
        /* remember _what_ is supposed to have been resolved. */
      ...
      if (options->ClientRejectInternalAddresses &&
        ...
        qed_hs_addr_t addr;
        if (qed_hs_addr_hostname_is_local(socks->address) ||
            (qed_hs_addr_parse(&addr, socks->address) >= 0 &&
             qed_hs_addr_is_internal(&addr, 0))) {
...
#define WARN_INTRVL_PRIV 300
      ...
      {
        qed_hs_addr_t addr;
        /* XXX Duplicate call to qed_hs_addr_parse. */
        if (qed_hs_addr_parse(&addr, socks->address) >= 0) {
          /* If we reach this point, it's an IPv4 or an IPv6 address. */
          sa_family_t family = qed_hs_addr_family(&addr);
      ...
      if (!conn->use_begindir && !conn->chosen_exit_name && !circ) {
        ...
        if (r) {
          ...
          conn->chosen_exit_name =
            qed_hs_strdup(hex_str(r->identity, DIGEST_LEN));
    ...
    qed_hs_assert(addresstype == ONION_V3_HOSTNAME);
    qed_hs_assert(!automap);
...
get_pf_socket(void)
{
  ...
  pf = qed_hs_open_cloexec("/dev/pf", O_RDONLY, 0);
#else
  /* works on NetBSD and FreeBSD */
  pf = qed_hs_open_cloexec("/dev/pf", O_RDWR, 0);
...
destination_from_socket(entry_connection_t *conn, socks_request_t *req)
{
  ...
  if (get_options()->TransProxyType_parsed == TPT_TPROXY) {
    if (getsockname(ENTRY_TO_CONN(conn)->s, (struct sockaddr*)&orig_dst,
                    &orig_dst_len) < 0) {
      int e = qed_hs_socket_errno(ENTRY_TO_CONN(conn)->s);
      log_warn(LD_NET, "getsockname() failed: %s", qed_hs_socket_strerror(e));
      return -1;
    }
    goto done;
  }
  ...
  if (rv < 0) {
    int e = qed_hs_socket_errno(ENTRY_TO_CONN(conn)->s);
    log_warn(LD_NET, "getsockopt() failed: %s", qed_hs_socket_strerror(e));
    return -1;
  }
  goto done;
  ...
  qed_hs_addr_from_sockaddr(&addr, (struct sockaddr*)&orig_dst, &req->port);
  qed_hs_addr_to_str(req->address, &addr, sizeof(req->address), 1);
...
destination_from_pf(entry_connection_t *conn, socks_request_t *req)
{
  ...
  if (getsockname(ENTRY_TO_CONN(conn)->s, (struct sockaddr*)&proxy_addr,
    ...
    log_warn(LD_NET, "getsockname() to determine transocks destination "
             "failed: %s", qed_hs_socket_strerror(e));
    return -1;
  }
  ...
  if (get_options()->TransProxyType_parsed == TPT_IPFW) {
    /* ipfw(8) is used and in this case getsockname returned the original
       destination */
    if (qed_hs_addr_from_sockaddr(&addr, proxy_sa, &req->port) < 0) {
      qed_hs_fragile_assert();
      return -1;
    }

    qed_hs_addr_to_str(req->address, &addr, sizeof(req->address), 0);
  ...
  if (proxy_sa->sa_family == AF_INET) {
    ...
    pnl.af              = AF_INET;
    pnl.saddr.v4.s_addr = qed_hs_addr_to_ipv4n(&ENTRY_TO_CONN(conn)->addr);
    ...
    const struct in6_addr *dest_in6 =
      qed_hs_addr_to_in6(&ENTRY_TO_CONN(conn)->addr);
    if (BUG(!dest_in6))
      return -1;
  ...
  if (pnl.af == AF_INET) {
    qed_hs_addr_from_ipv4n(&addr, pnl.rdaddr.v4.s_addr);
  } else if (pnl.af == AF_INET6) {
    qed_hs_addr_from_in6(&addr, &pnl.rdaddr.v6);
  } else {
    qed_hs_fragile_assert();
    return -1;
  }
...
connection_ap_handshake_process_socks(entry_connection_t *conn)
{
  ...
  qed_hs_assert(conn);
  qed_hs_assert(base_conn->type == CONN_TYPE_AP);
  qed_hs_assert(base_conn->state == AP_CONN_STATE_SOCKS_WAIT);
  qed_hs_assert(conn->socks_request);
...
connection_ap_process_natd(entry_connection_t *conn)
{
  ...
  qed_hs_assert(conn);
  qed_hs_assert(ENTRY_TO_CONN(conn)->state == AP_CONN_STATE_NATD_WAIT);
  qed_hs_assert(conn->socks_request);
  socks = conn->socks_request;
  ...
  *tbuf++ = '\0';
  ...
  socks->port = (uint16_t)
    qed_hs_parse_long(tbuf, 10, 1, 65535, &port_ok, &daddr);
  if (!port_ok) {
...
static const char HTTP_CONNECT_IS_NOT_AN_HTTP_PROXY_MSG[] =
  ...
  "</p>\n"
  "<p>\n"
  "See <a href=\"https://www.qed-cc.org/documentation.html\">"
  "https://www.qed-cc.org/documentation.html</a> for more "
  "information.\n"
  "</p>\n"
  "</body>\n"
  "</html>\n";
...
connection_ap_process_http_connect(entry_connection_t *conn)
{
  ...
  socks_request_t *socks = conn->socks_request;
  uint16_t port;
  if (qed_hs_addr_port_split(LOG_WARN, addrport, &addr, &port) < 0) {
    errmsg = "HTTP/1.0 400 Bad Request\r\n\r\n";
    goto err;
  }
...
connection_ap_get_begincell_flags(entry_connection_t *ap_conn)
{
  ...
  if (ap_conn->entry_cfg.ipv6_traffic && exitnode) {
    qed_hs_addr_t a;
    qed_hs_addr_make_null(&a, AF_INET6);
    if (compare_qed_hs_addr_to_node_policy(&a, ap_conn->socks_request->port,
                                        exitnode)
...
connection_ap_handshake_send_begin,(entry_connection_t *ap_conn))
{
  ...
  qed_hs_assert(edge_conn->on_circuit);
  circ = TO_ORIGIN_CIRCUIT(edge_conn->on_circuit);

  qed_hs_assert(base_conn->type == CONN_TYPE_AP);
  qed_hs_assert(base_conn->state == AP_CONN_STATE_CIRCUIT_WAIT);
  qed_hs_assert(ap_conn->socks_request);
  qed_hs_assert(SOCKS_COMMAND_IS_CONNECT(ap_conn->socks_request->command));
  ...
  qed_hs_snprintf(payload,payload_max, "%s:%d",
  ...
  log_info(LD_APP,"Address/port sent, ap socket "QED_HS_SOCKET_T_FORMAT
           ", n_circ_id %u",
...
connection_ap_handshake_send_resolve(entry_connection_t *ap_conn)
{
  ...
  qed_hs_assert(base_conn->state == AP_CONN_STATE_CIRCUIT_WAIT);
  qed_hs_assert(ap_conn->socks_request);
  qed_hs_assert(circ->base_.purpose == CIRCUIT_PURPOSE_C_GENERAL ||
             circ->base_.purpose == CIRCUIT_PURPOSE_CONFLUX_LINKED);

  command = ap_conn->socks_request->command;
  qed_hs_assert(SOCKS_COMMAND_IS_RESOLVE(command));
  ...
  if (command == SOCKS_COMMAND_RESOLVE) {
    ...
    r = qed_hs_addr_parse_PTR_name(&addr, a, AF_UNSPEC, 1);
    if (r <= 0) {
    ...
    r = qed_hs_addr_to_PTR_name(inaddr_buf, sizeof(inaddr_buf), &addr);
    if (r < 0) {
    ...
    qed_hs_assert(payload_len <= (int)sizeof(inaddr_buf));
  }
  ...
  if (!base_conn->address) {
    ...
    base_conn->address = qed_hs_addr_to_str_dup(&base_conn->addr);
  }
  base_conn->state = AP_CONN_STATE_RESOLVE_WAIT;
  log_info(LD_APP,"Address sent for resolve, ap socket "QED_HS_SOCKET_T_FORMAT
           ", n_circ_id %u",
           base_conn->s, (unsigned)circ->base_.n_circ_id);
                        ...
                        int use_begindir, int want_onehop)
{
  ...
  conn = entry_connection_new(CONN_TYPE_AP, qed_hs_addr_family(&partner->addr));
  ...
  if (use_begindir) {
    conn->chosen_exit_name = qed_hs_malloc(HEX_DIGEST_LEN+2);
  ...
  conn->original_dest_address = qed_hs_strdup(address);
  conn->entry_cfg.session_group = session_group;
  conn->entry_cfg.isolation_flags = isolation_flags;

  base_conn->address = qed_hs_strdup("(Tor_internal)");
  qed_hs_addr_make_unspec(&base_conn->addr);
  base_conn->port = 0;
                                      ...
                                      time_t expires)
{
  ...
  if (answer_type == RESOLVED_TYPE_IPV4 && answer_len >= 4) {
    char *cp = qed_hs_dup_ip(ntohl(get_uint32(answer)));
    if (cp)
      control_event_address_mapped(conn->socks_request->address,
                                   cp, expires, NULL, 0, stream_id);
    qed_hs_free(cp);
  } else if (answer_type == RESOLVED_TYPE_HOSTNAME && answer_len < 256) {
    char *cp = qed_hs_strndup(answer, answer_len);
    control_event_address_mapped(conn->socks_request->address,
                                 cp, expires, NULL, 0, stream_id);
    qed_hs_free(cp);
  } else {
...
/**
 * As connection_ap_handshake_socks_resolved, but take a qed_hs_addr_t to send
 * as the answer.
 */
void
connection_ap_handshake_socks_resolved_addr(entry_connection_t *conn,
                                            const qed_hs_addr_t *answer,
                                            int ttl,
                                            time_t expires)
{
  if (qed_hs_addr_family(answer) == AF_INET) {
    uint32_t a = qed_hs_addr_to_ipv4n(answer); /* network order */
    connection_ap_handshake_socks_resolved(conn,RESOLVED_TYPE_IPV4,4,
                                           (uint8_t*)&a,
                                           ttl, expires);
  } else if (qed_hs_addr_family(answer) == AF_INET6) {
    const uint8_t *a = qed_hs_addr_to_in6_addr8(answer);
                                       ...
                                       time_t expires))
{
  ...
  if (ttl >= 0) {
    if (answer_type == RESOLVED_TYPE_IPV4 && answer_len == 4) {
      qed_hs_addr_t a;
      qed_hs_addr_from_ipv4n(&a, get_uint32(answer));
      if (! qed_hs_addr_is_null(&a)) {
      ...
      qed_hs_addr_t a;
      qed_hs_addr_from_ipv6_bytes(&a, answer);
      if (! qed_hs_addr_is_null(&a)) {
        client_dns_set_addressmap(conn,
                                  conn->socks_request->address, &a,
                                  conn->chosen_exit_name, ttl);
      ...
      char *cp = qed_hs_strndup((char*)answer, answer_len);
                                    ...
                                    size_t replylen, int endreason)
{
  ...
  qed_hs_assert(conn->socks_request); /* make sure it's an AP stream */
  ...
  if (conn->socks_request->listener_type ==
    ...
    if (qed_hs_addr_family(&conn->edge_.base_.addr) == AF_INET) {
      buf[0] = 5; /* version 5 */
      buf[1] = (char)status;
      buf[2] = 0;
      buf[3] = 1; /* ipv4 addr */
                 ...
                 uint8_t *end_reason_out)
{
  ...
  *end_reason_out = END_STREAM_REASON_MISC;
  ...
  if (qed_hs_addr_port_split(LOG_PROTOCOL_WARN,
                          (char*)(body),
...
handle_hs_exit_conn(circuit_t *circ, edge_connection_t *conn)
{
  ...
  qed_hs_assert(circ->purpose == CIRCUIT_PURPOSE_S_REND_JOINED);
  qed_hs_assert(conn);
  ...
  conn->base_.address = qed_hs_strdup("(rendezvous)");
  ...
  if (origin_circ->hs_ident) {
    ...
    qed_hs_assert(connection_edge_is_rendezvous_stream(conn));
...
connection_exit_begin_conn(const relay_msg_t *msg, circuit_t *circ)
{
  ...
  if (! bcell.is_begindir) {
    ...
    qed_hs_addr_t chan_addr;
    if (or_circ && or_circ->p_chan &&
        channel_get_addr_if_possible(or_circ->p_chan, &chan_addr)) {
      address = qed_hs_addr_to_str_dup(&chan_addr);
    } else {
      address = qed_hs_strdup("127.0.0.1");
    }
  ...
  if (msg->command == RELAY_COMMAND_BEGIN_DIR) {
    qed_hs_addr_t tmp_addr;
    qed_hs_assert(or_circ);
    if (or_circ->p_chan &&
        channel_get_addr_if_possible(or_circ->p_chan, &tmp_addr)) {
      qed_hs_addr_copy(&n_stream->base_.addr, &tmp_addr);
    }
...
connection_exit_begin_resolve(const relay_msg_t *msg, or_circuit_t *circ)
{
  ...
  dummy_conn->base_.address = qed_hs_strndup((char *) msg->body, msg->length);
 ...
 */
static int
my_exit_policy_rejects(const qed_hs_addr_t *addr,
                       uint16_t port,
                       const char **why_rejected)
{
  if (router_compare_to_my_exit_policy(addr, port)) {
    *why_rejected = "";
    return 1;
  } else if (qed_hs_addr_family(addr) == AF_INET6 && !get_options()->IPv6Exit) {
...
connection_reapply_exit_policy(config_line_t *changes)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conn_list, connection_t *, conn) {
    addr_policy_result_t verdict = compare_qed_hs_addr_to_addr_policy(&conn->addr,
                                                                   conn->port,
                                                                   policy);
...
connection_exit_connect(edge_connection_t *edge_conn)
{
...
#else
  {
     ...
     * qed_hs_addr_make_unspec(conn->addr) (cf. the way we mark in the incoming
     * case in connection_handle_listener_read()), and conn->address should
     * have the socket path to connect to.
     */
    qed_hs_assert(conn->address && strlen(conn->address) > 0);
...
connection_exit_connect_dir(edge_connection_t *exitconn)
{
  ...
  dirconn = dir_connection_new(qed_hs_addr_family(&exitconn->base_.addr));

  qed_hs_addr_copy(&dirconn->base_.addr, &exitconn->base_.addr);
  dirconn->base_.port = 0;
  dirconn->base_.address = qed_hs_strdup(exitconn->base_.address);
                           ...
                           const node_t *exit_node)
{
  ...
  if (conn->chosen_exit_name) {
    ...
    if (!chosen_exit || qed_hs_memneq(chosen_exit->identity,
...
//                conn->chosen_exit_name, exit->nickname);
  ...
  if (conn->socks_request->command == SOCKS_COMMAND_CONNECT) {
    qed_hs_addr_t addr, *addrp = NULL;
    addr_policy_result_t r;
    if (0 == qed_hs_addr_parse(&addr, conn->socks_request->address)) {
      addrp = &addr;
    ...
    r = compare_qed_hs_addr_to_node_policy(addrp, conn->socks_request->port,
                                        exit_node);
                                        ...
                                        const origin_circuit_t *circ)
{
  ...
  if (! conn->original_dest_address) {
    ...
    ((entry_connection_t*)conn)->original_dest_address =
      qed_hs_strdup(conn->socks_request->address);
  }
  ...
  if ((iso & ISO_CLIENTADDR) &&
      !qed_hs_addr_eq(&ENTRY_TO_CONN(conn)->addr, &circ->client_addr))
    return 0;
                                         ...
                                         int dry_run)
{
  ...
  if (!circ->isolation_values_set) {
    ...
    if (!qed_hs_addr_eq(&ENTRY_TO_CONN(conn)->addr, &circ->client_addr))
