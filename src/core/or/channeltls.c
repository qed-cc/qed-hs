 ...
 */
channel_t *
channel_tls_connect(const qed_hs_addr_t *addr, uint16_t port,
                    const char *id_digest,
                    const ed25519_public_key_t *ed_id)
{
  channel_tls_t *tlschan = qed_hs_malloc_zero(sizeof(*tlschan));
...
channel_tls_start_listener(void)
{
  ...
  if (!channel_tls_listener) {
    listener = qed_hs_malloc_zero(sizeof(*listener));
...
channel_tls_free_all(void)
{
  ...
  if (channel_tls_listener) {
    ...
    qed_hs_assert(channel_tls_listener == NULL);
  }
...
channel_tls_from_base(channel_t *chan)
{
  ...
  qed_hs_assert(chan->magic == TLS_CHAN_MAGIC);
                                   ...
                                   qed_hs_addr_t *addr_out)
{
  ...
  qed_hs_addr_copy(addr_out, &TO_CONN(tlschan->conn)->addr);
...
channel_tls_get_transport_name_method(channel_t *chan, char **transport_out)
{
  ...
  qed_hs_assert(tlschan);
  qed_hs_assert(transport_out);
  qed_hs_assert(tlschan->conn);

  if (!tlschan->conn->ext_or_transport)
    return -1;

  *transport_out = qed_hs_strdup(tlschan->conn->ext_or_transport);
  return 0;
}
                                       ...
                                       extend_info_t *extend_info)
{
  ...
  const qed_hs_addr_port_t *orport = &tlschan->conn->canonical_orport;
  // If the canonical address is set, then we'll allow matches based on that.
  if (! qed_hs_addr_is_unspec(&orport->addr)) {
                                  ...
                                  const qed_hs_addr_t *target)
{
   ...
   */
  return qed_hs_addr_eq(&TO_CONN(tlschan->conn)->addr, target);
}
                                          ...
                                          uint8_t state)
{
  ...
  qed_hs_assert(chan);
  qed_hs_assert(conn);
  qed_hs_assert(conn->chan == chan);
  qed_hs_assert(chan->conn == conn);
  ...
  qed_hs_assert(CHANNEL_IS_OPENING(base_chan) ||
...
enter_v3_handshake_with_cell(var_cell_t *cell, channel_tls_t *chan)
{
  ...
  qed_hs_assert(cell);
  qed_hs_assert(chan);
  qed_hs_assert(chan->conn);

  started_here = connection_or_nonopen_was_started_here(chan->conn);

  qed_hs_assert(TO_CONN(chan->conn)->state == OR_CONN_STATE_TLS_HANDSHAKING ||
...
channel_tls_process_versions_cell(var_cell_t *cell, channel_tls_t *chan)
{
  ...
  qed_hs_assert(chan->conn->handshake_state);
  ...
  if (chan->conn->link_proto == 2) {
    ...
    qed_hs_assert(chan->conn->link_proto >= 3);
...
/**
 * Convert <b>netinfo_addr</b> into corresponding <b>qed_hs_addr</b>.
 * Return 0 on success; on failure, return -1 and log a warning.
 */
static int
qed_hs_addr_from_netinfo_addr(qed_hs_addr_t *qed_hs_addr,
                           const netinfo_addr_t *netinfo_addr) {
  qed_hs_assert(qed_hs_addr);
  qed_hs_assert(netinfo_addr);
...
can_process_netinfo_cell(const channel_tls_t *chan)
{
  ...
  qed_hs_assert(chan->conn->handshake_state);
  qed_hs_assert(chan->conn->handshake_state->received_versions);
...
mark_channel_tls_endpoint_as_client(channel_tls_t *chan)
{
  ...
  qed_hs_assert(qed_hs_digest_is_zero(
...
channel_tls_process_netinfo_cell(cell_t *cell, channel_tls_t *chan)
{
  ...
  qed_hs_addr_t my_apparent_addr = QED_HS_ADDR_NULL;
  int started_here = 0;
  ...
  if (qed_hs_addr_from_netinfo_addr(&my_apparent_addr, my_addr) == -1) {
    connection_or_close_for_error(chan->conn, 0);
  ...
  if (my_addr_type == NETINFO_ADDR_TYPE_IPV4 && my_addr_len == 4) {
    if (!get_options()->BridgeRelay && me &&
        qed_hs_addr_eq(&my_apparent_addr, &me->ipv4_addr)) {
      TLS_CHAN_TO_BASE(chan)->is_canonical_to_peer = 1;
    }
  } else if (my_addr_type == NETINFO_ADDR_TYPE_IPV6 &&
             my_addr_len == 16) {
    if (!get_options()->BridgeRelay && me &&
        !qed_hs_addr_is_null(&me->ipv6_addr) &&
        qed_hs_addr_eq(&my_apparent_addr, &me->ipv6_addr)) {
      TLS_CHAN_TO_BASE(chan)->is_canonical_to_peer = 1;
    }
  }
  ...
  if (me) {
    ...
    qed_hs_addr_copy(&TLS_CHAN_TO_BASE(chan)->addr_according_to_peer,
                  &my_apparent_addr);
  }
  ...
  for (uint8_t i = 0; i < n_other_addrs; i++) {
    ...
    if (qed_hs_addr_from_netinfo_addr(&addr, netinfo_addr) == -1) {
      log_fn(LOG_PROTOCOL_WARN,  LD_OR,
     ...
     * on the relay's TCP.
     */
    if (qed_hs_addr_eq(&addr, &TO_CONN(chan->conn)->addr)) {
  ...
  if (me && !TLS_CHAN_TO_BASE(chan)->is_canonical_to_peer &&
    ...
    log_info(LD_OR,
             ...
             safe_str(qed_hs_addr_is_null(&my_apparent_addr) ?
...
#define NETINFO_NOTICE_SKEW 3600
  ...
  if (connection_or_set_state_open(chan->conn) < 0) {
    ...
    log_info(LD_OR,
             ...
             qed_hs_addr_is_null(&my_apparent_addr) ?
               "<none>" :
...
#define MAX_CERT_TYPE_WANTED CERTTYPE_RSA1024_ID_EDID
   ...
   * of ed/x509 */
  qed_hs_x509_cert_t *x509_certs[MAX_CERT_TYPE_WANTED + 1];
  qed_hs_cert_t *ed_certs[MAX_CERT_TYPE_WANTED + 1];
...
#define ERR(s)                                                  \
  ...
  for (i = 0; i < n_certs; ++i) {
    ...
    switch (ct) {
      ...
      case CERT_ENCODING_X509: {
        qed_hs_x509_cert_t *x509_cert = qed_hs_x509_cert_decode(cert_body, cert_len);
        if (!x509_cert) {
          log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
          ...
          if (x509_certs[cert_type]) {
            qed_hs_x509_cert_free(x509_cert);
      ...
      case CERT_ENCODING_ED25519: {
        qed_hs_cert_t *ed_cert = qed_hs_cert_parse(cert_body, cert_len);
        if (!ed_cert) {
          log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
     ...
     case CERT_ENCODING_RSA_CROSSCERT: {
        if (rsa_ed_cc_cert) {
          ...
          rsa_ed_cc_cert = qed_hs_memdup(cert_body, cert_len);
   ...
   * structure. */
  qed_hs_x509_cert_t *id_cert = x509_certs[CERTTYPE_RSA1024_ID_ID];
  qed_hs_x509_cert_t *auth_cert = x509_certs[CERTTYPE_RSA1024_ID_AUTH];
  qed_hs_x509_cert_t *link_cert = x509_certs[CERTTYPE_RSA1024_ID_LINK];
  ...
  qed_hs_cert_t *ed_id_sign = ed_certs[CERTTYPE_ED_ID_SIGN];
  qed_hs_cert_t *ed_sign_link = ed_certs[CERTTYPE_ED_SIGN_LINK];
  qed_hs_cert_t *ed_sign_auth = ed_certs[CERTTYPE_ED_SIGN_AUTH];
  ...
  if (started_here) {
    ...
    chan->conn->handshake_state->authenticated_rsa = 1;
    {
      ...
      identity_rcvd = qed_hs_tls_cert_get_key(id_cert);
      if (!identity_rcvd) {
 ...
 err:
  for (unsigned u = 0; u < ARRAY_LENGTH(x509_certs); ++u) {
    qed_hs_x509_cert_free(x509_certs[u]);
  }
  for (unsigned u = 0; u < ARRAY_LENGTH(ed_certs); ++u) {
    qed_hs_cert_free(ed_certs[u]);
  }
  qed_hs_free(rsa_ed_cc_cert);
  certs_cell_free(cc);
#undef ERR
}
...
#define ERR(s)                                                  \
  ...
  if (BUG(authtype != AUTHTYPE_ED25519_SHA256_RFC5705)) {
    ...
    qed_hs_assert(V3_AUTH_BODY_LEN > ED25519_SIG_LEN);
  ...
  if (qed_hs_memneq(expected_cell->payload+4, auth, bodylen-24))
...

  {
    ...
    qed_hs_assert(authlen > ED25519_SIG_LEN);
  ...
  {
    qed_hs_x509_cert_t *id_cert = chan->conn->handshake_state->certs->id_cert;
    crypto_pk_t *identity_rcvd = qed_hs_tls_cert_get_key(id_cert);
    const common_digests_t *id_digests = qed_hs_x509_cert_get_id_digests(id_cert);
