...
check_extend_cell(const extend_cell_t *cell)
{
  ...
  if (qed_hs_digest_is_zero((const char*)cell->node_id))
    return -1;
  if (!qed_hs_addr_port_is_valid_ap(&cell->orport_ipv4, 0)) {
    ...
    if (!qed_hs_addr_port_is_valid_ap(&cell->orport_ipv6, 0)) {
      return -1;
    }
  }
                                   ...
                                   const extend2_cell_body_t *cell)
{
  ...
  memset(cell_out, 0, sizeof(*cell_out));
  qed_hs_addr_make_unspec(&cell_out->orport_ipv4.addr);
  qed_hs_addr_make_unspec(&cell_out->orport_ipv6.addr);
    ...
    switch (ls->ls_type) {
      case LS_IPV4:
        if (found_ipv4)
          continue;
        found_ipv4 = 1;
        qed_hs_addr_from_ipv4h(&cell_out->orport_ipv4.addr, ls->un_ipv4_addr);
      ...
      case LS_IPV6:
        if (found_ipv6)
          continue;
        found_ipv6 = 1;
        qed_hs_addr_from_ipv6_bytes(&cell_out->orport_ipv6.addr,
                                 ls->un_ipv6_addr);
                        ...
                        int relayed)
{
  ...
  case CELL_CREATE:
    ...
    FALLTHROUGH;
  case CELL_CREATE_FAST:
    qed_hs_assert(cell_in->handshake_len <= space);
    memcpy(p, cell_in->onionskin, cell_in->handshake_len);
    break;
  case CELL_CREATE2:
    qed_hs_assert(cell_in->handshake_len <= sizeof(cell_out->payload)-4);
...
created_cell_format(cell_t *cell_out, const created_cell_t *cell_in)
{
  ...
  case CELL_CREATED:
  case CELL_CREATED_FAST:
    qed_hs_assert(cell_in->handshake_len <= sizeof(cell_out->payload));
    memcpy(cell_out->payload, cell_in->reply, cell_in->handshake_len);
    break;
  case CELL_CREATED2:
    qed_hs_assert(cell_in->handshake_len <= sizeof(cell_out->payload)-2);
  ...
  case RELAY_COMMAND_EXTEND2:
    {
      ...
      *command_out = RELAY_COMMAND_EXTEND2;
      ...
      link_specifier_t *ls;
      if (qed_hs_addr_port_is_valid_ap(&cell_in->orport_ipv4, 0)) {
        ...
        ls->ls_type = LS_IPV4;
        ls->ls_len = 6;
        ls->un_ipv4_addr = qed_hs_addr_to_ipv4h(&cell_in->orport_ipv4.addr);
      ...
      if (qed_hs_addr_port_is_valid_ap(&cell_in->orport_ipv6, 0)) {
        ...
        ls->ls_type = LS_IPV6;
        ls->ls_len = 18;
        qed_hs_addr_copy_ipv6_bytes(ls->un_ipv6_addr,
