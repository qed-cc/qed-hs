...
channel_connect_for_circuit,(const extend_info_t *ei))
{
  ...
  const qed_hs_addr_port_t *orport = extend_info_pick_orport(ei);
  if (!orport)
    return NULL;
...
circuit_list_path_impl(origin_circuit_t *circ, int verbose, int verbose_names)
{
  ...
  do {
    ...
    if (verbose_names) {
      elt = qed_hs_malloc(MAX_VERBOSE_NICKNAME_LEN+1);
      ...
      elt = qed_hs_malloc(HEX_DIGEST_LEN+2);
      elt[0] = '$';
      base16_encode(elt+1, HEX_DIGEST_LEN+1, id, DIGEST_LEN);
    }
    qed_hs_assert(elt);
    if (verbose) {
      qed_hs_assert(hop->state <= 2);
...
circuit_log_path(int severity, unsigned int domain, origin_circuit_t *circ)
{
  ...
  qed_hs_log(severity,domain,"%s",s);
  qed_hs_free(s);
}
...
onion_populate_cpath(origin_circuit_t *circ)
{
  ...
  qed_hs_assert(circ);
  qed_hs_assert(circ->build_state);
   ...
   * edge cases. */
  qed_hs_assert(circuit_get_cpath_len(circ));
  ...
  if (circuit_get_cpath_len(circ) == 1) {
    ...
    qed_hs_assert(circ->cpath->extend_info);
...
origin_circuit_init(uint8_t purpose, int flags)
{
  ...
  circ->build_state = qed_hs_malloc_zero(sizeof(cpath_build_state_t));
...
circuit_establish_circuit(uint8_t purpose, extend_info_t *exit_ei, int flags)
{
  ...
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(establish), circ);
  return circ;
}
                                   ...
                                   int flags))
{
  ...
  qed_hs_assert(purpose == CIRCUIT_PURPOSE_CONFLUX_UNLINKED);

  circ = origin_circuit_init(purpose, flags);
  TO_CIRCUIT(circ)->conflux_pending_nonce =
    qed_hs_memdup(conflux_nonce, DIGEST256_LEN);
...
{
  ocirc_chan_msg_t *msg = qed_hs_malloc(sizeof(*msg));
...
circuit_handle_first_hop(origin_circuit_t *circ)
{
  ...
  qed_hs_assert(firsthop);
  qed_hs_assert(firsthop->extend_info);
  ...
  const qed_hs_addr_port_t *orport4 =
    extend_info_get_orport(firsthop->extend_info, AF_INET);
  const qed_hs_addr_port_t *orport6 =
  ...
  if (!n_chan) {
    ...
    qed_hs_assert(!circ->base_.n_hop);
                             ...
                             int relayed))
{
  ...
  qed_hs_assert(circ);
  qed_hs_assert(circ->n_chan);
  qed_hs_assert(create_cell);
  qed_hs_assert(create_cell->cell_type == CELL_CREATE ||
  ...
  qed_hs_assert_nonfatal_once(circ->n_chan->is_canonical);
...
circuit_send_next_onion_skin(origin_circuit_t *circ)
{
  ...
  qed_hs_assert(circ->cpath->state == CPATH_STATE_OPEN);
  qed_hs_assert(circ->base_.state == CIRCUIT_STATE_BUILDING);
...
circuit_send_first_onion_skin(origin_circuit_t *circ)
{
  ...
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(first_onion_skin), circ, circ->cpath);
...
circuit_build_no_more_hops(origin_circuit_t *circ)
{
  ...
  if (r == GUARD_USABLE_NOW) {
    ...
    qed_hs_assert_nonfatal(r == GUARD_USABLE_NEVER);
    return - END_CIRC_REASON_INTERNAL;
  }
                                     ...
                                     crypt_path_t *hop)
{
  ...
  qed_hs_addr_make_unspec(&ec.orport_ipv4.addr);
  qed_hs_addr_make_unspec(&ec.orport_ipv6.addr);
  ...
  int n_addrs_set = 0;
  if (orport4) {
    qed_hs_addr_copy(&ec.orport_ipv4.addr, &orport4->addr);
    ec.orport_ipv4.port = orport4->port;
    ++n_addrs_set;
  }
  if (orport6 && include_ipv6) {
    qed_hs_addr_copy(&ec.orport_ipv6.addr, &orport6->addr);
  ...
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(intermediate_onion_skin), circ, hop);
  return 0;
}
...
circuit_note_clock_jumped(int64_t seconds_elapsed, bool was_idle)
{
  ...
  if (was_idle) {
    qed_hs_log(severity, LD_GENERAL, "Tor has been idle for %"PRId64
                         ...
                         const created_cell_t *reply)
{
  ...
  qed_hs_assert(hop->state == CPATH_STATE_AWAITING_KEYS);
                                      ...
                                      int *need_capacity))
{
  ...
  qed_hs_assert(need_uptime);
  qed_hs_assert(need_capacity);
...
{ /* XXXX MOVE */
  ...
  for (i = 0; i < smartlist_len(needed_ports); ++i) {
    ...
    qed_hs_assert(port);
    if (node)
      r = compare_qed_hs_addr_to_node_policy(NULL, port, node);
    else
      continue;
...
//         n_pending_connections);
  ...
  n_supported = qed_hs_calloc(smartlist_len(the_nodes), sizeof(int));
...
//                 try, router->nickname);
    ...
    SMARTLIST_FOREACH(needed_ports, uint16_t *, cp, qed_hs_free(cp));
                        ...
                        router_crn_flags_t flags, int is_internal)
{
  ...
  switch (TO_CIRCUIT(circ)->purpose) {
    ...
    case CIRCUIT_PURPOSE_C_ESTABLISH_REND:
      ...
      qed_hs_assert_nonfatal(is_internal);
 ...
 * outer ones. See https://bugs.qed-cc.org/tpo/core/qed-hs/24487
 * for this.
 *
                          ...
                          int cur_len)
{
  ...
  const or_options_t *options = get_options();
  router_crn_flags_t flags = CRN_NEED_DESC;
  qed_hs_assert(CIRCUIT_PURPOSE_MIN_ <= purpose &&
...
onion_extend_cpath(origin_circuit_t *circ)
{
  ...
  if (cur_len == state->desired_path_len - 1) { /* Picking last node */
    ...
    if (r) {
      ...
      qed_hs_assert_nonfatal(info || client);
    }
  } else {
    const node_t *r =
...
client_circ_negotiation_message(const extend_info_t *ei,
                                ...
                                size_t *msg_len_out)
{
  qed_hs_assert(ei && msg_out && msg_len_out);
