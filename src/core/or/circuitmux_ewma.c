...
ewma_alloc_cmux_data(circuitmux_t *cmux)
{
  ...
  pol = qed_hs_malloc_zero(sizeof(*pol));
                     ...
                     unsigned int cell_count)
{
  ...
  qed_hs_assert(cmux);
  qed_hs_assert(pol_data);
  qed_hs_assert(circ);
  qed_hs_assert(direction == CELL_DIRECTION_OUT ||
             direction == CELL_DIRECTION_IN);
  /* Shut the compiler up without triggering -Wtautological-compare */
  (void)cell_count;

  cdata = qed_hs_malloc_zero(sizeof(*cdata));
              ...
              circuitmux_t *cmux_2, circuitmux_policy_data_t *pol_data_2)
{
  ...
  qed_hs_assert(pol_data_1);
  qed_hs_assert(cmux_2);
  qed_hs_assert(pol_data_2);
...
scale_active_circuits(ewma_policy_data_t *pol, unsigned cur_tick)
{
  ...
  qed_hs_assert(pol);
  qed_hs_assert(pol->active_circuit_pqueue);
  ...
  SMARTLIST_FOREACH_BEGIN(
      ...
      cell_ewma_t *, e) {
    qed_hs_assert(e->last_adjusted_tick ==
               pol->active_circuit_pqueue_last_recalibrated);
    e->cell_count *= factor;
...
add_cell_ewma(ewma_policy_data_t *pol, cell_ewma_t *ewma)
{
  ...
  qed_hs_assert(pol->active_circuit_pqueue);
  qed_hs_assert(ewma);
  qed_hs_assert(ewma->heap_index == -1);
