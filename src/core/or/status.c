...
secs_to_uptime(long secs)
{
  ...
  switch (days) {
  case 0:
    qed_hs_asprintf(&uptime_string, "%d:%02d hours", hours, minutes);
    break;
  case 1:
    qed_hs_asprintf(&uptime_string, "%ld day %d:%02d hours",
                 days, hours, minutes);
    break;
  default:
    qed_hs_asprintf(&uptime_string, "%ld days %d:%02d hours",
                 days, hours, minutes);
    break;
  }
...
bytes_to_usage(uint64_t bytes)
{
  ...
  if (bytes < (1<<20)) { /* Less than a megabyte. */
    qed_hs_asprintf(&bw_string, "%"PRIu64" kB", (bytes>>10));
  } else if (bytes < (1<<30)) { /* Megabytes. Let's add some precision. */
    double bw = ((double)bytes);
    qed_hs_asprintf(&bw_string, "%.2f MB", bw/(1<<20));
  } else { /* Gigabytes. */
    double bw = ((double)bytes);
    qed_hs_asprintf(&bw_string, "%.2f GB", bw/(1<<30));
  }
...
log_accounting(const time_t now, const or_options_t *options)
{
  ...
  qed_hs_free(acc_rcvd);
  qed_hs_free(acc_sent);
  qed_hs_free(acc_used);
  qed_hs_free(acc_max);
  qed_hs_free(remaining);
}
