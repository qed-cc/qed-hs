...
#ifndef QED_HS_CHANNEL_H
#define QED_HS_CHANNEL_H
...
struct channel_t {
   ...
   * qed_hs_addr_t.  Write the address out to the provided qed_hs_addr_t *, and
   * return 1 if successful or 0 if no address available.
   */
  int (*get_remote_addr)(const channel_t *, qed_hs_addr_t *);
  ...
  int (*matches_target)(channel_t *, const qed_hs_addr_t *);
   ...
   */
  QED_HS_LIST_ENTRY(channel_t) next_with_same_id;
...
channel_t * channel_connect(const qed_hs_addr_t *addr, uint16_t port,
...
#undef qed_hs_timer_t

#endif /* !defined(QED_HS_CHANNEL_H) */
