...
STATIC void
cc_stats_refill_bucket(cc_client_stats_t *stats, const qed_hs_addr_t *addr)
{
   ...
   * burst value */
  qed_hs_assert_nonfatal(new_circuit_bucket_count <= dos_cc_circuit_burst);
  ...
  qed_hs_assert_nonfatal(new_circuit_bucket_count >= stats->circuit_bucket ||
                      new_circuit_bucket_count == dos_cc_circuit_burst);
...
static void
conn_update_on_connect(conn_client_stats_t *stats, const qed_hs_addr_t *addr)
{
  qed_hs_assert(stats);
  qed_hs_assert(addr);
  ...
  log_debug(LD_DOS, "Client address %s has now %u concurrent connections. "
                    "Remaining %" QED_HS_PRIuSZ "/sec connections are allowed.",
 ...
 * The addr is used for logging purposes only. */
static void
conn_update_on_close(conn_client_stats_t *stats, const qed_hs_addr_t *addr)
{
...
dos_conn_addr_get_defense_type(const qed_hs_addr_t *addr)
{
  clientmap_entry_t *entry;
...
dos_geoip_entry_about_to_free(const clientmap_entry_t *geoip_ent)
{
  ...
  SMARTLIST_FOREACH_BEGIN(get_connection_array(), connection_t *, conn) {
    if (conn->type == CONN_TYPE_OR) {
      ...
      if (!qed_hs_addr_compare(&geoip_ent->addr, &TO_CONN(or_conn)->addr,
                            CMP_EXACT)) {
...
dos_log_heartbeat(void)
{
  ...
  qed_hs_free(msg);
  SMARTLIST_FOREACH(elems, char *, e, qed_hs_free(e));
...
dos_new_client_conn(or_connection_t *or_conn, const char *transport_name)
{
  ...
  qed_hs_assert_nonfatal(!or_conn->tracked_for_dos_mitigation);
