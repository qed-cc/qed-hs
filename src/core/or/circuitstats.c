...
circuit_build_times_close_quantile(void)
{
  ...
  int32_t min = (int)qed_hs_lround(100*circuit_build_times_quantile_cutoff());
  ...
  if (!circuit_build_times_disabled(get_options())) {
    ...
    if (num > 0) {
      if (num != cbt->liveness.num_recent_circs) {
        ...
        qed_hs_assert(cbt->liveness.timeouts_after_firsthop ||
         ...
         */
        recent_circs = qed_hs_calloc(num, sizeof(int8_t));
        ...
        qed_hs_free(cbt->liveness.timeouts_after_firsthop);
...
circuit_build_times_init(circuit_build_times_t *cbt)
{
  ...
  if (!circuit_build_times_disabled(get_options())) {
    ...
    cbt->liveness.timeouts_after_firsthop =
      qed_hs_calloc(cbt->liveness.num_recent_circs, sizeof(int8_t));
  } else {
    cbt->liveness.num_recent_circs = 0;
                                     ...
                                     build_time_t *nbins)
{
  ...
  *nbins = 1 + (max_build_time / CBT_BIN_WIDTH);
  histogram = qed_hs_calloc(*nbins, sizeof(build_time_t));
...
circuit_build_times_get_xm(circuit_build_times_t *cbt)
{
  ...
  qed_hs_assert(nbins > 0);
  qed_hs_assert(num_modes > 0);

  nth_max_bin = qed_hs_calloc(num_modes, sizeof(build_time_t));
  ...
  qed_hs_free(histogram);
  qed_hs_free(nth_max_bin);
                                 ...
                                 or_state_t *state)
{
  ...
  *next = NULL;
  ...
  for (i = 0; i < nbins; i++) {
    ...
    *next = line = qed_hs_malloc_zero(sizeof(config_line_t));
    line->key = qed_hs_strdup("CircuitBuildTimeBin");
    qed_hs_asprintf(&line->value, "%d %d",
                                ...
                                or_state_t *state)
{
  ...
  loaded_times = qed_hs_calloc(state->TotalBuildTimes, sizeof(build_time_t));
  ...
  for (line = state->BuildtimeHistogram; line; line = line->next) {
    ...
    if (smartlist_len(args) < 2) {
      ...
      err = 1;
      SMARTLIST_FOREACH(args, char*, cp, qed_hs_free(cp));
      smartlist_free(args);
      break;
    } else {
      ...
      build_time_t ms;
      int ok;
      ms = (build_time_t)qed_hs_parse_ulong(ms_str, 10, 0,
      ...
      count = (uint32_t)qed_hs_parse_ulong(count_str, 10, 0,
                                        UINT32_MAX, &ok, NULL);
      if (!ok) {
        log_warn(LD_GENERAL, "Unable to parse circuit build times: "
...
circuit_build_times_update_alpha(circuit_build_times_t *cbt)
{
  ...
  for (i=0; i< CBT_NCIRCUITS_TO_OBSERVE; i++) {
    ...
    if (x[i] < cbt->Xm) {
      a += qed_hs_mathlog(cbt->Xm);
      n++;
    } else if (x[i] == CBT_BUILD_ABANDONED) {
      abandoned_count++;
    } else {
      a += qed_hs_mathlog(x[i]);
      n++;
    }
  }
  ...
  qed_hs_assert_nonfatal(n==cbt->total_build_times-abandoned_count);
...
{
  double ret;
  qed_hs_assert(quantile >= 0);
  qed_hs_assert(1.0-quantile > 0);
  qed_hs_assert(cbt->Xm > 0);
                                    ...
                                    double q_lo, double q_hi)
{
  ...
  qed_hs_assert(q_lo >= 0);
  qed_hs_assert(q_hi < 1);
  qed_hs_assert(q_lo < q_hi);

  u = q_lo + (q_hi-q_lo)*randval;

  qed_hs_assert(0 <= u && u < 1.0);
  /* circuit_build_times_calculate_timeout returns <= INT32_MAX */
  ret = (build_time_t)
    qed_hs_lround(circuit_build_times_calculate_timeout(cbt, u));
  qed_hs_assert(ret > 0);
  return ret;
}
#endif /* defined(QED_HS_UNIT_TESTS) */
                                  ...
                                  double quantile, double timeout_ms)
{
  ...
  qed_hs_assert(quantile >= 0);
  qed_hs_assert(cbt->Xm > 0);
  cbt->alpha = qed_hs_mathlog(1.0-quantile)/
    (qed_hs_mathlog(cbt->Xm)-qed_hs_mathlog(timeout_ms));
  qed_hs_assert(cbt->alpha > 0);
}
#endif /* defined(QED_HS_UNIT_TESTS) */
...
#undef MAX_TIMEOUT
  ...
  log_notice(LD_CIRC,
            ...
            qed_hs_lround(cbt->timeout_ms), timeout_count, total_build_times);
...
{
  long prev_timeout = qed_hs_lround(cbt->timeout_ms/1000);
  double timeout_rate;
  ...
  if (prev_timeout > qed_hs_lround(cbt->timeout_ms/1000)) {
    log_info(LD_CIRC,
