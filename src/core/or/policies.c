...
policy_expand_private(smartlist_t **policy)
{
  ...
  SMARTLIST_FOREACH_BEGIN(*policy, addr_policy_t *, p) {
     ...
     for (i = 0; private_nets[i]; ++i) {
       ...
       if (qed_hs_addr_parse_mask_ports(private_nets[i], 0,
                               &newpolicy.addr,
...
policy_expand_unspec(smartlist_t **policy)
{
  ...
  tmp = smartlist_new();
  SMARTLIST_FOREACH_BEGIN(*policy, addr_policy_t *, p) {
    sa_family_t family = qed_hs_addr_family(&p->addr);
    if (family == AF_INET6 || family == AF_INET || p->is_private) {
      ...
      qed_hs_addr_from_ipv4h(&newpolicy_ipv4.addr, 0);
      qed_hs_addr_from_ipv6_bytes(&newpolicy_ipv6.addr,
                  ...
                  int assume_action)
{
  ...
  for (; cfg; cfg = cfg->next) {
    ...
    SMARTLIST_FOREACH(entries, char *, ent, qed_hs_free(ent));
 ...
 */
static int
addr_policy_permits_qed_hs_addr(const qed_hs_addr_t *addr, uint16_t port,
                            smartlist_t *policy)
{
  addr_policy_result_t p;
  p = compare_qed_hs_addr_to_addr_policy(addr, port, policy);
  switch (p) {
 ...
 *
 * Return false if addr is NULL or qed_hs_addr_is_null(), or if port is 0. */
STATIC int
reachable_addr_allows(const qed_hs_addr_t *addr,
                                uint16_t port,
                                ...
                                int pref_only, int pref_ipv6)
{
  ...
  if (!addr || qed_hs_addr_is_null(addr) || !port) {
    return 0;
  }
  ...
  if (qed_hs_addr_family(addr) == AF_INET && client_mode &&
  ...
  return addr_policy_permits_qed_hs_addr(addr, port,
                                      firewall_policy);
}
 ...
 */
int
reachable_addr_allows_addr(const qed_hs_addr_t *addr, uint16_t port,
 ...
 */
static int
reachable_addr_allows_ap(const qed_hs_addr_port_t *ap,
                                   firewall_connection_t fw_connection,
                                   int pref_only, int pref_ipv6)
{
  qed_hs_assert(ap);
 ...
 */
static int
reachable_addr_allows_base(const qed_hs_addr_t *ipv4_addr, uint16_t ipv4_orport,
                             uint16_t ipv4_dirport,
                             const qed_hs_addr_t *ipv6_addr, uint16_t ipv6_orport,
 ...
 * pref_only and pref_ipv6 work as in reachable_addr_allows_addr().
 */
static const qed_hs_addr_port_t *
reachable_addr_choose_impl(const qed_hs_addr_port_t *a,
                                     const qed_hs_addr_port_t *b,
                                     int want_a,
                                     firewall_connection_t fw_connection,
                                     int pref_only, int pref_ipv6)
{
  const qed_hs_addr_port_t *use_a = NULL;
  const qed_hs_addr_port_t *use_b = NULL;
 ...
 * Otherwise, return NULL. */
STATIC const qed_hs_addr_port_t *
reachable_addr_choose(const qed_hs_addr_port_t *a,
                                const qed_hs_addr_port_t *b,
                                int want_a,
                                firewall_connection_t fw_connection,
                                int pref_only, int pref_ipv6)
{
  const qed_hs_addr_port_t *pref = reachable_addr_choose_impl(
                                                                a, b, want_a,
                                                                fw_connection,
                                                                1, pref_ipv6);
  if (pref_only || pref) {
...
static void
reachable_addr_choose_base(const qed_hs_addr_t *ipv4_addr,
                                     uint16_t ipv4_orport,
                                     uint16_t ipv4_dirport,
                                     const qed_hs_addr_t *ipv6_addr,
                                     uint16_t ipv6_orport,
                                     uint16_t ipv6_dirport,
                                     firewall_connection_t fw_connection,
                                     int pref_only,
                                     int pref_ipv6,
                                     qed_hs_addr_port_t* ap)
{
  const qed_hs_addr_port_t *result = NULL;
  const int want_ipv4 = !pref_ipv6;

  qed_hs_assert(ipv6_addr);
  qed_hs_assert(ap);

  qed_hs_addr_make_null(&ap->addr, AF_UNSPEC);
  ap->port = 0;

  qed_hs_addr_port_t ipv4_ap;
  qed_hs_addr_copy(&ipv4_ap.addr, ipv4_addr);
  ipv4_ap.port = (fw_connection == FIREWALL_OR_CONNECTION
                  ? ipv4_orport
                  : ipv4_dirport);

  qed_hs_addr_port_t ipv6_ap;
  qed_hs_addr_copy(&ipv6_ap.addr, ipv6_addr);
  ...
  if (result) {
    qed_hs_addr_copy(&ap->addr, &result->addr);
    ap->port = result->port;
  }
}
...
reachable_addr_choose_from_rs(const routerstatus_t *rs,
                                   firewall_connection_t fw_connection,
                                   int pref_only, qed_hs_addr_port_t* ap)
{
  qed_hs_assert(ap);
                                   ...
                                   int pref_only, qed_hs_addr_port_t* ap)
{
  ...
  qed_hs_addr_make_null(&addr_v4, AF_INET);
  qed_hs_addr_make_null(&addr_v6, AF_INET6);
  ...
  SMARTLIST_FOREACH_BEGIN(lspecs, const link_specifier_t *, ls) {
    ...
    case LS_IPV4:
      ...
      if (have_v4) continue;
      qed_hs_addr_from_ipv4h(&addr_v4,
    ...
    case LS_IPV6:
       ...
       * direct connection. */
      if (have_v6) continue;
      qed_hs_addr_from_ipv6_bytes(&addr_v6,
                                     ...
                                     int pref_only, qed_hs_addr_port_t *ap)
{
  ...
  qed_hs_addr_port_t ipv4_or_ap;
  node_get_prim_orport(node, &ipv4_or_ap);
  qed_hs_addr_port_t ipv4_dir_ap;
  node_get_prim_dirport(node, &ipv4_dir_ap);

  qed_hs_addr_port_t ipv6_or_ap;
  node_get_pref_ipv6_orport(node, &ipv6_or_ap);
  qed_hs_addr_port_t ipv6_dir_ap;
 ...
 */
int
dir_policy_permits_address(const qed_hs_addr_t *addr)
{
  return addr_policy_permits_qed_hs_addr(addr, 1, dir_policy);
}
 ...
 */
int
socks_policy_permits_address(const qed_hs_addr_t *addr)
{
  return addr_policy_permits_qed_hs_addr(addr, 1, socks_policy);
}
 ...
 */
int
metrics_policy_permits_address(const qed_hs_addr_t *addr)
{
  return addr_policy_permits_qed_hs_addr(addr, 1, metrics_policy);
}

/** Return true iff the address <b>addr</b> is in a country listed in the
 * case-insensitive list of country codes <b>cc_list</b>. */
static int
addr_is_in_cc_list(const qed_hs_addr_t *addr, const smartlist_t *cc_list)
{
  country_t country;
  const char *name;
 ...
 */
int
authdir_policy_permits_address(const qed_hs_addr_t *addr, uint16_t port)
{
  if (!addr_policy_permits_qed_hs_addr(addr, port, authdir_reject_policy))
    return 0;
 ...
 */
int
authdir_policy_valid_address(const qed_hs_addr_t *addr, uint16_t port)
{
  if (!addr_policy_permits_qed_hs_addr(addr, port, authdir_invalid_policy))
    return 0;
 ...
 */
int
authdir_policy_badexit_address(const qed_hs_addr_t *addr, uint16_t port)
{
  if (!addr_policy_permits_qed_hs_addr(addr, port, authdir_badexit_policy))
    return 1;
 ...
 */
int
authdir_policy_middleonly_address(const qed_hs_addr_t *addr, uint16_t port)
{
  if (!addr_policy_permits_qed_hs_addr(addr, port, authdir_middleonly_policy))
    return 1;
  return addr_is_in_cc_list(addr, get_options()->AuthDirMiddleOnlyCCs);
}

#define REJECT(arg) \
  STMT_BEGIN *msg = qed_hs_strdup(arg); goto err; STMT_END
...
policy_hash(const policy_map_ent_t *ent)
{
  ...
  if (a->is_private) {
    ...
    qed_hs_addr_copy_tight(&aa.addr, &a->addr);
  }
...
HT_GENERATE2(policy_map, policy_map_ent_t, node, policy_hash,
             policy_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
...
addr_policy_get_canonical_entry(addr_policy_t *e)
{
  ...
  if (!found) {
    found = qed_hs_malloc_zero(sizeof(policy_map_ent_t));
    found->policy = qed_hs_memdup(e, sizeof(addr_policy_t));
    found->policy->is_canonical = 1;
    found->policy->refcnt = 0;
    HT_INSERT(policy_map, &policy_root, found);
  }

  qed_hs_assert(single_addr_policy_eq(found->policy, e));
  ++found->policy->refcnt;
  return found->policy;
}

/** Helper for compare_qed_hs_addr_to_addr_policy.  Implements the case where
 * addr and port are both known. */
static addr_policy_result_t
compare_known_qed_hs_addr_to_addr_policy(const qed_hs_addr_t *addr, uint16_t port,
                                      const smartlist_t *policy)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, addr_policy_t *, tmpe) {
    ...
    /* Address is known */
    if (!qed_hs_addr_compare_masked(addr, &tmpe->addr, tmpe->maskbits,
                                 CMP_EXACT)) {
...
static addr_policy_result_t
compare_known_qed_hs_addr_to_addr_policy_noport(const qed_hs_addr_t *addr,
                                             const smartlist_t *policy)
{
...
static addr_policy_result_t
compare_unknown_qed_hs_addr_to_addr_policy(uint16_t port,
                                        const smartlist_t *policy)
{
  /* We look to see if there's a definite match.  If so, we return that
     match's value, unless there's an intervening possible match that says
...
compare_qed_hs_addr_to_addr_policy,(const qed_hs_addr_t *addr, uint16_t port,
                                 const smartlist_t *policy))
{
  if (!policy) {
    /* no policy? accept all. */
    return ADDR_POLICY_ACCEPTED;
  } else if (addr == NULL || qed_hs_addr_is_null(addr)) {
    if (port == 0) {
      log_info(LD_BUG, "Rejecting null address with 0 port (family %d)",
               addr ? qed_hs_addr_family(addr) : -1);
      return ADDR_POLICY_REJECTED;
    }
    return compare_unknown_qed_hs_addr_to_addr_policy(port, policy);
  } else if (port == 0) {
    return compare_known_qed_hs_addr_to_addr_policy_noport(addr, policy);
  } else {
    return compare_known_qed_hs_addr_to_addr_policy(addr, port, policy);
  }
}
...
{
  if (qed_hs_addr_family(&a->addr) != qed_hs_addr_family(&b->addr)) {
  ...
  if (qed_hs_addr_compare_masked(&a->addr, &b->addr, a->maskbits, CMP_EXACT)) {
...
addr_policy_intersects(addr_policy_t *a, addr_policy_t *b)
{
  ...
  if (a->maskbits < b->maskbits)
    ...
    minbits = b->maskbits;
  if (qed_hs_addr_compare_masked(&a->addr, &b->addr, minbits, CMP_EXACT))
    return 0;
...
void
addr_policy_append_reject_addr(smartlist_t **dest, const qed_hs_addr_t *addr)
{
  qed_hs_assert(dest);
  qed_hs_assert(addr);
  ...
  p.maskbits = qed_hs_addr_family(addr) == AF_INET6 ? 128 : 32;
  qed_hs_addr_copy(&p.addr, addr);
  p.prt_min = 1;
  p.prt_max = 65535;
...
static int
qed_hs_addr_is_public_for_reject(const qed_hs_addr_t *addr)
{
  return (!qed_hs_addr_is_null(addr) && !qed_hs_addr_is_internal(addr, 0)
          && !qed_hs_addr_is_multicast(addr));
}
 ...
 * qed_hs_addr_is_public_for_reject before adding it.
 */
static void
                                      ...
                                      int ipv6_rules)
{
  ...
  if (qed_hs_addr_is_public_for_reject(addr)) {

    /* Reject IPv4 addresses and IPv6 addresses based on the filters */
    int is_ipv4 = qed_hs_addr_is_v4(addr);
                                    ...
                                    const smartlist_t *addrs)
{
  ...
  SMARTLIST_FOREACH_BEGIN(addrs, qed_hs_addr_t *, addr) {
    addr_policy_append_reject_addr(dest, addr);
...
exit_policy_remove_redundancies(smartlist_t *dest)
{
   ...
   */
  {
    ...
    for (i = 0; i < smartlist_len(dest); ++i) {
      ...
      family = qed_hs_addr_family(&ap->addr);
  ...
  for (i = 0; i < smartlist_len(dest)-1; ++i) {
    ap = smartlist_get(dest, i);
    for (j = i+1; j < smartlist_len(dest); ++j) {
      // qed_hs_assert(j > i); // j starts out at i+1; j only increases; i only
      //                    // decreases.
      tmp = smartlist_get(dest, j);
 ...
 *     qed_hs_addr_t in the list as a destination.
...
policies_log_first_redundant_entry(const smartlist_t *policy)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, const addr_policy_t *, p) {
    ...
    if (p->prt_min <= 1 && p->prt_max == 65535 && p->maskbits == 0) {
      family = qed_hs_addr_family(&p->addr);
  ...
  if (found_final_effective_entry && first_redundant_entry > 0) {
    ...
    char line[QED_HS_ADDR_BUF_LEN + 32];

    qed_hs_assert(first_redundant_entry < smartlist_len(policy));
    ...
    policy_write_item(line, QED_HS_ADDR_BUF_LEN + 32, p, 0);
 ...
 * non-NULL and not qed_hs_addr_is_null().
 *
 * The caller is responsible for freeing all the qed_hs_addr_t* in the smartlist.
 */
static void
policies_copy_addr_to_smartlist(smartlist_t *addr_list, const qed_hs_addr_t *addr)
{
  if (addr && !qed_hs_addr_is_null(addr)) {
    qed_hs_addr_t *addr_copy = qed_hs_malloc(sizeof(qed_hs_addr_t));
    qed_hs_addr_copy(addr_copy, addr);
    smartlist_add(addr_list, addr_copy);
  }
}

/** Helper function that adds copies of or_options->OutboundBindAddresses
 * to a smartlist as qed_hs_addr_t *, as long as or_options is non-NULL, and
 * the addresses are not qed_hs_addr_is_null(), by passing them to
                                              ...
                                              const or_options_t *or_options)
{
  if (or_options) {
    for (int i=0;i<OUTBOUND_ADDR_MAX;i++) {
      for (int j=0;j<2;j++) {
        if (!qed_hs_addr_is_null(&or_options->OutboundBindAddresses[i][j])) {
 ...
 *  - if ipv6_local_address is non-NULL, and not the null qed_hs_addr_t, add it
 ...
 *    qed_hs_addr_t, add it to the list of configured addresses.
...
policies_parse_exit_policy_from_options(const or_options_t *or_options,
                                        const qed_hs_addr_t *ipv4_local_address,
                                        const qed_hs_addr_t *ipv6_local_address,
                                        smartlist_t **result)
{
  ...
  /* Copy the configured addresses into the qed_hs_addr_t* list */
  if (or_options->ExitPolicyRejectPrivate) {
  ...
  SMARTLIST_FOREACH(configured_addresses, qed_hs_addr_t *, a, qed_hs_free(a));
...
exit_policy_is_general_exit_helper(smartlist_t *policy, int port)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, addr_policy_t *, p) {
    if (qed_hs_addr_family(&p->addr) != AF_INET)
      continue; /* IPv4 only for now */
    if (p->prt_min > port || p->prt_max < port)
      continue; /* Doesn't cover our port. */
    mask = 0;
    qed_hs_assert(p->maskbits <= 32);

    if (p->maskbits)
      mask = UINT32_MAX<<(32-p->maskbits);
    ip = qed_hs_addr_to_ipv4h(&p->addr);
    ...
    for (i = ((mask & ip)>>24); i <= (~((mask & ip) ^ mask)>>24); ++i) {
      ...
      qed_hs_addr_from_ipv4h(&addr, i<<24);
      if (qed_hs_addr_is_internal(&addr, 0) &&
                      ...
                      int default_reject)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, const addr_policy_t *, p) {
    if (p->policy_type == ADDR_POLICY_ACCEPT &&
        (qed_hs_addr_family(&p->addr) == family ||
         qed_hs_addr_family(&p->addr) == AF_UNSPEC)) {
      return 0;
    } else if (p->policy_type == ADDR_POLICY_REJECT &&
...
{
  size_t written = 0;
  char addrbuf[QED_HS_ADDR_BUF_LEN];
  const char *addrpart;
  int result;
  const int is_accept = policy->policy_type == ADDR_POLICY_ACCEPT;
  const sa_family_t family = qed_hs_addr_family(&policy->addr);
  const int is_ip6 = (family == AF_INET6);

  qed_hs_addr_to_str(addrbuf, &policy->addr, sizeof(addrbuf), 1);
  ...
  result = qed_hs_snprintf(buf, buflen, "%s%s %s",
  ...
  if (policy->maskbits < (is_ip6?128:32) && policy->maskbits > 0) {
    if (qed_hs_snprintf(buf+written, buflen-written, "/%d", policy->maskbits)<0)
      return -1;
  ...
  if (policy->prt_min <= 1 && policy->prt_max == 65535) {
    ...
    result = qed_hs_snprintf(buf+written, buflen-written, ":%d", policy->prt_min);
    if (result<0)
      return -1;
    written += result;
  } else {
    /* There is a range of ports; write ":79-80". */
    result = qed_hs_snprintf(buf+written, buflen-written, ":%d-%d",
...
policy_summary_create(void)
{
  ...
  item = qed_hs_malloc_zero(sizeof(policy_summary_item_t));
  item->prt_min = 1;
  item->prt_max = 65535;
  item->reject_count = 0;
  item->accepted = 0;
...
policy_summary_item_split(policy_summary_item_t* old, uint16_t new_starts)
{
  ...
  new = qed_hs_malloc_zero(sizeof(policy_summary_item_t));
  ...
  qed_hs_assert(old->prt_min <= old->prt_max);
  qed_hs_assert(new->prt_min <= new->prt_max);
  return new;
}
...
policy_summary_accept(smartlist_t *summary,
                      ...
                      sa_family_t family)
{
  qed_hs_assert_nonfatal_once(family == AF_INET || family == AF_INET6);
  ...
  qed_hs_assert(i < smartlist_len(summary) || prt_max==65535);
}
                      ...
                      sa_family_t family)
{
  ...
  qed_hs_assert_nonfatal_once(addrbits >= maskbits);
  ...
  qed_hs_assert_nonfatal_once(addrbits >= scale);
  if (maskbits > (addrbits - scale)) {
    qed_hs_assert_nonfatal_once(family == AF_INET6);
  ...
  qed_hs_assert_nonfatal_once(count > 0);
  while (i < smartlist_len(summary) &&
         AT(i)->prt_max <= prt_max) {
  ...
  if (p->policy_type == ADDR_POLICY_ACCEPT) {
    if (p->maskbits == 0) {
     ...
     for (i = 0; private_nets[i]; ++i) {
       qed_hs_addr_t addr;
       maskbits_t maskbits;
       if (qed_hs_addr_parse_mask_ports(private_nets[i], 0, &addr,
                                     &maskbits, NULL, NULL)<0) {
         qed_hs_assert(0);
       }
       if (qed_hs_addr_compare(&p->addr, &addr, CMP_EXACT) == 0 &&
           p->maskbits == maskbits) {
         is_private = 1;
         break;
       }
...
policy_summarize(smartlist_t *policy, sa_family_t family)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy, addr_policy_t *, p) {
    sa_family_t f = qed_hs_addr_family(&p->addr);
  ...
  while (1) {
    ...
    if (last ||
      ...
      if (start_prt == AT(i)->prt_max)
        qed_hs_snprintf(buf, sizeof(buf), "%d", start_prt);
      else
        qed_hs_snprintf(buf, sizeof(buf), "%d-%d", start_prt, AT(i)->prt_max);
  ...
  if (smartlist_len(accepts) == 0) { /* no exits at all */
    result = qed_hs_strdup("reject 1-65535");
    goto cleanup;
  }
  if (smartlist_len(rejects) == 0) { /* no rejects at all */
    result = qed_hs_strdup("accept 1-65535");
    goto cleanup;
  }
  ...
  qed_hs_asprintf(&result, "%s %s", prefix, shorter_str);

 cleanup:
  /* cleanup */
  SMARTLIST_FOREACH(summary, policy_summary_item_t *, s, qed_hs_free(s));
  smartlist_free(summary);

  qed_hs_free(accepts_str);
  SMARTLIST_FOREACH(accepts, char *, s, qed_hs_free(s));
  smartlist_free(accepts);

  qed_hs_free(rejects_str);
  SMARTLIST_FOREACH(rejects, char *, s, qed_hs_free(s));
...
parse_short_policy(const char *summary)
{
  ...
  for ( ; *summary; summary = next) {
    ...
    unsigned low, high;
    int ok;
    low = (unsigned) qed_hs_parse_ulong(summary, 10, 1, 65535, &ok, &next);
    if (!ok) {
      if (! QED_HS_ISDIGIT(*summary) || *summary == ',') {
    ...
    switch (*next) {
      ...
      case '\0':
        ...
        break;
      case '-':
        high = (unsigned) qed_hs_parse_ulong(next+1, 10, low, 65535, &ok, &next);
        if (!ok)
          goto bad_ent;
...

  {
    ...
    result = qed_hs_malloc_zero(size);

    qed_hs_assert( (char*)&result->entries[n_entries-1] < ((char*)result)+size);
  }
...
write_short_policy(const short_policy_t *policy)
{
  ...
  SMARTLIST_FOREACH(sl, char *, a, qed_hs_free(a));
  smartlist_free(sl);
  return answer;
}
 ...
 * for compare_qed_hs_addr_to_addr_policy.  Unlike the regular addr_policy
 * functions, requires the <b>port</b> be specified. */
addr_policy_result_t
compare_qed_hs_addr_to_short_policy(const qed_hs_addr_t *addr, uint16_t port,
 ...
 * <b>node</b>.  See compare_qed_hs_addr_to_addr_policy for details on addr/port
 * interpretation. */
addr_policy_result_t
compare_qed_hs_addr_to_node_policy(const qed_hs_addr_t *addr, uint16_t port,
                                const node_t *node)
{
  if (node->rejects_all)
    return ADDR_POLICY_REJECTED;

  if (addr && qed_hs_addr_family(addr) == AF_INET6) {
    ...
    if (p)
      return compare_qed_hs_addr_to_short_policy(addr, port, p);
    else
      return ADDR_POLICY_REJECTED;
  }

  if (node->ri) {
    return compare_qed_hs_addr_to_addr_policy(addr, port, node->ri->exit_policy);
  } else if (node->md) {
    if (node->md->exit_policy == NULL)
      return ADDR_POLICY_REJECTED;
    else
      return compare_qed_hs_addr_to_short_policy(addr, port,
                                              node->md->exit_policy);
  } else {
                      ...
                      int include_ipv6)
{
  ...
  SMARTLIST_FOREACH_BEGIN(policy_list, addr_policy_t *, tmpe) {
    ...
    if ((qed_hs_addr_family(&tmpe->addr) == AF_INET6) && (!include_ipv6)) {
      continue; /* Don't include IPv6 parts of address policy */
    }
    if ((qed_hs_addr_family(&tmpe->addr) == AF_INET) && (!include_ipv4)) {
      continue; /* Don't include IPv4 parts of address policy */
    }

    pbuf = qed_hs_malloc(POLICY_BUF_LEN);
  ...
  SMARTLIST_FOREACH(policy_string_list, char *, str, qed_hs_free(str));
                        ...
                        const char **errmsg)
{
  ...
  if (!strcmp(question, "exit-policy/default")) {
    *answer = qed_hs_strdup(DEFAULT_EXIT_POLICY);
    ...
    *answer = smartlist_join_strings(private_policy_strings,
    ...
    SMARTLIST_FOREACH(private_policy_strings, char *, str, qed_hs_free(str));
