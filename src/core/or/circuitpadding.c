/* Copyright (c) 2017 The QED Project, Inc. */
...
circpad_circuit_machineinfo_free_idx(circuit_t *circ, int idx)
{
  if (circ->padding_info[idx]) {
    ...
    qed_hs_free(circ->padding_info[idx]->histogram);
    timer_free(circ->padding_info[idx]->padding_timer);
    qed_hs_free(circ->padding_info[idx]);
  }
}
...
{
  circpad_machine_runtime_t *mi =
    qed_hs_malloc_zero(sizeof(circpad_machine_runtime_t));
...
circpad_is_token_removal_supported(circpad_machine_runtime_t *mi)
{
  ...
  if (mi->histogram == NULL) {
    ...
    qed_hs_assert_nonfatal(mi->histogram_len == 0);
    return 0;
  } else {
    ...
    qed_hs_assert_nonfatal(state->token_removal != CIRCPAD_TOKEN_REMOVAL_NONE);
    qed_hs_assert_nonfatal(state->histogram_len == mi->histogram_len);
    qed_hs_assert_nonfatal(mi->histogram_len != 0);
    return 1;
  }
...
circpad_machine_setup_tokens(circpad_machine_runtime_t *mi)
{
  ...
  if (!mi->histogram || (mi->histogram
    ...
    qed_hs_free(mi->histogram); // null ok
    mi->histogram = qed_hs_malloc_zero(sizeof(circpad_hist_token_t)
                                    *state->histogram_len);
  }
  mi->histogram_len = state->histogram_len;
                                      ...
                                      circpad_delay_t delay_shift)
{
  ...
  return (circpad_delay_t)MIN(qed_hs_llround(val), CIRCPAD_DELAY_INFINITE);
}
...
circpad_machine_sample_delay(circpad_machine_runtime_t *mi)
{
  ...
  qed_hs_assert(curr_bin < CIRCPAD_INFINITY_BIN(state));
 ...
 */
static void
circpad_send_padding_callback(qed_hs_timer_t *timer, void *args,
...
circpad_machine_schedule_padding,(circpad_machine_runtime_t *mi))
{
  ...
  timeout.tv_sec = in_usec/QED_HS_USEC_PER_SEC;
  timeout.tv_usec = (in_usec%QED_HS_USEC_PER_SEC);
                            ...
                            circpad_statenum_t num_states)
{
  ...
  machine->states = qed_hs_malloc_zero(sizeof(circpad_state_t)*num_states);
...
circpad_circ_client_machine_init(void)
{
  circpad_machine_spec_t *circ_client_machine
      = qed_hs_malloc_zero(sizeof(circpad_machine_spec_t));
...
{
  qed_hs_assert_nonfatal(origin_padding_machines == NULL);
  qed_hs_assert_nonfatal(relay_padding_machines == NULL);
...
circpad_machines_free(void)
{
  if (origin_padding_machines) {
    SMARTLIST_FOREACH(origin_padding_machines,
                      circpad_machine_spec_t *,
                      m, qed_hs_free(m->states); qed_hs_free(m));
