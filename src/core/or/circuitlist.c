...
#include "lib/cc/torint.h"  /* QED_HS_PRIuSZ */
...
{
  qed_hs_assert(x->magic == OR_CIRCUIT_MAGIC);
...
{
  qed_hs_assert(x->magic == ORIGIN_CIRCUIT_MAGIC);
...
HT_GENERATE2(chan_circid_map, chan_circid_circuit_map_t, node,
             ...
             qed_hs_reallocarray_, qed_hs_free_);
                               ...
                               channel_t *chan)
{
  ...
  *circid_ptr = id;
  ...
  if (found) {
    ...
    found = qed_hs_malloc_zero(sizeof(chan_circid_circuit_map_t));
    found->circ_id = id;
    found->chan = chan;
    found->circuit = circ;
...
channel_mark_circid_unusable(channel_t *chan, circid_t id)
{
  ...
  if (ent && ent->circuit) {
    ...
    ent = qed_hs_malloc_zero(sizeof(chan_circid_circuit_map_t));
    ent->chan = chan;
    ent->circ_id = id;
...
circuit_event_status(origin_circuit_t *circ, circuit_status_event_t tp,
                     int reason_code)
{
  ocirc_cevent_msg_t *msg = qed_hs_malloc(sizeof(*msg));
...
circuit_set_state(circuit_t *circ, uint8_t state)
{
  ...
  if (state == CIRCUIT_STATE_GUARD_WAIT || state == CIRCUIT_STATE_OPEN)
    qed_hs_assert(!circ->n_chan_create_cell);

  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(change_state), circ, circ->state, state);
  circ->state = state;
...
circuit_get_all_pending_on_channel(smartlist_t *out, channel_t *chan)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuits_pending_chans, circuit_t *, circ) {
    ...
    if (!circ->n_hop)
      continue;
    qed_hs_assert(circ->state == CIRCUIT_STATE_CHAN_WAIT);
    if (qed_hs_digest_is_zero(circ->n_hop->identity_digest)) {
      /* Look at addr/port. This is an unkeyed connection. */
      if (!channel_matches_extend_info(chan, circ->n_hop))
        continue;
    } else {
      /* We expected a key. See if it's the right one. */
      if (qed_hs_memneq(chan->identity_digest,
...
circuit_remove_from_origin_circuit_list(origin_circuit_t *origin_circ)
{
  ...
  if (origin_idx < 0)
    return;
  origin_circuit_t *c2;
  qed_hs_assert(origin_idx <= smartlist_len(global_origin_circuit_list));
  c2 = smartlist_get(global_origin_circuit_list, origin_idx);
  qed_hs_assert(origin_circ == c2);
...
{
  qed_hs_assert(origin_circ->global_origin_circuit_list_idx == -1);
...
circuit_close_all_marked(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuits_pending_close, circuit_t *, circ) {
    qed_hs_assert(circ->marked_for_close);
...
circuit_state_to_string(int state)
{
  ...
  switch (state) {
    ...
    default:
      ...
      qed_hs_snprintf(buf, sizeof(buf), "unknown state [%d]", state);
      return buf;
  }
}
...
circuit_purpose_to_controller_string(uint8_t purpose)
{
  ...
  switch (purpose) {
    ...
    default:
      qed_hs_snprintf(buf, sizeof(buf), "UNKNOWN_%d", (int)purpose);
      return buf;
  }
}
...
origin_circuit_new(void)
{
  ...
  circ = qed_hs_malloc_zero(sizeof(origin_circuit_t));
  ...
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(new_origin), circ);
  return circ;
}
...
or_circuit_new(circid_t p_circ_id, channel_t *p_chan)
{
  ...
  circ = qed_hs_malloc_zero(sizeof(or_circuit_t));
  ...
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(new_or), circ);
  return circ;
}
...
circuit_free_(circuit_t *circ)
{
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    ...
    qed_hs_assert(circ->magic == ORIGIN_CIRCUIT_MAGIC);
    ...
    qed_hs_assert(circ->magic == OR_CIRCUIT_MAGIC);
    ...
    if (ocirc->rend_splice) {
      ...
      qed_hs_assert(other->base_.magic == OR_CIRCUIT_MAGIC);
  ...
  if (circ->sendme_last_digests) {
    SMARTLIST_FOREACH(circ->sendme_last_digests, uint8_t *, d, qed_hs_free(d));
  ...
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(free), circ);
...
{
  qed_hs_log(severity, LD_CIRC, "Conn %d has %s circuit: circID %u "
                                   ...
                                   int *found_entry_out)
{
  ...
  {
    circuit_t *circ;
    QED_HS_LIST_FOREACH(circ, &global_circuitlist, head) {
      if (! CIRCUIT_IS_ORIGIN(circ)) {
...
circuit_get_next_by_purpose(origin_circuit_t *start, uint8_t purpose)
{
  int idx;
  smartlist_t *lst = circuit_get_global_list();
  qed_hs_assert(CIRCUIT_PURPOSE_IS_ORIGIN(purpose));
  if (start == NULL)
    idx = 0;
  else
    idx = TO_CIRCUIT(start)->global_circuitlist_idx + 1;
...
get_circuit_purpose_needed_to_cannibalize(uint8_t purpose)
{
  if (circuit_should_use_vanguards(purpose)) {
    ...
    qed_hs_assert_nonfatal(purpose != CIRCUIT_PURPOSE_CONFLUX_UNLINKED &&
                            ...
                            int flags)
{
   ...
   * cannibalization. */
  qed_hs_assert(!(flags & CIRCLAUNCH_ONEHOP_TUNNEL));

  purpose_to_search_for = get_circuit_purpose_needed_to_cannibalize(
                                                  purpose_to_produce);

  qed_hs_assert_nonfatal(purpose_to_search_for == CIRCUIT_PURPOSE_C_GENERAL ||
                      purpose_to_search_for == CIRCUIT_PURPOSE_HS_VANGUARDS);

  qed_hs_assert_nonfatal(purpose_to_search_for !=
                      CIRCUIT_PURPOSE_CONFLUX_UNLINKED);
  qed_hs_assert_nonfatal(purpose_to_produce != CIRCUIT_PURPOSE_CONFLUX_UNLINKED);
    ...
    if (CIRCUIT_IS_ORIGIN(circ_) &&
      ...
      if ((!need_uptime || circ->build_state->need_uptime) &&
        ...
        if (info) {
          ...
          do {
            const node_t *ri2;
            if (qed_hs_memeq(hop->extend_info->identity_digest,
 ...
 *
 * See https://bugs.qed-cc.org/tpo/core/qed-hs/23512
 */
void
                                         ...
                                         circuit_channel_direction_t dir)
{
  ...
  bool is_ipv6 = chan &&
    ...
    qed_hs_addr_family(&remote_addr) == AF_INET6;
                          ...
                          const char *file))
{
  ...
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(mark_for_close), circ);
}
...
single_conn_free_bytes(connection_t *conn)
{
  ...
  if (conn->type == CONN_TYPE_DIR) {
    ...
    if (dir_conn->compress_state) {
      result += qed_hs_compress_state_size(dir_conn->compress_state);
...
circuit_max_queued_cell_age(const circuit_t *c, uint32_t now)
{
  ...
  if (NULL != (cell = QED_HS_SIMPLEQ_FIRST(&c->n_chan_cells.head)))
    age = now - cell->inserted_timestamp;

  if (! CIRCUIT_IS_ORIGIN(c)) {
    const or_circuit_t *orcirc = CONST_TO_OR_CIRCUIT(c);
    if (NULL != (cell = QED_HS_SIMPLEQ_FIRST(&orcirc->p_chan_cells.head))) {
...
circuits_handle_oom(size_t current_allocation)
{
  ...
  log_notice(LD_GENERAL, "We're low on memory (cell queues total alloc:"
             " %"QED_HS_PRIuSZ" buffer total alloc: %" QED_HS_PRIuSZ ","
             " tor compress total alloc: %" QED_HS_PRIuSZ
             " (zlib: %" QED_HS_PRIuSZ ", zstd: %" QED_HS_PRIuSZ ","
             " lzma: %" QED_HS_PRIuSZ "),"
             " rendezvous cache total alloc: %" QED_HS_PRIuSZ "). Killing"
             " circuits withover-long queues. (This behavior is controlled by"
             " MaxMemInQueues.)",
             cell_queues_get_total_allocation(),
             buf_get_total_allocation(),
             qed_hs_compress_get_total_allocation(),
 ...
 done_recovering_mem:
  log_notice(LD_GENERAL, "Removed %"QED_HS_PRIuSZ" bytes by killing %d circuits; "
...
assert_circuit_ok,(const circuit_t *c))
{
  ...
  qed_hs_assert(c);
  qed_hs_assert(c->magic == ORIGIN_CIRCUIT_MAGIC || c->magic == OR_CIRCUIT_MAGIC);
  qed_hs_assert(c->purpose >= CIRCUIT_PURPOSE_MIN_ &&
  ...
  qed_hs_assert(c->deliver_window >= 0);
  qed_hs_assert(c->package_window >= 0);
  if (c->state == CIRCUIT_STATE_OPEN ||
      c->state == CIRCUIT_STATE_GUARD_WAIT) {
    qed_hs_assert(!c->n_chan_create_cell);
    if (or_circ) {
      relay_crypto_assert_ok(&or_circ->crypto);
    }
  }
  if (c->state == CIRCUIT_STATE_CHAN_WAIT && !c->marked_for_close) {
    qed_hs_assert(circuits_pending_chans &&
  ...
  if (c->purpose == CIRCUIT_PURPOSE_REND_ESTABLISHED) {
    ...
    if (!c->marked_for_close) {
      qed_hs_assert(or_circ->rend_splice);
      qed_hs_assert(or_circ->rend_splice->rend_splice == or_circ);
    }
    qed_hs_assert(or_circ->rend_splice != or_circ);
  } else {
    qed_hs_assert(!or_circ || !or_circ->rend_splice);
  }
}
