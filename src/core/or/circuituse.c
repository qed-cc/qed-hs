                      ...
                      time_t now)
{
  ...
  if (purpose == CIRCUIT_PURPOSE_C_GENERAL ||
    ...
    if (build_state->onehop_tunnel) {
      ...
      qed_hs_assert(conn->chosen_exit_name);
      if (build_state->chosen_exit) {
        ...
        if (qed_hs_memneq(digest, build_state->chosen_exit->identity_digest,
                          DIGEST_LEN))
          return 0; /* this is a circuit to somewhere else */
        if (qed_hs_digest_is_zero(digest)) {
          /* we don't know the digest; have to compare addr:port */
          const int family = qed_hs_addr_parse(&addr,
    ...
    if (origin_circ->prepend_policy) {
      if (qed_hs_addr_parse(&addr, conn->socks_request->address) != -1) {
        int r = compare_qed_hs_addr_to_addr_policy(&addr,
                                                conn->socks_request->port,
                 ...
                 int need_uptime, int need_internal)
{
  ...
  qed_hs_assert(purpose == CIRCUIT_PURPOSE_C_GENERAL ||
             purpose == CIRCUIT_PURPOSE_HS_VANGUARDS ||
             purpose == CIRCUIT_PURPOSE_C_HSDIR_GET ||
             purpose == CIRCUIT_PURPOSE_S_HSDIR_POST ||
...
#define SET_CUTOFF(target, msec) do {                       \
  ...
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *,victim) {
    ...
    qed_hs_trace(TR_SUBSYS(circuit), TR_EV(timeout), TO_ORIGIN_CIRCUIT(victim));
    if (victim->purpose == CIRCUIT_PURPOSE_C_MEASURE_TIMEOUT)
...
#define MAX_ANCIENT_ONEHOP_CIRCUITS_TO_LOG 10
  ...
  SMARTLIST_FOREACH_BEGIN(log_these, const origin_circuit_t *, ocirc) {
    ...
    if (circ->timestamp_dirty) {
      ...
      qed_hs_asprintf(&dirty, "Dirty since %s (%ld seconds vs %ld-second cutoff)",
                   dirty_since, (long)(now - circ->timestamp_dirty),
                   (long) options->MaxCircuitDirtiness);
    } else {
      dirty = qed_hs_strdup("Not marked dirty");
    }
    ...
    if (CIRCUIT_IS_ORIGIN(circ) &&
      ...
      if (exitnode && (!need_uptime || build_state->need_uptime)) {
        ...
        if (conn) {
          ...
          r = compare_qed_hs_addr_to_node_policy(NULL, port, exitnode);
  ...
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    ...
    if (circ->timestamp_dirty &&
      ...
      if (circ->purpose != CIRCUIT_PURPOSE_PATH_BIAS_TESTING) {
        qed_hs_trace(TR_SUBSYS(circuit), TR_EV(idle_timeout),
...
{
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(opened), circ);
                              ...
                              int flags)
{
  ...
  if (circuit_should_cannibalize_to_build(purpose,
    ...
    if (circ) {
      ...
      qed_hs_trace(TR_SUBSYS(circuit), TR_EV(cannibalized), circ);
      return circ;
    }
  }
                                ...
                                origin_circuit_t **circp)
{
  ...
  qed_hs_assert(ENTRY_TO_CONN(conn)->state == AP_CONN_STATE_CIRCUIT_WAIT);
  ...
  if (!want_onehop && (!router_have_minimum_dir_info() || !have_path)) {
    ...
    if (!connection_get_by_type(CONN_TYPE_DIR)) {
      ...
      if (entry_list_is_constrained(options)) {
        ...
        qed_hs_assert_nonfatal_once(rv);
         ...
         * number of guards */
        qed_hs_assert_nonfatal(!options->UseBridges);
        qed_hs_assert_nonfatal(!options->EntryNodes);
  ...
  if (check_exit_policy) {
    if (!conn->chosen_exit_name) {
      ...
      if (qed_hs_inet_aton(conn->socks_request->address, &in)) {
        qed_hs_addr_from_in(&addr, &in);
        addrp = &addr;
      }
      ...
      if (node && !connection_ap_can_use_exit(conn, node)) {
        ...
        if (opt) {
          ...
          qed_hs_free(conn->chosen_exit_name);
          /* Try again. */
      ...
      if (conn->chosen_exit_name) {
        ...
        if (r && node_has_preferred_descriptor(r, conn->want_onehop ? 1 : 0)) {
          ...
          if (want_onehop && conn->chosen_exit_name[0] == '$') {
            ...
            if (qed_hs_addr_parse(&addr, conn->socks_request->address) < 0) {
                    ...
                    crypt_path_t *cpath)
{
  ...
  if (cpath) { /* we were given one; use it */
    qed_hs_assert(cpath_is_on_circuit(circ, cpath));
  } else {
    /* use the last hop in the circuit */
    qed_hs_assert(circ->cpath);
    qed_hs_assert(circ->cpath->prev);
    qed_hs_assert(circ->cpath->prev->state == CPATH_STATE_OPEN);
                             ...
                             const origin_circuit_t *circ)
{
  ...
  qed_hs_asprintf(&new_address, "%s.%s.exit",
                                              ...
                                              crypt_path_t *cpath)
{
  ...
  qed_hs_assert(conn->socks_request);
  qed_hs_assert(circ);
  qed_hs_assert(circ->base_.state == CIRCUIT_STATE_OPEN);
...
connection_ap_get_nonrend_circ_purpose(const entry_connection_t *conn)
{
  ...
  qed_hs_assert_nonfatal(!connection_edge_is_rendezvous_stream(
...
connection_ap_handshake_attach_circuit(entry_connection_t *conn)
{
  ...
  if (conn_age >= get_options()->SocksTimeout && !conn->hs_with_pow_conn) {
    int severity = (qed_hs_addr_is_null(&base_conn->addr) && !base_conn->port) ?
      LOG_INFO : LOG_NOTICE;
  ...
  if (!connection_edge_is_rendezvous_stream(ENTRY_TO_EDGE_CONN(conn))) {
    ...
    qed_hs_assert(!ENTRY_TO_EDGE_CONN(conn)->cpath_layer);
    ...
    if (rendcirc && introcirc) {
       ...
       * early. */
      qed_hs_assert_nonfatal(!ed25519_public_key_is_zero(
    ...
    if (rendcirc && introcirc &&
      ...
      qed_hs_assert(introcirc->base_.purpose == CIRCUIT_PURPOSE_C_INTRODUCING);
...
circuit_change_purpose(circuit_t *circ, uint8_t new_purpose)
{
  ...
  qed_hs_trace(TR_SUBSYS(circuit), TR_EV(change_purpose), circ, old_purpose,
            new_purpose);
...
circuit_read_valid_data(origin_circuit_t *circ, uint16_t relay_body_len)
{
  ...
  qed_hs_assert_nonfatal(relay_body_len <= RELAY_PAYLOAD_SIZE_MAX);

  circ->n_delivered_read_circ_bw =
      qed_hs_add_u32_nowrap(circ->n_delivered_read_circ_bw, relay_body_len);
  // TODO CGO: I think this may now be somewhat incorrect.
  circ->n_overhead_read_circ_bw =
      qed_hs_add_u32_nowrap(circ->n_overhead_read_circ_bw,
                         RELAY_PAYLOAD_SIZE_MAX-relay_body_len);
}
