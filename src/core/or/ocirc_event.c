...
ocirc_state_fmt(msg_aux_data_t u)
{
  ...
  qed_hs_asprintf(&s, "<gid=%"PRIu32" state=%d onehop=%d>",
...
ocirc_chan_fmt(msg_aux_data_t u)
{
  ...
  qed_hs_asprintf(&s, "<gid=%"PRIu32" chan=%"PRIu64" onehop=%d>",
...
ocirc_cevent_fmt(msg_aux_data_t u)
{
  ...
  qed_hs_asprintf(&s, "<gid=%"PRIu32" evtype=%d reason=%d onehop=%d>",
...
int
ocirc_add_pubsub(struct pubsub_connecqed_hs_t *connector)
{
