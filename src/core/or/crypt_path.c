  ...
  do {
    ...
    if (cp != start) {
      if (cp->state == CPATH_STATE_AWAITING_KEYS) {
        qed_hs_assert(cp->prev->state == CPATH_STATE_OPEN);
...
{
//  qed_hs_assert(cp->addr); /* these are zero for rendezvous extra-hops */
//  qed_hs_assert(cp->port);
  qed_hs_assert(cp);
  qed_hs_assert(cp->magic == CRYPT_PATH_MAGIC);
  switch (cp->state)
    {
    ...
    case CPATH_STATE_CLOSED:
      ...
      qed_hs_assert(!cp->rend_dh_handshake_state);
      break;
    case CPATH_STATE_AWAITING_KEYS:
      /* qed_hs_assert(cp->dh_handshake_state); */
      break;
    default:
      log_fn(LOG_ERR, LD_BUG, "Unexpected state %d", cp->state);
      qed_hs_assert(0);
    }
  qed_hs_assert(cp->package_window >= 0);
  qed_hs_assert(cp->deliver_window >= 0);
}
