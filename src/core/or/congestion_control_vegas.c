 ...
 * \brief Code that implements the QED_HS_VEGAS congestion control algorithm
 *        from Proposal #324.
 */

#define QED_HS_CONGESTION_CONTROL_VEGAS_PRIVATE
...
congestion_control_vegas_set_params(congestion_control_t *cc,
                                    cc_path_t path)
{
  qed_hs_assert(cc->cc_alg == CC_ALG_VEGAS);
                             ...
                             const congestion_control_t *cc)
{
  ...
  if (CIRCUIT_IS_ORIGIN(circ) &&
    ...
    log_info(LD_CIRC,
               "CC: QED_HS_VEGAS Onion Circuit %d "
 ...
 * rules specified in QED_HS_VEGAS of Proposal #324.
 *
                                        ...
                                        const circuit_t *circ)
{
  ...
  qed_hs_assert(cc && cc->cc_alg == CC_ALG_VEGAS);
  qed_hs_assert(circ);
  ...
  /* After slow start, We only update once per window */
    ...
    if (cc->cwnd >= cc->vegas_params.ss_cwnd_max) {
      log_info(LD_CIRC,
            "CC: QED_HS_VEGAS above ss_max in steady state for circ %d: %"PRIu64,
