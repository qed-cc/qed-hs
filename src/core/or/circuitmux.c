...
circuitmux_alloc(void)
{
  ...
  rv = qed_hs_malloc_zero(sizeof(*rv));
  rv->chanid_circid_map = qed_hs_malloc_zero(sizeof(*( rv->chanid_circid_map)));
...
circuitmux_detach_all_circuits(circuitmux_t *cmux, smartlist_t *detached_out)
{
  ...
  while (i) {
    ...
    if (! to_remove) {
      ...
      if (chan) {
        ...
        if (circ) {
          ...
          if (to_remove->muxinfo.policy_data) {
             ...
             * free it
             */
            qed_hs_assert(cmux->policy);
            qed_hs_assert(cmux->policy->free_circ_data);
            /* Call free_circ_data() */
      ...
      qed_hs_assert(to_remove->muxinfo.policy_data == NULL);
    }

    i = HT_NEXT_RMV(chanid_circid_muxinfo_map, cmux->chanid_circid_map, i);

    /* Free it */
    qed_hs_free(to_remove);
  }
...
{
  destroy_cell_t *cell;
  QED_HS_SIMPLEQ_FOREACH(cell, &cmux->destroy_cell_queue.head, next) {
...
circuitmux_free_(circuitmux_t *cmux)
{
  ...
  qed_hs_assert(cmux->n_circuits == 0);
  qed_hs_assert(cmux->n_active_circuits == 0);
                      ...
                      const circuitmux_policy_t *pol)
{
  ...
  if (new_pol && new_pol->alloc_cmux_data) {
     ...
     */
    qed_hs_assert(new_pol->free_cmux_data);
    new_pol_data = new_pol->alloc_cmux_data(cmux);
    qed_hs_assert(new_pol_data);
  }
  ...
  while (i) {
    ...
    if ((*i)->muxinfo.policy_data) {
      ...
      qed_hs_assert(old_pol);
      qed_hs_assert(old_pol->free_circ_data);
      /* Free it */
    ...
    if (new_pol && new_pol->alloc_circ_data) {
       ...
       */
      qed_hs_assert(new_pol->free_circ_data);
      (*i)->muxinfo.policy_data =
  ...
  if (old_pol_data) {
     ...
     */
    qed_hs_assert(old_pol);
    qed_hs_assert(old_pol->free_cmux_data);
...
circuitmux_find_map_entry(circuitmux_t *cmux, circuit_t *circ)
{
  ...
  qed_hs_assert(cmux);
  qed_hs_assert(cmux->chanid_circid_map);
  qed_hs_assert(circ);
  ...
  if (hashent) {
     ...
     */
    qed_hs_assert(hashent->muxinfo.direction == CELL_DIRECTION_OUT);
  } else {
    ...
    if (circ->magic == OR_CIRCUIT_MAGIC) {
      ...
      if (TO_OR_CIRCUIT(circ)->p_chan) {
        ...
        if (hashent) {
          ...
          qed_hs_assert(hashent->muxinfo.direction == CELL_DIRECTION_IN);
        }
      }
    }
  }
                           ...
                           cell_direction_t direction))
{
  ...
  qed_hs_assert(cmux);
  qed_hs_assert(circ);
  qed_hs_assert(direction == CELL_DIRECTION_IN ||
  ...
  if (hashent) {
     ...
     */
    qed_hs_assert(hashent->muxinfo.direction == direction);
    ...
    hashent = qed_hs_malloc_zero(sizeof(*hashent));
    ...
    if (cmux->policy->alloc_circ_data) {
      ...
      qed_hs_assert(hashent->muxinfo.policy_data);
    }
...
circuitmux_detach_circuit,(circuitmux_t *cmux, circuit_t *circ))
{
  ...
  qed_hs_assert(last_searched_direction == CELL_DIRECTION_OUT
             || last_searched_direction == CELL_DIRECTION_IN);
  ...
  if (hashent) {
    ...
    qed_hs_assert(last_searched_direction == hashent->muxinfo.direction);
                                    ...
                                    destroy_cell_queue_t **destroy_queue_out)
{
  ...
  qed_hs_assert(cmux->policy->pick_active_circuit);
  qed_hs_assert(destroy_queue_out);
  ...
  *destroy_queue_out = NULL;
  ...
  if (cmux->destroy_cell_queue.n &&
    ...
    *destroy_queue_out = &cmux->destroy_cell_queue;
    ...
    qed_hs_assert(cmux->n_cells == 0);
    qed_hs_assert(cmux->destroy_cell_queue.n == 0);
  }
                             ...
                             unsigned int n_cells)
{
  ...
  qed_hs_assert(n_cells <= hashent->muxinfo.cell_count);
  hashent->muxinfo.cell_count -= n_cells;
...
circuitmux_compare_muxes, (circuitmux_t *cmux_1, circuitmux_t *cmux_2))
{
  ...
  qed_hs_assert(cmux_1);
  qed_hs_assert(cmux_2);
