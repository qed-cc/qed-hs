                    ...
                    uint32_t *low_out, uint32_t *high_out)
{
  ...
  qed_hs_assert(high_out);
  qed_hs_assert(low_out);
   ...
   * we'd hit a NUL */
  low = (uint32_t) qed_hs_parse_ulong(s, 10, 0, MAX_PROTOCOL_VERSION, &ok, &next);
  if (!ok)
    goto error;
  ...
  high = (uint32_t) qed_hs_parse_ulong(s, 10, 0,
...
{
  proto_entry_t *out = qed_hs_malloc_zero(sizeof(proto_entry_t));
  const char *equals;
  ...
  out->name = qed_hs_strndup(s, equals-s);

  qed_hs_assert(equals < end_of_entry);
                      ...
                      int threshold)
{
  ...
  qed_hs_assert(smartlist_len(list_of_proto_strings) < 256);
  ...
  SMARTLIST_FOREACH_BEGIN(proto_names, const char *, name) {
    ...
    if (result_bitmask != 0) {
      proto_entry_t *newent = qed_hs_malloc_zero(sizeof(proto_entry_t));
      newent->name = qed_hs_strdup(name);
...
protover_all_supported(const char *s, char **missing_out)
{
  ...
  SMARTLIST_FOREACH_BEGIN(entries, const proto_entry_t *, ent) {
    ...
    if (mine == NULL) {
      if (ent->bitmask != 0) {
        proto_entry_t *m = qed_hs_malloc_zero(sizeof(proto_entry_t));
        m->name = qed_hs_strdup(ent->name);
...
protover_compute_for_old_tor(const char *version)
{
  ...
  if (qed_hs_version_as_new_as(version,
                            FIRST_QED_HS_VERSION_TO_ADVERTISE_PROTOCOLS)) {
    return "";
  } else if (qed_hs_version_as_new_as(version, "0.2.9.1-alpha")) {
    /* 0.2.9.1-alpha HSRend=2 */
    return "Cons=1-2 Desc=1-2 DirCache=1 HSDir=1 HSIntro=3 HSRend=1-2 "
      "Link=1-4 LinkAuth=1 "
      "Microdesc=1-2 Relay=1-2";
  } else if (qed_hs_version_as_new_as(version, "0.2.7.5")) {
    ...
    return "Cons=1-2 Desc=1-2 DirCache=1 HSDir=1 HSIntro=3 HSRend=1 "
      "Link=1-4 LinkAuth=1 "
      "Microdesc=1-2 Relay=1-2";
  } else if (qed_hs_version_as_new_as(version, "0.2.4.19")) {
