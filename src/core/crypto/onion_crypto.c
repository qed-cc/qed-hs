...
server_onion_keys_new(void)
{
  ...
  server_onion_keys_t *keys = qed_hs_malloc_zero(sizeof(server_onion_keys_t));
 ...
 * <b>rend_authenticaqed_hs_out</b> to the "KH" field that can be used to
...
onion_skin_client_handshake(int type,
                      ...
                      uint8_t *keys_out, size_t keys_out_len,
                      uint8_t *rend_authenticaqed_hs_out,
                      circuit_params_t *params_out,
                      const char **msg_out)
{
  ...
  case ONION_HANDSHAKE_TYPE_NTOR:
    ...
    }
    {
      ...
      memcpy(keys_out, keys_tmp, keys_out_len);
      memcpy(rend_authenticaqed_hs_out, keys_tmp + keys_out_len, DIGEST_LEN);
      memwipe(keys_tmp, 0, keys_tmp_len);
      qed_hs_free(keys_tmp);
    }
    return 0;
