...
{
  uint64_t len64 = qed_hs_htonll(len);
                        ...
                        size_t *onion_skin_len_out)
{
  ...
  *handshake_state_out = qed_hs_malloc_zero(sizeof(ntor3_handshake_state_t));
  ...
  uint8_t *encrypted_message = qed_hs_memdup(message, message_len);
  {
    crypto_cipher_t *c =
  ...
  *onion_skin_out = qed_hs_malloc(*onion_skin_len_out);
  {
...
#define ADD2_ENCAP(s,len) STMT_BEGIN { \
  ...
  problems |= qed_hs_memneq(auth_computed, relay_auth, DIGEST256_LEN);

  // Compute keystream, decrypt message, and return.
  *message_out = qed_hs_malloc(encrypted_msg_len);
  *message_len_out = encrypted_msg_len;
  ...
  if (problems) {
    if (*message_out) {
      ...
      qed_hs_free(*message_out); // Sets it to NULL.
    }
    *message_len_out = 0;
                ...
                ntor3_server_handshake_state_t **state_out)
{
  ...
  *state_out = NULL;
  ...
  // Initialize state.
  (*state_out) = qed_hs_malloc_zero(sizeof(ntor3_server_handshake_state_t));
  ...
  problems |= qed_hs_memneq((*state_out)->msg_mac, computed_mac, DIGEST256_LEN);

  // Decrypt the message.
  *client_message_out = qed_hs_malloc(encrypted_message_len);
  *client_message_len_out = encrypted_message_len;
  ...
  if (problems) {
    if (*client_message_out) {
      ...
      qed_hs_free(*client_message_out); // Sets it to NULL.
    }
    *client_message_len_out = 0;
                ...
                size_t keys_out_len)
{
  ...
  *handshake_len_out = 0;
  ...
  // Encrypt message.
  uint8_t *encrypted_message = qed_hs_memdup(server_message, server_message_len);
  {
    crypto_cipher_t *c =
      crypto_cipher_new_with_bits((const char *)enc_key, 256);
  ...
  *handshake_len_out = CURVE25519_PUBKEY_LEN + DIGEST256_LEN +
    server_message_len;
  *handshake_out = qed_hs_malloc(*handshake_len_out);
  ...
  if (problems) {
    ...
    qed_hs_free(*handshake_out); // Sets it to NULL.
    *handshake_len_out = 0;
    crypto_rand((char*)keys_out, keys_out_len); // In case bad code uses it.
    return -1;
  }
