                                 ...
                                 size_t key_out_len)
{
  ...
  if (qed_hs_memneq(onion_skin, my_node_id, DIGEST_LEN))
    return -1;
  ...
  qed_hs_assert(si == s.secret_input + sizeof(s.secret_input));
  ...
  qed_hs_assert(ai == s.auth_input + sizeof(s.auth_input));
                             ...
                             const char **msg_out)
{
  ...
  bad |= (qed_hs_memneq(s.auth, auth_candidate, DIGEST256_LEN) << 2);
