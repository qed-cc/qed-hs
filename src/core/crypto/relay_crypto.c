                   ...
                   crypt_path_t **layer_hint, char *recognized)
{
  qed_hs_assert(circ);
  qed_hs_assert(cell);
  qed_hs_assert(recognized);
  qed_hs_assert(cell_direction == CELL_DIRECTION_IN ||
                  ...
                  int reverse, int is_hs_v3)
{
  ...
  qed_hs_assert(crypto);
  qed_hs_assert(key_data);
  qed_hs_assert(!(crypto->f_crypto || crypto->b_crypto ||
  ...
  qed_hs_assert(digest_len != 0);
  qed_hs_assert(cipher_key_len != 0);
...
{
  qed_hs_assert(crypto->f_crypto);
  qed_hs_assert(crypto->b_crypto);
  qed_hs_assert(crypto->f_digest);
  qed_hs_assert(crypto->b_digest);
}
