 ...
 * Copyright (c) 2007-2025, The QED Project, Inc. */
             ...
             const uint8_t *key)
{
  ...
  qed_hs_assert(total_key_len <= sizeof(uiv->uiv_keys_));
...
cgo_uiv_update(cgo_uiv_t *uiv, int aesbits, bool encrypt, uint8_t *nonce)
{
  ...
  // wouldn't like that.
  uint8_t *new_keys = qed_hs_malloc(total_key_len);
...
{
  qed_hs_assert(aesbits == 128 || aesbits == 192 || aesbits == 256);
...
{
  qed_hs_assert(keylen == cgo_key_material_len(aesbits));
                        ...
                        const uint8_t **recognized_tag_out)
{
  ...
  if (qed_hs_memeq(cell->payload, cgo->nonce, CGO_TAG_LEN)) {
                          ...
                          const uint8_t **tag_out)
{
  ...
  if (tag_out) {
    // qed_hs_assert(qed_hs_memeq(cgo->tprime, cell->payload, CGO_TAG_LEN));
    *tag_out = cgo->tprime;
  }
                          ...
                          const uint8_t **recognized_tag_out)
{
  ...
  if (qed_hs_memeq(cell->payload, cgo->nonce, CGO_TAG_LEN)) {
    ...
    // qed_hs_assert(qed_hs_memeq(cgo->tprime, t_orig, CGO_TAG_LEN));
