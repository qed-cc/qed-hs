 ...
 * control structures.  If you search for usage of qed_hs_event_new(), you
...
#define GREEDY_DESCRIPQED_HS_RETRY_INTERVAL (10)
/** How often do we check for router descriptors that we should download
 * when we have enough directory info? */
#define LAZY_DESCRIPQED_HS_RETRY_INTERVAL (60)
...
connection_add_impl(connection_t *conn, int is_connecting)
{
  ...
  qed_hs_assert(conn->conn_array_index == -1); /* can only connection_add once */
  ...
  if (SOCKET_OK(conn->s) || conn->linked) {
    conn->read_event = qed_hs_event_new(qed_hs_libevent_get_base(),
         conn->s, EV_READ|EV_PERSIST, conn_read_callback, conn);
    conn->write_event = qed_hs_event_new(qed_hs_libevent_get_base(),
...
connection_remove(connection_t *conn)
{
  ...
  qed_hs_assert(conn->conn_array_index >= 0);
...
void
qed_hs_init_connection_lists(void)
{
  if (!connection_array)
...
{
  qed_hs_assert(!smartlist_contains(closeable_connection_lst, conn));
  ...
  if (conn->linked) {
    ...
    if (event_del(conn->read_event))
      log_warn(LD_NET, "Error from libevent setting read event state for %d "
               "to unwatched: %s",
               (int)conn->s,
               qed_hs_socket_strerror(qed_hs_socket_errno(conn->s)));
  }
}
 ...
 * qed_hs_shutdown_event_loop_and_exit() somehow doesn't successfully exit the
...
static struct event *qed_hs_shutdown_event_loop_for_restart_event = NULL;
static void
qed_hs_shutdown_event_loop_for_restart_cb(
                      evutil_socket_t fd, short event, void *arg)
{
  (void)fd;
  (void)event;
  (void)arg;
  qed_hs_event_free(qed_hs_shutdown_event_loop_for_restart_event);
 ...
 */
void
qed_hs_shutdown_event_loop_and_exit(int exitcode)
{
  ...
  if (! qed_hs_libevent_is_initialized()) {
  ...
  shutdown_did_not_work_event = qed_hs_evtimer_new(
                  qed_hs_libevent_get_base(),
  ...
  qed_hs_libevent_exit_loop_after_callback(qed_hs_libevent_get_base());
}

/** Return true iff qed_hs_shutdown_event_loop_and_exit() has been called. */
int
qed_hs_event_loop_shutdown_is_pending(void)
{
...
connection_start_reading_from_linked_conn(connection_t *conn)
{
  ...
  if (!conn->active_on_link) {
    ...
    qed_hs_assert(smartlist_contains(active_linked_connection_lst, conn));
  }
}
...
conn_close_if_marked(int i)
{
  ...
  log_debug(LD_NET,"Cleaning up connection (fd "QED_HS_SOCKET_T_FORMAT").",
            conn->s);
  ...
  if ((SOCKET_OK(conn->s) || conn->linked_conn) &&
    ...
    if (!conn->hold_open_until_flushed)
      log_info(LD_NET,
               ...
               "to flush %"QED_HS_PRIuSZ" bytes. (Marked at %s:%d)",
...
directory_all_unreachable(time_t now)
{
  ...
  if (!directory_all_unreachable_cb_event) {
    ...
    qed_hs_assert(directory_all_unreachable_cb_event);
  }
...
initialize_periodic_events_cb(evutil_socket_t fd, short events, void *data)
{
  ...
  qed_hs_event_free(initialize_periodic_events_event);
...
/**
 * Periodic callback: Every {LAZY,GREEDY}_DESCRIPQED_HS_RETRY_INTERVAL,
...
launch_descripqed_hs_fetches_callback(time_t now, const or_options_t *options)
{
  ...
  if (router_have_minimum_dir_info())
    ...
    return GREEDY_DESCRIPQED_HS_RETRY_INTERVAL;
}
...
do_main_loop(void)
{
   ...
   */
  qed_hs_assert(periodic_events_initialized);
  initialize_mainloop_events();

  periodic_events_connect_all();

  struct timeval one_second = { 1, 0 };
  initialize_periodic_events_event = qed_hs_evtimer_new(
                  qed_hs_libevent_get_base(),
  ...
  if (sd_watchdog_enabled(1, &watchdog_delay) > 0) {
    if (! systemd_watchdog_timer) {
      ...
      systemd_watchdog_timer = periodic_timer_new(qed_hs_libevent_get_base(),
                                                  &watchdog,
                                                  systemd_watchdog_callback,
                                                  NULL);
      qed_hs_assert(systemd_watchdog_timer);
    }
  }
#endif /* defined(HAVE_SYSTEMD_209) */
#ifdef ENABLE_RESTART_DEBUGGING
  {
    ...
    if (first_time && getenv("QED_HS_DEBUG_RESTART")) {
      first_time = 0;
      const char *sec_str = getenv("QED_HS_DEBUG_RESTART_AFTER_SECONDS");
      long sec;
      int sec_ok=0;
      if (sec_str &&
          (sec = qed_hs_parse_long(sec_str, 10, 0, INT_MAX, &sec_ok, NULL)) &&
          sec_ok) {
      ...
      qed_hs_shutdown_event_loop_for_restart_event =
        qed_hs_evtimer_new(qed_hs_libevent_get_base(),
                        qed_hs_shutdown_event_loop_for_restart_cb, NULL);
      event_add(qed_hs_shutdown_event_loop_for_restart_event, &restart_after);
    }
  }
#endif /* defined(ENABLE_RESTART_DEBUGGING) */
...
run_main_loop_once(void)
{
  ...
  loop_result = qed_hs_libevent_run_event_loop(qed_hs_libevent_get_base(),
                                            called_loop_once);
  ...
  if (loop_result < 0) {
    ...
    if (e != EINTR && !ERRNO_IS_EINPROGRESS(e)) {
      log_err(LD_NET,"libevent call with %s failed: %s [%d]",
              qed_hs_libevent_get_method(), qed_hs_socket_strerror(e), e);
      return -1;
#ifndef _WIN32
    } else if (e == EINVAL) {
      ...
      qed_hs_assert_nonfatal_once(! ERRNO_IS_EINPROGRESS(e));
...
void
qed_hs_mainloop_free_all(void)
{
  smartlist_free(connection_array);
  smartlist_free(closeable_connection_lst);
  smartlist_free(active_linked_connection_lst);
  teardown_periodic_events();
  qed_hs_event_free(shutdown_did_not_work_event);
  qed_hs_event_free(initialize_periodic_events_event);
