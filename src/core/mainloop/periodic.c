...
{
  qed_hs_assert(next_interval < MAX_INTERVAL);
  struct timeval tv;
...
safe_timer_diff(time_t now, time_t next)
{
  if (next > now) {
     ...
     **/
    qed_hs_assert(next > TIME_MIN + LONGEST_TIMER_PERIOD);
