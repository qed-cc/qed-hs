...
static int connection_buf_read_from_socket(connection_t *conn,
                                           ssize_t *max_to_read,
                                           int *socket_error);
static void client_check_address_changed(qed_hs_socket_t sock);
static void set_constrained_socket_buffers(qed_hs_socket_t sock, int size);
...
const qed_hs_addr_t *conn_get_outbound_address(sa_family_t family,
 ...
 **/
static qed_hs_addr_t *last_interface_ipv4 = NULL;
/* DOCDOC last_interface_ipv6 */
static qed_hs_addr_t *last_interface_ipv6 = NULL;
/** A list of qed_hs_addr_t for addresses we've used in outgoing connections.
...
{
  qed_hs_assert(c->magic == LISTENER_CONNECTION_MAGIC);
...
conn_type_to_string(int type)
{
  ...
  switch (type) {
    ...
    default:
      ...
      qed_hs_snprintf(buf, sizeof(buf), "unknown [%d]", type);
      return buf;
  }
}
...
conn_state_to_string(int type, int state)
{
  ...
  qed_hs_assert_nonfatal_unreached_once();
  return buf;
}
                                  ...
                                  bool include_preposition)
{
  ...
  const qed_hs_addr_t *addr = &conn->addr;
  ...
  if (conn->socket_family == AF_UNIX) {
    ...
    if (fast_digest_is_zero(or_conn->identity_digest)) {
      ...
      qed_hs_snprintf(extra_buf, sizeof(extra_buf),
                   " ID=%s RSA_ID=%s", ed_id_buf, rsa_id_buf);
    }
    if (! scrub && (! qed_hs_addr_eq(addr, &or_conn->canonical_orport.addr) ||
                    conn->port != or_conn->canonical_orport.port)) {
      /* We report canonical address, if it's different */
      char canonical_addr_buf[QED_HS_ADDR_BUF_LEN];
      if (qed_hs_addr_to_str(canonical_addr_buf, &or_conn->canonical_orport.addr,
                          sizeof(canonical_addr_buf), 1)) {
        qed_hs_snprintf(extra_buf+strlen(extra_buf),
    ...
    if (qed_hs_addr_is_null(addr)) {
      address = conn->address;
      strlcpy(extra_buf, " (DNS lookup pending)", sizeof(extra_buf));
    }
  }

  char addr_buf[QED_HS_ADDR_BUF_LEN];
  if (address == NULL) {
    if (qed_hs_addr_family(addr) == 0) {
      address = "<unset>";
    } else {
      address = qed_hs_addr_to_str(addr_buf, addr, sizeof(addr_buf), 1);
      if (!address) {
  ...
  if (scrub && get_options()->SafeLogging_ != SAFELOG_SCRUB_NONE) {
    ...
    if (conn->port != 0) {
      qed_hs_snprintf(portbuf, sizeof(portbuf), ":%d", conn->port);
    }
  }
  ...
  qed_hs_snprintf(peer_buf, sizeof(peer_buf),
...
connection_describe(const connection_t *conn)
{
  ...
  qed_hs_snprintf(desc_buf, sizeof(desc_buf),
               "%s connection (%s) %s",
...
{
  dir_connection_t *dir_conn = qed_hs_malloc_zero(sizeof(dir_connection_t));
...
{
  or_connection_t *or_conn = qed_hs_malloc_zero(sizeof(or_connection_t));
  time_t now = time(NULL);
  qed_hs_assert(type == CONN_TYPE_OR || type == CONN_TYPE_EXT_OR);
  connection_init(now, TO_CONN(or_conn), type, socket_family);

  qed_hs_addr_make_unspec(&or_conn->canonical_orport.addr);
...
{
  entry_connection_t *entry_conn = qed_hs_malloc_zero(sizeof(entry_connection_t));
  qed_hs_assert(type == CONN_TYPE_AP);
...
{
  edge_connection_t *edge_conn = qed_hs_malloc_zero(sizeof(edge_connection_t));
  qed_hs_assert(type == CONN_TYPE_EXIT);
...
control_connection_new(int socket_family)
{
  control_connection_t *control_conn =
    qed_hs_malloc_zero(sizeof(control_connection_t));
...
listener_connection_new(int type, int socket_family)
{
  listener_connection_t *listener_conn =
    qed_hs_malloc_zero(sizeof(listener_connection_t));
...
connection_new(int type, int socket_family)
{
  switch (type) {
    ...
    default: {
      connection_t *conn = qed_hs_malloc_zero(sizeof(connection_t));
...
connection_init(time_t now, connection_t *conn, int type, int socket_family)
{
  ...
  conn->s = QED_HS_INVALID_SOCKET; /* give it a default of 'not used' */
...
{
  qed_hs_assert(! SOCKET_OK(conn_a->s));
  qed_hs_assert(! SOCKET_OK(conn_b->s));
...
connection_free_minimal(connection_t *conn)
{
  ...
  switch (conn->type) {
    case CONN_TYPE_OR:
    case CONN_TYPE_EXT_OR:
      qed_hs_assert(conn->magic == OR_CONNECTION_MAGIC);
      mem = TO_OR_CONN(conn);
      memlen = sizeof(or_connection_t);
      break;
    case CONN_TYPE_AP:
      qed_hs_assert(conn->magic == ENTRY_CONNECTION_MAGIC);
      mem = TO_ENTRY_CONN(conn);
      memlen = sizeof(entry_connection_t);
      break;
    case CONN_TYPE_EXIT:
      qed_hs_assert(conn->magic == EDGE_CONNECTION_MAGIC);
      mem = TO_EDGE_CONN(conn);
      memlen = sizeof(edge_connection_t);
      break;
    case CONN_TYPE_DIR:
      qed_hs_assert(conn->magic == DIR_CONNECTION_MAGIC);
      mem = TO_DIR_CONN(conn);
      memlen = sizeof(dir_connection_t);
      break;
    case CONN_TYPE_CONTROL:
      qed_hs_assert(conn->magic == CONTROL_CONNECTION_MAGIC);
      mem = TO_CONTROL_CONN(conn);
      memlen = sizeof(control_connection_t);
      break;
    CASE_ANY_LISTENER_TYPE:
      qed_hs_assert(conn->magic == LISTENER_CONNECTION_MAGIC);
      mem = TO_LISTENER_CONN(conn);
      memlen = sizeof(listener_connection_t);
      break;
    default:
      qed_hs_assert(conn->magic == BASE_CONNECTION_MAGIC);
      mem = conn;
  ...
  if (!connection_is_listener(conn)) {
    ...
    if (conn->socket_family == AF_UNIX) {
       ...
       * and listeners at the same time */
      qed_hs_assert(conn_listener_type_supports_af_unix(conn->type));
  ...
  qed_hs_str_wipe_and_free(conn->address);
  ...
  if (connection_speaks_cells(conn)) {
    ...
    if (or_conn->tls) {
      if (! SOCKET_OK(conn->s)) {
        ...
        qed_hs_tls_release_socket(or_conn->tls);
      } else {
        /* The qed_hs_tls_free() call below will close the socket; we must tell
         * the code below not to close it a second time. */
        qed_hs_release_socket_ownership(conn->s);
        conn->s = QED_HS_INVALID_SOCKET;
      }
      qed_hs_tls_free(or_conn->tls);
      or_conn->tls = NULL;
    }
    or_handshake_state_free(or_conn->handshake_state);
    or_conn->handshake_state = NULL;
    qed_hs_str_wipe_and_free(or_conn->nickname);
    if (or_conn->chan) {
      /* Owww, this shouldn't happen, but... */
      channel_t *base_chan = TLS_CHAN_TO_BASE(or_conn->chan);
      qed_hs_assert(base_chan);
      log_info(LD_CHANNEL,
  ...
  if (conn->type == CONN_TYPE_AP) {
    ...
    qed_hs_str_wipe_and_free(entry_conn->chosen_exit_name);
    qed_hs_str_wipe_and_free(entry_conn->original_dest_address);
  ...
  if (conn->type == CONN_TYPE_CONTROL) {
    ...
    qed_hs_free(control_conn->safecookie_client_hash);
    qed_hs_free(control_conn->incoming_cmd);
    qed_hs_free(control_conn->current_cmd);
  ...
  qed_hs_event_free(conn->read_event);
  qed_hs_event_free(conn->write_event);
  conn->read_event = conn->write_event = NULL;

  if (conn->type == CONN_TYPE_DIR) {
    dir_connection_t *dir_conn = TO_DIR_CONN(conn);
    qed_hs_free(dir_conn->requested_resource);

    qed_hs_compress_free(dir_conn->compress_state);
  ...
  if (SOCKET_OK(conn->s)) {
    ...
    qed_hs_close_socket(conn->s);
    conn->s = QED_HS_INVALID_SOCKET;
  }

  if (conn->type == CONN_TYPE_OR &&
      !qed_hs_digest_is_zero(TO_OR_CONN(conn)->identity_digest)) {
    log_warn(LD_BUG, "called on OR conn with non-zeroed identity_digest");
    connection_or_clear_identity(TO_OR_CONN(conn));
  }
  if (conn->type == CONN_TYPE_OR || conn->type == CONN_TYPE_EXT_OR) {
    qed_hs_free(TO_OR_CONN(conn)->ext_or_auth_correct_client_hash);
    qed_hs_free(TO_OR_CONN(conn)->ext_or_transport);
  }
...
connection_free_,(connection_t *conn))
{
  if (!conn)
    return;
  qed_hs_assert(!connection_is_on_closeable_list(conn));
  qed_hs_assert(!connection_in_array(conn));
  ...
  if (connection_speaks_cells(conn)) {
    if (!qed_hs_digest_is_zero(TO_OR_CONN(conn)->identity_digest)) {
      connection_or_clear_identity(TO_OR_CONN(conn));
    }
  }
  if (conn->type == CONN_TYPE_CONTROL) {
...
{
  qed_hs_assert(conn->marked_for_close);
...
connection_close_immediate(connection_t *conn)
{
  ...
  if (connection_get_outbuf_len(conn)) {
    log_info(LD_NET,"fd %d, type %s, state %s, %"QED_HS_PRIuSZ" bytes on outbuf.",
...
connection_mark_for_close_(connection_t *conn, int line, const char *file)
{
  assert_connection_ok(conn,0);
  qed_hs_assert(line);
  qed_hs_assert(line < 1<<16); /* marked_for_close can only fit a uint16_t. */
  qed_hs_assert(file);
                     ...
                     socklen_t *len_out)
{
  ...
  sockaddr = qed_hs_malloc_zero(sizeof(struct sockaddr_un));
  ...
  if (strlcpy(sockaddr->sun_path, listenaddress, sizeof(sockaddr->sun_path))
    ...
    qed_hs_free(sockaddr);
    return NULL;
  }

  if (readable_address)
    *readable_address = qed_hs_strdup(listenaddress);
                               ...
                               int purpose, const port_cfg_t *port)
{
  ...
  qed_hs_assert(is_valid_unix_socket_purpose(purpose));

  p = qed_hs_strdup(path);
 ...
 * -1 on failure. */
static int
make_socket_reuseable(qed_hs_socket_t sock)
{
#ifdef _WIN32
  (void) sock;
  return 0;
...
static int
make_win32_socket_exclusive(qed_hs_socket_t sock)
{
...
static int
qed_hs_listen(qed_hs_socket_t fd)
{
  int r;
                        ...
                        int *addr_in_use)
{
  ...
  qed_hs_socket_t s = QED_HS_INVALID_SOCKET;  /* the socket we're going to make */
  ...
  if (listensockaddr->sa_family == AF_INET ||
    ...
    qed_hs_addr_from_sockaddr(&addr, listensockaddr, &usePort);
    log_notice(LD_NET, "Opening %s on %s",
               conn_type_to_string(type), fmt_addrport(&addr, usePort));

    s = qed_hs_open_socket_nonblocking(qed_hs_addr_family(&addr),
      is_stream ? SOCK_STREAM : SOCK_DGRAM,
      is_stream ? IPPROTO_TCP: IPPROTO_UDP);
    if (!SOCKET_OK(s)) {
      int e = qed_hs_socket_errno(s);
    ...
    if (options->TransProxyType_parsed == TPT_TPROXY &&
      ...
      if (setsockopt(s, SOL_IP, IP_TRANSPARENT, (void*)&one,
        ...
        log_warn(LD_NET, "Error setting IP_TRANSPARENT flag: %s.%s",
                 qed_hs_socket_strerror(e), extra);
      }
    }
    ...
    if (bind(s,listensockaddr,socklen) < 0) {
      ...
      log_warn(LD_NET, "Could not bind to %s:%u: %s%s", address, usePort,
               qed_hs_socket_strerror(e), helpfulhint);
      goto err;
    }

    if (is_stream) {
      if (qed_hs_listen(s) < 0) {
        log_warn(LD_NET, "Could not listen on %s:%u: %s", address, usePort,
                 qed_hs_socket_strerror(qed_hs_socket_errno(s)));
        goto err;
      }
    }
    ...
    if (usePort != 0) {
      ...
      qed_hs_addr_from_sockaddr(&addr2, (struct sockaddr*)&ss, &gotPort);
    }
#ifdef HAVE_SYS_UN_H
  /*
   ...
   */
    ...
    qed_hs_assert(conn_listener_type_supports_af_unix(type));
    ...
    s = qed_hs_open_socket_nonblocking(AF_UNIX, SOCK_STREAM, 0);
    if (! SOCKET_OK(s)) {
      int e = qed_hs_socket_errno(s);
      if (ERRNO_IS_RESOURCE_LIMIT(e)) {
        socket_failed_from_fd_exhaustion();
...
#ifdef HAVE_PWD_H
    if (options->User) {
      pw = qed_hs_getpwnam(options->User);
      struct stat st;
      if (pw == NULL) {
  ...
  conn->s = s;
  s = QED_HS_INVALID_SOCKET; /* Prevent double-close */
  conn->address = qed_hs_strdup(address);
  conn->port = gotPort;
  qed_hs_addr_copy(&conn->addr, &addr);
  ...
  if (start_reading) {
    ...
    qed_hs_assert(type == CONN_TYPE_AP_DNS_LISTENER);
                                 ...
                                 int *defer, int *addr_in_use)
{
  ...
  qed_hs_assert(real_port <= UINT16_MAX);
  ...
  if (port->is_unix_addr) {
    ...
    listensockaddr = qed_hs_malloc(sizeof(struct sockaddr_storage));
    listensocklen = qed_hs_addr_to_sockaddr(&port->addr,
                                         real_port,
                                         listensockaddr,
                                         sizeof(struct sockaddr_storage));
    address = qed_hs_addr_to_str_dup(&port->addr);
  }
...
{
  qed_hs_socket_t news; /* the new socket */
  ...
  qed_hs_assert((size_t)remotelen >= sizeof(struct sockaddr_in));
  memset(&addrbuf, 0, sizeof(addrbuf));

  news = qed_hs_accept_socket_nonblocking(conn->s,remote,&remotelen);
  if (!SOCKET_OK(news)) { /* accept() error */
    int e = qed_hs_socket_errno(conn->s);
  ...
  if (make_socket_reuseable(news) < 0) {
    if (qed_hs_socket_errno(news) == EINVAL) {
  ...
  if (conn->socket_family == AF_INET || conn->socket_family == AF_INET6 ||
    ...
    qed_hs_addr_from_sockaddr(&addr, remote, &port);
    ...
    qed_hs_addr_copy(&newconn->addr, &addr);
    if (new_type == CONN_TYPE_AP && conn->socket_family == AF_UNIX) {
      newconn->port = 0;
      newconn->address = qed_hs_strdup(conn->address);
    } else {
      newconn->port = port;
      newconn->address = qed_hs_addr_to_str_dup(&addr);
    }
    ...
    qed_hs_assert(conn->type == CONN_TYPE_CONTROL_LISTENER);
    qed_hs_assert(new_type == CONN_TYPE_CONTROL);
    ...
    qed_hs_addr_make_unspec(&newconn->addr);
    newconn->port = 1;
    newconn->address = qed_hs_strdup(conn->address);
  } else {
    qed_hs_assert(0);
                            ...
                            int *socket_error))
{
  ...
  s = qed_hs_open_socket_nonblocking(protocol_family, SOCK_STREAM, proto);
  if (! SOCKET_OK(s)) {
    /*
     * Early OOS handler calls; it matters if it's an exhaustion-related
     * error or not.
     */
    *socket_error = qed_hs_socket_errno(s);
    if (ERRNO_IS_RESOURCE_LIMIT(*socket_error)) {
      socket_failed_from_fd_exhaustion();
      connection_check_oos(get_n_open_sockets(), 1);
    } else {
      log_warn(LD_NET,"Error creating network socket: %s",
               qed_hs_socket_strerror(*socket_error));
      connection_check_oos(get_n_open_sockets(), 0);
    }
    return -1;
  }
  ...
  if (bindaddr && try_ip_bind_address_no_port &&
    ...
    if (errno == EINVAL) {
      ...
      log_warn(LD_NET, "Error setting IP_BIND_ADDRESS_NO_PORT on new "
                       "connection: %s", qed_hs_socket_strerror(errno));
    }
  }
#endif
  ...
  log_fn(inprogress ? LOG_DEBUG : LOG_INFO, LD_NET,
         "Connection to socket %s (sock "QED_HS_SOCKET_T_FORMAT").",
...
connection_connect_log_client_use_ip_version(const connection_t *conn)
{
  ...
  qed_hs_addr_t real_addr;
  qed_hs_addr_copy(&real_addr, &conn->addr);

  /* Check if we broke a mandatory address family restriction */
  if ((must_ipv4 && qed_hs_addr_family(&real_addr) == AF_INET6)
      || (must_ipv6 && qed_hs_addr_family(&real_addr) == AF_INET)) {
  ...
  if (reachable_addr_use_ipv6(options)) {
    log_info(LD_NET, "Our outgoing connection is using IPv%d.",
             qed_hs_addr_family(&real_addr) == AF_INET6 ? 6 : 4);
  }

  /* Check if we couldn't satisfy an address family preference */
  if ((!pref_ipv6 && qed_hs_addr_family(&real_addr) == AF_INET6)
      || (pref_ipv6 && qed_hs_addr_family(&real_addr) == AF_INET)) {
...
conn_get_outbound_address(sa_family_t family,
             const or_options_t *options, unsigned int conn_type)
{
  const qed_hs_addr_t *ext_addr = NULL;
...
connection_connect(connection_t *conn, const char *address,
                   const qed_hs_addr_t *addr, uint16_t port, int *socket_error)
{
  struct sockaddr_storage addrbuf;
  ...
  if (!qed_hs_addr_is_loopback(addr)) {
    const qed_hs_addr_t *ext_addr = NULL;
    ext_addr = conn_get_outbound_address(qed_hs_addr_family(addr), get_options(),
                                         conn->type);
    if (ext_addr) {
      memset(&bind_addr_ss, 0, sizeof(bind_addr_ss));
      bind_addr_len = qed_hs_addr_to_sockaddr(ext_addr, 0,
  ...
  dest_addr_len = qed_hs_addr_to_sockaddr(addr, port, dest_addr, sizeof(addrbuf));
  qed_hs_assert(dest_addr_len > 0);
...
conn_get_proxy_type(const connection_t *conn)
{
  ...
  if (options->HTTPSProxy)
    ...
    qed_hs_assert(options->TCPProxyProtocol == TCP_PROXY_PROTOCOL_HAPROXY);
...
connection_https_proxy_connect(connection_t *conn)
{
  ...
  if (base64_authenticator) {
    ...
    qed_hs_snprintf(buf, sizeof(buf), "CONNECT %s HTTP/1.1\r\n"
        "Host: %s\r\n"
        "Proxy-Authorization: Basic %s\r\n\r\n",
        addrport,
        addrport,
        base64_authenticator);
    qed_hs_free(base64_authenticator);
  } else {
    qed_hs_snprintf(buf, sizeof(buf), "CONNECT %s HTTP/1.0\r\n\r\n",
...
connection_socks4_proxy_connect(connection_t *conn)
{
  ...
  if (qed_hs_addr_family(&conn->addr) != AF_INET) {
  ...
  buf = qed_hs_malloc_zero(buf_size);

  ip4addr = qed_hs_addr_to_ipv4n(&conn->addr);
...
{
  int ret = 0;
  qed_hs_addr_port_t *addr_port = qed_hs_addr_port_new(&conn->addr, conn->port);
...
connection_read_https_proxy_response(connection_t *conn)
{
  ...
  qed_hs_free(headers);
  if (!reason) reason = qed_hs_strdup("[no reason given]");
...
connection_send_socks5_connect(connection_t *conn)
{
  ...
  if (qed_hs_addr_family(&conn->addr) == AF_INET) {
    uint32_t addr = qed_hs_addr_to_ipv4n(&conn->addr);
    ...
    buf[3] = 4;
    reqsize += 16;
    memcpy(buf + 4, qed_hs_addr_to_in6_addr8(&conn->addr), 16);
    ...
    case PROXY_SOCKS5_WANT_AUTH_METHOD_RFC1929:
      ...
      if (ret == 1) {
        ...
        if (conn_get_proxy_type(conn) == PROXY_PLUGGABLE) {
          ...
          qed_hs_assert(strlen(socks_args_string) > 0);
          qed_hs_assert(strlen(socks_args_string) <= MAX_SOCKS5_AUTH_SIZE_TOTAL);
        ...
        qed_hs_assert(usize <= MAX_SOCKS5_AUTH_FIELD_SIZE &&
  ...
  SMARTLIST_FOREACH_BEGIN(old_conns, connection_t *, conn) {
    ...
    SMARTLIST_FOREACH_BEGIN(launch, const port_cfg_t *, wanted) {
      ...
      if (wanted->is_unix_addr) {
        ...
        if (port_matches && qed_hs_addr_eq(&wanted->addr, &conn->addr)) {
          found_port = wanted;
          break;
        }
        ...
        const int may_need_rebind =
          qed_hs_addr_family(&wanted->addr) == qed_hs_addr_family(&conn->addr) &&
          port_matches_exact && bool_neq(qed_hs_addr_is_null(&wanted->addr),
                                         qed_hs_addr_is_null(&conn->addr));
        if (replacements && may_need_rebind) {
          listener_replacement_t *replacement =
            qed_hs_malloc(sizeof(listener_replacement_t));
...
retry_all_listeners(smartlist_t *new_conns, int close_all_noncontrol)
{
  ...
  SMARTLIST_FOREACH_BEGIN(replacements, listener_replacement_t *, r) {
    ...
    qed_hs_assert(r->new_port);
    qed_hs_assert(r->old_conn);
  ...
  SMARTLIST_FOREACH(replacements, listener_replacement_t *, r, qed_hs_free(r));
...
connection_is_rate_limited(const connection_t *conn)
{
  ...
  if (conn->linked)
    return 0; /* Internal connection */
           ...
           (qed_hs_addr_family(&conn->addr) == AF_UNSPEC || /* no address */
            qed_hs_addr_family(&conn->addr) == AF_UNIX ||   /* no address */
            qed_hs_addr_is_internal(&conn->addr, 0)))
                             ...
                             size_t num_read, size_t num_written)
{
  if (num_written >= INT_MAX || num_read >= INT_MAX) {
    log_err(LD_BUG, "Value out of range. num_read=%lu, num_written=%lu, "
             ...
             conn_type_to_string(conn->type),
             conn_state_to_string(conn->type, conn->state));
    qed_hs_assert_nonfatal_unreached();
  ...
  if (connection_speaks_cells(conn) &&
    ...
    switch (result) {
      ...
      case QED_HS_TLS_WANTREAD:
        ...
        break;
      case QED_HS_TLS_DONE: /* no data read, so nothing to process */
        break; /* so we call bucket_decrement below */
      default:
        break;
    }
    pending = qed_hs_tls_get_pending_bytes(or_conn->tls);
    if (pending) {
    ...
    qed_hs_tls_get_n_raw_bytes(or_conn->tls, &n_read, &n_written);
 ...
 */
static void
update_send_buffer_size(qed_hs_socket_t sock)
{
#ifdef _WIN32
...
connection_handle_write_impl(connection_t *conn, int force)
{
  ...
  qed_hs_assert(!connection_is_listener(conn));
  ...
  if (connection_speaks_cells(conn) &&
    ...
    switch (result) {
      CASE_QED_HS_TLS_ERROR_ANY:
      case QED_HS_TLS_CLOSE:
        or_conn->tls_error = result;
        log_info(LD_NET, result != QED_HS_TLS_CLOSE ?
                 "tls error. breaking.":"TLS connection closed on flush");
        /* Don't flush; connection is dead. */
        connection_or_notify_error(or_conn,
                                   END_OR_CONN_REASON_MISC,
                                   result != QED_HS_TLS_CLOSE ?
      ...
      case QED_HS_TLS_WANTREAD:
        ...
        return 0;
      /* case QED_HS_TLS_DONE:
       * for QED_HS_TLS_DONE, fall through to check if the flushlen
...
alloc_http_authenticator(const char *authenticator)
{
  ...
  const size_t authenticaqed_hs_length = strlen(authenticator);
  const size_t base64_authenticaqed_hs_length =
      base64_encode_size(authenticaqed_hs_length, 0) + 1;
  char *base64_authenticator = qed_hs_malloc(base64_authenticaqed_hs_length);
  if (base64_encode(base64_authenticator, base64_authenticaqed_hs_length,
                    authenticator, authenticaqed_hs_length, 0) < 0) {
    qed_hs_free(base64_authenticator); /* free and set to null */
  }
 ...
 */
static void
client_check_address_changed(qed_hs_socket_t sock)
{
  qed_hs_addr_t out_addr, iface_addr;
  qed_hs_addr_t **last_interface_ip_ptr;
  sa_family_t family;

  if (!outgoing_addrs)
    outgoing_addrs = smartlist_new();

  if (qed_hs_addr_from_getsockname(&out_addr, sock) < 0) {
    int e = qed_hs_socket_errno(sock);
    log_warn(LD_NET, "getsockname() to check for address change failed: %s",
             qed_hs_socket_strerror(e));
    return;
  }
  family = qed_hs_addr_family(&out_addr);
  ...
  if (! *last_interface_ip_ptr) {
    qed_hs_addr_t *a = qed_hs_malloc_zero(sizeof(qed_hs_addr_t));
  ...
  SMARTLIST_FOREACH(outgoing_addrs, const qed_hs_addr_t *, a_ptr,
                    if (qed_hs_addr_eq(a_ptr, &out_addr))
                      return;
                    );
  ...
  if (qed_hs_addr_eq(&iface_addr, *last_interface_ip_ptr)) {
    /* Nope, it hasn't changed.  Add this address to the list. */
    smartlist_add(outgoing_addrs, qed_hs_memdup(&out_addr, sizeof(qed_hs_addr_t)));
  } else {
    ...
    qed_hs_addr_copy(*last_interface_ip_ptr, &iface_addr);
    SMARTLIST_FOREACH(outgoing_addrs, qed_hs_addr_t*, a_ptr, qed_hs_free(a_ptr));
 ...
 */
static void
set_constrained_socket_buffers(qed_hs_socket_t sock, int size)
{
  void *sz = (void*)&size;
  ...
  if (setsockopt(sock, SOL_SOCKET, SO_SNDBUF, sz, sz_sz) < 0) {
    ...
    log_warn(LD_NET, "setsockopt() to constrain send "
             "buffer to %d bytes failed: %s", size, qed_hs_socket_strerror(e));
  }
...
{
  int r = 0;
  qed_hs_assert(!conn->in_flushed_some);
...
static int
oos_victim_comparaqed_hs_for_orconns(or_connection_t *a, or_connection_t *b)
{
  int a_circs, b_circs;
...
oos_victim_comparator(const void **a_v, const void **b_v)
{
  ...
  qed_hs_assert(a != NULL);
  qed_hs_assert(b != NULL);
  ...
  if (a->type == CONN_TYPE_OR && b->type == CONN_TYPE_OR) {
    return oos_victim_comparaqed_hs_for_orconns(TO_OR_CONN(a), TO_OR_CONN(b));
  } else {
    /*
...
pick_oos_victims, (int n))
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, c) {
    /* Bump the counter */
    qed_hs_assert(c->type <= CONN_TYPE_MAX_);
...
assert_connection_ok(connection_t *conn, time_t now)
{
  ...
  qed_hs_assert(conn);
  qed_hs_assert(conn->type >= CONN_TYPE_MIN_);
  qed_hs_assert(conn->type <= CONN_TYPE_MAX_);
  ...
  if (conn->linked_conn) {
    qed_hs_assert(conn->linked_conn->linked_conn == conn);
    qed_hs_assert(conn->linked);
  }
  if (conn->linked)
    qed_hs_assert(!SOCKET_OK(conn->s));
  ...
  if (conn->type == CONN_TYPE_OR) {
    or_connection_t *or_conn = TO_OR_CONN(conn);
    if (conn->state == OR_CONN_STATE_OPEN) {
      /* qed_hs_assert(conn->bandwidth > 0); */
       ...
       */
//      qed_hs_assert(conn->read_bucket >= 0);
    }
//    qed_hs_assert(conn->addr && conn->port);
    qed_hs_assert(conn->address);
    if (conn->state > OR_CONN_STATE_PROXY_HANDSHAKING)
      qed_hs_assert(or_conn->tls);
  }
  ...
  if (CONN_IS_EDGE(conn)) {
    ...
    if (conn->type == CONN_TYPE_AP) {
      ...
      if (entry_conn->chosen_exit_optional || entry_conn->chosen_exit_retries)
        qed_hs_assert(entry_conn->chosen_exit_name);

      qed_hs_assert(entry_conn->socks_request);
      if (conn->state == AP_CONN_STATE_OPEN) {
        qed_hs_assert(entry_conn->socks_request->has_finished);
        if (!conn->marked_for_close) {
          qed_hs_assert(ENTRY_TO_EDGE_CONN(entry_conn)->cpath_layer);
          cpath_assert_layer_ok(ENTRY_TO_EDGE_CONN(entry_conn)->cpath_layer);
        }
      }
    }
    if (conn->type == CONN_TYPE_EXIT) {
      qed_hs_assert(conn->purpose == EXIT_PURPOSE_CONNECT ||
                 conn->purpose == EXIT_PURPOSE_RESOLVE);
    }
  } else if (conn->type == CONN_TYPE_DIR) {
  } else {
    /* Purpose is only used for dir and exit types currently */
    qed_hs_assert(!conn->purpose);
  }

  switch (conn->type)
    {
    CASE_ANY_LISTENER_TYPE:
      qed_hs_assert(conn->state == LISTENER_STATE_READY);
      break;
    case CONN_TYPE_OR:
      qed_hs_assert(conn->state >= OR_CONN_STATE_MIN_);
      qed_hs_assert(conn->state <= OR_CONN_STATE_MAX_);
      break;
    case CONN_TYPE_EXT_OR:
      qed_hs_assert(conn->state >= EXT_OR_CONN_STATE_MIN_);
      qed_hs_assert(conn->state <= EXT_OR_CONN_STATE_MAX_);
      break;
    case CONN_TYPE_EXIT:
      qed_hs_assert(conn->state >= EXIT_CONN_STATE_MIN_);
      qed_hs_assert(conn->state <= EXIT_CONN_STATE_MAX_);
      qed_hs_assert(conn->purpose >= EXIT_PURPOSE_MIN_);
      qed_hs_assert(conn->purpose <= EXIT_PURPOSE_MAX_);
      break;
    case CONN_TYPE_AP:
      qed_hs_assert(conn->state >= AP_CONN_STATE_MIN_);
      qed_hs_assert(conn->state <= AP_CONN_STATE_MAX_);
      qed_hs_assert(TO_ENTRY_CONN(conn)->socks_request);
      break;
    case CONN_TYPE_DIR:
      qed_hs_assert(conn->state >= DIR_CONN_STATE_MIN_);
      qed_hs_assert(conn->state <= DIR_CONN_STATE_MAX_);
      qed_hs_assert(conn->purpose >= DIR_PURPOSE_MIN_);
      qed_hs_assert(conn->purpose <= DIR_PURPOSE_MAX_);
      break;
    case CONN_TYPE_CONTROL:
      qed_hs_assert(conn->state >= CONTROL_CONN_STATE_MIN_);
      qed_hs_assert(conn->state <= CONTROL_CONN_STATE_MAX_);
      break;
    case CONN_TYPE_METRICS:
      /* No state. */
      break;
 ...
 */
int
get_proxy_addrport(qed_hs_addr_t *addr, uint16_t *port, int *proxy_type,
  ...
  *is_pt_out = 0;
  ...
  if (options->ClientTransportPlugin) {
    ...
    if (transport) { /* transport found */
      qed_hs_addr_copy(addr, &transport->addr);
      *port = transport->port;
  ...
  if (options->HTTPSProxy) {
    qed_hs_addr_copy(addr, &options->HTTPSProxyAddr);
    *port = options->HTTPSProxyPort;
    *proxy_type = PROXY_CONNECT;
    return 0;
  } else if (options->Socks4Proxy) {
    qed_hs_addr_copy(addr, &options->Socks4ProxyAddr);
    *port = options->Socks4ProxyPort;
    *proxy_type = PROXY_SOCKS4;
    return 0;
  } else if (options->Socks5Proxy) {
    qed_hs_addr_copy(addr, &options->Socks5ProxyAddr);
    *port = options->Socks5ProxyPort;
    *proxy_type = PROXY_SOCKS5;
    return 0;
  } else if (options->TCPProxy) {
    qed_hs_addr_copy(addr, &options->TCPProxyAddr);
...
connection_free_all(void)
{
  ...
  if (outgoing_addrs) {
    SMARTLIST_FOREACH(outgoing_addrs, qed_hs_addr_t *, addr, qed_hs_free(addr));
    smartlist_free(outgoing_addrs);
    outgoing_addrs = NULL;
  }

  qed_hs_free(last_interface_ipv4);
  qed_hs_free(last_interface_ipv6);
                     ...
                     const char *source))
{
  ...
  if (conn)
    qed_hs_asprintf(&ext_source, "%s:%s:%d", source,
                 fmt_and_decorate_addr(&conn->addr), conn->port);
  else
    ext_source = qed_hs_strdup(source);
  ...
  if (trusted) {
    ...
    qed_hs_asprintf(&warn, "Clock skew %ld in %s from %s", apparent_skew,
                 received, source);
    control_event_bootstrap_problem(warn, "CLOCK_SKEW", conn, 1);
  }
  qed_hs_free(warn);
  qed_hs_free(ext_source);
}
