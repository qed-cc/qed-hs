...
int
qed_hs_mainloop_connect_pubsub(struct pubsub_builder_t *builder)
{
  int rv = -1;
  qed_hs_mainloop_disconnect_pubsub();
 ...
 *
 * Invoke this after qed_hs_mainloop_connect_pubsub, and after libevent has been
 * initialized.
 */
void
qed_hs_mainloop_connect_pubsub_events(void)
{
  qed_hs_assert(the_dispatcher);
  qed_hs_assert(! alert_events);
 ...
 **/
int
qed_hs_mainloop_set_delivery_strategy(const char *msg_channel_name,
 ...
 **/
void
qed_hs_mainloop_disconnect_pubsub(void)
{
  if (the_pubsub_items) {
