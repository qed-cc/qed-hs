...
worker_state_new(void *arg)
{
  worker_state_t *ws;
  (void)arg;
  ws = qed_hs_malloc_zero(sizeof(worker_state_t));
...
cpuworker_onion_handshake_replyfn(void *work_)
{
  ...
  qed_hs_assert(total_pending_tasks > 0);
  --total_pending_tasks;

  /* Could avoid this, but doesn't matter. */
  memcpy(&rpl, &job->u.reply, sizeof(rpl));

  qed_hs_assert(rpl.magic == CPUWORKER_REPLY_MAGIC);
...
cpuworker_onion_handshake_threadfn(void *state_, void *work_)
{
  ...
  qed_hs_assert(req.magic == CPUWORKER_REQUEST_MAGIC);
                              ...
                              create_cell_t *onionskin)
{
  ...
  job = qed_hs_malloc_zero(sizeof(cpuworker_job_t));
  job->circ = circ;
