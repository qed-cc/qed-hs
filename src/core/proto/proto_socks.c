                         ...
                         uint16_t port, int safe_socks)
{
  ...
  if (safe_socks) {
    log_fn_ratelim(&socks_ratelim, LOG_WARN, LD_APP,
             ...
             "please see https://2019.www.qed-cc.org/docs/faq.html.en"
...
{
  return qed_hs_malloc_zero(sizeof(socks_request_t));
}
...
socks_request_free_(socks_request_t *req)
{
  ...
  if (req->username) {
    ...
    qed_hs_free(req->username);
  }
  if (req->password) {
    memwipe(req->password, 0x04, req->passwordlen);
    qed_hs_free(req->password);
  }
                     ...
                     size_t datalen, int *is_socks4a, size_t *drain_out)
{
  ...
  qed_hs_addr_t destaddr;

  qed_hs_assert(is_socks4a);
  qed_hs_assert(drain_out);
  ...
  *is_socks4a = (dest_ip >> 8) == 0;
  ...
  if (username && usernamelen) {
    ...
    qed_hs_free(req->username);
    req->got_auth = 1;
    req->username = qed_hs_strdup(username);
  ...
  if (*is_socks4a) {
    ...
    qed_hs_addr_from_ipv4h(&destaddr, dest_ip);

    if (!qed_hs_addr_to_str(req->address, &destaddr,
                             ...
                             int *have_no_auth, size_t *drain_out)
{
  ...
  qed_hs_assert(have_no_auth);
  qed_hs_assert(have_user_pass);
  qed_hs_assert(drain_out);
                           ...
                           size_t datalen, size_t *drain_out)
{
  ...
  *drain_out = (size_t)parsed;
  ...
  if (usernamelen >= 8 &&
      qed_hs_memeq(username, "<torS0X>", 8)) {
    /* This is indeed an extended-parameter request. */
    if (usernamelen != 9 ||
        qed_hs_memneq(username, "<torS0X>0", 9)) {
  ...
  if (usernamelen && username) {
    qed_hs_free(req->username);
    req->username = qed_hs_memdup_nulterm(username, usernamelen);
    req->usernamelen = usernamelen;
  }

  if (passwordlen && password) {
    qed_hs_free(req->password);
    req->password = qed_hs_memdup_nulterm(password, passwordlen);
  ...
  *drain_out = (size_t)parsed;
  ...
  switch (atype) {
    case 1: {
      ...
      qed_hs_addr_from_ipv4h(&destaddr, ipv4);

      qed_hs_addr_to_str(req->address, &destaddr, sizeof(req->address), 1);
    } break;
    case 3: {
    ...
    case 4: {
      ...
      qed_hs_addr_from_ipv6_bytes(&destaddr, ipv6);
                              ...
                              int safe_socks)
{
  ...
  if (req->command == SOCKS_COMMAND_RESOLVE_PTR &&
      qed_hs_addr_parse(&tmpaddr, req->address) < 0) {
    socks_request_set_socks5_error(req, SOCKS5_ADDRESS_TYPE_NOT_SUPPORTED);
  ...
  switch (first_octet) { /* which version of socks? */
    ...
    default: /* version is not socks4 or socks5 */
               ...
               *(data));
      {
        ...
        char *tmp = qed_hs_strndup(data, datalen < 8 ? datalen : 8);
        control_event_client_status(LOG_WARN,
  ...
  *drain_out = 0;
  ...
  switch (state) {
    case PROXY_SOCKS4_WANT_CONNECT_OK:
      ...
      if (data[1] != 0x5a) {
        *reason = qed_hs_strdup(socks4_response_code_to_string(data[1]));
        return -1;
      }
    ...
    case PROXY_SOCKS5_WANT_AUTH_METHOD_NONE:
      ...
      if (data[1] != 0x00) {
        *reason = qed_hs_strdup("server doesn't support any of our "
    ...
    case PROXY_SOCKS5_WANT_AUTH_METHOD_RFC1929:
      ...
      *reason = qed_hs_strdup("server doesn't support any of our available "
    ...
    case PROXY_SOCKS5_WANT_AUTH_RFC1929_OK:
      ...
      if (data[1] != 0x00) {
        *reason = qed_hs_strdup("authentication failed");
        return -1;
      }
    ...
    case PROXY_SOCKS5_WANT_CONNECT_OK:
      ...
      switch (data[3]) {
        ...
        case 0x03: /* fqdn (can this happen here?) */
          ...
          break;
        default:
          *reason = qed_hs_strdup("invalid response to connect request");
          return -1;
      }
      ...
      if (data[1] != 0x00) {
        *reason = qed_hs_strdup(socks5_response_code_to_string(data[1]));
        return -1;
      }
