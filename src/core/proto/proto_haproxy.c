 ...
 */
char *
haproxy_format_proxy_header_line(const qed_hs_addr_port_t *addr_port)
{
  qed_hs_assert(addr_port);

  sa_family_t family = qed_hs_addr_family(&addr_port->addr);
  ...
  qed_hs_addr_to_str(addrbuf, &addr_port->addr, sizeof(addrbuf), 0);

  qed_hs_asprintf(&buf, "PROXY %s %s %s 0 %d\r\n", family_string, src_addr_string,
