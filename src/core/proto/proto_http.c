                    ...
                    int force_complete)
{
  ...
  if (headers_out) {
    *headers_out = qed_hs_malloc(headerlen+1);
    buf_get_bytes(buf, *headers_out, headerlen);
    (*headers_out)[headerlen] = 0; /* NUL terminate it */
  }
  if (body_out) {
    qed_hs_assert(body_used);
    *body_used = bodylen;
    *body_out = qed_hs_malloc(bodylen+1);
...
#define CONTENT_LENGTH "\r\nContent-Length: "
  p = (char*) qed_hs_memstr(headers, headerlen, CONTENT_LENGTH);
  if (p == NULL)
    return 0;

  qed_hs_assert(p >= headers && p < headers+headerlen);
  ...
  len_str = qed_hs_memdup_nulterm(p, newline-p);
  /* We limit the size to INT_MAX because other parts of the buffer.c
   * code don't like buffers to be any bigger than that. */
  result = (size_t) qed_hs_parse_uint64(len_str, 10, 0, INT_MAX, &ok, &eos);
  if (eos && !qed_hs_strisspace(eos)) {
    ok = 0;
  } else {
    *result_out = result;
  }
