...
Tor has a single entry point: qed_hs_run_main() in main.c.  All the ways of
starting a Tor process (ntmain.c, qed_hs_main.c, and qed_hs_api.c) work by invoking qed_hs_run_main().

The qed_hs_run_main() function normally exits (@ref init_exceptwhen "1") by
returning: not by calling abort() or exit().  Before it returns, it calls
qed_hs_cleanup() in shutdown.c.

Conceptually, there are several stages in running Tor.

1. First, we initialize those modules that do not depend on the
   configuration.  This happens in the first half of qed_hs_run_main(), and the
   first half of qed_hs_init().  (@ref init_pending_refactor "2")
...
2. Second, we parse the command line and our configuration, and configure
   ...
   happens midway through qed_hs_init(), which invokes
   options_init_from_torrc().  We then initialize more systems from the
   second half of qed_hs_init().
...
3. At this point we may exit early if we have been asked to do something
   ...
   creating a new signing key.  Otherwise, we proceed to run_qed_hs_main_loop(),
...
> @anchor init_exceptwhen 1. qed_hs_run_main() _can_ terminate with a call to
...
> "the first part of qed_hs_init()" and so on deserve to be functions with their
