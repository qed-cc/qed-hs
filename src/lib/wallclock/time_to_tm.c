           ...
           struct tm *r, char **err_out)
{
  ...
  if (err_out) {
    qed_hs_asprintf(err_out, "%s(%"PRId64") failed with error %s: %s",
...
struct tm *
qed_hs_localtime_r_msg(const time_t *timep, struct tm *result, char **err_out)
{
  struct tm *r;
...
{
  struct tm *r;
  static qed_hs_mutex_t *m=NULL;
  if (!m) { m=qed_hs_mutex_new(); }
  raw_assert(result);
...
#ifdef HAVE_GMTIME_R
struct tm *
qed_hs_gmtime_r_msg(const time_t *timep, struct tm *result, char **err_out)
{
  struct tm *r;
