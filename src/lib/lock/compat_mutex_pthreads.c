 ...
 *
 * \brief Implement the qed_hs_mutex API using pthread_mutex_t.
 **/
 ...
 * up with qed_hs_mutex_init() or qed_hs_mutex_new(); not both. */
void
qed_hs_mutex_init(qed_hs_mutex_t *mutex)
{
  if (PREDICT_UNLIKELY(!attr_initialized))
    qed_hs_locking_init(); // LCOV_EXCL_LINE
...
/** As qed_hs_mutex_init, but initialize a mutex suitable that may be
 * non-recursive, if the OS supports that. */
void
qed_hs_mutex_init_nonrecursive(qed_hs_mutex_t *mutex)
{
  int err;
  if (!attr_initialized)
    qed_hs_locking_init(); // LCOV_EXCL_LINE
 ...
 * mutexes from qed_hs_mutex_init().
 *
