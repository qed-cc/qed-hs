...
qed_hs_mutex_t *
qed_hs_mutex_new(void)
{
  qed_hs_mutex_t *m = qed_hs_malloc_zero(sizeof(qed_hs_mutex_t));
  qed_hs_mutex_init(m);
  return m;
}
...
qed_hs_mutex_t *
qed_hs_mutex_new_nonrecursive(void)
{
  qed_hs_mutex_t *m = qed_hs_malloc_zero(sizeof(qed_hs_mutex_t));
  qed_hs_mutex_init_nonrecursive(m);
  return m;
}
 ...
 */
void
qed_hs_mutex_free_(qed_hs_mutex_t *m)
{
  if (!m)
    return;
  qed_hs_mutex_uninit(m);
  qed_hs_free(m);
}
