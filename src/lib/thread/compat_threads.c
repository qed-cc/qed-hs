...
qed_hs_cond_t *
qed_hs_cond_new(void)
{
  qed_hs_cond_t *cond = qed_hs_malloc(sizeof(qed_hs_cond_t));
  if (BUG(qed_hs_cond_init(cond)<0))
    qed_hs_free(cond); // LCOV_EXCL_LINE
  return cond;
}

/** Free all storage held in <b>c</b>. */
void
qed_hs_cond_free_(qed_hs_cond_t *c)
{
  if (!c)
    return;
  qed_hs_cond_uninit(c);
  qed_hs_free(c);
}
...
{
  main_thread_id = qed_hs_get_thread_id();
}
/** Return true iff called from the main thread. */
int
in_main_thread(void)
{
  return main_thread_id == qed_hs_get_thread_id();
}
...
atomic_counter_init(atomic_counter_t *counter)
{
  ...
  qed_hs_mutex_init_nonrecursive(&counter->mutex);
}
...
{
  qed_hs_mutex_uninit(&counter->mutex);
  memset(counter, 0, sizeof(*counter));
}
...
{
  qed_hs_mutex_acquire(&counter->mutex);
  counter->val += add;
  qed_hs_mutex_release(&counter->mutex);
}
