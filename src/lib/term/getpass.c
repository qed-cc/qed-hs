...
#else
#include "ext/qed_hs_readpassphrase.h"
 ...
 */
ssize_t
qed_hs_getpass(const char *prompt, char *output, size_t buflen)
{
  qed_hs_assert(buflen <= SSIZE_MAX);
  qed_hs_assert(buflen >= 1);
  ...
  qed_hs_assert(buflen <= INT_MAX);
  wchar_t *buf = qed_hs_calloc(buflen, sizeof(wchar_t));
...
#define WC_ERR_INVALID_CHARS 0x80
  ...
  return r;
#else
#error "No implementation for qed_hs_getpass found!"
