...
scan_unsigned(const char **bufp, unsigned long *out, int width, unsigned base)
{
  ...
  while (**bufp && (hex?QED_HS_ISXDIGIT(**bufp):QED_HS_ISDIGIT(**bufp))
...
scan_double(const char **bufp, double *out, int width)
{
  ...
  while (**bufp && QED_HS_ISDIGIT(**bufp) && scanned_so_far < width) {
...
scan_string(const char **bufp, char *out, int width)
{
  ...
  if (!bufp || !out || width < 0)
    return -1;
  while (**bufp && ! QED_HS_ISSPACE(**bufp) && scanned_so_far < width) {
    *out++ = *(*bufp)++;
    ++scanned_so_far;
  }
  *out = '\0';
 ...
 * qed_hs_sscanf() documentation.  */
int
qed_hs_vsscanf(const char *buf, const char *pattern, va_list ap)
{
  int n_matched = 0;
  ...
  while (*pattern) {
    if (*pattern != '%') {
      ...
      int longmod = 0;
      ++pattern;
      if (QED_HS_ISDIGIT(*pattern)) {
        width = digit_to_num(*pattern++);
        while (QED_HS_ISDIGIT(*pattern)) {
          width *= 10;
          width += digit_to_num(*pattern++);
 ...
 */
int
qed_hs_sscanf(const char *buf, const char *pattern, ...)
{
  int r;
  va_list ap;
  va_start(ap, pattern);
  r = qed_hs_vsscanf(buf, pattern, ap);
  va_end(ap);
  return r;
}
