...
or end of a string.
	...
	qed_hs_assert(!strcmpstart("Hello world","Hello"));
	qed_hs_assert(!strcmpend("Hello world","world"));

	qed_hs_assert(!strcasecmpstart("HELLO WORLD","Hello"));
	qed_hs_assert(!strcasecmpend("HELLO WORLD","world"));
...
terminated memory block, use qed_hs_memstr or qed_hs_memmem respectively.
...
recommend qed_hs_memeq() and qed_hs_memneq() for when you need a
...
For numbers in general, use the qed_hs_parse_{long,ulong,double,uint64}
...
most general is as follows:
      ...
      const char *next;
      int ok;
      long lng = qed_hs_parse_long("100", BASE, MINVAL, MAXVAL, &ok, &next);
...
For not-too-large blocks of text, we provide qed_hs_asprintf(), which
