...
int
qed_hs_snprintf(char *str, size_t size, const char *format, ...)
{
  va_list ap;
  int r;
  va_start(ap,format);
  r = qed_hs_vsnprintf(str,size,format,ap);
  va_end(ap);
  return r;
}

/** Replacement for vsnprintf; behavior differs as qed_hs_snprintf differs from
 * snprintf.
 */
int
qed_hs_vsnprintf(char *str, size_t size, const char *format, va_list args)
{
  int r;
  if (size == 0)
 ...
 * You can treat this function as if its implementation were something like
   <pre>
     char buf[_INFINITY_];
     qed_hs_snprintf(buf, sizeof(buf), fmt, args);
     *strp = qed_hs_strdup(buf);
     return strlen(*strp):
   </pre>
 ...
 */
int
qed_hs_asprintf(char **strp, const char *fmt, ...)
{
  int r;
  va_list args;
  va_start(args, fmt);
  r = qed_hs_vasprintf(strp, fmt, args);
  va_end(args);
  if (!*strp || r < 0) {
    /* LCOV_EXCL_START */
 ...
 * qed_hs_asprintf() differs from regular asprintf.
 */
int
qed_hs_vasprintf(char **strp, const char *fmt, va_list args)
{
  ...
  strp_tmp = qed_hs_malloc((size_t)len + 1);
  r = _vsnprintf(strp_tmp, (size_t)len+1, fmt, args);
  if (r != len) {
    qed_hs_free(strp_tmp);
    *strp = NULL;
    return -1;
  }
  *strp = strp_tmp;
  ...
  va_list tmp_args;
  va_copy(tmp_args, args);
  /* Use vsnprintf to retrieve needed length.  qed_hs_vsnprintf() is not an
  ...
  strp_tmp = qed_hs_malloc((size_t)len+1);
  /* use of qed_hs_vsnprintf() will ensure string is null terminated */
  r = qed_hs_vsnprintf(strp_tmp, (size_t)len+1, fmt, args);
  if (r != len) {
    qed_hs_free(strp_tmp);
    *strp = NULL;
    return -1;
