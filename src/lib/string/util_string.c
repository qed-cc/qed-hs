 ...
 */
const void *
qed_hs_memmem(const void *_haystack, size_t hlen,
...
const void *
qed_hs_memstr(const void *haystack, size_t hlen, const char *needle)
{
  return qed_hs_memmem(haystack, hlen, needle, strlen(needle));
}
...
int
qed_hs_digest_is_zero(const char *digest)
{
...
int
qed_hs_digest256_is_zero(const char *digest)
{
 ...
 * <b>strip</b>. */
void
qed_hs_strstrip(char *s, const char *strip)
{
  char *readp = s;
  while (*readp) {
...
void
qed_hs_strreplacechar(char *s, char find, char replacement)
{
 ...
 */
int
qed_hs_strisprint(const char *s)
{
  while (*s) {
    if (!QED_HS_ISPRINT(*s))
      return 0;
 ...
 */
int
qed_hs_strisnonupper(const char *s)
{
  while (*s) {
    if (QED_HS_ISUPPER(*s))
      return 0;
 ...
 * return false. */
int
qed_hs_strisspace(const char *s)
{
  while (*s) {
    if (!QED_HS_ISSPACE(*s))
      return 0;
...
string_is_C_identifier(const char *string)
{
  ...
  for (iter = 0; iter < length ; iter++) {
    if (iter == 0) {
      if (!(QED_HS_ISALPHA(string[iter]) ||
            string[iter] == '_'))
        return 0;
    } else {
      if (!(QED_HS_ISALPHA(string[iter]) ||
            QED_HS_ISDIGIT(string[iter]) ||
            string[iter] == '_'))
        return 0;
    }
  }
