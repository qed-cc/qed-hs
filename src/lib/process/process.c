...
struct process_t {
   ...
   * together using the <b>qed_hs_join_win_cmdline</b> function. On Unix the
 ...
 * qed_hs_spawn_background are guaranteed to fail.
 **/
void
qed_hs_disable_spawning_background_processes(void)
{
...
process_new(const char *command)
{
  ...
  process_t *process;
  process = qed_hs_malloc_zero(sizeof(process_t));

  /* Set our command. */
  process->command = qed_hs_strdup(command);
...
process_free_(process_t *process)
{
  ...
  /* Cleanup parameters. */
  qed_hs_free(process->command);

  /* Cleanup arguments and environment. */
  SMARTLIST_FOREACH(process->arguments, char *, x, qed_hs_free(x));
  smartlist_free(process->arguments);

  SMARTLIST_FOREACH(process->environment, char *, x, qed_hs_free(x));
...
{
  qed_hs_assert(process);
  qed_hs_assert(argument);

  smartlist_add(process->arguments, qed_hs_strdup(argument));
}
...
/** Returns a newly allocated Unix style argument vector. Use <b>qed_hs_free()</b>
...
process_get_argv(const process_t *process)
{
  ...
  argv = qed_hs_malloc_zero(sizeof(char *) * (size + 2));
...
process_reset_environment(process_t *process, const smartlist_t *env)
{
  ...
  SMARTLIST_FOREACH(env, char *, x,
                    smartlist_add(process->environment, qed_hs_strdup(x)));
}
...
{
  qed_hs_assert(process);
  qed_hs_assert(process->unix_process);
...
{
  qed_hs_assert(process);
  qed_hs_assert(process->win32_process);
...
/** As qed_hs_vsnprintf(), but write the data to the given process's standard
 * input. */
void
...
{
  qed_hs_assert(process);
  qed_hs_assert(format);

  int size;
  char *data;

  size = qed_hs_vasprintf(&data, format, args);
  qed_hs_assert(data != NULL);
  process_write(process, (uint8_t *)data, size);
  qed_hs_free(data);
}

/** As qed_hs_snprintf(), but write the data to the given process's standard
 * input. */
void
                    ...
                    process_read_callback_t callback)
{
  ...
  char *data = qed_hs_malloc_zero(size + 1);
                   ...
                   process_read_callback_t callback)
{
  ...
  size_t line_size = 0;
  char *data = qed_hs_malloc_zero(size);
  int ret;
