 ...
 *
 * The qed_hs_process_monitor*() code above doesn't use them, since it is for
...
HT_GENERATE2(process_map, waitpid_callback_t, node, process_map_entry_hash_,
             process_map_entries_eq_, 0.6, qed_hs_reallocarray_, qed_hs_free_);
...
set_waitpid_callback(pid_t pid, void (*fn)(int, void *), void *arg)
{
  ...
  waitpid_callback_t *ent = qed_hs_malloc_zero(sizeof(waitpid_callback_t));
  ent->pid = pid;
  ent->userfn = fn;
  ent->userdata = arg;
  ent->running = 1;
