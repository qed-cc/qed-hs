...
environment_variable_names_equal(const char *s1, const char *s2)
{
  ...
  return (s1_name_len == s2_name_len &&
          qed_hs_memeq(s1, s2, s1_name_len));
}
...
process_environment_free_(process_environment_t *env)
{
  ...
  qed_hs_free(env->unixoid_environment_block);
  qed_hs_free(env->windows_environment_block);
...
{
  process_environment_t *env = qed_hs_malloc_zero(sizeof(process_environment_t));
  int n_env_vars = smartlist_len(env_vars);
  int i;
  size_t total_env_length;
  smartlist_t *env_vars_sorted;

  qed_hs_assert(n_env_vars + 1 != 0);
  env->unixoid_environment_block = qed_hs_calloc(n_env_vars + 1, sizeof(char *));
  ...
  for (i = 0; i < n_env_vars; ++i) {
    ...
    qed_hs_assert(slen + 1 != 0);
    qed_hs_assert(slen + 1 < SIZE_MAX - total_env_length);
    total_env_length += slen + 1;
  }

  env->windows_environment_block = qed_hs_malloc_zero(total_env_length);
  ...
  /* Now copy the strings into the environment blocks. */
  {
    ...
    qed_hs_assert(cp == env->windows_environment_block + total_env_length - 1);
  }
