...
process_win32_new(void)
{
  ...
  win32_process = qed_hs_malloc_zero(sizeof(process_win32_t));
...
process_win32_exec(process_t *process)
{
   ...
   */
  char *joined_argv = qed_hs_join_win_cmdline((const char **)argv);
...
process_win32_timer_start(void)
{
  ...
  periodic_timer = periodic_timer_new(qed_hs_libevent_get_base(),
                                      &interval,
...
{
  qed_hs_assert(timer == periodic_timer);
...
{
  qed_hs_assert(read_pipe);
  qed_hs_assert(write_pipe);
  qed_hs_assert(attributes);
  ...
  qed_hs_snprintf(pipe_name, sizeof(pipe_name),
...
{
  qed_hs_assert(overlapped);
  qed_hs_assert(overlapped->hEvent);
                                     ...
                                     DWORD byte_count)
{
  ...
  if (error_code == 0) {
     ...
     * BUFFER_SIZE. */
    qed_hs_assert(byte_count <= BUFFER_SIZE);
...
format_win_cmdline_argument(const char *arg)
{
  ...
  const size_t formatted_arg_len = smartlist_len(arg_chars) +
    (need_quotes ? 2 : 0) + 1;
  formatted_arg = qed_hs_malloc_zero(formatted_arg_len);
...
STATIC char *
qed_hs_join_win_cmdline(const char *argv[])
{
