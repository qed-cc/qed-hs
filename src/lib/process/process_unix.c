...
process_unix_new(void)
{
  ...
  unix_process = qed_hs_malloc_zero(sizeof(process_unix_t));
...
process_unix_free_(process_unix_t *unix_process)
{
  ...
  qed_hs_event_free(unix_process->stdout_handle.event);
  qed_hs_event_free(unix_process->stderr_handle.event);
  qed_hs_event_free(unix_process->stdin_handle.event);

  qed_hs_free(unix_process);
}
                          ...
                          event_callback_fn callback)
{
  ...
  handle->event = qed_hs_event_new(qed_hs_libevent_get_base(),
                                handle->fd,
                                flags,
                                callback,
                                process);
