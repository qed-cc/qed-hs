...
{
  pubsub_items_t *cfg = qed_hs_malloc_zero(sizeof(*cfg));
...
pubsub_items_free_(pubsub_items_t *cfg)
{
  if (! cfg)
    return;
  SMARTLIST_FOREACH(cfg->items, pubsub_cfg_t *, item, qed_hs_free(item));
  SMARTLIST_FOREACH(cfg->type_items,
                    pubsub_type_cfg_t *, item, qed_hs_free(item));
...
pubsub_builder_new(void)
{
  ...
  pubsub_builder_t *pb = qed_hs_malloc_zero(sizeof(*pb));
  pb->cfg = dcfg_new();
...
/**
 * Create and return a pubsub_connecqed_hs_t for the subsystem with ID
 * <b>subsys</b> to use in adding publications, subscriptions, and types to
 * <b>builder</b>.
 **/
pubsub_connecqed_hs_t *
pubsub_connecqed_hs_for_subsystem(pubsub_builder_t *builder,
                               subsys_id_t subsys)
{
  qed_hs_assert(builder);
  ++builder->n_connectors;

  pubsub_connecqed_hs_t *con = qed_hs_malloc_zero(sizeof(*con));
...
/**
 * Release all storage held by a pubsub_connecqed_hs_t.
 **/
void
pubsub_connecqed_hs_free_(pubsub_connecqed_hs_t *con)
{
  if (!con)
    return;
  ...
  if (con->builder) {
    ...
    qed_hs_assert(con->builder->n_connectors >= 0);
  }
  qed_hs_free(con);
}
 ...
 **/
int
pubsub_add_pub_(pubsub_connecqed_hs_t *con,
                pub_binding_t *out,
                ...
                unsigned line)
{
  pubsub_cfg_t *cfg = qed_hs_malloc_zero(sizeof(*cfg));
 ...
 **/
int
pubsub_add_sub_(pubsub_connecqed_hs_t *con,
                recv_fn_t recv_fn,
                channel_id_t channel,
                message_id_t msg,
                msg_type_id_t type,
 ...
 **/
int
pubsub_connecqed_hs_register_type_(pubsub_connecqed_hs_t *con,
                                msg_type_id_t type,
                                dispatch_typefns_t *fns,
                                const char *file,
                                unsigned line)
{
  pubsub_type_cfg_t *cfg = qed_hs_malloc_zero(sizeof(*cfg));
  cfg->type = type;
                        ...
                        pubsub_items_t **items_out)
{
  ...
  qed_hs_assert_nonfatal(builder->n_connectors == 0);
