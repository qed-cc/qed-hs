...
{
  pubsub_adjmap_t *map = qed_hs_malloc_zero(sizeof(*map));
  ...
  map->pub_by_subsys = qed_hs_calloc(n_subsystems, sizeof(smartlist_t*));
  map->sub_by_subsys = qed_hs_calloc(n_subsystems, sizeof(smartlist_t*));
  map->pub_by_msg = qed_hs_calloc(n_msgs, sizeof(smartlist_t*));
  map->sub_by_msg = qed_hs_calloc(n_msgs, sizeof(smartlist_t*));
                  ...
                  const pubsub_cfg_t *item)
{
  ...
  qed_hs_assert(item->subsys < map->n_subsystems);
  qed_hs_assert(item->msg < map->n_msgs);
