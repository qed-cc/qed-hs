...
/* as fallback implementation for qed_hs_sleep_msec */
...
monotime_enable_test_mocking(void)
{
  ...
  qed_hs_assert_nonfatal(monotime_mocking_enabled == 0);
...
{
  qed_hs_assert_nonfatal(monotime_mocking_enabled == 1);
...
#if defined(_WIN32) || defined(QED_HS_UNIT_TESTS)
...
#endif /* defined(_WIN32) || defined(QED_HS_UNIT_TESTS) */

#if defined(MONOTIME_USING_GETTIMEOFDAY) || defined(QED_HS_UNIT_TESTS)
...
#endif /* defined(MONOTIME_USING_GETTIMEOFDAY) || defined(QED_HS_UNIT_TESTS) */
...
{
  qed_hs_assert(!monotime_initialized);
  int r = mach_timebase_info(&mach_time_info);
  qed_hs_assert(r == 0);
  qed_hs_assert(mach_time_info.denom != 0);

  {
    ...
    // requires that qed_hs_log2(0) == 0.
    monotime_shift = qed_hs_log2(ticks_per_ms);
  }
  {
...
monotime_init_internal(void)
{
  ...
  qed_hs_assert(n <= INT64_MAX);
  qed_hs_assert(d <= INT64_MAX);
...
static qed_hs_mutex_t monotime_lock;
...
{
  qed_hs_assert(!monotime_initialized);
  qed_hs_mutex_init(&monotime_lock);
}
