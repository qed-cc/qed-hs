                   ...
                   const char *effective_user))
{
  ...
  if (effective_user) {
    ...
    pw = qed_hs_getpwnam(effective_user);
    if (pw == NULL) {
  ...
  if (st.st_uid != running_uid) {
    ...
    {
      const struct passwd *pw_running = qed_hs_getpwuid(running_uid);
      process_ownername = pw_running ? qed_hs_strdup(pw_running->pw_name) :
        qed_hs_strdup("<unknown>");
    }

    {
      const struct passwd *pw_stat = qed_hs_getpwuid(st.st_uid);
      file_ownername = pw_stat ? qed_hs_strdup(pw_stat->pw_name) :
        qed_hs_strdup("<unknown>");
    }
    ...
    qed_hs_free(process_ownername);
    qed_hs_free(file_ownername);
    close(fd);
    return -1;
  }
  if ( (check & (CPD_GROUP_OK|CPD_GROUP_READ))
    ...
    process_groupname = gr ? qed_hs_strdup(gr->gr_name) : qed_hs_strdup("<unknown>");
...
qed_hs_listdir, (const char *dirname))
{
  smartlist_t *result;
#ifdef _WIN32
  char *pattern=NULL;
