...
int qed_hs_open_cloexec(const char *path, int flags, unsigned mode);
FILE *qed_hs_fopen_cloexec(const char *path, const char *mode);
int qed_hs_rename(const char *path_old, const char *path_new);

int replace_file(const char *from, const char *to);
int touch_file(const char *fname);

MOCK_DECL(int,qed_hs_unlink,(const char *pathname));
...
int64_t qed_hs_get_avail_disk_space(const char *path);
...
#endif /* !defined(HAVE_GETDELIM) || defined(QED_HS_UNIT_TESTS) */
 ...
 * raw_free() -- don't use qed_hs_malloc() and qed_hs_free() with this.
 */
#define qed_hs_getdelim(lineptr, n, delim, stream) \
 ...
 * getline() function.
 *
 * See qed_hs_getdelim() for usage notes.
 */
#define qed_hs_getline(lineptr, n, stream) \
  getline((lineptr), (n), (stream))
#else /* !defined(HAVE_GETLINE) */
#define qed_hs_getline(lineptr, n, stream) \
  qed_hs_getdelim((lineptr), (n), '\n', (stream))
#endif /* defined(HAVE_GETLINE) */

#endif /* !defined(QED_HS_FS_H) */
