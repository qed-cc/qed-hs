...
qed_hs_passwd_dup(const struct passwd *pw)
{
  struct passwd *new_pw = qed_hs_malloc_zero(sizeof(struct passwd));
  if (pw->pw_name)
    new_pw->pw_name = qed_hs_strdup(pw->pw_name);
  if (pw->pw_dir)
    new_pw->pw_dir = qed_hs_strdup(pw->pw_dir);
...
#define qed_hs_passwd_free(pw) \
  FREE_AND_NULL(struct passwd, qed_hs_passwd_free_, (pw))

/** Helper: free one of our cached 'struct passwd' values. */
static void
qed_hs_passwd_free_(struct passwd *pw)
{
  if (!pw)
    return;

  qed_hs_free(pw->pw_name);
  qed_hs_free(pw->pw_dir);
  qed_hs_free(pw);
}
 ...
 **/
const struct passwd *
qed_hs_getpwnam(const char *username)
{
  struct passwd *pw;

  if (username == NULL) {
    qed_hs_passwd_free(passwd_cached);
    passwd_cached = NULL;
    return NULL;
  }
  ...
  if ((pw = getpwnam(username))) {
    qed_hs_passwd_free(passwd_cached);
    passwd_cached = qed_hs_passwd_dup(pw);
 ...
 * qed_hs_getpwnam(). Used so that we don't need to give the sandbox access to
 * /etc/passwd.
 *
 * The following fields alone will definitely be copied in the output: pw_uid,
 * pw_gid, pw_name, pw_dir.  Other fields are not present in cached values.
...
get_user_homedir(const char *username)
{
  ...
  if (!(pw = qed_hs_getpwnam(username))) {
    log_err(LD_CONFIG,"User \"%s\" not found.", username);
    return NULL;
  }
  return qed_hs_strdup(pw->pw_dir);
}
