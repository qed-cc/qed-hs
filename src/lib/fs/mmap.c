...
MOCK_IMPL(qed_hs_mmap_t *,
qed_hs_mmap_file,(const char *filename))
{
  int fd; /* router file */
  char *string;
  int result;
  ...
  fd = qed_hs_open_cloexec(filename, O_RDONLY, 0);
  if (fd<0) {
...
MOCK_IMPL(int,
qed_hs_munmap_file,(qed_hs_mmap_t *handle))
{
  int res;
  ...
  if (res == 0) {
    ...
    log_warn(LD_FS, "Failed to munmap() in qed_hs_munmap_file(): %s",
             strerror(errno));
    res = -1;
  }
...
qed_hs_mmap_file,(const char *filename))
{
  ...
  qed_hs_mmap_t *res = qed_hs_malloc_zero(sizeof(qed_hs_mmap_t));
  int empty = 0;
...
qed_hs_munmap_file,(qed_hs_mmap_t *handle))
{
  ...
  if (handle->data) {
    /* This is an ugly cast, but without it, "data" in struct qed_hs_mmap_t would
       have to be redefined as non-const. */
    BOOL ok = UnmapViewOfFile( (LPVOID) handle->data);
    if (!ok) {
      log_warn(LD_FS, "Failed to UnmapViewOfFile() in qed_hs_munmap_file(): %d",
               (int)GetLastError());
    }
  }
...
#else
#error "cannot implement qed_hs_mmap_file"
