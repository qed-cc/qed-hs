 ...
 * otherwise.
 *
 * Return a <b>qed_hs_lockfile_t</b> on success, NULL on failure.
 *
 ...
 *  to do in-process locking, use qed_hs_mutex_t like a normal person.
 ...
 */
qed_hs_lockfile_t *
qed_hs_lockfile_lock(const char *filename, int blocking, int *locked_out)
{
  qed_hs_lockfile_t *result;
  int fd;
  *locked_out = 0;

  log_info(LD_FS, "Locking \"%s\"", filename);
  fd = qed_hs_open_cloexec(filename, O_RDWR|O_CREAT|O_TRUNC, 0600);
  if (fd < 0) {
  ...
  result = qed_hs_malloc(sizeof(qed_hs_lockfile_t));
  result->filename = qed_hs_strdup(filename);
  result->fd = fd;
  return result;
}
...
void
qed_hs_lockfile_unlock(qed_hs_lockfile_t *lockfile)
{
  qed_hs_assert(lockfile);
  ...
  close(lockfile->fd);
  lockfile->fd = -1;
  qed_hs_free(lockfile->filename);
  qed_hs_free(lockfile);
}
