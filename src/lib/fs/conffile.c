...
expand_glob(const char *pattern, smartlist_t *opened_files)
{
  ...
  smartlist_t *matches = qed_hs_glob(pattern);
  if (!matches) {
    if (errno == EPERM) {
  ...
  if (opened_files) {
    ...
    if (!glob_opened) {
      SMARTLIST_FOREACH(matches, char *, f, qed_hs_free(f));
...
config_get_file_list(const char *pattern, smartlist_t *opened_files)
{
  ...
  SMARTLIST_FOREACH_BEGIN(glob_matches, char *, path) {
    ...
    if (file_type == FN_FILE) {
      ...
      smartlist_t *all_files = qed_hs_listdir(path);
      if (!all_files) {
        error_found = true;
        break;
      }
      ...
      SMARTLIST_FOREACH_BEGIN(all_files, char *, f) {
        ...
        char *fullname;
        qed_hs_asprintf(&fullname, "%s"PATH_SEPARATOR"%s", path, f);
        ...
        if (file_status(fullname) != FN_FILE) {
          qed_hs_free(fullname);
          continue;
        }
        smartlist_add(file_list, fullname);
      } SMARTLIST_FOREACH_END(f);
      SMARTLIST_FOREACH(all_files, char *, f, qed_hs_free(f));
  ...
  SMARTLIST_FOREACH(glob_matches, char *, f, qed_hs_free(f));
  smartlist_free(glob_matches);

  if (error_found) {
    SMARTLIST_FOREACH(file_list, char *, f, qed_hs_free(f));
                       ...
                       smartlist_t *opened_lst)
{
  ...
  *list = ret_list;
  ...
  SMARTLIST_FOREACH(config_files, char *, f, qed_hs_free(f));
