 ...
 * set. */
int
qed_hs_open_cloexec(const char *path, int flags, unsigned mode)
{
  int fd;
...
FILE *
qed_hs_fopen_cloexec(const char *path, const char *mode)
{
  const char *p = sandbox_intern_string(path);
...
int
qed_hs_rename(const char *path_old, const char *path_new)
{
...
MOCK_IMPL(int,
qed_hs_unlink,(const char *pathname))
{
...
{
  open_file_t *new_file = qed_hs_malloc_zero(sizeof(open_file_t));
  const char *open_name;
  int append = 0;

  qed_hs_assert(fname);
  qed_hs_assert(data_out);
#if (O_BINARY != 0 && O_TEXT != 0)
  qed_hs_assert((open_flags & (O_BINARY|O_TEXT)) != 0);
#endif
  new_file->fd = -1;
  new_file->filename = qed_hs_strdup(fname);
  if (open_flags & O_APPEND) {
    ...
    qed_hs_asprintf(&new_file->tempname, "%s.tmp", fname);
  ...
  new_file->fd = qed_hs_open_cloexec(open_name, open_flags, mode);
  if (new_file->fd < 0) {
  ...
  if (append) {
    if (qed_hs_fd_seekend(new_file->fd) < 0) {
 ...
 err:
  if (new_file->fd >= 0)
    close(new_file->fd);
  *data_out = NULL;
  qed_hs_free(new_file->filename);
  qed_hs_free(new_file->tempname);
  qed_hs_free(new_file);
  return -1;
}
...
{
  qed_hs_assert(file_data);
  if (file_data->stdio_file)
    return file_data->stdio_file;
  qed_hs_assert(file_data->fd >= 0);
...
finish_writing_to_file_impl(open_file_t *file_data, int abort_write)
{
  ...
  qed_hs_assert(file_data && file_data->filename);
  if (file_data->stdio_file) {
  ...
  if (file_data->rename_on_close) {
    qed_hs_assert(file_data->tempname && file_data->filename);
    if (!abort_write) {
      qed_hs_assert(strcmp(file_data->filename, file_data->tempname));
  ...
  qed_hs_free(file_data->filename);
  qed_hs_free(file_data->tempname);
  qed_hs_free(file_data);
                          ...
                          int open_flags)
{
  ...
  SMARTLIST_FOREACH(chunks, sized_chunk_t *, chunk,
  {
    ...
    qed_hs_assert((size_t)result == chunk->len);
  });
...
read_file_to_str_until_eof(int fd, size_t max_bytes_to_read, size_t *sz_out)
{
  ...
  do {
    ...
    string = qed_hs_realloc(string, string_max);
...
read_file_to_str, (const char *filename, int flags, struct stat *stat_out))
{
  ...
  fd = qed_hs_open_cloexec(filename,O_RDONLY|(bin?O_BINARY:O_TEXT),0);
  if (fd<0) {
...
#define FIFO_READ_MAX (1024*1024)
  ...
  string = qed_hs_malloc((size_t)(statbuf.st_size+1));
...
#if !defined(HAVE_GETDELIM) || defined(QED_HS_UNIT_TESTS)
