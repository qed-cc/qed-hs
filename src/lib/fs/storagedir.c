...
storage_dir_new(const char *dirname, int max_files)
{
  ...
  storage_dir_t *d = qed_hs_malloc_zero(sizeof(storage_dir_t));
  d->directory = qed_hs_strdup(dirname);
...
storage_dir_free_(storage_dir_t *d)
{
  if (d == NULL)
    return;
  qed_hs_free(d->directory);
  if (d->contents) {
    SMARTLIST_FOREACH(d->contents, char *, cp, qed_hs_free(cp));
...
storage_dir_register_with_sandbox(storage_dir_t *d, sandbox_cfg_t **cfg)
{
  ...
  for (idx = FNAME_MIN_NUM; idx < FNAME_MIN_NUM + d->max_files; ++idx) {
    ...
    qed_hs_asprintf(&path, "%s/%d", d->directory, idx);
    qed_hs_asprintf(&tmppath, "%s/%d.tmp", d->directory, idx);

    problems += sandbox_cfg_allow_open_filename(cfg, qed_hs_strdup(path));
    problems += sandbox_cfg_allow_open_filename(cfg, qed_hs_strdup(tmppath));
    problems += sandbox_cfg_allow_stat_filename(cfg, qed_hs_strdup(path));
    problems += sandbox_cfg_allow_stat_filename(cfg, qed_hs_strdup(tmppath));
    problems += sandbox_cfg_allow_rename(cfg,
                                      qed_hs_strdup(tmppath), qed_hs_strdup(path));
...
storage_dir_clean_tmpfiles(storage_dir_t *d)
{
  ...
  SMARTLIST_FOREACH_BEGIN(d->contents, char *, fname) {
    if (strcmpend(fname, ".tmp"))
      continue;
    char *path = NULL;
    qed_hs_asprintf(&path, "%s/%s", d->directory, fname);
...
storage_dir_rescan(storage_dir_t *d)
{
  ...
  d->usage = 0;
  d->usage_known = 0;
  if (NULL == (d->contents = qed_hs_listdir(d->directory))) {
    return -1;
  }
...
storage_dir_get_usage(storage_dir_t *d)
{
  ...
  SMARTLIST_FOREACH_BEGIN(storage_dir_list(d), const char *, cp) {
    char *path = NULL;
    struct stat st;
    qed_hs_asprintf(&path, "%s/%s", d->directory, cp);
 ...
 *
 * On failure, return NULL and set errno as for qed_hs_mmap_file(). */
qed_hs_mmap_t *
storage_dir_map(storage_dir_t *d, const char *fname)
{
  char *path = NULL;
  qed_hs_asprintf(&path, "%s/%s", d->directory, fname);
  qed_hs_mmap_t *result = qed_hs_mmap_file(path);
  int errval = errno;
  qed_hs_free(path);
  if (result == NULL)
    errno = errval;
...
storage_dir_read(storage_dir_t *d, const char *fname, int bin, size_t *sz_out)
{
  ...
  if (contents && sz_out) {
    ...
    qed_hs_assert((uint64_t)st.st_size <= SIZE_MAX);
#endif
...
find_unused_fname(storage_dir_t *d)
{
  ...
  for (i = FNAME_MIN_NUM; i < FNAME_MIN_NUM + d->max_files; ++i) {
    qed_hs_snprintf(buf, sizeof(buf), "%d", i);
                                ...
                                char **fname_out)
{
  ...
  if (r == 0) {
    ...
    if (fname_out) {
      *fname_out = qed_hs_strdup(fname);
    }
    if (d->contents)
      smartlist_add(d->contents, qed_hs_strdup(fname));
  }
  qed_hs_free(fname);
  qed_hs_free(path);
  return r;
                                  ...
                                  char **fname_out)
{
  ...
  for (line = labels; line; line = line->next) {
    ...
    qed_hs_snprintf(bytes, allocated, "%s %s\n", line->key, line->value);
    sz->bytes = bytes;
 ...
 * the data's size into *<b>sz_out</b>. On success, also return a qed_hs_mmap_t
 ...
 *
 * On failure, set errno as for qed_hs_mmap_file() if the file was missing or
...
storage_dir_map_labeled(storage_dir_t *dir,
                         ...
                         size_t *sz_out)
{
  qed_hs_mmap_t *m = storage_dir_map(dir, fname);
  int errval;
  if (! m) {
    errval = errno;
    goto err;
                          ...
                          size_t *sz_out)
{
  ...
  qed_hs_mmap_t *m = storage_dir_map_labeled(dir, fname, labels_out,
                                           &data, sz_out);
  if (m == NULL)
    return NULL;
  uint8_t *result = qed_hs_memdup(data, *sz_out);
  qed_hs_munmap_file(m);
  return result;
}
                   ...
                   int min_to_remove)
{
  ...
  shrinking_dir_entry_t *ents = qed_hs_calloc(n, sizeof(shrinking_dir_entry_t));
  SMARTLIST_FOREACH_BEGIN(d->contents, const char *, fname) {
    shrinking_dir_entry_t *ent = &ents[fname_sl_idx];
    struct stat st;
    qed_hs_asprintf(&ent->path, "%s/%s", d->directory, fname);
