...
get_unquoted_path(const char *path)
{
  ...
  char *unquoted_path = qed_hs_malloc(len - has_start_quote - has_end_quote + 1);
...
expand_filename(const char *filename)
{
  ...
  if (*filename == '~') {
    ...
    if (filename[1] == '/' || filename[1] == '\0') {
      ...
      if (slash)
        username = qed_hs_strndup(filename+1,slash-filename-1);
      else
        username = qed_hs_strdup(filename+1);
      if (!(home = get_user_homedir(username))) {
        log_warn(LD_CONFIG,"Couldn't get homedir for \"%s\"",username);
        qed_hs_free(username);
        return NULL;
      }
      qed_hs_free(username);
      rest = slash ? (slash+1) : "";
    ...
    qed_hs_asprintf(&result,"%s"PATH_SEPARATOR"%s",home,rest);
    qed_hs_free(home);
    return result;
  } else {
    return qed_hs_strdup(filename);
...
path_is_relative(const char *filename)
{
  if (filename && filename[0] == '/')
    ...
    return 0;
  else if (filename && strlen(filename)>3 && QED_HS_ISALPHA(filename[0]) &&
...
alloc_getcwd(void)
{
  ...
  char *result = NULL;
  if (cwd) {
    /* We make a copy here, in case qed_hs_malloc() is not malloc(). */
    result = qed_hs_strdup(cwd);
...
make_path_absolute(const char *fname)
{
  ...
  /* We don't want to assume that qed_hs_free can free a string allocated
   * with malloc.  On failure, return fname (it's better than nothing). */
  char *absfname = qed_hs_strdup(absfname_malloced ? absfname_malloced : fname);
  ...
  if (fname[0] == '/') {
    absfname = qed_hs_strdup(fname);
  } else {
    path = alloc_getcwd();
    if (path) {
      qed_hs_asprintf(&absfname, "%s/%s", path, fname);
      qed_hs_free(path);
    } else {
...
/* The code below implements qed_hs_glob and get_glob_opened_files. Because it is
 * not easy to understand it by looking at individual functions, the big
 * picture explanation here should be read first.
 *
 * Purpose of the functions:
 * - qed_hs_glob - receives a pattern and returns all the paths that result from
 ...
 *   allowed list.
 *
 * Due to OS API differences explained below, the implementation of qed_hs_glob is
 * completely different for Windows and POSIX systems, so we ended up with
 * three different implementations:
 * - qed_hs_glob for POSIX - as POSIX glob does everything we need, we simply call
 *   it and process the results. This is completely implemented in qed_hs_glob.
 * - qed_hs_glob for WIN32 - because the WIN32 API only supports expanding globs
 ...
 *
 * As seen from the description above, both qed_hs_glob for WIN32 and
 ...
 *   while qed_hs_glob for WIN32 collects full paths resulting from the expansion
 *   of all globs.
 * - get_glob_opened_files can call qed_hs_glob to expand path fragments with
 *   globs while qed_hs_glob for WIN32 cannot because it IS qed_hs_glob. For qed_hs_glob
 *   for WIN32, an auxiliary function has to be used for this purpose.
 *
 * To avoid code duplication, the logic of qed_hs_glob for WIN32 and
 ...
 *   (implements qed_hs_glob). Otherwise, returns the paths before path fragments
 ...
 *   - unglob_win32 - uses qed_hs_listdir and PathMatchSpec (for qed_hs_glob WIN32)
 *   - unglob_opened_files - uses qed_hs_glob (for get_glob_opened_files)
 */
 ...
 * returns true otherwise. Used on qed_hs_glob for WIN32. */
static bool
add_non_glob_path(const char *path, struct smartlist_t *result)
{
  ...
  if (file_type == FN_ERROR) {
    ...
    char *to_add = qed_hs_strdup(path);
    clean_fname_for_stat(to_add);
    smartlist_add(result, to_add);
  }
  /* If WIN32 qed_hs_glob is called with a non-existing path, we want it to
   * return an empty list instead of error to match the regular version */
  return true;
}

/** Auxiliary function used by get_glob_opened_files and WIN32 qed_hs_glob.
 ...
 * of the glob expansion of <b>pattern</b> (implements qed_hs_glob). Otherwise,
...
get_glob_paths(const char *pattern, unglob_fn unglob, bool final)
{
  ...
  if (!final) {
    ...
    char *path_until_glob = qed_hs_strndup(pattern, len);
  ...
  if (!unglobbed_paths) {
    ...
    SMARTLIST_FOREACH_BEGIN(unglobbed_paths, char *, current_path) {
      char *next_path;
      qed_hs_asprintf(&next_path, "%s"PATH_SEPARATOR"%s", current_path,
                   &pattern[next_sep+1]);
      smartlist_t *opened_next = get_glob_paths(next_path, unglob, final);
      qed_hs_free(next_path);
      if (!opened_next) {
        error_found = true;
        break;
      }
    ...
    SMARTLIST_FOREACH(unglobbed_paths, char *, p, qed_hs_free(p));
    smartlist_free(unglobbed_paths);
  }

end:
  if (error_found) {
    SMARTLIST_FOREACH(result, char *, p, qed_hs_free(p));
 ...
 * failure. Used by the WIN32 implementation of qed_hs_glob. Implements unglob_fn,
...
unglob_win32(const char *pattern, int prev_sep, int next_sep)
{
  ...
  if (!is_file(file_status(path_until_glob))) {
    smartlist_t *filenames = qed_hs_listdir(path_until_glob);
    if (!filenames) {
      smartlist_free(result);
      result = NULL;
    } else {
      SMARTLIST_FOREACH_BEGIN(filenames, char *, filename) {
        ...
        char *full_path;
        qed_hs_asprintf(&full_path, "%s"PATH_SEPARATOR"%s",
                     path_until_glob, filename);
        char *path_curr_glob = qed_hs_strndup(pattern, next_sep + 1);
        ...
        if (PathMatchSpec(tfile, tpattern)) {
          ...
          qed_hs_free(full_path);
        }
        qed_hs_free(path_curr_glob);
      } SMARTLIST_FOREACH_END(filename);
      SMARTLIST_FOREACH(filenames, char *, p, qed_hs_free(p));
      smartlist_free(filenames);
    }
  }
  qed_hs_free(path_until_glob);
  return result;
}
#elif HAVE_GLOB
 ...
 */
struct smartlist_t *
qed_hs_glob(const char *pattern)
{
  smartlist_t *result = NULL;

#ifdef _WIN32
  // PathMatchSpec does not support forward slashes, change them to backslashes
  char *pattern_normalized = qed_hs_strdup(pattern);
  qed_hs_strreplacechar(pattern_normalized, '/', *PATH_SEPARATOR);
  result = get_glob_paths(pattern_normalized, unglob_win32, true);
  qed_hs_free(pattern_normalized);
  ...
  for (i = 0; i < matches.gl_pathc; i++) {
    char *match = qed_hs_strdup(matches.gl_pathv[i]);
...
/** Expands globs in <b>pattern</b> for the path fragment between
 * <b>prev_sep</b> and <b>next_sep</b> using qed_hs_glob. Returns NULL on
...
unglob_opened_files(const char *pattern, int prev_sep, int next_sep)
{
  ...
  if (has_glob(&pattern[next_sep+1])) {
    ...
    char *glob_path = qed_hs_strndup(pattern, next_sep);
    smartlist_t *child_paths = qed_hs_glob(glob_path);
    qed_hs_free(glob_path);
    if (!child_paths) {
      smartlist_free(result);
      result = NULL;
    } else {
...
/** Returns a list of files that are opened by the qed_hs_glob function when
