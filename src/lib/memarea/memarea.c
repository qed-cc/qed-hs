/* Copyright (c) 2008-2021, The QED Project, Inc. */
...
#define CHECK_SENTINEL(chunk)                                           \
  ...
  qed_hs_assert(sent_val == SENTINEL_VAL);                                 \
  STMT_END
...
{
  qed_hs_assert(sz < SIZE_T_CEILING);

  size_t chunk_size = sz < CHUNK_SIZE ? CHUNK_SIZE : sz;
  memarea_chunk_t *res;
  chunk_size += SENTINEL_LEN;
  res = qed_hs_malloc(chunk_size);
  res->next_chunk = NULL;
  res->mem_size = chunk_size - CHUNK_HEADER_SIZE - SENTINEL_LEN;
  res->next_mem = res->U_MEM;
  qed_hs_assert(res->next_mem+res->mem_size+SENTINEL_LEN ==
             ((char*)res)+chunk_size);
  qed_hs_assert(realign_pointer(res->next_mem) == res->next_mem);
  SET_SENTINEL(res);
  return res;
}
...
memarea_new(void)
{
  memarea_t *head = qed_hs_malloc(sizeof(memarea_t));
...
memarea_alloc(memarea_t *area, size_t sz)
{
  ...
  if (sz == 0)
    sz = 1;
  qed_hs_assert(chunk->next_mem <= chunk->U_MEM + chunk->mem_size);
...
memarea_strndup(memarea_t *area, const char *s, size_t n)
{
  size_t ln = 0;
  char *result;
  qed_hs_assert(n < SIZE_T_CEILING);
...
{
  memarea_chunk_t *chunk;
  qed_hs_assert(area->first);

  for (chunk = area->first; chunk; chunk = chunk->next_chunk) {
    CHECK_SENTINEL(chunk);
    qed_hs_assert(chunk->next_mem >= chunk->U_MEM);
    qed_hs_assert(chunk->next_mem <=
...
memarea_new(void)
{
  memarea_t *ma = qed_hs_malloc_zero(sizeof(memarea_t));
...
memarea_clear(memarea_t *area)
{
  SMARTLIST_FOREACH(area->pieces, void *, p, qed_hs_free_(p));
...
memarea_alloc_zero(memarea_t *area, size_t sz)
{
  void *result = qed_hs_malloc_zero(sz);
  smartlist_add(area->pieces, result);
  return result;
}
void *
