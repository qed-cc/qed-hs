...
static qed_hs_x509_cert_impl_t *qed_hs_x509_cert_decode_internal(
                      const uint8_t *certificate, int certificate_len);

static qed_hs_x509_cert_impl_t *
qed_hs_tls_create_certificate_internal(crypto_pk_t *rsa,
                                    ...
                                    time_t end_time)
{
  ...
  *cert->version.data = 2; /* 2 means version 3. */
  ...
  result_cert = qed_hs_x509_cert_decode_internal(signed_der.data, signed_der.len);

#if 1
  {
    // Can we check the cert we just signed?
    qed_hs_assert(result_cert);
    ...
    qed_hs_assert(cert_ok == SECSuccess);
  }
#endif /* 1 */
...
MOCK_IMPL(qed_hs_x509_cert_impl_t *,
                            ...
                            unsigned int cert_lifetime))
{
  ...
  qed_hs_asprintf(&cname_rfc_1485, "CN=%s", cname);
  qed_hs_asprintf(&cname_sign_rfc_1485, "CN=%s", cname_sign);
  ...
  qed_hs_tls_pick_certificate_lifetime(time(NULL), cert_lifetime,
                                    &start_time, &end_time);

  result = qed_hs_tls_create_certificate_internal(rsa,
                                               rsa_sign,
                                               subject_dn,
                                               issuer_dn,
                                               start_time,
                                               end_time);
 err:
  qed_hs_free(cname_rfc_1485);
  qed_hs_free(cname_sign_rfc_1485);
  if (subject_dn)
...
/**
 * As qed_hs_x509_cert_decode, but return the NSS certificate type
*/
static qed_hs_x509_cert_impl_t *
qed_hs_x509_cert_decode_internal(const uint8_t *certificate,
                              int certificate_len)
{
...
qed_hs_x509_cert_t *
qed_hs_x509_cert_decode(const uint8_t *certificate,
                     size_t certificate_len)
{
  CERTCertificate *cert = qed_hs_x509_cert_decode_internal(certificate,
                                                        (int)certificate_len);
  if (! cert) {
  ...
  qed_hs_x509_cert_t *newcert = qed_hs_x509_cert_new(cert);
                      ...
                      int check_rsa_1024)
{
  ...
  if (qed_hs_x509_check_cert_lifetime_internal(severity,
                                            cert->cert,
                                            now,
...
int
qed_hs_x509_check_cert_lifetime_internal(int severity,
                                 ...
                                 crypto_pk_t *signing_key)
{
  ...
  crypto_pk_t *subject_key = qed_hs_tls_cert_get_key((qed_hs_x509_cert_t *)inp);
  if (!subject_key)
    return NULL;

  CERTCertificate *newcert;

  newcert = qed_hs_tls_create_certificate_internal(subject_key,
                                                signing_key,
                                                &inp->cert->subject,
                                                &inp->cert->issuer,
                                                start_time,
  ...
  return newcert ? qed_hs_x509_cert_new(newcert) : NULL;
}
