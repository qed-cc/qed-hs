...
typedef struct qed_hs_nss_bytecounts_t {
  uint64_t n_read;
  uint64_t n_written;
} qed_hs_nss_bytecounts_t;

/**
 * Initialize this module, if it is not already initialized.
 **/
void
qed_hs_nss_countbytes_init(void)
{
  if (countbytes_initialized)
    return;

  countbytes_layer_id = PR_GetUniqueIdentity("Tor byte-counting layer");
  qed_hs_assert(countbytes_layer_id != PR_INVALID_IO_LAYER);
...
/**
 * Return the qed_hs_nss_bytecounts_t object for a given IO layer. Asserts that
 * the IO layer is in fact a layer created by this module.
 */
static qed_hs_nss_bytecounts_t *
get_counts(PRFileDesc *fd)
{
  qed_hs_assert(fd->identity == countbytes_layer_id);
  return (qed_hs_nss_bytecounts_t*) fd->secret;
}
...
countbytes_close_fn(PRFileDesc *fd)
{
  ...
  qed_hs_nss_bytecounts_t *counts = (qed_hs_nss_bytecounts_t *)fd->secret;
  qed_hs_free(counts);
  fd->secret = NULL;
 ...
 */
PRFileDesc *
qed_hs_wrap_prfiledesc_with_byte_counter(PRFileDesc *stack)
{
  if (BUG(! countbytes_initialized)) {
    qed_hs_nss_countbytes_init();
  }

  qed_hs_nss_bytecounts_t *bytecounts = qed_hs_malloc_zero(sizeof(*bytecounts));
  ...
  */
  ...
  qed_hs_assert(status == PR_SUCCESS);

  return stack;
}

/**
 * Given a PRFileDesc returned by qed_hs_wrap_prfiledesc_with_byte_counter(),
 ...
 */
int
qed_hs_get_prfiledesc_byte_counts(PRFileDesc *fd,
                               uint64_t *n_read_out,
                               uint64_t *n_written_out)
{
  ...
  qed_hs_nss_bytecounts_t *counts = get_counts(bclayer);
