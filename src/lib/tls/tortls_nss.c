...
qed_hs_tls_context_new(crypto_pk_t *identity,
                    unsigned int key_lifetime, unsigned flags, int is_client)
{
  SECStatus s;
  qed_hs_assert(identity);

  qed_hs_tls_init();

  qed_hs_tls_context_t *ctx = qed_hs_malloc_zero(sizeof(qed_hs_tls_context_t));
  ctx->refcnt = 1;

  if (! is_client) {
    if (qed_hs_tls_context_init_certificates(ctx, identity,
...
void
qed_hs_tls_context_impl_free_(qed_hs_tls_context_impl_t *ctx)
{
  if (!ctx)
    return;
  PR_Close(ctx);
...
{
  qed_hs_nss_countbytes_init();
}

void
tls_log_errors(qed_hs_tls_t *tls, int severity, int domain,
               const char *doing)
{
  ...
  if (!string)
    string = "<unrecognized>";
  if (!name) {
    qed_hs_snprintf(buf, sizeof(buf), "%d", code);
    name = buf;
  }
...
qed_hs_tls_t *
qed_hs_tls_new(qed_hs_socket_t sock, int is_server)
{
  (void)sock;
  qed_hs_tls_context_t *ctx = qed_hs_tls_context_get(is_server);
  ...
  PRFileDesc *count = qed_hs_wrap_prfiledesc_with_byte_counter(tcp);
  if (! count)
    return NULL;
  ...
  qed_hs_tls_t *tls = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));
  tls->magic = QED_HS_TLS_MAGIC;
  tls->context = ctx;
  qed_hs_tls_context_incref(ctx);
  tls->ssl = ssl;
  tls->socket = sock;
...
qed_hs_tls_release_socket(qed_hs_tls_t *tls)
{
   ...
   */
  qed_hs_socket_t sock =
    qed_hs_open_socket_nonblocking(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  if (! SOCKET_OK(sock)) {
  ...
  qed_hs_release_socket_ownership(sock);
}

void
qed_hs_tls_impl_free_(qed_hs_tls_impl_t *tls)
{
...
qed_hs_tls_read, (qed_hs_tls_t *tls, char *cp, size_t len))
{
  ...
  if (err == PR_WOULD_BLOCK_ERROR) {
    return QED_HS_TLS_WANTREAD; // XXXX ????
  } else {
...
qed_hs_tls_write(qed_hs_tls_t *tls, const char *cp, size_t n)
{
  ...
  if (err == PR_WOULD_BLOCK_ERROR) {
    return QED_HS_TLS_WANTWRITE; // XXXX ????
  } else {
...
qed_hs_tls_handshake(qed_hs_tls_t *tls)
{
  ...
  if (PORT_GetError() == PR_WOULD_BLOCK_ERROR)
    return QED_HS_TLS_WANTREAD; /* XXXX What about wantwrite? */

  return QED_HS_TLS_ERROR_MISC; // XXXX
}
...
void
qed_hs_tls_get_n_raw_bytes(qed_hs_tls_t *tls,
                        size_t *n_read, size_t *n_written)
{
  qed_hs_assert(tls);
  qed_hs_assert(n_read);
  qed_hs_assert(n_written);
  uint64_t r, w;
  if (qed_hs_get_prfiledesc_byte_counts(tls->ssl, &r, &w) < 0) {
    *n_read = *n_written = 0;
    return;
  }
...
{
  qed_hs_assert(tls);
  qed_hs_assert(rbuf_capacity);
  qed_hs_assert(rbuf_bytes);
  qed_hs_assert(wbuf_capacity);
  qed_hs_assert(wbuf_bytes);
...
MOCK_IMPL(int,
qed_hs_tls_cert_matches_key,(const qed_hs_tls_t *tls,
                          const struct qed_hs_x509_cert_t *cert))
{
  qed_hs_assert(cert);
  qed_hs_assert(cert->cert);

  int rv = 0;

  qed_hs_x509_cert_t *peercert = qed_hs_tls_get_peer_cert((qed_hs_tls_t *)tls);
  ...
  qed_hs_x509_cert_free(peercert);
...
{
  qed_hs_assert(tls);
  qed_hs_assert(secrets_out);
  qed_hs_assert(context);
  qed_hs_assert(label);
  qed_hs_assert(strlen(label) <= UINT_MAX);
  qed_hs_assert(context_len <= UINT_MAX);
...
#define SEC_OID_QED_HS_DEFAULT_ECDHE_GROUP SEC_OID_ANSIX962_EC_PRIME256V1
...
evaluate_ecgroup_for_tls(const char *ecgroup)
{
  ...
  if (!ecgroup)
    tag = SEC_OID_QED_HS_DEFAULT_ECDHE_GROUP;
