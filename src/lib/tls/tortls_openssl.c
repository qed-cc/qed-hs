 ...
 * are prefixed with qed_hs_ in order to avoid conflicting with OpenSSL
 ...
 * corresponding qed_hs_tls_t object. */
STATIC int qed_hs_tls_object_ex_data_index = -1;

/** Helper: Allocate qed_hs_tls_object_ex_data_index. */
void
qed_hs_tls_allocate_qed_hs_tls_object_ex_data_index(void)
{
  if (qed_hs_tls_object_ex_data_index == -1) {
    qed_hs_tls_object_ex_data_index =
      SSL_get_ex_new_index(0, NULL, NULL, NULL, NULL);
    qed_hs_assert(qed_hs_tls_object_ex_data_index != -1);
  }
}

/** Helper: given a SSL* pointer, return the qed_hs_tls_t object using that
 * pointer. */
qed_hs_tls_t *
qed_hs_tls_get_by_ssl(const SSL *ssl)
{
  qed_hs_tls_t *result = SSL_get_ex_data(ssl, qed_hs_tls_object_ex_data_index);
  if (result)
    qed_hs_assert(result->magic == QED_HS_TLS_MAGIC);
  return result;
}

/** True iff qed_hs_tls_init() has been called. */
static int tls_library_is_initialized = 0;

/* Module-internal error codes. */
#define QED_HS_TLS_SYSCALL_    (MIN_QED_HS_TLS_ERROR_VAL_ - 2)
#define QED_HS_TLS_ZERORETURN_ (MIN_QED_HS_TLS_ERROR_VAL_ - 1)

/** Write a description of the current state of <b>tls</b> into the
 * <b>sz</b>-byte buffer at <b>buf</b>. */
void
qed_hs_tls_get_state_description(qed_hs_tls_t *tls, char *buf, size_t sz)
{
  ...
  switch (tls->state) {
#define CASE(st) case QED_HS_TLS_ST_##st: tortls_state = " in "#st ; break
    CASE(HANDSHAKE);
    CASE(OPEN);
    CASE(GOTCLOSE);
    CASE(SENTCLOSE);
    ...
    CASE(RENEGOTIATE);
#undef CASE
  case QED_HS_TLS_ST_BUFFEREVENT:
    tortls_state = "";
    break;
  default:
  ...
  qed_hs_snprintf(buf, sz, "%s%s", ssl_state, tortls_state);
}
...
void
qed_hs_tls_log_one_error(qed_hs_tls_t *tls, unsigned long err,
                  int severity, int domain, const char *doing)
{
  ...
  if (!func) func = "(null)";
  if (doing) {
    qed_hs_log(severity, domain, "TLS error while %s%s%s: %s (in %s:%s:%s)",
        doing, addr?" with ":"", addr?addr:"",
        msg, lib, func, state);
  } else {
    qed_hs_log(severity, domain, "TLS error%s%s: %s (in %s:%s:%s)",
        addr?" with ":"", addr?addr:"",
        msg, lib, func, state);
  }
}
 ...
 */
void
tls_log_errors(qed_hs_tls_t *tls, int severity, int domain, const char *doing)
{
  unsigned long err;
  ...
  while ((err = ERR_get_error()) != 0) {
    if (tls)
      tls->last_error = err;
    qed_hs_tls_log_one_error(tls, err, severity, domain, doing);
  }
}
 ...
 **/
const char *
qed_hs_tls_get_last_error_msg(const qed_hs_tls_t *tls)
{
  IF_BUG_ONCE(!tls) {
    return NULL;
  }
 ...
 * which one.  Return one of QED_HS_TLS_{DONE|WANTREAD|WANTWRITE|ERROR}.
 * If extra&CATCH_SYSCALL is true, return QED_HS_TLS_SYSCALL_ instead of
 * reporting syscall errors.  If extra&CATCH_ZERO is true, return
 * QED_HS_TLS_ZERORETURN_ instead of reporting zero-return errors.
 *
 * If an error has occurred, log it at level <b>severity</b> and describe the
 * current action as <b>doing</b>.
 */
int
qed_hs_tls_get_error(qed_hs_tls_t *tls, int r, int extra,
                  const char *doing, int severity, int domain)
{
  int err = SSL_get_error(tls->ssl, r);
  int qed_hs_error = QED_HS_TLS_ERROR_MISC;
  switch (err) {
    case SSL_ERROR_NONE:
      return QED_HS_TLS_DONE;
    case SSL_ERROR_WANT_READ:
      return QED_HS_TLS_WANTREAD;
    case SSL_ERROR_WANT_WRITE:
      return QED_HS_TLS_WANTWRITE;
    case SSL_ERROR_SYSCALL:
      if (extra&CATCH_SYSCALL)
        return QED_HS_TLS_SYSCALL_;
      if (r == 0) {
        qed_hs_log(severity, LD_NET, "TLS error: unexpected close while %s (%s)",
            doing, SSL_state_string_long(tls->ssl));
        qed_hs_error = QED_HS_TLS_ERROR_IO;
      } else {
        int e = qed_hs_socket_errno(tls->socket);
        qed_hs_log(severity, LD_NET,
            "TLS error: <syscall error while %s> (errno=%d: %s; state=%s)",
            doing, e, qed_hs_socket_strerror(e),
            SSL_state_string_long(tls->ssl));
        qed_hs_error = qed_hs_errno_to_tls_error(e);
      }
      tls_log_errors(tls, severity, domain, doing);
      return qed_hs_error;
    case SSL_ERROR_ZERO_RETURN:
      if (extra&CATCH_ZERO)
        return QED_HS_TLS_ZERORETURN_;
      qed_hs_log(severity, LD_NET, "TLS connection closed while %s in state %s",
          doing, SSL_state_string_long(tls->ssl));
      tls_log_errors(tls, severity, domain, doing);
      return QED_HS_TLS_CLOSE;
    default:
      tls_log_errors(tls, severity, domain, doing);
 ...
 */
MOCK_IMPL(int,
qed_hs_tls_cert_matches_key,(const qed_hs_tls_t *tls, const qed_hs_x509_cert_t *cert))
{
  qed_hs_x509_cert_t *peer = qed_hs_tls_get_peer_cert((qed_hs_tls_t *)tls);
  if (!peer)
    return 0;
...
void
qed_hs_tls_context_impl_free_(struct ssl_ctx_st *ctx)
{
  if (!ctx)
    return;
  SSL_CTX_free(ctx);
...
#define  NID_qed_hs_default_ecdhe_group NID_X9_62_prime256v1
...
qed_hs_tls_context_new(crypto_pk_t *identity, unsigned int key_lifetime,
                    unsigned flags, int is_client)
{
  EVP_PKEY *pkey = NULL;
  qed_hs_tls_context_t *result = NULL;

  qed_hs_tls_init();

  result = qed_hs_malloc_zero(sizeof(qed_hs_tls_context_t));
  result->refcnt = 1;

  if (! is_client) {
    if (qed_hs_tls_context_init_certificates(result, identity, key_lifetime,
                                          flags) < 0) {
      goto error;
    }
  }
  ...
  * historically been chosen for fingerprinting resistance. */
...

  {
    ...
    bool success = false;
    long our_version = qed_hs_OpenSSL_version_num();
 ...
 error:
  ...
  if (pkey)
    EVP_PKEY_free(pkey);
  qed_hs_tls_context_decref(result);
  return NULL;
}

/** Invoked when a TLS state changes: log the change at severity 'debug' */
void
qed_hs_tls_debug_state_callback(const SSL *ssl, int type, int val)
{
 ...
 */
qed_hs_tls_t *
qed_hs_tls_new(qed_hs_socket_t sock, int isServer)
{
  BIO *bio = NULL;
  qed_hs_tls_t *result = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));
  qed_hs_tls_context_t *context = qed_hs_tls_context_get(isServer);
  result->magic = QED_HS_TLS_MAGIC;

  check_no_tls_errors();
  qed_hs_assert(context); /* make sure somebody made it first */
  ...
  {
    int set_worked =
      SSL_set_ex_data(result->ssl, qed_hs_tls_object_ex_data_index, result);
    if (!set_worked) {
      log_warn(LD_BUG,
  ...
  qed_hs_tls_context_incref(context);
  result->context = context;
  result->state = QED_HS_TLS_ST_HANDSHAKE;
  ...
  SSL_set_info_callback(result->ssl, qed_hs_tls_debug_state_callback);
 ...
 err:
  ...
  tls_log_errors(NULL, LOG_WARN, LD_NET, "creating qed_hs_tls_t object");
  return result;
}
 ...
 */
void
qed_hs_tls_release_socket(qed_hs_tls_t *tls)
{
  if (! tls)
    return;
...
void
qed_hs_tls_impl_free_(qed_hs_tls_impl_t *ssl)
{
  if (!ssl)
    return;
 ...
 * number of characters read.  On failure, returns QED_HS_TLS_ERROR,
 * QED_HS_TLS_CLOSE, QED_HS_TLS_WANTREAD, or QED_HS_TLS_WANTWRITE.
 */
MOCK_IMPL(int,
qed_hs_tls_read,(qed_hs_tls_t *tls, char *cp, size_t len))
{
  int r, err;
  qed_hs_assert(tls);
  qed_hs_assert(tls->ssl);
  qed_hs_assert(tls->state == QED_HS_TLS_ST_OPEN);
  qed_hs_assert(len<INT_MAX);
  r = SSL_read(tls->ssl, cp, (int)len);
  if (r > 0) {
    return r;
  }
  err = qed_hs_tls_get_error(tls, r, CATCH_ZERO, "reading", LOG_DEBUG, LD_NET);
  if (err == QED_HS_TLS_ZERORETURN_ || err == QED_HS_TLS_CLOSE) {
    log_debug(LD_NET,"read returned r=%d; TLS is closed",r);
    tls->state = QED_HS_TLS_ST_CLOSED;
    return QED_HS_TLS_CLOSE;
  } else {
    qed_hs_assert(err != QED_HS_TLS_DONE);
 ...
 * number of characters written.  On failure, returns QED_HS_TLS_ERROR,
 * QED_HS_TLS_WANTREAD, or QED_HS_TLS_WANTWRITE.
 */
int
qed_hs_tls_write(qed_hs_tls_t *tls, const char *cp, size_t n)
{
  int r, err;
  qed_hs_assert(tls);
  qed_hs_assert(tls->ssl);
  qed_hs_assert(tls->state == QED_HS_TLS_ST_OPEN);
  qed_hs_assert(n < INT_MAX);
  if (n == 0)
    return 0;
  if (tls->wantwrite_n) {
    /* if WANTWRITE last time, we must use the _same_ n as before */
    qed_hs_assert(n >= tls->wantwrite_n);
  ...
  err = qed_hs_tls_get_error(tls, r, 0, "writing", LOG_INFO, LD_NET);
  if (err == QED_HS_TLS_DONE) {
    total_bytes_written_over_tls += r;
    return r;
  }
  if (err == QED_HS_TLS_WANTWRITE || err == QED_HS_TLS_WANTREAD) {
    tls->wantwrite_n = n;
  }
  return err;
}
 ...
 * QED_HS_TLS_DONE.  On failure, returns QED_HS_TLS_ERROR, QED_HS_TLS_WANTREAD,
 * or QED_HS_TLS_WANTWRITE.
 */
int
qed_hs_tls_handshake(qed_hs_tls_t *tls)
{
  int r;
  qed_hs_assert(tls);
  qed_hs_assert(tls->ssl);
  qed_hs_assert(tls->state == QED_HS_TLS_ST_HANDSHAKE);
  ...
  r = qed_hs_tls_get_error(tls,r,0, "handshaking", LOG_INFO, LD_HANDSHAKE);
  if (ERR_peek_error() != 0) {
    tls_log_errors(tls, tls->isServer ? LOG_INFO : LOG_WARN, LD_HANDSHAKE,
                   "handshaking");
    return QED_HS_TLS_ERROR_MISC;
  }
  if (r == QED_HS_TLS_DONE) {
 ...
 */
int
qed_hs_tls_peer_has_cert(qed_hs_tls_t *tls)
{
  X509 *cert;
 ...
 * isn't one. */
MOCK_IMPL(qed_hs_x509_cert_t *,
qed_hs_tls_get_peer_cert,(qed_hs_tls_t *tls))
{
  X509 *cert;
  cert = SSL_get_peer_certificate(tls->ssl);
  tls_log_errors(tls, LOG_WARN, LD_HANDSHAKE, "getting peer certificate");
...
qed_hs_tls_get_own_cert,(qed_hs_tls_t *tls))
{
  ...
  if (BUG(duplicate == NULL))
    return NULL;
  return qed_hs_x509_cert_new(duplicate);
}

/** Return the number of bytes available for reading from <b>tls</b>.
 */
int
qed_hs_tls_get_pending_bytes(qed_hs_tls_t *tls)
{
  qed_hs_assert(tls);
...
size_t
qed_hs_tls_get_forced_write_size(qed_hs_tls_t *tls)
{
...
void
qed_hs_tls_get_n_raw_bytes(qed_hs_tls_t *tls, size_t *n_read, size_t *n_written)
{
  BIO *wbio, *tmpbio;
  unsigned long r, w;
   ...
   * save the original BIO for  tls->ssl in the qed_hs_tls_t structure, but
  ...
  *n_written = (size_t)(w - tls->last_write_count);
  if (*n_read > INT_MAX || *n_written > INT_MAX) {
    log_warn(LD_BUG, "Preposterously large value in qed_hs_tls_get_n_raw_bytes. "
...
check_no_tls_errors_(const char *fname, int line)
{
  ...
  log_warn(LD_CRYPTO, "Unhandled OpenSSL errors found at %s:%d: ",
      qed_hs_fix_source_file(fname), line);
 ...
 */
MOCK_IMPL(int,
qed_hs_tls_export_key_material,(qed_hs_tls_t *tls, uint8_t *secrets_out,
...
int
qed_hs_tls_get_buffer_sizes(qed_hs_tls_t *tls,
...
evaluate_ecgroup_for_tls(const char *ecgroup)
{
  ...
  if (!ecgroup)
    nid = NID_qed_hs_default_ecdhe_group;
