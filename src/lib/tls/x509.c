...
void
qed_hs_tls_pick_certificate_lifetime(time_t now,
                                  unsigned int cert_lifetime,
                                  time_t *start_time_out,
                                  time_t *end_time_out)
{
  qed_hs_assert(cert_lifetime < INT_MAX);
...
qed_hs_x509_cert_get_id_digests(const qed_hs_x509_cert_t *cert)
{
...
const common_digests_t *
qed_hs_x509_cert_get_cert_digests(const qed_hs_x509_cert_t *cert)
{
...
void
qed_hs_x509_cert_free_(qed_hs_x509_cert_t *cert)
{
  if (! cert)
    return;
  qed_hs_x509_cert_impl_free(cert->cert);
...
/**
 * Allocate a new qed_hs_x509_cert_t to hold the certificate "x509_cert".
 *
 * Steals a reference to x509_cert.
 */
MOCK_IMPL(qed_hs_x509_cert_t *,
qed_hs_x509_cert_new,(qed_hs_x509_cert_impl_t *x509_cert))
{
  ...
  if (qed_hs_x509_cert_set_cached_der_encoding(cert) < 0)
    goto err;

  {
    const uint8_t *encoded=NULL;
    size_t encoded_len=0;
    qed_hs_x509_cert_get_der(cert, &encoded, &encoded_len);
    qed_hs_assert(encoded);
    crypto_common_digests(&cert->cert_digests, (char *)encoded, encoded_len);
  }

  {
    crypto_pk_t *pk = qed_hs_tls_cert_get_key(cert);
    if (pk) {
...
qed_hs_x509_cert_t *
qed_hs_x509_cert_dup(const qed_hs_x509_cert_t *cert)
{
  qed_hs_assert(cert);
  qed_hs_assert(cert->cert);
  return qed_hs_x509_cert_new(qed_hs_x509_cert_impl_dup_(cert->cert));
}
