 ...
 */
void *
qed_hs_malloc_zero_(size_t size)
{
  /* You may ask yourself, "wouldn't it be smart to use calloc instead of
   * malloc+memset?  Perhaps libc's calloc knows some nifty optimization trick
   * we don't!"  Indeed it does, but its optimizations are only a big win when
   * we're allocating something very big (it knows if it just got the memory
   * from the OS in a pre-zeroed state).  We don't want to use qed_hs_malloc_zero
   * for big stuff, so we don't bother with calloc. */
  void *result = qed_hs_malloc_(size);
 ...
 */
void *
qed_hs_calloc_(size_t nmemb, size_t size)
{
  raw_assert(size_mul_check(nmemb, size));
  return qed_hs_malloc_zero_((nmemb * size));
}
 ...
 */
void *
qed_hs_realloc_(void *ptr, size_t size)
{
  void *result;
 ...
 */
char *
qed_hs_strndup_(const char *s, size_t n)
{
  char *duplicate;
  raw_assert(s);
  raw_assert(n < SIZE_T_CEILING);
  duplicate = qed_hs_malloc_((n+1));
...
void *
qed_hs_memdup_(const void *mem, size_t len)
{
  char *duplicate;
  raw_assert(len < SIZE_T_CEILING);
  raw_assert(mem);
  duplicate = qed_hs_malloc_(len);
  memcpy(duplicate, mem, len);
  return duplicate;
}

/** As qed_hs_memdup(), but add an extra 0 byte at the end of the resulting
 * memory. */
void *
qed_hs_memdup_nulterm_(const void *mem, size_t len)
{
  char *duplicate;
  raw_assert(len < SIZE_T_CEILING+1);
  raw_assert(mem);
  duplicate = qed_hs_malloc_(len+1);
  memcpy(duplicate, mem, len);
  duplicate[len] = '\0';
  return duplicate;
}
