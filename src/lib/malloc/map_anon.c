...
nodump_mem(void *mem, size_t sz)
{
  ...
  if (rv == 0) {
    ...
    qed_hs_log_err_sigsafe("Unexpected error from madvise: ",
                        strerror(errno),
                        NULL);
    return -1;
  }
...
noinherit_mem(void *mem, size_t sz, inherit_res_t *inherit_result_out)
{
  ...
  if (errno == ENOSYS || errno == EINVAL) {
    ...
    qed_hs_log_err_sigsafe("Unexpected error from minherit: ",
                        strerror(errno),
                        NULL);
    return -1;
  }
 ...
 */
void *
qed_hs_mmap_anonymous(size_t sz, unsigned flags,
 ...
 * <b>mapping</b> by qed_hs_mmap_anonymous().
 **/
void
qed_hs_munmap_anonymous(void *mapping, size_t sz)
{
  if (!mapping)
    return;
