...
static int sigsafe_log_fds[QED_HS_SIGSAFE_LOG_MAX_FDS] = { STDERR_FILENO };
 ...
 * on failure. */
static int
qed_hs_log_err_sigsafe_write(const char *s)
{
  int i;
  ssize_t r;
  size_t len = strlen(s);
...
void
qed_hs_log_err_sigsafe(const char *m, ...)
{
  va_list ap;
  const char *x;
  char timebuf[33];
  ...
  qed_hs_log_err_sigsafe_write("\n=========================================="
                             "================== T=");
  qed_hs_log_err_sigsafe_write(timebuf);
  qed_hs_log_err_sigsafe_write("\n");
  qed_hs_log_err_sigsafe_write(m);
  va_start(ap, m);
  while ((x = va_arg(ap, const char*))) {
    qed_hs_log_err_sigsafe_write(x);
  }
  va_end(ap);
}
...
int
qed_hs_log_get_sigsafe_err_fds(const int **out)
{
  *out = sigsafe_log_fds;
 ...
 */
void
qed_hs_log_set_sigsafe_err_fds(const int *fds, int n)
{
  if (n > QED_HS_SIGSAFE_LOG_MAX_FDS) {
    n = QED_HS_SIGSAFE_LOG_MAX_FDS;
  }
 ...
 */
void
qed_hs_log_reset_sigsafe_err_fds(void)
{
  int fds[] = { STDERR_FILENO };
  qed_hs_log_set_sigsafe_err_fds(fds, 1);
}
 ...
 */
void
qed_hs_log_flush_sigsafe_err_fds(void)
{
  ...
  n_fds = qed_hs_log_get_sigsafe_err_fds(&fds);
  for (i = 0; i < n_fds; ++i) {
 ...
 */
void
qed_hs_log_sigsafe_err_set_granularity(int ms)
{
  log_granularity = ms;
}
 ...
 */
void
qed_hs_raw_assertion_failed_msg_(const char *file, int line, const char *expr,
                              const char *msg)
{
  char linebuf[16];
  format_dec_number_sigsafe(line, linebuf, sizeof(linebuf));
  qed_hs_log_err_sigsafe("INTERNAL ERROR: Raw assertion failed in ",
                      get_qed_hs_backtrace_version(), " at ",
                      file, ":", linebuf, ": ", expr, "\n", NULL);
  if (msg) {
    qed_hs_log_err_sigsafe_write(msg);
    qed_hs_log_err_sigsafe_write("\n");
  }
 ...
 **/
void
qed_hs_raw_abort_(void)
{
  qed_hs_log_flush_sigsafe_err_fds();
  abort();
}
                      ...
                      unsigned int radix)
{
  ...
  /* NOT qed_hs_assert. This needs to be safe to run from within a signal
   * handler, and from within the 'qed_hs_assert() has failed' code.  Not even
  ...
  *cp = '\0';
  ...
  /* NOT qed_hs_assert; see above. */
  if (cp != buf) {
    qed_hs_raw_abort_(); // LCOV_EXCL_LINE
  }
 ...
 *
 * For most purposes, you'd want to use qed_hs_snprintf("%x") instead of this
