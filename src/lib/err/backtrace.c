 ...
 * that with a backtrace log.  Send messages via the qed_hs_log function at
 * logger". */
void
log_backtrace_impl(int severity, log_domain_mask_t domain, const char *msg,
                   qed_hs_log_fn logger)
{
  size_t depth;
  char **symbols;
  size_t i;
...
crash_handler(int sig, siginfo_t *si, void *ctx_)
{
  ...
  qed_hs_log_err_sigsafe(bt_version, " died: Caught signal ", buf, "\n",
                      NULL);
...
const char *
get_qed_hs_backtrace_version(void)
{
  return bt_version;
}
...
int
configure_backtrace_handler(const char *qed_hs_version)
{
  ...
  if (qed_hs_version) {
    ...
    snp_rv = snprintf(version, sizeof(version), "Tor %s", qed_hs_version);
