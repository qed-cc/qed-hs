...
void
qed_hs_init_weak_random(qed_hs_weak_rng_t *rng, unsigned seed)
{
  rng->state = (uint32_t)(seed & 0x7fffffff);
}

/** Return a randomly chosen value in the range 0..QED_HS_WEAK_RANDOM_MAX based
 ...
 * predict. */
int32_t
qed_hs_weak_random(qed_hs_weak_rng_t *rng)
{
...
int32_t
qed_hs_weak_random_range(qed_hs_weak_rng_t *rng, int32_t top)
{
  /* We don't want to just do qed_hs_weak_random() % top, since random() is often
   * implemented with an LCG whose modulus is a power of 2, and those are
   * cyclic in their low-order bits. */
  int divisor, result;
  raw_assert(top > 0);
  divisor = QED_HS_WEAK_RANDOM_MAX / top;
  do {
    result = (int32_t)(qed_hs_weak_random(rng) / divisor);
  } while (result >= top);
  return result;
}
