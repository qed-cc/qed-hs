                      ...
                      char **errmsg, const var_type_def_t *def)
{
  ...
  if (rv < 0 && *errmsg != NULL) {
    ...
    char *oldmsg = *errmsg;
    qed_hs_asprintf(errmsg, "Could not parse %s: %s", line->key, oldmsg);
    qed_hs_free(oldmsg);
  }
  return rv;
}
                      ...
                      const var_type_def_t *def)
{
  ...
  config_line_t *result = qed_hs_malloc_zero(sizeof(config_line_t));
  result->key = qed_hs_strdup(key);
