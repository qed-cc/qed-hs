...
int_parse(void *target, const char *value, char **errmsg, const void *params)
{
  ...
  int *p = target;
  int ok=0;
  *p = (int)qed_hs_parse_long(value, 10, pp->minval, pp->maxval, &ok, NULL);
  if (!ok) {
    qed_hs_asprintf(errmsg, "Integer %s is malformed or out of bounds. "
             ...
             const void *params)
{
  ...
  uint64_t *p = target;
  int ok=0;
  *p = qed_hs_parse_uint64(value, 10, 0, UINT64_MAX, &ok, NULL);
  if (!ok) {
    qed_hs_asprintf(errmsg, "Integer %s is malformed or out of bounds.",
                 value);
    return -1;
...
uint64_encode(const void *value, const void *params)
{
  ...
  char *result;
  qed_hs_asprintf(&result, "%"PRIu64, v);
  return result;
}
                ...
                const void *params)
{
  ...
  *v = config_parse_units(value, table, &ok, &msg);
  if (!ok) {
    qed_hs_asprintf(errmsg, "Provided value is malformed or out of bounds: %s",
                 msg);
    qed_hs_free(msg);
    return -1;
  }
               ...
               const void *params)
{
  ...
  if (u64 > INT_MAX) {
    qed_hs_asprintf(errmsg, "Provided value %s is too large", value);
    return -1;
  }
  *v = (int) u64;
  return 0;
             ...
             const void *params)
{
  ...
  *v = strtod(value, &endptr);
  if (endptr == value || *endptr != '\0') {
    ...
    qed_hs_asprintf(errmsg, "Could not convert %s to a number.", escaped(value));
    return -1;
  }
  if (errno == ERANGE) {
    // strtod will set errno to ERANGE on underflow or overflow.
    bool underflow = -.00001 < *v && *v < .00001;
    qed_hs_asprintf(errmsg,
           ...
           const void *params_)
{
  ...
  qed_hs_asprintf(errmsg, "Unrecognized value %s. %s",
...
enum_encode(const void *value, const void *params_)
{
  ...
  for (; table->name; ++table) {
    if (v == table->value)
      return qed_hs_strdup(table->name);
  }
  return NULL; // error.
}
           ...
           const void *params)
{
  ...
  if (parse_iso_time(value, p) < 0) {
    qed_hs_asprintf(errmsg, "Invalid time %s", escaped(value));
    return -1;
  }
  return 0;
}
...
time_encode(const void *value, const void *params)
{
  ...
  char *result = qed_hs_malloc(ISO_TIME_LEN+1);
                          ...
                          const void *params)
{
  ...
  char *tmp = NULL;
  if (comma) {
    tmp = qed_hs_strndup(val, comma - val);
    val = tmp;
  }
                         ...
                         char **errmsg, const void *params)
{
  ...
  (void)line;
  (void)params;
  *errmsg = qed_hs_strdup("Cannot assign directly to virtual option.");
  return -1;
}
