...
{
  config_suite_t *suite = qed_hs_malloc_zero(sizeof(config_suite_t));
...
{
  config_mgr_t *mgr = qed_hs_malloc_zero(sizeof(config_mgr_t));
...
{
  qed_hs_assert(mgr);
  qed_hs_assert(toplevel);
  if (idx == IDX_TOPLEVEL)
    return toplevel;

  qed_hs_assertf(idx >= 0 && idx < smartlist_len(mgr->subconfigs),
              "Index %d is out of range.", idx);
  config_suite_t **suite = config_mgr_get_suite_ptr(mgr, toplevel);
  qed_hs_assert(suite);
  qed_hs_assert(smartlist_len(mgr->subconfigs) ==
                           ...
                           const void *options)
{
  ...
  qed_hs_assert(mgr->frozen);
  struct_check_magic(options, &mgr->toplevel_magic);

  config_suite_t **suitep = config_mgr_get_suite_ptr(mgr, (void*)options);
  if (suitep == NULL) {
    qed_hs_assert(smartlist_len(mgr->subconfigs) == 0);
    return;
  }

  qed_hs_assert(smartlist_len((*suitep)->configs) ==
                    ...
                    config_line_t *c, char **msg)
{
  ...
  qed_hs_assert(var);
  qed_hs_assert(!strcmp(c->key, var->cvar->member.name));
                   ...
                   bitarray_t *options_seen, char **msg)
{
  ...
  if (strcmp(cvar->member.name, c->key)) {
    qed_hs_free(c->key);
    c->key = qed_hs_strdup(cvar->member.name);
  }
             ...
             const managed_var_t *var, int use_defaults)
{
  ...
  if (var->cvar->initvalue) {
    c = qed_hs_malloc_zero(sizeof(config_line_t));
    c->key = qed_hs_strdup(var->cvar->member.name);
    c->value = qed_hs_strdup(var->cvar->initvalue);
    if (config_assign_value(mgr, options, c, &msg) < 0) {
      // LCOV_EXCL_START
      log_warn(LD_BUG, "Failed to assign default: %s", msg);
      qed_hs_free(msg); /* if this happens it's a bug */
      // LCOV_EXCL_STOP
    }
    config_free_lines(c);
  }
...
config_free_(const config_mgr_t *mgr, void *options)
{
  ...
  if (suitep) {
    SMARTLIST_FOREACH((*suitep)->configs, void *, obj, qed_hs_free(obj));
                   ...
                   const void *options1, const void *options2)
{
  ...
  SMARTLIST_FOREACH_BEGIN(mgr->all_vars, managed_var_t *, mv) {
    ...
    if (line) {
      ...
      *next = qed_hs_malloc_zero(sizeof(config_line_t));
      (*next)->key = qed_hs_strdup(varname);
    }
    while (*next)
      next = &(*next)->next;
  } SMARTLIST_FOREACH_END(mv);
