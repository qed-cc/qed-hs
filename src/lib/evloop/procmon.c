...
#ifdef PROCMON_POLLS
static void qed_hs_process_moniqed_hs_poll_cb(periodic_timer_t *ev,
                                        void *procmon_);
#endif
                        ...
                        const char **msg)
{
  ...
  pid_l = qed_hs_parse_long(process_spec, 10, 1, LONG_MAX, &pid_ok, &pspec_next);
...
/* DOCDOC qed_hs_process_moniqed_hs_t */
struct qed_hs_process_moniqed_hs_t {
...
int
qed_hs_validate_process_specifier(const char *process_spec,
                               const char **msg)
{
  struct parsed_process_specifier_t ppspec;

  qed_hs_assert(msg != NULL);
  *msg = NULL;
 ...
 * qed_hs_process_moniqed_hs_t on success; return NULL and store an error
 ...
 * <b>cb</b>(<b>cb_arg</b>).
 */
qed_hs_process_moniqed_hs_t *
qed_hs_process_moniqed_hs_new(struct event_base *base,
                        const char *process_spec,
                        log_domain_mask_t log_domain,
                        qed_hs_procmon_callback_t cb, void *cb_arg,
                        const char **msg)
{
  qed_hs_process_moniqed_hs_t *procmon = qed_hs_malloc_zero(
                                       sizeof(qed_hs_process_moniqed_hs_t));
  struct parsed_process_specifier_t ppspec;
  ...
  *msg = NULL;
  ...
  procmon->e = periodic_timer_new(base,
                                  &poll_interval_tv,
                                  qed_hs_process_moniqed_hs_poll_cb, procmon);
...
#error OOPS?
  ...
  return procmon;
 err:
  qed_hs_process_moniqed_hs_free(procmon);
  return NULL;
}
...
static void
qed_hs_process_moniqed_hs_poll_cb(periodic_timer_t *event, void *procmon_)
{
  (void)event;
  qed_hs_process_moniqed_hs_t *procmon = (qed_hs_process_moniqed_hs_t *)(procmon_);
  int its_dead_jim;

  qed_hs_assert(procmon != NULL);
  ...
  qed_hs_log(its_dead_jim ? LOG_NOTICE : LOG_INFO,
...
void
qed_hs_process_moniqed_hs_free_(qed_hs_process_moniqed_hs_t *procmon)
{
  if (procmon == NULL)
    return;
