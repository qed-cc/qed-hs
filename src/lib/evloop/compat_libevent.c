...
/* Wrapper for event_free() that tolerates qed_hs_event_free(NULL) */
void
qed_hs_event_free_(struct event *ev)
{
  if (ev == NULL)
    return;
  event_free(ev);
...
void
qed_hs_libevent_initialize(qed_hs_libevent_cfg_t *torcfg)
{
  qed_hs_assert(the_event_base == NULL);
  ...
  log_info(LD_GENERAL,
      ...
      event_get_version(), qed_hs_libevent_get_method());
}
 ...
 **/
bool
qed_hs_libevent_is_initialized(void)
{
...
const char *
qed_hs_libevent_get_method(void)
{
 ...
 * version of Libevent. */
const char *
qed_hs_libevent_get_version_str(void)
{
...
* at compilation time. */
const char *
qed_hs_libevent_get_header_version_str(void)
{
                   ...
                   void *data)
{
  ...
  qed_hs_assert(tv);
  qed_hs_assert(cb);
  timer = qed_hs_malloc_zero(sizeof(periodic_timer_t));
  if (!(timer->ev = qed_hs_event_new(base, -1, EV_PERSIST,
                        ...
                        void *userdata)
{
  ...
  mainloop_event_t *mev = qed_hs_malloc_zero(sizeof(mainloop_event_t));
  mev->ev = qed_hs_event_new(base, -1, 0,
                  postloop ? mainloop_event_postloop_cb : mainloop_event_cb,
                  mev);
  qed_hs_assert(mev->ev);
  mev->cb = cb;
...
mainloop_event_free_(mainloop_event_t *event)
{
  if (!event)
    return;
  qed_hs_event_free(event->ev);
  memset(event, 0xb8, sizeof(*event));
  qed_hs_free(event);
}

int
qed_hs_init_libevent_rng(void)
{
  int rv = 0;
  char buf[256];
 ...
 */
void
qed_hs_libevent_free_all(void)
{
  qed_hs_event_free(rescan_mainloop_ev);
  if (the_event_base)
 ...
 */
int
qed_hs_libevent_run_event_loop(struct event_base *base, int once)
{
...
void
qed_hs_libevent_exit_loop_after_delay(struct event_base *base,
 ...
 * active. */
void
qed_hs_libevent_exit_loop_after_callback(struct event_base *base)
{
  event_base_loopbreak(base);
}

#if defined(QED_HS_UNIT_TESTS)
/** For testing: called post-fork to make libevent reinitialize
 * kernel structures. */
void
qed_hs_libevent_postfork(void)
{
  int r = event_reinit(qed_hs_libevent_get_base());
  qed_hs_assert(r == 0);
}
