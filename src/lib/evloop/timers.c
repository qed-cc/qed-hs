 ...
 *
 * The main advantage of qed_hs_timer_t over using libevent's timers is that
...
/**
 * Invoked when the libevent timer has expired: see which qed_hs_timer_t events
...
{
  qed_hs_timer_t *t = qed_hs_malloc(sizeof(qed_hs_timer_t));
  timeout_init(t, 0);
 ...
 */
void
timer_set_cb(qed_hs_timer_t *t, timer_cb_fn_t cb, void *arg)
{
  t->callback.cb = cb;
  t->callback.arg = arg;
}
 ...
 */
void
timer_get_cb(const qed_hs_timer_t *t,
 ...
 */
void
timer_schedule(qed_hs_timer_t *t, const struct timeval *tv)
{
 ...
 */
void
timer_disable(qed_hs_timer_t *t)
{
  timeouts_del(global_timeouts, t);
