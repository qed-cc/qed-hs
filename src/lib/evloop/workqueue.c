...
/* copyright (c) 2013-2024, The QED Project, Inc. */
...
QED_HS_TAILQ_HEAD(work_tailq_t, workqueue_entry_t);
...
struct workqueue_entry_t {
   ...
   * reply queue. */
  QED_HS_TAILQ_ENTRY(workqueue_entry_t) next_work;
...
struct replyqueue_t {
  ...
  QED_HS_TAILQ_HEAD(, workqueue_entry_t) answers;
...
workqueue_entry_new(workqueue_reply_t (*fn)(void*, void*),
                    ...
                    void *arg)
{
  workqueue_entry_t *ent = qed_hs_malloc_zero(sizeof(workqueue_entry_t));
  ent->fn = fn;
...
workqueue_entry_cancel(workqueue_entry_t *ent)
{
  int cancelled = 0;
  void *result = NULL;
  qed_hs_mutex_acquire(&ent->on_pool->lock);
  workqueue_priority_t prio = ent->priority;
  if (ent->pending) {
    QED_HS_TAILQ_REMOVE(&ent->on_pool->work[prio], ent, next_work);
    cancelled = 1;
    result = ent->arg;
  }
  qed_hs_mutex_release(&ent->on_pool->lock);
...
worker_thread_has_work(workerthread_t *thread)
{
  ...
  for (i = WORKQUEUE_PRIORITY_FIRST; i <= WORKQUEUE_PRIORITY_LAST; ++i) {
    if (!QED_HS_TAILQ_EMPTY(&thread->in_pool->work[i]))
        return 1;
  }
...
worker_thread_extract_next_work(workerthread_t *thread)
{
  ...
  for (i = WORKQUEUE_PRIORITY_FIRST; i <= WORKQUEUE_PRIORITY_LAST; ++i) {
    ...
    if (!QED_HS_TAILQ_EMPTY(this_queue)) {
      queue = this_queue;
  ...
  workqueue_entry_t *work = QED_HS_TAILQ_FIRST(queue);
  QED_HS_TAILQ_REMOVE(queue, work, next_work);
  work->pending = 0;
  return work;
}
  ...
  while (1) {
    ...
    while (worker_thread_has_work(thread)) {
      ...
      if (thread->in_pool->generation != thread->generation) {
        ...
        qed_hs_mutex_release(&pool->lock);
    ...
    if (qed_hs_cond_wait(&pool->condition, &pool->lock, NULL) < 0) {
      log_warn(LD_GENERAL, "Fail qed_hs_cond_wait.");
    }
  }
...
exit:
  ...
  log_debug(LD_GENERAL, "Worker thread %u/%u has exited [TID: %lu].",
            ...
            pool->n_threads_max, qed_hs_get_thread_id());
...
{
  int was_empty;
  qed_hs_mutex_acquire(&queue->lock);
  was_empty = QED_HS_TAILQ_EMPTY(&queue->answers);
  QED_HS_TAILQ_INSERT_TAIL(&queue->answers, work, next_work);
  qed_hs_mutex_release(&queue->lock);
...
{
  workerthread_t *thr = qed_hs_malloc_zero(sizeof(workerthread_t));
  thr->state = state;
...
threadpool_queue_work_priority(threadpool_t *pool,
                               ...
                               void *arg)
{
  qed_hs_assert(((int)prio) >= WORKQUEUE_PRIORITY_FIRST &&
  ...
  QED_HS_TAILQ_INSERT_TAIL(&pool->work[prio], ent, next_work);
                         ...
                         void *arg)
{
  ...
  new_args = qed_hs_calloc(n_threads, sizeof(void*));
  ...
  qed_hs_cond_signal_all(&pool->condition);
...
threadpool_start_threads(threadpool_t *pool, int n)
{
  ...
  if (pool->n_threads < n)
    pool->threads = qed_hs_reallocarray(pool->threads,
   ...
   * Wait max 30 seconds */
  status = qed_hs_cond_wait(&pool->condition, &pool->control_lock, &tv);
...
{
  threadpool_t *pool;
  pool = qed_hs_malloc_zero(sizeof(threadpool_t));
  qed_hs_mutex_init_nonrecursive(&pool->lock);
  qed_hs_cond_init(&pool->condition);
  qed_hs_mutex_init_nonrecursive(&pool->control_lock);
  pool->exit = 0;

  unsigned i;
  for (i = WORKQUEUE_PRIORITY_FIRST; i <= WORKQUEUE_PRIORITY_LAST; ++i) {
    QED_HS_TAILQ_INIT(&pool->work[i]);
  }
...
threadpool_free_(threadpool_t *pool)
{
  ...
  qed_hs_cond_uninit(&pool->condition);
  qed_hs_mutex_uninit(&pool->lock);
  qed_hs_mutex_uninit(&pool->control_lock);

  if (pool->threads) {
    for (int i = 0; i != pool->n_threads; ++i)
      workerthread_free(pool->threads[i]);

    qed_hs_free(pool->threads);
  }
  ...
  if (pool->reply_event) {
    if (qed_hs_event_del(pool->reply_event) == -1)
      log_warn(LD_GENERAL, "libevent error: deleting reply event failed.");
    else
      qed_hs_event_free(pool->reply_event);
  }
...
replyqueue_new(uint32_t alertsocks_flags)
{
  ...
  rq = qed_hs_malloc_zero(sizeof(replyqueue_t));
  ...
  qed_hs_mutex_init(&rq->lock);
  QED_HS_TAILQ_INIT(&rq->answers);
...
replyqueue_free_(replyqueue_t *queue)
{
  ...
  while (!QED_HS_TAILQ_EMPTY(&queue->answers)) {
    work = QED_HS_TAILQ_FIRST(&queue->answers);
    QED_HS_TAILQ_REMOVE(&queue->answers, work, next_work);
...
{
  struct event_base *base = qed_hs_libevent_get_base();

  if (tp->reply_event) {
    qed_hs_event_free(tp->reply_event);
  }
  tp->reply_event = qed_hs_event_new(base,
                                  tp->reply_queue->alert.read_fd,
                                  EV_READ|EV_PERSIST,
                                  reply_event_cb,
                                  tp);
  qed_hs_assert(tp->reply_event);
  tp->reply_cb = cb;
...
replyqueue_process(replyqueue_t *queue)
{
  ...
  while (!QED_HS_TAILQ_EMPTY(&queue->answers)) {
    /* lock must be held at this point.*/
    workqueue_entry_t *work = QED_HS_TAILQ_FIRST(&queue->answers);
    QED_HS_TAILQ_REMOVE(&queue->answers, work, next_work);
    qed_hs_mutex_release(&queue->lock);
    work->on_pool = NULL;
