...
buf_chunk_free_unchecked(chunk_t *chunk)
{
  ...
  qed_hs_assert(CHUNK_ALLOC_SIZE(chunk->memlen) == chunk->DBG_alloc);
#endif
  qed_hs_assert(total_bytes_allocated_in_chunks >=
...
chunk_grow(chunk_t *chunk, size_t sz)
{
  ...
  qed_hs_assert(sz > chunk->memlen);
  offset = chunk->data - chunk->mem;
  chunk = qed_hs_realloc(chunk, new_alloc);
  chunk->memlen = sz;
  chunk->data = chunk->mem + offset;
#ifdef DEBUG_CHUNK_ALLOC
  qed_hs_assert(chunk->DBG_alloc == orig_alloc);
...
{
  qed_hs_assert(target <= SIZE_T_CEILING - CHUNK_OVERHEAD);
...
buf_pullup(buf_t *buf, size_t bytes, const char **head_out, size_t *len_out)
{
  ...
  if (buf->head->memlen >= capacity) {
    ...
    qed_hs_assert(CHUNK_REMAINING_CAPACITY(buf->head) >= needed);
  } else {
    chunk_t *newhead;
    size_t newsize;
    ...
    qed_hs_assert(newhead->memlen >= capacity);
    if (newhead != buf->head) {
  ...
  while (dest->datalen < bytes) {
    ...
    if (n >= src->datalen) {
      ...
      src->data += n;
      src->datalen -= n;
      qed_hs_assert(dest->datalen == bytes);
    }
  }
...
buf_new_with_data(const char *cp, size_t sz)
{
  ...
  qed_hs_assert(sz < SSIZE_T_CEILING);

  /* Allocate a buffer */
  buf_t *buf = buf_new_with_capacity(sz);
  qed_hs_assert(buf);
  buf_assert_ok(buf);
  qed_hs_assert(!buf->head);
  ...
  qed_hs_assert(buf->head);
  buf_assert_ok(buf);
  qed_hs_assert(buf_allocation(buf) >= sz);

  /* Copy the data and size the buffers */
  qed_hs_assert(sz <= buf_slack(buf));
  qed_hs_assert(sz <= CHUNK_REMAINING_CAPACITY(buf->head));
  ...
  qed_hs_assert(buf_allocation(buf) >= sz);
  qed_hs_assert(buf_allocation(buf) >= buf_datalen(buf) + buf_slack(buf));
  ...
  qed_hs_assert(buf_datalen(buf) == sz);
  qed_hs_assert(buf_slack(buf) == 0);
...
buf_new(void)
{
  buf_t *buf = qed_hs_malloc_zero(sizeof(buf_t));
...
{
  chunk_t *newch = qed_hs_memdup(in_chunk, CHUNK_ALLOC_SIZE(in_chunk->memlen));
...
buf_add(buf_t *buf, const char *string, size_t string_len)
{
  ...
  check();
  qed_hs_assert(buf->datalen <= BUF_MAX_LEN);
  return (int)buf->datalen;
}
...
/** As qed_hs_snprintf, but write the results into a buf_t */
void
...
/** As qed_hs_vsnprintf, but write the results into a buf_t. */
void
buf_add_vprintf(buf_t *buf, const char *format, va_list args)
{
  ...
  char *tmp;
  qed_hs_vasprintf(&tmp, format, args);
  qed_hs_assert(tmp != NULL);
...
buf_peek(const buf_t *buf, char *string, size_t string_len)
{
  ...
  qed_hs_assert(string_len <= buf->datalen);
  /* buf_assert_ok(buf); */
...
buf_find_pos_of_char(char ch, buf_pos_t *out)
{
  ...
  if (out->chunk) {
    if (out->chunk->datalen) {
      qed_hs_assert(out->pos < (ptrdiff_t)out->chunk->datalen);
    } else {
      qed_hs_assert(out->pos == 0);
    }
  }
  pos = out->pos;
  for (chunk = out->chunk; chunk; chunk = chunk->next) {
    ...
    if (cp) {
      out->chunk = chunk;
      qed_hs_assert(cp - chunk->data <= BUF_MAX_LEN);
...
{
  qed_hs_assert(pos->pos < BUF_MAX_LEN);
  ++pos->pos;
...
buf_find_string_offset(const buf_t *buf, const char *s, size_t n)
{
  ...
  while (buf_find_pos_of_char(*s, &pos) >= 0) {
    if (buf_matches_at_pos(&pos, s, n)) {
      qed_hs_assert(pos.chunk_pos + pos.pos <= BUF_MAX_LEN);
...
{
  qed_hs_assert(buf);
  qed_hs_assert(buf->magic == BUFFER_MAGIC);

  if (! buf->head) {
    qed_hs_assert(!buf->tail);
    qed_hs_assert(buf->datalen == 0);
  } else {
    chunk_t *ch;
    size_t total = 0;
    qed_hs_assert(buf->tail);
    for (ch = buf->head; ch; ch = ch->next) {
      total += ch->datalen;
      qed_hs_assert(ch->datalen <= ch->memlen);
      qed_hs_assert(ch->datalen <= BUF_MAX_LEN);
      qed_hs_assert(ch->data >= &ch->mem[0]);
      qed_hs_assert(ch->data <= &ch->mem[0]+ch->memlen);
      ...
      qed_hs_assert(ch->data+ch->datalen <= &ch->mem[0] + ch->memlen);
      if (!ch->next)
        qed_hs_assert(ch == buf->tail);
    }
    qed_hs_assert(buf->datalen == total);
  }
}
