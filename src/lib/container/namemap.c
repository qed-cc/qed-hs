...
HT_GENERATE2(namemap_ht, mapped_name_t, node, mapped_name_hash,
             mapped_name_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
...
namemap_fmt_name(const namemap_t *map, unsigned id)
{
  ...
  qed_hs_snprintf(buf, sizeof(buf), "{%u}", id);
                         ...
                         size_t namelen)
{
  ...
  if (found) {
    qed_hs_assert(map->names);
    qed_hs_assert(smartlist_get(map->names, found->intval) == found);
    return found->intval;
  }
                         ...
                         const char *name)
{
  ...
  mapped_name_t *insert = qed_hs_malloc_zero(
