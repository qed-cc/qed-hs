...
/** Append the string produced by qed_hs_asprintf(<b>pattern</b>, <b>...</b>)
 * to <b>sl</b>. */
void
                        ...
                        va_list args)
{
  ...
  qed_hs_vasprintf(&str, pattern, args);
...
smartlist_string_remove(smartlist_t *sl, const char *element)
{
  int i;
  qed_hs_assert(sl);
  qed_hs_assert(element);
  for (i = 0; i < sl->num_used; ++i) {
    if (!strcmp(element, sl->list[i])) {
      qed_hs_free(sl->list[i]);
...
smartlist_contains_int_as_string(const smartlist_t *sl, int num)
{
  ...
  qed_hs_snprintf(buf,sizeof(buf),"%d", num);
...
/** Return true iff <b>sl</b> has some element E such that
 * qed_hs_memeq(E,<b>element</b>,DIGEST_LEN)
 */
int
smartlist_contains_digest(const smartlist_t *sl, const char *element)
{
  ...
  if (!sl) return 0;
  for (i=0; i < sl->num_used; i++)
    if (qed_hs_memeq((const char*)sl->list[i],element,DIGEST_LEN))
      return 1;
  return 0;
}
                      ...
                      int *found_out)
{
  ...
  qed_hs_assert(sl);
  qed_hs_assert(compare);
  qed_hs_assert(found_out);
   ...
   */
  qed_hs_assert(lo == hi + 1); /* All other cases should have been handled */
  qed_hs_assert(lo >= 0);
  qed_hs_assert(lo <= len);
  qed_hs_assert(hi >= 0);
  qed_hs_assert(hi <= len);
...
/** Remove duplicate strings from a sorted list, and free them with qed_hs_free().
 */
void
smartlist_uniq_strings(smartlist_t *sl)
{
  smartlist_uniq(sl, compare_string_ptrs_, qed_hs_free_);
}
                        ...
                        void *item)
{
  ...
  qed_hs_assert(idx >= 0);
  qed_hs_assert(sl->list[idx] == item);
  --sl->num_used;
  *IDXP(item) = -1;
  if (idx == sl->num_used) {
    sl->list[sl->num_used] = NULL;
                           ...
                           ptrdiff_t idx_field_offset)
{
  ...
  for (i = sl->num_used - 1; i >= 0; --i) {
    if (i>0)
      qed_hs_assert(compare(sl->list[PARENT(i)], sl->list[i]) <= 0);
    qed_hs_assert(IDX_OF_ITEM(sl->list[i]) == i);
  }
}
...
{
  return qed_hs_memcmp((const char*)*_a, (const char*)*_b, DIGEST_LEN);
}
...
/** Remove duplicate digests from a sorted list, and free them with qed_hs_free().
 */
void
smartlist_uniq_digests(smartlist_t *sl)
{
  smartlist_uniq(sl, compare_digests_, qed_hs_free_);
}
...
{
  return qed_hs_memcmp((const char*)*_a, (const char*)*_b, DIGEST256_LEN);
}
...
{
  smartlist_uniq(sl, compare_digests256_, qed_hs_free_);
}
