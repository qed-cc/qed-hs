...
{
  return qed_hs_memeq(a->key, b->key, DIGEST_LEN);
}
...
{
  return qed_hs_memeq(a->key, b->key, DIGEST256_LEN);
}
...
HT_GENERATE2(strmap_impl, strmap_entry_t, node, strmap_entry_hash,
             strmap_entries_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);

HT_PROTOTYPE(digestmap_impl, digestmap_entry_t, node, digestmap_entry_hash,
             digestmap_entries_eq);
HT_GENERATE2(digestmap_impl, digestmap_entry_t, node, digestmap_entry_hash,
             digestmap_entries_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
...
HT_GENERATE2(digest256map_impl, digest256map_entry_t, node,
             digest256map_entry_hash,
             digest256map_entries_eq, 0.6, qed_hs_reallocarray_, qed_hs_free_);
...
#define IMPLEMENT_MAP_FNS(maptype, keytype, prefix)                     \
  ...
  {                                                                     \
    maptype *result;                                                    \
    result = qed_hs_malloc(sizeof(maptype));                               \
    ...
    HT_FIND_OR_INSERT_(prefix##_impl, node, prefix##_entry_hash,        \
                       ...
                       {                                                \
                         ...
                         prefix##_entry_t *newent =                     \
                           qed_hs_malloc_zero(sizeof(prefix##_entry_t));   \
  ...
  {                                                                     \
    qed_hs_assert(!prefix##_impl_HT_REP_IS_BAD_(&map->head));              \
  }                                                                     \
                                                                        \
  ...
  {                                                                     \
    ...
    qed_hs_assert(HT_EMPTY(&map->head));                                   \
   ...
   *       free(val);                                                   \
   *    } else {                                                        \
   *       for (;*cp;cp++) *cp = QED_HS_TOUPPER(*cp);                      \
   */                                                                   \
  prefix##_iter_t *                                                     \
...
strmap_set_lc(strmap_t *map, const char *key, void *val)
{
  ...
  void *v;
  char *lc_key = qed_hs_strdup(key);
  qed_hs_strlower(lc_key);
