...
#ifndef QED_HS_HANDLE_H
#define QED_HS_HANDLE_H
...
#define HANDLE_IMPL(name, structname, linkage)                          \
  ...
  {                                                                     \
    qed_hs_assert(object);                                                 \
    name ## _handle_head_t *head = object->handle_head;                 \
    if (PREDICT_UNLIKELY(head == NULL)) {                               \
      head = object->handle_head = qed_hs_malloc_zero(sizeof(*head));      \
      head->object = object;                                            \
    }                                                                   \
    name ## _handle_t *new_ref = qed_hs_malloc_zero(sizeof(*new_ref));     \
    new_ref->head = head;                                               \
    ++head->references;                                                 \
    return new_ref;                                                     \
  }                                                                     \
