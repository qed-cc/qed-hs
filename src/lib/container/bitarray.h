...
#ifndef QED_HS_BITARRAY_H
#define QED_HS_BITARRAY_H
...
bitarray_init_zero(unsigned int n_bits)
{
  ...
  return qed_hs_calloc(sz, sizeof(unsigned int));
}
                ...
                unsigned int n_bits_old, unsigned int n_bits_new)
{
  ...
  if (sz_new <= sz_old)
    return ba;
  ptr = qed_hs_reallocarray(ba, sz_new, sizeof(unsigned int));
...
#endif /* !defined(QED_HS_BITARRAY_H) */
