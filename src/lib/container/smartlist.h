 ...
 *                    qed_hs_memcmp(rs->identity_digest, ri->identity_digest, 20),
 ...
 *      rs_ri_cmp = qed_hs_memcmp(rs->identity_digest, ri->identity_digest, 20);
 *      if (rs_ri_cmp > 0) {
 *        break;
 *      } else if (rs_ri_cmp == 0) {
 *        goto matched_ri;
...
#endif /* !defined(QED_HS_SMARTLIST_H) */
