 ...
 **/
void
crypto_seed_weak_rng(qed_hs_weak_rng_t *rng)
{
  unsigned seed;
  crypto_rand((void*)&seed, sizeof(seed));
  qed_hs_init_weak_random(rng, seed);
}
...
{
  qed_hs_assert(out_len <= MAX_STRONGEST_RAND_SIZE);
  ...
  if (PREDICT_LIKELY(getrandom_works)) {
    ...
    if (PREDICT_UNLIKELY(ret == -1)) {
      ...
      qed_hs_assert(errno != EAGAIN);
      qed_hs_assert(errno != EINTR);
    ...
    qed_hs_assert(ret == (long)out_len);
    return 0;
  }
...
crypto_rand_unmocked(char *to, size_t n)
{
  ...
  if (s != SECSuccess) {
     ...
     * state-size of SHA3. */
#define BUFLEN 512
    qed_hs_assert(PR_GetError() == SEC_ERROR_INVALID_ARGS && n > BUFLEN);
                       ...
                       const char *suffix)
{
  ...
  rand_bytes = qed_hs_malloc(rand_bytes_len);
