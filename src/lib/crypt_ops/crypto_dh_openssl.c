...
#ifndef ENABLE_NSS
static int qed_hs_check_dh_key(int severity, const BIGNUM *bn,
                            const BIGNUM *dh_p);
...
crypto_validate_dh_params(const BIGNUM *p, const BIGNUM *g)
{
  ...
  if (!DH_check(dh, &codes))
    goto out;
  if (BN_is_word(g, DH_GENERAQED_HS_2)) {
...
crypto_dh_init_openssl(void)
{
  ...
  qed_hs_assert(dh_param_g == NULL);
  qed_hs_assert(dh_param_p == NULL);
  qed_hs_assert(dh_param_p_tls == NULL);
...
#if 0
  qed_hs_assert(0 == crypto_validate_dh_params(dh_param_p, dh_param_g));
  qed_hs_assert(0 == crypto_validate_dh_params(dh_param_p_tls, dh_param_g));
#endif
}
...
{
  crypto_dh_t *res = qed_hs_malloc_zero(sizeof(crypto_dh_t));

  qed_hs_assert(dh_type == DH_TYPE_CIRCUIT || dh_type == DH_TYPE_TLS ||
  ...
  if (res->dh == NULL)
    qed_hs_free(res); // sets res to NULL.
  return res;
}
#endif /* !defined(ENABLE_NSS) */
...
{
  crypto_dh_t *dh_new = qed_hs_malloc_zero(sizeof(crypto_dh_t));
  qed_hs_assert(dh);
  qed_hs_assert(dh->dh);
  dh_new->dh = dh->dh;
  DH_up_ref(dh->dh);
...
crypto_dh_generate_public(crypto_dh_t *dh)
{
  ...
  if (qed_hs_check_dh_key(LOG_WARN, pub_key, DH_get0_p(dh->dh))<0) {
...
crypto_dh_get_public(crypto_dh_t *dh, char *pubkey, size_t pubkey_len)
{
  ...
  qed_hs_assert(dh_pub);
  bytes = BN_num_bytes(dh_pub);
  qed_hs_assert(bytes >= 0);
 ...
 */
static int
qed_hs_check_dh_key(int severity, const BIGNUM *bn, const BIGNUM *dh_p)
{
  BIGNUM *x;
  char *s;
  qed_hs_assert(bn);
                    ...
                    unsigned char *secret_out, size_t secret_bytes_out)
{
  ...
  qed_hs_assert(dh);
  qed_hs_assert(secret_bytes_out/DIGEST_LEN <= 255);
  qed_hs_assert(pubkey_len < INT_MAX);
  ...
  if (!(pubkey_bn = BN_bin2bn((const unsigned char*)pubkey,
                              (int)pubkey_len, NULL)))
    goto error;
  if (qed_hs_check_dh_key(severity, pubkey_bn, DH_get0_p(dh->dh))<0) {
