                                     ...
                                     size_t datalen)
{
  ...
  if (qed_hs_snprintf(header, sizeof(header),
                                      ...
                                      ssize_t data_out_len)
{
  ...
  *tag_out = qed_hs_strndup(prefix+5+strlen(typestring),
                            ...
                            const curve25519_public_key_t *pkey, bool pad)
{
   ...
   * base64_encode(). */
  qed_hs_assert(n == expected_len);
  qed_hs_assert(output[expected_len] == '\0');
}
                            ...
                            const ed25519_signature_t *sig)
{
  ...
  /* These asserts should always succeed, unless there is a bug in
   * base64_encode_nopad(). */
  qed_hs_assert(n == ED25519_SIG_BASE64_LEN);
  qed_hs_assert(buf[ED25519_SIG_BASE64_LEN] == '\0');
...
digest_to_base64(char *d64, const char *digest)
{
  ...
  /* These asserts should always succeed, unless there is a bug in
   * base64_encode_nopad(). */
  qed_hs_assert(n == BASE64_DIGEST_LEN);
  qed_hs_assert(buf[BASE64_DIGEST_LEN] == '\0');
...
digest256_to_base64(char *d64, const char *digest)
{
  ...
  /* These asserts should always succeed, unless there is a bug in
   * base64_encode_nopad(). */
  qed_hs_assert(n == BASE64_DIGEST256_LEN);
  qed_hs_assert(buf[BASE64_DIGEST256_LEN] == '\0');
