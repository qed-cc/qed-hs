              ...
              size_t secret_len, const char *s2k_specifier)
{
  ...
  qed_hs_assert(key_out_len < SIZE_T_CEILING);
                    ...
                    const char *secret, size_t secret_len)
{
  ...
  spec_len = secret_to_key_spec_len(type);
  key_len = secret_to_key_key_len(type);
  qed_hs_assert(spec_len > 0);
  qed_hs_assert(key_len > 0);
  qed_hs_assert(key_len <= (int) sizeof(buf));
  qed_hs_assert((int)spec_and_key_len == spec_len + key_len);
  ...
  if (qed_hs_memeq(buf, spec_and_key + spec_len, key_len))
    rv = S2K_OKAY;
  else
    rv = S2K_BAD_SECRET;
