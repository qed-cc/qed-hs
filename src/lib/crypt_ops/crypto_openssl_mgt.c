...
crypto_openssl_log_errors(int severity, const char *doing)
{
  ...
  while ((err = ERR_get_error()) != 0) {
    ...
    qed_hs_log(severity, LD_CRYPTO, "crypto error while %s: %s (in %s:%s)",
              doing, msg, lib, func);
  }
}
...
parse_openssl_version_str(const char *raw_version)
{
  ...
  if (end_of_version)
    return qed_hs_strndup(raw_version,
                      end_of_version-raw_version);
  else
    return qed_hs_strdup(raw_version);
}
...
{
  qed_hs_free(crypto_openssl_version_str);
  qed_hs_free(crypto_openssl_header_version_str);
}
...
crypto_openssl_early_init(void)
{
    ...
    unsigned long version_num = qed_hs_OpenSSL_version_num();
                         ...
                         const char *accelDir)
{
  ...
  if (qed_hs_OpenSSL_version_num() < OPENSSL_V_SERIES(3,0,0)) {
