...
crypto_new_pk_from_openssl_rsa_(RSA *rsa)
{
  crypto_pk_t *env;
  qed_hs_assert(rsa);
  env = qed_hs_malloc(sizeof(crypto_pk_t));
  env->refs = 1;
  env->key = rsa;
  return env;
}

/** Helper, used by qed-hs-gencert.c.  Return a copy of the private RSA from a
 * crypto_pk_t. */
RSA *
...
crypto_pk_generate_key_with_bits,(crypto_pk_t *env, int bits))
{
...

  {
    ...
    if (!e)
      goto done;
    if (! BN_set_word(e, QED_HS_RSA_EXPONENT))
      goto done;
    r = RSA_new();
    if (!r)
      goto done;
...
crypto_pk_public_exponent_ok(const crypto_pk_t *env)
{
  ...
  return BN_is_word(e, QED_HS_RSA_EXPONENT);
}
...
crypto_pk_cmp_keys(const crypto_pk_t *a, const crypto_pk_t *b)
{
  ...
  qed_hs_assert(a_n != NULL && a_e != NULL);
  qed_hs_assert(b_n != NULL && b_e != NULL);
                         ...
                         const char *from, size_t fromlen, int padding)
{
  ...
  qed_hs_assert(tolen >= crypto_pk_keysize(env));
