...
static unsigned char dh_generaqed_hs_data[1];
...
{
  crypto_dh_t *r = qed_hs_malloc_zero(sizeof(crypto_dh_t));
  r->dh_type = dh_type;
  return r;
}
                    ...
                    size_t secret_bytes_out)
{
  ...
  peer_key.u.dh.base.data = dh_generaqed_hs_data;
  ...
  qed_hs_assert(result); // This cannot fail.
