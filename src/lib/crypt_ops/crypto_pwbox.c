             ...
             unsigned s2k_flags)
{
  ...
  set_uint32(encrypted_portion, qed_hs_htonl((uint32_t)input_len));
  ...
  if (BUG(result_len < 0))
    goto err;
  result = qed_hs_malloc(result_len);
  enc_len = pwbox_encoded_encode(result, result_len, enc);
  if (BUG(enc_len < 0))
    goto err;
  qed_hs_assert(enc_len == result_len);
               ...
               const char *secret, size_t secret_len)
{
  ...
  if (qed_hs_memneq(hmac, enc->hmac, DIGEST256_LEN)) {
  ...
  result_len = qed_hs_ntohl(result_len);
  if (encrypted_len < result_len + 4)
    goto err;

  /* Allocate a buffer and decrypt */
  result = qed_hs_malloc_zero(result_len);
