                 ...
                 size_t *final_msg_len_out)
{
  ...
  qed_hs_assert(prefix_str);
  qed_hs_assert(final_msg_len_out);
  ...
  prefixed_msg = qed_hs_malloc_zero(prefixed_msg_len);
                        ...
                        int n_checkable))
{
  ...
  if (impl->open_batch == NULL) {
    ...
    ms = qed_hs_calloc(n_checkable, sizeof(uint8_t*));
    lens = qed_hs_calloc(n_checkable, sizeof(size_t));
    pks = qed_hs_calloc(n_checkable, sizeof(uint8_t*));
    sigs = qed_hs_calloc(n_checkable, sizeof(uint8_t*));
    oks = okay_out ? okay_out : qed_hs_calloc(n_checkable, sizeof(int));
     ...
     */
    qed_hs_assert(((res == 0) && !all_ok) || ((res < 0) && all_ok));
                                        ...
                                        const curve25519_keypair_t *inp)
{
  ...
  *signbit_out = out->pubkey.pubkey[31] >> 7;
  ...
  qed_hs_assert(fast_memeq(pubkey_check.pubkey, out->pubkey.pubkey, 32));
                  ...
                  const ed25519_public_key_t *key2)
{
  qed_hs_assert(key1);
  qed_hs_assert(key2);
  return qed_hs_memeq(key1->pubkey, key2->pubkey, ED25519_PUBKEY_LEN);
}
...
ed25519_point_is_identity_element(const uint8_t *point)
{
  ...
  static const uint8_t ed25519_identity[32] = {
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  qed_hs_assert(sizeof(ed25519_identity) == ED25519_PUBKEY_LEN);
  return qed_hs_memeq(point, ed25519_identity, sizeof(ed25519_identity));
}
