                    ...
                    const char *msg, size_t msg_len)
{
  if (alg == SEC_OID_UNKNOWN)
    return -1;
  qed_hs_assert(msg_len <= UINT_MAX);
                 ...
                 digest_algorithm_t algorithm)
{
  qed_hs_assert(m);
  qed_hs_assert(digest);
  qed_hs_assert(algorithm == DIGEST_SHA256 || algorithm == DIGEST_SHA3_256);
                 ...
                 digest_algorithm_t algorithm)
{
  qed_hs_assert(m);
  qed_hs_assert(digest);
  qed_hs_assert(algorithm == DIGEST_SHA512 || algorithm == DIGEST_SHA3_512);
...
#define END_OF_FIELD(f) (offsetof(crypto_digest_t, f) + \
  ...
  switch (alg) {
    ...
    default:
      qed_hs_assert(0); // LCOV_EXCL_LINE
...
{
  crypto_digest_t *r = qed_hs_malloc(crypto_digest_alloc_bytes(algorithm));
                        ...
                        size_t len)
{
  ...
  switch (digest->algorithm) {
    ...
    case DIGEST_SHA256: FALLTHROUGH;
    case DIGEST_SHA512:
      qed_hs_assert(len <= UINT_MAX);
      SECStatus s = PK11_DigestOp(digest->d.ctx,
                                  (const unsigned char *)data,
                                  (unsigned int)len);
      qed_hs_assert(s == SECSuccess);
      break;
    case DIGEST_SHA3_256: FALLTHROUGH;
    case DIGEST_SHA3_512:
                         ...
                         char *out, size_t out_len)
{
  ...
  qed_hs_assert(digest);
  qed_hs_assert(out);
  qed_hs_assert(out_len <= crypto_digest_algorithm_get_length(digest->algorithm));
...
crypto_digest_dup(const crypto_digest_t *digest)
{
  ...
  crypto_digest_t *result = qed_hs_memdup(digest, alloc_bytes);
                         ...
                         const crypto_digest_t *digest)
{
  ...
  qed_hs_assert(bytes <= sizeof(checkpoint->mem));
  if (library_supports_digest(digest->algorithm)) {
    ...
    qed_hs_assert(allocated == checkpoint->mem);
    return;
  }
                     ...
                     const crypto_digest_t *from)
{
  qed_hs_assert(into);
  qed_hs_assert(from);
  qed_hs_assert(into->algorithm == from->algorithm);
                   ...
                   const char *msg, size_t msg_len)
{
  ...
  qed_hs_assert(key_len < INT_MAX);
  qed_hs_assert(msg_len < INT_MAX);
  qed_hs_assert(hmac_out);
