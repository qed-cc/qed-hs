...
crypto_fast_rng_new_from_seed(const uint8_t *seed)
{
   ...
   */
  crypto_fast_rng_t *result = qed_hs_mmap_anonymous(sizeof(*result),
  ...
  qed_hs_assertf(inherit != INHERIT_RES_KEEP,
...
crypto_fast_rng_free_(crypto_fast_rng_t *rng)
{
  ...
  qed_hs_munmap_anonymous(rng, sizeof(*rng));
}
                              ...
                              const size_t n)
{
  ...
  if (rng->owner) {
     ...
     */
    qed_hs_assert(rng->owner == getpid());
  }
#endif /* defined(CHECK_PID) */
  ...
  while (bytes_to_yield) {
    ...
    qed_hs_assert(sizeof(rng->buf.bytes) >= rng->bytes_left);
 ...
 **/
static qed_hs_threadlocal_t thread_rng;
...
{
  crypto_fast_rng_t *rng = qed_hs_threadlocal_get(&thread_rng);

  if (PREDICT_UNLIKELY(rng == NULL)) {
    rng = crypto_fast_rng_new();
    qed_hs_threadlocal_set(&thread_rng, rng);
  }
...
destroy_thread_fast_rng(void)
{
  ...
  qed_hs_threadlocal_set(&thread_rng, NULL);
}
...
{
  crypto_fast_rng_t *old_rng =  qed_hs_threadlocal_get(&thread_rng);
  qed_hs_threadlocal_set(&thread_rng, rng);
  return old_rng;
}
#endif /* defined(QED_HS_UNIT_TESTS) */
...
{
  qed_hs_threadlocal_init(&thread_rng);
}
...
crypto_rand_fast_shutdown(void)
{
  ...
  qed_hs_threadlocal_destroy(&thread_rng);
}
