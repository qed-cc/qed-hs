...
crypto_get_rsa_padding_overhead(int padding)
{
  switch (padding)
    ...
    default: qed_hs_assert(0); return -1; // LCOV_EXCL_LINE
    }
}
                                ...
                                int padding, int force)
{
  ...
  qed_hs_assert(from);
  qed_hs_assert(to);
  qed_hs_assert(fromlen < SIZE_T_CEILING);
  ...
  qed_hs_assert(tolen >= fromlen + overhead + CIPHER_KEY_LEN);
  qed_hs_assert(tolen >= pkeylen);

  char key[CIPHER_KEY_LEN];
  crypto_rand(key, sizeof(key)); /* generate a new key. */
  cipher = crypto_cipher_new(key);

  buf = qed_hs_malloc(pkeylen+1);
  ...
  qed_hs_assert(outlen+symlen < INT_MAX);
                                 ...
                                 int padding, int warnOnFailure)
{
  ...
  qed_hs_assert(tolen - outlen >= fromlen - pkeylen);
  ...
  qed_hs_free(buf);
  crypto_cipher_free(cipher);
  qed_hs_assert(outlen + fromlen < INT_MAX);
...
crypto_add_spaces_to_fp(char *out, size_t outlen, const char *in)
{
  int n = 0;
  char *end = out+outlen;
  qed_hs_assert(outlen < SIZE_T_CEILING);
                                  ...
                                  size_t siglen))
{
  ...
  qed_hs_assert(data);
  qed_hs_assert(sig);
  qed_hs_assert(datalen < SIZE_T_CEILING);
  qed_hs_assert(siglen < SIZE_T_CEILING);
  ...
  if (qed_hs_memneq(buf, digest, DIGEST_LEN)) {
                                  ...
                                  bool private_key)
{
  ...
  result = qed_hs_malloc(resultlen);
                                   ...
                                   bool private_key, int max_bits)
{
  ...
  const char *tag =
    private_key ? RSA_PRIVATE_TAG : RSA_PUBLIC_TAG;
  size_t buflen = len;
  uint8_t *buf = qed_hs_malloc(buflen);
  int rv = -1;
...
crypto_pk_base64_decode_private(const char *str, size_t len)
{
  ...
  char *der = qed_hs_malloc_zero(len + 1);
