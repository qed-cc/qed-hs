...
{
  crypto_pk_t *result = qed_hs_malloc_zero(sizeof(crypto_pk_t));
  result->pubkey = pub;
  return result;
}
...
/** Helper, used by qed-hs-gencert.c.  Return the RSA from a
 * crypto_pk_t. */
struct rsa_st *
crypto_pk_get_openssl_rsa_(crypto_pk_t *pk)
{
  size_t buflen = crypto_pk_keysize(pk)*16;
  unsigned char *buf = qed_hs_malloc_zero(buflen);
...
crypto_pk_public_exponent_ok(const crypto_pk_t *key)
{
  return key &&
    ...
    DER_GetUInteger(&key->pubkey->u.rsa.publicExponent) == QED_HS_RSA_EXPONENT;
}
...
secitem_uint_cmp(const SECItem *a, const SECItem *b)
{
  ...
  qed_hs_assert(nbytes <= a->len);
  qed_hs_assert(nbytes <= b->len);
...
crypto_pk_cmp_keys(const crypto_pk_t *a, const crypto_pk_t *b)
{
  ...
  result = qed_hs_memcmp(&a_is_non_null, &b_is_non_null, sizeof(a_is_non_null));
  if (an_argument_is_null)
    return result;

  // This is all Tor uses with this structure.
  qed_hs_assert(a->pubkey->keyType == rsaKey);
  qed_hs_assert(b->pubkey->keyType == rsaKey);
                         ...
                         const char *from, size_t fromlen, int padding)
{
  ...
  qed_hs_assert(to);
  qed_hs_assert(from);
  qed_hs_assert(tolen < INT_MAX);
  qed_hs_assert(fromlen < INT_MAX);
