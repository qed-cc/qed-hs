...
crypto_rand_int(unsigned int max)
{
   ...
   */
  qed_hs_assert(max <= ((unsigned int)INT_MAX)+1);
...
{
  qed_hs_assert(min < max);
  qed_hs_assert(max <= INT_MAX);
...
{
  qed_hs_assert(limit < UINT64_MAX);
