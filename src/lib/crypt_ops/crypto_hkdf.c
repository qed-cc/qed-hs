...
{
  int i, r = -1;
  uint8_t *cp, *tmp = qed_hs_malloc(key_in_len+1);
  uint8_t digest[DIGEST_LEN];

  /* If we try to get more than this amount of key data, we'll repeat blocks.*/
  qed_hs_assert(key_out_len <= DIGEST_LEN*256);
                                    ...
                                    uint8_t *key_out, size_t key_out_len)
{
  ...
  qed_hs_assert(evp_pkey_ctx);
  qed_hs_assert(key_in_len != 0);
  qed_hs_assert(salt_in_len <= 1024);
                                    ...
                                    uint8_t *key_out, size_t key_out_len)
{
  ...
  /* If we try to get more than this amount of key data, we'll repeat blocks.*/
  qed_hs_assert(key_out_len <= DIGEST256_LEN * 256);
  qed_hs_assert(info_in_len <= 128);
