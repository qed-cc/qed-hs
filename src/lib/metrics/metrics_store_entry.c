...
{
  metrics_store_entry_t *entry = qed_hs_malloc_zero(sizeof(*entry));

  qed_hs_assert(name);

  entry->type = type;
  entry->name = qed_hs_strdup(name);
  entry->labels = smartlist_new();
  if (help) {
    entry->help = qed_hs_strdup(help);
  }

  if (type == METRICS_TYPE_HISTOGRAM && bucket_count > 0) {
    qed_hs_assert(buckets);

    entry->u.histogram.bucket_count = bucket_count;
    entry->u.histogram.buckets =
        qed_hs_malloc_zero(sizeof(metrics_histogram_bucket_t) * bucket_count);
...
metrics_store_entry_free_(metrics_store_entry_t *entry)
{
  ...
  SMARTLIST_FOREACH(entry->labels, char *, l, qed_hs_free(l));
  smartlist_free(entry->labels);
  qed_hs_free(entry->name);
  qed_hs_free(entry->help);

  if (entry->type == METRICS_TYPE_HISTOGRAM) {
    qed_hs_free(entry->u.histogram.buckets);
  }
                                   ...
                                   const int64_t bucket)
{
  ...
  qed_hs_assertf_nonfatal(false, "attempted to get the value of non-existent "
                              ...
                              const char *label)
{
  ...
  smartlist_add(entry->labels, qed_hs_strdup(label));
}
