...
smartlist_new,(void))
{
  smartlist_t *sl = qed_hs_malloc(sizeof(smartlist_t));
  sl->num_used = 0;
  sl->capacity = SMARTLIST_DEFAULT_CAPACITY;
  sl->list = qed_hs_calloc(sizeof(void *), sl->capacity);
  return sl;
}
...
#define MAX_CAPACITY (int)((SIZE_MAX / (sizeof(void*))))
  ...
  if (size > (size_t) sl->capacity) {
    ...
    sl->list = qed_hs_reallocarray(sl->list, sizeof(void *),
                                ((size_t)higher));
