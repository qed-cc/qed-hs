 ...
 * overkill: it would be better to use qed_hs_memeq() or qed_hs_memneq().
 */
int
qed_hs_memcmp(const void *a, const void *b, size_t len)
{
 ...
 * !qed_hs_memcmp(a,b,sz) by being faster.
 */
int
qed_hs_memeq(const void *a, const void *b, size_t sz)
{
                ...
                const uint8_t *key, void *val)
{
  ...
  new_ent = qed_hs_malloc_zero(sizeof(di_digest256_map_t));
  new_ent->next = *map;
             ...
             void *dflt_val)
{
  ...
  while (map) {
    uintptr_t r = (uintptr_t) qed_hs_memeq(map->key, key, 32);
