                   ...
                   const char *val)
{
  ...
  newline = qed_hs_malloc_zero(sizeof(config_line_t));
  newline->key = qed_hs_strdup(key);
  newline->value = qed_hs_strdup(val);
  newline->next = NULL;
  while (*lst)
    lst = &((*lst)->next);
    ...
    if (k && v) {
      ...
      if (allow_include && !strcmp(k, "%include") && handle_include) {
        ...
        *next = include_list;
         ...
         * n^2 performance. */
        *next = qed_hs_malloc_zero(sizeof(**next));
        (*next)->key = k;
        (*next)->value = v;
        (*next)->next = NULL;
                            ...
                            const char *key)
{
  ...
  while (inp) {
    ...
    *next_out = qed_hs_malloc_zero(sizeof(config_line_t));
    (*next_out)->key = qed_hs_strdup(inp->key);
    (*next_out)->value = qed_hs_strdup(inp->value);
    inp = inp->next;
                                   ...
                                   const char **err_out)
{
  ...
  *key_out = *value_out = NULL;
  ...
  key = line;
  while (*line && !QED_HS_ISSPACE(*line) && *line != '#' &&
         ! (line[0] == '\\' && line[1] == '\n'))
    ++line;
  *key_out = qed_hs_strndup(key, line-key);
  ...
  if (*line == '\"') { // XXX No continuation handling is done here
    ...
    while (cp>val && QED_HS_ISSPACE(*(cp-1)))
      --cp;

    qed_hs_assert(cp >= val);

    /* Now copy out and decode the value. */
    *value_out = qed_hs_strndup(val, cp-val);
    if (continuation) {
      char *v_out, *v_in;
  ...
  while (QED_HS_ISSPACE(*line)) ++line;
