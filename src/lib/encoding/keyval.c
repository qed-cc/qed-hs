...
string_is_key_value(int severity, const char *string)
{
  ...
  if (strlen(string) < 2) { /* "x=" is shortest args string */
    qed_hs_log(severity, LD_GENERAL, "'%s' is too short to be a k=v value.",
            escaped(string));
    return 0;
  }
  ...
  if (!equal_sign_pos) {
    qed_hs_log(severity, LD_GENERAL, "'%s' is not a k=v value.", escaped(string));
    return 0;
  }

  /* validate that the '=' is not in the beginning of the string. */
  if (equal_sign_pos == string) {
    qed_hs_log(severity, LD_GENERAL, "'%s' is not a valid k=v value.",
            escaped(string));
    return 0;
  }
