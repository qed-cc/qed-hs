...
needs_escape(const char *s, bool as_keyless_val)
{
  ...
  for (; *s; ++s) {
    if (*s >= 127 || QED_HS_ISSPACE(*s) || ! QED_HS_ISPRINT(*s) ||
        *s == '\'' || *s == '\"') {
      return true;
    }
  }
...
kvline_encode(const config_line_t *line,
              unsigned flags)
{
  qed_hs_assert(! (flags & KV_QUOTED_QSTRING));

  qed_hs_assert((flags & (KV_OMIT_KEYS|KV_OMIT_VALS)) !=
             (KV_OMIT_KEYS|KV_OMIT_VALS));
  qed_hs_assert((flags & (KV_QUOTED|KV_RAW)) != (KV_QUOTED|KV_RAW));
...
kvline_parse(const char *line, unsigned flags)
{
  ...
  while (*cp) {
    ...
    if (*cp != '\"') {
      ...
      if (cp[idx] == '=') {
        key = qed_hs_memdup_nulterm(cp, idx);
        cp += idx + 1;
      } else if (omit_vals) {
        key = qed_hs_memdup_nulterm(cp, idx);
        cp += idx;
    ...
    if (*cp == '\"') {
      ...
      size_t idx = strcspn(cp, " \t\r\v\n");
      val = qed_hs_memdup_nulterm(cp, idx);
      cp += idx;
    }
  ...
  commit:
    ...
    *next_line = qed_hs_malloc_zero(sizeof(config_line_t));
    (*next_line)->key = key ? key : qed_hs_strdup("");
    (*next_line)->value = val ? val : qed_hs_strdup("");
