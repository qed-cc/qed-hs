...
#define QED_HS_ISODIGIT(c) ('0' <= (c) && (c) <= '7')
...
unescape_string(const char *s, char **result, size_t *size_out)
{
  ...
  while (1) {
    switch (*cp) {
      ...
      case '\\':
        if (cp[1] == 'x' || cp[1] == 'X') {
          if (!(QED_HS_ISXDIGIT(cp[2]) && QED_HS_ISXDIGIT(cp[3])))
            return NULL;
          cp += 4;
        } else if (QED_HS_ISODIGIT(cp[1])) {
          cp += 2;
          if (QED_HS_ISODIGIT(*cp)) ++cp;
          if (QED_HS_ISODIGIT(*cp)) ++cp;
 ...
 end_of_loop:
  out = *result = qed_hs_malloc(cp-s + 1);
  cp = s+1;
  while (1) {
    switch (*cp)
      {
      ...
      case '\\':
        switch (cp[1])
          ...
          case '6': case '7':
            {
              int n = cp[1]-'0';
              cp += 2;
              if (QED_HS_ISODIGIT(*cp)) { n = n*8 + *cp-'0'; cp++; }
              if (QED_HS_ISODIGIT(*cp)) { n = n*8 + *cp-'0'; cp++; }
              if (n > 255) { qed_hs_free(*result); return NULL; }
              *out++ = (char)n;
            }
            break;
          case '\'':
          ...
          default:
            /* we caught this above in the initial loop. */
            qed_hs_assert_nonfatal_unreached();
            qed_hs_free(*result); return NULL;
            /* LCOV_EXCL_STOP */
          }
        break;
      default:
