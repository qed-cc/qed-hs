 ...
 */
struct tm *
qed_hs_localtime_r(const time_t *timep, struct tm *result)
{
  char *err = NULL;
  struct tm *r = qed_hs_localtime_r_msg(timep, result, &err);
  if (err) {
 ...
 */
struct tm *
qed_hs_gmtime_r(const time_t *timep, struct tm *result)
{
  char *err = NULL;
  struct tm *r = qed_hs_gmtime_r_msg(timep, result, &err);
  if (err) {
    log_warn(LD_BUG, "%s", err);
    qed_hs_free(err);
  }
...
/** Number of days per month in non-leap year; used by qed_hs_timegm and
 * parse_rfc1123_time. */
...
qed_hs_timegm_impl(const struct tm *tm, time_t *time_out)
{
  ...
  *time_out = 0;
  ...
  if (invalid_year ||
    ...
    log_warn(LD_BUG, "Out-of-range argument to qed_hs_timegm");
    return -1;
  }
  ...
  if (seconds < TIME_MIN || seconds > TIME_MAX) {
    log_warn(LD_BUG, "Result does not fit in qed_hs_timegm");
    return -1;
  }
 ...
 */
int
qed_hs_timegm(const struct tm *tm, time_t *time_out)
{
#ifdef HAVE_TIMEGM
  ...
  return qed_hs_timegm_impl(tm, time_out);
#endif
}
...
format_rfc1123_time(char *buf, time_t t)
{
  ...
  qed_hs_gmtime_r(&t, &tm);

  strftime(buf, RFC1123_TIME_LEN+1, "___, %d ___ %Y %H:%M:%S GMT", &tm);
  qed_hs_assert(tm.tm_wday >= 0);
  qed_hs_assert(tm.tm_wday <= 6);
  memcpy(buf, WEEKDAY_NAMES[tm.tm_wday], 3);
  qed_hs_assert(tm.tm_mon >= 0);
  qed_hs_assert(tm.tm_mon <= 11);
...
parse_rfc1123_time(const char *buf, time_t *t)
{
  ...
  if (qed_hs_sscanf(buf, "%3s, %2u %3s %u %2u:%2u:%2u GMT", weekday,
...
{
  struct tm tm;
  strftime(buf, ISO_TIME_LEN+1, "%Y-%m-%d %H:%M:%S", qed_hs_localtime_r(&t, &tm));
}
...
{
  struct tm tm;
  strftime(buf, ISO_TIME_LEN+1, "%Y-%m-%d %H:%M:%S", qed_hs_gmtime_r(&t, &tm));
}
...
format_iso_time_nospace_usec(char *buf, const struct timeval *tv)
{
  ...
  qed_hs_snprintf(buf+ISO_TIME_LEN, 8, ".%06d", (int)tv->tv_usec);
}
...
parse_iso_time_(const char *cp, time_t *t, int strict, int nospace)
{
  ...
  int n_fields;
  char extra_char, separaqed_hs_char;
  n_fields = qed_hs_sscanf(cp, "%u-%2u-%2u%c%2u:%2u:%2u%c",
                        &year, &month, &day,
                        &separaqed_hs_char,
  ...
  if (separaqed_hs_char != (nospace ? 'T' : ' ')) {
    char *esc = esc_for_log(cp);
    log_warn(LD_GENERAL, "ISO time %s was unparseable", esc);
    qed_hs_free(esc);
    return -1;
...
parse_http_time(const char *date, struct tm *tm)
{
  ...
  if ((cp = strchr(date, ','))) {
    ...
    if (*cp != ' ')
      return -1;
    ++cp;
    if (qed_hs_sscanf(cp, "%2u %3s %4u %2u:%2u:%2u GMT",
               &tm_mday, month, &tm_year,
               &tm_hour, &tm_min, &tm_sec) == 6) {
      /* rfc1123-date */
      tm_year -= 1900;
    } else if (qed_hs_sscanf(cp, "%2u-%3s-%2u %2u:%2u:%2u GMT",
                      &tm_mday, month, &tm_year,
                      &tm_hour, &tm_min, &tm_sec) == 6) {
      /* rfc850-date */
    } else {
    ...
    if (qed_hs_sscanf(date, "%3s %3s %2u %2u:%2u:%2u %4u",
               wkday, month, &tm_mday,
...
format_time_interval(char *out, size_t out_len, long interval)
{
  ...
  if (day) {
    return qed_hs_snprintf(out, out_len, "%ld days, %ld hours, %ld minutes",
                        day, hour, min);
  } else if (hour) {
    return qed_hs_snprintf(out, out_len, "%ld hours, %ld minutes", hour, min);
  } else if (min) {
    return qed_hs_snprintf(out, out_len, "%ld minutes, %ld seconds", min, sec);
  } else {
    return qed_hs_snprintf(out, out_len, "%ld seconds", sec);
  }
}
