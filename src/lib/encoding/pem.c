...
pem_encode(char *dest, size_t destlen, const uint8_t *src, size_t srclen,
           const char *objtype)
{
  if (qed_hs_snprintf(dest, destlen, "-----BEGIN %s-----\n", objtype) < 0)
    return -1;
  ...
  if (qed_hs_snprintf(dest + offset, destlen - offset,
                   "-----END %s-----\n", objtype) < 0)
    return -1;

  qed_hs_assert(strlen(dest) + 1 <= pem_encoded_size(srclen, objtype));
  return 0;
}
           ...
           const char *objtype)
{
  ...
  char *tag = NULL;
  qed_hs_asprintf(&tag, "-----BEGIN %s-----", objtype);
  ...
  qed_hs_asprintf(&tag, "\n-----END %s-----", objtype);
  const char *end_of_base64 = qed_hs_memstr(src, eos-src, tag);
  qed_hs_free(tag);
