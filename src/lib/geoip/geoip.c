...
static void
geoip_add_entry(const qed_hs_addr_t *low, const qed_hs_addr_t *high,
                const char *country)
{
  intptr_t idx;
  void *idxplus1_;

  IF_BUG_ONCE(qed_hs_addr_family(low) != qed_hs_addr_family(high))
    return;
  IF_BUG_ONCE(qed_hs_addr_compare(high, low, CMP_EXACT) < 0)
    return;

  idxplus1_ = strmap_get_lc(country_idxplus1_by_lc_code, country);

  if (!idxplus1_) {
    geoip_country_t *c = qed_hs_malloc_zero(sizeof(geoip_country_t));
    strlcpy(c->countrycode, country, sizeof(c->countrycode));
    qed_hs_strlower(c->countrycode);
  ...
  }
  {
    ...
    qed_hs_assert(!strcasecmp(c->countrycode, country));
  }

  if (qed_hs_addr_family(low) == AF_INET) {
    geoip_ipv4_entry_t *ent = qed_hs_malloc_zero(sizeof(geoip_ipv4_entry_t));
    ent->ip_low = qed_hs_addr_to_ipv4h(low);
    ent->ip_high = qed_hs_addr_to_ipv4h(high);
    ent->country = idx;
    smartlist_add(geoip_ipv4_entries, ent);
  } else if (qed_hs_addr_family(low) == AF_INET6) {
    geoip_ipv6_entry_t *ent = qed_hs_malloc_zero(sizeof(geoip_ipv6_entry_t));
    ent->ip_low = *qed_hs_addr_to_in6_assert(low);
    ent->ip_high = *qed_hs_addr_to_in6_assert(high);
    ent->country = idx;
...
{
  qed_hs_addr_t low_addr, high_addr;
  char c[3];
  char *country = NULL;
  ...
  if (family == AF_INET) {
    ...
    if (qed_hs_sscanf(line,"%u,%u,%2s", &low, &high, c) == 3 ||
        qed_hs_sscanf(line,"\"%u\",\"%u\",\"%2s\",", &low, &high, c) == 3) {
      qed_hs_addr_from_ipv4h(&low_addr, low);
      qed_hs_addr_from_ipv4h(&high_addr, high);
    } else
      goto fail;
    country = c;
  } else {                      /* AF_INET6 */
    ...
    low_str = qed_hs_strtok_r(buf, ",", &strtok_state);
    if (!low_str)
      goto fail;
    high_str = qed_hs_strtok_r(NULL, ",", &strtok_state);
    if (!high_str)
      goto fail;
    country = qed_hs_strtok_r(NULL, "\n", &strtok_state);
    if (!country)
      goto fail;
    if (strlen(country) != 2)
      goto fail;
    if (qed_hs_inet_pton(AF_INET6, low_str, &low) <= 0)
      goto fail;
    qed_hs_addr_from_in6(&low_addr, &low);
    if (qed_hs_inet_pton(AF_INET6, high_str, &high) <= 0)
      goto fail;
    qed_hs_addr_from_in6(&high_addr, &high);
  }
...
init_geoip_countries(void)
{
  ...
  geoip_unresolved = qed_hs_malloc_zero(sizeof(geoip_country_t));
...
geoip_load_file(sa_family_t family, const char *filename, int severity)
{
  ...
  qed_hs_assert(family == AF_INET || family == AF_INET6);

  if (!(f = qed_hs_fopen_cloexec(filename, "r"))) {
 ...
 */
MOCK_IMPL(int,
geoip_get_country_by_addr,(const qed_hs_addr_t *addr))
{
  if (qed_hs_addr_family(addr) == AF_INET) {
    return geoip_get_country_by_ipv4(qed_hs_addr_to_ipv4h(addr));
  } else if (qed_hs_addr_family(addr) == AF_INET6) {
    return geoip_get_country_by_ipv6(qed_hs_addr_to_in6(addr));
  } else {
    return -1;
  }
}
...
{
  if (geoip_countries) {
    SMARTLIST_FOREACH(geoip_countries, geoip_country_t *, c, qed_hs_free(c));
