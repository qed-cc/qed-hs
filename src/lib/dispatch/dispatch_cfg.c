...
dcfg_new(void)
{
  dispatch_cfg_t *cfg = qed_hs_malloc(sizeof(dispatch_cfg_t));
                  ...
                  msg_type_id_t type)
{
  ...
  if (!oldval)
    smartlist_set(cfg->type_by_msg, msg, qed_hs_memdup(&type, sizeof(type)));
  return 0;
}
                  ...
                  channel_id_t chan)
{
  ...
  if (!oldval)
    smartlist_set(cfg->chan_by_msg, msg, qed_hs_memdup(&chan, sizeof(chan)));
  return 0;
}
                  ...
                  const dispatch_typefns_t *fns)
{
  ...
  if (oldfns && (oldfns->free_fn != fns->free_fn ||
    ...
    return -1;
  if (!oldfns)
    smartlist_set(cfg->fns_by_type, type, qed_hs_memdup(fns, sizeof(*fns)));
  return 0;
}
              ...
              subsys_id_t sys, recv_fn_t fn)
{
  ...
  dispatch_rcv_t *rcv = qed_hs_malloc(sizeof(dispatch_rcv_t));
  rcv->sys = sys;
  rcv->enabled = true;
  rcv->fn = fn;
...
dcfg_free_(dispatch_cfg_t *cfg)
{
  ...
  SMARTLIST_FOREACH(cfg->type_by_msg, msg_type_id_t *, id, qed_hs_free(id));
  SMARTLIST_FOREACH(cfg->chan_by_msg, channel_id_t *, id, qed_hs_free(id));
  SMARTLIST_FOREACH(cfg->fns_by_type, dispatch_typefns_t *, f, qed_hs_free(f));
  ...
  SMARTLIST_FOREACH_BEGIN(cfg->recv_by_msg, smartlist_t *, receivers) {
    if (!receivers)
      continue;
    SMARTLIST_FOREACH(receivers, dispatch_rcv_t *, rcv, qed_hs_free(rcv));
