...
dispatch_free_(dispatch_t *d)
{
  ...
  for (size_t i = 0; i < n_queues; ++i) {
    msg_t *m, *mtmp;
    QED_HS_SIMPLEQ_FOREACH_SAFE(m, &d->queues[i].queue, next, mtmp) {
  ...
  for (size_t i = 0; i < n_msgs; ++i) {
    qed_hs_free(d->table[i]);
  }
  qed_hs_free(d->table);
  qed_hs_free(d->typefns);
  qed_hs_free(d->queues);
              ...
              msg_aux_data_t auxdata)
{
  ...
  msg_t *m = qed_hs_malloc(sizeof(msg_t));
...
dispatch_send_msg_unchecked(dispatch_t *d, msg_t *m)
{
  ...
  bool was_empty = QED_HS_SIMPLEQ_EMPTY(&q->queue);

  /* Append the message. */
  QED_HS_SIMPLEQ_INSERT_TAIL(&q->queue, m, next);
...
dispatch_flush(dispatch_t *d, channel_id_t ch, int max_msgs)
{
  ...
  while (n_flushed < max_msgs) {
    msg_t *m = QED_HS_SIMPLEQ_FIRST(&q->queue);
    if (!m)
      break;
    QED_HS_SIMPLEQ_REMOVE_HEAD(&q->queue, next);
