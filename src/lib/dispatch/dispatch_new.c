...
dtbl_entry_from_lst(smartlist_t *receivers)
{
  ...
  dtbl_entry_t *ent;
  ent = qed_hs_malloc_zero(offsetof(dtbl_entry_t, rcv) +
...
{
  dispatch_t *d = qed_hs_malloc_zero(sizeof(dispatch_t));
  ...
  d->typefns = qed_hs_calloc(n_types, sizeof(dispatch_typefns_t));
  ...
  d->queues = qed_hs_calloc(d->n_queues, sizeof(dqueue_t));
  for (size_t i = 0; i < d->n_queues; ++i) {
    QED_HS_SIMPLEQ_INIT(&d->queues[i].queue);
    d->queues[i].alert_fn = alert_fn_nop;
  }

  /* Build the dispatch tables mapping message IDs to receivers. */
  d->table = qed_hs_calloc(d->n_msgs, sizeof(dtbl_entry_t *));
