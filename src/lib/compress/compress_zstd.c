...
int
qed_hs_zstd_method_supported(void)
{
#ifdef HAVE_ZSTD
  return 1;
#else
...
static void
qed_hs_zstd_format_version(char *buf, size_t buflen, unsigned version_number)
{
  qed_hs_snprintf(buf, buflen,
               "%u.%u.%u",
...
const char *
qed_hs_zstd_get_version_str(void)
{
#ifdef HAVE_ZSTD
  static char version_str[VERSION_STR_MAX_LEN];

  qed_hs_zstd_format_version(version_str, sizeof(version_str),
                          ZSTD_versionNumber());
...
const char *
qed_hs_zstd_get_header_version_str(void)
{
#ifdef HAVE_ZSTD
...
int
qed_hs_zstd_can_use_static_apis(void)
{
...
struct qed_hs_zstd_compress_state_t {
#ifdef HAVE_ZSTD
  union {
 ...
 * based on inspecting the zstd source: qed_hs_zstd_state_size_precalc() is
 * more accurate when it's allowed to use "static-only" functions */
static size_t
qed_hs_zstd_state_size_precalc_fake(int compress, int preset)
{
  qed_hs_assert(preset > 0);

  size_t memory_usage = sizeof(qed_hs_zstd_compress_state_t);
...
static size_t
qed_hs_zstd_state_size_precalc(int compress, int preset)
{
#ifdef ZSTD_STATIC_LINKING_ONLY
  if (qed_hs_zstd_can_use_static_apis()) {
    if (compress) {
  ...
  return qed_hs_zstd_state_size_precalc_fake(compress, preset);
}
#endif /* defined(HAVE_ZSTD) */

/** Construct and return a qed_hs_zstd_compress_state_t object using
...
{
  qed_hs_assert(method == ZSTD_METHOD);

#ifdef HAVE_ZSTD
  const int preset = memory_level(level);
  qed_hs_zstd_compress_state_t *result;
  size_t retval;

  result = qed_hs_malloc_zero(sizeof(qed_hs_zstd_compress_state_t));
  result->compress = compress;
  result->allocation = qed_hs_zstd_state_size_precalc(compress, preset);
                          ...
                          int finish)
{
  ...
  qed_hs_assert(*in_len <= UINT_MAX);
  qed_hs_assert(*out_len <= UINT_MAX);
  ...
  *in_len = input.size - input.pos;
  ...
  if (! state->compress &&
      qed_hs_compress_is_compression_bomb(state->input_so_far,
                                       state->output_so_far)) {
  ...
  if (!finish) {
    ...
    *out_len = output.size - output.pos;
    ...
    return QED_HS_COMPRESS_DONE;
  } else /* if (!state->compress) */ {
    // ZSTD_decompressStream returns 0 if the frame is done, or >0 if it
    // is incomplete.
    // We check this above.
    qed_hs_assert_nonfatal(!ZSTD_isError(retval));
...
void
qed_hs_zstd_compress_free_(qed_hs_zstd_compress_state_t *state)
{
  if (state == NULL)
    return;
...
size_t
qed_hs_zstd_compress_state_size(const qed_hs_zstd_compress_state_t *state)
{
  qed_hs_assert(state != NULL);
 ...
 * states. */
size_t
qed_hs_zstd_get_total_allocation(void)
{
...
void
qed_hs_zstd_warn_if_version_mismatched(void)
{
  ...
  if (! qed_hs_zstd_can_use_static_apis()) {
    ...
    qed_hs_zstd_format_version(header_version, sizeof(header_version),
                            ZSTD_VERSION_NUMBER);
    qed_hs_zstd_format_version(runtime_version, sizeof(runtime_version),
                            ZSTD_versionNumber());
...
void
qed_hs_zstd_set_static_apis_disabled_for_testing(int disabled)
{
