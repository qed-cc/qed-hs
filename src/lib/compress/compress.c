...
MOCK_IMPL(int,
qed_hs_compress_is_compression_bomb,(size_t size_in, size_t size_out))
{
  ...
  if (size_out / size_in > MAX_UNCOMPRESSION_FACTOR) {
    log_warn(LD_GENERAL,
             ...
             "input size = %"QED_HS_PRIuSZ " and output size = %"QED_HS_PRIuSZ,
             size_in, size_out);
    return 1;
  }
...
/** Internal function to implement qed_hs_compress/qed_hs_uncompress, depending on
 * whether <b>compress</b> is set.  All arguments are as for qed_hs_compress or
 * qed_hs_uncompress. */
static int
qed_hs_compress_impl(int compress,
...
{
  qed_hs_compress_state_t *stream;
  int rv;

  stream = qed_hs_compress_new(compress, method, compression_level);
  ...
  *out = outptr = qed_hs_malloc(out_remaining);

  const int finish = complete_only || compress;

  while (1) {
    switch (qed_hs_compress_process(stream,
                                 &outptr, &out_remaining,
                                 &in, &in_len, finish)) {
      case QED_HS_COMPRESS_DONE:
      ...
      case QED_HS_COMPRESS_OK:
        ...
        break;
      case QED_HS_COMPRESS_BUFFER_FULL: {
        ...
        if (!compress &&
            qed_hs_compress_is_compression_bomb(in_len_orig, out_alloc)) {
        ...
        out_alloc *= 2;
        *out = qed_hs_realloc(*out, out_alloc);
        outptr = *out + offset;
        out_remaining = out_alloc - offset;
        break;
      }
      case QED_HS_COMPRESS_ERROR:
  ...
  *out_len = outptr - *out;
  if (compress && qed_hs_compress_is_compression_bomb(*out_len, in_len_orig)) {
  ...
  if (!compress) {
    ...
    if (out_alloc == *out_len)
      *out = qed_hs_realloc(*out, out_alloc + 1);
    (*out)[*out_len] = '\0';
  }
  rv = 0;
  goto out;
 ...
 */
int
qed_hs_compress(char **out, size_t *out_len,
             const char *in, size_t in_len,
             compress_method_t method)
{
  return qed_hs_compress_impl(1, out, out_len, in, in_len, method,
                           BEST_COMPRESSION,
                           1, LOG_WARN);
}
 ...
 */
int
qed_hs_uncompress(char **out, size_t *out_len,
               const char *in, size_t in_len,
               compress_method_t method,
               int complete_only,
               int protocol_warn_level)
{
  return qed_hs_compress_impl(0, out, out_len, in, in_len, method,
                           BEST_COMPRESSION,
...
detect_compression_method(const char *in, size_t in_len)
{
  if (in_len > 2 && fast_memeq(in, "\x1f\x8b", 2)) {
    return GZIP_METHOD;
             ...
             (qed_hs_ntohs(get_uint16(in)) % 31) == 0) {
    return ZLIB_METHOD;
  } else if (in_len > 2 &&
...
int
qed_hs_compress_supports_method(compress_method_t method)
{
  switch (method) {
    case GZIP_METHOD:
    case ZLIB_METHOD:
      return qed_hs_zlib_method_supported();
    case LZMA_METHOD:
      return qed_hs_lzma_method_supported();
    case ZSTD_METHOD:
      return qed_hs_zstd_method_supported();
    case NO_METHOD:
      return 1;
    case UNKNOWN_METHOD:
    default:
 ...
 */
unsigned
qed_hs_compress_get_supported_method_bitmask(void)
{
  static unsigned supported = 0;
  if (supported == 0) {
    compress_method_t m;
    for (m = NO_METHOD; m <= UNKNOWN_METHOD; ++m) {
      if (qed_hs_compress_supports_method(m)) {
        supported |= (1u << m);
      }
    }
  }
...
const char *
qed_hs_compress_version_str(compress_method_t method)
{
  switch (method) {
    case GZIP_METHOD:
    case ZLIB_METHOD:
      return qed_hs_zlib_get_version_str();
    case LZMA_METHOD:
      return qed_hs_lzma_get_version_str();
    case ZSTD_METHOD:
      return qed_hs_zstd_get_version_str();
    case NO_METHOD:
    case UNKNOWN_METHOD:
    default:
      return NULL;
...
const char *
qed_hs_compress_header_version_str(compress_method_t method)
{
  switch (method) {
    case GZIP_METHOD:
    case ZLIB_METHOD:
      return qed_hs_zlib_get_header_version_str();
    case LZMA_METHOD:
      return qed_hs_lzma_get_header_version_str();
    case ZSTD_METHOD:
      return qed_hs_zstd_get_header_version_str();
    case NO_METHOD:
    case UNKNOWN_METHOD:
    default:
      return NULL;
...
size_t
qed_hs_compress_get_total_allocation(void)
{
  return atomic_counter_get(&total_compress_allocation) +
         qed_hs_zlib_get_total_allocation() +
         qed_hs_lzma_get_total_allocation() +
         qed_hs_zstd_get_total_allocation();
}

/** Internal state for an incremental compression/decompression.  The body of
 * this struct is not exposed. */
struct qed_hs_compress_state_t {
  compress_method_t method; /**< The compression method. */

  union {
    qed_hs_zlib_compress_state_t *zlib_state;
    qed_hs_lzma_compress_state_t *lzma_state;
    qed_hs_zstd_compress_state_t *zstd_state;
  } u; /**< Compression backend state. */
};

/** Construct and return a qed_hs_compress_state_t object using <b>method</b>.  If
 * <b>compress</b>, it's for compression; otherwise it's for decompression. */
qed_hs_compress_state_t *
qed_hs_compress_new(int compress, compress_method_t method,
                 compression_level_t compression_level)
{
  qed_hs_compress_state_t *state;

  state = qed_hs_malloc_zero(sizeof(qed_hs_compress_state_t));
  state->method = method;
  ...
  switch (method) {
    ...
    case ZLIB_METHOD: {
      qed_hs_zlib_compress_state_t *zlib_state =
        qed_hs_zlib_compress_new(compress, method, compression_level);
    ...
    case LZMA_METHOD: {
      qed_hs_lzma_compress_state_t *lzma_state =
        qed_hs_lzma_compress_new(compress, method, compression_level);
    ...
    case ZSTD_METHOD: {
      qed_hs_zstd_compress_state_t *zstd_state =
        qed_hs_zstd_compress_new(compress, method, compression_level);
  ...
  atomic_counter_add(&total_compress_allocation,
                     sizeof(qed_hs_compress_state_t));
  return state;
 ...
 *
 * Return QED_HS_COMPRESS_DONE if we've finished the entire
 * compression/decompression.
 * Return QED_HS_COMPRESS_OK if we're processed everything from the input.
 * Return QED_HS_COMPRESS_BUFFER_FULL if we're out of space on <b>out</b>.
 * Return QED_HS_COMPRESS_ERROR if the stream is corrupt.
 */
qed_hs_compress_output_t
qed_hs_compress_process(qed_hs_compress_state_t *state,
                     char **out, size_t *out_len,
                     const char **in, size_t *in_len,
                     int finish)
{
  qed_hs_assert(state != NULL);
  const size_t in_len_orig = *in_len;
  const size_t out_len_orig = *out_len;
  qed_hs_compress_output_t rv;
  ...
  if (*out_len == 0 && (*in_len > 0 || finish)) {
    ...
    // variable.
    return QED_HS_COMPRESS_BUFFER_FULL;
  }

  switch (state->method) {
    case GZIP_METHOD:
    case ZLIB_METHOD:
      rv = qed_hs_zlib_compress_process(state->u.zlib_state,
                                     out, out_len, in, in_len,
                                     finish);
      break;
    case LZMA_METHOD:
      rv = qed_hs_lzma_compress_process(state->u.lzma_state,
                                     out, out_len, in, in_len,
                                     finish);
      break;
    case ZSTD_METHOD:
      rv = qed_hs_zstd_compress_process(state->u.zstd_state,
                                     out, out_len, in, in_len,
                                     finish);
      break;
    case NO_METHOD:
      rv = qed_hs_cnone_compress_process(out, out_len, in, in_len,
                                      finish);
      break;
    default:
    case UNKNOWN_METHOD:
...
void
qed_hs_compress_free_(qed_hs_compress_state_t *state)
{
  if (state == NULL)
    return;
  ...
  switch (state->method) {
    case GZIP_METHOD:
    case ZLIB_METHOD:
      qed_hs_zlib_compress_free(state->u.zlib_state);
      break;
    case LZMA_METHOD:
      qed_hs_lzma_compress_free(state->u.lzma_state);
      break;
    case ZSTD_METHOD:
      qed_hs_zstd_compress_free(state->u.zstd_state);
      break;
    case NO_METHOD:
      break;
    case UNKNOWN_METHOD:
...
size_t
qed_hs_compress_state_size(const qed_hs_compress_state_t *state)
{
  qed_hs_assert(state != NULL);

  size_t size = sizeof(qed_hs_compress_state_t);

  switch (state->method) {
    case GZIP_METHOD:
    case ZLIB_METHOD:
      size += qed_hs_zlib_compress_state_size(state->u.zlib_state);
      break;
    case LZMA_METHOD:
      size += qed_hs_lzma_compress_state_size(state->u.lzma_state);
      break;
    case ZSTD_METHOD:
      size += qed_hs_zstd_compress_state_size(state->u.zstd_state);
      break;
    case NO_METHOD:
    case UNKNOWN_METHOD:
      break;
 ...
 *
 * (This isn't part of qed_hs_compress_init, since the logs aren't set up yet.)
 */
void
qed_hs_compress_log_init_warnings(void)
{
  // XXXX can we move this into qed_hs_compress_init() after all?  log.c queues
  // XXXX log messages at startup.
  qed_hs_zstd_warn_if_version_mismatched();
}

static int
subsys_compress_initialize(void)
{
  return qed_hs_compress_init();
}
