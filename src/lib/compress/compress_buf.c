...
int
buf_add_compress(buf_t *buf, qed_hs_compress_state_t *state,
                 ...
                 const int done)
{
  ...
  do {
    ...
    switch (qed_hs_compress_process(state, &next, &avail,
                                 &data, &data_len, done)) {
      ...
      case QED_HS_COMPRESS_BUFFER_FULL:
        if (avail) {
           ...
           * (QED_HS_COMPRESS_BUFFER_FULL).  Start a new chunk automatically,
