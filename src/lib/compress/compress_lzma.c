...
int
qed_hs_lzma_method_supported(void)
{
#ifdef HAVE_LZMA
  return 1;
#else
...
const char *
qed_hs_lzma_get_version_str(void)
{
#ifdef HAVE_LZMA
...
const char *
qed_hs_lzma_get_header_version_str(void)
{
#ifdef HAVE_LZMA
  return LZMA_VERSION_STRING;
#else
...
struct qed_hs_lzma_compress_state_t {
#ifdef HAVE_LZMA
...
static size_t
qed_hs_lzma_state_size_precalc(int compress, compression_level_t level)
{
  ...
  if (memory_usage + sizeof(qed_hs_lzma_compress_state_t) > SIZE_MAX)
    memory_usage = SIZE_MAX;
  else
    memory_usage += sizeof(qed_hs_lzma_compress_state_t);
...
/** Construct and return a qed_hs_lzma_compress_state_t object using
 * <b>method</b>. If <b>compress</b>, it's for compression; otherwise it's for
 * decompression. */
qed_hs_lzma_compress_state_t *
qed_hs_lzma_compress_new(int compress,
                      compress_method_t method,
                      compression_level_t level)
{
  qed_hs_assert(method == LZMA_METHOD);

#ifdef HAVE_LZMA
  qed_hs_lzma_compress_state_t *result;
  lzma_ret retval;
  ...
  // also what `qed_hs_malloc_zero()` does.
  result = qed_hs_malloc_zero(sizeof(qed_hs_lzma_compress_state_t));
  result->compress = compress;
  result->allocation = qed_hs_lzma_state_size_precalc(compress, level);
...
qed_hs_lzma_compress_process(qed_hs_lzma_compress_state_t *state,
...
void
qed_hs_lzma_compress_free_(qed_hs_lzma_compress_state_t *state)
{
  if (state == NULL)
    return;
...
size_t
qed_hs_lzma_compress_state_size(const qed_hs_lzma_compress_state_t *state)
{
  qed_hs_assert(state != NULL);
...
size_t
qed_hs_lzma_get_total_allocation(void)
{
