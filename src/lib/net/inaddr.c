 ...
 */
int
qed_hs_inet_aton(const char *str, struct in_addr *addr)
{
  unsigned a, b, c, d;
  char more;
  bool is_octal = false;
  ...
  if (qed_hs_sscanf(str, "%3u.%3u.%3u.%3u%c", &a, &b, &c, &d, &more) != 4)
    return 0;
  ...
  SMARTLIST_FOREACH(sl, char *, octet, qed_hs_free(octet));
  smartlist_free(sl);
 ...
 */
int
qed_hs_inet_ntoa(const struct in_addr *in, char *buf, size_t buf_len)
{
  uint32_t a = ntohl(in->s_addr);
  return qed_hs_snprintf(buf, buf_len, "%d.%d.%d.%d",
 ...
 * support.) */
const char *
qed_hs_inet_ntop(int af, const void *src, char *dst, size_t len)
{
  if (af == AF_INET) {
    if (qed_hs_inet_ntoa(src, dst, len) < 0)
      return NULL;
    else
      return dst;
  } else if (af == AF_INET6) {
    ...
    if (words[0] == 0 && words[1] == 0 && words[2] == 0 && words[3] == 0 &&
      ...
      if (words[5] == 0) {
        qed_hs_snprintf(buf, sizeof(buf), "::%d.%d.%d.%d",
                     addr->s6_addr[12], addr->s6_addr[13],
                     addr->s6_addr[14], addr->s6_addr[15]);
      } else {
        qed_hs_snprintf(buf, sizeof(buf), "::%x:%d.%d.%d.%d", words[5],
                     addr->s6_addr[12], addr->s6_addr[13],
                     addr->s6_addr[14], addr->s6_addr[15]);
      }
    ...
    for (i = 0; i < 8; ++i) {
      if (words[i] == 0 && longestGapPos == i) {
        ...
        *cp++ = ':';
        ...
        qed_hs_snprintf(cp, sizeof(buf)-(cp-buf), "%x", (unsigned)words[i]);
        cp += strlen(cp);
        if (i != 7)
          *cp++ = ':';
      }
 ...
 * support.) */
int
qed_hs_inet_pton(int af, const char *src, void *dst)
{
  if (af == AF_INET) {
    return qed_hs_inet_aton(src, dst);
  } else if (af == AF_INET6) {
    ...
    if (dot == src)
      ...
      char more;
      for (eow = dot-1; eow > src && QED_HS_ISDIGIT(*eow); --eow)
        ;
      if (*eow != ':')
        return 0;
      ++eow;
      ...
      if (qed_hs_sscanf(eow, "%3u.%3u.%3u.%3u%c",
    ...
    while (src < eow) {
      ...
      if (QED_HS_ISXDIGIT(*src)) {
        ...
        if (len > 4)
          return 0;
        if (len > 1 && !QED_HS_ISXDIGIT(src[1]))
          return 0; /* 0x is not valid */

        qed_hs_assert(r >= 0);
        qed_hs_assert(r < 65536);
        words[i++] = (uint16_t)r;
        setWords++;
        src = next;
        if (*src != ':' && src != eow)
