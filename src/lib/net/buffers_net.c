 ...
 * *<b>reached_eof</b> to 1. Uses <b>qed_hs_socket_recv()</b> iff <b>is_socket</b>
 ...
 * otherwise. */
static inline int
read_to_chunk(buf_t *buf, chunk_t *chunk, qed_hs_socket_t fd, size_t at_most,
              int *reached_eof, int *error, bool is_socket)
{
  ...
  if (is_socket)
    read_result = qed_hs_socket_recv(fd, CHUNK_WRITE_PTR(chunk), at_most, 0);
  else
    read_result = read(fd, CHUNK_WRITE_PTR(chunk), at_most);

  if (read_result < 0) {
    int e = is_socket ? qed_hs_socket_errno(fd) : errno;
    ...
    *reached_eof = 1;
    ...
    log_debug(LD_NET,"Read %ld bytes. %d on inbuf.", (long)read_result,
              (int)buf->datalen);
    qed_hs_assert(read_result <= BUF_MAX_LEN);
                 ...
                 bool is_socket)
{
  ...
  check();
  qed_hs_assert(reached_eof);
  qed_hs_assert(SOCKET_OK(fd));
 ...
 */
static inline int
flush_chunk(qed_hs_socket_t fd, buf_t *buf, chunk_t *chunk, size_t sz,
            bool is_socket)
{
  ...
  if (is_socket)
    write_result = qed_hs_socket_send(fd, chunk->data, sz, 0);
  else
  ...
  if (write_result < 0) {
    ...
    qed_hs_assert(write_result <= BUF_MAX_LEN);
 ...
 */
int
buf_flush_to_socket(buf_t *buf, qed_hs_socket_t s, size_t sz)
{
 ...
 */
int
buf_read_from_socket(buf_t *buf, qed_hs_socket_t s, size_t at_most,
                     int *reached_eof,
                     int *socket_error)
{
