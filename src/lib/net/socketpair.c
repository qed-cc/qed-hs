...
get_local_listener(int family, int type)
{
  ...
  qed_hs_socket_t sock = QED_HS_INVALID_SOCKET;
  ...
  if (family == AF_INET) {
    ...
    sin.sin_addr.s_addr = qed_hs_htonl(0x7f000001);
    len = sizeof(sin);
  } else {
 ...
 *
 * See qed_hs_socketpair() for details.
 *
 * The direct connection defaults to IPv4, but falls back to IPv6 if
 * IPv4 is not supported.
 **/
int
qed_hs_ersatz_socketpair(int family, int type, int protocol, qed_hs_socket_t fd[2])
{
   ...
   */
  qed_hs_socket_t listener = QED_HS_INVALID_SOCKET;
  qed_hs_socket_t connector = QED_HS_INVALID_SOCKET;
  qed_hs_socket_t acceptor = QED_HS_INVALID_SOCKET;
  ...
  if (getsockname(connector, connect_addr, &size) == -1)
    goto tidy_up_and_fail;
  /* Set *_qed_hs_addr and *_port to the address and port that was used */
