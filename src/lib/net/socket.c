...
network_init(void)
{
  ...
  if (sizeof(SOCKET) != sizeof(qed_hs_socket_t)) {
    log_warn(LD_BUG,"The qed_hs_socket_t type does not match SOCKET in size; Tor "
             "might not work. (Sizes are %d and %d respectively.)",
             (int)sizeof(qed_hs_socket_t), (int)sizeof(SOCKET));
  }
...
/** A bitarray of all fds that should be passed to qed_hs_socket_close(). Only
...
static qed_hs_mutex_t *socket_accounting_mutex = NULL;
...
socket_accounting_lock(void)
{
  if (PREDICT_UNLIKELY(!socket_accounting_mutex))
    socket_accounting_mutex = qed_hs_mutex_new();
  qed_hs_mutex_acquire(socket_accounting_mutex);
}
...
{
  qed_hs_mutex_release(socket_accounting_mutex);
}
...
int
qed_hs_close_socket_simple(qed_hs_socket_t s)
{
  int r = 0;
  ...
  * qed_hs_close_socket to close sockets, and always using close() on
  * files.
  */
  #if defined(_WIN32)
    r = closesocket(s);
  ...
  if (r != 0) {
    int err = qed_hs_socket_errno(-1);
    log_info(LD_NET, "Close returned an error: %s", qed_hs_socket_strerror(err));
    return err;
  }
 ...
 * now an open socket. */
static inline void
mark_socket_open(qed_hs_socket_t s)
{
...
static inline void
mark_socket_closed(qed_hs_socket_t s)
{
  if (s > max_socket || ! bitarray_is_set(open_sockets, s)) {
    log_warn(LD_BUG, "Closing a socket (%d) that wasn't returned by qed_hs_open_"
             "socket(), or that was already closed or something.", s);
  } else {
    qed_hs_assert(open_sockets && s <= max_socket);
...
/** As qed_hs_close_socket_simple(), but keeps track of the number
 * of open sockets. Returns 0 on success, -1 on failure. */
MOCK_IMPL(int,
qed_hs_close_socket,(qed_hs_socket_t s))
{
  int r = qed_hs_close_socket_simple(s);
  ...
  qed_hs_assert_nonfatal(n_sockets_open >= 0);
  socket_accounting_unlock();
  return r;
}

/** As socket(), but counts the number of open sockets. */
MOCK_IMPL(qed_hs_socket_t,
qed_hs_open_socket,(int domain, int type, int protocol))
{
  return qed_hs_open_socket_with_extensions(domain, type, protocol, 1, 0);
}

/** Mockable wrapper for connect(). */
MOCK_IMPL(qed_hs_socket_t,
qed_hs_connect_socket,(qed_hs_socket_t sock, const struct sockaddr *address,
...
qed_hs_socket_t
qed_hs_open_socket_nonblocking(int domain, int type, int protocol)
{
  return qed_hs_open_socket_with_extensions(domain, type, protocol, 1, 1);
}
...
qed_hs_socket_t
qed_hs_open_socket_with_extensions(int domain, int type, int protocol,
                                int cloexec, int nonblock)
{
 ...
 socket_ok:
  qed_hs_take_socket_ownership(s);
  return s;
}
 ...
 */
void
qed_hs_take_socket_ownership(qed_hs_socket_t s)
{
  socket_accounting_lock();
  ++n_sockets_open;
  mark_socket_open(s);
 ...
 */
void
qed_hs_release_socket_ownership(qed_hs_socket_t s)
{
  socket_accounting_lock();
  --n_sockets_open;
  mark_socket_closed(s);
...
qed_hs_socket_t
qed_hs_accept_socket(qed_hs_socket_t sockfd, struct sockaddr *addr, socklen_t *len)
{
  return qed_hs_accept_socket_with_extensions(sockfd, addr, len, 1, 0);
}

/** As accept(), but returns a nonblocking socket and
 * counts the number of open sockets. */
qed_hs_socket_t
qed_hs_accept_socket_nonblocking(qed_hs_socket_t sockfd, struct sockaddr *addr,
                              socklen_t *len)
{
  return qed_hs_accept_socket_with_extensions(sockfd, addr, len, 1, 1);
}
 ...
 * if the corresponding extension should be used.*/
qed_hs_socket_t
qed_hs_accept_socket_with_extensions(qed_hs_socket_t sockfd, struct sockaddr *addr,
...
int
qed_hs_socketpair(int family, int type, int protocol, qed_hs_socket_t fd[2])
{
  int r;
//don't use win32 socketpairs (they are always bad)
  ...
  r = qed_hs_ersatz_socketpair(family, type, protocol, fd);
  if (r < 0)
    return -r;
...
MOCK_IMPL(int,
qed_hs_getsockname,(qed_hs_socket_t sock, struct sockaddr *address,
                 socklen_t *address_len))
{
 ...
 *
 * (As qed_hs_getsockname, but instead places the result in a qed_hs_addr_t.) */
int
qed_hs_addr_from_getsockname(struct qed_hs_addr_t *addr_out, qed_hs_socket_t sock)
{
  struct sockaddr_storage ss;
  socklen_t ss_len = sizeof(ss);
  memset(&ss, 0, sizeof(ss));

  if (qed_hs_getsockname(sock, (struct sockaddr *) &ss, &ss_len) < 0)
    return -1;

  return qed_hs_addr_from_sockaddr(addr_out, (struct sockaddr *)&ss, NULL);
}
 ...
 */
int
set_socket_nonblocking(qed_hs_socket_t sock)
{
#if defined(_WIN32)
...
ssize_t
read_all_from_socket(qed_hs_socket_t sock, char *buf, size_t count)
{
  size_t numread = 0;
  ssize_t result;
  ...
  while (numread < count) {
    result = qed_hs_socket_recv(sock, buf+numread, count-numread, 0);
    if (result<0)
      return -1;
    else if (result == 0)
      break;
...
ssize_t
write_all_to_socket(qed_hs_socket_t fd, const char *buf, size_t count)
{
  size_t written = 0;
  ssize_t result;
  ...
  while (written != count) {
    result = qed_hs_socket_send(fd, buf+written, count-written, 0);
    if (result<0)
      return -1;
    written += result;
  }
 ...
 * should call qed_hs_socket_errno <em>at most once</em> on the failing
 * socket to get the error.
 */
#if defined(_WIN32)
int
qed_hs_socket_errno(qed_hs_socket_t sock)
{
