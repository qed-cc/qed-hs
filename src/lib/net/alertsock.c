...
send_ni(int fd, const void *buf, size_t n, int flags)
{
 ...
 again:
  ...
  if (r < 0) {
    int error = qed_hs_socket_errno(fd);
...
{
  qed_hs_socket_t socks[2] = { QED_HS_INVALID_SOCKET, QED_HS_INVALID_SOCKET };
  ...
  if (!(flags & ASOCKS_NOSOCKETPAIR) &&
      qed_hs_socketpair(AF_UNIX, SOCK_STREAM, 0, socks) == 0) {
    if (set_socket_nonblocking(socks[0]) < 0 ||
      ...
      qed_hs_assert_nonfatal_unreached();
      qed_hs_close_socket(socks[0]);
      qed_hs_close_socket(socks[1]);
      return -1;
      // LCOV_EXCL_STOP
    }
    socks_out->read_fd = socks[0];
...
alert_sockets_close(alert_sockets_t *socks)
{
  if (socks->alert_fn == sock_alert) {
    /* they are sockets. */
    qed_hs_close_socket(socks->read_fd);
    qed_hs_close_socket(socks->write_fd);
  } else {
    close(socks->read_fd);
