...
MOCK_IMPL(int,
qed_hs_lookup_hostname,(const char *name, uint32_t *addr))
{
  qed_hs_addr_t myaddr;
  int ret;
  ...
  *addr = 0;
  ...
  if ((ret = qed_hs_addr_lookup(name, AF_INET, &myaddr)))
    return ret;

  if (qed_hs_addr_family(&myaddr) == AF_INET) {
    *addr = qed_hs_addr_to_ipv4h(&myaddr);
    return ret;
  }
...
/* Host lookup helper for qed_hs_addr_lookup(), when getaddrinfo() is
 * available on this system.
 *
 * See qed_hs_addr_lookup() for details.
 */
MOCK_IMPL(STATIC int,
qed_hs_addr_lookup_host_impl,(const char *name,
                          uint16_t family,
                          qed_hs_addr_t *addr))
{
  int err;
  ...
  err = qed_hs_getaddrinfo(name, NULL, &hints, &res);
  ...
  if (!err && res) {
    ...
    if (best->ai_family == AF_INET) {
      qed_hs_addr_from_in(addr,
                       &((struct sockaddr_in*)best->ai_addr)->sin_addr);
      result = 0;
    } else if (best->ai_family == AF_INET6) {
      qed_hs_addr_from_in6(addr,
                        &((struct sockaddr_in6*)best->ai_addr)->sin6_addr);
      result = 0;
    }
    qed_hs_freeaddrinfo(res);
    return result;
  }
...
/* Host lookup helper for qed_hs_addr_lookup(), which calls gethostbyname().
                           ...
                           qed_hs_addr_t *addr))
{
  ...
  if (ent) {
    if (ent->h_addrtype == AF_INET) {
      qed_hs_addr_from_in(addr, (struct in_addr*) ent->h_addr);
    } else if (ent->h_addrtype == AF_INET6) {
      qed_hs_addr_from_in6(addr, (struct in6_addr*) ent->h_addr);
    } else {
      qed_hs_assert(0); // LCOV_EXCL_LINE: gethostbyname() returned bizarre type
    }
    return 0;
  }
#ifdef _WIN32
 ...
 *
 * Like qed_hs_addr_parse(), this function accepts IPv6 addresses with or without
 * square brackets.
 *
 * Return 0 on success, -1 on failure; 1 on transient failure.
 */
MOCK_IMPL(int,
qed_hs_addr_lookup,(const char *name, uint16_t family, qed_hs_addr_t *addr))
{
  /* Perhaps eventually this should be replaced by a qed_hs_getaddrinfo or
   * something.
   */
  int parsed_family = 0;
  int result = -1;
  ...
  qed_hs_assert(name);
  qed_hs_assert(addr);
  qed_hs_assert(family == AF_INET || family == AF_INET6 || family == AF_UNSPEC);
  ...
  /* Is it an IP address? */
  parsed_family = qed_hs_addr_parse(addr, name);
  ...
  if (parsed_family >= 0) {
    ...
    /* Clear the address after a failed qed_hs_addr_parse(). */
    memset(addr, 0, sizeof(qed_hs_addr_t));
    result = qed_hs_addr_lookup_host_impl(name, family, addr);
    goto done;
  }
...
int
qed_hs_addr_port_lookup(const char *s, qed_hs_addr_t *addr_out, uint16_t *port_out)
{
  qed_hs_addr_t addr;
  uint16_t portval = 0;
  char *tmp = NULL;
  ...
  rv = qed_hs_addr_port_parse(LOG_WARN, s, &addr, &portval, 0);
  /* That was easy, no DNS required. */
  if (rv == 0)
    goto success;

  /* Now let's check for malformed IPv6 addresses and ports:
   * qed_hs_addr_port_parse() requires squared brackes if there is a port,
   * and we want qed_hs_addr_port_lookup() to have the same requirement.
   * But we strip the port using qed_hs_addr_port_split(), so qed_hs_addr_lookup()
   * only sees the address, and will accept it without square brackets. */
  int family = qed_hs_addr_parse(&addr, s);
  /* If qed_hs_addr_parse() succeeds where qed_hs_addr_port_parse() failed, we need
   * to reject this address as malformed. */
  if (family >= 0) {
    /* Double-check it's an IPv6 address. If not, we have a parsing bug.
     */
    qed_hs_assertf_nonfatal(family == AF_INET6,
  ...
  rv = qed_hs_addr_port_split(LOG_WARN, s, &tmp, &portval);
  if (rv < 0)
    goto err;

  /* And feed the hostname to the lookup function. */
  if (qed_hs_addr_lookup(tmp, AF_UNSPEC, &addr) != 0)
    goto err;

 success:
  if (port_out)
    *port_out = portval;
  qed_hs_addr_copy(addr_out, &addr);
  result = 0;
  goto done;
 ...
 * qed-hs-resolve, when we have no intention of initializing crypto or of
...
void
qed_hs_freeaddrinfo(struct addrinfo *ai)
{
...
int
qed_hs_getaddrinfo(const char *name, const char *servname,
                ...
                struct addrinfo **res)
{
  ...
  *res = NULL;
  ...
  if (! sandbox_getaddrinfo_is_active) {
    ...
    if (! item) {
      item = qed_hs_malloc_zero(sizeof(*item));
      item->name = qed_hs_strdup(name);
...
int
qed_hs_add_addrinfo(const char *name)
{
  struct addrinfo *res;
  struct addrinfo hints;
  int i;
  ...
  for (i = 0; i < 3; ++i) {
    ...
    res = NULL;
    (void) qed_hs_getaddrinfo(name, NULL, &hints, &res);
    if (res)
      qed_hs_freeaddrinfo(res);
  }
...
void
qed_hs_free_getaddrinfo_cache(void)
{
...
void
qed_hs_make_getaddrinfo_cache_active(void)
{
