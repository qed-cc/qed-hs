 ...
 * log wrapper at src/rust/qed_hs_log) */
...
sev_to_string(int severity)
{
  switch (severity) {
    ...
    default:     /* Call raw_assert, not qed_hs_assert, since qed_hs_assert
...
should_log_function_name(log_domain_mask_t domain, int severity)
{
  switch (severity) {
    ...
    default:
      /* Call raw_assert, not qed_hs_assert, since qed_hs_assert calls
       * log on failure. */
...
static qed_hs_mutex_t log_mutex;
...
#define LOCK_LOGS() STMT_BEGIN                                          \
  ...
  qed_hs_mutex_acquire(&log_mutex);                                        \
  STMT_END
/** Unlock the log_mutex */
#define UNLOCK_LOGS() STMT_BEGIN                                        \
  raw_assert(log_mutex_initialized);                                    \
  qed_hs_mutex_release(&log_mutex);                                        \
  STMT_END
...
{
  qed_hs_free(appname);
  appname = name ? qed_hs_strdup(name) : NULL;
}
...
/**
 * As qed_hs_log, but takes an optional function name, and does not treat its
 * <b>string</b> as a printf format.
 *
 * For use by Rust integration.
 */
void
qed_hs_log_string(int severity, log_domain_mask_t domain,
...
set_log_time_granularity,(int granularity_msec))
{
  ...
  qed_hs_log_sigsafe_err_set_granularity(granularity_msec);
}
...
log_prefix_(char *buf, size_t buf_len, int severity)
{
  ...
  n = strftime(buf, buf_len, "%b %d %H:%M:%S",
               qed_hs_localtime_r_msg(&t, &tm, NULL));
  r = qed_hs_snprintf(buf+n, buf_len-n, ".%.3i [%s] ", ms,
 ...
 */
static int
log_qed_hs_version(logfile_t *lf, int reset)
{
  char buf[256];
  size_t n;
  int is_new;
  ...
  is_new = lf->fd >= 0 && qed_hs_fd_getpos(lf->fd) == 0;
  ...
  if (appname) {
    qed_hs_snprintf(buf+n, sizeof(buf)-n,
                 "%s opening %slog file.\n", appname, is_new?"new ":"");
  } else {
    qed_hs_snprintf(buf+n, sizeof(buf)-n,
           ...
           const char *format, va_list ap, size_t *msg_len_out)
{
  ...
  if (funcname && should_log_function_name(domain, severity)) {
    r = qed_hs_snprintf(buf+n, buf_len-n,
  ...
  r = qed_hs_vsnprintf(buf+n,buf_len-n,format,ap);
  if (r < 0) {
  ...
  if (domain == LD_BUG &&
      buf_len - n > strlen(qed_hs_bug_suffix)+1) {
    memcpy(buf+n, qed_hs_bug_suffix, strlen(qed_hs_bug_suffix));
    n += strlen(qed_hs_bug_suffix);
  }
...
{
  pending_log_message_t *m = qed_hs_malloc(sizeof(pending_log_message_t));
  m->severity = severity;
  m->domain = domain;
  m->fullmsg = fullmsg ? qed_hs_strdup(fullmsg) : NULL;
  m->msg = qed_hs_strdup(shortmsg);
  return m;
}
...
#warning "MAXLINE is very low; it might not be from syslog.h."
    ...
    if (msg_len >= MAXLINE)
      m = qed_hs_strndup(msg_after_prefix, MAXLINE-1);
    syslog(severity, "%s", m);
 ...
 * message.  The actual message is derived as from qed_hs_snprintf(format,ap).
 */
     ...
     const char *suffix, const char *format, va_list ap))
{
  ...
  /* Call raw_assert, not qed_hs_assert, since qed_hs_assert calls log on failure. */
  raw_assert(format);
 ...
 * */
void
qed_hs_log(int severity, log_domain_mask_t domain, const char *format, ...)
{
  va_list ap;
...
void
qed_hs_log_update_sigsafe_err_fds(void)
{
  const logfile_t *lf;
  ...
  int fds[QED_HS_SIGSAFE_LOG_MAX_FDS];
  int n_fds;
  ...
  for (lf = logfiles; lf; lf = lf->next) {
    ...
    if (lf->severities->masks[SEVERITY_MASK_IDX(LOG_ERR)] &
      ...
      fds[n_fds] = lf->fd;
      n_fds++;
      if (n_fds == QED_HS_SIGSAFE_LOG_MAX_FDS)
        break;
    }
  }
  ...
  if (!found_real_stderr &&
     ...
     * it. */
    raw_assert(n_fds >= 2); /* Don't qed_hs_assert inside log fns */
    --n_fds;
    fds[0] = fds[n_fds];
  }
  ...
  qed_hs_log_set_sigsafe_err_fds(fds, n_fds);
}
...
void
qed_hs_log_get_logfile_names(smartlist_t *out)
{
  logfile_t *lf;
  raw_assert(out);
...
log_free_(logfile_t *victim)
{
  if (!victim)
    return;
  qed_hs_free(victim->severities);
  qed_hs_free(victim->filename);
  qed_hs_free(victim);
}
...
{
  logfile_t *lf;
  lf = qed_hs_malloc_zero(sizeof(logfile_t));
  lf->fd = fd;
  lf->filename = qed_hs_strdup(name);
  lf->severities = qed_hs_memdup(severity, sizeof(log_severity_list_t));
  lf->next = logfiles;
...
init_logging(int disable_startup_queue)
{
  if (!log_mutex_initialized) {
    qed_hs_mutex_init(&log_mutex);
    qed_hs_bug_init_counter();
...
{
  log_severity_list_t *s = qed_hs_malloc_zero(sizeof(log_severity_list_t));
...
add_callback_log(const log_severity_list_t *severity, log_callback cb)
{
  ...
  lf->fd = -1;
  lf->severities = qed_hs_memdup(severity, sizeof(log_severity_list_t));
  lf->filename = qed_hs_strdup("<callback>");
  lf->callback = cb;
  lf->next = logfiles;
               ...
               const char* syslog_identity_tag)
{
  ...
  if (syslog_count++ == 0) {
    ...
    if (syslog_identity_tag) {
      qed_hs_snprintf(buf, sizeof(buf), "Tor-%s", syslog_identity_tag);
    } else {
      qed_hs_snprintf(buf, sizeof(buf), "Tor");
    }
  ...
  lf->fd = -1;
  lf->severities = qed_hs_memdup(severity, sizeof(log_severity_list_t));
  lf->filename = qed_hs_strdup("<syslog>");
  lf->is_syslog = 1;
...
domain_to_string(log_domain_mask_t domain, char *buf, size_t buflen)
{
  ...
  while (1) {
    const char *d;
    int bit = qed_hs_log2(domain);
    size_t n;
    if ((unsigned)bit >= ARRAY_LENGTH(domain_list)-1 ||
        bit >= N_LOGGING_DOMAINS) {
      qed_hs_snprintf(buf, buflen, "<BUG:Unknown domain %lx>", (long)domain);
      return buf+strlen(buf);
    }
    d = domain_list[bit];
    n = strlcpy(cp, d, eos-cp);
    if (n >= buflen) {
      qed_hs_snprintf(buf, buflen, "<BUG:Truncating domain %lx>", (long)domain);
      return buf+strlen(buf);
    }
    cp += n;
    domain &= ~(1<<bit);
                          ...
                          log_severity_list_t *severity_out)
{
  ...
  while (*cfg) {
    ...
    if (*cfg == '[') {
      ...
      SMARTLIST_FOREACH(domains_list, char *, d, qed_hs_free(d));
    ...
    dash = strchr(cfg, '-');
    if (dash && dash < space) {
      sev_lo = qed_hs_strndup(cfg, dash-cfg);
      sev_hi = qed_hs_strndup(dash+1, space-(dash+1));
    } else {
      sev_lo = qed_hs_strndup(cfg, space-cfg);
      sev_hi = qed_hs_strdup("ERR");
    }
