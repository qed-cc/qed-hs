...
esc_for_log(const char *s)
{
  ...
  for (cp = s; *cp; ++cp) {
    switch (*cp) {
      ...
      case '\t':
        ...
        break;
      default:
        if (QED_HS_ISPRINT(*cp) && ((uint8_t)*cp)<127)
          ++len;
        else
          len += 4;
        break;
  ...
  qed_hs_assert(len <= SSIZE_MAX);

  result = outp = qed_hs_malloc(len);
  *outp++ = '\"';
  for (cp = s; *cp; ++cp) {
    ...
    qed_hs_assert((outp-result) < (ssize_t)len-2);
    switch (*cp) {
      case '\\':
      case '\"':
      case '\'':
      ...
      default:
        if (QED_HS_ISPRINT(*cp) && ((uint8_t)*cp)<127) {
          *outp++ = *cp;
          ...
          qed_hs_assert((outp-result) < (ssize_t)len-4);
          qed_hs_snprintf(outp, 5, "\\%03o", (int)(uint8_t) *cp);
          outp += 4;
        }
        break;
    }
...
{
  char *string = qed_hs_strndup(chars, n);
