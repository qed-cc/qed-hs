...
void qed_hs_log(int severity, log_domain_mask_t domain, const char *format, ...)
  CHECK_PRINTF(3,4);

void qed_hs_log_update_sigsafe_err_fds(void);

struct smartlist_t;
void qed_hs_log_get_logfile_names(struct smartlist_t *out);

extern int log_global_min_severity_;

#ifdef QED_HS_COVERAGE
...
void qed_hs_log_string(int severity, log_domain_mask_t domain,
 ...
 *
 * C_RUST_COUPLED src/rust/qed_hs_log LogSeverity, LogDomain
 */
...
#if defined(LOG_PRIVATE) || defined(QED_HS_UNIT_TESTS)
/** Given a severity, yields an index into log_severity_list_t.masks to use
 * for that severity. */
#define SEVERITY_MASK_IDX(sev) ((sev) - LOG_ERR)
#endif

#endif /* !defined(QED_HS_TORLOG_H) */
