...
is_libc_at_least(int major, int minor)
{
  ...
  qed_hs_sscanf(version, "%d.%d", &libc_major, &libc_minor);
  if (libc_major > major)
    return 1;
...
new_element2(int syscall, char *value, char *value2)
{
  ...
  sandbox_cfg_t *elem = qed_hs_malloc_zero(sizeof(sandbox_cfg_t));
  param = elem->param = qed_hs_malloc_zero(sizeof(smp_param_t));
...
sigsys_debugging(int nr, siginfo_t *info, void *void_context)
{
  ...
  qed_hs_log_err_sigsafe("(Sandbox) Caught a bad syscall attempt (syscall ",
                      syscall_name,
                      ")\n",
                      NULL);
