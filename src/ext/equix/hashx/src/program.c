...
typedef struct generaqed_hs_ctx {
	int cycle;
	int sub_cycle;
	int mul_count;
	bool chain_mul;
...
static const instr_template* select_template(generaqed_hs_ctx* ctx, instr_type last_instr, int attempt) {
...
static bool select_destination(const instr_template* tpl, instruction* instr, generaqed_hs_ctx* ctx, int cycle) {
...
static bool select_source(const instr_template* tpl, instruction* instr, generaqed_hs_ctx* ctx, int cycle) {
	int available_regs[8];
	int regs_count = 0;
...
static int schedule_uop(execution_port uop, generaqed_hs_ctx* ctx, int cycle, bool commit) {
...
static int schedule_instr(const instr_template* tpl, generaqed_hs_ctx* ctx, bool commit) {
...
static void print_registers(const generaqed_hs_ctx* ctx) {
	for (int i = 0; i < 8; ++i) {
