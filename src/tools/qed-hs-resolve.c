...
#define log_sock_error(act, _s)                                         \
  STMT_BEGIN                                                            \
    ...
    qed_hs_socket_strerror(qed_hs_socket_errno(_s)));                         \
  STMT_END
...
{
  qed_hs_assert(out);
  qed_hs_assert(username);
  qed_hs_assert(hostname);
  ...
  output = qed_hs_malloc(encoded_len);
                             ...
                             int reverse)
{
  ...
  size_t addrlen;
  int ipv6;
  is_ip_address = qed_hs_addr_parse(&addr, hostname) != -1;
  if (!is_ip_address && reverse) {
    log_err(LD_GENERAL, "Tried to do a reverse lookup on a non-IP!");
    return -1;
  }
  ipv6 = reverse && qed_hs_addr_family(&addr) == AF_INET6;
  ...
  switch (atype) {
    case SOCKS5_ATYPE_IPV4: {
      socks5_client_request_set_dest_addr_ipv4(rq,
                                        qed_hs_addr_to_ipv4h(&addr));
    } break;
    case SOCKS5_ATYPE_IPV6: {
      uint8_t *ipv6_array =
        socks5_client_request_getarray_dest_addr_ipv6(rq);

      qed_hs_assert(ipv6_array);

      memcpy(ipv6_array, qed_hs_addr_to_in6_addr8(&addr), 16);
    } break;
  ...
  outbuf = qed_hs_malloc(encoded_len);
                            ...
                            int version)
{
  ...
  qed_hs_assert(version == 4 || version == 5);
                               ...
                               qed_hs_addr_t *addr_out)
{
  ...
  qed_hs_addr_from_ipv4h(addr_out, socks4_server_reply_get_addr(reply));
...
do_resolve(const char *hostname,
           const qed_hs_addr_t *sockshost, uint16_t socksport,
           int reverse, int version,
           qed_hs_addr_t *result_addr, char **result_hostname)
{
  int s = -1;
  ...
  qed_hs_assert(hostname);
  qed_hs_assert(result_addr);
  qed_hs_assert(version == 4 || version == 5);

  qed_hs_addr_make_unspec(result_addr);
  *result_hostname = NULL;

  s = qed_hs_open_socket(sockshost->family,SOCK_STREAM,IPPROTO_TCP);
  if (s<0) {
    log_sock_error("creating_socket", -1);
    return -1;
  }

  socklen = qed_hs_addr_to_sockaddr(sockshost, socksport,
  ...
  if (version == 4) {
    ...
    if (atype == SOCKS5_ATYPE_IPV4) {
      ...
      *result_hostname = qed_hs_strdup(domainname_getstr_name(dn));
    }
...
usage(void)
{
  puts("Syntax: qed-hs-resolve [-4] [-5] [-v] [-x] [-p port] "
       "hostname [sockshost[:socksport]]");
  exit(1);
}

/** Entry point to qed-hs-resolve */
int
main(int argc, char **argv)
{
  qed_hs_addr_t sockshost;
  uint16_t socksport = 0, port_option = 0;
  int isSocks4 = 0, isVerbose = 0, isReverse = 0;
  char **arg;
  int n_args;
  qed_hs_addr_t result;
  ...
  qed_hs_free(severities);

  if (n_args == 1) {
    log_debug(LD_CONFIG, "defaulting to localhost");
    qed_hs_addr_from_ipv4h(&sockshost, 0x7f000001u); /* localhost */
    if (port_option) {
    ...
    if (qed_hs_addr_port_lookup(arg[1], &sockshost, &socksport)<0) {
