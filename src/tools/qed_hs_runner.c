...
/**
 * @file qed_hs_runner.c
 * @brief Experimental module to emulate qed_hs_run_main() API with fork+exec
 *
 * The functions here are meant to allow the application developer to
 * use the qed_hs_run_main() API without having to care whether Tor is
 * running in-process or out-of-process.  For in-process usage, the
 * developer can link Tor as a library and call qed_hs_run_main(); for
...
static void child(const qed_hs_main_configuration_t *cfg)
  __attribute__((noreturn));

const char *
qed_hs_api_get_provider_version(void)
{
  return "libqed_hsrunner " VERSION;
}

int
qed_hs_run_main(const qed_hs_main_configuration_t *cfg)
{
  pid_t pid = fork();
  if (pid == 0) {
    child(cfg);
...
static void
child(const qed_hs_main_configuration_t *cfg)
{
