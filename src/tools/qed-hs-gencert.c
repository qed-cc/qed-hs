...
/** Write a usage message for qed-hs-gencert to stderr. */
static void
show_help(void)
{
  fprintf(stderr, "Syntax:\n"
          "qed-hs-gencert [-h|--help] [-v] [-r|--reuse] [--create-identity-key]\n"
...
load_passphrase(void)
{
  ...
  passphrase = qed_hs_strndup(buf, passphrase_len);
...
parse_commandline(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; ++i) {
    if (!strcmp(argv[i], "--help") || !strcmp(argv[i], "-h")) {
      ...
      identity_key_file = qed_hs_strdup(argv[++i]);
      ...
      signing_key_file = qed_hs_strdup(argv[++i]);
      ...
      certificate_file = qed_hs_strdup(argv[++i]);
      ...
      if (qed_hs_addr_port_lookup(addr_arg, &addr, &port)<0) {
      ...
      qed_hs_free(address);
      address = qed_hs_strdup(fmt_addrport(&addr, port));
  ...
  if (!identity_key_file) {
    identity_key_file = qed_hs_strdup("./authority_identity_key");
    log_info(LD_GENERAL, "No identity key file given; defaulting to %s",
             identity_key_file);
  }
  if (!signing_key_file) {
    signing_key_file = qed_hs_strdup("./authority_signing_key");
    log_info(LD_GENERAL, "No signing key file given; defaulting to %s",
             signing_key_file);
  }
  if (!certificate_file) {
    certificate_file = qed_hs_strdup("./authority_certificate");
    log_info(LD_GENERAL, "No signing key file given; defaulting to %s",
             certificate_file);
  }
  if (passphrase_fd >= 0) {
...
key_to_string(EVP_PKEY *key)
{
  ...
  result = qed_hs_malloc(buf->length + 1);
...
main(int argc, char **argv)
{
  ...
  qed_hs_free(address);
  qed_hs_free(identity_key_file);
  qed_hs_free(signing_key_file);
  qed_hs_free(certificate_file);
  qed_hs_free(address);
