...
#define SET_TEST_ADDRESS(i) do {                \
    if ((i) & 1) {                              \
      SET_TEST_IPV6(i);                         \
      qed_hs_addr_from_in6(&addr, &in6);           \
    } else {                                    \
      qed_hs_addr_from_ipv4h(&addr, (uint32_t) i); \
    }                                           \
  } while (0)
...
test_geoip_load_file(void *arg)
{
  ...
  dhex = qed_hs_strdup(hex_str((char*)d, DIGEST_LEN));
...
test_geoip6_load_file(void *arg)
{
  ...
  /* Any lookup attempt should say "-1" because we have no info */
  qed_hs_inet_pton(AF_INET6, "2001:4860:4860::8888", &iaddr6);
  ...
  qed_hs_inet_pton(AF_INET6, caddr6, &iaddr6);
  ...
  tt_int_op(1, OP_EQ, geoip_get_n_countries());
  tt_str_op("??", OP_EQ, geoip_get_country_name(0));
  qed_hs_inet_pton(AF_INET6, "::1:2:3:4", &iaddr6);
  tt_int_op(-1, OP_EQ, geoip_get_country_by_ipv6(&iaddr6));
  tt_str_op("0000000000000000000000000000000000000000", OP_EQ,
            geoip_db_digest(AF_INET6));
...
test_geoip_load_2nd_file(void *arg)
{
  ...
  char *fname_geoip = qed_hs_strdup(get_fname("geoip_data"));
  char *fname_empty = qed_hs_strdup(get_fname("geoip_empty"));
  ...
  qed_hs_free(fname_geoip);
  qed_hs_free(fname_empty);
}
