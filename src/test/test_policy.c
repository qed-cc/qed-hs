/* Copyright (c) 2013-2021, The QED Project, Inc. */
...
test_policies_general(void *arg)
{
  ...
  qed_hs_addr_from_ipv4h(&tar, 0xc0a80000u);
  tt_int_op(0,OP_EQ, qed_hs_addr_compare(&p->addr, &tar, CMP_EXACT));
  ...
  qed_hs_addr_from_ipv4h(&tar, 0x01020304u);
  tt_assert(ADDR_POLICY_ACCEPTED ==
          compare_qed_hs_addr_to_addr_policy(&tar, 2, policy));
  qed_hs_addr_make_unspec(&tar);
  tt_assert(ADDR_POLICY_PROBABLY_ACCEPTED ==
          compare_qed_hs_addr_to_addr_policy(&tar, 2, policy));
  qed_hs_addr_from_ipv4h(&tar, 0xc0a80102);
  ...
  qed_hs_addr_from_ipv4h(&tar, 0x0306090cu);
  qed_hs_addr_parse(&tar2, "[2000::1234]");
...
#define TT_BAD_SHORT_POLICY(s)                                          \
  ...
  /* Test a too-long policy. */
  {
    ...
    SMARTLIST_FOREACH(chunks, char *, ch, qed_hs_free(ch));
    smartlist_free(chunks);
    short_parsed = parse_short_policy(policy_strng);/* shouldn't be accepted */
    qed_hs_free(policy_strng);
  ...
  for (i=1; i<2000; i+=2) {
    ...
    qed_hs_snprintf(buf, sizeof(buf), "reject *:%d", i);
  ...
  qed_hs_free(policy_str);
  if (sm) {
    SMARTLIST_FOREACH(sm, char *, s, qed_hs_free(s));
    smartlist_free(sm);
  }
...
test_policies_reject_exit_address(void *arg)
{
  ...
  qed_hs_addr_t ipv4_addr, ipv6_addr;
  smartlist_t *ipv4_list, *ipv6_list, *both_list, *dupl_list;
  (void)arg;

  qed_hs_addr_parse(&ipv4_addr, TEST_IPV4_ADDR);
  qed_hs_addr_parse(&ipv6_addr, TEST_IPV6_ADDR);
...
test_policies_reject_port_address(void *arg)
{
  ...
  qed_hs_addr_parse(&ipv4_port->addr, TEST_IPV4_ADDR);
  smartlist_add(test_configured_ports, ipv4_port);

  ipv6_port = port_cfg_new(0);
  qed_hs_addr_parse(&ipv6_port->addr, TEST_IPV6_ADDR);
                            ...
                            int include_internal)
{
  ...
  SMARTLIST_FOREACH_BEGIN(template_list, qed_hs_addr_t *, src_addr) {
    qed_hs_addr_t *dest_addr = qed_hs_malloc(sizeof(qed_hs_addr_t));
    memset(dest_addr, 0, sizeof(*dest_addr));
    qed_hs_addr_copy_tight(dest_addr, src_addr);
...
test_policies_getinfo_helper_policies(void *arg)
{
  ...
  qed_hs_addr_parse(&mock_my_routerinfo.ipv4_addr, TEST_IPV4_ADDR);
  qed_hs_addr_parse(&mock_my_routerinfo.ipv6_addr, TEST_IPV6_ADDR);
  ...
  routerinfo_err = QED_HS_ROUTERINFO_ERROR_NO_EXT_ADDR;
  rv = getinfo_helper_policies(NULL, "exit-policy/full", &answer,
                               &errmsg);
  ...
  routerinfo_err = QED_HS_ROUTERINFO_ERROR_CANNOT_PARSE;
  rv = getinfo_helper_policies(NULL, "exit-policy/full", &answer,
                               &errmsg);
  tt_int_op(rv, OP_EQ, -1);
  tt_ptr_op(answer, OP_EQ, NULL);
  ...
  routerinfo_err = QED_HS_ROUTERINFO_ERROR_NOT_A_SERVER;
  rv = getinfo_helper_policies(NULL, "exit-policy/full", &answer,
                               &errmsg);
  tt_int_op(rv, OP_EQ, 0);
  tt_ptr_op(answer, OP_EQ, NULL);
  ...
  routerinfo_err = QED_HS_ROUTERINFO_ERROR_DIGEST_FAILED;
  rv = getinfo_helper_policies(NULL, "exit-policy/full", &answer,
                               &errmsg);
  ...
  routerinfo_err = QED_HS_ROUTERINFO_ERROR_CANNOT_GENERATE;
  rv = getinfo_helper_policies(NULL, "exit-policy/full", &answer,
                               &errmsg);
  tt_int_op(rv, OP_EQ, -1);
  tt_ptr_op(answer, OP_EQ, NULL);
  ...
  routerinfo_err = QED_HS_ROUTERINFO_ERROR_DESC_REBUILDING;
  rv = getinfo_helper_policies(NULL, "exit-policy/full", &answer,
                               &errmsg);
  tt_int_op(rv, OP_EQ, -1);
  tt_ptr_op(answer, OP_EQ, NULL);
...
{
  (void)arg;
  qed_hs_addr_t ipv4_addr, ipv6_addr, r_ipv4_addr, r_ipv6_addr;
  qed_hs_addr_t n_ipv4_addr, n_ipv6_addr;
  ...
  /* Parse the addresses */
  qed_hs_addr_parse(&ipv4_addr, TEST_IPV4_ADDR_STR);
  qed_hs_addr_parse(&ipv6_addr, TEST_IPV6_ADDR_STR);
  qed_hs_addr_parse(&r_ipv4_addr, OTHER_IPV4_ADDR_STR);
  qed_hs_addr_parse(&r_ipv6_addr, OTHER_IPV6_ADDR_STR);
  qed_hs_addr_make_null(&n_ipv4_addr, AF_INET);
  qed_hs_addr_make_null(&n_ipv6_addr, AF_INET6);
  ...
  /* NULL and qed_hs_addr_is_null addresses are rejected */
...
#define CHECK_CHOSEN_ADDR_RS(fake_rs, fw_connection, pref_only, expect_rv, \
                             expect_ap) \
  STMT_BEGIN \
    qed_hs_addr_port_t chosen_rs_ap; \
    qed_hs_addr_make_null(&chosen_rs_ap.addr, AF_INET); \
    chosen_rs_ap.port = 0; \
    reachable_addr_choose_from_rs(&(fake_rs), (fw_connection), \
                                       (pref_only), &chosen_rs_ap); \
    tt_assert(qed_hs_addr_eq(&(expect_ap).addr, &chosen_rs_ap.addr)); \
...
#define CHECK_CHOSEN_ADDR_NODE(fake_node, fw_connection, pref_only, \
                               expect_rv, expect_ap) \
  STMT_BEGIN \
    qed_hs_addr_port_t chosen_node_ap; \
    qed_hs_addr_make_null(&chosen_node_ap.addr, AF_INET); \
    chosen_node_ap.port = 0; \
    reachable_addr_choose_from_node(&(fake_node),(fw_connection), \
                                         (pref_only), &chosen_node_ap); \
    tt_assert(qed_hs_addr_eq(&(expect_ap).addr, &chosen_node_ap.addr)); \
...
#define CHECK_CHOSEN_ADDR_NULL_LS() \
  STMT_BEGIN \
    qed_hs_addr_port_t chosen_ls_ap; \
    qed_hs_addr_make_null(&chosen_ls_ap.addr, AF_UNSPEC); \
    chosen_ls_ap.port = 0; \
...
#define CHECK_CHOSEN_ADDR_LS(fake_ls, pref_only, expect_rv, expect_ap) \
  STMT_BEGIN \
    ...
    if (smartlist_len(fake_ls) == 0) { \
      ...
      expect_no_log_entry(); \
      tt_assert(qed_hs_addr_eq(&(expect_ap).addr, &chosen_ls_ap.addr)); \
...
#define CHECK_HS_EXTEND_INFO_ADDR_LS(fake_ls, direct_conn, expect_ap) \
  STMT_BEGIN \
    ...
    if (fake_ls == NULL) { \
      ...
      expect_no_log_entry(); \
      tt_assert(qed_hs_addr_eq(&(expect_ap).addr, &ei->orports[0].addr)); \
...
{
  (void)arg;
  qed_hs_addr_port_t ipv4_or_ap, ipv4_dir_ap, ipv6_or_ap, ipv6_dir_ap;
  qed_hs_addr_port_t n_ipv4_ap, n_ipv6_ap;
  ...
  /* Parse the addresses */
  qed_hs_addr_parse(&ipv4_or_ap.addr, TEST_IPV4_ADDR_STR);
  ipv4_or_ap.port = TEST_IPV4_OR_PORT;
  qed_hs_addr_parse(&ipv4_dir_ap.addr, TEST_IPV4_ADDR_STR);
  ipv4_dir_ap.port = TEST_IPV4_DIR_PORT;

  qed_hs_addr_parse(&ipv6_or_ap.addr, TEST_IPV6_ADDR_STR);
  ipv6_or_ap.port = TEST_IPV6_OR_PORT;
  qed_hs_addr_parse(&ipv6_dir_ap.addr, TEST_IPV6_ADDR_STR);
  ipv6_dir_ap.port = TEST_IPV6_DIR_PORT;

  qed_hs_addr_make_null(&n_ipv4_ap.addr, AF_INET);
  n_ipv4_ap.port = 0;
  qed_hs_addr_make_null(&n_ipv6_ap.addr, AF_INET6);
  n_ipv6_ap.port = 0;
  ...
  qed_hs_addr_copy(&fake_rs.ipv4_addr, &ipv4_or_ap.addr);
  fake_rs.ipv4_orport = ipv4_or_ap.port;
  fake_rs.ipv4_dirport = ipv4_dir_ap.port;

  qed_hs_addr_copy(&fake_rs.ipv6_addr, &ipv6_or_ap.addr);
  ...
  * find the node from the rs, because they're not in the hash table. */
  ...
  link_specifier_set_un_ipv4_addr(fake_ls,
                                  qed_hs_addr_to_ipv4h(&ipv4_or_ap.addr));
  ...
  const uint8_t *in6_addr = qed_hs_addr_to_in6_addr8(&ipv6_or_ap.addr);
  ...
  qed_hs_addr_port_t null_ap; \
  qed_hs_addr_make_null(&null_ap.addr, AF_UNSPEC); \
  null_ap.port = 0; \
