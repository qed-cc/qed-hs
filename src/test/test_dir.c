                     ...
                     crypto_pk_t **rsa_onion_keypair_out)
{
  ...
  routerinfo_t *r1 = qed_hs_malloc_zero(sizeof(routerinfo_t));

  r1->nickname = qed_hs_strdup(nickname);
  r1->platform = qed_hs_strdup(platform);

  qed_hs_addr_from_ipv4h(&r1->ipv4_addr, ipv4_addr);
  ...
  r1->protocol_list = qed_hs_strdup(protover_get_supported_protocols());
...
get_new_published_line(const routerinfo_t *r1)
{
  ...
  qed_hs_assert(r1->cache_info.published_on >= 0);
  qed_hs_assert(r1->cache_info.published_on <= 59);
...
get_new_fingerprint_line(const routerinfo_t *r1)
{
  ...
  qed_hs_assert(!crypto_pk_get_fingerprint(r1->identity_pkey, fingerprint, 1));
  qed_hs_assert(strlen(fingerprint) > 0);
...
get_new_rsa_key_block(const char *key_name, crypto_pk_t *pk1)
{
  ...
  qed_hs_assert(key_name);
  qed_hs_assert(pk1);

  qed_hs_assert(!crypto_pk_write_public_key_to_string(pk1, &pk1_str,
                                                   &pk1_str_len));
  qed_hs_assert(pk1_str);
  qed_hs_assert(pk1_str_len);

  qed_hs_asprintf(&block,
               "%s\n%s",
               key_name,
               pk1_str);
  qed_hs_free(pk1_str);
...
get_new_bridge_distribution_request_line(const or_options_t *options)
{
  if (options->BridgeRelay) {
    return qed_hs_strdup("bridge-distribution-request any\n");
  } else {
    return qed_hs_strdup("");
  }
}
...
static struct qed_hs_cert_st *mocked_signing_key_cert = NULL;

/* Returns mocked_signing_key_cert with no checks. */
static const struct qed_hs_cert_st *
...
cleanup_mock_configured_ports(void)
{
  ...
  if (mocked_configured_ports) {
    SMARTLIST_FOREACH(mocked_configured_ports, port_cfg_t *, p, qed_hs_free(p));
...
setup_mock_configured_ports(uint16_t or_port, uint16_t dir_port)
{
  ...
  if (or_port) {
    port_cfg_t *or_port_cfg = qed_hs_malloc_zero(sizeof(*or_port_cfg));
  ...
  if (dir_port) {
    port_cfg_t *dir_port_cfg = qed_hs_malloc_zero(sizeof(*dir_port_cfg));
...
{
  qed_hs_free(get_options_mutable()->Nickname);
                                 ...
                                 crypto_pk_t *rsa_onion_keypair)
{
  ...
  get_options_mutable()->Nickname = qed_hs_strdup(r1->nickname);
...
STMT_BEGIN \
  tt_assert(r1); \
  tt_assert(rp1); \
  tt_assert(qed_hs_addr_eq(&rp1->ipv4_addr, &r1->ipv4_addr)); \
...
STMT_BEGIN \
 ...
 \
  ...
  tt_assert(qed_hs_addr_is_null(&p->addr)); \
 ...
 \
  ...
  tt_assert(qed_hs_addr_eq(&p->addr, &ex2->addr)); \
...
test_dir_formats_rsa_ed25519(void *arg)
{
  ...
  r2->onion_curve25519_pkey = qed_hs_memdup(&r2_onion_keypair.pubkey,
  ...
  r2->cache_info.signing_key_cert = qed_hs_cert_create_ed25519(&kp1,
  ...
  ex1 = qed_hs_malloc_zero(sizeof(addr_policy_t));
  ex2 = qed_hs_malloc_zero(sizeof(addr_policy_t));
  ex1->policy_type = ADDR_POLICY_ACCEPT;
  qed_hs_addr_from_ipv4h(&ex1->addr, 0);
  ex1->maskbits = 0;
  ex1->prt_min = ex1->prt_max = 80;
  ex2->policy_type = ADDR_POLICY_REJECT;
  qed_hs_addr_from_ipv4h(&ex2->addr, 18<<24);
  ex2->maskbits = 8;
  ...
  SMARTLIST_FOREACH(chunks, char *, s, qed_hs_free(s));
  ...
  ed25519_keypair_generate(&family_2, 0);
  {
    ...
    smartlist_add(family_keys, qed_hs_memdup(&family_1, sizeof(family_1)));
    smartlist_add(family_keys, qed_hs_memdup(&family_2, sizeof(family_2)));
  ...
  tt_int_op(smartlist_len(rp2->family_ids), OP_EQ, 2);
 {
    ...
    qed_hs_snprintf(b, sizeof(b), "ed25519:%s", k);
...
test_dir_parse_router_list(void *arg)
{
  ...
  SMARTLIST_FOREACH(invalid, uint8_t *, dig, qed_hs_free(dig));
  ...
  SMARTLIST_FOREACH(chunks, char *, chunk, qed_hs_free(chunk));
  smartlist_free(chunks);
  routerinfo_free(ri);
  if (map) {
    digestmap_free_((digestmap_t*)map, routerinfo_free_wrapper_);
...
{
  qed_hs_version_t ver1;

  /* Try out version parsing functionality */
  (void)arg;
  tt_int_op(0,OP_EQ, qed_hs_version_parse("0.3.4pre2-cvs", &ver1));
  ...
  tt_int_op(0,OP_EQ, qed_hs_version_parse("0.3.4rc1", &ver1));
  tt_int_op(0,OP_EQ, ver1.major);
  tt_int_op(3,OP_EQ, ver1.minor);
  tt_int_op(4,OP_EQ, ver1.micro);
  tt_int_op(VER_RC,OP_EQ, ver1.status);
  tt_int_op(1,OP_EQ, ver1.patchlevel);
  tt_int_op(0,OP_EQ, qed_hs_version_parse("1.3.4", &ver1));
  tt_int_op(1,OP_EQ, ver1.major);
  tt_int_op(3,OP_EQ, ver1.minor);
  tt_int_op(4,OP_EQ, ver1.micro);
  tt_int_op(VER_RELEASE,OP_EQ, ver1.status);
  tt_int_op(0,OP_EQ, ver1.patchlevel);
  tt_int_op(0,OP_EQ, qed_hs_version_parse("1.3.4.999", &ver1));
  tt_int_op(1,OP_EQ, ver1.major);
  tt_int_op(3,OP_EQ, ver1.minor);
  tt_int_op(4,OP_EQ, ver1.micro);
  tt_int_op(VER_RELEASE,OP_EQ, ver1.status);
  tt_int_op(999,OP_EQ, ver1.patchlevel);
  tt_int_op(0,OP_EQ, qed_hs_version_parse("0.1.2.4-alpha", &ver1));
  tt_int_op(0,OP_EQ, ver1.major);
  ...
  tt_int_op(0,OP_EQ, qed_hs_version_parse("0.1.2.4", &ver1));
  tt_int_op(0,OP_EQ, ver1.major);
  tt_int_op(1,OP_EQ, ver1.minor);
  tt_int_op(2,OP_EQ, ver1.micro);
  tt_int_op(4,OP_EQ, ver1.patchlevel);
  ...
  tt_int_op(0, OP_EQ, qed_hs_version_parse("10.1", &ver1));
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("", OP_EQ, ver1.status_tag);
  tt_int_op(0, OP_EQ, qed_hs_version_parse("5.99.999", &ver1));
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("", OP_EQ, ver1.status_tag);
  tt_int_op(0, OP_EQ, qed_hs_version_parse("10.1-alpha", &ver1));
  tt_int_op(10, OP_EQ, ver1.major);
  tt_int_op(1, OP_EQ, ver1.minor);
  tt_int_op(0, OP_EQ, ver1.micro);
  tt_int_op(0, OP_EQ, ver1.patchlevel);
  ...
  tt_int_op(0, OP_EQ, qed_hs_version_parse("2.1.700-alpha", &ver1));
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("alpha", OP_EQ, ver1.status_tag);
  tt_int_op(0, OP_EQ, qed_hs_version_parse("1.6.8-alpha-dev", &ver1));
  tt_int_op(1, OP_EQ, ver1.major);
  ...
  tt_int_op(0, OP_EQ, qed_hs_version_parse("0.2.9.5-rc", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  ...
  tt_int_op(0, OP_EQ, qed_hs_version_parse("0.2.9.6-rc-dev", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  tt_int_op(2, OP_EQ, ver1.minor);
  tt_int_op(9, OP_EQ, ver1.micro);
  ...
  tt_int_op(0, OP_EQ, qed_hs_version_parse("0.2.9.8", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  tt_int_op(2, OP_EQ, ver1.minor);
  tt_int_op(9, OP_EQ, ver1.micro);
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("", OP_EQ, ver1.status_tag);
  tt_int_op(0, OP_EQ, qed_hs_version_parse("0.2.9.9-dev", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  tt_int_op(2, OP_EQ, ver1.minor);
  tt_int_op(9, OP_EQ, ver1.micro);
   ...
   * between i386 and x86_64, as we used qed_hs_parse_long, and then cast to int
   */
  tt_int_op(0, OP_EQ, qed_hs_version_parse("0.2147483647.0", &ver1));
  tt_int_op(0, OP_EQ, ver1.major);
  ...
  tt_int_op(VER_RELEASE, OP_EQ, ver1.status);
  tt_str_op("", OP_EQ, ver1.status_tag);
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.2147483648.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.4294967295.0", &ver1));
  /* In #21278, we reject negative version components */
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.-1.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.-2147483648.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.-4294967295.0", &ver1));
  /* In #21507, we reject version components with non-numeric prefixes */
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.-0.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("+1.0.0", &ver1));
  /* use the list in isspace() */
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.\t0.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.\n0.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.\v0.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.\f0.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0.\r0.0", &ver1));
  tt_int_op(-1, OP_EQ, qed_hs_version_parse("0. 0.0", &ver1));
...
#define test_v_i_o(val, ver, lst)                                       \
  tt_versionstatus_op(val, OP_EQ, qed_hs_version_is_obsolete(ver, lst))

  /* make sure qed_hs_version_is_obsolete() works */
  ...
  tt_int_op(0,OP_EQ, qed_hs_version_as_new_as("Tor 0.0.5", "0.0.9pre1-cvs"));
  tt_int_op(1,OP_EQ, qed_hs_version_as_new_as(
          "Tor 0.0.8 on Darwin 64-121-192-100.c3-0."
          "sfpo-ubr1.sfrn-sfpo.ca.cable.rcn.com Power Macintosh",
          "0.0.8rc2"));
  tt_int_op(0,OP_EQ, qed_hs_version_as_new_as(
          "Tor 0.0.8 on Darwin 64-121-192-100.c3-0."
          "sfpo-ubr1.sfrn-sfpo.ca.cable.rcn.com Power Macintosh", "0.0.8.2"));

  /* Now try svn revisions. */
  tt_int_op(1,OP_EQ, qed_hs_version_as_new_as("Tor 0.2.1.0-dev (r100)",
                                   "Tor 0.2.1.0-dev (r99)"));
  tt_int_op(1,OP_EQ, qed_hs_version_as_new_as(
  ...
  tt_int_op(0,OP_EQ, qed_hs_version_as_new_as("Tor 0.2.1.0-dev (r99)",
                                   "Tor 0.2.1.0-dev (r100)"));
  tt_int_op(0,OP_EQ, qed_hs_version_as_new_as("Tor 0.2.1.0-dev (r99) on MCP",
                                   "Tor 0.2.1.0-dev (r100) on AM"));
  tt_int_op(0,OP_EQ, qed_hs_version_as_new_as("Tor 0.2.1.0-dev",
                                   "Tor 0.2.1.0-dev (r99)"));
  tt_int_op(1,OP_EQ, qed_hs_version_as_new_as("Tor 0.2.1.1",
                                   "Tor 0.2.1.0-dev (r99)"));
  /* And git revisions */
  tt_int_op(1,OP_EQ, qed_hs_version_as_new_as(
   ...
   * longer possible to cause an integer overflow in qed_hs_version_compare() */
  tt_int_op(0,OP_EQ, qed_hs_version_as_new_as(
                                           "Tor 0.0.0.0",
  ...
  tt_int_op(0,OP_EQ, qed_hs_version_parse("0.5.6.7 (git-ff00ff)", &ver1));
  tt_int_op(0,OP_EQ, ver1.major);
  ...
  tt_int_op(-1,OP_EQ, qed_hs_version_parse("0.5.6.7 (git-ff00xx)", &ver1));
  /* reject odd hex digit count */
  tt_int_op(-1,OP_EQ, qed_hs_version_parse("0.5.6.7 (git-ff00fff)", &ver1));
  /* ignore "git " */
  tt_int_op(0,OP_EQ, qed_hs_version_parse("0.5.6.7 (git ff00fff)", &ver1));
  /* standard length is 16 hex digits */
  tt_int_op(0,OP_EQ, qed_hs_version_parse("0.5.6.7 (git-0010203040506070)",
                                       &ver1));
  /* length limit is 40 hex digits */
  tt_int_op(0,OP_EQ, qed_hs_version_parse(
                     "0.5.6.7 (git-000102030405060708090a0b0c0d0e0f10111213)",
                     &ver1));
  tt_int_op(-1,OP_EQ, qed_hs_version_parse(
...
test_dir_fp_pairs(void *arg)
{
  ...
  SMARTLIST_FOREACH(sl, fp_pair_t *, pair_to_free, qed_hs_free(pair_to_free));
  smartlist_free(sl);
}
...
test_dir_dirserv_read_measured_bandwidths(void *arg)
{
  ...
  char *fname = qed_hs_strdup(get_fname("V3BandwidthsFile"));
  ...
  int i;
  const char *header_lines_v110_no_terminaqed_hs_no_timestamp =
    "version=1.1.0\n"
    "software=sbws\n"
    "software_version=0.1.0\n"
    "earliest_bandwidth=2018-05-08T16:13:26\n"
    "file_created=2018-04-16T21:49:18\n"
    "generaqed_hs_started=2018-05-08T16:13:25\n"
  ...
  qed_hs_asprintf(&header_lines_v100, "%ld\n", (long)timestamp);
  qed_hs_asprintf(&header_lines_v110_no_terminator, "%ld\n%s", (long)timestamp,
               header_lines_v110_no_terminaqed_hs_no_timestamp);
  qed_hs_asprintf(&header_lines_v110, "%s%s",
               header_lines_v110_no_terminator, BW_FILE_HEADERS_TERMINATOR);

  qed_hs_asprintf(&bw_file_headers_str_v100, "timestamp=%ld",(long)timestamp);
  qed_hs_asprintf(&bw_file_headers_str_v110, "timestamp=%ld %s",
               (long)timestamp, bw_file_headers_str_v110_no_timestamp);
  qed_hs_asprintf(&bw_file_headers_str_bad, "%s "
               "node_id=$68A483E05A2ABDCA6DA5A3EF8DB5177638A",
  ...
  strlcat(bw_file_headers_str_long, "foo=bar",
          sizeof(bw_file_headers_str_long));
  qed_hs_asprintf(&bw_file_headers_str_extra,
               "%s %s",
               bw_file_headers_str_v110,
  ...
  bw_file_headers = smartlist_new();
  qed_hs_asprintf(&content, "%s%s", header_lines_v100, relay_lines_v100);
  write_str_to_file(fname, content, 0);
  qed_hs_free(content);
  tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL,
                                                       bw_file_headers,
  ...
  bw_file_headers = smartlist_new();
  qed_hs_asprintf(&content, "%s%s%s", header_lines_v100, header_lines_v110,
               relay_lines_v100);
  write_str_to_file(fname, content, 0);
  qed_hs_free(content);
  tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL,
  ...
  bw_file_headers = smartlist_new();
  qed_hs_asprintf(&content, "%s%s%s", header_lines_v100, relay_lines_v100,
               header_lines_v110);
  write_str_to_file(fname, content, 0);
  qed_hs_free(content);
  tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL,
  ...
  bw_file_headers = smartlist_new();
  qed_hs_asprintf(&content, "%s%s%s%s", header_lines_v100, relay_lines_v100,
   ...
   * bw_file_headers will contain the v1.1.0 headers. */
  bw_file_headers = smartlist_new();
  qed_hs_asprintf(&content, "%s%s%s%s", header_lines_v110, relay_lines_bad,
  ...
  unlink(fname);
  qed_hs_free(fname);
  qed_hs_free(header_lines_v100);
  qed_hs_free(header_lines_v110_no_terminator);
  qed_hs_free(header_lines_v110);
  qed_hs_free(bw_file_headers_str_v100);
  qed_hs_free(bw_file_headers_str_v110);
  qed_hs_free(bw_file_headers_str_bad);
  qed_hs_free(bw_file_headers_str_extra);
}
                          ...
                          int total_authorities)
{
  ...
  SMARTLIST_FOREACH(s, char *, cp, qed_hs_free(cp));
  smartlist_free(s);
  return res;
}
...
test_dir_param_voting(void *arg)
{
  ...
  qed_hs_free(res);
  SMARTLIST_FOREACH(vote1.net_params, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(vote2.net_params, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(vote3.net_params, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(vote4.net_params, char *, cp, qed_hs_free(cp));
...
test_dir_param_voting_lookup(void *arg)
{
  ...
  qed_hs_capture_bugs_(1);
  tt_int_op(-100, OP_EQ,
            dirvote_get_intermediate_param_value(lst, "moomin", -100));
  tt_int_op(smartlist_len(qed_hs_get_captured_bug_log_()), OP_EQ, 1);
  tt_str_op(smartlist_get(qed_hs_get_captured_bug_log_(), 0), OP_EQ,
            "n_found == 0");
  ...
  SMARTLIST_FOREACH(lst, char *, cp, qed_hs_free(cp));
  smartlist_free(lst);
  qed_hs_end_capture_bugs_();
}
                ...
                networkstatus_voter_info_t *v2)
{
  ...
  tt_assert(qed_hs_addr_eq(&v1->ipv4_addr, &v2->ipv4_addr));
...
get_detached_sigs(networkstatus_t *ns, networkstatus_t *ns2)
{
  char *r;
  smartlist_t *sl;
  qed_hs_assert(ns && ns->flavor == FLAV_NS);
  sl = smartlist_new();
  smartlist_add(sl,ns);
  if (ns2)
...
test_vrs_for_v3ns(vote_routerstatus_t *vrs, int voter, time_t now)
{
  ...
  if (qed_hs_memeq(rs->identity_digest,
  ...
  } else if (qed_hs_memeq(rs->identity_digest,
    ...
    qed_hs_addr_parse(&addr_ipv6, "[1:2:3::4]");
    tt_assert(qed_hs_addr_eq(&rs->ipv6_addr, &addr_ipv6));
    ...
    void (*rs_test)(routerstatus_t *rs, time_t now))
{
  ...
  tt_assert(qed_hs_addr_eq_ipv4h(&voter->ipv4_addr, 0x01020304));
  ...
  qed_hs_free(cp);
  smartlist_free(votes);
  qed_hs_free(consensus_text);
  qed_hs_free(consensus_text_md);
  ...
  qed_hs_free(consensus_text2);
  qed_hs_free(consensus_text3);
  qed_hs_free(consensus_text_md2);
  qed_hs_free(consensus_text_md3);
  qed_hs_free(detached_text1);
  qed_hs_free(detached_text2);
...
gen_routerstatus_for_umbw(int idx, time_t now)
{
  ...
  switch (idx) {
    case 0:
      ...
      vrs = qed_hs_malloc_zero(sizeof(vote_routerstatus_t));
      rs = &vrs->status;
      ...
      vrs->protocols = qed_hs_strdup("Link=2 Wombat=40");
      break;
    case 1:
      /* Generate the second routerstatus. */
      vrs = qed_hs_malloc_zero(sizeof(vote_routerstatus_t));
      rs = &vrs->status;
      vrs->version = qed_hs_strdup("0.2.0.5");
      ...
      rs->ipv4_dirport = 0;
      qed_hs_addr_parse(&addr_ipv6, "[1:2:3::4]");
      qed_hs_addr_copy(&rs->ipv6_addr, &addr_ipv6);
      rs->ipv6_orport = 4711;
    ...
    case 2:
      ...
      vrs = qed_hs_malloc_zero(sizeof(vote_routerstatus_t));
      rs = &vrs->status;
      vrs->version = qed_hs_strdup("0.1.0.3");
      vrs->published_on = now-1000;
    ...
    case 3:
      ...
      vrs = qed_hs_malloc_zero(sizeof(vote_routerstatus_t));
      rs = &vrs->status;
      vrs->version = qed_hs_strdup("0.1.6.3");
      vrs->published_on = now-1000;
  ...
  if (vrs) {
    vrs->microdesc = qed_hs_malloc_zero(sizeof(vote_microdesc_hash_t));
    qed_hs_asprintf(&vrs->microdesc->microdesc_hash_line,
                 "m 32,33 "
...
vote_tweaks_for_umbw(networkstatus_t *v, int voter, time_t now)
{
  ...
  SMARTLIST_FOREACH(v->supported_methods, char *, c, qed_hs_free(c));
  ...
  if (alternate_clip_bw > 0) {
    qed_hs_asprintf(&maxbw_param, "maxunmeasuredbw=%u", alternate_clip_bw);
                   ...
                   uint32_t ipv4_addr)
{
  ...
  qed_hs_addr_from_ipv4h(&rs->ipv4_addr, ipv4_addr);
}
...
test_dir_dirserv_set_routerstatus_testing(void *arg)
{
  ...
  dirauth_options_t *dirauth_options =
    qed_hs_malloc_zero(sizeof(dirauth_options_t));

  mock_options = qed_hs_malloc(sizeof(or_options_t));
  ...
  /* Init routerstatuses */
  routerstatus_t *rs_a = qed_hs_malloc(sizeof(routerstatus_t));
  reset_routerstatus(rs_a, ROUTER_A_ID_STR, ROUTER_A_IPV4);

  routerstatus_t *rs_b = qed_hs_malloc(sizeof(routerstatus_t));
  ...
  qed_hs_free(mock_options);
  qed_hs_free(dirauth_options);
  mock_options = NULL;
...
test_dir_http_handling(void *args)
{
  ...
  /* Good headers */
  tt_int_op(parse_http_url("GET /qed-hs/a/b/c.txt HTTP/1.1\r\n"
                           "Host: example.com\r\n"
                           "User-Agent: Mozilla/5.0 (Windows;"
                           " U; Windows NT 6.1; en-US; rv:1.9.1.5)\r\n",
                           &url),OP_EQ, 0);
  tt_str_op(url,OP_EQ, "/qed-hs/a/b/c.txt");
  qed_hs_free(url);

  tt_int_op(parse_http_url("GET /qed-hs/a/b/c.txt HTTP/1.0\r\n", &url),OP_EQ, 0);
  tt_str_op(url,OP_EQ, "/qed-hs/a/b/c.txt");
  qed_hs_free(url);

  tt_int_op(parse_http_url("GET /qed-hs/a/b/c.txt HTTP/1.600\r\n", &url),
            OP_EQ, 0);
  tt_str_op(url,OP_EQ, "/qed-hs/a/b/c.txt");
  qed_hs_free(url);

  /* Should prepend '/qed-hs/' to url if required */
  ...
  tt_int_op(parse_http_url("GET /qed-hs/a/b/c.txt", &url),OP_EQ, -1);
  tt_ptr_op(url, OP_EQ, NULL);

  tt_int_op(parse_http_url("GET /qed-hs/a/b/c.txt HTTP/1.1", &url),OP_EQ, -1);
  tt_ptr_op(url, OP_EQ, NULL);

  tt_int_op(parse_http_url("GET /qed-hs/a/b/c.txt HTTP/1.1x\r\n", &url),
            OP_EQ, -1);
  tt_ptr_op(url, OP_EQ, NULL);

  tt_int_op(parse_http_url("GET /qed-hs/a/b/c.txt HTTP/1.", &url),OP_EQ, -1);
  tt_ptr_op(url, OP_EQ, NULL);

  tt_int_op(parse_http_url("GET /qed-hs/a/b/c.txt HTTP/1.\r", &url),OP_EQ, -1);
  tt_ptr_op(url, OP_EQ, NULL);
...
test_dir_purpose_needs_anonymity_returns_true_by_default(void *arg)
{
  ...
  tt_int_op(1, OP_EQ, smartlist_len(qed_hs_get_captured_bug_log_()));
...
#define GOOD(s) \
  ...
  smartlist_add(votes, qed_hs_malloc_zero(sizeof(networkstatus_t)));
  smartlist_add(votes, qed_hs_malloc_zero(sizeof(networkstatus_t)));
  smartlist_add(votes, qed_hs_malloc_zero(sizeof(networkstatus_t)));
  smartlist_add(votes, qed_hs_malloc_zero(sizeof(networkstatus_t)));
  smartlist_add(votes, qed_hs_malloc_zero(sizeof(networkstatus_t)));
...
#undef GOOD
  ...
  SMARTLIST_FOREACH(votes, networkstatus_t *, ns,
                    { smartlist_free(ns->package_lines); qed_hs_free(ns); });
  smartlist_free(votes);
  qed_hs_free(res);
}
                       ...
                       const char *suffix)
{
  ...
  if (sub2) {
    ...
    rv = qed_hs_strdup(sub2);
  } else {
    rv = qed_hs_strdup(sub1);
  }
...
{
  qed_hs_free(last_unlinked_path);
  unlinked_count = 0;
}
...
mock_unlink(const char *path)
{
  ...
  qed_hs_free(last_unlinked_path);
  last_unlinked_path = qed_hs_strdup(path);
  ++unlinked_count;
...
{
  qed_hs_free(last_write_str_path);
  write_str_count = 0;
}
...
mock_write_str_to_file(const char *path, const char *str, int bin)
{
  ...
  qed_hs_free(last_write_str_path);
  last_write_str_path = qed_hs_strdup(path);
...
test_dir_dump_unparseable_descriptors(void *data)
{
   ...
   */
  MOCK(qed_hs_unlink, mock_unlink);
  mock_unlink_reset();
...
{
  qed_hs_free(expected_filename);
  qed_hs_free(file_content);
  file_content_len = 0;
...
read_file_to_str_mock(const char *filename, int flags,
  ...
  if (expected_filename != NULL &&
     ...
     */
    result = qed_hs_malloc(file_content_len + 1);
...
test_dir_populate_dump_desc_fifo(void *data)
{
  ...
  qed_hs_asprintf(&expected_filename, "%s%s%s", dirname, PATH_SEPARATOR, fname);
  file_content = qed_hs_strdup("hanc culpam maiorem an illam dicam?");
...
pop_one_mock(const char *dirname, const char *f)
{
  ...
  if (dirname != NULL && strcmp(dirname, "d") == 0) {
    if (f != NULL && strcmp(f, "foo") == 0) {
      ent = qed_hs_malloc_zero(sizeof(*ent));
      ent->filename = qed_hs_strdup("d/foo");
      ent->len = 123;
      ent->digest_sha256[0] = 1;
      ent->when = 1024;
    } else if (f != NULL && strcmp(f, "bar") == 0) {
      ent = qed_hs_malloc_zero(sizeof(*ent));
      ent->filename = qed_hs_strdup("d/bar");
      ent->len = 456;
      ...
      ent = qed_hs_malloc_zero(sizeof(*ent));
      ent->filename = qed_hs_strdup("d/baz");
      ent->len = 789;
...
test_dir_populate_dump_desc_fifo_2(void *data)
{
  ...
  /* Set up the mocks */
  MOCK(qed_hs_listdir, listdir_mock);
...
test_dir_post_parsing(void *arg)
{
  ...
  {
    const char *end;
    const char *prefix = "/qed-hs/hs/";
    int version = parse_hs_version_from_post("/qed-hs/hs//publish", prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/qed-hs/hs/a/publish", prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/qed-hs/hs/3/publish", prefix, &end);
    tt_int_op(version, OP_EQ, 3);
    tt_str_op(end, OP_EQ, "/publish");
    version = parse_hs_version_from_post("/qed-hs/hs/42/publish", prefix, &end);
    tt_int_op(version, OP_EQ, 42);
    tt_str_op(end, OP_EQ, "/publish");
    version = parse_hs_version_from_post("/qed-hs/hs/18163/publish",prefix, &end);
    tt_int_op(version, OP_EQ, 18163);
    tt_str_op(end, OP_EQ, "/publish");
    version = parse_hs_version_from_post("JUNKJUNKJUNK", prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/qed-hs/hs/3/publish", "blah", &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    /* Missing the '/' at the end of the prefix. */
    version = parse_hs_version_from_post("/qed-hs/hs/3/publish", "/qed-hs/hs", &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/random/blah/qed-hs/hs/3/publish",
                                         prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    version = parse_hs_version_from_post("/qed-hs/hs/3/publish/random/junk",
                                         prefix, &end);
    tt_int_op(version, OP_EQ, 3);
    tt_str_op(end, OP_EQ, "/publish/random/junk");
    version = parse_hs_version_from_post("/qed-hs/hs/-1/publish", prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
    /* INT_MAX */
    version = parse_hs_version_from_post("/qed-hs/hs/2147483647/publish",
                                         prefix, &end);
    tt_int_op(version, OP_EQ, INT_MAX);
    tt_str_op(end, OP_EQ, "/publish");
    /* INT_MAX + 1*/
    version = parse_hs_version_from_post("/qed-hs/hs/2147483648/publish",
                                         prefix, &end);
    tt_int_op(version, OP_EQ, -1);
    tt_ptr_op(end, OP_EQ, NULL);
  }
...
{
  (void)arg;
  char *fname = qed_hs_strdup(get_fname("approved-routers"));
...
test_dir_dirserv_router_get_status(void *arg)
{
  ...
  ri = qed_hs_malloc_zero(sizeof(routerinfo_t));
  qed_hs_addr_from_ipv4h(&ri->ipv4_addr, 0xc0a80001u);
  ri->ipv4_orport = 9001;
  ri->platform = qed_hs_strdup("0.4.0.1-alpha");
  ri->nickname = qed_hs_strdup("Jessica");
  ri->identity_pkey = crypto_pk_dup_key(pk);

  curve25519_keypair_t ri_onion_keypair;
  curve25519_keypair_generate(&ri_onion_keypair, 0);
  ri->onion_curve25519_pkey = qed_hs_memdup(&ri_onion_keypair.pubkey,
                                         sizeof(curve25519_public_key_t));
  ...
  ed25519_secret_key_from_seed(&kp2.seckey,
                          (const uint8_t*)"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
  ed25519_public_key_generate(&kp2.pubkey, &kp2.seckey);
  ri->cache_info.signing_key_cert = qed_hs_cert_create_ed25519(&kp1,
                                         CERT_TYPE_ID_SIGNING,
                                         &kp2.pubkey,
                                         now, 86400,
                                         CERT_FLAG_INCLUDE_SIGNING_KEY);
...
test_dir_dirserv_would_reject_router(void *arg)
{
  ...
  memset(&rs, 0, sizeof(rs));
  qed_hs_addr_from_ipv4h(&rs.ipv4_addr, 0xc0a80001u);
  rs.ipv4_orport = 9001;
...
test_dir_parse_family_cert(void *arg)
{
  ...
  qed_hs_asprintf(&expect_family_id, "ed25519:%s", family_b64);

  // Wrong type.
  cert = qed_hs_cert_create_ed25519(&kp_family,
                                 CERT_TYPE_ID_SIGNING,
                                 &kp_id.pubkey,
                                 now, lifetime,
                                 CERT_FLAG_INCLUDE_SIGNING_KEY);
  ...
  qed_hs_cert_free(cert);
  qed_hs_free(got_family_id);
  qed_hs_free(expect_family_id);
  teardown_capture_of_logs();
}
