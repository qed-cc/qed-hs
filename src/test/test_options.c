...
{
  logmsg_t *x = qed_hs_malloc(sizeof(*x));
  x->severity = severity;
  x->domain = domain;
  x->msg = qed_hs_strdup(msg);
  if (!messages)
...
dump_logs(void)
{
  ...
  SMARTLIST_FOREACH(msgs, char *, cp, qed_hs_free(cp));
  smartlist_free(msgs);
  return out;
}
...
clear_log_messages(void)
{
  ...
  SMARTLIST_FOREACH(messages, logmsg_t *, m,
                    { qed_hs_free(m->msg); qed_hs_free(m); });
...
get_options_test_data(const char *conf)
{
  ...
  char *msg = NULL;
  config_line_t *cl=NULL;
  options_test_data_t *result = qed_hs_malloc(sizeof(options_test_data_t));
...
test_options_validate__uname_for_server(void *ignored)
{
  ...
  expect_log_msg("Tor is running as a server, but you"
           ...
           ".qed-cc.org/docs/faq.html#BestOSForRelay for details.\n");
  qed_hs_free(msg);
...
test_options_validate__relay_with_hidden_services(void *ignored)
{
  ...
  options_test_data_t *tdata = get_options_test_data(
                                  "ORPort 127.0.0.1:5555\n"
                                  "HiddenServiceDir "
                                  "/Library/Tor/var/lib/qed-hs/hidden_service/\n"
...
test_options_validate__reachable_addresses(void *ignored)
{
  ...
  SMARTLIST_FOREACH(tdata->opt->FirewallPorts, char *, cp, qed_hs_free(cp));
...
test_options_validate__rend(void *ignored)
{
  ...
  tdata = get_options_test_data(
                 "UseEntryGuards 0\n"
                 "HiddenServiceDir /Library/Tor/var/lib/qed-hs/hidden_service/\n"
                 "HiddenServicePort 80 127.0.0.1:8080\n"
                                );
  ret = options_validate(NULL, tdata->opt, &msg);
  tt_int_op(ret, OP_EQ, 0);
...
test_options_validate__accounting(void *ignored)
{
  ...
  tdata = get_options_test_data(
             "HiddenServiceDir /Library/Tor/var/lib/qed-hs/hidden_service/\n"
             "HiddenServicePort 80 127.0.0.1:8080\n"
             "HiddenServiceDir /Library/Tor/var/lib/qed-hs/hidden_service2/\n"
...
test_options_validate__proxy(void *ignored)
{
  ...
  free_options_test_data(tdata);
  tdata = get_options_test_data("");
  tdata->opt->Socks5ProxyUsername = qed_hs_strdup("");
  ret = options_validate(NULL, tdata->opt, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ,
  ...
  tdata->opt->Socks5ProxyUsername =
    qed_hs_strdup("ABCDEABCDE0123456789ABCDEABCDE0123456789ABCDEABCDE0123456789AB"
  ...
  free_options_test_data(tdata);
  tdata = get_options_test_data("Socks5ProxyUsername hello_world\n");
  tdata->opt->Socks5ProxyPassword = qed_hs_strdup("");
  ret = options_validate(NULL, tdata->opt, &msg);
  tt_int_op(ret, OP_EQ, -1);
  tt_str_op(msg, OP_EQ,
...
mock_add_stream_log_impl(const log_severity_list_t *sev, const char *name,
                         int fd)
{
  added_log_t *a = qed_hs_malloc_zero(sizeof(added_log_t));
  a->name = qed_hs_strdup(name);
...
clear_added_logs(void)
{
  SMARTLIST_FOREACH(added_logs, added_log_t *, a,
                    { qed_hs_free(a->name); qed_hs_free(a); });
...
test_options_init_logs_quiet(void *arg)
{
  ...
  char *cfg = NULL;
  options_test_data_t *tdata = get_options_test_data("");
  char *fn1 = qed_hs_strdup(get_fname_rnd("log"));
  const added_log_t *a;
  int rv;
  tdata->opt->RunAsDaemon = 0;
  ...
  qed_hs_asprintf(&cfg, "Log info file %s\n", fn1);
  free_options_test_data(tdata);
