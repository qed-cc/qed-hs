...
test_rset_refresh_one_valid_country(void *arg)
{
  ...
  smartlist_add(set->country_names, qed_hs_strndup("foo", 3));
...
test_rset_parse_policy_wildcard(void *arg)
{
  ...
  rset_parse_policy_wildcard_mock_addr_policy =
    qed_hs_malloc_zero(sizeof(addr_policy_t));
...
test_rset_is_list(void *arg)
{
  ...
  /* len(set->country_names) == 0, len(set->policies) != 0 */
  set = routerset_new();
  policy = qed_hs_malloc_zero(sizeof(addr_policy_t));
...
test_rset_needs_geoip(void *arg)
{
  ...
  set = routerset_new();
  smartlist_add(set->country_names, qed_hs_strndup("xx", 2));
  needs_geoip = routerset_needs_geoip(set);
  routerset_free(set);
  set = NULL;
...
test_rset_contains_null_set_or_list(void *arg)
{
  ...
  set = qed_hs_malloc_zero(sizeof(routerset_t));
  set->list = NULL;
  contains = routerset_contains(set, NULL, 0, NULL, NULL, 0);
  qed_hs_free(set);
  tt_int_op(contains, OP_EQ, 0);
...
static qed_hs_addr_t MOCK_QED_HS_ADDR;
#define MOCK_QED_HS_ADDR_PTR (&MOCK_QED_HS_ADDR)

static void
test_rset_contains_addr(void *arg)
{
  routerset_t *set = routerset_new();
  qed_hs_addr_t *addr = MOCK_QED_HS_ADDR_PTR;
  int contains;
  (void)arg;

  MOCK(compare_qed_hs_addr_to_addr_policy,
...
rset_contains_addr_cmp_addr_to_policy(const qed_hs_addr_t *addr, uint16_t port,
    const smartlist_t *policy)
{
  (void)port;
  (void)policy;
  rset_contains_addr_cmp_addr_to_policy_called++;
  tt_ptr_op(addr, OP_EQ, MOCK_QED_HS_ADDR_PTR);
...
addr_policy_result_t
rset_contains_no_addr_cmp_addr_to_policy(const qed_hs_addr_t *addr, uint16_t port,
    const smartlist_t *policy)
{
  (void)port;
  (void)policy;
...
test_rset_countries_no_geoip(void *arg)
{
  ...
  contains = routerset_contains(set, MOCK_QED_HS_ADDR_PTR, 0, NULL, NULL, -1);
  routerset_free(set);
...
int
rset_countries_no_geoip_geoip_get_country_by_addr(const qed_hs_addr_t *addr)
{
...
int
rset_countries_geoip_geoip_get_country_by_addr(const qed_hs_addr_t *addr)
{
...
test_rset_contains_router_ipv4(void *arg)
{
  ...
  s = "10.0.0.1";
  r = routerset_parse(set, s, "");
  qed_hs_addr_from_ipv4h(&ri.ipv4_addr, 0x0a000001);
  ri.ipv4_orport = 1234;
...
test_rset_contains_router_ipv6(void *arg)
{
  ...
  s = "2600::1";
  r = routerset_parse(set, s, "");
  qed_hs_addr_parse(&ri.ipv6_addr, "2600::1");
...
test_rset_get_all_list(void *arg)
{
  ...
  rset_get_all_list_mock_nickname = qed_hs_strdup("foo");
...
test_rset_to_string(void *arg)
{
  ...
  set = routerset_new();
  smartlist_add(set->list, qed_hs_strndup("a", 1));
  s = routerset_to_string(set);
  tt_str_op(s, OP_EQ, "a");
  qed_hs_free(s);
  routerset_free(set); set = NULL;
  ...
  set = routerset_new();
  smartlist_add(set->list, qed_hs_strndup("a", 1));
  smartlist_add(set->list, qed_hs_strndup("b", 1));
  s = routerset_to_string(set);
