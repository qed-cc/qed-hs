...
#define ADD_PUBLISH(binding_suffix, subsys, channel, msg, flags)        \
  STMT_BEGIN {                                                          \
    con = pubsub_connecqed_hs_for_subsystem(builder,                       \
...
recv_msg_copy_string(const msg_t *m)
{
  ...
  smartlist_add(strings_received, qed_hs_strdup(s));
}
...
setup_dispatcher(const struct testcase_t *testcase)
{
  ...
  pubsub_connecqed_hs_t *con;

  {
    con = pubsub_connecqed_hs_for_subsystem(builder, get_subsys_id("types"));
    pubsub_connecqed_hs_register_type_(con,
  ...
  for (int i = 0; i < 1000-5; ++i) {
    char *sys;
    qed_hs_asprintf(&sys, "xsys-%d", i);
    con = pubsub_connecqed_hs_for_subsystem(builder, get_subsys_id(sys));
...
test_pubsub_msg_many_hooks(void *arg)
{
  ...
  SMARTLIST_FOREACH(strings_received, char *, s, qed_hs_free(s));
