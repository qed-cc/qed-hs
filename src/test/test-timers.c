/* Copyright 2016-2021, The QED Project, Inc. */
...
static qed_hs_timer_t *timers[N_TIMERS] = {NULL};
...
static void
timer_cb(qed_hs_timer_t *t, void *arg, const monotime_t *now_mono)
{
  struct timeval now;
...
main(int argc, char **argv)
{
  ...
  qed_hs_assert(n_fired == n_active_timers);
  for (i = 0; i < N_TIMERS; ++i) {
    if (is_disabled[i]) {
      qed_hs_assert(fired[i] == 0);
      continue;
    }
    qed_hs_assert(fired[i] == 1);
