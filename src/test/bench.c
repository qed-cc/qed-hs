...
bench_aes(void)
{
  ...
  for (len = 1; len <= 8192; len *= 2) {
    ...
    b1 = qed_hs_malloc_zero(len);
    b2 = qed_hs_malloc_zero(len);
    start = perftime();
    for (i = 0; i < iters; ++i) {
...
bench_rand_len(int len)
{
  ...
  if (len == 4) {
    qed_hs_weak_rng_t weak;
    qed_hs_init_weak_random(&weak, 1337);

    start = perftime();
    uint32_t t=0;
    for (i = 0; i < N; ++i) {
      t += qed_hs_weak_random(&weak);
      (void) t;
    }
    end = perftime();
...
bench_cell_aes(void)
{
  ...
  char *b = qed_hs_malloc(len+max_misalign);
  crypto_cipher_t *c;
  int i, misalign;
  char key[CIPHER_KEY_LEN];
  crypto_rand(key, sizeof(key));
...
bench_dmap(void)
{
  ...
  for (i = 0; i < elts; ++i) {
    crypto_rand(d, 20);
    smartlist_add(sl, qed_hs_memdup(d, 20));
  }
  for (i = 0; i < elts; ++i) {
    crypto_rand(d, 20);
    smartlist_add(sl2, qed_hs_memdup(d, 20));
  }
  //printf("nbits=%d\n", ds->mask+1);
...
bench_cell_ops_tor1(void)
{
  ...
  cell_t *cell = qed_hs_malloc(sizeof(cell_t));
  int outbound;
  uint64_t start, end;
...
bench_cell_ops_cgo(void)
{
  ...
  uint8_t *keys = qed_hs_malloc(keylen);
...
bench_dh(void)
{
  ...
  for (i = 0; i < iters; ++i) {
    ...
    qed_hs_assert(slen_a == slen_b);
    qed_hs_assert(fast_memeq(secret_a, secret_b, slen_a));
    crypto_dh_free(dh_a);
    crypto_dh_free(dh_b);
  }
  end = perftime();
...
main(int argc, const char **argv)
{
  ...
  options->DataDirectory = qed_hs_strdup("");
  options->KeyDirectory = qed_hs_strdup("");
  options->CacheDirectory = qed_hs_strdup("");
