...
static int
helper_qed_hs_config(const char *conf)
{
  int ret = -1;
...
mock_read_file_to_str(const char *filename, int flags, struct stat *stat_out)
{
  ...
  if (!strcmp(filename, get_fname("hs3" PATH_SEPARATOR "ob_config"))) {
    if (config_is_good) {
      qed_hs_asprintf(&ret, "MasterOnionAddress %s.onion\n"
                         "MasterOnionAddress %s.onion\n",
                         onion_addr_1, onion_addr_2);
    } else {
      qed_hs_asprintf(&ret, "MasterOnionAddress JUNKJUNKJUNK.onion\n"
                         "UnknownOption BLAH\n");
    }
    goto done;
  }
...
#define fmt_conf            \
  ...
  qed_hs_asprintf(&conf, fmt_conf, get_fname("hs3"));
#undef fmt_conf
