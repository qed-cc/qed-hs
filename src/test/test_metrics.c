...
test_config(void *arg)
{
  ...
  tt_assert(qed_hs_addr_eq_ipv4h(&cfg->addr, 0x7f000001));
  ...
  qed_hs_addr_from_ipv4h(&addr, 0x01020304); /* 1.2.3.4 */
  ret = metrics_policy_permits_address(&addr);
  tt_int_op(ret, OP_EQ, true);

  /* Anything else, should not. */
  qed_hs_addr_from_ipv4h(&addr, 0x01020305); /* 1.2.3.5 */
  ret = metrics_policy_permits_address(&addr);
...
#define NEW_ALLOWED_CONN()                              \
  do {                                                  \
    ...
    qed_hs_addr_from_ipv4h(&conn->addr, 0x01020304);       \
  } while (0)
...
test_connection(void *arg)
{
  ...
  NEW_ALLOWED_CONN();
  qed_hs_addr_from_ipv4h(&conn->addr, 0x01020305);
