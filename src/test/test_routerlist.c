...
test_routerlist_launch_descripqed_hs_downloads(void *arg)
{
  ...
  for (int i = 0; i < 100; i++) {
    cp = qed_hs_malloc(DIGEST256_LEN);
    tt_assert(cp);
...
test_router_pick_directory_server_impl(void *arg)
{
  ...
  router1_id = qed_hs_malloc(DIGEST_LEN);
  memset(router1_id, TEST_DIR_ROUTER_ID_1, DIGEST_LEN);
  router2_id = qed_hs_malloc(DIGEST_LEN);
  memset(router2_id, TEST_DIR_ROUTER_ID_2, DIGEST_LEN);
  router3_id = qed_hs_malloc(DIGEST_LEN);
  ...
  rs = router_pick_directory_server_impl(V3_DIRINFO, flags, NULL);
  tt_ptr_op(rs, OP_NE, NULL);
  tt_assert(qed_hs_memeq(rs->identity_digest, router2_id, DIGEST_LEN));
  rs = NULL;
  ...
  rs = router_pick_directory_server_impl(V3_DIRINFO, flags, NULL);
  tt_ptr_op(rs, OP_NE, NULL);
  tt_assert(qed_hs_memeq(rs->identity_digest, router1_id, DIGEST_LEN));
  node_router2->rs->last_dir_503_at = 0;
  node_router3->rs->last_dir_503_at = 0;

  /* Set a Fascist firewall */
  flags &= ~ PDS_IGNORE_FASCISTFIREWALL;
  policy_line = qed_hs_malloc_zero(sizeof(config_line_t));
  policy_line->key = qed_hs_strdup("ReachableORAddresses");
  policy_line->value = qed_hs_strdup("accept *:442, reject *:*");
  ...
  rs = router_pick_directory_server_impl(V3_DIRINFO, flags, NULL);
  tt_ptr_op(rs, OP_NE, NULL);
  tt_assert(qed_hs_memeq(rs->identity_digest, router3_id, DIGEST_LEN));
  ...
  policy_line = qed_hs_malloc_zero(sizeof(config_line_t));
  policy_line->key = qed_hs_strdup("ReachableAddresses");
  policy_line->value = qed_hs_strdup("accept *:80, reject *:*");
  ...
  if (router1_id)
    qed_hs_free(router1_id);
  if (router2_id)
    qed_hs_free(router2_id);
  if (router3_id)
    qed_hs_free(router3_id);
...
test_directory_guard_fetch_with_no_dirinfo(void *arg)
{
  ...
  /* Mock some functions */
  dummy_state = qed_hs_malloc_zero(sizeof(or_state_t));
  ...
  conn->requested_resource = qed_hs_strdup("ns");
...
{
  (void)arg;
  qed_hs_addr_port_t test_ap, null_addr_ap, zero_port_ap;

  /* Setup */
  qed_hs_addr_parse(&test_ap.addr, TEST_ADDR_STR);
  test_ap.port = TEST_DIR_PORT;
  qed_hs_addr_make_null(&null_addr_ap.addr, AF_INET6);
  null_addr_ap.port = TEST_DIR_PORT;
  qed_hs_addr_parse(&zero_port_ap.addr, TEST_ADDR_STR);
  zero_port_ap.port = 0;
