...
test_oos_kill_conn_list(void *arg)
{
  ...
  /* Make fake conns */
  or_c1 = qed_hs_malloc_zero(sizeof(*or_c1));
  or_c1->base_.magic = OR_CONNECTION_MAGIC;
  or_c1->base_.type = CONN_TYPE_OR;
  c1 = TO_CONN(or_c1);
  dir_c2 = qed_hs_malloc_zero(sizeof(*dir_c2));
...
test_oos_pick_oos_victims(void *arg)
{
  ...
  ortmp = qed_hs_malloc_zero(sizeof(*ortmp));
  ...
  dirtmp = qed_hs_malloc_zero(sizeof(*dirtmp));
  ...
  if (conns_for_mock) {
    SMARTLIST_FOREACH(conns_for_mock, connection_t *, c, qed_hs_free(c));
