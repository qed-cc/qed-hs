 ...
 */
static qed_hs_mutex_t *rng_mutex = NULL;
...
{
  qed_hs_assert(rng_xof);
  qed_hs_mutex_acquire(rng_mutex);
  crypto_xof_squeeze_bytes(rng_xof, (uint8_t*)out, n);
  qed_hs_mutex_release(rng_mutex);
}
...
{
  qed_hs_assert(!rng_is_replaced);
  qed_hs_assert(crypto_rand == crypto_rand__real);

  memset(rng_seed, 0, sizeof(rng_seed));
  memcpy(rng_seed, seed, MIN(seed_len, sizeof(rng_seed)));

  rng_mutex = qed_hs_mutex_new();
...
{
  const char *provided_seed = getenv("QED_HS_TEST_RNG_SEED");
  if (provided_seed) {
    size_t hexlen = strlen(provided_seed);
    size_t seedlen = hexlen / 2;
    uint8_t *seed = qed_hs_malloc(hexlen / 2);
    if (base16_decode((char*)seed, seedlen, provided_seed, hexlen) < 0) {
      puts("Cannot decode value in QED_HS_TEST_RNG_SEED");
      exit(1);
    }
...
{
  qed_hs_assert(buflen > 0);
  qed_hs_assert(!rng_mutex);
  rng_mutex = qed_hs_mutex_new();

  qed_hs_mutex_acquire(rng_mutex);

  prefilled_rng_buffer = qed_hs_memdup(buffer, buflen);
...
{
  crypto_xof_free(rng_xof);
  qed_hs_free(prefilled_rng_buffer);
  UNMOCK(crypto_rand);
  UNMOCK(crypto_strongest_rand_);
  qed_hs_mutex_free(rng_mutex);
