...
static int
chan_test_get_remote_addr(const channel_t *ch, qed_hs_addr_t *out)
{
  (void)ch;
  qed_hs_addr_from_ipv4h(out, 0x7f000001);
  return 1;
}
...
{
  channel_t *chan = qed_hs_malloc_zero(sizeof(channel_t));
  channel_init(chan);
...
static int
test_chan_matches_target(channel_t *chan, const qed_hs_addr_t *target)
{
  (void) chan;
  (void) target;
...
test_channel_dumpstats(void *arg)
{
  ...
  cell = qed_hs_malloc_zero(sizeof(*cell));
...
#define N_CHAN 6
  ...
  for (i = 0; i < N_CHAN; ++i) {
    ...
    ed_id[i] = qed_hs_malloc_zero(sizeof(*ed_id[i]));
...
test_channel_duplicates(void *arg)
{
  ...
  mock_ns = qed_hs_malloc_zero(sizeof(*mock_ns));
...
test_channel_for_extend(void *arg)
{
  ...
  qed_hs_addr_make_null(&ipv4_addr, AF_INET);
  qed_hs_addr_make_null(&ipv6_addr, AF_INET6);
...
test_channel_listener(void *arg)
{
  ...
  chan = qed_hs_malloc_zero(sizeof(*chan));
  tt_assert(chan);
...
#define TEST_SETUP_MATCHES_ADDR(orcon, addr, src, rv) STMT_BEGIN \
    rv = qed_hs_inet_pton(addr.family, src, &addr.addr); \
...
test_channel_matches_target_addr_for_extend(void *arg)
{
  ...
  or_connection_t *orcon = qed_hs_malloc_zero(sizeof(*orcon));
  ...
  qed_hs_inet_pton(addr.family, "2.5.3.4", &addr.addr);
  ...
  qed_hs_inet_pton(addr.family, "::", &addr.addr);
