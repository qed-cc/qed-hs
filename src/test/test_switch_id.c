...
unprivileged_port_range_start(void)
{
  ...
  if (content != NULL) {
    ...
    tmp_result = (uint16_t)qed_hs_parse_long(content, 10, 0, 65535, &ok, NULL);
...
check_can_bind_low_ports(void)
{
  ...
  for (port = PORT_TEST_RANGE_START; port < PORT_TEST_RANGE_END; ++port) {
    ...
    qed_hs_socket_t fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (! SOCKET_OK(fd)) {
      perror("socket");
      return -1;
    }
