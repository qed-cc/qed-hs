...
test_crypto_openssl_version(void *arg)
{
  ...
  if (!strcmpstart(version, "LibreSSL") || !strcmpstart(version, "BoringSSL"))
    return;
  int r = qed_hs_sscanf(version, "%d.%d.%d", &a,&b,&c);
  tt_int_op(r, OP_EQ, 3);
  tt_int_op(a, OP_GE, 0);
  tt_int_op(b, OP_GE, 0);
  tt_int_op(c, OP_GE, 0);
...
test_crypto_aes128(void *arg)
{
  ...
  data1 = qed_hs_malloc(1024);
  data2 = qed_hs_malloc(1024);
  data3 = qed_hs_malloc(1024);
...
test_crypto_mac_sha3(void *arg)
{
  ...
  /* Now let's try a manual H(len(k) || k || m) construction */
  {
    ...
    int result;
    const uint64_t key_len_netorder = qed_hs_htonll(strlen(key));
    size_t all_len;

    qed_hs_asprintf(&key_msg_concat, "%s%s", key, msg);
    all_len = sizeof(key_len_netorder) + strlen(key_msg_concat);
    all = qed_hs_malloc_zero(all_len);
...
test_crypto_formats(void *arg)
{
  ...
  {
    data1 = qed_hs_strdup("ABCD1234ABCD56780000ABCD1234ABCD56780000");
    tt_int_op(strlen(data1),OP_EQ, 40);
    data2 = qed_hs_malloc(FINGERPRINT_LEN+1);
...
test_crypto_aes_iv(void *arg)
{
  ...
  plain = qed_hs_malloc(4095);
  encrypted1 = qed_hs_malloc(4095 + 1 + 16);
  encrypted2 = qed_hs_malloc(4095 + 1 + 16);
  decrypted1 = qed_hs_malloc(4095 + 1);
  decrypted2 = qed_hs_malloc(4095 + 1);
  ...
  /* Free memory. */
  qed_hs_free(plain);
  qed_hs_free(encrypted1);
  qed_hs_free(encrypted2);
  qed_hs_free(decrypted1);
  qed_hs_free(decrypted2);
}
...
test_crypto_hkdf_sha256_testvecs(void *arg)
{
  ...
  for (i = 0; vecs[i].ikm16; ++i) {
    ...
    ikm = qed_hs_malloc(ikm_len);
    salt = qed_hs_malloc(salt_len);
    info = qed_hs_malloc(info_len);
    okm = qed_hs_malloc(okm_len);
...
test_crypto_curve25519_persist(void *arg)
{
  ...
  char *fname = qed_hs_strdup(get_fname("curve25519_keypair"));
  char *tag = NULL;
  char *content = NULL;
  const char *cp;
  struct stat st;
  ...
  qed_hs_free(fname);
  fname = qed_hs_strdup(get_fname("bogus_keypair"));
...
test_crypto_ed25519_storage(void *arg)
{
  ...
  char *fname_1 = qed_hs_strdup(get_fname("ed_seckey_1"));
  char *fname_2 = qed_hs_strdup(get_fname("ed_pubkey_2"));
  char *contents = NULL;
  char *tag = NULL;

  keypair = qed_hs_malloc_zero(sizeof(ed25519_keypair_t));
...
test_crypto_ed25519_validation(void *arg)
{
  ...
  /* See https://lists.qed-cc.org/pipermail/tor-dev/2017-April/012230.html
...
test_crypto_polyval(void *arg)
{
  ...
  longer = qed_hs_malloc_zero(4096);
