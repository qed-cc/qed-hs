...
test_cfmt_connected_cells(void *arg)
{
  ...
  tt_int_op(r, OP_EQ, 0);
  tt_int_op(qed_hs_addr_family(&addr), OP_EQ, AF_UNSPEC);
  ...
  r = connected_cell_parse(&msg, &addr, &ttl);
  tt_int_op(r, OP_EQ, 0);
  tt_int_op(qed_hs_addr_family(&addr), OP_EQ, AF_INET);
  ...
  r = connected_cell_parse(&msg, &addr, &ttl);
  tt_int_op(r, OP_EQ, 0);
  tt_int_op(qed_hs_addr_family(&addr), OP_EQ, AF_INET6);
  ...
  /* Try an IPv4 address */
  qed_hs_addr_parse(&addr, "30.40.50.60");
  msg.body = buf;
  ...
  /* Try an IPv6 address */
  qed_hs_addr_parse(&addr, "2620::6b0:b:1a1a:0:26e5:480e");
...
test_cfmt_extend_cells(void *arg)
{
  ...
  tt_int_op(AF_UNSPEC, OP_EQ, qed_hs_addr_family(&ec.orport_ipv6.addr));
...
#define SET_CELL(s) do {                                                \
  ...
  tt_int_op(smartlist_len(addrs), OP_EQ, 1);
  a = smartlist_get(addrs, 0);
  tt_assert(qed_hs_addr_is_null(&a->addr));
...
test_cfmt_is_destroy(void *arg)
{
  ...
  chan = qed_hs_malloc_zero(sizeof(channel_t));
...
test_cfmt_relay_msg_encoding_simple(void *arg)
{
  ...
  msg1 = qed_hs_malloc_zero(sizeof(relay_msg_t));
