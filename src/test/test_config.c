...
test_config_addressmap(void *arg)
{
  ...
  strlcpy(buf, "MapAddress .invalidwildcard.com *.torserver.exit\n" // invalid
          ...
          "MapAddress www.qed-cc.org 1.1.1.1\n"
          "MapAddress other.qed-cc.org "
            "this.qed-cc.org.otherserver.exit\n"
          "MapAddress test.qed-cc.org 2.2.2.2\n"
...
#define addressmap_rewrite(a,s,eo,ao)                                   \
  ...
  strlcpy(address, "www.qed-cc.org", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "1.1.1.1");

  strlcpy(address, "other.qed-cc.org", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
  tt_str_op(address,OP_EQ, "this.qed-cc.org.otherserver.exit");

  strlcpy(address, "test.qed-cc.org", sizeof(address));
  tt_assert(addressmap_rewrite(address, sizeof(address), &expires, NULL));
...
test_config_check_or_create_data_subdir(void *arg)
{
  ...
  datadir = options->DataDirectory = qed_hs_strdup(get_fname("datadir-0"));
...
test_config_write_to_data_subdir(void *arg)
{
  ...
  qed_hs_free(options->DataDirectory);
  datadir = options->DataDirectory = qed_hs_strdup(get_fname("datadir-1"));
                      ...
                      const smartlist_t *test_socks_args)
{
  ...
  /* test addrport */
  tmp = qed_hs_strdup(fmt_addrport(&bridge_line->addr, bridge_line->port));
  ...
  if (test_digest && qed_hs_digest_is_zero(bridge_line->digest))
    tt_abort();
  ...
  if (test_digest) {
    tmp = qed_hs_strdup(hex_str(bridge_line->digest, DIGEST_LEN));
    qed_hs_strlower(tmp);
...
test_config_parse_bridge_line(void *arg)
{
...

  {
    ...
    SMARTLIST_FOREACH(sl_tmp, char *, s, qed_hs_free(s));
...
test_config_parse_transport_options_line(void *arg)
{
  ...
  { /* correct -- no transport specified */
    ...
    smartlist_free(sl_tmp);
    sl_tmp = NULL;
    SMARTLIST_FOREACH(options_sl, char *, s, qed_hs_free(s));
...
static int transport_add_from_config_mock(const qed_hs_addr_t *addr,
...
static int
transport_add_from_config_mock(const qed_hs_addr_t *addr,
                               uint16_t port, const char *name,
                               int socks_ver)
{
  (void) addr;
...
test_config_parse_tcp_proxy_line(void *arg)
{
  ...
  MOCK(qed_hs_addr_lookup, mock_qed_hs_addr_lookup__fail_on_bad_addrs);
  ...
  /* Correct the address. */
  tt_assert(qed_hs_addr_eq_ipv4h(&options->TCPProxyAddr, 0x5fd8a324));
...
{
  char *err = NULL;
  config_line_t *family = qed_hs_malloc_zero(sizeof(config_line_t));
  family->key = qed_hs_strdup("MyFamily");
  family->value = qed_hs_strdup("$1111111111111111111111111111111111111111, "
                             "1111111111111111111111111111111111111112, "
                             "$1111111111111111111111111111111111111113");

  config_line_t *family2 = qed_hs_malloc_zero(sizeof(config_line_t));
  family2->key = qed_hs_strdup("MyFamily");
  family2->value = qed_hs_strdup("1111111111111111111111111111111111111114");

  config_line_t *family3 = qed_hs_malloc_zero(sizeof(config_line_t));
  family3->key = qed_hs_strdup("MyFamily");
  family3->value = qed_hs_strdup("$1111111111111111111111111111111111111115");
...
/** This mock function is meant to replace qed_hs_addr_lookup().
 ...
 */
static int
qed_hs_addr_lookup_01010101(const char *name, uint16_t family, qed_hs_addr_t *addr)
{
  n_hostname_01010101++;

  if (family == AF_INET) {
    if (name && addr) {
      int ret = qed_hs_addr_parse(addr, ret_addr_lookup_01010101[0]);
      tt_int_op(ret, OP_EQ, family);
    }
  } else if (family == AF_INET6) {
    if (name && addr) {
      int ret = qed_hs_addr_parse(addr, ret_addr_lookup_01010101[1]);
      tt_int_op(ret, OP_EQ, family);
    }
  }
 done:
 ...
 */
static int
qed_hs_addr_lookup_localhost(const char *name, uint16_t family, qed_hs_addr_t *addr)
{
  n_hostname_localhost++;

  if (family == AF_INET) {
    if (name && addr) {
      qed_hs_addr_from_ipv4h(addr, 0x7f000001);
    }
  } else if (family == AF_INET6) {
    if (name && addr) {
      int ret = qed_hs_addr_parse(addr, "::1");
 ...
 */
static int
qed_hs_addr_lookup_failure(const char *name, uint16_t family, qed_hs_addr_t *addr)
{
  (void)name;
  (void)family;
  (void)addr;
...
/** Mock function for qed_hs_addr_lookup().
 *
 ...
 *
 * Possible hostnames:
 *  - www.qed-cc.org.v4 for IPv4 -> 1.1.1.1
 *  - www.qed-cc.org.v6 for IPv6 -> [0101::0101]
 */
static int
qed_hs_addr_lookup_mixed(const char *name, uint16_t family, qed_hs_addr_t *addr)
{
  tt_assert(addr);
  tt_assert(name);

  if (!strcmp(name, "www.qed-cc.org.v4")) {
    if (family == AF_INET) {
      qed_hs_addr_from_ipv4h(addr, 0x01010101);
      return 0;
    }
  ...
  if (!strcmp(name, "www.qed-cc.org.v6")) {
    if (family == AF_INET6) {
      int ret = qed_hs_addr_parse(addr, "0101::0101");
      tt_int_op(ret, OP_EQ, AF_INET6);
      return 0;
    }
 ...
 */
static int
qed_hs_gethostname_localhost(char *name, size_t namelen)
{
                                ...
                                qed_hs_addr_t *addr)
{
  ...
  if (family == AF_INET) {
    if (addr) {
      int ret = qed_hs_addr_parse(addr, ret_get_interface_address6_08080808[0]);
      tt_int_op(ret, OP_EQ, AF_INET);
    }
  } else if (family == AF_INET6) {
    if (addr) {
      int ret = qed_hs_addr_parse(addr, ret_get_interface_address6_08080808[1]);
      tt_int_op(ret, OP_EQ, AF_INET6);
    }
  }
 done:
                                   ...
                                   qed_hs_addr_t *addr)
{
  ...
  qed_hs_addr_from_ipv4h(addr,0x09090909);
...
#define VALIDATE_FOUND_ADDRESS(ret, method, hostname)     \
  do {                                                    \
    ...
    if (ret == true) {                                    \
      tt_assert(qed_hs_addr_eq(&resolved_addr, &test_addr)); \
    }                                                     \
  } while (0)
...
#define CLEANUP_FOUND_ADDRESS                 \
  do {                                        \
    ...
    qed_hs_free(options->DirAuthorities);        \
    qed_hs_free(hostname_out);                   \
    qed_hs_addr_make_unspec(&resolved_addr);     \
    qed_hs_addr_make_unspec(&test_addr);         \
  } while (0)
...
test_config_find_my_address_mixed(void *arg)
{
  ...
  qed_hs_addr_t resolved_addr, test_addr;
  ...
  qed_hs_addr_parse(&test_addr, "2a01:4f8:fff0:4f:266:37ff:fe2c:5d19");
  ...
  qed_hs_addr_parse(&test_addr, "1.1.1.1");
   ...
   */
  config_line_append(&options->Address, "Address", "www.qed-cc.org.v4");
  config_line_append(&options->Address, "Address", "www.qed-cc.org.v6");

  /* Looks at specific hostname to learn which address family to use. */
  MOCK(qed_hs_addr_lookup, qed_hs_addr_lookup_mixed);
  ...
  qed_hs_addr_parse(&test_addr, "0101::0101");
  retval = find_my_address(options, AF_INET6, LOG_NOTICE, &resolved_addr,
                           &method_used, &hostname_out);
  VALIDATE_FOUND_ADDRESS(true, RESOLVED_ADDR_RESOLVED,
                         "www.qed-cc.org.v6");
...
typedef struct find_my_address_params_t {
   ...
   * qed_hs_addr_lookup_01010101() will have its returned value depending on the
...
test_config_find_my_address(void *arg)
{
  ...
  options = options_new();
  options_init(options);
  options->PublishServerDescripqed_hs_ = V3_DIRINFO;
  ...
  if (p->family == AF_INET6) {
    ...
    qed_hs_addr_parse(&test_addr, p->public_ip);
   ...
   */
  MOCK(qed_hs_addr_lookup, qed_hs_addr_lookup_01010101);

  config_line_append(&options->Address, "Address", "www.qed-cc.org");
  qed_hs_addr_parse(&test_addr, ret_addr_lookup_01010101[p->idx]);
  ...
  VALIDATE_FOUND_ADDRESS(true, RESOLVED_ADDR_RESOLVED, "www.qed-cc.org");
  CLEANUP_FOUND_ADDRESS;
   ...
   */
  config_line_append(&options->Address, "Address", p->internal_ip);
  options->DirAuthorities = qed_hs_malloc_zero(sizeof(config_line_t));
  qed_hs_addr_parse(&test_addr, p->internal_ip);
  ...
  qed_hs_addr_parse(&test_addr, ret_get_interface_address6_08080808[p->idx]);
  ...
  MOCK(qed_hs_gethostname, qed_hs_gethostname_replacement);
  MOCK(qed_hs_addr_lookup, qed_hs_addr_lookup_01010101);
  ...
  MOCK(get_interface_address6, get_interface_address6_failure);
  MOCK(qed_hs_gethostname, qed_hs_gethostname_localhost);
  MOCK(qed_hs_addr_lookup, qed_hs_addr_lookup_localhost);
  ...
  MOCK(get_interface_address6, get_interface_address6_failure);
  MOCK(qed_hs_gethostname, qed_hs_gethostname_failure);
  ...
  MOCK(get_interface_address6, get_interface_address6_failure);
  MOCK(qed_hs_gethostname, qed_hs_gethostname_replacement);
  MOCK(qed_hs_addr_lookup, qed_hs_addr_lookup_failure);
   ...
   */
  {
    ...
    options->Address = NULL;
    qed_hs_asprintf(&orport_line, "%s:9001", p->public_ip);
    config_line_append(&options->ORPort_lines, "ORPort", orport_line);
    qed_hs_free(orport_line);
   ...
   */
  {
    ...
    options->Address = NULL;
    qed_hs_asprintf(&orport_line, "%s:9001", p->internal_ip);
    config_line_append(&options->ORPort_lines, "ORPort", orport_line);
    qed_hs_free(orport_line);
   ...
   * PublishServerDescripqed_hs_ so we are allowed to hold it.
   */
  options->PublishServerDescripqed_hs_ = NO_DIRINFO;
  if (p->family == AF_INET) {
...
test_config_adding_trusted_dir_server(void *arg)
{
  ...
  rv = qed_hs_addr_port_parse(LOG_WARN, "[::1]:9061", &ipv6.addr, &ipv6.port, -1);
...
test_config_adding_fallback_dir_server(void *arg)
{
  ...
  rv = qed_hs_addr_parse(&ipv4, "127.0.0.1");
...
test_config_adding_dir_servers(void *arg)
{
   ...
   */
  config_line_t *test_dir_authority = qed_hs_malloc_zero(sizeof(config_line_t));
  test_dir_authority->key = qed_hs_strdup("DirAuthority");
  test_dir_authority->value = qed_hs_strdup(
    "D0 orport=9000 "
    "v3ident=0023456789012345678901234567890123456789 "
    "127.0.0.1:60090 0123 4567 8901 2345 6789 0123 4567 8901 2345 6789"
    );

  config_line_t *test_alt_bridge_authority = qed_hs_malloc_zero(
                                                      sizeof(config_line_t));
  test_alt_bridge_authority->key = qed_hs_strdup("AlternateBridgeAuthority");
  test_alt_bridge_authority->value = qed_hs_strdup(
    "B1 orport=9001 bridge "
    "127.0.0.1:60091 1123 4567 8901 2345 6789 0123 4567 8901 2345 6789"
    );

  config_line_t *test_alt_dir_authority = qed_hs_malloc_zero(
                                                      sizeof(config_line_t));
  test_alt_dir_authority->key = qed_hs_strdup("AlternateDirAuthority");
  test_alt_dir_authority->value = qed_hs_strdup(
    "A2 orport=9002 "
  ...
  config_line_t *test_fallback_directory = qed_hs_malloc_zero(
                                                      sizeof(config_line_t));
  test_fallback_directory->key = qed_hs_strdup("FallbackDir");
  test_fallback_directory->value = qed_hs_strdup(
...
#define n_default_authorities ((n_default_alt_bridge_authority) \
  ...
  */
  ...
  qed_hs_free(test_dir_authority->key);
  qed_hs_free(test_dir_authority->value);
  qed_hs_free(test_dir_authority);

  qed_hs_free(test_alt_dir_authority->key);
  qed_hs_free(test_alt_dir_authority->value);
  qed_hs_free(test_alt_dir_authority);

  qed_hs_free(test_alt_bridge_authority->key);
  qed_hs_free(test_alt_bridge_authority->value);
  qed_hs_free(test_alt_bridge_authority);

  qed_hs_free(test_fallback_directory->key);
  qed_hs_free(test_fallback_directory->value);
  qed_hs_free(test_fallback_directory);
...
mock_relay_find_addr_to_publish(const or_options_t *options, int family,
                                int flags, qed_hs_addr_t *addr_out)
{
  (void) options;
  (void) family;
  (void) flags;
...
{
  config_line_t *config_line = qed_hs_malloc(sizeof(config_line_t));
  memset(config_line, 0, sizeof(config_line_t));
  config_line->key = qed_hs_strdup(key);
  config_line->value = qed_hs_strdup(val);
  return config_line;
}
...
test_config_parse_port_config__ports__ports_given(void *data)
{
  ...
  qed_hs_addr_parse(&addr, "127.0.0.46");
  tt_assert(qed_hs_addr_eq(&port_cfg->addr, &addr));
  ...
  port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
  tt_int_op(port_cfg->port, OP_EQ, CFG_AUTO_PORT);
  qed_hs_addr_parse(&addr, "127.0.0.122");
  tt_assert(qed_hs_addr_eq(&port_cfg->addr, &addr));
  ...
  qed_hs_addr_parse(&addr, "127.0.0.123");
  tt_assert(qed_hs_addr_eq(&port_cfg->addr, &addr));
...
test_config_parse_log_severity(void *data)
{
  ...
  severity = qed_hs_malloc(sizeof(log_severity_list_t));
...
test_config_include_limit(void *data)
{
  ...
  char *dir = qed_hs_strdup(get_fname("test_include_limit"));
  ...
  qed_hs_asprintf(&torrc_path, "%s"PATH_SEPARATOR"torrc", dir);
  char torrc_contents[1000];
  qed_hs_snprintf(torrc_contents, sizeof(torrc_contents), "%%include %s",
               torrc_path);
...
test_config_include_does_not_exist(void *data)
{
  ...
  config_line_t *result = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_does_not_exist"));
  ...
  qed_hs_asprintf(&missing_path, "%s"PATH_SEPARATOR"missing", dir);
  char torrc_contents[1000];
  qed_hs_snprintf(torrc_contents, sizeof(torrc_contents), "%%include %s",
               missing_path);
...
test_config_include_error_in_included_file(void *data)
{
  ...
  char *dir = qed_hs_strdup(get_fname("test_error_in_included_file"));
  ...
  qed_hs_asprintf(&invalid_path, "%s"PATH_SEPARATOR"invalid", dir);
...
test_config_include_empty_file_folder(void *data)
{
  ...
  char *file_path = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_empty_file_folder"));
  tt_ptr_op(dir, OP_NE, NULL);
  ...
  qed_hs_asprintf(&folder_path, "%s"PATH_SEPARATOR"empty_dir", dir);
#ifdef _WIN32
  tt_int_op(mkdir(folder_path), OP_EQ, 0);
#else
  tt_int_op(mkdir(folder_path, 0700), OP_EQ, 0);
#endif
  qed_hs_asprintf(&file_path, "%s"PATH_SEPARATOR"empty_file", dir);
  tt_int_op(write_str_to_file(file_path, "", 0), OP_EQ, 0);

  char torrc_contents[1000];
  qed_hs_snprintf(torrc_contents, sizeof(torrc_contents),
               "%%include %s\n"
               "%%include %s\n",
  ...
  config_free_lines(result);
  qed_hs_free(folder_path);
  qed_hs_free(file_path);
  qed_hs_free(dir);
}
...
test_config_include_no_permission(void *data)
{
  ...
  dir = qed_hs_strdup(get_fname("test_include_forbidden_folder"));
  tt_ptr_op(dir, OP_NE, NULL);

  tt_int_op(mkdir(dir, 0700), OP_EQ, 0);

  qed_hs_asprintf(&folder_path, "%s"PATH_SEPARATOR"forbidden_dir", dir);
...
test_config_include_recursion_before_after(void *data)
{
  ...
  config_line_t *result = NULL;
  char *torrc_path = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_recursion_before_after"));
  tt_ptr_op(dir, OP_NE, NULL);
  ...
  for (i = limit; i > 0; i--) {
    if (i < limit) {
      qed_hs_snprintf(file_contents, sizeof(file_contents),
                   "Test %d\n"
                   "%%include %s%d\n"
                   "Test %d\n",
                   i, torrc_path, i + 1, 2 * limit - i);
    } else {
      qed_hs_snprintf(file_contents, sizeof(file_contents), "Test %d\n", i);
    }

    if (i > 1) {
      char *file_path = NULL;
      qed_hs_asprintf(&file_path, "%s%d", torrc_path, i);
  ...
  for (next = result; next != NULL; next = next->next) {
    char expected[10];
    qed_hs_snprintf(expected, sizeof(expected), "%d", len + 1);
...
test_config_include_recursion_after_only(void *data)
{
  ...
  config_line_t *result = NULL;
  char *torrc_path = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_recursion_after_only"));
  tt_ptr_op(dir, OP_NE, NULL);
...
test_config_include_folder_order(void *data)
{
  ...
  char *path = NULL;
  char *path2 = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_folder_order"));
  tt_ptr_op(dir, OP_NE, NULL);
  ...
  qed_hs_asprintf(&torrcd, "%s"PATH_SEPARATOR"%s", dir, "torrc.d");
  ...
  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, "subfolder");
  ...
  qed_hs_asprintf(&path2, "%s"PATH_SEPARATOR"%s", path, "01_ignore");
  tt_int_op(write_str_to_file(path2, "ShouldNotSee 1\n", 0), OP_EQ, 0);
  qed_hs_free(path);

  // test that files starting with . are ignored
  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, ".dot");
  tt_int_op(write_str_to_file(path, "ShouldNotSee 2\n", 0), OP_EQ, 0);
  qed_hs_free(path);

  // test file order
  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, "01_1st");
  tt_int_op(write_str_to_file(path, "Test 1\n", 0), OP_EQ, 0);
  qed_hs_free(path);

  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, "02_2nd");
  tt_int_op(write_str_to_file(path, "Test 2\n", 0), OP_EQ, 0);
  qed_hs_free(path);

  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, "aa_3rd");
  tt_int_op(write_str_to_file(path, "Test 3\n", 0), OP_EQ, 0);
  qed_hs_free(path);

  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, "ab_4th");
...
test_config_include_blank_file_last(void *data)
{
  ...
  char *torrcd = NULL;
  char *path = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_blank_file_last"));
  tt_ptr_op(dir, OP_NE, NULL);
  ...
  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, "aa_1st");
  tt_int_op(write_str_to_file(path, "Test 1\n", 0), OP_EQ, 0);
  qed_hs_free(path);

  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, "bb_2nd");
  tt_int_op(write_str_to_file(path, "Test 2\n", 0), OP_EQ, 0);
  qed_hs_free(path);

  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", torrcd, "cc_comment");
...
test_config_include_path_syntax(void *data)
{
  ...
  config_line_t *result = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_path_syntax"));
  ...
  qed_hs_asprintf(&dir_with_pathsep, "%s%s", dir, PATH_SEPARATOR);
  esc_dir_with_pathsep = esc_for_log(dir_with_pathsep);

  qed_hs_asprintf(&torrc_contents,
               "%%include %s\n"
  ...
  config_free_lines(result);
  qed_hs_free(dir);
  qed_hs_free(torrc_contents);
  qed_hs_free(esc_dir);
  qed_hs_free(dir_with_pathsep);
  qed_hs_free(esc_dir_with_pathsep);
}
...
test_config_include_has_include(void *data)
{
  ...
  config_line_t *result = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_has_include"));
  tt_ptr_op(dir, OP_NE, NULL);
...
test_config_include_flag_both_without(void *data)
{
  ...
  qed_hs_snprintf(conf_empty, sizeof(conf_empty),
               "DataDirectory %s\n",
               get_fname(NULL));
...
test_config_include_flag_torrc_only(void *data)
{
  ...
  char *errmsg = NULL;
  char *path = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_flag_torrc_only"));
  tt_ptr_op(dir, OP_NE, NULL);
  ...
  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", dir, "dummy");
  ...
  qed_hs_snprintf(conf_include, sizeof(conf_include), "%%include %s", path);
...
test_config_include_flag_defaults_only(void *data)
{
  ...
  char *errmsg = NULL;
  char *path = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_flag_defaults_only"));
  tt_ptr_op(dir, OP_NE, NULL);
...
test_config_include_wildcards(void *data)
{
  ...
  config_line_t *result = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_wildcards"));
  tt_ptr_op(dir, OP_NE, NULL);
  ...
  qed_hs_asprintf(&temp, "%s"PATH_SEPARATOR"%s", dir, "01_one.conf");
  tt_int_op(write_str_to_file(temp, "Test 1\n", 0), OP_EQ, 0);
  qed_hs_free(temp);

  qed_hs_asprintf(&temp, "%s"PATH_SEPARATOR"%s", dir, "02_two.conf");
  tt_int_op(write_str_to_file(temp, "Test 2\n", 0), OP_EQ, 0);
  qed_hs_free(temp);

  qed_hs_asprintf(&temp, "%s"PATH_SEPARATOR"%s", dir, "aa_three.conf");
  tt_int_op(write_str_to_file(temp, "Test 3\n", 0), OP_EQ, 0);
  qed_hs_free(temp);

  qed_hs_asprintf(&temp, "%s"PATH_SEPARATOR"%s", dir, "foo");
  tt_int_op(write_str_to_file(temp, "Test 6\n", 0), OP_EQ, 0);
  qed_hs_free(temp);

  qed_hs_asprintf(&folder, "%s"PATH_SEPARATOR"%s", dir, "folder");
  ...
  qed_hs_asprintf(&temp, "%s"PATH_SEPARATOR"%s", folder, "04_four.conf");
  tt_int_op(write_str_to_file(temp, "Test 4\n", 0), OP_EQ, 0);
  qed_hs_free(temp);

  qed_hs_asprintf(&temp, "%s"PATH_SEPARATOR"%s", folder, "05_five.conf");
  ...
  len = 0;
  for (next = result; next != NULL; next = next->next) {
    qed_hs_snprintf(expected, sizeof(expected), "%d", len + 1 + 3);
    tt_str_op(next->key, OP_EQ, "Test");
    tt_str_op(next->value, OP_EQ, expected);
    len++;
  }
...
test_config_include_hidden(void *data)
{
  ...
  char *temp = NULL, *folder = NULL;
  config_line_t *result = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_hidden"));
  tt_ptr_op(dir, OP_NE, NULL);
  ...
  qed_hs_asprintf(&folder, "%s"PATH_SEPARATOR"%s", dir, ".dotdir");
  ...
  qed_hs_asprintf(&temp, "%s"PATH_SEPARATOR"%s", folder, ".dotfile");
  tt_int_op(write_str_to_file(temp, "Test 1\n", 0), OP_EQ, 0);
  qed_hs_free(temp);

  qed_hs_asprintf(&temp, "%s"PATH_SEPARATOR"%s", folder, "file");
  tt_int_op(write_str_to_file(temp, "Test 2\n", 0), OP_EQ, 0);
  qed_hs_free(temp);
  ...
  for (next = result; next != NULL; next = next->next) {
    qed_hs_snprintf(expected, sizeof(expected), "%d", len + 2);
    tt_str_op(next->key, OP_EQ, "Test");
    tt_str_op(next->value, OP_EQ, expected);
    len++;
  }
...
test_config_include_opened_file_list(void *data)
{
  ...
  char *file = NULL;
  char *dot = NULL;
  char *dir = qed_hs_strdup(get_fname("test_include_opened_file_list"));
  tt_ptr_op(dir, OP_NE, NULL);
  ...
  qed_hs_asprintf(&subfolder, "%s"PATH_SEPARATOR"%s", torrcd, "subfolder");
  ...
  qed_hs_asprintf(&in_subfolder, "%s"PATH_SEPARATOR"%s", subfolder,
               "01_file_in_subfolder");
  tt_int_op(write_str_to_file(in_subfolder, "Test 1\n", 0), OP_EQ, 0);

  qed_hs_asprintf(&empty, "%s"PATH_SEPARATOR"%s", torrcd, "empty");
  tt_int_op(write_str_to_file(empty, "", 0), OP_EQ, 0);

  qed_hs_asprintf(&file, "%s"PATH_SEPARATOR"%s", torrcd, "file");
  tt_int_op(write_str_to_file(file, "Test 2\n", 0), OP_EQ, 0);

  qed_hs_asprintf(&dot, "%s"PATH_SEPARATOR"%s", torrcd, ".dot");
  ...
  config_free_lines(result);
  qed_hs_free(torrcd);
  qed_hs_free(subfolder);
  qed_hs_free(in_subfolder);
  qed_hs_free(empty);
  qed_hs_free(file);
  qed_hs_free(dot);
  qed_hs_free(dir);
...
test_config_kvline_parse(void *arg)
{
  ...
  lines = qed_hs_malloc_zero(sizeof(*lines));
  lines->key = qed_hs_strdup("A=B");
  lines->value = qed_hs_strdup("CD");
  enc = kvline_encode(lines, 0);
  tt_assert(!enc);
  config_free_lines(lines);
