...
{
  process_data_t *process_data = qed_hs_malloc_zero(sizeof(process_data_t));
...
process_data_free(process_data_t *process_data)
{
  ...
  SMARTLIST_FOREACH(process_data->stdout_data, char *, x, qed_hs_free(x));
  SMARTLIST_FOREACH(process_data->stderr_data, char *, x, qed_hs_free(x));
  SMARTLIST_FOREACH(process_data->stdin_data, char *, x, qed_hs_free(x));

  smartlist_free(process_data->stdout_data);
  smartlist_free(process_data->stderr_data);
  smartlist_free(process_data->stdin_data);
  qed_hs_free(process_data);
}
...
process_stdout_callback(process_t *process, const char *data, size_t size)
{
  ...
  smartlist_add(process_data->stdout_data, qed_hs_strdup(data));
...
process_stderr_callback(process_t *process, const char *data, size_t size)
{
  ...
  process_data_t *process_data = process_get_data(process);
  smartlist_add(process_data->stderr_data, qed_hs_strdup(data));
...
run_main_loop(process_data_t *process_data)
{
  ...
  timer_tick_count = 0;
  main_loop_timeout_timer = periodic_timer_new(qed_hs_libevent_get_base(),
                                               &interval,
...
test_callbacks(void *arg)
{
  ...
  process_set_environment(process, "QED_HS_TEST_ENV", "Hello, from Tor!");
  ...
  qed_hs_snprintf(argv0_expected, sizeof(argv0_expected),
               "argv[0] = '%s'", filename);
  ...
  tt_str_op(smartlist_get(process_data->stdout_data, 4), OP_EQ,
            "Environment variable QED_HS_TEST_ENV = 'Hello, from Tor!'");
