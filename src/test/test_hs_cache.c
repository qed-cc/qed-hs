...
helper_fetch_desc_from_hsdir(const ed25519_public_key_t *blinded_key)
{
     ...
     query, and then build the actual query string. */
  {
    ...
    qed_hs_asprintf(&hsdir_query_str, GET("/qed-hs/hs/3/%s"), hsdir_cache_key);
  }
  ...
  or_circ->p_chan = qed_hs_malloc_zero(sizeof(channel_t));
  ...
  qed_hs_free(hsdir_query_str);
  if (conn) {
    qed_hs_free(or_circ->p_chan);
...
test_upload_and_download_hs_desc(void *arg)
{
  ...
  {
    retval = handle_post_hs_descriptor("/qed-hs/hs/3/publish",published_desc_str);
...
test_client_cache(void *arg)
{
  ...
  /* Test handle_response_fetch_hsdesc_v3() */
  {
    ...
    conn->hs_ident = qed_hs_malloc_zero(sizeof(hs_ident_dir_conn_t));
