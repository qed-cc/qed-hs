...
test_md_cache(void *data)
{
  ...
  options->CacheDirectory = qed_hs_strdup(get_fname("md_datadir_test"));
#ifdef _WIN32
  ...
  smartlist_add(wanted, qed_hs_memdup(d2, DIGEST256_LEN));
  smartlist_add(wanted, qed_hs_memdup(d3, DIGEST256_LEN));
  added = microdescs_add_to_cache(mc, test_md2, NULL, SAVED_NOWHERE, 0,
                                  time2, wanted);
  ...
  qed_hs_asprintf(&fn, "%s"PATH_SEPARATOR"cached-microdescs.new",
  ...
  /* read the cache. */
  qed_hs_asprintf(&fn, "%s"PATH_SEPARATOR"cached-microdescs",
               options->CacheDirectory);
  s = read_file_to_str(fn, RFTS_BIN, NULL);
...
test_md_cache_broken(void *data)
{
  ...
  tt_assert(options);
  qed_hs_free(options->CacheDirectory);
  options->CacheDirectory = qed_hs_strdup(get_fname("md_datadir_test2"));
...
test_md_parse(void *arg)
{
  ...
  tt_assert(qed_hs_addr_family(&md->ipv6_addr) == AF_INET6);
  ...
  smartlist_free(mds);
  SMARTLIST_FOREACH(invalid, char *, cp, qed_hs_free(cp));
...
test_md_reject_cache(void *arg)
{
  ...
  qed_hs_free(options->CacheDirectory);
  options->CacheDirectory = qed_hs_strdup(get_fname("md_datadir_test_rej"));
  mock_rgsbd_val_a = qed_hs_malloc_zero(sizeof(routerstatus_t));
  mock_rgsbd_val_b = qed_hs_malloc_zero(sizeof(routerstatus_t));
...
#define ADD(hex)                                                        \
  do {                                                                  \
    tt_int_op(sizeof(buf),OP_EQ,base16_decode(buf,sizeof(buf),          \
                hex,strlen(hex)));\
    smartlist_add(wanted, qed_hs_memdup(buf, DIGEST256_LEN));              \
  } while (0)
  ...
  SMARTLIST_FOREACH(wanted, char *, cp, qed_hs_free(cp));
  smartlist_free(wanted);
  qed_hs_free(mock_rgsbd_val_a);
  qed_hs_free(mock_rgsbd_val_b);
