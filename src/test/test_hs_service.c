...
test_e2e_rend_circuit_setup(void *arg)
{
  ...
  /* Now make dummy circuit */
  {
    ...
    or_circ->build_state = qed_hs_malloc_zero(sizeof(cpath_build_state_t));
...
helper_create_origin_circuit(int purpose, int flags)
{
  ...
  qed_hs_assert(circ);
  circ->cpath = qed_hs_malloc_zero(sizeof(crypt_path_t));
  ...
  circ->hs_ident = qed_hs_malloc_zero(sizeof(hs_ident_circuit_t));
...
helper_clone_authorized_client(const hs_service_authorized_client_t *client)
{
  ...
  client_out = qed_hs_malloc_zero(sizeof(hs_service_authorized_client_t));
...
test_load_keys(void *arg)
{
  int ret;
  char *conf = NULL;
  char *hsdir_v3 = qed_hs_strdup(get_fname("hs3"));
...
#define conf_fmt \
  ...
  /* v3 service. */
  qed_hs_asprintf(&conf, conf_fmt, hsdir_v3, HS_VERSION_THREE);
...
mock_read_file_to_str(const char *filename, int flags, struct stat *stat_out)
{
  ...
  if (!strcmp(filename, get_fname("hs3" PATH_SEPARATOR
                                  ...
                                  "client1.auth"))) {
    ret = qed_hs_strdup("descriptor:x25519:"
...
static smartlist_t *
mock_qed_hs_listdir(const char *dirname)
{
  smartlist_t *file_list = smartlist_new();

  (void) dirname;

  smartlist_add(file_list, qed_hs_strdup("client1.auth"));
  smartlist_add(file_list, qed_hs_strdup("dummy.xxx"));
  smartlist_add(file_list, qed_hs_strdup("client2.auth"));
...
test_load_keys_with_client_auth(void *arg)
{
  ...
  hs_init();
  smartlist_add(pubkey_b32_list, qed_hs_strdup(
                "dz4q5xqlb4ldnbs72iarrml4ephk3du4i7o2cgiva5lwr6wkquja"));
  smartlist_add(pubkey_b32_list, qed_hs_strdup(
                "okoi2gml3wd6x7jganlk5d66xxyjgg24sxw4y7javx4giqr66zta"));
...
#undef conf_fmt
  ...
  MOCK(qed_hs_listdir, mock_qed_hs_listdir);
  ...
  SMARTLIST_FOREACH_BEGIN(pubkey_b32_list, char *, pubkey_b32) {
    ...
    SMARTLIST_FOREACH_BEGIN(service->config.clients,
                            hs_service_authorized_client_t *, client) {
      if (qed_hs_memeq(&pubkey, &client->client_pk, sizeof(pubkey))) {
        is_found = 1;
        break;
      }
  ...
  SMARTLIST_FOREACH(pubkey_b32_list, char *, s, qed_hs_free(s));
...
mock_node_get_by_id(const char *digest)
{
  ...
  if (!qed_hs_memcmp(mock_node.identity, digest, DIGEST_LEN)) {
    return &mock_node;
  }
  return NULL;
}
...
test_bad_introduce2(void *arg)
{
  ...
  entries = metrics_store_get_all(service->metrics.store,
                                  "qed_hs_hs_intro_rejected_intro_req_count");
  ...
  /* Make sure the qed_hs_hs_intro_rejected_intro_req_count metric was
  ...
  /* Make sure the qed_hs_hs_intro_rejected_intro_req_count metric was incremented
...
test_build_update_descriptors(void *arg)
{
  ...
  /* Now, we'll setup a node_t. */
  {
    ...
    qed_hs_addr_parse(&ri.ipv4_addr, "127.0.0.1");
    ri.ipv4_orport = 1337;
    ...
    ri.cache_info.signing_key_cert = qed_hs_malloc_zero(sizeof(qed_hs_cert_t));
  ...
  setup_full_capture_of_logs(LOG_INFO);
  update_all_descriptors_intro_points(now);
  qed_hs_free(node->ri->onion_curve25519_pkey); /* Avoid memleak. */
  qed_hs_free(node->ri->cache_info.signing_key_cert);
...
test_authorized_client_config_equal(void *arg)
{
  ...
  config1 = qed_hs_malloc_zero(sizeof(*config1));
  config2 = qed_hs_malloc_zero(sizeof(*config2));
...
mock_build_state_get_exit_node(cpath_build_state_t *state)
{
  ...
  if (!fake_node) {
    ...
    fake_node = qed_hs_malloc_zero(sizeof(node_t));
    fake_node->ri = qed_hs_malloc_zero(sizeof(routerinfo_t));
...
test_intro2_handling(void *arg)
{
  ...
  intro_circ->cpath = qed_hs_malloc_zero(sizeof(crypt_path_t));
  intro_circ->cpath->prev = intro_circ->cpath;
  intro_circ->hs_ident = qed_hs_malloc_zero(sizeof(*intro_circ->hs_ident));
  origin_circuit_t rend_circ;
  TO_CIRCUIT(&rend_circ)->ccontrol = NULL;
  rend_circ.hs_ident = qed_hs_malloc_zero(sizeof(*rend_circ.hs_ident));
  ...
  const smartlist_t *entries = metrics_store_get_all(
      x_service.metrics.store, "qed_hs_hs_intro_rejected_intro_req_count");
  const metrics_store_entry_t *entry = NULL;
  ...
  qed_hs_free(rend_circ.hs_ident);

  if (fake_node) {
    qed_hs_free(fake_node->ri->onion_curve25519_pkey);
    qed_hs_free(fake_node->ri);
