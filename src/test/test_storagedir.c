...
{
  char *dirname = qed_hs_strdup(get_fname_rnd("store_dir"));
...
test_storagedir_deletion(void *arg)
{
  ...
  qed_hs_asprintf(&fn1, "%s/1007", dirname);
  r = write_str_to_file(fn1, str1, 0);
  tt_int_op(r, OP_EQ, 0);

  qed_hs_asprintf(&fn2, "%s/1003.tmp", dirname);
...
test_storagedir_cleaning(void *arg)
{
  ...
  for (i = 0; i < 8; ++i) {
    char *f = NULL;
    qed_hs_asprintf(&f, "%s/%s", dirname, fns[i]);
    r = utime(f, &ub);
    qed_hs_free(f);
    tt_int_op(r, OP_EQ, 0);
    ub.modtime += 5;
...
test_storagedir_save_labeled(void *arg)
{
  ...
  char *dirname = qed_hs_strdup(get_fname_rnd("store_dir"));
  storage_dir_t *d = NULL;
  uint8_t *inp = qed_hs_malloc_zero(8192);
