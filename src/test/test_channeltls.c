...
static bool tlschan_resolved_addr_is_local_mock(const qed_hs_addr_t *addr);
...
tlschan_connection_or_connect_mock(const qed_hs_addr_t *addr,
                                   uint16_t port,
                                   const char *digest,
                                   const ed25519_public_key_t *ed_id,
                                   channel_tls_t *tlschan)
{
  ...
  /* Make a fake orconn */
  result = qed_hs_malloc_zero(sizeof(*result));
  ...
  result->base_.address = qed_hs_strdup("<fake>");
  memcpy(&(result->base_.addr), addr, sizeof(qed_hs_addr_t));
  result->base_.port = port;
  memcpy(result->identity_digest, digest, DIGEST_LEN);
  result->chan = tlschan;
  memcpy(&result->base_.addr, addr, sizeof(qed_hs_addr_t));
  result->tls = (qed_hs_tls_t *)((void *)(&fake_tortls));
...
tlschan_fake_close_method(channel_t *chan)
{
  ...
  qed_hs_free(tlschan->conn->base_.address);
  qed_hs_free(tlschan->conn);
...
static bool
tlschan_resolved_addr_is_local_mock(const qed_hs_addr_t *addr)
{
  tt_ptr_op(addr, OP_NE, NULL);
