...
rtr_tests_gen_routerinfo(crypto_pk_t *ident_key, crypto_pk_t *tap_key)
{
  ...
  qed_hs_assert(ident_key != NULL);
  qed_hs_assert(tap_key != NULL);

  mock_routerinfo = qed_hs_malloc_zero(sizeof(routerinfo_t));
  mock_routerinfo->nickname = qed_hs_strdup("ConlonNancarrow");
  qed_hs_addr_from_ipv4h(&mock_routerinfo->ipv4_addr, 123456789);
  mock_routerinfo->ipv4_orport = 443;
  mock_routerinfo->platform = qed_hs_strdup("unittest");
  ...
  mock_routerinfo->protocol_list =
    qed_hs_strdup("Cons=1-2 Desc=1-2 DirCache=1-2");
...
test_router_dump_router_to_string_no_bridge_distribution_method(void *arg)
{
  ...
  router->cache_info.signing_key_cert =
    qed_hs_cert_create_ed25519(&identity_keypair,
