...
test_bwmgt_dir_conn_global_write_low(void *arg)
{
  ...
  qed_hs_addr_parse(&relay_addr, "1.2.3.4");

  /* We'll now add a relay into our routerlist and see if we let it. */
  dummy_ns = qed_hs_malloc_zero(sizeof(*dummy_ns));
  dummy_ns->flavor = FLAV_MICRODESC;
  dummy_ns->routerstatus_list = smartlist_new();

  md = qed_hs_malloc_zero(sizeof(*md));
  ri = qed_hs_malloc_zero(sizeof(*ri));
  ...
  qed_hs_addr_copy(&ri->ipv4_addr, &rs->ipv4_addr);
  ...
  dirauth_opts = qed_hs_malloc_zero(sizeof(dirauth_options_t));
  ...
  addr_family = qed_hs_addr_parse(&conn->addr, "1.1.1.1");
  ...
  addr_family = qed_hs_addr_parse(&conn->addr, "128.31.0.39");
  tt_int_op(addr_family, OP_EQ, AF_INET);
  ret = connection_dir_is_global_write_low(conn, INT_MAX);
  tt_int_op(ret, OP_EQ, 0);

  /* IPv6 testing of gabelmoo. */
  addr_family = qed_hs_addr_parse(&conn->addr, "[2001:638:a000:4140::ffff:189]");
  ...
  memcpy(&conn->addr, &relay_addr, sizeof(qed_hs_addr_t));
  ret = connection_dir_is_global_write_low(conn, INT_MAX);
  tt_int_op(ret, OP_EQ, 0);

  /* Lets retry with a random IP that is not an authority nor a relay. */
  addr_family = qed_hs_addr_parse(&conn->addr, "1.2.3.4");
  tt_int_op(addr_family, OP_EQ, AF_INET);
  ret = connection_dir_is_global_write_low(conn, INT_MAX);
  tt_int_op(ret, OP_EQ, 0);
  ...
  addr_family = qed_hs_addr_parse(&conn->addr, "[CAFE::ACAB]");
  tt_int_op(addr_family, OP_EQ, AF_INET6);
  ret = connection_dir_is_global_write_low(conn, INT_MAX);
  tt_int_op(ret, OP_EQ, 1);
