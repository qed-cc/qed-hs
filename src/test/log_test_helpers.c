...
mock_clean_saved_logs(void)
{
  ...
  SMARTLIST_FOREACH(saved_logs, mock_saved_log_entry_t *, m,
                    { qed_hs_free(m->generated_msg); qed_hs_free(m); });
...
mock_saving_logv(int severity, log_domain_mask_t domain,
                 ...
                 const char *format, va_list ap)
{
  char *buf = qed_hs_malloc_zero(10240);
  int n;
  n = qed_hs_vsnprintf(buf,10240,format,ap);
  qed_hs_assert(n < 10240-1);
  buf[n]='\n';
  buf[n+1]='\0';

  if (echo_to_real_logs) {
    qed_hs_log(severity, domain|LD_NO_MOCK, "%s", buf);
  }
  ...
  mock_saved_log_entry_t *e = qed_hs_malloc_zero(sizeof(mock_saved_log_entry_t));
  e->severity = severity;
  e->funcname = funcname;
  e->suffix = suffix;
  e->format = format;
  e->generated_msg = qed_hs_strdup(buf);
  qed_hs_free(buf);
