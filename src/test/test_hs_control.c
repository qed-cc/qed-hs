...
test_hs_desc_event(void *arg)
{
  ...
  hs_control_desc_event_requested(&identity_kp.pubkey, base64_blinded_pk,
                                  &hsdir_rs);
  qed_hs_asprintf(&expected_msg, "650 HS_DESC REQUESTED %s NO_AUTH "
  ...
  qed_hs_free(received_msg);
  qed_hs_free(expected_msg);

  /* HS_DESC CREATED... */
  hs_control_desc_event_created(onion_address, &blinded_pk);
  qed_hs_asprintf(&expected_msg, "650 HS_DESC CREATED %s UNKNOWN "
                              "UNKNOWN %s\r\n",
               onion_address, base64_blinded_pk);
  tt_assert(received_msg);
  tt_str_op(received_msg, OP_EQ, expected_msg);
  ...
  qed_hs_asprintf(&expected_msg, "650 HS_DESC UPLOAD %s UNKNOWN "
  ...
  qed_hs_asprintf(&expected_msg, "650 HS_DESC FAILED %s NO_AUTH "
                              STR_HSDIR_EXIST_LONGNAME " %s "
                              "REASON=BAD_DESC\r\n",
               onion_address, base64_blinded_pk);
  tt_assert(received_msg);
  ...
  qed_hs_asprintf(&expected_msg, "650 HS_DESC RECEIVED %s NO_AUTH "
                              STR_HSDIR_EXIST_LONGNAME " %s\r\n",
               onion_address, base64_blinded_pk);
  tt_assert(received_msg);
  tt_str_op(received_msg, OP_EQ, expected_msg);
  ...
  qed_hs_asprintf(&expected_msg, "650 HS_DESC UPLOADED %s UNKNOWN "
...
test_hs_control_good_onion_client_auth_add(void *arg)
{
  ...
  { /* Setup the control conn */
    ...
    conn.current_cmd = qed_hs_strdup("ONION_CLIENT_AUTH_ADD");
  }
  ...
  args = qed_hs_strdup("2fvhjskjet3n5syd6yfg5lhvwcs62bojmthr35ko5bllr3iqdb4ctdyd "
  ...
  args = qed_hs_strdup("jt4grrjwzyz3pjkylwfau5xnjaj23vxmhskqaeyfhrfylelw4hvxcuyd "
  ...
  /* Register second service (even with an unrecognized argument) */
  args = qed_hs_strdup("jamie3vkiwibfiwucd6vxijskbhpjdyajmzeor4mc4i7yopvpo4p7cyd "
  ...
  conn.current_cmd = qed_hs_strdup("ONION_CLIENT_AUTH_VIEW");
...
#define VIEW_CORRECT_REPLY_NO_ADDR "250-ONION_CLIENT_AUTH_VIEW\r\n"   \
  ...
  qed_hs_free(args);
  args =
    qed_hs_strdup("jt4grrjwzyz3pjkylwfau5xnjaj23vxmhskqaeyfhrfylelw4hvxcuyd");
...
#define VIEW_CORRECT_REPLY_JT4 "250-ONION_CLIENT_AUTH_VIEW " \
  ...
  qed_hs_free(conn.current_cmd);
  conn.current_cmd = qed_hs_strdup("ONION_CLIENT_AUTH_REMOVE");

  /* First try with a wrong addr */
  qed_hs_free(args);
  args = qed_hs_strdup("thatsok");
  ...
  qed_hs_free(args);
  args =
    qed_hs_strdup("2fvhjskjet3n5syd6yfg5lhvwcs62bojmthr35ko5bllr3iqdb4ctdyd");
  ...
  qed_hs_free(args);
  args =
    qed_hs_strdup("jamie3vkiwibfiwucd6vxijskbhpjdyajmzeor4mc4i7yopvpo4p7cyd");
...
test_hs_control_store_permanent_creds(void *arg)
{
  ...
  { /* Setup ClientOnionAuthDir */
    int ret;
    char *perm_creds_dir = qed_hs_strdup(get_fname("permanent_credentials"));
  ...
  /* Check file contents! */
  creds_fname = qed_hs_strdup(get_fname("permanent_credentials/"
  ...
  qed_hs_free(get_options_mutable()->ClientOnionAuthDir);
  qed_hs_free(args);
  qed_hs_free(cp1);
  buf_free(TO_CONN(&conn)->outbuf);
  qed_hs_free(conn.current_cmd);
  qed_hs_free(creds_fname);
  qed_hs_free(creds_file_str);
  hs_client_free_all();
}
...
test_hs_control_add_onion_with_bad_pubkey(void *arg)
{
  ...
  { /* Setup the control conn */
    memset(&conn, 0, sizeof(control_connection_t));
    TO_CONN(&conn)->outbuf = buf_new();
    conn.current_cmd = qed_hs_strdup("ADD_ONION");
  }

  args = qed_hs_strdup("ED25519-V3:AAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
                    "AAAAAAAAAAAAAAAAAAAAAAA"
...
test_hs_control_add_auth_onion_service(void *arg)
{
  ...
  TO_CONN(&conn)->outbuf = buf_new();
  conn.current_cmd = qed_hs_strdup("ADD_ONION");
  args = qed_hs_strdup("ED25519-V3:KLMQ4CLKwlDCHuMPn8j3od33cU5LhnrLNoZh7CWChl3VkY"
  ...
  args = qed_hs_strdup("ED25519-V3:iIU8EBi71qE7G6UTsROU1kWN0JMrRP/YukC0Xk5WLGyil3"
...
test_hs_control_add_onion_helper_add_service(void *arg)
{
  ...
  qed_hs_free(key_new_blob_good);
  qed_hs_free(key_new_blob_bad);
  qed_hs_free(address_out_good);
  qed_hs_free(address_out_bad);
