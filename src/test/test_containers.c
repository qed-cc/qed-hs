...
{
  smartlist_t *sl;
  char *v0 = qed_hs_strdup("v0");
  char *v1 = qed_hs_strdup("v1");
  char *v2 = qed_hs_strdup("v2");
  char *v3 = qed_hs_strdup("v3");
  char *v4 = qed_hs_strdup("v4");
  char *v22 = qed_hs_strdup("v22");
  char *v99 = qed_hs_strdup("v99");
  char *v555 = qed_hs_strdup("v555");
...
test_container_smartlist_strings(void *arg)
{
  ...
  SMARTLIST_FOREACH(sl, char *, str, qed_hs_free(str));
  smartlist_clear(sl);
  ...
  smartlist_split_string(sl,
                     ...
                     ",", 0, 0);
  smartlist_sort(sl, compare_strs_);
  smartlist_uniq(sl, compare_strs_, qed_hs_free_);
  cp_alloc = smartlist_join_strings(sl, ",", 0, NULL);
...
test_container_smartlist_digests(void *arg)
{
  ...
  /* contains_digest */
  (void)arg;
  smartlist_add(sl, qed_hs_memdup("AAAAAAAAAAAAAAAAAAAA", DIGEST_LEN));
  smartlist_add(sl, qed_hs_memdup("\00090AAB2AAAAaasdAAAAA", DIGEST_LEN));
  smartlist_add(sl, qed_hs_memdup("\00090AAB2AAAAaasdAAAAA", DIGEST_LEN));
...
test_container_smartlist_join(void *arg)
{
  ...
  smartlist_free(sl4);
  smartlist_free(sl3);
  SMARTLIST_FOREACH(sl2, char *, cp, qed_hs_free(cp));
  smartlist_free(sl2);
  SMARTLIST_FOREACH(sl, char *, str, qed_hs_free(str));
  smartlist_free(sl);
  qed_hs_free(joined);
...
test_container_smartlist_ints_eq(void *arg)
{
  ...
  x = 5;
  smartlist_add(sl1, qed_hs_memdup(&x, sizeof(int)));
  smartlist_add(sl2, qed_hs_memdup(&x, sizeof(int)));
  x = 90;
  smartlist_add(sl1, qed_hs_memdup(&x, sizeof(int)));
  smartlist_add(sl2, qed_hs_memdup(&x, sizeof(int)));
  tt_assert(smartlist_ints_eq(sl1, sl2));
  ...
  *(int*)smartlist_get(sl2, 1) = 101010;
  ...
  if (sl1)
    SMARTLIST_FOREACH(sl1, int *, ip, qed_hs_free(ip));
  if (sl2)
    SMARTLIST_FOREACH(sl2, int *, ip, qed_hs_free(ip));
  smartlist_free(sl1);
  smartlist_free(sl2);
}
...
test_container_digestset(void *arg)
{
  ...
  for (i = 0; i < 1000; ++i) {
    ...
    smartlist_add(included, qed_hs_memdup(d, DIGEST_LEN));
  }
  ...
  if (set)
    digestset_free(set);
  SMARTLIST_FOREACH(included, char *, cp, qed_hs_free(cp));
...
test_container_strmap(void *arg)
{
  ...
  char *v1 = qed_hs_strdup("v1");
  char *v99 = qed_hs_strdup("v99");
  char *v100 = qed_hs_strdup("v100");
  char *v101 = qed_hs_strdup("v101");
  char *v102 = qed_hs_strdup("v102");
  char *v103 = qed_hs_strdup("v103");
  char *v104 = qed_hs_strdup("v104");
  char *v105 = qed_hs_strdup("v105");
  ...
  if (map)
    strmap_free(map,NULL);
  if (found_keys) {
    SMARTLIST_FOREACH(found_keys, char *, cp, qed_hs_free(cp));
...
test_container_di_map(void *arg)
{
  ...
  char *v1 = qed_hs_strdup(", it came close to causing a disaster...");
  char *v2 = qed_hs_strdup("I regret to have to advise you that the mission");
  char *v3 = qed_hs_strdup("was actually initiated...");
...
test_container_fp_pair_map(void *arg)
{
  ...
  while (!fp_pair_map_iter_done(iter)) {
    ...
    if (qed_hs_memeq(&fp2, &k, sizeof(fp2))) {
...
#define EQ_SHOULD_SAY(s1,s2,val)                                \
  do {                                                          \
    SMARTLIST_FOREACH(sl1, char *, cp, qed_hs_free(cp));           \
    SMARTLIST_FOREACH(sl2, char *, cp, qed_hs_free(cp));           \
    smartlist_clear(sl1);                                       \
    smartlist_clear(sl2);                                       \
