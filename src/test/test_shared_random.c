...
test_sr_setup_srv(int also_current)
{
  ...
  sr_srv_t *srv = qed_hs_malloc_zero(sizeof(sr_srv_t));
  srv->num_reveals = 42;
  memcpy(srv->value,
 ...
 if (also_current) {
   srv = qed_hs_malloc_zero(sizeof(sr_srv_t));
   srv->num_reveals = 128;
   memcpy(srv->value,
...
{
  int ret;
  char *dir = qed_hs_strdup(get_fname("test_sr_state"));
  char *sr_state_path = qed_hs_strdup(get_fname("test_sr_state/sr_state"));
...
test_sr_setup_commits(void)
{
  ...
  sr_commit_t *place_holder = qed_hs_malloc_zero(sizeof(*place_holder));
  authority_cert_t *auth_cert = NULL;
...
{
  networkstatus_t *vote = qed_hs_malloc_zero(sizeof(networkstatus_t));

  vote->type = NS_TYPE_VOTE;
  vote->sr_info.participate = 1;
  vote->sr_info.current_srv = qed_hs_malloc_zero(sizeof(sr_srv_t));
...
test_keep_commit(void *arg)
{
  ...
  dup_commit = qed_hs_malloc_zero(sizeof(*dup_commit));
