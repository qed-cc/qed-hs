...
dir_common_gen_routerstatus_for_v3ns(int idx, time_t now)
{
  ...
  switch (idx) {
    case 0:
      ...
      rs->is_v2_dir = 1;
      rs->is_valid = 1; /* xxxxx */
      vrs->protocols = qed_hs_strdup("Link=7 HSDir=3");
      break;
    case 1:
      ...
      vrs->protocols = qed_hs_strdup("Link=3,4 HSDir=2,3");
      break;
    case 2:
    ...
    case 3:
      ...
      rs->is_v2_dir = 1;
      vrs->protocols = qed_hs_strdup("Link=3,4 HSDir=3");
...
{
  *vote = qed_hs_malloc_zero(sizeof(networkstatus_t));
                        ...
                        time_t now, int clear_rl)
{
  ...
  (*vote)->client_versions = qed_hs_strdup("0.1.2.14,0.1.2.15");
  (*vote)->server_versions = qed_hs_strdup("0.1.2.14,0.1.2.15,0.1.2.16");
  smartlist_split_string((*vote)->known_flags,
                     "Authority Exit Fast Guard Running Stable V2Dir Valid",
                     0, SPLIT_SKIP_SPACE|SPLIT_IGNORE_BLANK, 0);
  voter = qed_hs_malloc_zero(sizeof(networkstatus_voter_info_t));
  voter->nickname = qed_hs_strdup("Voter1");
  voter->address = qed_hs_strdup("1.2.3.4");
  qed_hs_addr_from_ipv4h(&voter->ipv4_addr, 0x01020304);
  voter->ipv4_dirport = 80;
  voter->ipv4_orport = 9000;
  voter->contact = qed_hs_strdup("voter@example.com");
                        ...
                        time_t now, int clear_rl)
{
  ...
  voter = qed_hs_malloc_zero(sizeof(networkstatus_voter_info_t));
  voter->nickname = qed_hs_strdup("Voter2");
  voter->address = qed_hs_strdup("2.3.4.5");
  qed_hs_addr_from_ipv4h(&voter->ipv4_addr, 0x02030405);
  voter->ipv4_dirport = 80;
  voter->ipv4_orport = 9000;
  voter->contact = qed_hs_strdup("voter@example.com");
  crypto_pk_get_digest(cert->identity_key, voter->identity_digest);
                        ...
                        time_t now, int clear_rl)
{
  ...
  (*vote)->client_versions = qed_hs_strdup("0.1.2.14,0.1.2.17");
  (*vote)->server_versions = qed_hs_strdup("0.1.2.10,0.1.2.15,0.1.2.16");
  smartlist_split_string((*vote)->known_flags,
                     "Authority Exit Fast Guard Running Stable V2Dir Valid",
                     0, SPLIT_SKIP_SPACE|SPLIT_IGNORE_BLANK, 0);
  voter = qed_hs_malloc_zero(sizeof(networkstatus_voter_info_t));
  voter->nickname = qed_hs_strdup("Voter2");
  voter->address = qed_hs_strdup("3.4.5.6");
  qed_hs_addr_from_ipv4h(&voter->ipv4_addr, 0x03040506);
  voter->ipv4_dirport = 80;
  voter->ipv4_orport = 9000;
  voter->contact = qed_hs_strdup("voter@example.com");
  crypto_pk_get_digest(cert->identity_key, voter->identity_digest);
