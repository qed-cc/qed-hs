...
static qed_hs_tls_t *mock_peer_cert_expect_tortls = NULL;
static qed_hs_x509_cert_t *mock_peer_cert = NULL;
static qed_hs_x509_cert_t *
mock_get_peer_cert(qed_hs_tls_t *tls)
{
  if (mock_peer_cert_expect_tortls &&
      mock_peer_cert_expect_tortls != tls)
    return NULL;
  return qed_hs_x509_cert_dup(mock_peer_cert);
}
...
static int
mock_export_key_material(qed_hs_tls_t *tls, uint8_t *secrets_out,
...
static qed_hs_x509_cert_t *mock_own_cert = NULL;
static qed_hs_x509_cert_t *
mock_get_own_cert(qed_hs_tls_t *tls)
{
  (void)tls;
  return qed_hs_x509_cert_dup(mock_own_cert);
}
...
test_link_handshake_certs_ok(void *arg)
{
  ...
  qed_hs_addr_from_ipv4h(&c1->base_.addr, 0x7f000001);
  qed_hs_addr_from_ipv4h(&c2->base_.addr, 0x7f000001);
  ...
  MOCK(qed_hs_tls_get_peer_cert, mock_get_peer_cert);
  MOCK(qed_hs_tls_get_own_cert, mock_get_own_cert);
  ...
  /* c1 has started_here == 1 */
  {
    ...
    tt_assert(!qed_hs_tls_get_my_certs(1, &link_cert, NULL));
    mock_own_cert = qed_hs_x509_cert_dup(link_cert);
  }
  ...
  chan1 = qed_hs_malloc_zero(sizeof(*chan1));
  channel_tls_common_init(chan1);
  c1->chan = chan1;
  chan1->conn = c1;
  c1->base_.address = qed_hs_strdup("C1");
  c1->tls = qed_hs_tls_new(-1, 0);
  c1->link_proto = 4;
  c1->base_.conn_array_index = -1;
  crypto_pk_get_digest(key2, c1->identity_digest);

  if (with_ed) {
    const qed_hs_x509_cert_t *linkc, *idc;
    qed_hs_tls_get_my_certs(1, &linkc, &idc);
    mock_peer_cert_expect_tortls = c1->tls; /* We should see this tls... */
    mock_peer_cert = qed_hs_x509_cert_dup(linkc); /* and when we do, the peer's
                                                *  cert is this... */
  }
  channel_tls_process_certs_cell(cell2, chan1);
  mock_peer_cert_expect_tortls = NULL;
  qed_hs_x509_cert_free(mock_peer_cert);
  mock_peer_cert = NULL;

  qed_hs_assert(c1->handshake_state->authenticated);
  ...
  chan2 = qed_hs_malloc_zero(sizeof(*chan2));
  channel_tls_common_init(chan2);
  c2->chan = chan2;
  chan2->conn = c2;
  c2->base_.address = qed_hs_strdup("C2");
  c2->tls = qed_hs_tls_new(-1, 1);
  c2->link_proto = 4;
  ...
  UNMOCK(qed_hs_tls_get_own_cert);
  qed_hs_x509_cert_free(mock_own_cert);
  qed_hs_x509_cert_free(mock_peer_cert);
...
{
  (void)test;
  certs_data_t *d = qed_hs_malloc_zero(sizeof(*d));
  ...
  qed_hs_assert(is_ed != is_rsa);
  qed_hs_assert(is_link != is_auth);

  d->c = or_connection_new(CONN_TYPE_OR, AF_INET);
  d->chan = qed_hs_malloc_zero(sizeof(*d->chan));
  d->c->chan = d->chan;
  d->c->base_.address = qed_hs_strdup("HaveAnAddress");
  qed_hs_addr_from_ipv4h(&d->c->base_.addr, 0x801f0127);
  ...
  const qed_hs_x509_cert_t *a,*b;
  const uint8_t *enca, *encb;
  size_t lena, lenb;
  qed_hs_tls_get_my_certs(is_link ? 1 : 0, &a, &b);
  qed_hs_x509_cert_get_der(a, &enca, &lena);
  qed_hs_x509_cert_get_der(b, &encb, &lenb);
  ...
  if (is_ed) {
    ...
    const qed_hs_cert_t *id_sign = get_master_signing_key_cert();
    const qed_hs_cert_t *secondary =
  ...
  if (is_link) {
    ...
    mock_peer_cert = qed_hs_x509_cert_dup(a);
  }
...
CERTS_FAIL(missing_signing_key, /* ed25519 */
  {
    ...
    qed_hs_cert_t *bad_cert = qed_hs_cert_create_ed25519(mk, CERT_TYPE_ID_SIGNING,
...
CERTS_FAIL(link_mismatch, /* ed25519 */
  {
    require_failure_message = "Link certificate does not match "
      "TLS certificate";
    const qed_hs_x509_cert_t *idc;
    qed_hs_tls_get_my_certs(1, NULL, &idc);
    qed_hs_x509_cert_free(mock_peer_cert);
    /* Pretend that the peer cert was something else. */
    mock_peer_cert = qed_hs_x509_cert_dup(idc);
    /* No reencode needed. */
  })
...
CERTS_FAIL(bad_rsa_id_cert, /*ed25519*/
  {
    ...
    const qed_hs_x509_cert_t *idc;
    qed_hs_x509_cert_t *newc;
    qed_hs_tls_get_my_certs(1, NULL, &idc);
    time_t new_end = time(NULL) + 86400 * 10;
    newc = qed_hs_x509_cert_replace_expiration(idc, new_end, d->key2);
    const uint8_t *encoded;
    size_t encoded_len;
    qed_hs_x509_cert_get_der(newc, &encoded, &encoded_len);
...
CERTS_FAIL(wrong_labels_2,
  {
    const qed_hs_x509_cert_t *a;
    const qed_hs_x509_cert_t *b;
    const uint8_t *enca;
    size_t lena;
    require_failure_message = "The link certificate was not valid";
    qed_hs_tls_get_my_certs(1, &a, &b);
    qed_hs_x509_cert_get_der(a, &enca, &lena);
...
recv_authchallenge_setup(const struct testcase_t *test)
{
  ...
  authchallenge_data_t *d = qed_hs_malloc_zero(sizeof(*d));
  d->c = or_connection_new(CONN_TYPE_OR, AF_INET);
  d->chan = qed_hs_malloc_zero(sizeof(*d->chan));
  d->c->chan = d->chan;
  d->c->base_.address = qed_hs_strdup("HaveAnAddress");
...
authenticate_data_cleanup(const struct testcase_t *test, void *arg)
{
  ...
  UNMOCK(qed_hs_tls_export_key_material);
...
{
  authenticate_data_t *d = qed_hs_malloc_zero(sizeof(*d));
  ...
  MOCK(qed_hs_tls_export_key_material, mock_export_key_material);
  d->c1 = or_connection_new(CONN_TYPE_OR, AF_INET);
  d->c2 = or_connection_new(CONN_TYPE_OR, AF_INET);
  qed_hs_addr_from_ipv4h(&d->c1->base_.addr, 0x01020304);
  qed_hs_addr_from_ipv4h(&d->c2->base_.addr, 0x05060708);
  ...
  d->chan2 = qed_hs_malloc_zero(sizeof(*d->chan2));
  channel_tls_common_init(d->chan2);
  d->c2->chan = d->chan2;
  d->chan2->conn = d->c2;
  d->c2->base_.address = qed_hs_strdup("C2");
  d->c2->tls = qed_hs_tls_new(-1, 1);
  d->c2->handshake_state->received_certs_cell = 1;

  const qed_hs_x509_cert_t *id_cert=NULL, *link_cert=NULL;
  tt_assert(! qed_hs_tls_get_my_certs(1, &link_cert, &id_cert));

  const uint8_t *der;
  size_t sz;
  qed_hs_x509_cert_get_der(id_cert, &der, &sz);
  d->c1->handshake_state->certs->id_cert = qed_hs_x509_cert_decode(der, sz);
  d->c2->handshake_state->certs->id_cert = qed_hs_x509_cert_decode(der, sz);

  {
    d->c1->handshake_state->certs->ed_id_sign =
      qed_hs_cert_dup(get_master_signing_key_cert());
    d->c2->handshake_state->certs->ed_id_sign =
      qed_hs_cert_dup(get_master_signing_key_cert());
    d->c2->handshake_state->certs->ed_sign_auth =
      qed_hs_cert_dup(get_current_auth_key_cert());
  }

  qed_hs_x509_cert_get_der(link_cert, &der, &sz);
  mock_peer_cert = qed_hs_x509_cert_decode(der, sz);
  tt_assert(mock_peer_cert);

  mock_own_cert = qed_hs_x509_cert_decode(der, sz);
...
AUTHENTICATE_FAIL(noidcert,
                  require_failure_message = "We never got an identity "
                    "certificate";
                  qed_hs_x509_cert_free(d->c2->handshake_state->certs->id_cert);
...
AUTHENTICATE_FAIL(missing_ed_id,
                {
                  qed_hs_cert_free(d->c2->handshake_state->certs->ed_id_sign);
...
AUTHENTICATE_FAIL(missing_ed_auth,
                {
                  qed_hs_cert_free(d->c2->handshake_state->certs->ed_sign_auth);
