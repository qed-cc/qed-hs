...
legacy_validate_pasture(const void *old_, void *obj, char **msg_out)
{
  ...
  if (p->address)
    qed_hs_strupper(p->address);

  if (old && old->address &&
      (!p->address || strcmp(old->address, p->address))) {
    *msg_out = qed_hs_strdup("You can't move a pasture.");
    return -1;
  }
...
validate_llama(const void *obj, char **msg_out)
{
  ...
  qed_hs_assert(llama->magic == 0x11aa11);

  if (! llama->llamaname || strlen(llama->llamaname) == 0) {
    *msg_out = qed_hs_strdup("A llama has no name!?");
    return -1;
  }

  if (strspn(llama->llamaname, "0123456789") == strlen(llama->llamaname)) {
    *msg_out = qed_hs_strdup("It is not a number; it is a free llama!");
    return -1;
  }
...
check_transition_alpaca(const void *old_, const void *new_, char **msg_out)
{
  ...
  qed_hs_assert(old_alpaca && new_alpaca);
  qed_hs_assert(old_alpaca->magic == 0xa15aca);
  qed_hs_assert(new_alpaca->magic == 0xa15aca);

  if (old_alpaca->fuzziness > new_alpaca->fuzziness) {
    *msg_out = qed_hs_strdup("An alpaca only becomes more fuzzy over time.");
    return -1;
  }
...
post_normalize_llama(void *obj, char **msg_out)
{
  ...
  qed_hs_assert(llama->magic == 0x11aa11);
  qed_hs_assert(llama->llamaname); // we have already checked for a NULL name.
  qed_hs_free(llama->description);
  qed_hs_asprintf(&llama->description, "A llama called %s.", llama->llamaname);
  return 0;
}
...
pre_normalize_alpaca(void *obj, char **msg_out)
{
  ...
  qed_hs_assert(alpaca->magic == 0xa15aca);
...
test_confmgr_parse(void *arg)
{
  ...
  mut_lc->description = qed_hs_strdup("A llama named Hugo.");
  config_free(mgr, p);
                   ...
                   const char *inp, const pasture_cfg_t *old, char **msg_out)
{
  ...
  tt_int_op(r, OP_EQ, 0);
  qed_hs_free(*msg_out); // sets it to NULL
