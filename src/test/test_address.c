                       ...
                       struct sockaddr_in6 *sockaddr2)
{
   return ((sockaddr1->sin6_family == sockaddr2->sin6_family) &&
           ...
           (qed_hs_memeq(sockaddr1->sin6_addr.s6_addr,
...
sockaddr_in_from_string(const char *ip_str, struct sockaddr_in *out)
{
  ...
  if (!out)
    out = qed_hs_malloc_zero(sizeof(struct sockaddr_in));

  out->sin_family = AF_INET;
  out->sin_port = 0;
  qed_hs_inet_pton(AF_INET,ip_str,&(out->sin_addr));

  return out;
}

/** Return 1 iff <b>smartlist</b> contains a qed_hs_addr_t structure
 * that is an IPv4 or IPv6 localhost address. Otherwise, return 0.
 */
static int
smartlist_contains_localhost_qed_hs_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, qed_hs_addr_t *, qed_hs_addr) {
    if (qed_hs_addr_is_loopback(qed_hs_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(qed_hs_addr);
 ...
 */
static int
smartlist_contains_multicast_qed_hs_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, qed_hs_addr_t *, qed_hs_addr) {
    if (qed_hs_addr_is_multicast(qed_hs_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(qed_hs_addr);
 ...
 */
static int
smartlist_contains_internal_qed_hs_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, qed_hs_addr_t *, qed_hs_addr) {
    if (qed_hs_addr_is_internal(qed_hs_addr, 0)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(qed_hs_addr);
...
/** Return 1 iff <b>smartlist</b> contains a qed_hs_addr_t structure
 * that is NULL or the null qed_hs_addr_t. Otherwise, return 0.
 */
static int
smartlist_contains_null_qed_hs_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, qed_hs_addr_t *, qed_hs_addr) {
    if (qed_hs_addr == NULL || qed_hs_addr_is_null(qed_hs_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(qed_hs_addr);
 ...
 */
static int
smartlist_contains_ipv4_qed_hs_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, qed_hs_addr_t *, qed_hs_addr) {
    if (qed_hs_addr_is_v4(qed_hs_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(qed_hs_addr);
 ...
 */
static int
smartlist_contains_ipv6_qed_hs_addr(smartlist_t *smartlist)
{
  SMARTLIST_FOREACH_BEGIN(smartlist, qed_hs_addr_t *, qed_hs_addr) {
    /* Since there's no qed_hs_addr_is_v6, assume all non-v4s are v6 */
    if (!qed_hs_addr_is_v4(qed_hs_addr)) {
      return 1;
    }
  } SMARTLIST_FOREACH_END(qed_hs_addr);
...
test_address_ifaddrs_to_smartlist(void *arg)
{
   ...
   qed_hs_addr_t *qed_hs_addr = NULL;
   ...
   ipv6_sockaddr = qed_hs_malloc(sizeof(struct sockaddr_in6));
   ipv6_sockaddr->sin6_family = AF_INET6;
   ipv6_sockaddr->sin6_port = 0;
   qed_hs_inet_pton(AF_INET6, "2001:db8:8714:3a90::12",
                 &(ipv6_sockaddr->sin6_addr));

   ifa = qed_hs_malloc(sizeof(struct ifaddrs));
   ifa_ipv4 = qed_hs_malloc(sizeof(struct ifaddrs));
   ifa_ipv6 = qed_hs_malloc(sizeof(struct ifaddrs));

   ifa->ifa_next = ifa_ipv4;
   ifa->ifa_name = qed_hs_strdup("eth0");
   ...
   ifa_ipv4->ifa_name = qed_hs_strdup("eth1");
   ...
   ifa_ipv6->ifa_name = qed_hs_strdup("eth2");
   ...
   sockaddr_to_check = qed_hs_malloc(sizeof(struct sockaddr_in6));

   qed_hs_addr = smartlist_get(smartlist,0);
   addr_len =
   qed_hs_addr_to_sockaddr(qed_hs_addr,0,sockaddr_to_check,
   ...
   qed_hs_addr = smartlist_get(smartlist,1);
   addr_len =
   qed_hs_addr_to_sockaddr(qed_hs_addr,0,sockaddr_to_check,
                        sizeof(struct sockaddr_in));
   ...
   qed_hs_addr = smartlist_get(smartlist,2);
   addr_len =
   qed_hs_addr_to_sockaddr(qed_hs_addr,0,sockaddr_to_check,
   ...
   qed_hs_free(netmask_slash8);
   qed_hs_free(ipv4_sockaddr_local);
   qed_hs_free(ipv4_sockaddr_remote);
   qed_hs_free(ipv6_sockaddr);
   qed_hs_free(ifa->ifa_name);
   qed_hs_free(ifa_ipv4->ifa_name);
   qed_hs_free(ifa_ipv6->ifa_name);
   qed_hs_free(ifa);
   qed_hs_free(ifa_ipv4);
   qed_hs_free(ifa_ipv6);
   qed_hs_free(sockaddr_to_check);
   if (smartlist) {
     SMARTLIST_FOREACH(smartlist, qed_hs_addr_t *, t, qed_hs_free(t));
...
test_address_get_if_addrs_ifaddrs(void *arg)
{
   ...
   * all. */
  tt_assert(!smartlist_contains_null_qed_hs_addr(results));

  /* If there are addresses, they must be IPv4 or IPv6 */
  if (smartlist_len(results) > 0) {
    tt_assert(smartlist_contains_ipv4_qed_hs_addr(results)
              || smartlist_contains_ipv6_qed_hs_addr(results));
  }

  done:
  if (results) {
    SMARTLIST_FOREACH(results, qed_hs_addr_t *, t, qed_hs_free(t));
  }
...
test_address_get_if_addrs_win32(void *arg)
{
  ...
  tt_assert(smartlist_contains_localhost_qed_hs_addr(results));
  tt_assert(!smartlist_contains_null_qed_hs_addr(results));
...
test_address_ip_adapter_addresses_to_smartlist(void *arg)
{
  ...
  qed_hs_addr_t *qed_hs_addr;

  (void)arg;
  (void)sockaddr_in6_are_equal;

  sockaddr_to_check = qed_hs_malloc_zero(sizeof(struct sockaddr_in));

  addrs1 =
  qed_hs_malloc_zero(sizeof(IP_ADAPTER_ADDRESSES));

  addrs1->FirstUnicastAddress =
  unicast11 = qed_hs_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
  ...
  qed_hs_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
  ...
  unicast21 = qed_hs_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
  ...
  qed_hs_addr = smartlist_get(result,0);

  qed_hs_addr_to_sockaddr(qed_hs_addr,0,(struct sockaddr *)sockaddr_to_check,
                       sizeof(struct sockaddr_in));

  tt_assert(sockaddr_in_are_equal(sockaddr_test1,sockaddr_to_check));

  qed_hs_addr = smartlist_get(result,1);
  ...
  qed_hs_addr = smartlist_get(result,2);
  ...
  SMARTLIST_FOREACH(result, qed_hs_addr_t *, t, qed_hs_free(t));
  smartlist_free(result);
  qed_hs_free(addrs1);
  qed_hs_free(addrs2);
  qed_hs_free(unicast11->Address.lpSockaddr);
  qed_hs_free(unicast11);
  qed_hs_free(unicast12->Address.lpSockaddr);
  qed_hs_free(unicast12);
  qed_hs_free(unicast21->Address.lpSockaddr);
  qed_hs_free(unicast21);
  qed_hs_free(sockaddr_to_check);
  return;
}
...
{
  smartlist_t *results = NULL;
  const qed_hs_addr_t *qed_hs_addr = NULL;
  ...
  sockaddr_to_check = qed_hs_malloc(sizeof(struct sockaddr_in));

  ifr = qed_hs_malloc(sizeof(struct ifreq));
  ...
  ifc = qed_hs_malloc(sizeof(struct ifconf));
  ...
  qed_hs_addr = smartlist_get(results, 0);
  addr_len =
  qed_hs_addr_to_sockaddr(qed_hs_addr,0,(struct sockaddr *)sockaddr_to_check,
                       sizeof(struct sockaddr_in));
  ...
  ifr = qed_hs_realloc(ifr,2*sizeof(struct ifreq));
  ifr_next = ifr+1;
  ...
  qed_hs_addr = smartlist_get(results, 1);
  addr_len =
  qed_hs_addr_to_sockaddr(qed_hs_addr,0,(struct sockaddr *)sockaddr_to_check,
                       sizeof(struct sockaddr_in));
...
test_address_get_if_addrs_ioctl(void *arg)
{
  ...
  */
   ...
   * all. */
  if (result) {
    tt_assert(!smartlist_contains_null_qed_hs_addr(result));

    /* If there are addresses, they must be IPv4 or IPv6.
     * (AIX supports IPv6 from SIOCGIFCONF.) */
    if (smartlist_len(result) > 0) {
      tt_assert(smartlist_contains_ipv4_qed_hs_addr(result)
                || smartlist_contains_ipv6_qed_hs_addr(result));
    }
  }
...
static int
fake_close_socket(qed_hs_socket_t s)
{
  (void)s;
  return 0;
}
...
static qed_hs_socket_t
pretend_to_connect(qed_hs_socket_t sock, const struct sockaddr *address,
...
static int
fake_getsockname(qed_hs_socket_t sock, struct sockaddr *address,
                 socklen_t *address_len)
{
...
{
  int hack_retval;
  qed_hs_addr_t *addr_from_hack = qed_hs_malloc_zero(sizeof(qed_hs_addr_t));
  struct sockaddr_in6 *mock_addr6;
  struct sockaddr_in6 *ipv6_to_check =
  qed_hs_malloc_zero(sizeof(struct sockaddr_in6));

  (void)arg;

  MOCK(qed_hs_open_socket,fake_open_socket);
  MOCK(qed_hs_connect_socket,pretend_to_connect);
  MOCK(qed_hs_getsockname,fake_getsockname);
  MOCK(qed_hs_close_socket,fake_close_socket);

  mock_addr = qed_hs_malloc_zero(sizeof(struct sockaddr_storage));
  ...
  tt_assert(qed_hs_addr_eq_ipv4h(addr_from_hack, 0x1720f676));
  ...
  qed_hs_inet_pton(AF_INET6,"2001:cdba::3257:9652",&(mock_addr6->sin6_addr));
  ...
  qed_hs_addr_to_sockaddr(addr_from_hack,0,(struct sockaddr *)ipv6_to_check,
                       sizeof(struct sockaddr_in6));

  tt_assert(sockaddr_in6_are_equal(mock_addr6,ipv6_to_check));

 done:
  UNMOCK(qed_hs_open_socket);
  UNMOCK(qed_hs_connect_socket);
  UNMOCK(qed_hs_getsockname);
  UNMOCK(qed_hs_close_socket);

  qed_hs_free(ipv6_to_check);
  qed_hs_free(mock_addr);
  qed_hs_free(addr_from_hack);
  return;
}
...
test_address_udp_socket_trick_blackbox(void *arg)
{
  ...
  qed_hs_addr_t addr4;
  qed_hs_addr_t addr4_to_check;
  qed_hs_addr_t addr6;
  qed_hs_addr_t addr6_to_check;
  ...
  tt_assert( (retval == -1 && retval_reference == -1) ||
             (qed_hs_addr_compare(&addr4,&addr4_to_check,CMP_EXACT) == 0) );
  ...
  tt_int_op(retval,OP_EQ,retval_reference);
  tt_assert( (retval == -1 && retval_reference == -1) ||
             (qed_hs_addr_compare(&addr6,&addr6_to_check,CMP_EXACT) == 0) );
...
test_address_get_if_addrs_list_internal(void *arg)
{
  ...
  tt_assert(!smartlist_contains_localhost_qed_hs_addr(results));
  tt_assert(!smartlist_contains_multicast_qed_hs_addr(results));
  ...
  tt_assert(!smartlist_contains_ipv6_qed_hs_addr(results));
...
test_address_get_if_addrs_list_no_internal(void *arg)
{
  ...
  tt_assert(!smartlist_contains_localhost_qed_hs_addr(results));
  tt_assert(!smartlist_contains_multicast_qed_hs_addr(results));
  tt_assert(!smartlist_contains_internal_qed_hs_addr(results));
  tt_assert(!smartlist_contains_null_qed_hs_addr(results));
...
test_address_get_if_addrs_internal_fail(void *arg)
{
  ...
  qed_hs_addr_t ipv6_addr;

  memset(&ipv6_addr, 0, sizeof(qed_hs_addr_t));
...
test_address_get_if_addrs(void *arg)
{
   ...
   * Unit tests shouldn't fail because of this. */
  if (rv == 0) {
    qed_hs_addr_from_ipv4h(&qed_hs_addr, addr_h);

    tt_assert(!qed_hs_addr_is_loopback(&qed_hs_addr));
    tt_assert(!qed_hs_addr_is_multicast(&qed_hs_addr));
    /* The address may or may not be an internal address */

    tt_assert(qed_hs_addr_is_v4(&qed_hs_addr));
  }
...
test_address_get_if_addrs6(void *arg)
{
  ...
  rv = get_interface_address6(LOG_WARN, AF_INET6, &qed_hs_addr);
...
static void
test_address_qed_hs_addr_to_in6(void *ignored)
{
  (void)ignored;
...
static void
test_address_qed_hs_addr_to_in(void *ignored)
{
  (void)ignored;
  qed_hs_addr_t *a = qed_hs_malloc_zero(sizeof(qed_hs_addr_t));
...
static void
test_address_qed_hs_addr_to_ipv4n(void *ignored)
{
  (void)ignored;
  qed_hs_addr_t *a = qed_hs_malloc_zero(sizeof(qed_hs_addr_t));
  uint32_t res;
...
static void
test_address_qed_hs_addr_to_mapped_ipv4h(void *ignored)
{
  (void)ignored;
  qed_hs_addr_t *a = qed_hs_malloc_zero(sizeof(qed_hs_addr_t));
  uint32_t res;
  ...
  a->family = AF_INET;
  res = qed_hs_addr_to_mapped_ipv4h(a);
  tt_assert(!res);
...
static void
test_address_qed_hs_addr_eq_ipv4h(void *ignored)
{
  (void)ignored;
  qed_hs_addr_t *a = qed_hs_malloc_zero(sizeof(qed_hs_addr_t));
  int res;
  ...
  a->addr.in_addr.s_addr = 52;
  res = qed_hs_addr_eq_ipv4h(a, ntohl(52));
  tt_assert(res);
...
static void
test_address_qed_hs_addr_in_same_network_family(void *ignored)
{
  (void)ignored;
  qed_hs_addr_t a, b;

  qed_hs_addr_parse(&a, "8.8.8.8");
  qed_hs_addr_parse(&b, "8.8.4.4");
  tt_int_op(router_addrs_in_same_network(&a, &b), OP_EQ, 1);

  qed_hs_addr_parse(&a, "8.8.8.8");
  qed_hs_addr_parse(&b, "1.1.1.1");
  tt_int_op(router_addrs_in_same_network(&a, &b), OP_EQ, 0);

  qed_hs_addr_parse(&a, "8.8.8.8");
  qed_hs_addr_parse(&b, "2001:4860:4860::8844");
  tt_int_op(router_addrs_in_same_network(&a, &b), OP_EQ, 0);

  qed_hs_addr_parse(&a, "2001:4860:4860::8888");
  qed_hs_addr_parse(&b, "2001:4860:4860::8844");
  tt_int_op(router_addrs_in_same_network(&a, &b), OP_EQ, 1);

  qed_hs_addr_parse(&a, "2001:4860:4860::8888");
  qed_hs_addr_parse(&b, "2001:470:20::2");
  tt_int_op(router_addrs_in_same_network(&a, &b), OP_EQ, 0);
...
{
  node_t *node = qed_hs_malloc_zero(sizeof(node_t));
  routerinfo_t *ri = qed_hs_malloc_zero(sizeof(routerinfo_t));
  qed_hs_addr_make_null(&ri->ipv6_addr, AF_INET6);
  node->ri = ri;
...
#define NODE_SET_IPV4(node, ipv4_addr_str, ipv4_port) { \
    qed_hs_addr_parse(&(node)->ri->ipv4_addr, ipv4_addr_str); \
    node->ri->ipv4_orport = ipv4_port; \
  }

#define NODE_CLEAR_IPV4(node) { \
    qed_hs_addr_make_unspec(&node->ri->ipv4_addr); \
    node->ri->ipv4_orport = 0; \
  }

#define NODE_SET_IPV6(node, ipv6_addr_str, ipv6_port) { \
    qed_hs_addr_parse(&node->ri->ipv6_addr, ipv6_addr_str); \
    node->ri->ipv6_orport = ipv6_port; \
  }

static void
test_address_qed_hs_node_in_same_network_family(void *ignored)
{
  (void)ignored;
...
#define TEST_ROUTER_VALID_ADDRESS_HELPER(ipv4_addr_str, ipv6_addr_str, rv) \
  STMT_BEGIN \
    ...
    qed_hs_addr_parse(&ri->ipv4_addr, (ipv4_addr_str));   \
    qed_hs_addr_parse(&ri->ipv6_addr, (ipv6_addr_str)); \
...
test_address_dirserv_router_addr_private(void *opt_dir_allow_private)
{
  ...
  {
    ri = qed_hs_malloc_zero(sizeof(routerinfo_t));
    qed_hs_addr_parse(&ri->ipv4_addr, "1.0.0.1");
...
#endif
  ADDRESS_TEST(qed_hs_addr_to_in6, 0),
  ADDRESS_TEST(qed_hs_addr_to_in, 0),
  ADDRESS_TEST(qed_hs_addr_to_ipv4n, 0),
  ADDRESS_TEST(qed_hs_addr_to_mapped_ipv4h, 0),
  ADDRESS_TEST(qed_hs_addr_eq_ipv4h, 0),
  ADDRESS_TEST(qed_hs_addr_in_same_network_family, 0),
  ADDRESS_TEST(qed_hs_node_in_same_network_family, 0),
