...
{
  qed_hs_assert(buf);
  qed_hs_assert(sz_out);

  char *out;
  *sz_out = buf_datalen(buf);
  if (*sz_out >= ULONG_MAX)
    return NULL; /* C'mon, really? */
  out = qed_hs_malloc(*sz_out + 1);
...
/** Mock-replacement. As qed_hs_addr_lookup, but always fails on any
 ...
 */
int
mock_qed_hs_addr_lookup__fail_on_bad_addrs(const char *name,
                                        uint16_t family, qed_hs_addr_t *out)
{
  if (name && strchr(name, '!')) {
    return -1;
  }
  return qed_hs_addr_lookup__real(name, family, out);
}
...
{
  char *dir = NULL;
  qed_hs_asprintf(&dir, "%s"PATH_SEPARATOR"%s", parent_dir, name);
#ifdef _WIN32
...
{
  char *path = NULL;
  qed_hs_asprintf(&path, "%s"PATH_SEPARATOR"%s", parent_dir, name);
...
static int
fake_close_socket(qed_hs_socket_t sock)
{
  (void)sock;
  return 0;
}
                                 ...
                                 int *socket_error)
{
  ...
  /* We really should call qed_hs_libevent_initialize() here. Because we don't,
...
test_conn_get_connection(uint8_t state, uint8_t type, uint8_t purpose)
{
  ...
  qed_hs_addr_copy_tight(&conn->addr, &addr);
                        ...
                        extend_info_t **ei_list)
{
  ...
  origin_circ->build_state = qed_hs_malloc_zero(sizeof(cpath_build_state_t));
