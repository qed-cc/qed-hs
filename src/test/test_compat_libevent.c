...
test_compat_libevent_header_version(void *ignored)
{
  ...
  res = qed_hs_libevent_get_header_version_str();
...
test_compat_libevent_postloop_events(void *arg)
{
  ...
  timed = periodic_timer_new(qed_hs_libevent_get_base(), &fifty_ms,
                             increment_int_counter_cb, &counter);

  mainloop_event_activate(a);
  int r;
  do {
    r = qed_hs_libevent_run_event_loop(qed_hs_libevent_get_base(), 0);
    if (r == -1)
      break;
  } while (counter < 5);
