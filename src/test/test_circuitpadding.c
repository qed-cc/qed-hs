...
nodes_init(void)
{
  padding_node.rs = qed_hs_malloc_zero(sizeof(routerstatus_t));
  padding_node.rs->pv.supports_hs_setup_padding = 1;

  non_padding_node.rs = qed_hs_malloc_zero(sizeof(routerstatus_t));
...
nodes_free(void)
{
  qed_hs_free(padding_node.rs);

  qed_hs_free(non_padding_node.rs);
}
...
test_circuitpadding_state_length(void *arg)
{
  ...
  qed_hs_free(client_machine->states);
  qed_hs_free(client_machine);
...
test_circuitpadding_hs_machines(void *arg)
{
  ...
  */
  ...
  monotime_init();
  monotime_enable_test_mocking();
  monotime_set_mock_time_nsec(1*QED_HS_NSEC_PER_USEC);
  monotime_coarse_set_mock_time_nsec(1*QED_HS_NSEC_PER_USEC);
  curr_mocked_time = 1*QED_HS_NSEC_PER_USEC;
