...
test_ext_or_init_auth(void *arg)
{
  ...
  options->DataDirectory = qed_hs_strdup("foo");
...
test_ext_or_cookie_auth(void *arg)
{
  ...
  qed_hs_free(reply);
  qed_hs_free(client_hash);
  qed_hs_free(reply2);
  qed_hs_free(client_hash2);
}
...
test_ext_or_handshake(void *arg)
{
  ...
  tt_int_op(qed_hs_addr_to_ipv4h(&TO_CONN(conn)->addr), OP_EQ, 0x01020304);
