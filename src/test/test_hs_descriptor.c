...
test_cert_encoding(void *arg)
{
  ...
  cert = qed_hs_cert_create_ed25519(&kp, CERT_TYPE_SIGNING_AUTH, &signed_key,
                         now, 3600 * 2, CERT_FLAG_INCLUDE_SIGNING_KEY);
  tt_assert(cert);

  /* Test the certificate encoding function. */
  ret = qed_hs_cert_encode_ed22519(cert, &encoded);
  ...
  /* Validated the certificate string. */
  {
    ...
    /* Parseable? */
    parsed_cert = qed_hs_cert_parse((uint8_t *) buf, ret);
    tt_assert(parsed_cert);
    /* Signature is valid? */
    ret = qed_hs_cert_checksig(parsed_cert, &kp.pubkey, now + 10);
    tt_int_op(ret, OP_EQ, 0);
    ret = qed_hs_cert_eq(cert, parsed_cert);
...
test_decode_descriptor(void *arg)
{
  ...
  /* Decode a descriptor with auth clients. */
  {
    ...
    client = qed_hs_malloc_zero(sizeof(hs_desc_authorized_client_t));
  ...
  {
    qed_hs_assert(!desc->encrypted_data.pow_params);
    ...
    qed_hs_asprintf(&extra_plaintext,
                "pow-params v1 %s %u %s\n",
    ...
    desc->encrypted_data.pow_params =
      qed_hs_malloc_zero(sizeof(hs_pow_desc_params_t));
    ...
    qed_hs_free(desc->encrypted_data.pow_params);
  }
...
#define template \
  ...
  {
    char *plaintext;
    qed_hs_asprintf(&plaintext, template, bad_value, "180", "42", "MESSAGE");
  ...
  /* Max length. */
  {
    ...
    char *plaintext = qed_hs_malloc_zero(big);
  ...
  {
    char *plaintext;
    qed_hs_asprintf(&plaintext, template, "3", bad_value, "42", "MESSAGE");
    ret = hs_desc_decode_plaintext(plaintext, &desc_plaintext);
    qed_hs_free(plaintext);
    tt_int_op(ret, OP_EQ, HS_DESC_DECODE_PLAINTEXT_ERROR);
  }
  ...
  {
    char *plaintext;
    qed_hs_asprintf(&plaintext, template, "3", "7181615", "42", "MESSAGE");
    ret = hs_desc_decode_plaintext(plaintext, &desc_plaintext);
    qed_hs_free(plaintext);
    tt_int_op(ret, OP_EQ, HS_DESC_DECODE_PLAINTEXT_ERROR);
  }
  ...
  {
    char *plaintext;
    qed_hs_asprintf(&plaintext, template, "3", "180", "42", bad_value);
    ret = hs_desc_decode_plaintext(plaintext, &desc_plaintext);
    qed_hs_free(plaintext);
    tt_int_op(ret, OP_EQ, HS_DESC_DECODE_PLAINTEXT_ERROR);
  }
  ...
  {
    char *plaintext;
    qed_hs_asprintf(&plaintext, template, "3", "180", bad_value, "MESSAGE");
    ret = hs_desc_decode_plaintext(plaintext, &desc_plaintext);
    qed_hs_free(plaintext);
    tt_int_op(ret, OP_EQ, HS_DESC_DECODE_PLAINTEXT_ERROR);
  }
...
test_validate_cert(void *arg)
{
  ...
  cert = qed_hs_cert_create_ed25519(&kp, CERT_TYPE_AUTH_HS_IP_KEY,
                                     &kp.pubkey, now, 3600,
...
test_desc_signature(void *arg)
{
  ...
  qed_hs_asprintf(&data, "This is a signed descriptor\n");
  ...
  qed_hs_asprintf(&desc, "%ssignature %s\n", data, sig_b64);
