...
test_pt_parsing(void *arg)
{
  ...
  qed_hs_addr_parse(&test_addr, "127.0.0.1");
  tt_assert(qed_hs_addr_eq(&test_addr, &transport->addr));
  ...
  transport = smartlist_get(mp->transports, 0);
  /* test registered address of transport */
  qed_hs_addr_parse(&test_addr, "127.0.0.2");
  tt_assert(qed_hs_addr_eq(&test_addr, &transport->addr));
  /* test registered port of transport */
...
{
  char line[200];
  char *test_binary = qed_hs_strdup("test-pt");
  char *argv[] = {
    test_binary,
    NULL,
  };
...
test_pt_get_transport_options(void *arg)
{
  ...
  execve_args = qed_hs_malloc(sizeof(char*)*2);
  execve_args[0] = qed_hs_strdup("cheeseshop");
  execve_args[1] = NULL;
  ...
  cl = qed_hs_malloc_zero(sizeof(config_line_t));
  cl->value = qed_hs_strdup("gruyere melty=10 hardness=se;ven");
  options->ServerTransportOptions = cl;

  cl = qed_hs_malloc_zero(sizeof(config_line_t));
  cl->value = qed_hs_strdup("stnectaire melty=4 hardness=three");
  cl->next = options->ServerTransportOptions;
  options->ServerTransportOptions = cl;

  cl = qed_hs_malloc_zero(sizeof(config_line_t));
  cl->value = qed_hs_strdup("pepperjack melty=12 hardness=five");
  cl->next = options->ServerTransportOptions;
  options->ServerTransportOptions = cl;
...
test_pt_protocol(void *arg)
{
  ...
  mp->conf_state = PT_PROTO_LAUNCHED;
  mp->transports = smartlist_new();
  mp->argv = qed_hs_calloc(2, sizeof(char *));
  mp->argv[0] = qed_hs_strdup("<testcase>");
...
test_pt_get_extrainfo_string(void *arg)
{
  ...
  argv1 = qed_hs_malloc_zero(sizeof(char*)*3);
  argv1[0] = qed_hs_strdup("ewige");
  argv1[1] = qed_hs_strdup("Blumenkraft");
  argv1[2] = NULL;
  argv2 = qed_hs_malloc_zero(sizeof(char*)*4);
  argv2[0] = qed_hs_strdup("und");
  argv2[1] = qed_hs_strdup("ewige");
  argv2[2] = qed_hs_strdup("Schlangenkraft");
  argv2[3] = NULL;
...
test_pt_configure_proxy(void *arg)
{
  ...
  mp = qed_hs_malloc_zero(sizeof(managed_proxy_t));
  mp->conf_state = PT_PROTO_ACCEPTING_METHODS;
  mp->transports = smartlist_new();
  mp->transports_to_launch = smartlist_new();
  mp->argv = qed_hs_malloc_zero(sizeof(char*)*2);
  mp->argv[0] = qed_hs_strdup("<testcase>");
  mp->is_server = 1;
  ...
  { /* check that the transport info were saved properly in the tor state */
    ...
    /* Get the bindaddr for "mock1" and check it against the bindaddr
       that the mocked qed_hs_get_lines_from_handle() generated. */
    ...
    SMARTLIST_FOREACH(transport_info_sl, char *, cp, qed_hs_free(cp));
  ...
  if (controlevent_msgs) {
    SMARTLIST_FOREACH(controlevent_msgs, char *, cp, qed_hs_free(cp));
...
test_get_pt_proxy_uri(void *arg)
{
  ...
  options->Socks4Proxy = qed_hs_strdup("192.0.2.1:1080");
  ret = qed_hs_addr_port_lookup(options->Socks4Proxy,
  ...
  qed_hs_free(uri);
  qed_hs_free(options->Socks4Proxy);

  /* Test with a SOCKS5 proxy, no username/password. */
  options->Socks5Proxy = qed_hs_strdup("192.0.2.1:1080");
  ret = qed_hs_addr_port_lookup(options->Socks5Proxy,
  ...
  options->Socks5ProxyUsername = qed_hs_strdup("hwest");
  options->Socks5ProxyPassword = qed_hs_strdup("r34n1m470r");
  uri = get_pt_proxy_uri();
  tt_str_op(uri, OP_EQ, "socks5://hwest:r34n1m470r@192.0.2.1:1080");
  qed_hs_free(uri);
  qed_hs_free(options->Socks5Proxy);
  qed_hs_free(options->Socks5ProxyUsername);
  qed_hs_free(options->Socks5ProxyPassword);

  /* Test with a HTTPS proxy, no authenticator. */
  options->HTTPSProxy = qed_hs_strdup("192.0.2.1:80");
  ret = qed_hs_addr_port_lookup(options->HTTPSProxy,
  ...
  options->HTTPSProxyAuthenticator = qed_hs_strdup("hwest:r34n1m470r");
  uri = get_pt_proxy_uri();
  tt_str_op(uri, OP_EQ, "http://hwest:r34n1m470r@192.0.2.1:80");
  qed_hs_free(uri);
  qed_hs_free(options->HTTPSProxy);
  qed_hs_free(options->HTTPSProxyAuthenticator);

  /* Token nod to the fact that IPv6 exists. */
  options->Socks4Proxy = qed_hs_strdup("[2001:db8::1]:1080");
  ret = qed_hs_addr_port_lookup(options->Socks4Proxy,
                             &options->Socks4ProxyAddr,
                             &options->Socks4ProxyPort);
  tt_int_op(ret, OP_EQ, 0);
