...
test_rephist_mtbf(void *arg)
{
  ...
  char *ddir_fname = qed_hs_strdup(get_fname_rnd("datadir_mtbf"));
  qed_hs_free(get_options_mutable()->DataDirectory);
  get_options_mutable()->DataDirectory = qed_hs_strdup(ddir_fname);
...
test_commit_max(void *arg)
{
  ...
  b->max_total = 10;
  commit_max(b);
  qed_hs_assert(b->total_in_period == 0);
  qed_hs_assert(b->max_total == 0);
...
#define test_fill_bw(b, buf, rv, str, checkrv) STMT_BEGIN \
    buf = qed_hs_malloc_zero(MAX_HIST_VALUE_LEN); \
...
test_get_bandwidth_lines(void *arg)
{
  ...
  checkstr = qed_hs_malloc_zero(len);
  ...
  qed_hs_snprintf(checkstr, len, "write-history %s (86400 s) "
...
test_load_stats_file(void *arg)
{
  ...
  tt_assert(fname);
  read_file_content = qed_hs_strdup(
