...
{
  qed_hs_assert(f == FLAV_MICRODESC);
  return dummy_ns;
}
...
#define N_NODES 5
  ...
  for (i = 0; i < N_NODES; ++i) {
    rs[i] = qed_hs_malloc_zero(sizeof(*rs[i]));
    md[i] = qed_hs_malloc_zero(sizeof(*md[i]));
    ri[i] = qed_hs_malloc_zero(sizeof(*ri[i]));

    crypto_rand(md[i]->digest, sizeof(md[i]->digest));
    md[i]->ed25519_identity_pkey = qed_hs_malloc(sizeof(ed25519_public_key_t));
  ...
  for (i = 0; i < N_NODES; ++i) {
    qed_hs_free(rs[i]);
    qed_hs_free(md[i]->ed25519_identity_pkey);
    qed_hs_free(md[i]);
    qed_hs_free(ri[i]->cache_info.signing_key_cert);
    qed_hs_free(ri[i]);
  }
...
test_nodelist_nodefamily(void *arg)
{
  ...
  qed_hs_asprintf(&enc, "$%s hello", h1);
  nf1 = nodefamily_parse(enc, NULL, 0);
  tt_assert(nf1);
  qed_hs_free(enc);

  qed_hs_asprintf(&enc, "hello hello $%s hello", h1);
  nf2 = nodefamily_parse(enc, NULL, 0);
  tt_assert(nf2);
  tt_ptr_op(nf1, OP_EQ, nf2);
  qed_hs_free(enc);

  qed_hs_asprintf(&enc, "%s $%s hello", h1, h1);
  ...
  nodefamily_free(nf3);
  qed_hs_asprintf(&enc, "hello $%s hello", h3);
  nf3 = nodefamily_parse(enc, NULL, 0);
  tt_assert(nf3);
  ...
  /* Try one with several digests, all with nicknames appended, in different
     formats. */
  qed_hs_asprintf(&enc, "%s $%s $%s=res $%s~ist", h1, h2, h3, h4);
  ...
  qed_hs_free(enc);
  qed_hs_asprintf(&enc, "$%s $%s $%s $%s $%s", h4, h3, h1, h5, h2);
  enc2 = nodefamily_format(nf1);
...
test_nodelist_nodefamily_lookup(void *arg)
{
  ...
  SMARTLIST_FOREACH(sl, node_t *, fake_node, qed_hs_free(fake_node));
  smartlist_free(sl);
  nodefamily_free(nf1);
...
test_nodelist_node_nodefamily(void *arg)
{
  ...
  mock_ri.nickname = qed_hs_strdup("nodetwo");
  ...
  SMARTLIST_FOREACH(nodes, node_t *, x, qed_hs_free(x));
  ...
  nodefamily_free(mock_md.family);
  qed_hs_free(mock_ri.nickname);
  // use qed_hs_free, these aren't real nodes
  SMARTLIST_FOREACH(nodes, node_t *, x, qed_hs_free(x));
...
test_nodelist_format_node_description(void *arg)
{
  ...
  qed_hs_addr_t mock_null_ip;
  qed_hs_addr_t mock_ipv4;
  qed_hs_addr_t mock_ipv6;
  ...
  qed_hs_addr_parse(&mock_ipv4, "111.222.233.244");
  qed_hs_addr_parse(&mock_ipv6, "[1111:2222:3333:4444:5555:6666:7777:8888]");
...
test_nodelist_router_describe(void *arg)
{
  ...
  qed_hs_addr_parse(&mock_ri_dual.ipv4_addr, "111.222.233.244");
  qed_hs_addr_parse(&mock_ri_dual.ipv6_addr,
                 "[1111:2222:3333:4444:5555:6666:7777:8888]");
  ...
  qed_hs_addr_make_unspec(&mock_ri_ipv6.ipv4_addr);
...
test_nodelist_node_describe(void *arg)
{
  ...
  qed_hs_addr_parse(&mock_rs_dual.ipv4_addr, "2.2.2.2");
  qed_hs_addr_parse(&mock_rs_dual.ipv6_addr,
                 "[bbbb::bbbb]");
  ...
  qed_hs_addr_parse(&mock_md_ipv6.ipv6_addr,
                 "[eeee::6000:6000]");
...
test_nodelist_routerstatus_describe(void *arg)
{
  ...
  strlcpy(mock_rs_dual.nickname, "TestOR7890123456789",
          sizeof(mock_rs_dual.nickname));
  qed_hs_addr_parse(&mock_rs_dual.ipv4_addr, "111.222.233.244");
  qed_hs_addr_parse(&mock_rs_dual.ipv6_addr,
                 "[1111:2222:3333:4444:5555:6666:7777:8888]");
  ...
  /* Clear the unnecessary addresses */
  memset(&mock_rs_ipv4.ipv6_addr, 0, sizeof(mock_rs_ipv4.ipv6_addr));
  qed_hs_addr_make_unspec(&mock_rs_ipv6.ipv4_addr);
...
test_nodelist_extend_info_describe(void *arg)
{
  ...
  qed_hs_addr_parse(&mock_ei_ipv4.orports[0].addr, "111.222.233.244");

  /* Create and modify the other extend info. */
  memcpy(&mock_ei_ipv6, &mock_ei_ipv4, sizeof(mock_ei_ipv6));
  qed_hs_addr_parse(&mock_ei_ipv6.orports[0].addr,
                 "[1111:2222:3333:4444:5555:6666:7777:8888]");
...
#define ASSERT_CHANGED_NO_FORMAT() \
  ...
  qed_hs_addr_parse(&rs.ipv6_addr, "1234::56");
  ASSERT_CHANGED();

  qed_hs_addr_parse(&rs_orig.ipv6_addr, "1234::56");
