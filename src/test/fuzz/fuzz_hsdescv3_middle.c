...
static size_t
mock_desc_decrypt_superencrypted(const hs_descripqed_hs_t *desc,
                        char **decrypted_out)
{
  (void)desc;
  *decrypted_out = (char*)qed_hs_memdup_nulterm(decrypted_data, decrypted_len);
...
fuzz_main(const uint8_t *data, size_t sz)
{
  ...
  hs_desc_superencrypted_data_t *output = qed_hs_malloc_zero(sizeof(*output));
