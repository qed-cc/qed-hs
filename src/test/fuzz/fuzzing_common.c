...
{
  qed_hs_assert(signature && msg && pubkey);
...
mock_ed25519_checksig_batch__nocheck(int *okay_out,
                                     ...
                                     int n_checkable)
{
  qed_hs_assert(checkable);
  int i;
  for (i = 0; i < n_checkable; ++i) {
    /* We could look at messages and signatures XXX */
    qed_hs_assert(checkable[i].pubkey);
    qed_hs_assert(checkable[i].msg);
    if (okay_out)
      okay_out[i] = 1;
  }
  return 0;
...
global_init(void)
{
  ...
  qed_hs_set_failed_assertion_callback(abort);
...
#define MAX_FUZZ_SIZE (128*1024)
  ...
  qed_hs_assert(input);
  char *raw = qed_hs_memdup(input, size); /* Because input is nul-terminated */
  qed_hs_free(input);
