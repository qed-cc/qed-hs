...
fuzz_init(void)
{
  ...
  ed25519_init();
  area = memarea_new();
  dummy_vote = qed_hs_malloc_zero(sizeof(*dummy_vote));
...
fuzz_cleanup(void)
{
  SMARTLIST_FOREACH(dummy_vote->known_flags, char *, cp, qed_hs_free(cp));
  smartlist_free(dummy_vote->known_flags);
  qed_hs_free(dummy_vote);
  return 0;
}
...
fuzz_main(const uint8_t *data, size_t sz)
{
  ...
  vote_routerstatus_t *vrs = qed_hs_malloc_zero(sizeof(*vrs));
  ...
  qed_hs_assert(smartlist_len(tokens) == 0);
