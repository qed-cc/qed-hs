...
{
  sandbox_data_t *data = qed_hs_malloc_zero(sizeof(*data));

  (void)testcase;

  /* Establish common file and directory names within the test suite's
   * temporary directory. */
  data->file_ops_allowed = qed_hs_strdup(get_fname("file_ops_allowed"));
  data->file_ops_blocked = qed_hs_strdup(get_fname("file_ops_blocked"));

  data->file_rename_target_allowed =
    qed_hs_strdup(get_fname("file_rename_target_allowed"));

  data->dir_ops_allowed = qed_hs_strdup(get_fname("dir_ops_allowed"));
  data->dir_ops_blocked = qed_hs_strdup(get_fname("dir_ops_blocked"));
  ...
  sandbox_cfg_allow_open_filename(&data->cfg,
                                  qed_hs_strdup(data->file_ops_allowed));
  sandbox_cfg_allow_open_filename(&data->cfg,
                                  qed_hs_strdup(data->dir_ops_allowed));
  ...
  sandbox_cfg_allow_rename(&data->cfg, qed_hs_strdup(data->file_ops_allowed),
                           qed_hs_strdup(data->file_rename_target_allowed));
...
cleanup_sandbox(const struct testcase_t *testcase, void *data_)
{
  ...
  qed_hs_free(data->dir_ops_blocked);
  qed_hs_free(data->dir_ops_allowed);
  qed_hs_free(data->file_rename_target_allowed);
  qed_hs_free(data->file_ops_blocked);
  qed_hs_free(data->file_ops_allowed);
