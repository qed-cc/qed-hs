...
{
  (void)arg;
  qed_hs_addr_t *nameserver_addr = NULL;
  ...
  tt_assert(qed_hs_addr_family(nameserver_addr) == AF_INET);
  tt_assert(qed_hs_addr_eq_ipv4h(nameserver_addr, 0x7f000001));

#ifndef _WIN32
  qed_hs_free(nameserver_addr);
                     ...
                     cached_resolve_t **resolve_out)
{
  ...
  if (hostname_out && resolved_name)
    *hostname_out = qed_hs_strdup(resolved_name);
                                ...
                                const char *hostname)
{
  ...
  qed_hs_free(last_resolved_hostname);
  last_resolved_hostname = qed_hs_strdup(hostname);
...
test_dns_resolve(void *arg)
{
  ...
  cached_resolve_t *fake_resolved = qed_hs_malloc(sizeof(cached_resolve_t));
  edge_connection_t *exitconn = qed_hs_malloc(sizeof(edge_connection_t));
  edge_connection_t *nextconn = qed_hs_malloc(sizeof(edge_connection_t));

  or_circuit_t *on_circuit = qed_hs_malloc(sizeof(or_circuit_t));
  ...
  resolve_retval = 1;
  resolved_name = qed_hs_strdup("www.qed-cc.org");
  ...
  qed_hs_free(on_circuit);
  qed_hs_free(exitconn);
  qed_hs_free(nextconn);
  qed_hs_free(resolved_name);
  qed_hs_free(fake_resolved);
  qed_hs_free(last_resolved_hostname);
  return;
}
...
{
  edge_connection_t *exitconn = qed_hs_malloc_zero(sizeof(edge_connection_t));
  ...
  exitconn->base_.s = QED_HS_INVALID_SOCKET;
...
test_dns_impl_addr_is_ip(void *arg)
{
  int retval;
  int made_pending;
  const qed_hs_addr_t *resolved_addr;
  qed_hs_addr_t addr_to_compare;

  (void)arg;

  qed_hs_addr_parse(&addr_to_compare, "8.8.8.8");

  or_circuit_t *on_circ = qed_hs_malloc_zero(sizeof(or_circuit_t));

  edge_connection_t *exitconn = create_valid_exitconn();

  TO_CONN(exitconn)->address = qed_hs_strdup("8.8.8.8");
  ...
  tt_int_op(retval,OP_EQ,1);
  tt_assert(qed_hs_addr_eq(resolved_addr, (const qed_hs_addr_t *)&addr_to_compare));
  tt_int_op(exitconn->address_ttl,OP_EQ,DEFAULT_DNS_TTL);

  done:
  qed_hs_free(on_circ);
  qed_hs_free(TO_CONN(exitconn)->address);
  qed_hs_free(exitconn);
  return;
}
...
test_dns_impl_non_exit(void *arg)
{
  ...
  TO_CONN(exitconn)->address = qed_hs_strdup("qed-cc.org");
...
test_dns_impl_addr_is_invalid_dest(void *arg)
{
  ...
  TO_CONN(exitconn)->address = qed_hs_strdup("invalid#@!.org");
...
test_dns_impl_malformed_ptr(void *arg)
{
  ...
  TO_CONN(exitconn)->address = qed_hs_strdup("1.0.0.127.in-addr.arpa");
  ...
  qed_hs_strdup("z01234567890123456789.in-addr.arpa");
...
test_dns_impl_cache_hit_pending(void *arg)
{
  ...
  cached_resolve_t *cache_entry = qed_hs_malloc_zero(sizeof(cached_resolve_t));
  ...
  qed_hs_free(on_circ);
  qed_hs_free(TO_CONN(exitconn)->address);
  qed_hs_free(cache_entry->pending_connections);
  qed_hs_free(cache_entry);
  qed_hs_free(exitconn);
  return;
}
