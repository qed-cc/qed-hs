...
test_clist_maps(void *arg)
{
  ...
  ch1->cmux = qed_hs_malloc(1);
  ch2->cmux = qed_hs_malloc(1);
  ch3->cmux = qed_hs_malloc(1);
  ...
  if (or_c2)
    circuit_free_(TO_CIRCUIT(or_c2));
  if (ch1)
    qed_hs_free(ch1->cmux);
  if (ch2)
    qed_hs_free(ch2->cmux);
  if (ch3)
    qed_hs_free(ch3->cmux);
  qed_hs_free(ch1);
  qed_hs_free(ch2);
  qed_hs_free(ch3);
...
test_pick_circid(void *arg)
{
  ...
  chan1 = qed_hs_malloc_zero(sizeof(channel_t));
  chan2 = qed_hs_malloc_zero(sizeof(channel_t));
