...
test_relay_close_circuit(void *arg)
{
  ...
  /* Make a cell */
  cell = qed_hs_malloc_zero(sizeof(cell_t));
  make_fake_cell(cell);
...
test_suggested_address(void *arg)
{
  ...
  qed_hs_addr_t ipv4_addr, ipv6_addr, cache_addr;
  qed_hs_addr_t trusted_addr, untrusted_addr;
  qed_hs_addr_port_t trusted_ap_v6 = { .port = 443 };
  ...
  ret = qed_hs_addr_parse(&untrusted_addr, "8.8.8.8");
  tt_int_op(ret, OP_EQ, AF_INET);

  /* Add gabelmoo as a trusted directory authority. */
  ret = qed_hs_addr_parse(&trusted_addr, "[2001:638:a000:4140::ffff:189]");
  tt_int_op(ret, OP_EQ, AF_INET6);
  qed_hs_addr_copy(&trusted_ap_v6.addr, &trusted_addr);
  ...
  resolved_addr_get_suggested(AF_INET, &cache_addr);
  tt_assert(qed_hs_addr_is_unspec(&cache_addr));
  ...
  ret = qed_hs_addr_parse(&ipv4_addr, "127.0.0.1");
  relay_address_new_suggestion(&ipv4_addr, &ds->ipv4_addr, ds->digest);
  resolved_addr_get_suggested(AF_INET, &cache_addr);
  tt_assert(qed_hs_addr_is_unspec(&cache_addr));

  /* 6. Internal IPv6 from a trusted authority (gabelmoo). */
  ret = qed_hs_addr_parse(&ipv6_addr, "[::1]");
  relay_address_new_suggestion(&ipv6_addr, &ds->ipv6_addr, ds->digest);
  resolved_addr_get_suggested(AF_INET6, &cache_addr);
  tt_assert(qed_hs_addr_is_unspec(&cache_addr));
...
test_find_addr_to_publish(void *arg)
{
  ...
  family = qed_hs_addr_parse(&ipv4_addr, "1.2.3.4");
  tt_int_op(family, OP_EQ, AF_INET);
  resolved_addr_set_last(&ipv4_addr, RESOLVED_ADDR_CONFIGURED, NULL);
  resolved_addr_get_last(AF_INET, &cache_addr);
  tt_assert(qed_hs_addr_eq(&ipv4_addr, &cache_addr));

  family = qed_hs_addr_parse(&ipv6_addr, "[4242::4242]");
  ...
  tt_assert(qed_hs_addr_eq(&ipv6_addr, &cache_addr));
