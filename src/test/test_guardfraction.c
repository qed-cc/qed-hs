...
test_parse_guardfraction_file_bad(void *arg)
{
  ...
  qed_hs_asprintf(&guardfraction_bad,
...
test_parse_guardfraction_file_good(void *arg)
{
  ...
  qed_hs_asprintf(&guardfraction_good,
               "guardfraction-file-version 1\n"
               "written-at %s\n"
               "n-inputs 420 3\n"
               "guard-seen %s %d 420\n",
  ...
  qed_hs_free(guardfraction_good);
}
...
test_should_apply_guardfraction(void *arg)
{
  ...
  SMARTLIST_FOREACH(vote_enabled.net_params, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(vote_disabled.net_params, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(vote_missing.net_params, char *, cp, qed_hs_free(cp));
