...
{
  SSL_METHOD *method = qed_hs_malloc_zero(sizeof(SSL_METHOD));
...
static int
mock_tls_cert_matches_key(const qed_hs_tls_t *tls, const qed_hs_x509_cert_t *cert)
{
  (void) tls;
...
static void
test_tortls_qed_hs_tls_new(void *data)
{
  (void) data;
  MOCK(qed_hs_tls_cert_matches_key, mock_tls_cert_matches_key);
  ...
  qed_hs_tls_t *tls = NULL;
  tt_int_op(qed_hs_tls_context_init(QED_HS_TLS_CTX_IS_PUBLIC_SERVER,
                                 key1, key2, 86400), OP_EQ, 0);
  tls = qed_hs_tls_new(-1, 0);
  tt_want(tls);
  qed_hs_tls_free(tls); tls = NULL;
  ...
  UNMOCK(qed_hs_tls_cert_matches_key);
  crypto_pk_free(key1);
  crypto_pk_free(key2);
  qed_hs_tls_free(tls);
  qed_hs_free(method);
...
test_tortls_get_state_description(void *ignored)
{
  ...
  buf = qed_hs_malloc_zero(1000);
  tls = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));

  qed_hs_tls_get_state_description(NULL, buf, 20);
  tt_str_op(buf, OP_EQ, "(No SSL object)");

  SSL_free(tls->ssl);
  tls->ssl = NULL;
  qed_hs_tls_get_state_description(tls, buf, 20);
  tt_str_op(buf, OP_EQ, "(No SSL object)");

  tls->ssl = SSL_new(ctx);
  qed_hs_tls_get_state_description(tls, buf, 200);
  tt_str_op(buf, OP_EQ, SSL_STATE_STR " in HANDSHAKE");

  tls->state = QED_HS_TLS_ST_OPEN;
  qed_hs_tls_get_state_description(tls, buf, 200);
  tt_str_op(buf, OP_EQ, SSL_STATE_STR " in OPEN");

  tls->state = QED_HS_TLS_ST_GOTCLOSE;
  qed_hs_tls_get_state_description(tls, buf, 200);
  tt_str_op(buf, OP_EQ, SSL_STATE_STR " in GOTCLOSE");

  tls->state = QED_HS_TLS_ST_SENTCLOSE;
  qed_hs_tls_get_state_description(tls, buf, 200);
  tt_str_op(buf, OP_EQ, SSL_STATE_STR " in SENTCLOSE");

  tls->state = QED_HS_TLS_ST_CLOSED;
  qed_hs_tls_get_state_description(tls, buf, 200);
  tt_str_op(buf, OP_EQ, SSL_STATE_STR " in CLOSED");

  tls->state = QED_HS_TLS_ST_RENEGOTIATE;
  qed_hs_tls_get_state_description(tls, buf, 200);
  tt_str_op(buf, OP_EQ, SSL_STATE_STR " in RENEGOTIATE");

  tls->state = QED_HS_TLS_ST_BUFFEREVENT;
  qed_hs_tls_get_state_description(tls, buf, 200);
...
test_tortls_get_by_ssl(void *ignored)
{
  ...
  library_init();
  qed_hs_tls_allocate_qed_hs_tls_object_ex_data_index();

  ctx = SSL_CTX_new(SSLv23_method());
  tls = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));
  tls->magic = QED_HS_TLS_MAGIC;

  ssl = SSL_new(ctx);

  res = qed_hs_tls_get_by_ssl(ssl);
  tt_assert(!res);

  SSL_set_ex_data(ssl, qed_hs_tls_object_ex_data_index, tls);
...
static void
test_tortls_allocate_qed_hs_tls_object_ex_data_index(void *ignored)
{
  (void)ignored;
  int first;
  ...
  first = qed_hs_tls_object_ex_data_index;
  qed_hs_tls_allocate_qed_hs_tls_object_ex_data_index();
  tt_int_op(first, OP_EQ, qed_hs_tls_object_ex_data_index);
...
test_tortls_log_one_error(void *ignored)
{
  ...
  qed_hs_tls_log_one_error(NULL, 0, LOG_WARN, 0, "something");
  expect_log_msg("TLS error while something: "
            "(null) (in (null):(null):---)\n");

  mock_clean_saved_logs();
  qed_hs_tls_log_one_error(tls, 0, LOG_WARN, 0, NULL);
  expect_log_msg("TLS error: (null) "
            "(in (null):(null):---)\n");

  mock_clean_saved_logs();
  tls->address = qed_hs_strdup("127.hello");
  qed_hs_tls_log_one_error(tls, 0, LOG_WARN, 0, NULL);
  ...
  mock_clean_saved_logs();
  tls->address = qed_hs_strdup("127.hello");
  qed_hs_tls_log_one_error(tls, 0, LOG_WARN, 0, "blarg");
  expect_log_msg("TLS error while blarg with "
            "127.hello: (null) (in (null):(null):---)\n");

  mock_clean_saved_logs();
  qed_hs_tls_log_one_error(tls, ERR_PACK(1, 2, 3), LOG_WARN, 0, NULL);
  expect_log_msg_containing("TLS error with 127.hello");

  mock_clean_saved_logs();
  qed_hs_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_HTTP_REQUEST),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);

  mock_clean_saved_logs();
  qed_hs_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_HTTPS_PROXY_REQUEST),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);

  mock_clean_saved_logs();
  qed_hs_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_RECORD_LENGTH_MISMATCH),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);

  mock_clean_saved_logs();
  qed_hs_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_UNKNOWN_PROTOCOL),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);

  mock_clean_saved_logs();
  qed_hs_tls_log_one_error(tls, ERR_PACK(1, 2, SSL_R_UNSUPPORTED_PROTOCOL),
                        LOG_WARN, 0, NULL);
  expect_log_severity(LOG_INFO);
...
test_tortls_get_error(void *ignored)
{
  ...
  ret = qed_hs_tls_get_error(tls, 0, 0, "something", LOG_WARN, 0);
  tt_int_op(ret, OP_EQ, QED_HS_TLS_ERROR_IO);
  expect_log_msg("TLS error: unexpected close while"
            " something (before/accept initialization)\n");

  mock_clean_saved_logs();
  ret = qed_hs_tls_get_error(tls, 2, 0, "something", LOG_WARN, 0);
  tt_int_op(ret, OP_EQ, 0);
  expect_no_log_entry();

  mock_clean_saved_logs();
  ret = qed_hs_tls_get_error(tls, 0, 1, "something", LOG_WARN, 0);
  ...
  ret = qed_hs_tls_get_error(tls, 0, 0, "something", LOG_WARN, 0);
  tt_int_op(ret, OP_EQ, QED_HS_TLS_ERROR_MISC);
  ...
  ret = qed_hs_tls_get_error(tls, -1, 0, "something", LOG_WARN, 0);
  tt_int_op(ret, OP_EQ, QED_HS_TLS_WANTREAD);
  expect_no_log_entry();
  ...
  ret = qed_hs_tls_get_error(tls, -1, 0, "something", LOG_WARN, 0);
  tt_int_op(ret, OP_EQ, QED_HS_TLS_WANTWRITE);
  expect_no_log_entry();
  ...
  ret = qed_hs_tls_get_error(tls, 0, 0, "something", LOG_WARN, 0);
  tt_int_op(ret, OP_EQ, QED_HS_TLS_CLOSE);
  expect_log_entry();

  mock_clean_saved_logs();
  ret = qed_hs_tls_get_error(tls, 0, 2, "something", LOG_WARN, 0);
...
{
  (void)ignored;
  qed_hs_x509_cert_t *cert;

  cert = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  qed_hs_x509_cert_free(cert);

  cert = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  cert->cert = X509_new();
  cert->encoded = qed_hs_malloc_zero(1);
  qed_hs_x509_cert_free(cert);
}
#endif /* !defined(OPENSSL_OPAQUE) */
...
{
  return qed_hs_malloc_zero(sizeof(X509));
}
    ...
    if (cert->cert_info) {
      if (cert->cert_info->key) {
        if (cert->cert_info->key->pkey) {
          qed_hs_free(cert->cert_info->key->pkey);
        }
        qed_hs_free(cert->cert_info->key);
      }
      qed_hs_free(cert->cert_info);
    }
    qed_hs_free(cert);
  }
}
...
test_tortls_cert_get_key(void *ignored)
{
  ...
  res = qed_hs_tls_cert_get_key(cert);
  tt_assert(!res);

  cert->cert = key;
  key->cert_info = qed_hs_malloc_zero(sizeof(X509_CINF));
  key->cert_info->key = qed_hs_malloc_zero(sizeof(X509_PUBKEY));
  key->cert_info->key->pkey = qed_hs_malloc_zero(sizeof(EVP_PKEY));
...
test_tortls_get_pending_bytes(void *ignored)
{
  ...
  tls = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));
  tls->ssl = qed_hs_malloc_zero(sizeof(SSL));
  method = qed_hs_malloc_zero(sizeof(SSL_METHOD));
  method->ssl_pending = fixed_ssl_pending;
  tls->ssl->method = method;

  fixed_ssl_pending_result = 42;
  ret = qed_hs_tls_get_pending_bytes(tls);
...
test_tortls_get_buffer_sizes(void *ignored)
{
  ...
  tls = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));
  tls->ssl = qed_hs_malloc_zero(sizeof(SSL));
  tls->ssl->s3 = qed_hs_malloc_zero(sizeof(SSL3_STATE));
  ...
  ret = qed_hs_tls_get_buffer_sizes(tls, &rbuf_c, &rbuf_b, &wbuf_c, &wbuf_b);
  tt_int_op(ret, OP_EQ, -1);

 done:
  qed_hs_free(tls->ssl->s3->rbuf.buf);
  qed_hs_free(tls->ssl->s3->wbuf.buf);
  qed_hs_free(tls->ssl->s3);
  qed_hs_free(tls->ssl);
  qed_hs_free(tls);
}
#endif /* !defined(OPENSSL_OPAQUE) */
...
test_tortls_get_peer_cert(void *ignored)
{
  ...
  tls = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));
  tls->ssl = qed_hs_malloc_zero(sizeof(SSL));
  tls->ssl->session = qed_hs_malloc_zero(sizeof(SSL_SESSION));

  ret = qed_hs_tls_get_peer_cert(tls);
  tt_assert(!ret);
  ...
  qed_hs_x509_cert_free(ret);
  qed_hs_free(tls->ssl->session);
  qed_hs_free(tls->ssl);
  qed_hs_free(tls);
  X509_free(cert);
}
...
test_tortls_peer_has_cert(void *ignored)
{
  ...
  ret = qed_hs_tls_peer_has_cert(tls);
  tt_assert(!ret);
...
test_tortls_session_secret_cb(void *ignored)
{
  ...
  tls->ssl = SSL_new(ctx);
  SSL_set_ex_data(tls->ssl, qed_hs_tls_object_ex_data_index, tls);

  SSL_set_session_secret_cb(tls->ssl, qed_hs_tls_session_secret_cb, NULL);

  qed_hs_tls_session_secret_cb(tls->ssl, NULL, NULL, NULL, NULL, NULL);
  ...
  qed_hs_tls_session_secret_cb(tls->ssl, NULL, NULL, ciphers, NULL, NULL);
  tt_assert(!tls->ssl->tls_session_secret_cb);
...
#define SSL3_FLAGS_ALLOW_UNSAFE_LEGACY_RENEGOTIATION 0
  ...
  qed_hs_tls_block_renegotiation(tls);
...
test_tortls_unblock_renegotiation(void *ignored)
{
  ...
  tls = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));
  tls->ssl = qed_hs_malloc_zero(sizeof(SSL));
  qed_hs_tls_unblock_renegotiation(tls);
...
test_tortls_set_logged_address(void *ignored)
{
  ...
  qed_hs_tls_set_logged_address(tls, "foo bar");

  tt_str_op(tls->address, OP_EQ, "foo bar");

  qed_hs_tls_set_logged_address(tls, "foo bar 2");
...
#ifndef OPENSSL_OPAQUE
static void
example_cb(qed_hs_tls_t *t, void *arg)
{
  (void)t;
  (void)arg;
}
...
test_tortls_set_renegotiate_callback(void *ignored)
{
  ...
  qed_hs_tls_set_renegotiate_callback(tls, example_cb, (void*)arg);
  ...
  tt_assert(tls->ssl->info_callback == qed_hs_tls_server_info_callback);

  qed_hs_tls_set_renegotiate_callback(tls, NULL, (void*)arg);
  tt_assert(tls->ssl->info_callback == qed_hs_tls_debug_state_callback);
...
test_tortls_debug_state_callback(void *ignored)
{
  (void)ignored;
  SSL *ssl;
  char *buf = qed_hs_malloc_zero(1000);
  int n;

  setup_capture_of_logs(LOG_DEBUG);

  ssl = qed_hs_malloc_zero(sizeof(SSL));

  qed_hs_tls_debug_state_callback(ssl, 32, 45);

  n = qed_hs_snprintf(buf, 1000, "SSL %p is now in state unknown"
...
test_tortls_server_info_callback(void *ignored)
{
  ...
  mock_clean_saved_logs();
  qed_hs_tls_server_info_callback(ssl, SSL_CB_ACCEPT_LOOP, 0);
...
static void
negotiated_callback_setter(qed_hs_tls_t *t, void *arg)
{
  (void)t;
  (void)arg;
...
test_tortls_read(void *ignored)
{
  ...
  ret = qed_hs_tls_read(tls, buf, 10);
  tt_int_op(ret, OP_EQ, -9);
  ...
  ret = qed_hs_tls_read(tls, buf, 10);
  tt_int_op(ret, OP_EQ, QED_HS_TLS_CLOSE);
  tt_int_op(tls->state, OP_EQ, QED_HS_TLS_ST_CLOSED);
...
test_tortls_write(void *ignored)
{
  ...
  ret = qed_hs_tls_write(tls, buf, 0);
  tt_int_op(ret, OP_EQ, 0);

  ret = qed_hs_tls_write(tls, buf, 10);
  tt_int_op(ret, OP_EQ, -9);
...
test_tortls_handshake(void *ignored)
{
  ...
  tls = qed_hs_malloc_zero(sizeof(qed_hs_tls_t));
  tls->ssl = SSL_new(ctx);
  tls->state = QED_HS_TLS_ST_HANDSHAKE;

  ret = qed_hs_tls_handshake(tls);
  tt_int_op(ret, OP_EQ, -9);
  ...
  tls->state = QED_HS_TLS_ST_HANDSHAKE;
  ret = qed_hs_tls_handshake(tls);
  tt_int_op(tls->state, OP_EQ, QED_HS_TLS_ST_OPEN);
...
test_tortls_finish_handshake(void *ignored)
{
  ...
  ret = qed_hs_tls_finish_handshake(tls);
  tt_int_op(ret, OP_EQ, 0);
  ...
  sess = qed_hs_malloc_zero(sizeof(SESS_CERT_local));
  ...
  if (tls->ssl && tls->ssl->session) {
    qed_hs_free(tls->ssl->session->sess_cert);
  }
  SSL_free(tls->ssl);
  qed_hs_free(tls);
  SSL_CTX_free(ctx);
...
static int fixed_qed_hs_tls_create_certificate_result_index;
static X509 *fixed_qed_hs_tls_create_certificate_result[5];
static int fixed_qed_hs_x509_cert_new_result_index;
static qed_hs_x509_cert_t *fixed_qed_hs_x509_cert_new_result[5];
...
static X509 *
fixed_qed_hs_tls_create_certificate(crypto_pk_t *rsa,
                                 ...
                                 unsigned int cert_lifetime)
{
  ...
  (void)cname_sign;
  (void)cert_lifetime;
  X509 *result = fixed_qed_hs_tls_create_certificate_result[
                             fixed_qed_hs_tls_create_certificate_result_index++];
  if (result)
...
static void
fixed_qed_hs_tls_create_certificate_results_free(void)
{
  unsigned i;
  for (i = 0; i < ARRAY_LENGTH(fixed_qed_hs_tls_create_certificate_result); ++i) {
    X509 *cert = fixed_qed_hs_tls_create_certificate_result[i];
    if (cert)
      X509_free(cert);
    fixed_qed_hs_tls_create_certificate_result[i] = NULL;
  }
}

static void
fixed_qed_hs_x509_cert_new_results_free(void)
{
  unsigned i;
  for (i = 0; i < ARRAY_LENGTH(fixed_qed_hs_x509_cert_new_result); ++i) {
    qed_hs_x509_cert_free(fixed_qed_hs_x509_cert_new_result[i]);
  }
}

static qed_hs_x509_cert_t *
fixed_qed_hs_x509_cert_new(qed_hs_x509_cert_impl_t *x509_cert)
{
  (void) x509_cert;
  qed_hs_x509_cert_t **certp =
    &fixed_qed_hs_x509_cert_new_result[fixed_qed_hs_x509_cert_new_result_index++];
  qed_hs_x509_cert_t *cert = *certp;
  *certp = NULL;
  return cert;
}
...
test_tortls_context_new(void *ignored)
{
  ...
  ret = qed_hs_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  ...
  MOCK(qed_hs_tls_create_certificate, fixed_qed_hs_tls_create_certificate);
  ...
  fixed_qed_hs_tls_create_certificate_result_index = 0;
  fixed_qed_hs_tls_create_certificate_result[0] = NULL;
  fixed_qed_hs_tls_create_certificate_result[1] = X509_new();
  fixed_qed_hs_tls_create_certificate_result[2] = X509_new();
  ret = qed_hs_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  fixed_qed_hs_tls_create_certificate_results_free();
  ...
  fixed_crypto_pk_generate_key_with_bits_result_index = 0;
  fixed_qed_hs_tls_create_certificate_result_index = 0;
  fixed_qed_hs_tls_create_certificate_result[0] = X509_new();
  fixed_qed_hs_tls_create_certificate_result[1] = NULL;
  fixed_qed_hs_tls_create_certificate_result[2] = X509_new();
  ret = qed_hs_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  fixed_qed_hs_tls_create_certificate_results_free();
  ...
  fixed_qed_hs_tls_create_certificate_result[0] = X509_new();
  fixed_qed_hs_tls_create_certificate_result[1] = X509_new();
  fixed_qed_hs_tls_create_certificate_result[2] = NULL;
  ret = qed_hs_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  fixed_qed_hs_tls_create_certificate_results_free();

  MOCK(qed_hs_x509_cert_new, fixed_qed_hs_x509_cert_new);
  fixed_crypto_pk_new_result_index = 0;
  ...
  fixed_qed_hs_tls_create_certificate_result[1] = X509_new();
  fixed_qed_hs_tls_create_certificate_result[2] = X509_new();
  fixed_qed_hs_x509_cert_new_result_index = 0;
  fixed_qed_hs_x509_cert_new_result[0] = NULL;
  fixed_qed_hs_x509_cert_new_result[1] = NULL;
  fixed_qed_hs_x509_cert_new_result[2] = NULL;
  ret = qed_hs_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  fixed_qed_hs_tls_create_certificate_results_free();
  ...
  fixed_qed_hs_tls_create_certificate_result[2] = X509_new();
  fixed_qed_hs_x509_cert_new_result_index = 0;
  fixed_qed_hs_x509_cert_new_result[0] = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  fixed_qed_hs_x509_cert_new_result[1] = NULL;
  fixed_qed_hs_x509_cert_new_result[2] = NULL;
  ret = qed_hs_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  fixed_qed_hs_tls_create_certificate_results_free();
  fixed_qed_hs_x509_cert_new_results_free();
  ...
  fixed_qed_hs_x509_cert_new_result_index = 0;
  fixed_qed_hs_x509_cert_new_result[0] = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  fixed_qed_hs_x509_cert_new_result[1] = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  fixed_qed_hs_x509_cert_new_result[2] = NULL;
  ret = qed_hs_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  fixed_qed_hs_tls_create_certificate_results_free();
  ...
  fixed_qed_hs_x509_cert_new_result[0] = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  fixed_qed_hs_x509_cert_new_result[1] = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  fixed_qed_hs_x509_cert_new_result[2] = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  ret = qed_hs_tls_context_new(NULL, 0, 0, 0);
  tt_assert(!ret);
  ...
  fixed_qed_hs_tls_create_certificate_results_free();
  fixed_qed_hs_x509_cert_new_results_free();
  UNMOCK(qed_hs_x509_cert_new);
  UNMOCK(qed_hs_tls_create_certificate);
...
test_tortls_create_certificate(void *ignored)
{
  ...
  ret = qed_hs_tls_create_certificate(pk1, pk2, "hello", "hello2", 1);
  tt_assert(!ret);
...
test_tortls_cert_new(void *ignored)
{
  ...
  ret = qed_hs_x509_cert_new(NULL);
  tt_assert(!ret);

  ret = qed_hs_x509_cert_new(cert);
  tt_assert(ret);
  qed_hs_x509_cert_free(ret);
  ret = NULL;
...
test_tortls_cert_is_valid(void *ignored)
{
  (void)ignored;
  int ret;
  qed_hs_x509_cert_t *cert = NULL, *scert = NULL;
  time_t now = cert_strings_valid_at;

  scert = qed_hs_malloc_zero(sizeof(qed_hs_x509_cert_t));
  ret = qed_hs_tls_cert_is_valid(LOG_WARN, cert, scert, now, 0);
  tt_int_op(ret, OP_EQ, 0);
  ...
  cert = qed_hs_x509_cert_new(read_cert_from(validCertString));
  scert = qed_hs_x509_cert_new(read_cert_from(caCertString));
  ret = qed_hs_tls_cert_is_valid(LOG_WARN, cert, scert, now, 0);
  tt_int_op(ret, OP_EQ, 1);

#ifndef OPENSSL_OPAQUE
  qed_hs_x509_cert_free(cert);
  qed_hs_x509_cert_free(scert);
  cert = qed_hs_x509_cert_new(read_cert_from(validCertString));
  scert = qed_hs_x509_cert_new(read_cert_from(caCertString));
  ...
  ret = qed_hs_tls_cert_is_valid(LOG_WARN, cert, scert, now, 1);
  tt_int_op(ret, OP_EQ, 0);
#endif /* !defined(OPENSSL_OPAQUE) */
...
test_tortls_context_init_one(void *ignored)
{
  (void)ignored;
  int ret;
  qed_hs_tls_context_t *old = NULL;

  MOCK(crypto_pk_new, fixed_crypto_pk_new);

  fixed_crypto_pk_new_result_index = 0;
  fixed_crypto_pk_new_result[0] = NULL;
  ret = qed_hs_tls_context_init_one(&old, NULL, 0, 0, 0);
  tt_int_op(ret, OP_EQ, -1);
...
struct testcase_t tortls_openssl_tests[] = {
  LOCAL_TEST_CASE(qed_hs_tls_new, TT_FORK),
  LOCAL_TEST_CASE(get_state_description, TT_FORK),
  LOCAL_TEST_CASE(get_by_ssl, TT_FORK),
  LOCAL_TEST_CASE(allocate_qed_hs_tls_object_ex_data_index, TT_FORK),
