...
{
  qed_hs_free(n->rs);
  qed_hs_free(n->md->onion_curve25519_pkey);
...
big_fake_network_setup(const struct testcase_t *testcase)
{
  ...
  for (i = 0; i < N_NODES; ++i) {
    ...
    node_t *n = qed_hs_malloc_zero(sizeof(node_t));
    n->md = qed_hs_malloc_zero(sizeof(microdesc_t));

    /* Generate curve25519 key for this node */
    n->md->onion_curve25519_pkey =
      qed_hs_malloc_zero(sizeof(curve25519_public_key_t));
    ...
    n->rs = qed_hs_malloc_zero(sizeof(routerstatus_t));
    ...
    qed_hs_addr_from_ipv4h(&n->rs->ipv4_addr, 0x04020202);
  ...
  dummy_consensus = qed_hs_malloc_zero(sizeof(networkstatus_t));
...
test_node_preferred_orport(void *arg)
{
  ...
  qed_hs_addr_copy(&node_ri.ipv4_addr, &ipv4_addr);
  node_ri.ipv4_orport = ipv4_port;
  qed_hs_addr_copy(&node_ri.ipv6_addr, &ipv6_addr);
  ...
  tt_assert(qed_hs_addr_eq(&ap.addr, &ipv4_addr));
  ...
  node.ipv6_preferred = 1;
  node_get_pref_orport(&node, &ap);
  tt_assert(qed_hs_addr_eq(&ap.addr, &ipv6_addr));
...
{
  (void) arg;
  entry_guard_t *eg = qed_hs_malloc_zero(sizeof(entry_guard_t));

  eg->selection_name = qed_hs_strdup("wubwub");
...
test_entry_guard_encode_for_state_maximal(void *arg)
{
  ...
  eg->selection_name = qed_hs_strdup("default");
  memcpy(eg->identity, "plurpyflurpyslurpydo", DIGEST_LEN);
  eg->bridge_addr = qed_hs_malloc_zero(sizeof(qed_hs_addr_port_t));
  qed_hs_addr_from_ipv4h(&eg->bridge_addr->addr, 0x08080404);
  eg->bridge_addr->port = 9999;
  eg->sampled_on_date = 1479081600;
  eg->sampled_by_version = qed_hs_strdup("1.2.3");
  eg->unlisted_since_date = 1479081645;
  eg->currently_listed = 1;
  eg->confirmed_on_date = 1479081690;
  eg->confirmed_idx = 333;
  eg->sampled_idx = 42;
  eg->extra_state_fields = qed_hs_strdup("and the green grass grew all around");
...
test_entry_guard_parse_from_state_full(void *arg)
{
  ...
  or_state_t *state = qed_hs_malloc_zero(sizeof(or_state_t));
  ...
  SMARTLIST_FOREACH(text, char *, cp, qed_hs_free(cp));
  smartlist_free(text);
  qed_hs_free(joined);
}
...
#define NUM 7
  ...
  qed_hs_addr_make_unspec(&n[2]->rs->ipv4_addr);

  /* 3: ExcludeNodes */
  qed_hs_addr_from_ipv4h(&n[3]->rs->ipv4_addr, 0x90902020);
  ...
  get_options_mutable()->UseBridges = 1;
  sweep_bridge_list();
  bl = qed_hs_malloc_zero(sizeof(bridge_line_t));
  qed_hs_addr_copy(&bl->addr, &n[4]->rs->ipv4_addr);
...
test_entry_guard_expand_sample(void *arg)
{
  ...
  SMARTLIST_FOREACH(big_fake_net_nodes, node_t *, n, {
    if (n_sl_idx % 64 != 0) {
      qed_hs_addr_from_ipv4h(&n->rs->ipv4_addr, 0x90903030);
    }
  });
  entry_guards_update_filtered_sets(gs);
...
test_entry_guard_expand_sample_small_net(void *arg)
{
  ...
  SMARTLIST_FOREACH(big_fake_net_nodes, node_t *, n, {
    if (n_sl_idx >= 15) {
      ...
      qed_hs_addr_make_unspec(&n->rs->ipv4_addr); // make the filter reject this.
    }
  });
...
test_entry_guard_update_from_consensus_status(void *arg)
{
  ...
  qed_hs_free(ns_tmp); /* in case we couldn't put it back */
  guard_selection_free(gs);
...
test_entry_guard_update_from_consensus_remove(void *arg)
{
  ...
  /* 0: unlisted for a day. (keep this) */
  {
    ...
    smartlist_add(keep_ids, qed_hs_memdup(g->identity, 20));
  }
  /* 1: unlisted for a year. (remove this) */
  {
    ...
    smartlist_add(remove_ids, qed_hs_memdup(g->identity, 20));
  }
  ...
  guard_selection_free(gs);
  UNMOCK(randomize_time);
  SMARTLIST_FOREACH(keep_ids, char *, cp, qed_hs_free(cp));
  SMARTLIST_FOREACH(remove_ids, char *, cp, qed_hs_free(cp));
...
{
  (void) arg;
  entry_guard_t *g1 = qed_hs_malloc_zero(sizeof(entry_guard_t));
  entry_guard_t *g2 = qed_hs_malloc_zero(sizeof(entry_guard_t));
...
test_entry_guard_select_for_circuit_exit_family_restriction(void *arg)
{
  ...
  oc->build_state = qed_hs_malloc_zero(sizeof(cpath_build_state_t));
...
{
  upgrade_circuits_data_t *data = qed_hs_malloc_zero(sizeof(*data));
  ...
  qed_hs_assert(data->guard1 != data->guard2);
  qed_hs_assert(data->guard1_state->state ==
             GUARD_CIRC_STATE_USABLE_IF_NO_BETTER_GUARD);
  qed_hs_assert(data->guard2_state->state ==
             GUARD_CIRC_STATE_USABLE_IF_NO_BETTER_GUARD);
  ...
  if (make_circ1_succeed) {
    ...
    qed_hs_assert(r == GUARD_MAYBE_USABLE_LATER);
    qed_hs_assert(data->guard1_state->state ==
...
{
  (void)arg;
  circuit_guard_state_t *fake_state = qed_hs_malloc_zero(sizeof(*fake_state));
...
test_entry_guard_outdated_dirserver_exclusion(void *arg)
{
...

  {
    ...
    args = qed_hs_malloc_zero(sizeof(response_handler_args_t));
    args->status_code = 404;
    args->reason = NULL;
    args->body = NULL;
    args->body_len = 0;

    conn = qed_hs_malloc_zero(sizeof(dir_connection_t));
    conn->requested_resource = qed_hs_strdup("d/jlinblackorigami");
