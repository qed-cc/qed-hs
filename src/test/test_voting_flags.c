...
setup_cfg(flag_vote_test_cfg_t *c)
{
  ...
  qed_hs_addr_from_ipv4h(&c->ri.ipv4_addr, 0x7f010105);
  qed_hs_addr_from_ipv4h(&c->expected.ipv4_addr, 0x7f010105);
  c->ri.ipv4_orport = 9090;
  c->expected.ipv4_orport = 9090;

  qed_hs_addr_make_null(&c->ri.ipv6_addr, AF_INET6);
  qed_hs_addr_make_null(&c->expected.ipv6_addr, AF_INET6);
...
check_result(flag_vote_test_cfg_t *c)
{
  ...
  tt_assert(qed_hs_addr_eq(&rs.ipv6_addr, &c->expected.ipv6_addr));
...
test_voting_flags_ipv6(void *arg)
{
  ...
  tt_assert(qed_hs_addr_parse(&cfg->ri.ipv6_addr, "f00::b42") == AF_INET6);
  ...
  tt_assert(qed_hs_addr_parse(&cfg->expected.ipv6_addr, "f00::b42") == AF_INET6);
...
{
  (void)testcase;
  flag_vote_test_cfg_t *cfg = qed_hs_malloc_zero(sizeof(*cfg));
  setup_cfg(cfg);
  return cfg;
}
