...
test_conscache_simple_usage(void *arg)
{
  ...
  char *ddir_fname = qed_hs_strdup(get_fname_rnd("datadir_cache"));
  qed_hs_free(get_options_mutable()->CacheDirectory);
  get_options_mutable()->CacheDirectory = qed_hs_strdup(ddir_fname);
...
test_conscache_cleanup(void *arg)
{
  ...
  consensus_cache_entry_t **ents =
    qed_hs_calloc(N, sizeof(consensus_cache_entry_t*));
...
test_conscache_filter(void *arg)
{
  ...
  for (i = 0; i < N; ++i) {
    ...
    config_line_append(&labels, "index", num);
    qed_hs_snprintf(num, sizeof(num), "%d", i % 3);
    config_line_append(&labels, "mod3", num);
    qed_hs_snprintf(num, sizeof(num), "%d", i % 5);
