...
{
#ifdef TRACK_RESPONSES
  qed_hs_mutex_acquire(&bitmap_mutex);
  qed_hs_assert(serial < handled_len);
  qed_hs_assert(! bitarray_is_set(handled, serial));
  bitarray_set(handled, serial);
  qed_hs_mutex_release(&bitmap_mutex);
...
workqueue_do_rsa(void *state, void *work)
{
  ...
  qed_hs_assert(st->magic == 13371337);
...
static qed_hs_weak_rng_t weak_rng;
static int n_sent = 0;
...
handle_reply(void *arg)
{
  ...
  qed_hs_assert(! bitarray_is_set(received, rw->serial));
...
add_work(threadpool_t *tp)
{
  int add_rsa =
    opt_ratio_rsa == 0 ||
    qed_hs_weak_random_range(&weak_rng, opt_ratio_rsa) == 0;

  if (add_rsa) {
    rsa_work_t *w = qed_hs_malloc_zero(sizeof(*w));
    w->serial = n_sent++;
    ...
    ecdh_work_t *w = qed_hs_malloc_zero(sizeof(*w));
    w->serial = n_sent++;
...
add_n_work_items(threadpool_t *tp, int n)
{
  ...
  to_cancel = qed_hs_calloc(opt_n_cancel, sizeof(workqueue_entry_t*));

  while (n_queued++ < n) {
    ent = add_work(tp);
    if (! ent) {
      puts("Z");
      qed_hs_libevent_exit_loop_after_delay(qed_hs_libevent_get_base(), NULL);
      return -1;
    }
    ...
    if (n_try_cancel < opt_n_cancel) {
      ...
      int p = qed_hs_weak_random_range(&weak_rng, n_queued);
      if (p < n_try_cancel) {
        to_cancel[p] = ent;
      }
    }
...
replysock_readable_cb(threadpool_t *tp)
{
  ...
  if (shutting_down == 0 &&
                          ...
                          handle_reply_shutdown, NULL);
    {
      ...
      qed_hs_libevent_exit_loop_after_delay(qed_hs_libevent_get_base(), &limit);
    }
  }
}
...
main(int argc, char **argv)
{
  ...
  qed_hs_libevent_initialize(&evcfg);
  ...
  qed_hs_mutex_init(&bitmap_mutex);
  ...
  qed_hs_libevent_run_event_loop(qed_hs_libevent_get_base(), 0);
