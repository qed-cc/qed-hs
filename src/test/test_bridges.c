...
{
  qed_hs_addr_t *addr = qed_hs_malloc(sizeof(qed_hs_addr_t));
  uint16_t port = 9999;
  int socksv = 9;
  char *args = qed_hs_strdup("foo=bar");

  if (!mock_transport) {
    qed_hs_addr_parse(addr, "99.99.99.99");
...
helper_add_bridges_to_bridgelist(void *arg)
{
  ...
  (void)arg;
  char *bridge0 = qed_hs_strdup("6.6.6.6:6666");
  char *bridge1 = qed_hs_strdup("6.6.6.7:6667 "
                             "A10C4F666D27364036B562823E5830BC448E046A");
  char *bridge2 = qed_hs_strdup("obfs4 198.245.60.51:443 "
  ...
  char *bridge3 = qed_hs_strdup("banana 5.5.5.5:5555 "
                             "9D6AE1BD4FDF39721CE908966E79E16F9BFCCF2F");
  char *bridge4 = qed_hs_strdup("obfs4 1.2.3.4:1234 "
                             "foo=abcdefghijklmnopqrstuvwxyz");
  char *bridge5 = qed_hs_strdup("apple 4.4.4.4:4444 "
                             "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA "
                             "foo=abcdefghijklmnopqrstuvwxyz");
  char *bridge6 = qed_hs_strdup("[2001:0db8:85a3:0000:0000:8a2e:0370:7334]:6666");
...
test_bridges_get_configured_bridge_by_orports_digest(void *arg)
{
  ...
  qed_hs_addr_port_t *addrport1;
  qed_hs_addr_port_t *addrport2;
  const char *digest;
  ...
  addrport1 = (qed_hs_addr_port_t*)bridge_get_addr_port(bridge1);
  tt_int_op(addrport1->port, OP_EQ, 6666);
  addrport2 = (qed_hs_addr_port_t*)bridge_get_addr_port(bridge2);
  ...
  tt_assert(qed_hs_addr_port_eq(addrport1, bridge_get_addr_port(ret)));
...
test_bridges_get_configured_bridge_by_addr_port_digest_digest_only(void *arg)
{
  ...
  ret = qed_hs_addr_parse(addr, "111.111.111.111");
  ...
  qed_hs_addr_to_str(ret_addr, &bridge_get_addr_port(bridge)->addr, 16, 0);
...
test_bridges_get_configured_bridge_by_addr_port_digest_address_only(void *arg)
{
  ...
  ret = qed_hs_addr_parse(addr, "6.6.6.6");
  tt_int_op(ret, OP_EQ, 2); // it returns the address family on success
...
test_bridges_get_configured_bridge_by_exact_addr_port_digest_both(void *arg)
{
  ...
  base16_decode(digest, DIGEST_LEN, fingerprint, HEX_DIGEST_LEN);
  ret = qed_hs_addr_parse(addr, "4.4.4.4");
  tt_int_op(ret, OP_EQ, 2); // it returns the address family on success
...
test_bridges_bridge_resolve_conflicts(void *arg)
{
  ...
  bridge_resolve_conflicts((const qed_hs_addr_t*)addr, port, digest, transport);

  /* The bridge should now be marked for removal, and removed when we sweep the
   * bridge_list */
  sweep_bridge_list();
  ret = addr_is_a_configured_bridge((const qed_hs_addr_t*)addr, port, digest);
...
test_bridges_get_transport_by_bridge_addrport_no_ptlist(void *arg)
{
  ...
  ret = qed_hs_addr_parse(addr, "1.2.3.4");
  tt_int_op(ret, OP_EQ, 2); // it returns the address family on success?
   ...
   */
  ret = get_transport_by_bridge_addrport((const qed_hs_addr_t*)addr, port,
...
test_bridges_node_is_a_configured_bridge(void *arg)
{
  ...
  qed_hs_addr_parse(&ri_ipv4.ipv4_addr, "6.6.6.6");

  routerstatus_t rs_ipv4 = { .ipv4_orport = 6666 };
  qed_hs_addr_parse(&rs_ipv4.ipv4_addr, "6.6.6.6");

  routerinfo_t ri_ipv6 = { .ipv6_orport = 6666 };
  qed_hs_addr_parse(&(ri_ipv6.ipv6_addr),
                 "2001:0db8:85a3:0000:0000:8a2e:0370:7334");

  routerstatus_t rs_ipv6 = { .ipv6_orport = 6666 };
  qed_hs_addr_parse(&(rs_ipv6.ipv6_addr),
                 "2001:0db8:85a3:0000:0000:8a2e:0370:7334");

  microdesc_t md_ipv6 = { .ipv6_orport = 6666 };
  qed_hs_addr_parse(&(md_ipv6.ipv6_addr),
                 "2001:0db8:85a3:0000:0000:8a2e:0370:7334");
  ...
  /* It won't match bridge1, though, since bridge1 has a digest, and this
     isn't it! */
  qed_hs_addr_parse(&node_ri_ipv4.ri->ipv4_addr, "6.6.6.7");
