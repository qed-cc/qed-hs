...
test_routerkeys_write_fingerprint(void *arg)
{
  ...
  options->DataDirectory = qed_hs_strdup(ddir);
  options->Nickname = qed_hs_strdup("haflinger");
  ...
  qed_hs_asprintf(&cp2, "haflinger %s\n", fp);
...
test_routerkeys_ed_certs(void *args)
{
  ...
  qed_hs_cert_t *cert[2] = {NULL, NULL}, *nocert = NULL;
  qed_hs_cert_t *parsed_cert[2] = {NULL, NULL};
  ...
  for (int i = 0; i <= 1; ++i) {
    ...
    cert[i] = qed_hs_cert_create_ed25519(&kp1, 5, &kp2.pubkey, now, 10000, flags);
    tt_assert(cert[i]);
    ...
    parsed_cert[i] = qed_hs_cert_parse(cert[i]->encoded, cert[i]->encoded_len);
    ...
    /* Expired */
    tt_int_op(qed_hs_cert_checksig(parsed_cert[i], &kp1.pubkey, now + 30000),
              OP_LT, 0);
    tt_uint_op(parsed_cert[i]->cert_expired, OP_EQ, 1);
    parsed_cert[i]->cert_expired = 0;

    /* Wrong key */
    tt_int_op(qed_hs_cert_checksig(parsed_cert[i], &kp2.pubkey, now), OP_LT, 0);
    tt_uint_op(parsed_cert[i]->sig_bad, OP_EQ, 1);
    parsed_cert[i]->sig_bad = 0;

    /* Missing key */
    int ok = qed_hs_cert_checksig(parsed_cert[i], NULL, now);
    ...
    /* Right key */
    tt_int_op(qed_hs_cert_checksig(parsed_cert[i], &kp1.pubkey, now), OP_EQ, 0);
    tt_uint_op(parsed_cert[i]->sig_bad, OP_EQ, 0);
  ...
  /* - Truncated */
  nocert = qed_hs_cert_parse(cert[0]->encoded, cert[0]->encoded_len-1);
  tt_ptr_op(NULL, OP_EQ, nocert);

  /* - First byte modified */
  cert[0]->encoded[0] = 99;
  nocert = qed_hs_cert_parse(cert[0]->encoded, cert[0]->encoded_len);
  tt_ptr_op(NULL, OP_EQ, nocert);
  cert[0]->encoded[0] = 1;

  /* - Extra byte at the end*/
  junk = qed_hs_malloc_zero(cert[0]->encoded_len + 1);
  memcpy(junk, cert[0]->encoded, cert[0]->encoded_len);
  nocert = qed_hs_cert_parse(junk, cert[0]->encoded_len+1);
  tt_ptr_op(NULL, OP_EQ, nocert);

  /* - Multiple signing key instances */
  qed_hs_free(junk);
  junk = qed_hs_malloc_zero(104 + 36 * 2);
  junk[0] = 1; /* version */
  junk[1] = 5; /* cert type */
  junk[6] = 1; /* key type */
  ...
  junk[77] = 32; /* extlen */
  junk[78] = 4; /* exttype */
  nocert = qed_hs_cert_parse(junk, 104 + 36 * 2);
  tt_ptr_op(NULL, OP_EQ, nocert);

 done:
  qed_hs_cert_free(cert[0]);
  qed_hs_cert_free(cert[1]);
  qed_hs_cert_free(parsed_cert[0]);
  qed_hs_cert_free(parsed_cert[1]);
  qed_hs_cert_free(nocert);
  qed_hs_free(junk);
  qed_hs_free(base64);
}
...
test_routerkeys_ed_key_init_basic(void *arg)
{
  ...
  qed_hs_cert_t *cert = NULL, *cert2 = NULL;
  ed25519_keypair_t *kp1 = NULL, *kp2 = NULL, *kp3 = NULL;
  time_t now = time(NULL);
  char *fname1 = qed_hs_strdup(get_fname("test_ed_key_1"));
  char *fname2 = qed_hs_strdup(get_fname("test_ed_key_2"));
  struct stat st;
  ...
  qed_hs_cert_free(cert2); cert2 = NULL;
...
test_routerkeys_ed_key_init_split(void *arg)
{
  ...
  ed25519_keypair_t *kp1 = NULL, *kp2 = NULL;
  time_t now = time(NULL);
  char *fname1 = qed_hs_strdup(get_fname("test_ed_key_3"));
  char *fname2 = qed_hs_strdup(get_fname("test_ed_key_4"));
  struct stat st;
...
{
  (void)arg;
  char *dir = qed_hs_strdup(get_fname("test_ed_keys_init_all"));
  char *keydir = qed_hs_strdup(get_fname("test_ed_keys_init_all/KEYS"));
  or_options_t *options = qed_hs_malloc_zero(sizeof(or_options_t));
  time_t now = time(NULL);
  ed25519_public_key_t id;
  ed25519_keypair_t sign, auth;
  qed_hs_cert_t *link_cert = NULL;
  ...
  link_cert = qed_hs_cert_dup(get_current_link_cert_cert());
  ...
  tt_assert(qed_hs_cert_eq(link_cert, get_current_link_cert_cert()));
...
test_routerkeys_cross_certify_ntor(void *args)
{
  ...
  tt_int_op(0, OP_EQ, qed_hs_cert_checksig(cert, &onion_check_key, now));
...
test_routerkeys_rsa_ed_crosscert(void *arg)
{
  ...
  cc_len = qed_hs_make_rsa_ed25519_crosscert(&ed, rsa, expires_in, &cc);
...
{
  (void) arg;
  char *dname = qed_hs_strdup(get_fname_rnd("fkeys"));
  char *fname = NULL;
  or_options_t *options = get_options_mutable();
  ...
  qed_hs_asprintf(&fname, "%s"PATH_SEPARATOR"junk.1", dname);
  ...
  qed_hs_asprintf(&fname, "%s"PATH_SEPARATOR"cg.secret_family_key", dname);
  tt_int_op(0, OP_EQ, create_family_id_key(fname, &pubkey));
  qed_hs_free(fname);
  smartlist_add(options->FamilyIds, qed_hs_memdup(&pubkey, sizeof(pubkey)));

  tt_int_op(0, OP_EQ, load_family_id_keys_impl(options, dname));
  tt_int_op(1, OP_EQ, smartlist_len(get_current_family_id_keys()));

  //Try a second key.
  qed_hs_asprintf(&fname, "%s"PATH_SEPARATOR"eb.secret_family_key", dname);
  tt_int_op(0, OP_EQ, create_family_id_key(fname, &pubkey));
  smartlist_add(options->FamilyIds, qed_hs_memdup(&pubkey, sizeof(pubkey)));
  qed_hs_free(fname);
  ...
  qed_hs_asprintf(&fname, "%s"PATH_SEPARATOR"gt.secret_family_key", dname);
  tt_int_op(0, OP_EQ, create_family_id_key(fname, &pubkey));
  ...
  qed_hs_asprintf(&fname, "%s"PATH_SEPARATOR"xyz.secret_family_key", dname);
  write_str_to_file(fname, "hello world", 0);
  qed_hs_free(fname);
