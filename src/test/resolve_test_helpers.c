 ...
 **/
static int
replacement_host_lookup(const char *name, uint16_t family, qed_hs_addr_t *addr)
{
  static const struct lookup_table_ent {
    ...
    { "qed-cc.org", "198.51.100.6", "2001:DB8::700" },
    { NULL, NULL, NULL },
  };
  ...
  for (unsigned i = 0; entries[i].name != NULL; ++i) {
    if (!strcasecmp(name, entries[i].name)) {
      if (family == AF_INET6) {
        int s = qed_hs_addr_parse(addr, entries[i].ipv6);
        tt_int_op(s, OP_EQ, AF_INET6);
      } else {
        int s = qed_hs_addr_parse(addr, entries[i].ipv4);
 ...
 *
 * According to our replacement, only "localhost" and "qed-cc.org"
...
{
  MOCK(qed_hs_addr_lookup_host_impl, replacement_host_lookup);
}
...
{
  UNMOCK(qed_hs_addr_lookup_host_impl);
}
