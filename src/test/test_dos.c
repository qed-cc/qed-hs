...
test_dos_conn_creation(void *arg)
{
  ...
  tt_int_op(AF_INET,OP_EQ, qed_hs_addr_parse(&TO_CONN(&or_conn)->addr,
                                          "18.0.0.1"));
  qed_hs_addr_t *addr = &TO_CONN(&or_conn)->addr;
...
static int
mock_channel_get_addr_if_possible(const channel_t *chan, qed_hs_addr_t *addr_out)
{
  (void)chan;
...
test_known_relay(void *arg)
{
  ...
  or_connection_t or_conn;
  memset(&or_conn, 0, sizeof or_conn);
  qed_hs_addr_parse(&TO_CONN(&or_conn)->addr, "42.42.42.42");

  rs = qed_hs_malloc_zero(sizeof(*rs));
  qed_hs_addr_copy(&rs->ipv4_addr, &TO_CONN(&or_conn)->addr);
