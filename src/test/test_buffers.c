...
test_buffers_basic(void *arg)
{
  ...
  {
    char *mem = qed_hs_malloc_zero(65536);
    buf = buf_new();
...
test_buffer_pullup(void *arg)
{
  ...
  size_t sz;
  (void)arg;
  stuff = qed_hs_malloc(16384);
  tmp = qed_hs_malloc(16384);
                                            ...
                                            compression_level_t level)
{
  ...
  char *expanded = NULL;
  buf_t *buf = NULL;
  qed_hs_compress_state_t *compress_state = NULL;
  size_t out_len, in_len;
  size_t sz, headerjunk;

  buf = buf_new_with_capacity(128); /* will round up */
  sz = buf_get_default_chunk_size(buf);
  msg = qed_hs_malloc_zero(sz);
  ...
  compress_state = qed_hs_compress_new(1, method, level);
  tt_int_op(buf_add_compress(buf, compress_state, "", 0, 1), OP_EQ, 0);

  in_len = buf_datalen(buf);
  contents = qed_hs_malloc(in_len);
  ...
  tt_int_op(0, OP_EQ, qed_hs_uncompress(&expanded, &out_len,
                                     contents + headerjunk,
                                     in_len - headerjunk,
                                     method, 1,
                                     LOG_WARN));
  ...
  buf_free(buf);
  qed_hs_compress_free(compress_state);
  qed_hs_free(contents);
  qed_hs_free(expanded);
  qed_hs_free(msg);
}
...
static int
mock_tls_read(qed_hs_tls_t *tls, char *cp, size_t len)
{
  (void)tls;
...
test_buffers_tls_read_mocked(void *arg)
{
  ...
  mem = qed_hs_malloc(64*1024);
  crypto_rand((char*)mem, 64*1024);
  tls_read_ptr = mem;
  n_remaining = 64*1024;

  MOCK(qed_hs_tls_read, mock_tls_read);
...
test_buffers_find_contentlen(void *arg)
{
  ...
  for (i = 0; results[i].headers; ++i) {
    ...
    char * tmp = qed_hs_memdup(results[i].headers, headerlen);/* ensure no eos */
