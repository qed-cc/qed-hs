/* Copyright (c) 2015-2021, The QED Project, Inc. */
                                ...
                                const char *s)
{
  ...
  (void)code;
  (void)c;
  qed_hs_free(reply_str);
  reply_str = qed_hs_strdup(s);
}
...
test_add_onion_helper_keyarg_v3(void *arg)
{
  ...
  /* Test passing a key blob. */
  {
    ...
    qed_hs_asprintf(&key_blob, "ED25519-V3:%s", base64_sk);
    tt_assert(key_blob);
    qed_hs_free(reply_str);
    ret = add_onion_helper_keyarg(key_blob, 1, &key_new_alg,
                                  &key_new_blob, &pk, &hs_version,
                                  NULL);
    qed_hs_free(key_blob);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(hs_version, OP_EQ, HS_VERSION_THREE);
    tt_assert(pk.v3);
...
test_getinfo_helper_onion(void *arg)
{
  ...
  service_id = qed_hs_strdup("dummy_onion_id");
...
test_download_status_cert(void *arg)
{
  ...
  memcpy(&auth_def_cert_download_status_1, &dls_sample_1,
         sizeof(download_status_t));
  qed_hs_asprintf(&question, "downloads/cert/fp/%s", auth_id_digest_1_str);
  tt_ptr_op(question, OP_NE, NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_ptr_op(answer, OP_NE, NULL);
  tt_ptr_op(errmsg, OP_EQ, NULL);
  tt_str_op(answer, OP_EQ, dls_sample_1_str);
  qed_hs_free(question);
  qed_hs_free(answer);
  errmsg = NULL;
  ...
  memcpy(&auth_def_cert_download_status_2, &dls_sample_2,
         sizeof(download_status_t));
  qed_hs_asprintf(&question, "downloads/cert/fp/%s", auth_id_digest_2_str);
  tt_ptr_op(question, OP_NE, NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_ptr_op(answer, OP_NE, NULL);
  tt_ptr_op(errmsg, OP_EQ, NULL);
  ...
  qed_hs_asprintf(&question, "downloads/cert/fp/%s/sks", auth_id_digest_1_str);
  tt_ptr_op(question, OP_NE, NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_ptr_op(answer, OP_NE, NULL);
  tt_ptr_op(errmsg, OP_EQ, NULL);
  ...
  qed_hs_asprintf(&question, "downloads/cert/fp/%s/sks", auth_id_digest_2_str);
  tt_ptr_op(question, OP_NE, NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_ptr_op(answer, OP_NE, NULL);
  tt_ptr_op(errmsg, OP_EQ, NULL);
  ...
  memcpy(&auth_1_sk_1_dls, &dls_sample_3,
         sizeof(download_status_t));
  qed_hs_asprintf(&question, "downloads/cert/fp/%s/%s",
...
test_download_status_desc(void *arg)
{
  ...
  memcpy(&descbr_digest_1_dl, &dls_sample_1,
         sizeof(download_status_t));
  qed_hs_asprintf(&question, "downloads/desc/%s", descbr_digest_1_str);
  tt_ptr_op(question, OP_NE, NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_ptr_op(answer, OP_NE, NULL);
  tt_ptr_op(errmsg, OP_EQ, NULL);
  ...
  memcpy(&descbr_digest_2_dl, &dls_sample_2,
         sizeof(download_status_t));
  qed_hs_asprintf(&question, "downloads/desc/%s", descbr_digest_2_str);
  tt_ptr_op(question, OP_NE, NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_ptr_op(answer, OP_NE, NULL);
  tt_ptr_op(errmsg, OP_EQ, NULL);
...
test_download_status_bridge(void *arg)
{
  ...
  memcpy(&descbr_digest_1_dl, &dls_sample_3,
         sizeof(download_status_t));
  qed_hs_asprintf(&question, "downloads/bridge/%s", descbr_digest_1_str);
  tt_ptr_op(question, OP_NE, NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_ptr_op(answer, OP_NE, NULL);
  tt_ptr_op(errmsg, OP_EQ, NULL);
  ...
  memcpy(&descbr_digest_2_dl, &dls_sample_4,
         sizeof(download_status_t));
  qed_hs_asprintf(&question, "downloads/bridge/%s", descbr_digest_2_str);
  tt_ptr_op(question, OP_NE, NULL);
  getinfo_helper_downloads(&dummy, question, &answer, &errmsg);
  tt_ptr_op(answer, OP_NE, NULL);
  tt_ptr_op(errmsg, OP_EQ, NULL);
...
mock_dirserv_get_consensus(const char *flavor_name)
{
  if (!strcmp(flavor_name, "ns")) {
    mock_ns_consensus_cache = qed_hs_malloc_zero(sizeof(cached_dir_t));
    mock_ns_consensus_cache->dir = qed_hs_strdup("mock_ns_consensus");
    return mock_ns_consensus_cache;
  } else {
    mock_microdesc_consensus_cache = qed_hs_malloc_zero(sizeof(cached_dir_t));
    mock_microdesc_consensus_cache->dir = qed_hs_strdup(
...
mock_qed_hs_mmap_file(const char* filename)
{
  qed_hs_mmap_t *res;
  res = qed_hs_malloc_zero(sizeof(qed_hs_mmap_t));
...
static int
mock_qed_hs_munmap_file(qed_hs_mmap_t *handle)
{
  qed_hs_free(handle);
  return 0;
}
...
test_getinfo_helper_current_consensus_from_file(void *arg)
{
  ...
  MOCK(qed_hs_mmap_file, mock_qed_hs_mmap_file);
  MOCK(qed_hs_munmap_file, mock_qed_hs_munmap_file);
  ...
  qed_hs_free(answer);
  UNMOCK(qed_hs_mmap_file);
  UNMOCK(qed_hs_munmap_file);
  return;
}
...
test_getinfo_helper_current_consensus_from_cache(void *arg)
{
  ...
  tt_ptr_op(errmsg, OP_EQ, NULL);
  qed_hs_free(answer);
  qed_hs_free(mock_ns_consensus_cache->dir);
  qed_hs_free(mock_ns_consensus_cache);
  errmsg = NULL;
  ...
  tt_str_op(answer, OP_EQ, "mock_microdesc_consensus");
  tt_ptr_op(errmsg, OP_EQ, NULL);
  qed_hs_free(mock_microdesc_consensus_cache->dir);
  qed_hs_free(answer);
  errmsg = NULL;
  ...
  qed_hs_free(answer);
  qed_hs_free(mock_microdesc_consensus_cache);
...
test_getinfo_md_all(void *arg)
{
  ...
  node_t *node1 = qed_hs_malloc(sizeof(node_t));
  memset(node1, 0, sizeof(node_t));
  node1->md = qed_hs_malloc(sizeof(microdesc_t));
  memset(node1->md, 0, sizeof(microdesc_t));
  node1->md->body = qed_hs_strdup("md1\n");
  node1->md->bodylen = 4;

  node_t *node2 = qed_hs_malloc(sizeof(node_t));
  memset(node2, 0, sizeof(node_t));
  node2->md = qed_hs_malloc(sizeof(microdesc_t));
  memset(node2->md, 0, sizeof(microdesc_t));
  node2->md->body = qed_hs_strdup("md2\n");
  node2->md->bodylen = 4;
  ...
  qed_hs_free(node1->md->body);
  qed_hs_free(node1->md);
  qed_hs_free(node1);
  qed_hs_free(node2->md->body);
  qed_hs_free(node2->md);
  qed_hs_free(node2);
  qed_hs_free(answer);
...
test_control_reply(void *arg)
{
  ...
  control_reply_clear(lines);
  SMARTLIST_FOREACH(reply_strs, char *, p, qed_hs_free(p));
...
test_control_getconf(void *arg)
{
  ...
  conn.current_cmd = qed_hs_strdup("GETCONF");
  ...
  args = qed_hs_strdup("NoSuch1 NoSuch2");
  r = handle_control_command(&conn, (uint32_t)strlen(args), args);
  tt_int_op(r, OP_EQ, 0);
  tt_int_op(smartlist_len(reply_strs), OP_EQ, 2);
  tt_str_op((char *)smartlist_get(reply_strs, 0), OP_EQ,
  ...
  args = qed_hs_strdup("ControlPort NoSuch");
  r = handle_control_command(&conn, (uint32_t)strlen(args), args);
  tt_int_op(r, OP_EQ, 0);
  ...
  args = qed_hs_strdup("BridgeRelay ClientOnly");
  r = handle_control_command(&conn, (uint32_t)strlen(args), args);
  tt_int_op(r, OP_EQ, 0);
  tt_int_op(smartlist_len(reply_strs), OP_EQ, 2);
