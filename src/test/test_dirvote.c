...
#define ALLOCATE_MOCK_NODES()                    \
  running_node = qed_hs_malloc(sizeof(node_t));     \
  running_node->is_running = 1;                  \
  non_running_node = qed_hs_malloc(sizeof(node_t)); \
  non_running_node->is_running = 0;

/* Free the memory allocated to the mock nodes */
#define FREE_MOCK_NODES() \
  qed_hs_free(running_node); \
  qed_hs_free(non_running_node);
...
{
  router_values_t *status = qed_hs_malloc(sizeof(router_values_t));
...
test_dirvote_compare_routerinfo_usefulness(void *arg)
{
  ...
  qed_hs_free(status_one);
  qed_hs_free(status_two);
  qed_hs_free(status_three);
  qed_hs_free(status_four);
  qed_hs_free(status_five);
  qed_hs_free(first);
}
...
test_dirvote_compare_routerinfo_by_ipv4(void *arg)
{
  ...
  a = compare_routerinfo_by_ipv4((const void **)&second,
                                 (const void **)&first);
  tt_assert(a > 0);
  qed_hs_addr_copy(&(second->ipv4_addr), &(first->ipv6_addr));
...
test_dirvote_compare_routerinfo_by_ipv6(void *arg)
{
  ...
  a = compare_routerinfo_by_ipv6((const void **)&second,
                                 (const void **)&first);
  tt_assert(a > 0);
  qed_hs_addr_copy(&(first->ipv6_addr), &(second->ipv6_addr));
  // The addresses are equal, they are compared by usefulness,
  // and first is less useful than second
...
#define ROUTER_FREE(name) \
  qed_hs_free(name##_val);   \
  qed_hs_free(name##_ri);
