...
#define conf_fmt "ClientOnionAuthDir %s\n"
  qed_hs_asprintf(&conf, conf_fmt, get_fname("auth_keys"));
#undef conf_fmt
  ...
  hs_client_service_authorization_t *auth =
    qed_hs_malloc_zero(sizeof(hs_client_service_authorization_t));
                                    ...
                                    connection_t **conn_out)
{
  ...
  TO_ENTRY_CONN(conn)->original_dest_address =
    qed_hs_strdup(TO_ENTRY_CONN(conn)->socks_request->address);
  ...
  /* fake n_chan */
  n_chan = qed_hs_malloc_zero(sizeof(channel_tls_t));
...
test_e2e_rend_circuit_setup(void *arg)
{
  ...
  if (or_circ)
    qed_hs_free(TO_CIRCUIT(or_circ)->n_chan);
    ...
    SMARTLIST_FOREACH_BEGIN(desc->encrypted_data.intro_points,
      ...
      if (intro_ei == NULL) {
        ...
        intro_ei = hs_get_extend_info_from_lspecs(ip->link_specifiers,
                                                  &ip->onion_key, 1);
        tt_assert(qed_hs_addr_family(&intro_ei->orports[0].addr) == AF_INET6);
      }
      tt_assert(intro_ei);
      if (intro_ei) {
        const char *ptr;
        char ip_addr[QED_HS_ADDR_BUF_LEN];
        /* We need to decorate in case it is an IPv6 else routerset_parse()
         * doesn't like it. */
        ptr = qed_hs_addr_to_str(ip_addr, &intro_ei->orports[0].addr,
                              sizeof(ip_addr), 1);
...
test_descripqed_hs_fetch(void *arg)
{
  ...
  /* 4. We do have a pending directory request. */
  {
    ...
    dir_conn->hs_ident = qed_hs_malloc_zero(sizeof(hs_ident_dir_conn_t));
...
mock_qed_hs_listdir(const char *dirname)
{
  ...
  smartlist_add(file_list, qed_hs_strdup("client1.auth_private"));
  smartlist_add(file_list, qed_hs_strdup("dummy.xxx"));
  smartlist_add(file_list, qed_hs_strdup("client2.auth_private"));
...
test_config_client_authorization(void *arg)
{
  ...
  char *key_dir = qed_hs_strdup(get_fname("auth_keys"));
...
#define conf_fmt \
  ...
  qed_hs_asprintf(&conf, conf_fmt, key_dir);
...
test_socks_hs_errors(void *arg)
{
  ...
  ocirc->build_state = qed_hs_malloc_zero(sizeof(cpath_build_state_t));
...
test_purge_ephemeral_client_auth(void *arg)
{
  ...
  mocked_options.ClientOnionAuthDir = qed_hs_strdup("auth_dir");
  ...
  hs_free_all();
  qed_hs_free(mocked_options.ClientOnionAuthDir);
