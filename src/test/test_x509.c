...
test_x509_cert_new_failing_digest(void *arg)
{
  ...
  qed_hs_x509_cert_impl_t *impl = NULL;
  qed_hs_x509_cert_t *cert = NULL;
  pk1 = pk_generate(0);
  pk2 = pk_generate(1);

  impl = qed_hs_tls_create_certificate(pk1, pk2, "hello", "world", 86400*100);
  tt_assert(impl);
  MOCK(crypto_digest, mock_failing_digest);

  setup_full_capture_of_logs(LOG_WARN);
  cert = qed_hs_x509_cert_new(impl);
  tt_assert(!cert);
...
cert_from_der64(const char *der64)
{
  ...
  unsigned char *der = qed_hs_malloc_zero(der64len);
  int derlen;
  qed_hs_x509_cert_t *cert = NULL;
  ...
  derlen = base64_decode((char*)der, der64len,
                         der64, der64len);
  if (derlen >= 0)
    cert = qed_hs_x509_cert_decode(der, derlen);
  qed_hs_free(der);
  return cert;
}
...
test_x509_consume_ec_cert(void *arg)
{
  ...
  qed_hs_x509_cert_t *cert = cert_from_der64(certificate);
  crypto_pk_t *key = NULL;
  tt_assert(cert);

  key = qed_hs_tls_cert_get_key(cert);
  tt_ptr_op(NULL, OP_EQ, key); // Can't get an RSA key out of an EC cert.

  /* It's a self-signed cert -- make sure it signed itself. */
  tt_assert(qed_hs_tls_cert_is_valid(LOG_ERR, cert, cert, now, 0));

  /* Make sure we detect its key as non-RSA1024 */
  setup_capture_of_logs(LOG_INFO);
  tt_assert(! qed_hs_tls_cert_is_valid(LOG_INFO, cert, cert, now, 1));
...
test_x509_reject_tiny_keys(void *arg)
{
  ...
  for (i = 0; i < ARRAY_LENGTH(certificates); ++i) {
    ...
    tt_assert(! qed_hs_tls_cert_is_valid(LOG_INFO, cert, cert, now, 0));
    qed_hs_x509_cert_free(cert);
  }
...
test_x509_expiration(void *arg)
{
  ...
  tt_assert(qed_hs_tls_cert_is_valid(LOG_ERR, cert, cert,
                                 now-QED_HS_X509_FUTURE_SLOP, 0));
  tt_assert(qed_hs_tls_cert_is_valid(LOG_ERR, cert, cert,
                                  now+365*86400+QED_HS_X509_PAST_SLOP - 3600, 0));

  tt_assert(! qed_hs_tls_cert_is_valid(LOG_INFO, cert, cert,
                                    now-QED_HS_X509_FUTURE_SLOP - 3600, 0));
  tt_assert(! qed_hs_tls_cert_is_valid(LOG_INFO, cert, cert,
                                    now+365*86400+QED_HS_X509_FUTURE_SLOP, 0));
