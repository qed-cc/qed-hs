...
#define QED_HS_CONGESTION_CONTROL_COMMON_PRIVATE
#define QED_HS_CONGESTION_CONTROL_PRIVATE
...
#define MONOTIME_MOCK_START   (monotime_absolute_nsec()+\
                               QED_HS_NSEC_PER_USEC*QED_HS_USEC_PER_SEC)
...
{
  curr_mocked_time += msec_update*QED_HS_NSEC_PER_MSEC;
...
{
  circ_pair_t *pair = qed_hs_malloc_zero(sizeof(circ_pair_t));
  pair->client = client;
  pair->exit = exit;
                                   ...
                                   int flags)
{
  ...
  circ->base_.conflux_pending_nonce = qed_hs_memdup(conflux_nonce, DIGEST256_LEN);
  ...
  circ->cpath->prev->ccontrol = qed_hs_malloc_zero(sizeof(congestion_control_t));
                           ...
                           const char *filename, int lineno)
{
  ...
  cell_delivery_t *delivery = qed_hs_malloc_zero(sizeof(cell_delivery_t));
  delivery->circ = dest_circ;
  delivery->cell = qed_hs_memdup(cell, sizeof(cell_t));
...
process_mock_cell_delivery(void)
{
  ...
  case RELAY_COMMAND_CONFLUX_LINK:
    qed_hs_assert(!CIRCUIT_IS_ORIGIN(dest_circ));
...
static int
channel_get_addr_if_possible_mock(const channel_t *chan, qed_hs_addr_t *addr_out)
{
  (void)chan;
  tt_int_op(AF_INET,OP_EQ, qed_hs_addr_parse(addr_out, "18.0.0.1"));
  return 1;
...
simulate_single_hop_extend(origin_circuit_t *client, int exit)
{
  ...
  curr_mocked_time += 10 * QED_HS_NSEC_PER_MSEC;
  monotime_coarse_set_mock_time_nsec(curr_mocked_time);
  monotime_set_mock_time_nsec(curr_mocked_time);

  // Add a hop to cpath
  crypt_path_t *hop = qed_hs_malloc_zero(sizeof(crypt_path_t));
...
test_clear_circs(void)
{
  ...
  qed_hs_assert(smartlist_len(mock_cell_delivery) == 0);
...
simulate_circuit_build(circuit_t *client_circ)
{
  ...
  relay_side->ccontrol = qed_hs_malloc_zero(sizeof(congestion_control_t));
...
simulate_close_retry(circuit_t *close, bool manual_launch)
{
  ...
  if (manual_launch) {
    nonce = qed_hs_memdup(close->conflux->nonce, DIGEST256_LEN);
  }
  ...
  //qed_hs_assert(digest256map_size(get_unlinked_pool(true)) != 0);

  // Find these legs in our circuit lists, and free them
  qed_hs_assert(CIRCUIT_IS_ORIGIN(close));
  qed_hs_assert(!CIRCUIT_IS_ORIGIN(dest));
  SMARTLIST_FOREACH_BEGIN(client_circs, circuit_t *, client_side) {
...
test_conflux_switch(void *arg)
{
  ...
  for (int i = 0; i < 500; i++) {
    ...
    qed_hs_assert(client3);
    qed_hs_assert(client3->conflux);
    qed_hs_assert(client3->conflux->legs);
    ...
    qed_hs_assert(circ_close);
    qed_hs_assert(circ_close->conflux);
    qed_hs_assert(circ_close->conflux->legs);
