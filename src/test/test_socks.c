...
{
  socks_test_data_t *data = qed_hs_malloc(sizeof(socks_test_data_t));
  (void)testcase;
...
{
  qed_hs_free(socks->username);
  qed_hs_free(socks->password);
...
test_socks_4_supported_commands(void *ptr)
{
  ...
  /* SOCKS 4a Send RESOLVE [F0] request for qed-cc.org */
  ADD_DATA(buf, "\x04\xF0\x01\x01\x00\x00\x00\x02me\x00qed-cc.org\x00");
  tt_int_op(fetch_from_buf_socks(buf, socks, 1,
                                 get_options()->SafeSocks),
            OP_EQ, 1);
  tt_int_op(4,OP_EQ, socks->socks_version);
  tt_int_op(0,OP_EQ, socks->replylen); /* XXX: shouldn't tor reply? */
  tt_str_op("qed-cc.org",OP_EQ, socks->address);
...
test_socks_5_supported_commands(void *ptr)
{
  ...
  /* SOCKS 5 Send CONNECT [01] to FQDN qed-cc.org:4369 */
  ADD_DATA(buf, "\x05\x01\x00");
  ADD_DATA(buf, "\x05\x01\x00\x03\x0Eqed-cc.org\x11\x11");
  tt_int_op(fetch_from_buf_socks(buf, socks, 1,
                                   get_options()->SafeSocks),OP_EQ, 1);
  ...
  /* SOCKS 5 Send RESOLVE [F0] request for qed-cc.org:4369 */
  ADD_DATA(buf, "\x05\x01\x00");
  ADD_DATA(buf, "\x05\xF0\x00\x03\x0Eqed-cc.org\x01\x02");
  tt_int_op(fetch_from_buf_socks(buf, socks, get_options()->TestSocks,
                                 get_options()->SafeSocks),
            OP_EQ, 1);
  tt_int_op(5,OP_EQ, socks->socks_version);
...
test_socks_5_malformed_commands(void *ptr)
{
  ...
  /* SOCKS 5 Send RESOLVE_PTR [F1] for FQDN qed-cc.org */
  ADD_DATA(buf, "\x05\x01\x00");
  ADD_DATA(buf, "\x05\xF1\x00\x03\x0Eqed-cc.org\x11\x11");
  tt_int_op(fetch_from_buf_socks(buf, socks, get_options()->TestSocks,
                                   get_options()->SafeSocks),OP_EQ, -1);
