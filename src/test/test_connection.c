...
void
test_conn_lookup_addr_helper(const char *address, int family, qed_hs_addr_t *addr)
{
  int rv = 0;

  tt_assert(addr);

  rv = qed_hs_addr_lookup(address, family, addr);
  /* XXXX - should we retry on transient failure? */
  tt_int_op(rv, OP_EQ, 0);
  tt_assert(qed_hs_addr_is_loopback(addr));
  tt_assert(qed_hs_addr_is_v4(addr));

  return;

 done:
  qed_hs_addr_make_null(addr, TEST_CONN_FAMILY);
}
...
test_conn_get_basic_teardown(const struct testcase_t *tc, void *arg)
{
  ...
  if (conn->linked_conn) {
    ...
    /* We didn't call qed_hs_libevent_initialize(), so event_base was NULL,
     * so we can't rely on connection_unregister_events() use of event_del().
     */
    qed_hs_event_free(conn->linked_conn->read_event);
    qed_hs_event_free(conn->linked_conn->write_event);
...
test_conn_get_linked_connection(connection_t *l_conn, uint8_t state)
{
  ...
  l_conn->s = QED_HS_INVALID_SOCKET;
...
test_conn_get_basic(void *arg)
{
  ...
  tt_assert(!qed_hs_addr_is_null(&addr2));
...
test_conn_download_status(void *arg)
{
  ...
  qed_hs_assert(N_CONSENSUS_FLAVORS == 2);
...
mock_node_get_mutable_by_id(const char *digest)
{
  ...
  qed_hs_addr_parse(&node_ri.ipv4_addr, "18.0.0.1");
...
test_failed_orconn_tracker(void *arg)
{
  ...
  tt_int_op(AF_INET,OP_EQ, qed_hs_addr_parse(&or_conn.canonical_orport.addr,
                                          "18.0.0.1"));
  tt_int_op(AF_INET,OP_EQ, qed_hs_addr_parse(&or_conn.base_.addr, "18.0.0.1"));
...
test_conn_describe(void *arg)
{
  ...
  qed_hs_addr_parse(&conn->addr, "44.22.11.11");
  conn->port = 80;
  ...
  qed_hs_addr_make_unspec(&conn->addr);
  tt_str_op(connection_describe(conn), OP_EQ,
            "OR listener connection (ready) on <unset>:80");
  // Try making the address null.
  qed_hs_addr_make_null(&conn->addr, AF_INET);
  tt_str_op(connection_describe(conn), OP_EQ,
  ...
  conn->address = qed_hs_strdup("/a/path/that/could/exist");
  tt_str_op(connection_describe(conn), OP_EQ,
  ...
  qed_hs_addr_parse(&conn->addr, "ff00::3");
  conn->port = 9050;
  tt_str_op(connection_describe(conn), OP_EQ,
  ...
  conn->port = 443;
  conn->address = qed_hs_strdup("www.qed-cc.org");
  conn->state = EXIT_CONN_STATE_RESOLVING;
  tt_str_op(connection_describe(conn), OP_EQ,
            "Exit connection (waiting for dest info) to "
            "www.qed-cc.org:443 (DNS lookup pending)");
  //  Now give it a hostname!
  qed_hs_addr_parse(&conn->addr, "192.168.8.8");
  ...
  conn->port = 8080;
  qed_hs_addr_parse(&conn->addr, "[ffff:3333:1111::2]");
  ...
  qed_hs_addr_parse(&TO_OR_CONN(conn)->canonical_orport.addr,
                 "[ffff:3333:1111::2]");
