...
#define RAND_PATH_BYTES 5
...
#ifdef _WIN32
  {
    ...
    qed_hs_snprintf(temp_dir, sizeof(temp_dir),
                 "%s%sqed_hs_test_%d_%s", tmp, extra_backslash,
                 (int)getpid(), rnd32);
    r = mkdir(temp_dir);
  }
  ...
  qed_hs_snprintf(temp_dir, sizeof(temp_dir),
               "/data/local/tmp/qed_hs_%d_%d_%s",
...
#else /* !defined(_WIN32) */
  qed_hs_snprintf(temp_dir, sizeof(temp_dir), "/tmp/qed_hs_test_%d_%s",
               (int) getpid(), rnd32);
  r = mkdir(temp_dir, 0700);
  if (!r) {
...
get_fname_suffix(const char *name, const char *suffix)
{
  ...
  if (!name)
    return temp_dir;
  qed_hs_snprintf(buf,sizeof(buf),"%s%s%s%s%s", temp_dir, PATH_SEPARATOR, name,
...
rm_rf(const char *dir)
{
  ...
  elements = qed_hs_listdir(dir);
  if (elements) {
    SMARTLIST_FOREACH_BEGIN(elements, const char *, cp) {
         char *tmp = NULL;
         qed_hs_asprintf(&tmp, "%s"PATH_SEPARATOR"%s", dir, cp);
...
main(int c, const char **v)
{
  ...
  struct qed_hs_libevent_cfg_t cfg;
  ...
  options->DataDirectory = qed_hs_strdup(temp_dir);
  options->DataDirectory_option = qed_hs_strdup(temp_dir);
  qed_hs_asprintf(&options->KeyDirectory, "%s"PATH_SEPARATOR"keys",
               options->DataDirectory);
  options->CacheDirectory = qed_hs_strdup(temp_dir);
  ...
  qed_hs_set_failed_assertion_callback(an_assertion_failed);
  ...
  /* Look for QED_HS_SKIP_TESTCASES: a space-separated list of tests to skip. */
  const char *skip_tests = getenv("QED_HS_SKIP_TESTCASES");
  if (skip_tests) {
