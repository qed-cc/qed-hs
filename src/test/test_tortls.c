...
// Tor www.qed-cc.org certificate.
...
static qed_hs_x509_cert_t *fixed_x509_cert = NULL;
static qed_hs_x509_cert_t *
get_peer_cert_mock_return_fixed(qed_hs_tls_t *tls)
{
  (void)tls;
  if (fixed_x509_cert)
    return qed_hs_x509_cert_dup(fixed_x509_cert);
  else
    return NULL;
}
...
read_cert_from(const char *str)
{
  ...
  uint8_t *raw_cert = qed_hs_malloc(len);
  ssize_t true_len = pem_decode(raw_cert, len, str, len, "CERTIFICATE");
  if (true_len < 0) {
    qed_hs_free(raw_cert);
    return NULL;
  }
  qed_hs_x509_cert_t *cert = qed_hs_x509_cert_decode(raw_cert, true_len);
  qed_hs_free(raw_cert);
  if (! cert) {
    return NULL;
  }
  qed_hs_x509_cert_impl_t *res = qed_hs_x509_cert_impl_dup_(cert->cert);
...
{
  (void) data;
  tt_int_op(qed_hs_errno_to_tls_error(SOCK_ERRNO(ECONNRESET)),OP_EQ,
            QED_HS_TLS_ERROR_CONNRESET);
  tt_int_op(qed_hs_errno_to_tls_error(SOCK_ERRNO(ETIMEDOUT)),OP_EQ,
            QED_HS_TLS_ERROR_TIMEOUT);
  tt_int_op(qed_hs_errno_to_tls_error(SOCK_ERRNO(EHOSTUNREACH)),OP_EQ,
            QED_HS_TLS_ERROR_NO_ROUTE);
  tt_int_op(qed_hs_errno_to_tls_error(SOCK_ERRNO(ENETUNREACH)),OP_EQ,
            QED_HS_TLS_ERROR_NO_ROUTE);
  tt_int_op(qed_hs_errno_to_tls_error(SOCK_ERRNO(ECONNREFUSED)),OP_EQ,
            QED_HS_TLS_ERROR_CONNREFUSED);
  tt_int_op(qed_hs_errno_to_tls_error(0),OP_EQ,QED_HS_TLS_ERROR_MISC);
 done:
  (void)1;
}
...
{
  (void) data;
  tt_str_op(qed_hs_tls_err_to_string(1),OP_EQ,"[Not an error.]");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_ERROR_MISC),OP_EQ,"misc error");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_ERROR_IO),OP_EQ,"unexpected close");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_ERROR_CONNREFUSED),OP_EQ,
            "connection refused");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_ERROR_CONNRESET),OP_EQ,
            "connection reset");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_ERROR_NO_ROUTE),OP_EQ,
            "host unreachable");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_ERROR_TIMEOUT),OP_EQ,
            "connection timed out");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_CLOSE),OP_EQ,"closed");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_WANTREAD),OP_EQ,"want to read");
  tt_str_op(qed_hs_tls_err_to_string(QED_HS_TLS_WANTWRITE),OP_EQ,"want to write");
  tt_str_op(qed_hs_tls_err_to_string(-100),OP_EQ,"(unknown error code)");
 done:
  (void)1;
}
...
static void
test_tortls_qed_hs_tls_get_error(void *data)
{
  (void) data;
...
test_tortls_x509_cert_get_id_digests(void *ignored)
{
  ...
  d = qed_hs_malloc_zero(sizeof(common_digests_t));
  d->d[0][0] = 42;

  res = qed_hs_x509_cert_get_id_digests(cert);
  tt_assert(!res);
...
test_tortls_get_my_certs(void *ignored)
{
  (void)ignored;
  int ret;
  qed_hs_tls_context_t *ctx;
  const qed_hs_x509_cert_t *link_cert_out = NULL;
  const qed_hs_x509_cert_t *id_cert_out = NULL;

  ctx = qed_hs_malloc_zero(sizeof(qed_hs_tls_context_t));

  client_tls_context = NULL;
  ret = qed_hs_tls_get_my_certs(0, NULL, NULL);
  tt_int_op(ret, OP_EQ, -1);

  server_tls_context = NULL;
  ret = qed_hs_tls_get_my_certs(1, NULL, NULL);
  tt_int_op(ret, OP_EQ, -1);
  ...
  client_tls_context = ctx;
  ret = qed_hs_tls_get_my_certs(0, &link_cert_out, &id_cert_out);
  tt_int_op(ret, OP_EQ, 0);

  server_tls_context = ctx;
  ret = qed_hs_tls_get_my_certs(1, &link_cert_out, &id_cert_out);
  tt_int_op(ret, OP_EQ, 0);
...
test_tortls_get_forced_write_size(void *ignored)
{
  ...
  tls->wantwrite_n = 43;
  ret = qed_hs_tls_get_forced_write_size(tls);
...
{
  (void) arg;
  /* If we call qed_hs_tls_context_init() a second time, nothing should go
   * wrong.
   */
  ...
  int r = qed_hs_tls_context_init(QED_HS_TLS_CTX_IS_PUBLIC_SERVER,
                               pk1, pk2, 86400);
  tt_int_op(r, OP_EQ, 0);

  r = qed_hs_tls_context_init(QED_HS_TLS_CTX_IS_PUBLIC_SERVER,
                               pk2, pk1, 86400);
  tt_int_op(r, OP_EQ, 0);
  /* For a public server context, these are the same */
  tt_ptr_op(qed_hs_tls_context_get(0), OP_EQ, qed_hs_tls_context_get(1));
...
test_tortls_bridge_init(void *arg)
{
  ...
  /* If we pass in a server identity key but not the
     QED_HS_TLS_CTX_IS_PUBLIC_SERVER flag, we should get a bridge-style
     configuration, with two distinct contexts. */
  int r = qed_hs_tls_context_init(0 /* flags */, pk1, pk2, 86400);

  tt_int_op(r, OP_EQ, 0);
  tt_ptr_op(qed_hs_tls_context_get(0), OP_NE, qed_hs_tls_context_get(1));
 done:
  crypto_pk_free(pk1);
  crypto_pk_free(pk2);
}
...
test_tortls_address(void *arg)
{
  ...
  qed_hs_tls_set_logged_address(tls, "zombo.com");

  /* This write should fail, since the fd is -1. */
  setup_capture_of_logs(LOG_INFO);
  int n = qed_hs_tls_write(tls, "welcome", 7);
  tt_int_op(n, OP_LT, 0);
...
test_tortls_is_server(void *arg)
{
  (void)arg;
  crypto_pk_t *pk1=NULL, *pk2=NULL;
  qed_hs_tls_t *tls1=NULL, *tls2=NULL;
  pk1 = pk_generate(2);
  pk2 = pk_generate(0);
  ...
  int r = qed_hs_tls_context_init(QED_HS_TLS_CTX_IS_PUBLIC_SERVER,
                               pk1, pk2, 86400);
  tt_int_op(r, OP_EQ, 0);
  tls1 = qed_hs_tls_new(-1, 0);
  tls2 = qed_hs_tls_new(-1, 1);

  tt_assert(! qed_hs_tls_is_server(tls1));
  tt_assert(qed_hs_tls_is_server(tls2));

 done:
  qed_hs_tls_free(tls1);
  qed_hs_tls_free(tls2);
  crypto_pk_free(pk1);
  crypto_pk_free(pk2);
}
...
test_tortls_cert_matches_key(void *ignored)
{
  ...
  qed_hs_x509_cert_impl_t *cert1 = NULL,
                       *cert2 = NULL,
                       *cert3 = NULL,
                       *cert4 = NULL;
  qed_hs_x509_cert_t *c1 = NULL, *c2 = NULL, *c3 = NULL, *c4 = NULL;
  ...
  cert1 = qed_hs_tls_create_certificate(k1, k2, "A", "B", 1000);
  cert2 = qed_hs_tls_create_certificate(k1, k3, "C", "D", 1000);
  cert3 = qed_hs_tls_create_certificate(k2, k3, "C", "D", 1000);
  cert4 = qed_hs_tls_create_certificate(k3, k2, "E", "F", 1000);

  tt_assert(cert1 && cert2 && cert3 && cert4);

  c1 = qed_hs_x509_cert_new(cert1); cert1 = NULL;
  c2 = qed_hs_x509_cert_new(cert2); cert2 = NULL;
  c3 = qed_hs_x509_cert_new(cert3); cert3 = NULL;
  c4 = qed_hs_x509_cert_new(cert4); cert4 = NULL;

  tt_assert(c1 && c2 && c3 && c4);

  MOCK(qed_hs_tls_get_peer_cert, get_peer_cert_mock_return_fixed);

  fixed_x509_cert = NULL;
  /* If the peer has no certificate, it shouldn't match anything. */
  tt_assert(! qed_hs_tls_cert_matches_key(NULL, c1));
  tt_assert(! qed_hs_tls_cert_matches_key(NULL, c2));
  tt_assert(! qed_hs_tls_cert_matches_key(NULL, c3));
  tt_assert(! qed_hs_tls_cert_matches_key(NULL, c4));
  fixed_x509_cert = c1;
  ...
  qed_hs_x509_cert_free(c1);
  qed_hs_x509_cert_free(c2);
  qed_hs_x509_cert_free(c3);
  qed_hs_x509_cert_free(c4);
  if (cert1) qed_hs_x509_cert_impl_free(cert1);
  if (cert2) qed_hs_x509_cert_impl_free(cert2);
  if (cert3) qed_hs_x509_cert_impl_free(cert3);
  if (cert4) qed_hs_x509_cert_impl_free(cert4);
  crypto_pk_free(k1);
  crypto_pk_free(k2);
  crypto_pk_free(k3);
  UNMOCK(qed_hs_tls_get_peer_cert);
}
...
#ifdef ENABLE_OPENSSL
  LOCAL_TEST_CASE(qed_hs_tls_get_error, 0),
