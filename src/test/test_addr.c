...
test_addr_basic(void *arg)
{
  ...
  /* Test inet_ntop */
  {
    char tmpbuf[QED_HS_ADDR_BUF_LEN];
    const char *ip = "176.192.208.224";
    struct in_addr in;

    /* good round trip */
    tt_int_op(qed_hs_inet_pton(AF_INET, ip, &in), OP_EQ, 1);
    tt_ptr_op(qed_hs_inet_ntop(AF_INET, &in, tmpbuf, sizeof(tmpbuf)),
              OP_EQ, &tmpbuf);
    tt_str_op(tmpbuf,OP_EQ, ip);

    /* just enough buffer length */
    tt_str_op(qed_hs_inet_ntop(AF_INET, &in, tmpbuf, strlen(ip) + 1), OP_EQ, ip);

    /* too short buffer */
    tt_ptr_op(qed_hs_inet_ntop(AF_INET, &in, tmpbuf, strlen(ip)),OP_EQ, NULL);
  }
...
#define test_op_ip6_(a,op,b,e1,e2)                               \
  ...
  tt_assert_test_fmt_type(a,b,e1" "#op" "e2,struct in6_addr*,    \
    ...
    char *, "%s",                                                \
    { char *cp;                                                  \
      cp = print_ = qed_hs_malloc(64);                              \
      for (int ii_=0;ii_<16;++ii_) {                             \
        qed_hs_snprintf(cp, 3,"%02x", (unsigned)value_->s6_addr[ii_]);     \
        cp += 2;                                                 \
        if (ii_ != 15) *cp++ = ':';                              \
      }                                                          \
    },                                                           \
 ...
 * qed_hs_inet_pton(), and both decode to the same address. */
#define test_pton6_same(a,b) STMT_BEGIN                 \
     tt_int_op(qed_hs_inet_pton(AF_INET6, a, &a1), OP_EQ, 1); \
     tt_int_op(qed_hs_inet_pton(AF_INET6, b, &a2), OP_EQ, 1); \
...
#define test_pton6_bad(a)                       \
  tt_int_op(0, OP_EQ, qed_hs_inet_pton(AF_INET6, a, &a1))

/** Helper: assert that <b>a</b>, when parsed by qed_hs_inet_pton() and displayed
 * with qed_hs_inet_ntop(), yields <b>b</b>. Also assert that <b>b</b> parses to
 * the same value as <b>a</b>. */
#define test_ntop6_reduces(a,b) STMT_BEGIN                          \
  tt_int_op(qed_hs_inet_pton(AF_INET6, a, &a1), OP_EQ, 1);                \
  tt_str_op(qed_hs_inet_ntop(AF_INET6, &a1, buf, sizeof(buf)), OP_EQ, b); \
  tt_int_op(qed_hs_inet_pton(AF_INET6, b, &a2), OP_EQ, 1);     \
  test_op_ip6_(&a1, OP_EQ, &a2, a, b);                      \
  STMT_END

/** Helper: assert that <b>a</b> parses by qed_hs_inet_pton() into a address that
 * passes qed_hs_addr_is_internal() with <b>for_listening</b>. */
#define test_internal_ip(a,for_listening) STMT_BEGIN           \
    tt_int_op(qed_hs_inet_pton(AF_INET6, a, &t1.addr.in6_addr), OP_EQ, 1); \
    t1.family = AF_INET6;                                      \
    if (!qed_hs_addr_is_internal(&t1, for_listening))             \
      TT_DIE(("%s was not internal", a));                      \
  STMT_END

/** Helper: assert that <b>a</b> parses by qed_hs_inet_pton() into a address that
 * does not pass qed_hs_addr_is_internal() with <b>for_listening</b>. */
 ...
 * qed_hs_inet_pton(), give addresses that compare in the order defined by
 * <b>op</b> with qed_hs_addr_compare(). */
#define test_addr_compare(a, op, b) STMT_BEGIN                    \
    tt_int_op(qed_hs_inet_pton(AF_INET6, a, &t1.addr.in6_addr), OP_EQ, 1); \
    tt_int_op(qed_hs_inet_pton(AF_INET6, b, &t2.addr.in6_addr), OP_EQ, 1); \
    t1.family = t2.family = AF_INET6;                             \
    r = qed_hs_addr_compare(&t1,&t2,CMP_SEMANTIC);                   \
    if (!(r op 0))                                                \
      TT_DIE(("Failed: qed_hs_addr_compare(%s,%s) %s 0", a, b, #op));\
  STMT_END

/** Helper: Assert that <b>a</b> and <b>b</b>, when parsed by
 * qed_hs_inet_pton(), give addresses that compare in the order defined by
 * <b>op</b> with qed_hs_addr_compare_masked() with <b>m</b> masked. */
#define test_addr_compare_masked(a, op, b, m) STMT_BEGIN          \
    tt_int_op(qed_hs_inet_pton(AF_INET6, a, &t1.addr.in6_addr), OP_EQ, 1);    \
    tt_int_op(qed_hs_inet_pton(AF_INET6, b, &t2.addr.in6_addr), OP_EQ, 1);    \
    t1.family = t2.family = AF_INET6;                             \
    r = qed_hs_addr_compare_masked(&t1,&t2,m,CMP_SEMANTIC);          \
    if (!(r op 0))                                                \
      TT_DIE(("Failed: qed_hs_addr_compare_masked(%s,%s,%d) %s 0", \
              a, b, m, #op));                                   \
  STMT_END
#endif /* !defined(COCCI) */

/** Helper: assert that <b>xx</b> is parseable as a masked IPv6 address with
 * ports by qed_hs_parse_mask_addr_ports(), with family <b>f</b>, IP address
...
#define test_addr_mask_ports_parse(xx, f, ip1, ip2, ip3, ip4, mm, pt1, pt2) \
  STMT_BEGIN                                                                \
    tt_int_op(qed_hs_addr_parse_mask_ports(xx, 0, &t1, &mask, &port1, &port2),   \
              OP_EQ, f);                                                   \
    p1=qed_hs_inet_ntop(AF_INET6, &t1.addr.in6_addr, bug, sizeof(bug));        \
    tt_int_op(htonl(ip1), OP_EQ, qed_hs_addr_to_in6_addr32(&t1)[0]);            \
    tt_int_op(htonl(ip2), OP_EQ, qed_hs_addr_to_in6_addr32(&t1)[1]);            \
    tt_int_op(htonl(ip3), OP_EQ, qed_hs_addr_to_in6_addr32(&t1)[2]);            \
    tt_int_op(htonl(ip4), OP_EQ, qed_hs_addr_to_in6_addr32(&t1)[3]);            \
...
{
  char buf[QED_HS_ADDR_BUF_LEN], bug[QED_HS_ADDR_BUF_LEN];
  ...
  /* Test qed_hs_inet_ntop and qed_hs_inet_pton: IPv6 */
  (void)arg;
  {
    ...
    /* good round trip */
    tt_int_op(qed_hs_inet_pton(AF_INET6, ip, &a1),OP_EQ, 1);
    tt_ptr_op(qed_hs_inet_ntop(AF_INET6, &a1, buf, sizeof(buf)),OP_EQ, &buf);
    tt_str_op(buf,OP_EQ, ip);

    /* good round trip - ::ffff:0:0 style */
    tt_int_op(qed_hs_inet_pton(AF_INET6, ip_ffff, &a2),OP_EQ, 1);
    tt_ptr_op(qed_hs_inet_ntop(AF_INET6, &a2, buf, sizeof(buf)),OP_EQ, &buf);
    tt_str_op(buf,OP_EQ, ip_ffff);

    /* just long enough buffer (remember \0) */
    tt_str_op(qed_hs_inet_ntop(AF_INET6, &a1, buf, strlen(ip)+1),OP_EQ, ip);
    tt_str_op(qed_hs_inet_ntop(AF_INET6, &a2, buf, strlen(ip_ffff)+1),OP_EQ,
               ip_ffff);

    /* too short buffer (remember \0) */
    tt_ptr_op(qed_hs_inet_ntop(AF_INET6, &a1, buf, strlen(ip)),OP_EQ, NULL);
    tt_ptr_op(qed_hs_inet_ntop(AF_INET6, &a2, buf, strlen(ip_ffff)),OP_EQ, NULL);
  }
  ...
  qed_hs_addr_from_sockaddr(&t1, (struct sockaddr *)sin, &port1);
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ, AF_INET);
  tt_int_op(qed_hs_addr_to_ipv4h(&t1),OP_EQ, 0x7f7f0102);
  tt_int_op(port1, OP_EQ, 9090);

  memset(&sa_storage, 0, sizeof(sa_storage));
  tt_int_op(sizeof(struct sockaddr_in),OP_EQ,
          qed_hs_addr_to_sockaddr(&t1, 1234, (struct sockaddr *)&sa_storage,
                               sizeof(sa_storage)));
  ...
  qed_hs_addr_from_sockaddr(&t1, (struct sockaddr *)sin6, &port1);
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ, AF_INET6);
  tt_int_op(port1, OP_EQ, 7070);
  p1 = qed_hs_addr_to_str(buf, &t1, sizeof(buf), 0);
  tt_str_op(p1,OP_EQ, "8000::");

  memset(&sa_storage, 0, sizeof(sa_storage));
  tt_int_op(sizeof(struct sockaddr_in6),OP_EQ,
          qed_hs_addr_to_sockaddr(&t1, 9999, (struct sockaddr *)&sa_storage,
                               sizeof(sa_storage)));
  ...
  /* ==== qed_hs_addr_lookup: static cases.  (Can't test dns without knowing we
   * have a good resolver. */
  tt_int_op(0,OP_EQ, qed_hs_addr_lookup("127.128.129.130", AF_UNSPEC, &t1));
  tt_int_op(AF_INET,OP_EQ, qed_hs_addr_family(&t1));
  tt_int_op(qed_hs_addr_to_ipv4h(&t1),OP_EQ, 0x7f808182);

  tt_int_op(0,OP_EQ, qed_hs_addr_lookup("9000::5", AF_UNSPEC, &t1));
  tt_int_op(AF_INET6,OP_EQ, qed_hs_addr_family(&t1));
  tt_int_op(0x90,OP_EQ, qed_hs_addr_to_in6_addr8(&t1)[0]);
  tt_assert(fast_mem_is_zero((char*)qed_hs_addr_to_in6_addr8(&t1)+1, 14));
  tt_int_op(0x05,OP_EQ, qed_hs_addr_to_in6_addr8(&t1)[15]);

  /* === Test pton: valid af_inet6 */
  /* Simple, valid parsing. */
  r = qed_hs_inet_pton(AF_INET6,
  ...
  /* Bad af param */
  tt_int_op(qed_hs_inet_pton(AF_UNSPEC, 0, 0),OP_EQ, -1);
  ...
  /* qed_hs_addr_compare(qed_hs_addr_t x2) */
  ...
  qed_hs_addr_parse_mask_ports("[::ffff:2.3.4.5]", 0, &t1, NULL, NULL, NULL);
  qed_hs_addr_parse_mask_ports("2.3.4.5", 0, &t2, NULL, NULL, NULL);
  tt_int_op(qed_hs_addr_compare(&t1, &t2, CMP_SEMANTIC), OP_EQ, 0);
  qed_hs_addr_parse_mask_ports("[::ffff:2.3.4.4]", 0, &t1, NULL, NULL, NULL);
  qed_hs_addr_parse_mask_ports("2.3.4.5", 0, &t2, NULL, NULL, NULL);
  tt_int_op(qed_hs_addr_compare(&t1, &t2, CMP_SEMANTIC), OP_LT, 0);
  ...
  /* Test undecorated qed_hs_addr_to_str */
  tt_int_op(AF_INET6,OP_EQ, qed_hs_addr_parse(&t1, "[123:45:6789::5005:11]"));
  p1 = qed_hs_addr_to_str(buf, &t1, sizeof(buf), 0);
  tt_str_op(p1,OP_EQ, "123:45:6789::5005:11");
  tt_int_op(AF_INET,OP_EQ, qed_hs_addr_parse(&t1, "18.0.0.1"));
  p1 = qed_hs_addr_to_str(buf, &t1, sizeof(buf), 0);
  tt_str_op(p1,OP_EQ, "18.0.0.1");

  /* Test decorated qed_hs_addr_to_str */
  tt_int_op(AF_INET6,OP_EQ, qed_hs_addr_parse(&t1, "[123:45:6789::5005:11]"));
  p1 = qed_hs_addr_to_str(buf, &t1, sizeof(buf), 1);
  tt_str_op(p1,OP_EQ, "[123:45:6789::5005:11]");
  tt_int_op(AF_INET,OP_EQ, qed_hs_addr_parse(&t1, "18.0.0.1"));
  p1 = qed_hs_addr_to_str(buf, &t1, sizeof(buf), 1);
  tt_str_op(p1,OP_EQ, "18.0.0.1");

  /* Test buffer bounds checking of qed_hs_addr_to_str */
  tt_int_op(AF_INET6,OP_EQ, qed_hs_addr_parse(&t1, "::")); /* 2 + \0 */
  tt_ptr_op(qed_hs_addr_to_str(buf, &t1, 2, 0),OP_EQ, NULL); /* too short buf */
  tt_str_op(qed_hs_addr_to_str(buf, &t1, 3, 0),OP_EQ, "::");
  tt_ptr_op(qed_hs_addr_to_str(buf, &t1, 4, 1),OP_EQ, NULL); /* too short buf */
  tt_str_op(qed_hs_addr_to_str(buf, &t1, 5, 1),OP_EQ, "[::]");

  tt_int_op(AF_INET6,OP_EQ, qed_hs_addr_parse(&t1, "2000::1337")); /* 10 + \0 */
  tt_ptr_op(qed_hs_addr_to_str(buf, &t1, 10, 0),OP_EQ, NULL); /* too short buf */
  tt_str_op(qed_hs_addr_to_str(buf, &t1, 11, 0),OP_EQ, "2000::1337");
  tt_ptr_op(qed_hs_addr_to_str(buf, &t1, 12, 1),OP_EQ, NULL); /* too short buf */
  tt_str_op(qed_hs_addr_to_str(buf, &t1, 13, 1),OP_EQ, "[2000::1337]");

  tt_int_op(AF_INET,OP_EQ, qed_hs_addr_parse(&t1, "1.2.3.4")); /* 7 + \0 */
  tt_ptr_op(qed_hs_addr_to_str(buf, &t1, 7, 0),OP_EQ, NULL); /* too short buf */
  tt_str_op(qed_hs_addr_to_str(buf, &t1, 8, 0),OP_EQ, "1.2.3.4");

  tt_int_op(AF_INET, OP_EQ,
            qed_hs_addr_parse(&t1, "255.255.255.255")); /* 15 + \0 */
  tt_ptr_op(qed_hs_addr_to_str(buf, &t1, 15, 0),OP_EQ, NULL); /* too short buf */
  tt_str_op(qed_hs_addr_to_str(buf, &t1, 16, 0),OP_EQ, "255.255.255.255");
  tt_ptr_op(qed_hs_addr_to_str(buf, &t1, 15, 1),OP_EQ, NULL); /* too short buf */
  tt_str_op(qed_hs_addr_to_str(buf, &t1, 16, 1),OP_EQ, "255.255.255.255");

  t1.family = AF_UNSPEC;
  tt_ptr_op(qed_hs_addr_to_str(buf, &t1, sizeof(buf), 0),OP_EQ, NULL);

  /* Test qed_hs_addr_parse_PTR_name */
  i = qed_hs_addr_parse_PTR_name(&t1, "Foobar.baz", AF_UNSPEC, 0);
  tt_int_op(0,OP_EQ, i);
  i = qed_hs_addr_parse_PTR_name(&t1, "Foobar.baz", AF_UNSPEC, 1);
  tt_int_op(0,OP_EQ, i);
  i = qed_hs_addr_parse_PTR_name(&t1, "9999999999999999999999999999.in-addr.arpa",
                              AF_UNSPEC, 1);
  tt_int_op(-1,OP_EQ, i);
  i = qed_hs_addr_parse_PTR_name(&t1, "1.0.168.192.in-addr.arpa",
                                         AF_UNSPEC, 1);
  tt_int_op(1,OP_EQ, i);
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ, AF_INET);
  p1 = qed_hs_addr_to_str(buf, &t1, sizeof(buf), 1);
  tt_str_op(p1,OP_EQ, "192.168.0.1");
  i = qed_hs_addr_parse_PTR_name(&t1, "192.168.0.99", AF_UNSPEC, 0);
  tt_int_op(0,OP_EQ, i);
  i = qed_hs_addr_parse_PTR_name(&t1, "192.168.0.99", AF_UNSPEC, 1);
  tt_int_op(1,OP_EQ, i);
  p1 = qed_hs_addr_to_str(buf, &t1, sizeof(buf), 1);
  tt_str_op(p1,OP_EQ, "192.168.0.99");
  memset(&t1, 0, sizeof(t1));
  i = qed_hs_addr_parse_PTR_name(&t1,
  ...
  i = qed_hs_addr_parse_PTR_name(&t1,
                                         ...
                                         AF_UNSPEC, 0);
  tt_int_op(i,OP_EQ, -1);
  i = qed_hs_addr_parse_PTR_name(&t1, "32.1.1.in-addr.arpa",
                                         AF_UNSPEC, 0);
  tt_int_op(i,OP_EQ, -1);
  i = qed_hs_addr_parse_PTR_name(&t1, ".in-addr.arpa",
                                         AF_UNSPEC, 0);
  tt_int_op(i,OP_EQ, -1);
  i = qed_hs_addr_parse_PTR_name(&t1, "1.2.3.4.5.in-addr.arpa",
                                         AF_UNSPEC, 0);
  tt_int_op(i,OP_EQ, -1);
  i = qed_hs_addr_parse_PTR_name(&t1, "1.2.3.4.5.in-addr.arpa",
                                         AF_INET6, 0);
  ...
  qed_hs_addr_from_sockaddr(&t1, (struct sockaddr *)sin, NULL);

  /* Check IPv4 PTR - too short buffer */
  tt_int_op(qed_hs_addr_to_PTR_name(rbuf, 1, &t1),OP_EQ, -1);
  tt_int_op(qed_hs_addr_to_PTR_name(rbuf,
                               strlen("3.2.1.127.in-addr.arpa") - 1,
                               &t1),OP_EQ, -1);

  /* Check IPv4 PTR - valid addr */
  tt_int_op(qed_hs_addr_to_PTR_name(rbuf, sizeof(rbuf), &t1),OP_EQ,
          strlen("3.2.1.127.in-addr.arpa"));
  tt_str_op(rbuf,OP_EQ, "3.2.1.127.in-addr.arpa");

  /* Invalid addr family */
  t1.family = AF_UNSPEC;
  tt_int_op(qed_hs_addr_to_PTR_name(rbuf, sizeof(rbuf), &t1),OP_EQ, -1);
  ...
  qed_hs_addr_from_sockaddr(&t1, (struct sockaddr *)sin6, NULL);

  {
    ...
    tt_int_op(qed_hs_addr_to_PTR_name(rbuf, 0, &t1),OP_EQ, -1);
    tt_int_op(qed_hs_addr_to_PTR_name(rbuf, strlen(addr_PTR) - 1, &t1),OP_EQ, -1);
  ...
  /* test qed_hs_addr_parse_mask_ports */
  ...
  r=qed_hs_addr_parse_mask_ports("[ffff:1111:1111:1111:1111:1111:1111:1111]",
                              0, &t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, AF_INET6);
  r=qed_hs_addr_parse_mask_ports("[ffff:1111:1111:1111:1111:1111:1111:11111]",
                              0, &t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[ffff:1111:1111:1111:1111:1111:1111:1111:1]",
                              0, &t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports(
  ...
  r=qed_hs_addr_parse_mask_ports("[fefef::]/112", 0, &t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[fefe::/112", 0, &t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[fefe::", 0, &t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[fefe::X]", 0, &t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("efef::/112", 0, &t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[f:f:f:f:f:f:f:f::]",0,&t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[::f:f:f:f:f:f:f:f]",0,&t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[f:f:f:f:f:f:f:f:f]",0,&t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[f:f:f:f:f::]/fred",0,&t1,&mask, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("[f:f:f:f:f::]/255.255.0.0",
                              0,&t1, NULL, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  /* This one will get rejected because it isn't a pure prefix. */
  r=qed_hs_addr_parse_mask_ports("1.1.2.3/255.255.64.0",0,&t1, &mask,NULL,NULL);
  tt_int_op(r, OP_EQ, -1);
  /* Test for V4-mapped address with mask < 96.  (arguably not valid) */
  r=qed_hs_addr_parse_mask_ports("[::ffff:1.1.2.2/33]",0,&t1, &mask, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("1.1.2.2/33",0,&t1, &mask, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  /* Try extended wildcard addresses with out TAPMP_EXTENDED_STAR*/
  r=qed_hs_addr_parse_mask_ports("*4",0,&t1, &mask, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("*6",0,&t1, &mask, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  tt_int_op(r, OP_EQ, -1);
  /* Try a mask with a wildcard. */
  r=qed_hs_addr_parse_mask_ports("*/16",0,&t1, &mask, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("*4/16",TAPMP_EXTENDED_STAR,
                              &t1, &mask, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("*6/30",TAPMP_EXTENDED_STAR,
                              &t1, &mask, NULL, NULL);
  tt_int_op(r, OP_EQ, -1);
  /* Basic mask tests*/
  r=qed_hs_addr_parse_mask_ports("1.1.2.2/31",0,&t1, &mask, NULL, NULL);
  tt_int_op(r, OP_EQ, AF_INET);
  tt_int_op(mask,OP_EQ,31);
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ,AF_INET);
  tt_int_op(qed_hs_addr_to_ipv4h(&t1),OP_EQ,0x01010202);
  r=qed_hs_addr_parse_mask_ports("3.4.16.032:1-2",0,&t1, &mask, &port1, &port2);
  tt_int_op(r, OP_EQ, -1);
  r=qed_hs_addr_parse_mask_ports("1.1.2.3/255.255.128.0",0,&t1, &mask,NULL,NULL);
  tt_int_op(r, OP_EQ, AF_INET);
  tt_int_op(mask,OP_EQ,17);
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ,AF_INET);
  tt_int_op(qed_hs_addr_to_ipv4h(&t1),OP_EQ,0x01010203);
  r=qed_hs_addr_parse_mask_ports("[efef::]/112",0,&t1, &mask, &port1, &port2);
  tt_int_op(r, OP_EQ, AF_INET6);
  tt_uint_op(port1, OP_EQ, 1);
  tt_uint_op(port2, OP_EQ, 65535);
  /* Try regular wildcard behavior without TAPMP_EXTENDED_STAR */
  r=qed_hs_addr_parse_mask_ports("*:80-443",0,&t1,&mask,&port1,&port2);
  tt_int_op(r,OP_EQ,AF_INET); /* Old users of this always get inet */
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ,AF_INET);
  tt_int_op(qed_hs_addr_to_ipv4h(&t1),OP_EQ,0);
  tt_int_op(mask,OP_EQ,0);
  tt_int_op(port1,OP_EQ,80);
  tt_int_op(port2,OP_EQ,443);
  /* Now try wildcards *with* TAPMP_EXTENDED_STAR */
  r=qed_hs_addr_parse_mask_ports("*:8000-9000",TAPMP_EXTENDED_STAR,
                              &t1,&mask,&port1,&port2);
  tt_int_op(r,OP_EQ,AF_UNSPEC);
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ,AF_UNSPEC);
  tt_int_op(mask,OP_EQ,0);
  tt_int_op(port1,OP_EQ,8000);
  tt_int_op(port2,OP_EQ,9000);
  r=qed_hs_addr_parse_mask_ports("*4:6667",TAPMP_EXTENDED_STAR,
                              &t1,&mask,&port1,&port2);
  tt_int_op(r,OP_EQ,AF_INET);
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ,AF_INET);
  tt_int_op(qed_hs_addr_to_ipv4h(&t1),OP_EQ,0);
  ...
  r=qed_hs_addr_parse_mask_ports("*6",TAPMP_EXTENDED_STAR,
                              &t1,&mask,&port1,&port2);
  tt_int_op(r,OP_EQ,AF_INET6);
  tt_int_op(qed_hs_addr_family(&t1),OP_EQ,AF_INET6);
  tt_assert(fast_mem_is_zero((const char*)qed_hs_addr_to_in6_addr32(&t1), 16));
  tt_int_op(mask,OP_EQ,0);
  ...
  tt_int_op(QED_HS_ADDR_BUF_LEN, OP_GE,
            sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255"));

  tt_assert(sizeof(qed_hs_addr_t) >= sizeof(struct in6_addr));
 ...
 *  - the fmt_decorated result of qed_hs_addr_to_str() is expect_str.
 */
#define TEST_ADDR_PARSE_FMT(addr_str, expect_family, fmt_decorated, \
                            expect_str) \
  STMT_BEGIN \
    r = qed_hs_addr_parse(&addr, addr_str); \
    tt_int_op(r, OP_EQ, expect_family); \
    sv = qed_hs_addr_to_str(buf, &addr, sizeof(buf), fmt_decorated); \
...
#define TEST_ADDR_PARSE_XFAIL(addr_str) \
  STMT_BEGIN \
    r = qed_hs_addr_parse(&addr, addr_str); \
    tt_int_op(r, OP_EQ, -1); \
    tt_assert(qed_hs_addr_is_null(&addr)); \
  STMT_END
...
#define TEST_ADDR_PORT_PARSE_FMT(addr_port_str, default_port, expect_family, \
  ...
  STMT_BEGIN \
    r = qed_hs_addr_port_parse(LOG_DEBUG, addr_port_str, &addr, &port, \
                            default_port); \
    tt_int_op(r, OP_EQ, 0); \
    tt_int_op(qed_hs_addr_family(&addr), OP_EQ, expect_family); \
    sv = qed_hs_addr_to_str(buf, &addr, sizeof(buf), fmt_decorated); \
    tt_str_op(sv, OP_EQ, buf); \
    tt_str_op(buf, OP_EQ, expect_str); \
...
#define TEST_HOST_V4_LOOKUP(host_str) \
  STMT_BEGIN \
    r = qed_hs_lookup_hostname(host_str, &addr32h); \
    tt_int_op(r, OP_GE, -1); \
    tt_int_op(r, OP_LE, 1); \
    if (r != 0) \
      tt_int_op(addr32h, OP_EQ, 0); \
...
#define TEST_ADDR_LOOKUP_FMT(addr_str, require_family, expect_family, \
  ...
  STMT_BEGIN \
    r = qed_hs_addr_lookup(addr_str, require_family, &addr); \
    tt_int_op(r, OP_EQ, 0); \
    tt_int_op(qed_hs_addr_family(&addr), OP_EQ, expect_family); \
    sv = qed_hs_addr_to_str(buf, &addr, sizeof(buf), fmt_decorated); \
    tt_str_op(sv, OP_EQ, buf); \
 ...
 * qed_hs_addr_lookup(), with a permanent failure, and:
 *  - the returned address is null.
 */
#define TEST_ADDR_LOOKUP_XFAIL(bad_str, require_family) \
  STMT_BEGIN \
    r = qed_hs_addr_lookup(bad_str, require_family, &addr); \
    tt_int_op(r, OP_EQ, -1); \
    tt_assert(qed_hs_addr_is_null(&addr)); \
  STMT_END

/* Test that looking up host_string as a require_family IP address using
 * qed_hs_addr_lookup(),  does something sensible:
 *  - the result is -1, 0, or 1.
...
#define TEST_HOST_LOOKUP(host_str, require_family) \
  STMT_BEGIN \
    r = qed_hs_addr_lookup(host_str, require_family, &addr); \
    tt_int_op(r, OP_GE, -1); \
    tt_int_op(r, OP_LE, 1); \
    if (r != 0) \
      tt_assert(qed_hs_addr_is_null(&addr)); \
 ...
 * using qed_hs_addr_port_lookup(), and:
 *  - the address has family expect_family,
 *  - the fmt_decorated result of qed_hs_addr_to_str() is expect_str,
 *  - the port is expect_port.
 */
#define TEST_ADDR_PORT_LOOKUP_FMT(addr_port_str, expect_family, \
                                  fmt_decorated, expect_str, expect_port) \
  STMT_BEGIN \
    r = qed_hs_addr_port_lookup(addr_port_str, &addr, &port); \
    tt_int_op(r, OP_EQ, 0); \
    tt_int_op(qed_hs_addr_family(&addr), OP_EQ, expect_family); \
    sv = qed_hs_addr_to_str(buf, &addr, sizeof(buf), fmt_decorated); \
    tt_str_op(sv, OP_EQ, buf); \
...
#define TEST_ADDR_PORT_LOOKUP_XFAIL(bad_str) \
  STMT_BEGIN \
    r = qed_hs_addr_port_lookup(bad_str, &addr, &port); \
    tt_int_op(r, OP_EQ, -1); \
    tt_assert(qed_hs_addr_is_null(&addr)); \
    tt_int_op(port, OP_EQ, 0); \
  STMT_END
 ...
 * qed_hs_addr_port_lookup(),  does something sensible:
...
#define TEST_HOST_PORT_LOOKUP(host_port_str, expect_success_port) \
  STMT_BEGIN \
    r = qed_hs_addr_port_lookup(host_port_str, &addr, &port); \
    tt_int_op(r, OP_GE, -1); \
    tt_int_op(r, OP_LE, 0); \
    if (r == -1) { \
      tt_assert(qed_hs_addr_is_null(&addr)); \
      ...
      tt_assert(qed_hs_addr_family(&addr) == AF_INET || \
                qed_hs_addr_family(&addr) == AF_INET6); \
 ...
 *  - qed_hs_addr_parse(),
 *  - qed_hs_addr_port_parse() with a default port,
 *  - qed_hs_lookup_hostname(),
 *  - qed_hs_addr_lookup() with AF_INET,
 *  - qed_hs_addr_lookup() with AF_UNSPEC,
 *  - qed_hs_addr_port_lookup(), with a zero port.
 * Check for failures using:
 *  - qed_hs_addr_port_parse() without a default port, because there is no port,
 *  - qed_hs_addr_lookup() with AF_INET6,
 *  - qed_hs_addr_port_lookup(), because there is no port.
 */
 ...
 * Check for failures using:
 *  - qed_hs_addr_port_parse() without a default port, because there is no port,
 *  - qed_hs_lookup_hostname(), because it only supports IPv4,
 *  - qed_hs_addr_lookup() with AF_INET.
 */
 ...
 * Check for successful parsing using:
 *  - qed_hs_addr_port_parse() without a default port,
 *  - qed_hs_addr_port_parse() with a default port,
 *  - qed_hs_addr_port_lookup().
 * Check for failures using:
 *  - qed_hs_addr_parse(), because there is a port,
 *  - qed_hs_lookup_hostname(), because there is a port.
 *  - qed_hs_addr_lookup(), regardless of the address family, because there is a
 *    port.
 */
 ...
 * Check for failures using:
 *  - qed_hs_addr_parse(), because there is a port,
 *  - qed_hs_lookup_hostname(), because there is a port, and because it only
 *    supports IPv4,
 *  - qed_hs_addr_lookup(), regardless of the address family, because there is a
 *    port.
 */
 ...
 *  - qed_hs_addr_port_parse() with a default port,
 *  - qed_hs_lookup_hostname(),
 *  - qed_hs_addr_lookup(), regardless of the address family,
 *  - qed_hs_addr_port_lookup().
 */
 ...
 *  - qed_hs_lookup_hostname(),
 *  - qed_hs_addr_lookup(), regardless of the address family,
 *  - qed_hs_addr_port_lookup(), expecting a zero port.
 * Check for failures using:
 *  - qed_hs_addr_parse(),
 *  - qed_hs_addr_port_parse() without a default port,
 *  - qed_hs_addr_port_parse() with a default port.
 ...
 *  - qed_hs_addr_port_lookup(), expecting expect_success_port if the lookup is
 *    successful.
 * Check for failures using:
 *  - qed_hs_addr_parse(),
 *  - qed_hs_addr_port_parse() without a default port,
 *  - qed_hs_addr_port_parse() with a default port,
 *  - qed_hs_lookup_hostname(), because it doesn't support ports,
 *  - qed_hs_addr_lookup(), regardless of the address family, because it doesn't
 *    support ports.
 */
...
/** Test qed_hs_addr_parse() and qed_hs_addr_port_parse(). */
static void
test_addr_parse(void *arg)
{
  ...
  TEST_HOSTNAME("qed-cc.org");
  TEST_HOSTNAME_PORT("qed-cc.org:56", 56);
...
update_difference(int ipv6, uint8_t *d,
                  const qed_hs_addr_t *a, const qed_hs_addr_t *b)
{
  ...
  if (ipv6) {
    ba = qed_hs_addr_to_in6_addr8(a);
    bb = qed_hs_addr_to_in6_addr8(b);
  } else {
    set_uint32(a_tmp, qed_hs_addr_to_ipv4n(a));
    set_uint32(b_tmp, qed_hs_addr_to_ipv4n(b));
    ba = a_tmp; bb = b_tmp;
  }
...
test_virtaddrmap(void *data)
{
  ...
  qed_hs_addr_parse(&cfg[0].addr, "64.65.0.0");
  qed_hs_addr_parse(&cfg[1].addr, "3491:c0c0::");

  for (ipv6 = 0; ipv6 <= 1; ++ipv6) {
    for (bits = 0; bits < 18; ++bits) {
      qed_hs_addr_t last_a;
      cfg[ipv6].bits = bits;
      memset(bytes, 0, sizeof(bytes));
      qed_hs_addr_copy(&last_a, &cfg[ipv6].addr);
      ...
      for (iter = 0; iter < 128; ++iter) {
        ...
        tt_int_op(0, OP_EQ, qed_hs_addr_compare_masked(&a, &cfg[ipv6].addr,
                                                 bits, CMP_EXACT));
...
test_virtaddrmap_persist(void *data)
{
  ...
  qed_hs_addr_parse(&addr, a);
  tt_int_op(AF_INET, OP_EQ, qed_hs_addr_family(&addr));
  ...
  a = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4,
                                          qed_hs_strdup("wumble.onion"));
  b = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4,
                                          qed_hs_strdup("wumpus.onion"));
  ...
  qed_hs_addr_parse(&addr, a);
  tt_int_op(AF_INET6, OP_EQ, qed_hs_addr_family(&addr));
  ...
  a = addressmap_register_virtual_address(RESOLVED_TYPE_IPV6,
                                          qed_hs_strdup("wuffle.baz"));
  b = addressmap_register_virtual_address(RESOLVED_TYPE_IPV6,
                                          qed_hs_strdup("gribble.baz"));
  c = addressmap_register_virtual_address(RESOLVED_TYPE_IPV6,
                                      qed_hs_strdup("surprisingly-legible.baz"));
  ...
  canned_data = ones = qed_hs_malloc(canned_data_len);
  ...
  a = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4,
                                          qed_hs_strdup("might-work.onion"));
  if (a) {
...
{
  (void)arg;
  tt_assert(qed_hs_addr_hostname_is_local("localhost"));
  tt_assert(qed_hs_addr_hostname_is_local("LOCALHOST"));
  tt_assert(qed_hs_addr_hostname_is_local("LocalHost"));
  tt_assert(qed_hs_addr_hostname_is_local("local"));
  tt_assert(qed_hs_addr_hostname_is_local("LOCAL"));
  tt_assert(qed_hs_addr_hostname_is_local("here.now.local"));
  tt_assert(qed_hs_addr_hostname_is_local("here.now.LOCAL"));

  tt_assert(!qed_hs_addr_hostname_is_local(" localhost"));
  tt_assert(!qed_hs_addr_hostname_is_local("www.qed-cc.org"));
 done:
  ;
}
...
#define CHECK(sa, s) do {                                       \
    v = qed_hs_sockaddr_to_str((const struct sockaddr*) &(sa));    \
    tt_str_op(v,OP_EQ,(s));                                        \
    qed_hs_free(v);                                                \
  } while (0)
  (void)arg;
...
test_addr_is_loopback(void *data)
{
  ...
  for (i=0; loopback_items[i].name; ++i) {
    tt_int_op(qed_hs_addr_parse(&addr, loopback_items[i].name), OP_GE, 0);
    tt_int_op(qed_hs_addr_is_loopback(&addr), OP_EQ,
              loopback_items[i].is_loopback);
  }

  qed_hs_addr_make_unspec(&addr);
  tt_int_op(qed_hs_addr_is_loopback(&addr), OP_EQ, 0);
...
{
  qed_hs_addr_t *addr = qed_hs_malloc(sizeof(*addr));
  qed_hs_addr_t *zeros = qed_hs_malloc_zero(sizeof(*addr));
  char buf[QED_HS_ADDR_BUF_LEN];
  (void) data;
  /* Ensure that before qed_hs_addr_make_null, addr != 0's */
  memset(addr, 1, sizeof(*addr));
  tt_int_op(fast_memcmp(addr, zeros, sizeof(*addr)), OP_NE, 0);
  /* Test with AF == AF_INET */
  zeros->family = AF_INET;
  qed_hs_addr_make_null(addr, AF_INET);
  tt_int_op(fast_memcmp(addr, zeros, sizeof(*addr)), OP_EQ, 0);
  tt_str_op(qed_hs_addr_to_str(buf, addr, sizeof(buf), 0), OP_EQ, "0.0.0.0");
  /* Test with AF == AF_INET6 */
  memset(addr, 1, sizeof(*addr));
  zeros->family = AF_INET6;
  qed_hs_addr_make_null(addr, AF_INET6);
  tt_int_op(fast_memcmp(addr, zeros, sizeof(*addr)), OP_EQ, 0);
  tt_str_op(qed_hs_addr_to_str(buf, addr, sizeof(buf), 0), OP_EQ, "::");
 done:
  qed_hs_free(addr);
  qed_hs_free(zeros);
}
...
#define TEST_ADDR_INTERNAL(a, for_listening, rv) STMT_BEGIN \
    qed_hs_addr_t t; \
    tt_int_op(qed_hs_inet_pton(AF_INET, a, &t.addr.in_addr), OP_EQ, 1); \
    t.family = AF_INET; \
    tt_int_op(qed_hs_addr_is_internal(&t, for_listening), OP_EQ, rv); \
  STMT_END;
...
#define TEST_ADDR_ATON(a, rv) STMT_BEGIN \
    struct in_addr addr; \
    tt_int_op(qed_hs_inet_aton(a, &addr), OP_EQ, rv); \
  STMT_END;
...
#define get_ipv4(test_addr, str, iprv) STMT_BEGIN               \
    test_addr = qed_hs_malloc(sizeof(qed_hs_addr_t));                 \
    test_addr->family = AF_INET;                                \
    iprv = qed_hs_inet_aton(str, &test_addr->addr.in_addr);        \
    qed_hs_assert(iprv);                                           \
  STMT_END;
...
#define get_ipv6(test_addr, str, iprv) STMT_BEGIN                       \
    ...
    iprv = qed_hs_inet_pton(AF_INET6, str, &test_addr->addr.in6_addr);     \
    qed_hs_assert(iprv);                                                   \
  STMT_END;

#define get_af_unix(test_addr) STMT_BEGIN                       \
    test_addr = qed_hs_malloc_zero(sizeof(qed_hs_addr_t));            \
...
#define TEST_ADDR_VALIDITY(a, lis, rv) STMT_BEGIN               \
    qed_hs_assert(a);                                              \
    tt_int_op(qed_hs_addr_is_valid(a, lis), OP_EQ, rv);            \
  STMT_END;
...
#define TEST_ADDR_IS_NULL(a, rv) STMT_BEGIN                 \
    qed_hs_assert(a);                                          \
    tt_int_op(qed_hs_addr_is_null(a), OP_EQ, rv);              \
  STMT_END;
