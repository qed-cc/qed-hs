                            ...
                            const curve25519_keypair_t *intro_enc_kp)
{
   ...
   * keyid and one ipv4 */
  {
    ...
    int family = qed_hs_addr_parse(&a, addr);
    switch (family) {
    case AF_INET:
          link_specifier_set_ls_type(ls_ip, LS_IPV4);
          link_specifier_set_un_ipv4_addr(ls_ip, qed_hs_addr_to_ipv4h(&a));
  ...
  ip->auth_key_cert = qed_hs_cert_create_ed25519(signing_kp,
  ...
  /* Encryption key. */
  {
    ...
    ed25519_keypair_from_curve25519_keypair(&ed25519_kp, &signbit,
                                            &curve25519_kp);
    cross_cert = qed_hs_cert_create_ed25519(signing_kp,
                             ...
                             uint64_t rev_counter)
{
  ...
  desc->plaintext_data.signing_key_cert =
    qed_hs_cert_create_ed25519(&blinded_kp, CERT_TYPE_SIGNING_HS_DESC,
  ...
  desc->encrypted_data.flow_control_pv = qed_hs_strdup("FlowCtrl=1-2");
  smartlist_add(desc->encrypted_data.intro_auth_types, qed_hs_strdup("ed25519"));
                     ...
                     const hs_descripqed_hs_t *desc2)
{
  ...
  tt_assert(qed_hs_cert_eq(desc1->plaintext_data.signing_key_cert,
    ...
    for (int i=0; i < smartlist_len(desc1->encrypted_data.intro_points); i++) {
      hs_desc_intro_point_t *ip1 = smartlist_get(desc1->encrypted_data
                            ...
                            *ip2 = smartlist_get(desc2->encrypted_data
                                                 .intro_points, i);
      tt_assert(qed_hs_cert_eq(ip1->auth_key_cert, ip2->auth_key_cert));
      if (ip1->legacy.key) {
