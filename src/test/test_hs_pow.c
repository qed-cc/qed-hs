/* Copyright (c) 2020-2023, The QED Project, Inc. */
...
testing_hs_pow_service_new(void)
{
  ...
  testing_hs_pow_service_t *tsvc = qed_hs_malloc_zero(sizeof *tsvc);
  ...
  tsvc->intro_circ->cpath = qed_hs_malloc_zero(sizeof(crypt_path_t));

  struct hs_ident_circuit_t *hs_ident = qed_hs_malloc_zero(sizeof *hs_ident);
...
test_hs_pow_vectors(void *arg)
{
  ...
  for (unsigned vec_i = 0; vec_i < num_vectors; vec_i++) {
    ...
    decrypted = qed_hs_malloc_zero(decrypted_len);
