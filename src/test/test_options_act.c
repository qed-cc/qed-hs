...
test_options_act_create_dirs(void *arg)
{
  ...
  char *fn;
  fn = qed_hs_strdup(get_fname_rnd("ddir"));
  opts->DataDirectory = qed_hs_strdup(fn);
  opts->CacheDirectory = qed_hs_strdup(fn);
  qed_hs_asprintf(&opts->KeyDirectory, "%s/keys", fn);
  ...
  qed_hs_free(fn);
  qed_hs_free(opts->KeyDirectory);
  or_options_free(opts);

  /* Success case 2: all directories are different. */
  opts = mock_opts = options_new();
  opts->DataDirectory = qed_hs_strdup(get_fname_rnd("ddir"));
  opts->CacheDirectory = qed_hs_strdup(get_fname_rnd("cdir"));
  opts->KeyDirectory = qed_hs_strdup(get_fname_rnd("kdir"));
  ...
  opts->DataDirectory = qed_hs_strdup(fn);
  opts->CacheDirectory = qed_hs_strdup(fn);
  opts->KeyDirectory = qed_hs_strdup(fn);
  opts->DataDirectoryGroupReadable = 1;
  opts->CacheDirectoryGroupReadable = -1; /* default. */
