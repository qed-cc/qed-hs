...
/* Thing to cast to fake qed_hs_tls_t * to appease assert_connection_ok() */
...
setup_fake_connection_for_channel(channel_tls_t *chan)
{
  ...
  conn->base_.address = qed_hs_strdup("<fake>");

  conn->base_.port = 4242;

  conn->tls = (qed_hs_tls_t *)((void *)(&fake_tortls));
...
{
  channel_tls_t *chan = qed_hs_realloc(new_fake_channel(), sizeof(channel_tls_t));
...
setup_mock_consensus(void)
{
  current_md_consensus = current_ns_consensus
        = qed_hs_malloc_zero(sizeof(networkstatus_t));
...
free_mock_consensus(void)
{
  ...
  qed_hs_free(current_ns_consensus);
}
...
setup_mock_network(void)
{
  ...
  relay = qed_hs_malloc_zero(sizeof(routerstatus_t));
...
static void
dummy_timer_cb(qed_hs_timer_t *t, void *arg, const monotime_t *now_mono)
{
...
{
  qed_hs_timer_t *dummy_timer = timer_new(dummy_timer_cb, NULL);
...
test_channelpadding_killonehop(void *arg)
{
  ...
  routerstatus_t *relay = qed_hs_malloc_zero(sizeof(routerstatus_t));
  monotime_init();
  monotime_enable_test_mocking();
