...
test_upgrade_from_guard_wait(void *arg)
{
  ...
  orig_circ->build_state = qed_hs_malloc_zero(sizeof(cpath_build_state_t));

  circuit_set_state(circ, CIRCUIT_STATE_GUARD_WAIT);

  /* Put it in guard wait state. */
  guard = qed_hs_malloc_zero(sizeof(*guard));
...
{
  (void)arg;
  circuit_t *circ = qed_hs_malloc_zero(sizeof(circuit_t));
  ...
  /* n_chan must be NULL */
  circ->n_chan = qed_hs_malloc_zero(sizeof(channel_t));
  server = 1;
  tt_int_op(circuit_extend_state_valid_helper(circ), OP_EQ, -1);
  expect_log_msg("n_chan already set. Bug/attack. Closing.\n");
  mock_clean_saved_logs();
  qed_hs_free(circ->n_chan);

  /* n_hop must be NULL */
  circ->n_hop = qed_hs_malloc_zero(sizeof(extend_info_t));
  server = 1;
  tt_int_op(circuit_extend_state_valid_helper(circ), OP_EQ, -1);
  expect_log_msg("conn to next hop already launched. Bug/attack. Closing.\n");
  mock_clean_saved_logs();
  qed_hs_free(circ->n_hop);
...
{
  (void)arg;
  extend_cell_t *ec = qed_hs_malloc_zero(sizeof(extend_cell_t));
  extend_cell_t *old_ec = qed_hs_malloc_zero(sizeof(extend_cell_t));
  extend_cell_t *zero_ec = qed_hs_malloc_zero(sizeof(extend_cell_t));

  node_t *fake_node = qed_hs_malloc_zero(sizeof(node_t));
  ed25519_public_key_t *fake_ed25519_id = NULL;
  fake_ed25519_id = qed_hs_malloc_zero(sizeof(ed25519_public_key_t));
  ...
  qed_hs_free(fake_ed25519_id);
  qed_hs_free(fake_node);
}
...
test_circuit_extend_lspec_valid(void *arg)
{
  (void)arg;
  extend_cell_t *ec = qed_hs_malloc_zero(sizeof(extend_cell_t));
  channel_t *p_chan = qed_hs_malloc_zero(sizeof(channel_t));
  or_circuit_t *or_circ = qed_hs_malloc_zero(sizeof(or_circuit_t));
  ...
  tt_int_op(smartlist_len(qed_hs_get_captured_bug_log_()), OP_GE, 0);
  tt_int_op(smartlist_len(qed_hs_get_captured_bug_log_()), OP_LE, 2);
  qed_hs_end_capture_bugs_();
  mock_clean_saved_logs();
#endif /* !defined(ALL_BUGS_ARE_FATAL) */
  ...
  qed_hs_addr_parse(&ec->orport_ipv4.addr, PUBLIC_IPV4);
  tt_int_op(circuit_extend_lspec_valid_helper(ec, circ), OP_EQ, -1);
  expect_log_msg("Client asked me to extend to a zero destination port "
                 "or IPv4 address '1.2.3.4:0'.\n");
  mock_clean_saved_logs();
  qed_hs_addr_port_make_null_ap(&ec->orport_ipv4, AF_INET);
  qed_hs_addr_port_make_null_ap(&ec->orport_ipv6, AF_INET6);
  ...
  qed_hs_addr_parse(&ec->orport_ipv4.addr, INTERNAL_IPV4);
  ec->orport_ipv4.port = VALID_PORT;
  ...
  qed_hs_addr_parse(&ec->orport_ipv6.addr, PUBLIC_IPV6);
  tt_int_op(circuit_extend_lspec_valid_helper(ec, circ), OP_EQ, -1);
  expect_log_msg("Client asked me to extend to a zero destination port "
   ...
   * Result depends on ExtendAllowPrivateAddresses. */
  qed_hs_addr_parse(&ec->orport_ipv6.addr, INTERNAL_IPV6);
  ec->orport_ipv6.port = VALID_PORT;
  ...
  tt_int_op(smartlist_len(qed_hs_get_captured_bug_log_()), OP_GE, 0);
  tt_int_op(smartlist_len(qed_hs_get_captured_bug_log_()), OP_LE, 1);
  qed_hs_end_capture_bugs_();
  mock_clean_saved_logs();
  fake_options->ExtendAllowPrivateAddresses = 0;
#endif /* !defined(ALL_BUGS_ARE_FATAL) */
  ...
  qed_hs_addr_make_null(&ec->orport_ipv4.addr, AF_INET);
  ...
  qed_hs_addr_make_null(&ec->orport_ipv4.addr, AF_UNSPEC);
  ec->orport_ipv4.port = 0;
  or_circ->p_chan = NULL;
...
test_circuit_extend_add_ip(void *arg)
{
  ...
  qed_hs_addr_copy(&ipv4_tmp, &fake_node->ri->ipv4_addr);
  /* The IPv4 should match */
  tt_int_op(qed_hs_addr_compare(&ec->orport_ipv4.addr, &ipv4_tmp, CMP_SEMANTIC),
            OP_EQ, 0);
  ...
  tt_int_op(qed_hs_addr_compare(&ec->orport_ipv6.addr, &fake_node->ri->ipv6_addr,
...
test_circuit_choose_ip_ap_for_extend(void *arg)
{
  ...
  qed_hs_addr_parse(&ipv4_ap.addr, PUBLIC_IPV4);
  ipv4_ap.port = VALID_PORT;
  qed_hs_addr_parse(&ipv6_ap.addr, PUBLIC_IPV6);
  ...
  /* Two valid addresses */
  const qed_hs_addr_port_t *chosen_addr = NULL;
...
test_circuit_open_connection_for_extend(void *arg)
{
  ...
  qed_hs_assert(use_ipv4 || use_ipv6);

  extend_cell_t *ec = qed_hs_malloc_zero(sizeof(extend_cell_t));
  circuit_t *circ = qed_hs_malloc_zero(sizeof(circuit_t));
  channel_t *fake_n_chan = qed_hs_malloc_zero(sizeof(channel_t));
...
{
  (void)arg;
  relay_msg_t *msg = qed_hs_malloc_zero(sizeof(relay_msg_t));
  channel_t *p_chan = qed_hs_malloc_zero(sizeof(channel_t));
  or_circuit_t *or_circ = qed_hs_malloc_zero(sizeof(or_circuit_t));
  circuit_t *circ = TO_CIRCUIT(or_circ);
  channel_t *fake_n_chan = qed_hs_malloc_zero(sizeof(channel_t));
  ...
  qed_hs_addr_parse(&mock_extend_cell_parse_cell_out.orport_ipv4.addr,
                 PUBLIC_IPV4);
...
{
  (void)arg;
  created_cell_t *created_cell = qed_hs_malloc_zero(sizeof(created_cell_t));
  or_circuit_t *or_circ = qed_hs_malloc_zero(sizeof(or_circuit_t));
...
test_circuit_send_next_onion_skin(void *arg)
{
  ...
  qed_hs_addr_parse(&ipv6_hop.orports[0].addr, "1::2");
  ...
  qed_hs_addr_from_ipv4h(&ipv4_hop.orports[0].addr, 0x20304050);
