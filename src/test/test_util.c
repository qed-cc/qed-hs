...
/** Test the qed_hs_isinf() wrapper */
static void
test_qed_hs_isinf(void *arg)
{
  (void) arg;

  tt_assert(qed_hs_isinf(INFINITY_DBL));

  tt_assert(!qed_hs_isinf(NAN_DBL));
  tt_assert(!qed_hs_isinf(DBL_EPSILON));
  tt_assert(!qed_hs_isinf(DBL_MAX));
  tt_assert(!qed_hs_isinf(DBL_MIN));

  tt_assert(!qed_hs_isinf(0.0));
  tt_assert(!qed_hs_isinf(0.1));
  tt_assert(!qed_hs_isinf(3));
  tt_assert(!qed_hs_isinf(3.14));
 ...
 * changed qed_hs_timegm interface. */
static time_t
qed_hs_timegm_wrapper(const struct tm *tm)
{
  time_t t;
  if (qed_hs_timegm_impl(tm, &t) < 0)
    return -1;
  return t;
}

#define qed_hs_timegm qed_hs_timegm_wrapper
                              ...
                              size_t read_limit)
{
  ...
  fifo_name = qed_hs_strdup(get_fname(fname));
  test_str = qed_hs_malloc(file_len);
  ...
  unlink(fifo_name);
  qed_hs_free(fifo_name);
  qed_hs_free(test_str);
  qed_hs_free(str);
  if (fd >= 0)
    close(fd);
}
...
test_util_read_file_eof_tiny_limit(void *arg)
{
  ...
  test_util_read_until_eof_impl("qed_hs_test_fifo_tiny", 5, 4);
}
...
{
  (void)arg;
  test_util_read_until_eof_impl("qed_hs_test_fifo_1ka", 1024, 1023);
}
...
{
  (void)arg;
  test_util_read_until_eof_impl("qed_hs_test_fifo_1kb", 1024, 1024);
}
...
test_util_read_file_eof_two_loops(void *arg)
{
  ...
  test_util_read_until_eof_impl("qed_hs_test_fifo_2k", 2048, 10000);
}
...
test_util_read_file_eof_two_loops_b(void *arg)
{
  ...
  test_util_read_until_eof_impl("qed_hs_test_fifo_2kb", 2048, 2048);
}
...
test_util_read_file_eof_zero_bytes(void *arg)
{
  (void)arg;
  // zero-byte fifo
  test_util_read_until_eof_impl("qed_hs_test_fifo_empty", 0, 10000);
}
...
test_util_read_file_endlines(void *arg)
{
  ...
  fname = qed_hs_strdup(get_fname("file_with_crlf_ending"));
...
test_util_write_chunks_to_file(void *arg)
{
  ...
  char *data_str = qed_hs_malloc(data_str_len);
  char *temp_str = qed_hs_malloc(temp_str_len);
  ...
  */
  fname = qed_hs_strdup(get_fname("write_chunks_with_tempfile"));
  qed_hs_asprintf(&tempname, "%s.tmp", fname);
  ...
  tt_int_op(r, OP_EQ, 0);
  qed_hs_free(fname);
  qed_hs_free(tempname);

  /*
  *  Check if it skips using a tempfile with flags
  */
  fname = qed_hs_strdup(get_fname("write_chunks_with_no_tempfile"));
  qed_hs_asprintf(&tempname, "%s.tmp", fname);
  ...
  qed_hs_free(tempname);
  qed_hs_free(str);
  qed_hs_free(data_str);
  qed_hs_free(temp_str);
}
...
{
  (void)arg;
  char *fname = qed_hs_strdup(get_fname("write_if_changed"));
  char *s = NULL;
  int rv;
 ...
 * this is also the set we use to test qed_hs_timegm */
#define TM_EQUAL(a, b)           \
          TT_STMT_BEGIN          \
            _TFE(a, b, tm_year); \
            _TFE(a, b, tm_mon ); \
...
#define TV_USEC_MAX UINT_MAX
  ...
  end.tv_usec = QED_HS_USEC_PER_SEC;
  ...
  start.tv_sec = TV_SEC_MIN;
  start.tv_usec = QED_HS_USEC_PER_SEC;
  ...
  /* Test qed_hs_timegm & qed_hs_gmtime_r */
   ...
   * Zeroing may not be needed for b_time, as long as qed_hs_gmtime_r
  ...
  a_time.tm_sec = 55;
  t_res = 1062224095UL;
  tt_int_op(t_res, OP_EQ, qed_hs_timegm(&a_time));
  qed_hs_gmtime_r(&t_res, &b_time);
  ...
  tt_int_op((time_t) -1,OP_EQ, qed_hs_timegm(&a_time));
  //expect_single_log_msg_containing("Result does not fit in qed_hs_timegm");
  ...
  /* Test qed_hs_timegm out of range */
...
#define CHECK_TIMEGM_ARG_OUT_OF_RANGE(msg) \
    CHECK_TIMEGM_WARNING("Out-of-range argument to qed_hs_timegm")
  ...
  /* Test qed_hs_gmtime_r out of range */
  ...
  }
  {
    ...
    t_res = -9223372036854775LL;
    CAPTURE();
    qed_hs_localtime_r(&t_res, &b_time);
    CHECK_POSSIBLE_EINVAL();
    tt_assert(b_time.tm_year == (1970-1900) ||
              b_time.tm_year == (1-1900));
  ...
  CAPTURE();
  i = parse_rfc1123_time(timestr, &t_res);
  // CHECK_TIMEGM_WARNING("does not fit in qed_hs_timegm");
  tt_int_op(-1,OP_EQ, i);
#elif SIZEOF_TIME_T == 8
...
#define T(s) do {                               \
    format_iso_time(b, qed_hs_timegm(&a_time));    \
    tt_str_op(b, OP_EQ, (s));                      \
    b[0]='\0';                                  \
  } while (0)
  ...
  tt_int_op(0,OP_EQ,
            parse_http_time("Wednesday, 04 Aug 1994 00:48:22 GMT", &a_time));
  tt_int_op((time_t)775961302UL,OP_EQ, qed_hs_timegm(&a_time));
  ...
  tt_int_op((time_t)-1,OP_EQ, qed_hs_timegm(&a_time));
  //expect_single_log_msg_containing("does not fit in qed_hs_timegm");
  teardown_capture_of_logs();
#elif SIZEOF_TIME_T == 8
  tt_int_op(0,OP_EQ,parse_http_time("Wed, 17 Feb 2038 06:13:20 GMT", &a_time));
  tt_int_op((time_t)2150000000UL,OP_EQ, qed_hs_timegm(&a_time));
...
#undef qed_hs_timegm
  ...
  struct tm tm, *p;
  p = qed_hs_gmtime_r(&now, &tm);
  tt_ptr_op(p, OP_NE, NULL);

  int r = qed_hs_timegm(&tm, &time2);
  tt_int_op(r, OP_EQ, 0);
...
/** Test qed_hs_escape_str_for_pt_args(). */
static void
test_util_escape_string_socks(void *arg)
{
  ...
  (void)arg;
  escaped_string = qed_hs_escape_str_for_pt_args("This is a backslash: \\",";\\");
  tt_assert(escaped_string);
  tt_str_op(escaped_string,OP_EQ, "This is a backslash: \\\\");
  qed_hs_free(escaped_string);

  /** Simple semicolon escape. */
  escaped_string = qed_hs_escape_str_for_pt_args("First rule:Do not use ;",";\\");
  tt_assert(escaped_string);
  tt_str_op(escaped_string,OP_EQ, "First rule:Do not use \\;");
  qed_hs_free(escaped_string);

  /** Empty string. */
  escaped_string = qed_hs_escape_str_for_pt_args("", ";\\");
  tt_assert(escaped_string);
...
test_util_strmisc(void *arg)
{
  ...
  tt_int_op(-1,OP_EQ, qed_hs_snprintf(buf, 0, "Foo"));
  tt_int_op(-1,OP_EQ, qed_hs_snprintf(buf, 2, "Foo"));
  tt_int_op(-1,OP_EQ, qed_hs_snprintf(buf, 3, "Foo"));
  tt_int_op(-1,OP_NE, qed_hs_snprintf(buf, 4, "Foo"));
  /* Always NUL-terminate the output */
  qed_hs_snprintf(buf, 5, "abcdef");
  tt_int_op(0,OP_EQ, buf[4]);
  qed_hs_snprintf(buf, 10, "abcdef");
  tt_int_op(0,OP_EQ, buf[6]);
  /* uint64 */
  qed_hs_snprintf(buf, sizeof(buf), "x!%"PRIu64"!x",
  ...
  memset(buf,0,20);
  buf[20] = 'x';
  tt_assert(qed_hs_digest_is_zero(buf));
  buf[19] = 'x';
  tt_assert(!qed_hs_digest_is_zero(buf));
  ...
  cp_tmp = qed_hs_strdup("abcdef");
  tt_assert(qed_hs_strisnonupper(cp_tmp));
  cp_tmp[3] = 'D';
  tt_assert(!qed_hs_strisnonupper(cp_tmp));
  qed_hs_strupper(cp_tmp);
  tt_str_op(cp_tmp,OP_EQ, "ABCDEF");
  qed_hs_strlower(cp_tmp);
  tt_str_op(cp_tmp,OP_EQ, "abcdef");
  tt_assert(qed_hs_strisnonupper(cp_tmp));
  tt_assert(qed_hs_strisprint(cp_tmp));
  cp_tmp[3] = 3;
  tt_assert(!qed_hs_strisprint(cp_tmp));
  qed_hs_free(cp_tmp);
  ...
  /* Test memmem and memstr */
  {
    ...
    tt_ptr_op(qed_hs_memmem(haystack, 5, "ef", 2), OP_EQ, NULL);
    tt_ptr_op(qed_hs_memmem(haystack, 5, "cd", 2),OP_EQ, haystack + 2);
    tt_ptr_op(qed_hs_memmem(haystack, 5, "cde", 3),OP_EQ, haystack + 2);
    tt_ptr_op(qed_hs_memmem(haystack, 4, "cde", 3), OP_EQ, NULL);
    haystack = "ababcad";
    tt_ptr_op(qed_hs_memmem(haystack, 7, "abc", 3),OP_EQ, haystack + 2);
    tt_ptr_op(qed_hs_memmem(haystack, 7, "ad", 2),OP_EQ, haystack + 5);
    tt_ptr_op(qed_hs_memmem(haystack, 7, "cad", 3),OP_EQ, haystack + 4);
    tt_ptr_op(qed_hs_memmem(haystack, 7, "dadad", 5), OP_EQ, NULL);
    tt_ptr_op(qed_hs_memmem(haystack, 7, "abcdefghij", 10), OP_EQ, NULL);
    /* memstr */
    tt_ptr_op(qed_hs_memstr(haystack, 7, "abc"),OP_EQ, haystack + 2);
    tt_ptr_op(qed_hs_memstr(haystack, 7, "cad"),OP_EQ, haystack + 4);
    tt_ptr_op(qed_hs_memstr(haystack, 6, "cad"), OP_EQ, NULL);
    tt_ptr_op(qed_hs_memstr(haystack, 7, "cadd"), OP_EQ, NULL);
    tt_ptr_op(qed_hs_memstr(haystack, 7, "fe"), OP_EQ, NULL);
    tt_ptr_op(qed_hs_memstr(haystack, 7, "ababcade"), OP_EQ, NULL);
  }
...
test_util_parse_integer(void *arg)
{
  ...
  /* Test parse_long */
  /* Empty/zero input */
  tt_int_op(0L,OP_EQ, qed_hs_parse_long("",10,0,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0L,OP_EQ, qed_hs_parse_long("0",10,0,100,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  /* Normal cases */
  tt_int_op(10L,OP_EQ, qed_hs_parse_long("10",10,0,100,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(10L,OP_EQ, qed_hs_parse_long("10",10,0,10,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(10L,OP_EQ, qed_hs_parse_long("10",10,10,100,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(-50L,OP_EQ, qed_hs_parse_long("-50",10,-100,100,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(-50L,OP_EQ, qed_hs_parse_long("-50",10,-100,0,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  tt_int_op(-50L,OP_EQ, qed_hs_parse_long("-50",10,-50,0,&i,NULL));
  tt_int_op(1,OP_EQ, i);
  /* Extra garbage */
  tt_int_op(0L,OP_EQ, qed_hs_parse_long("10m",10,0,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0L,OP_EQ, qed_hs_parse_long("-50 plus garbage",10,-100,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(10L,OP_EQ, qed_hs_parse_long("10m",10,0,100,&i,&cp));
  tt_int_op(1,OP_EQ, i);
  tt_str_op(cp,OP_EQ, "m");
  tt_int_op(-50L,OP_EQ, qed_hs_parse_long("-50 plus garbage",10,-100,100,&i,&cp));
  tt_int_op(1,OP_EQ, i);
  tt_str_op(cp,OP_EQ, " plus garbage");
  /* Illogical min max */
  tt_int_op(0L,OP_EQ,  qed_hs_parse_long("10",10,50,4,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0L,OP_EQ,   qed_hs_parse_long("-50",10,100,-100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  /* Out of bounds */
  tt_int_op(0L,OP_EQ,  qed_hs_parse_long("10",10,50,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0L,OP_EQ,   qed_hs_parse_long("-50",10,0,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  /* Base different than 10 */
  tt_int_op(2L,OP_EQ,   qed_hs_parse_long("10",2,0,100,NULL,NULL));
  tt_int_op(0L,OP_EQ,   qed_hs_parse_long("2",2,0,100,NULL,NULL));
  tt_int_op(68284L,OP_EQ, qed_hs_parse_long("10abc",16,0,70000,NULL,NULL));
  tt_int_op(68284L,OP_EQ, qed_hs_parse_long("10ABC",16,0,70000,NULL,NULL));
  tt_int_op(0L,OP_EQ,   qed_hs_parse_long("10",-2,0,100,NULL,NULL));
  tt_int_op(0,OP_EQ, qed_hs_parse_long("10ABC",-1,0,70000,&i,NULL));
  tt_int_op(i,OP_EQ, 0);

  /* Test parse_ulong */
  tt_int_op(0UL,OP_EQ, qed_hs_parse_ulong("",10,0,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, qed_hs_parse_ulong("0",10,0,100,NULL,NULL));
  tt_int_op(10UL,OP_EQ, qed_hs_parse_ulong("10",10,0,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, qed_hs_parse_ulong("10",10,50,100,NULL,NULL));
  tt_int_op(10UL,OP_EQ, qed_hs_parse_ulong("10",10,0,10,NULL,NULL));
  tt_int_op(10UL,OP_EQ, qed_hs_parse_ulong("10",10,10,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, qed_hs_parse_ulong("8",8,0,100,NULL,NULL));
  tt_int_op(50UL,OP_EQ, qed_hs_parse_ulong("50",10,50,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, qed_hs_parse_ulong("-50",10,0,100,NULL,NULL));
  tt_int_op(0UL,OP_EQ, qed_hs_parse_ulong("50",-1,50,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);
  tt_int_op(0UL,OP_EQ, qed_hs_parse_ulong("-50",10,0,100,&i,NULL));
  tt_int_op(0,OP_EQ, i);

  /* Test parse_uint64 */
  tt_assert(UINT64_C(10) == qed_hs_parse_uint64("10 x",10,0,100, &i, &cp));
  tt_int_op(1,OP_EQ, i);
  tt_str_op(cp,OP_EQ, " x");
  tt_assert(UINT64_C(12345678901) ==
              qed_hs_parse_uint64("12345678901",10,0,UINT64_MAX, &i, &cp));
  tt_int_op(1,OP_EQ, i);
  tt_str_op(cp,OP_EQ, "");
  tt_assert(UINT64_C(0) ==
              qed_hs_parse_uint64("12345678901",10,500,INT32_MAX, &i, &cp));
  tt_int_op(0,OP_EQ, i);
  tt_assert(UINT64_C(0) ==
              qed_hs_parse_uint64("123",-1,0,INT32_MAX, &i, &cp));
  tt_int_op(0,OP_EQ, i);

  {
  /* Test parse_double */
  double d = qed_hs_parse_double("10", 0, (double)UINT64_MAX,&i,NULL);
  tt_int_op(1,OP_EQ, i);
  tt_assert(((uint64_t)d) == 10);
  d = qed_hs_parse_double("0", 0, (double)UINT64_MAX,&i,NULL);
  tt_int_op(1,OP_EQ, i);
  tt_assert(((uint64_t)d) == 0);
  d = qed_hs_parse_double(" ", 0, (double)UINT64_MAX,&i,NULL);
  tt_double_op(fabs(d), OP_LT, 1e-10);
  tt_int_op(0,OP_EQ, i);
  d = qed_hs_parse_double(".0a", 0, (double)UINT64_MAX,&i,NULL);
  tt_double_op(fabs(d), OP_LT, 1e-10);
  tt_int_op(0,OP_EQ, i);
  d = qed_hs_parse_double(".0a", 0, (double)UINT64_MAX,&i,&cp);
  tt_double_op(fabs(d), OP_LT, 1e-10);
  tt_int_op(1,OP_EQ, i);
  d = qed_hs_parse_double("-.0", 0, (double)UINT64_MAX,&i,NULL);
  tt_int_op(1,OP_EQ, i);
  tt_assert(((uint64_t)d) == 0);
  d = qed_hs_parse_double("-10", -100.0, 100.0,&i,NULL);
  tt_int_op(1,OP_EQ, i);
  tt_double_op(fabs(d - -10.0),OP_LT, 1E-12);
  }

  {
    /* Test qed_hs_parse_* where we overflow/underflow the underlying type. */
    /* This string should overflow 64-bit ints. */
#define TOOBIG "100000000000000000000000000"
    tt_int_op(0L, OP_EQ,
              qed_hs_parse_long(TOOBIG, 10, LONG_MIN, LONG_MAX, &i, NULL));
    tt_int_op(i,OP_EQ, 0);
    tt_int_op(0L,OP_EQ,
              qed_hs_parse_long("-"TOOBIG, 10, LONG_MIN, LONG_MAX, &i, NULL));
    tt_int_op(i,OP_EQ, 0);
    tt_int_op(0UL,OP_EQ, qed_hs_parse_ulong(TOOBIG, 10, 0, ULONG_MAX, &i, NULL));
    tt_int_op(i,OP_EQ, 0);
    tt_u64_op(UINT64_C(0), OP_EQ, qed_hs_parse_uint64(TOOBIG, 10,
                                             0, UINT64_MAX, &i, NULL));
    tt_int_op(i,OP_EQ, 0);
  }
 done:
...
test_util_pow2(void *arg)
{
  /* Test qed_hs_log2(). */
  (void)arg;
  tt_int_op(qed_hs_log2(64),OP_EQ, 6);
  tt_int_op(qed_hs_log2(65),OP_EQ, 6);
  tt_int_op(qed_hs_log2(63),OP_EQ, 5);
  /* incorrect mathematically, but as specified: */
  tt_int_op(qed_hs_log2(0),OP_EQ, 0);
  tt_int_op(qed_hs_log2(1),OP_EQ, 0);
  tt_int_op(qed_hs_log2(2),OP_EQ, 1);
  tt_int_op(qed_hs_log2(3),OP_EQ, 1);
  tt_int_op(qed_hs_log2(4),OP_EQ, 2);
  tt_int_op(qed_hs_log2(5),OP_EQ, 2);
  tt_int_op(qed_hs_log2(UINT64_C(40000000000000000)),OP_EQ, 55);
  tt_int_op(qed_hs_log2(UINT64_MAX),OP_EQ, 63);
...
test_util_compress_impl(compress_method_t method)
{
  ...
  tt_assert(qed_hs_compress_supports_method(method));

  if (method != NO_METHOD) {
    tt_ptr_op(qed_hs_compress_version_str(method), OP_NE, NULL);
    tt_ptr_op(qed_hs_compress_header_version_str(method), OP_NE, NULL);
  }

  buf1 = qed_hs_strdup("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAZAAAAAAAAAAAAAAAAAAAZ");
  tt_assert(detect_compression_method(buf1, strlen(buf1)) == UNKNOWN_METHOD);

  tt_assert(!qed_hs_compress(&buf2, &len1, buf1, strlen(buf1)+1, method));
  ...
  tt_assert(!qed_hs_uncompress(&buf3, &len2, buf2, len1, method, 1, LOG_INFO));
  ...
  qed_hs_free(buf3);
  buf2 = qed_hs_reallocarray(buf2, len1, 2);
  memcpy(buf2+len1, buf2, len1);
  tt_assert(!qed_hs_uncompress(&buf3, &len2, buf2, len1*2, method, 1, LOG_INFO));
  ...
  buf1 = qed_hs_malloc(b1len);
  crypto_rand(buf1, b1len);
  tt_assert(!qed_hs_compress(&buf2, &len1, buf1, b1len, method));
  tt_int_op(len1, OP_GT, 16);
  /* when we allow an incomplete output we should succeed.*/
  tt_assert(!qed_hs_uncompress(&buf3, &len2, buf2, len1-16,
                            method, 0, LOG_INFO));
                               ...
                               compression_level_t level)
{
  ...
  qed_hs_compress_state_t *state = NULL;
  state = qed_hs_compress_new(1, method, level);
  tt_assert(state);
  cp1 = buf1 = qed_hs_malloc(1024);
  len1 = 1024;
  ccp2 = "ABCDEFGHIJABCDEFGHIJ";
  len2 = 21;
  tt_int_op(qed_hs_compress_process(state, &cp1, &len1, &ccp2, &len2, 0),
            OP_EQ, QED_HS_COMPRESS_OK);
  tt_int_op(0, OP_EQ, len2); /* Make sure we compressed it all. */
  tt_assert(cp1 > buf1);

  len2 = 0;
  cp2 = cp1;
  tt_int_op(qed_hs_compress_process(state, &cp1, &len1, &ccp2, &len2, 1),
            OP_EQ, QED_HS_COMPRESS_DONE);
  ...
  tt_int_op(qed_hs_compress_state_size(state), OP_GT, 0);

  tt_assert(!qed_hs_uncompress(&buf3, &len2, buf1, 1024-len1,
                            method, 1, LOG_WARN));
...
{
  qed_hs_assert(testcase->setup_data);
  qed_hs_assert(testcase->setup_data != (void*)TT_SKIP);
  const char *methodname = testcase->setup_data;

  if (!strcmp(methodname, "x-zstd:nostatic")) {
    methodname = "x-zstd";
    qed_hs_zstd_set_static_apis_disabled_for_testing(1);
  }
...
compression_test_cleanup(const struct testcase_t *testcase, void *ptr)
{
  (void)testcase;
  (void)ptr;
  qed_hs_zstd_set_static_apis_disabled_for_testing(0);
  return 1;
}
...
test_util_compress(void *arg)
{
  ...
  if (! qed_hs_compress_supports_method(method)) {
    tt_skip();
  }
...
test_util_decompress_concatenated_impl(compress_method_t method)
{
  ...
  r = qed_hs_compress(&c1, &sz1, input, 2048, method);
  tt_int_op(r, OP_EQ, 0);
  r = qed_hs_compress(&c2, &sz2, input+2048, 2048, method);
  tt_int_op(r, OP_EQ, 0);

  /* concatenate the chunks. */
  sz3 = sz1 + sz2;
  c3 = qed_hs_malloc(sz3);
  memcpy(c3, c1, sz1);
  memcpy(c3+sz1, c2, sz2);

  /* decompress the concatenated result */
  r = qed_hs_uncompress(&result, &szr, c3, sz3, method, 0, LOG_WARN);
  tt_int_op(r, OP_EQ, 0);
...
test_util_decompress_junk_impl(compress_method_t method)
{
  ...
  r = qed_hs_uncompress(&result, &szr, input, sz, method, 0, LOG_WARN);
  tt_int_op(r, OP_EQ, -1);
  ...
  /* Now try again, with a compressed object that starts out good and turns to
     junk. */
  crypto_rand(input, sizeof(input));
  r = qed_hs_compress(&result, &szr, input, sizeof(input), method);
  tt_int_op(r, OP_EQ, 0);
  crypto_rand(result+szr/2, szr-(szr/2)); // trash the 2nd half of the result
  r = qed_hs_uncompress(&result2, &szr2, result, szr, method, 0, LOG_WARN);
  tt_int_op(r, OP_EQ, -1);
  expect_log_msg_containing("Error while uncompressing data: bad input?");
...
/* mock replacement for qed_hs_compress_is_compression_bomb that doesn't
...
test_util_decompress_dos_impl(compress_method_t method)
{
  ...
  /* one megabyte of 0s. */
  input = qed_hs_malloc_zero(big);

  /* Compress it into "result": it should fail. */
  setup_full_capture_of_logs(LOG_WARN);
  r = qed_hs_compress(&result, &szr, input, big, method);
  tt_int_op(r, OP_EQ, -1);
  ...
  MOCK(qed_hs_compress_is_compression_bomb, mock_is_never_compression_bomb);
  r = qed_hs_compress(&result, &szr, input, big, method);
  UNMOCK(qed_hs_compress_is_compression_bomb);
  tt_int_op(r, OP_EQ, 0);
...
test_util_gzip_compression_bomb(void *arg)
{
  ...
  char *one_mb = qed_hs_malloc_zero(one_million);
  char *result = NULL;
  size_t result_len = 0;
  qed_hs_compress_state_t *state = NULL;
  ...
  setup_full_capture_of_logs(LOG_WARN);
  tt_int_op(-1, OP_EQ, qed_hs_compress(&result, &result_len,
                                    one_mb, one_million,
                                    ZLIB_METHOD));
  expect_log_msg_containing(
  ...
  tt_int_op(-1, OP_EQ, qed_hs_uncompress(&result, &result_len,
                                      compression_bomb, 1039,
                                      ZLIB_METHOD, 0, LOG_WARN));

  /* Now try streaming that. */
  state = qed_hs_compress_new(0, ZLIB_METHOD, HIGH_COMPRESSION);
  qed_hs_compress_output_t r;
  const char *inp = compression_bomb;
  size_t inlen = 1039;
  do {
    char *outp = one_mb;
    size_t outleft = 4096; /* small on purpose */
    r = qed_hs_compress_process(state, &outp, &outleft, &inp, &inlen, 0);
    tt_int_op(inlen, OP_NE, 0);
  } while (r == QED_HS_COMPRESS_BUFFER_FULL);

  tt_int_op(r, OP_EQ, QED_HS_COMPRESS_ERROR);
...
{
  char *fname1 = qed_hs_strdup(get_fname("mapped_1"));
  char *fname2 = qed_hs_strdup(get_fname("mapped_2"));
  char *fname3 = qed_hs_strdup(get_fname("mapped_3"));
  const size_t buflen = 17000;
  char *buf = qed_hs_malloc(17000);
  qed_hs_mmap_t *mapping = NULL;

  (void)arg;
  crypto_rand(buf, buflen);

  mapping = qed_hs_mmap_file(fname1);
...
#ifdef _WIN32
  tt_int_op(0, OP_EQ, qed_hs_munmap_file(mapping));
  mapping = NULL;
  ...
  mapping = qed_hs_mmap_file(fname2);
  tt_assert(mapping);
  ...
  mapping = qed_hs_mmap_file(fname3);
  tt_assert(mapping);
...
test_util_sscanf(void *arg)
{
  ...
  (void)arg;
  tt_int_op(-1,OP_EQ, qed_hs_sscanf("123", "%i", &r)); /* %i is not supported */
  tt_int_op(-1,OP_EQ,
            qed_hs_sscanf("wrong", "%5c", s1)); /* %c cannot have a number. */
  tt_int_op(-1,OP_EQ, qed_hs_sscanf("hello", "%s", s1)); /* %s needs a number. */
  /* this will fail because we don't allow widths longer than 9999 */
  {
    huge = qed_hs_malloc(1000000);
    r = qed_hs_sscanf("prettylongstring", "%99999s", huge);
    qed_hs_free(huge);
    tt_int_op(-1,OP_EQ, r);
  }
#if 0
  /* GCC thinks these two are illegal. */
  test_eq(-1, qed_hs_sscanf("prettylongstring", "%0s", s1));
  test_eq(0, qed_hs_sscanf("prettylongstring", "%10s", NULL));
#endif
  /* No '%'-strings: always "success" */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("hello world", "hello world"));
  tt_int_op(0,OP_EQ, qed_hs_sscanf("hello world", "good bye"));
  /* Excess data */
  tt_int_op(0,OP_EQ,
            qed_hs_sscanf("hello 3", "%u", &u1));  /* have to match the start */
  tt_int_op(0,OP_EQ, qed_hs_sscanf(" 3 hello", "%u", &u1));
  tt_int_op(0,OP_EQ,
            qed_hs_sscanf(" 3 hello", "%2u", &u1)); /* not even in this case */
  tt_int_op(1,OP_EQ,
            qed_hs_sscanf("3 hello", "%u", &u1));  /* but trailing is alright */

  /* Numbers (ie. %u) */
  tt_int_op(0,OP_EQ,
            qed_hs_sscanf("hello world 3", "hello worlb %u", &u1)); /* d vs b */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("12345", "%u", &u1));
  tt_int_op(12345u,OP_EQ, u1);
  tt_int_op(1,OP_EQ, qed_hs_sscanf("12346 ", "%u", &u1));
  tt_int_op(12346u,OP_EQ, u1);
  tt_int_op(0,OP_EQ, qed_hs_sscanf(" 12347", "%u", &u1));
  tt_int_op(1,OP_EQ, qed_hs_sscanf(" 12348", " %u", &u1));
  tt_int_op(12348u,OP_EQ, u1);
  tt_int_op(1,OP_EQ, qed_hs_sscanf("0", "%u", &u1));
  tt_int_op(0u,OP_EQ, u1);
  tt_int_op(1,OP_EQ, qed_hs_sscanf("0000", "%u", &u2));
  tt_int_op(0u,OP_EQ, u2);
  tt_int_op(0,OP_EQ, qed_hs_sscanf("", "%u", &u1)); /* absent number */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("A", "%u", &u1)); /* bogus number */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("-1", "%u", &u1)); /* negative number */

  /* Numbers with size (eg. %2u) */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("-1", "%2u", &u1));
  tt_int_op(2,OP_EQ, qed_hs_sscanf("123456", "%2u%u", &u1, &u2));
  tt_int_op(12u,OP_EQ, u1);
  tt_int_op(3456u,OP_EQ, u2);
  tt_int_op(1,OP_EQ, qed_hs_sscanf("123456", "%8u", &u1));
  tt_int_op(123456u,OP_EQ, u1);
  tt_int_op(1,OP_EQ, qed_hs_sscanf("123457  ", "%8u", &u1));
  tt_int_op(123457u,OP_EQ, u1);
  tt_int_op(0,OP_EQ, qed_hs_sscanf("  123456", "%8u", &u1));
  tt_int_op(3,OP_EQ, qed_hs_sscanf("!12:3:456", "!%2u:%2u:%3u", &u1, &u2, &u3));
  tt_int_op(12u,OP_EQ, u1);
  tt_int_op(3u,OP_EQ, u2);
  tt_int_op(456u,OP_EQ, u3);
  tt_int_op(3,OP_EQ,
            qed_hs_sscanf("67:8:099", "%2u:%2u:%3u", &u1, &u2, &u3)); /* 0s */
  tt_int_op(67u,OP_EQ, u1);
  tt_int_op(8u,OP_EQ, u2);
  tt_int_op(99u,OP_EQ, u3);
  /* %u does not match space.*/
  tt_int_op(2,OP_EQ, qed_hs_sscanf("12:3: 45", "%2u:%2u:%3u", &u1, &u2, &u3));
  tt_int_op(12u,OP_EQ, u1);
  tt_int_op(3u,OP_EQ, u2);
  /* %u does not match negative numbers. */
  tt_int_op(2,OP_EQ, qed_hs_sscanf("67:8:-9", "%2u:%2u:%3u", &u1, &u2, &u3));
  tt_int_op(67u,OP_EQ, u1);
  tt_int_op(8u,OP_EQ, u2);
  /* Arbitrary amounts of 0-padding are okay */
  tt_int_op(3,OP_EQ, qed_hs_sscanf("12:03:000000000000000099", "%2u:%2u:%u",
                        &u1, &u2, &u3));
  tt_int_op(12u,OP_EQ, u1);
  tt_int_op(3u,OP_EQ, u2);
  tt_int_op(99u,OP_EQ, u3);
  ...
  /* Hex (ie. %x) */
  tt_int_op(3,OP_EQ,
            qed_hs_sscanf("1234 02aBcdEf ff", "%x %x %x", &u1, &u2, &u3));
  tt_int_op(0x1234,OP_EQ, u1);
  tt_int_op(0x2ABCDEF,OP_EQ, u2);
  tt_int_op(0xFF,OP_EQ, u3);
  /* Width works on %x */
  tt_int_op(3,OP_EQ, qed_hs_sscanf("f00dcafe444", "%4x%4x%u", &u1, &u2, &u3));
  tt_int_op(0xf00d,OP_EQ, u1);
  tt_int_op(0xcafe,OP_EQ, u2);
  tt_int_op(444,OP_EQ, u3);

  /* Literal '%' (ie. '%%') */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("99% fresh", "%3u%% fresh", &u1));
  tt_int_op(99,OP_EQ, u1);
  tt_int_op(0,OP_EQ, qed_hs_sscanf("99 fresh", "%% %3u %s", &u1, s1));
  tt_int_op(1,OP_EQ, qed_hs_sscanf("99 fresh", "%3u%% %s", &u1, s1));
  tt_int_op(2,OP_EQ, qed_hs_sscanf("99 fresh", "%3u %5s %%", &u1, s1));
  tt_int_op(99,OP_EQ, u1);
  tt_str_op(s1,OP_EQ, "fresh");
  tt_int_op(1,OP_EQ, qed_hs_sscanf("% boo", "%% %3s", s1));
  tt_str_op("boo",OP_EQ, s1);

  /* Strings (ie. %s) */
  tt_int_op(2,OP_EQ, qed_hs_sscanf("hello", "%3s%7s", s1, s2));
  tt_str_op(s1,OP_EQ, "hel");
  tt_str_op(s2,OP_EQ, "lo");
  tt_int_op(2,OP_EQ, qed_hs_sscanf("WD40", "%2s%u", s3, &u1)); /* %s%u */
  tt_str_op(s3,OP_EQ, "WD");
  tt_int_op(40,OP_EQ, u1);
  tt_int_op(2,OP_EQ, qed_hs_sscanf("WD40", "%3s%u", s3, &u1)); /* %s%u */
  tt_str_op(s3,OP_EQ, "WD4");
  tt_int_op(0,OP_EQ, u1);
  tt_int_op(2,OP_EQ, qed_hs_sscanf("76trombones", "%6u%9s", &u1, s1)); /* %u%s */
  tt_int_op(76,OP_EQ, u1);
  tt_str_op(s1,OP_EQ, "trombones");
  {
    huge = qed_hs_malloc(1000);
    r = qed_hs_sscanf("prettylongstring", "%999s", huge);
    tt_int_op(1,OP_EQ, r);
    tt_str_op(huge,OP_EQ, "prettylongstring");
    qed_hs_free(huge);
  }
  /* %s doesn't eat spaces */
  tt_int_op(2,OP_EQ, qed_hs_sscanf("hello world", "%9s %9s", s1, s2));
  tt_str_op(s1,OP_EQ, "hello");
  tt_str_op(s2,OP_EQ, "world");
  tt_int_op(2,OP_EQ, qed_hs_sscanf("bye   world?", "%9s %9s", s1, s2));
  tt_str_op(s1,OP_EQ, "bye");
  tt_str_op(s2,OP_EQ, "");
  tt_int_op(3,OP_EQ,
            qed_hs_sscanf("hi", "%9s%9s%3s", s1, s2, s3)); /* %s can be empty. */
  tt_str_op(s1,OP_EQ, "hi");
  tt_str_op(s2,OP_EQ, "");
  tt_str_op(s3,OP_EQ, "");

  tt_int_op(3,OP_EQ, qed_hs_sscanf("1.2.3", "%u.%u.%u%c", &u1, &u2, &u3, &ch));
  tt_int_op(4,OP_EQ,
            qed_hs_sscanf("1.2.3 foobar", "%u.%u.%u%c", &u1, &u2, &u3, &ch));
  tt_int_op(' ',OP_EQ, ch);

  r = qed_hs_sscanf("12345 -67890 -1", "%d %ld %d", &int1, &lng1, &int2);
  tt_int_op(r,OP_EQ, 3);
  ...
  tt_int_op(1,OP_EQ, qed_hs_sscanf("4294967295", "%u", &u1));
  tt_int_op(4294967295U,OP_EQ, u1);

  /* But UINT32_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("4294967296", "%u", &u1));
  /* but parsing only 9... */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("4294967296", "%9u", &u1));
  tt_int_op(429496729U,OP_EQ, u1);

  /* %x */
  /* UINT32_MAX should work */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("FFFFFFFF", "%x", &u1));
  tt_int_op(0xFFFFFFFF,OP_EQ, u1);

  /* But UINT32_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("100000000", "%x", &u1));

  /* %d */
  /* INT32_MIN and INT32_MAX should work */
  r = qed_hs_sscanf("-2147483648. 2147483647.", "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 2);
  tt_int_op(int1,OP_EQ, -2147483647 - 1);
  tt_int_op(int2,OP_EQ, 2147483647);

  /* But INT32_MIN - 1 and INT32_MAX + 1 shouldn't work */
  r = qed_hs_sscanf("-2147483649.", "%d.", &int1);
  tt_int_op(r,OP_EQ, 0);

  r = qed_hs_sscanf("2147483648.", "%d.", &int1);
  tt_int_op(r,OP_EQ, 0);

  /* and the first failure stops further processing */
  r = qed_hs_sscanf("-2147483648. 2147483648.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 1);

  r = qed_hs_sscanf("-2147483649. 2147483647.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 0);

  r = qed_hs_sscanf("2147483648. -2147483649.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 0);
#elif SIZEOF_INT == 8
  /* %u */
  /* UINT64_MAX should work */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("18446744073709551615", "%u", &u1));
  tt_int_op(18446744073709551615U,OP_EQ, u1);

  /* But UINT64_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("18446744073709551616", "%u", &u1));
  /* but parsing only 19... */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("18446744073709551616", "%19u", &u1));
  tt_int_op(1844674407370955161U,OP_EQ, u1);

  /* %x */
  /* UINT64_MAX should work */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("FFFFFFFFFFFFFFFF", "%x", &u1));
  tt_int_op(0xFFFFFFFFFFFFFFFF,OP_EQ, u1);

  /* But UINT64_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("10000000000000000", "%x", &u1));

  /* %d */
  /* INT64_MIN and INT64_MAX should work */
  r = qed_hs_sscanf("-9223372036854775808. 9223372036854775807.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 2);
  ...
  r = qed_hs_sscanf("-9223372036854775809.", "%d.", &int1);
  tt_int_op(r,OP_EQ, 0);

  r = qed_hs_sscanf("9223372036854775808.", "%d.", &int1);
  tt_int_op(r,OP_EQ, 0);

  /* and the first failure stops further processing */
  r = qed_hs_sscanf("-9223372036854775808. 9223372036854775808.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 1);

  r = qed_hs_sscanf("-9223372036854775809. 9223372036854775807.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 0);

  r = qed_hs_sscanf("9223372036854775808. -9223372036854775809.",
                 "%d. %d.", &int1, &int2);
  tt_int_op(r,OP_EQ, 0);
#endif /* SIZEOF_INT == 4 || ... */
  ...
  /* %lu */
  /* UINT32_MAX should work */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("4294967295", "%lu", &ulng));
  tt_int_op(4294967295UL,OP_EQ, ulng);

  /* But UINT32_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("4294967296", "%lu", &ulng));
  /* but parsing only 9... */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("4294967296", "%9lu", &ulng));
  tt_int_op(429496729UL,OP_EQ, ulng);

  /* %lx */
  /* UINT32_MAX should work */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("FFFFFFFF", "%lx", &ulng));
  tt_int_op(0xFFFFFFFFUL,OP_EQ, ulng);

  /* But UINT32_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("100000000", "%lx", &ulng));

  /* %ld */
  /* INT32_MIN and INT32_MAX should work */
  r = qed_hs_sscanf("-2147483648. 2147483647.", "%ld. %ld.", &lng1, &lng2);
  tt_int_op(r,OP_EQ, 2);
  tt_int_op(lng1,OP_EQ, -2147483647L - 1L);
  tt_int_op(lng2,OP_EQ, 2147483647L);

  /* But INT32_MIN - 1 and INT32_MAX + 1 shouldn't work */
  r = qed_hs_sscanf("-2147483649.", "%ld.", &lng1);
  tt_int_op(r,OP_EQ, 0);

  r = qed_hs_sscanf("2147483648.", "%ld.", &lng1);
  tt_int_op(r,OP_EQ, 0);
  ...
  /* %lu */
  /* UINT64_MAX should work */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("18446744073709551615", "%lu", &ulng));
  tt_int_op(18446744073709551615UL,OP_EQ, ulng);

  /* But UINT64_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("18446744073709551616", "%lu", &ulng));
  /* but parsing only 19... */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("18446744073709551616", "%19lu", &ulng));
  tt_int_op(1844674407370955161UL,OP_EQ, ulng);

  /* %lx */
  /* UINT64_MAX should work */
  tt_int_op(1,OP_EQ, qed_hs_sscanf("FFFFFFFFFFFFFFFF", "%lx", &ulng));
  tt_int_op(0xFFFFFFFFFFFFFFFFUL,OP_EQ, ulng);

  /* But UINT64_MAX + 1 shouldn't work */
  tt_int_op(0,OP_EQ, qed_hs_sscanf("10000000000000000", "%lx", &ulng));
  ...
  /* But INT64_MIN - 1 and INT64_MAX + 1 shouldn't work */
  r = qed_hs_sscanf("-9223372036854775809.", "%ld.", &lng1);
  tt_int_op(r,OP_EQ, 0);

  r = qed_hs_sscanf("9223372036854775808.", "%ld.", &lng1);
  tt_int_op(r,OP_EQ, 0);
  ...
  r = qed_hs_sscanf("123.456 .000007 -900123123.2000787 00003.2",
  ...
  /* missing float */
  r = qed_hs_sscanf("3 ", "%d %lf", &int1, &d1);
  tt_int_op(r, OP_EQ, 1);
  tt_int_op(int1, OP_EQ, 3);

  /* not a float */
  r = qed_hs_sscanf("999 notafloat", "%d %lf", &int1, &d1);
  tt_int_op(r, OP_EQ, 1);
  tt_int_op(int1, OP_EQ, 999);

  /* %s but no buffer. */
  char *nullbuf = NULL;
  r = qed_hs_sscanf("hello", "%3s", nullbuf);
  tt_int_op(r, OP_EQ, 0);
...
#define tt_ci_char_op(a,op,b) \
  tt_char_op(QED_HS_TOLOWER((int)a),op,QED_HS_TOLOWER((int)b))
...
#define TL_       T_ " " L_
  ...
  r = qed_hs_sscanf(dbuf, TL_, &sec, label_s);
  tt_int_op(r,OP_EQ, 2);
  ...
  tt_int_op(strnlen(dbuf, DBUF_SIZE),OP_LE, DBUF_SIZE - 1);
  r = qed_hs_sscanf(dbuf, TL_, &min, label_m);
  tt_int_op(r,OP_EQ, 2);
  ...
  tt_int_op(strnlen(dbuf, DBUF_SIZE),OP_LE, DBUF_SIZE - 1);
  r = qed_hs_sscanf(dbuf, TL_, &hour, label_h);
  tt_int_op(r,OP_EQ, 2);
  ...
  tt_int_op(strnlen(dbuf, DBUF_SIZE),OP_LE, DBUF_SIZE - 1);
  r = qed_hs_sscanf(dbuf, TL_, &day, label_d);
  tt_int_op(r,OP_EQ, 2);
  ...
  format_time_interval(dbuf, sizeof(dbuf), 24*60*60 + 60);
  tt_int_op(strnlen(dbuf, DBUF_SIZE),OP_LE, DBUF_SIZE - 1);
  r = qed_hs_sscanf(dbuf, TL_ " " TL_ " " TL_,
...
test_util_memarea(void *arg)
{
  ...
  {
    malloced_ptr = qed_hs_malloc(64);
    tt_assert(!memarea_owns_ptr(area, malloced_ptr));
    qed_hs_free(malloced_ptr);
  }
...
test_util_datadir(void *arg)
{
  ...
  qed_hs_snprintf(buf, sizeof(buf), "%s"PATH_SEPARATOR"state", temp_dir);
  tt_str_op(f,OP_EQ, buf);
  qed_hs_free(f);
  f = get_datadir_fname2("cache", "thingy");
  qed_hs_snprintf(buf, sizeof(buf),
  ...
  qed_hs_snprintf(buf, sizeof(buf), "%s"PATH_SEPARATOR"cache.foo",
               temp_dir);
  tt_str_op(f,OP_EQ, buf);

 done:
  qed_hs_free(f);
  qed_hs_free(temp_dir);
}
...
test_util_strtok(void *arg)
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    qed_hs_snprintf(buf, sizeof(buf), "%s", pad1);
    qed_hs_snprintf(buf2, sizeof(buf2), "%s", pad2);
    tt_ptr_op(qed_hs_strtok_r_impl(buf, " ", &cp1), OP_EQ, NULL);
    tt_ptr_op(qed_hs_strtok_r_impl(buf2, ".!..;!", &cp2), OP_EQ, NULL);

    qed_hs_snprintf(buf, sizeof(buf),
                 "%sGraved on the dark  in gestures of descent%s", pad1, pad1);
    qed_hs_snprintf(buf2, sizeof(buf2),
                "%sthey.seemed;;their!.own;most.perfect;monument%s",pad2,pad2);
    /*  -- "Year's End", Richard Wilbur */

    tt_str_op("Graved",OP_EQ, qed_hs_strtok_r_impl(buf, " ", &cp1));
    tt_str_op("they",OP_EQ, qed_hs_strtok_r_impl(buf2, ".!..;!", &cp2));
#define S1() qed_hs_strtok_r_impl(NULL, " ", &cp1)
#define S2() qed_hs_strtok_r_impl(NULL, ".!..;!", &cp2)
  ...
  buf[0] = 0;
  tt_ptr_op(NULL,OP_EQ, qed_hs_strtok_r_impl(buf, " ", &cp1));
  tt_ptr_op(NULL,OP_EQ, qed_hs_strtok_r_impl(buf, "!", &cp1));

  strlcpy(buf, "Howdy!", sizeof(buf));
  tt_str_op("Howdy",OP_EQ, qed_hs_strtok_r_impl(buf, "!", &cp1));
  tt_ptr_op(NULL,OP_EQ, qed_hs_strtok_r_impl(NULL, "!", &cp1));
  ...
  tt_str_op("something",OP_EQ, qed_hs_strtok_r_impl(buf, " ", &cp1));
  tt_ptr_op(NULL,OP_EQ, qed_hs_strtok_r_impl(NULL, ";", &cp1));
 done:
  ;
}
...
static void
test_util_qed_hs_strreplacechar(void *ptr)
{
  (void)ptr;
  char empty[] = "";
  ...
  qed_hs_strreplacechar(empty, 'a', 'b');
  tt_str_op(empty, OP_EQ, "");

  qed_hs_strreplacechar(not_contain, 'a', 'b');
  tt_str_op(not_contain, OP_EQ, "bbb");

  qed_hs_strreplacechar(contains, 'a', 'b');
  tt_str_op(contains, OP_EQ, "bbb");

  qed_hs_strreplacechar(contains_all, 'a', 'b');
...
#define LOREMIPSUM                                              \
  ...
  /* simple string */
  r = qed_hs_asprintf(&cp, "simple string 100%% safe");
  tt_assert(cp);
  ...
  /* numbers (%i) */
  r = qed_hs_asprintf(&cp, "I like numbers-%2i, %i, etc.", -1, 2);
  tt_assert(cp);
  ...
  /* numbers (%d) */
  r = qed_hs_asprintf(&cp2, "First=%d, Second=%d", 101, 202);
  tt_assert(cp2);
  ...
  r = qed_hs_asprintf(&cp, "Lorem1: %sLorem2: %s", LOREMIPSUM, LOREMIPSUM);
  tt_assert(cp);
  ...
  r = qed_hs_asprintf(&cp, "1: %s 2: %s 3: %s",
...
test_util_listdir(void *ptr)
{
  ...
  fname1 = qed_hs_strdup(get_fname("hopscotch"));
  fname2 = qed_hs_strdup(get_fname("mumblety-peg"));
  fname3 = qed_hs_strdup(get_fname(".hidden-file"));
  dir1   = qed_hs_strdup(get_fname("some-directory"));
  dirname = qed_hs_strdup(get_fname(NULL));
  ...
  dir_contents = qed_hs_listdir(dirname);
  ...
  qed_hs_free(dirname);
  if (dir_contents) {
    SMARTLIST_FOREACH(dir_contents, char *, cp, qed_hs_free(cp));
...
test_util_glob(void *ptr)
{
  ...
  dirname = qed_hs_strdup(get_fname("test_glob"));
  ...
  qed_hs_asprintf(&dir1, "%s"PATH_SEPARATOR"dir1", dirname);
  qed_hs_asprintf(&dir1_forbidden,
               "%s"PATH_SEPARATOR"dir1"PATH_SEPARATOR"forbidden", dirname);
  tt_int_op(0, OP_EQ, create_test_directory_structure(dir1));
  qed_hs_asprintf(&dir2, "%s"PATH_SEPARATOR"dir2", dirname);
  qed_hs_asprintf(&dir2_forbidden,
               "%s"PATH_SEPARATOR"dir2"PATH_SEPARATOR"forbidden", dirname);
  tt_int_op(0, OP_EQ, create_test_directory_structure(dir2));
  qed_hs_asprintf(&forbidden, "%s"PATH_SEPARATOR"forbidden", dirname);
  qed_hs_asprintf(&forbidden_forbidden,
...
#define TEST(input) \
  do { \
    qed_hs_asprintf(&pattern, "%s"PATH_SEPARATOR"%s", dirname, input); \
    results = qed_hs_glob(pattern); \
    qed_hs_free(pattern); \
    tt_assert(results); \
...
#define EXPECT(result) \
  do { \
    ...
    SMARTLIST_FOREACH_BEGIN(results, const char *, f) { \
      qed_hs_asprintf(&expected, "%s"PATH_SEPARATOR"%s", dirname, result[i]); \
      tt_str_op(f, OP_EQ, expected); \
      i++; \
      qed_hs_free(expected); \
    } SMARTLIST_FOREACH_END(f); \
    SMARTLIST_FOREACH(results, char *, f, qed_hs_free(f)); \
...
#define EXPECT_EMPTY() \
  ...
  if (getuid() != 0) {
    ...
    qed_hs_asprintf(&pattern, "%s"PATH_SEPARATOR"*"PATH_SEPARATOR"*", dirname);
    results = qed_hs_glob(pattern);
    qed_hs_free(pattern);
    tt_assert(!results);
  }
...
#undef EXPECT_EMPTY
  ...
  qed_hs_free(dir1);
  qed_hs_free(dir2);
  qed_hs_free(forbidden);
  qed_hs_free(dirname);
  qed_hs_free(dir1_forbidden);
  qed_hs_free(dir2_forbidden);
  qed_hs_free(forbidden_forbidden);
  qed_hs_free(expected);
  qed_hs_free(pattern);
  if (results) {
    SMARTLIST_FOREACH(results, char *, f, qed_hs_free(f));
...
test_util_get_glob_opened_files(void *ptr)
{
  ...
  dirname = qed_hs_strdup(get_fname("test_get_glob_opened_files"));
  tt_ptr_op(dirname, OP_NE, NULL);
...
#define TEST(input) \
  do { \
    if (*input) { \
      ...
      qed_hs_asprintf(&pattern, "%s", dirname); \
    } \
  ...
  do { \
    ...
    SMARTLIST_FOREACH_BEGIN(results, const char *, f) { \
      if (*result[i]) { \
        ...
        qed_hs_asprintf(&expected, "%s", dirname); \
      } \
      tt_str_op(f, OP_EQ, expected); \
      i++; \
      qed_hs_free(expected); \
...
test_util_ftruncate(void *ptr)
{
  ...
  tt_int_op((int)qed_hs_fd_getpos(fd), OP_EQ, strlen(message2));
  tt_int_op(0, OP_EQ, fstat(fd, &st));
...
#define EQ 0
  ...
  for (i = 0; examples[i].a; ++i) {
    ...
    eq1 = qed_hs_memeq(examples[i].a, examples[i].b, len);
    eq2 = qed_hs_memeq(examples[i].b, examples[i].a, len);
    neq1 = qed_hs_memneq(examples[i].a, examples[i].b, len);
    neq2 = qed_hs_memneq(examples[i].b, examples[i].a, len);
    cmp1 = qed_hs_memcmp(examples[i].a, examples[i].b, len);
    cmp2 = qed_hs_memcmp(examples[i].b, examples[i].a, len);
...

  {
    ...
    /* exhaustively test qed_hs_memeq and qed_hs_memcmp
    ...
    for (z = 0; z < 256; z++) {
      for (i = 0; i < 256; i++) {
        ii = (uint8_t)i;
        zz = (uint8_t)z;
        tt_int_op(qed_hs_memeq(&zz, &ii, 1),OP_EQ, zz == ii);
        tt_int_op(qed_hs_memcmp(&zz, &ii, 1) > 0 ? GT : EQ,OP_EQ,
                  zz > ii ? GT : EQ);
        tt_int_op(qed_hs_memcmp(&ii, &zz, 1) < 0 ? LT : EQ,OP_EQ,
                  ii < zz ? LT : EQ);
      }
    }
  }
...
test_util_di_map(void *arg)
{
  ...
  char *str1 = qed_hs_strdup("You are precisely as big as what you love"
                          " and precisely as small as what you allow"
                          " to annoy you.");
  char *str2 = qed_hs_strdup("Let us hope that Lysenko's success in Russia will"
  ...
  char *str3 = qed_hs_strdup("Don't write naughty words on walls "
                          "if you can't spell.");
...
test_util_sl_new_from_text_lines(void *ptr)
{
  ...
  { /* Normal usage */
    ...
    SMARTLIST_FOREACH(sl, void *, x, qed_hs_free(x));
    smartlist_free(sl);
  }
...
test_util_make_environment(void *ptr)
{
  ...
  tt_want(qed_hs_memeq(expected_windows_env_block, env->windows_environment_block,
  ...
  SMARTLIST_FOREACH(env_vars, char *, x, qed_hs_free(x));
...
test_util_set_env_var_in_sl(void *ptr)
{
  ...
  /* Clean up. */
  SMARTLIST_FOREACH(merged_env_vars, char *, x, qed_hs_free(x));
  smartlist_free(merged_env_vars);

  smartlist_free(new_env_vars);

  SMARTLIST_FOREACH(expected_resulting_env_vars, char *, x, qed_hs_free(x));
...
{
  int i, j, n[16];
  qed_hs_weak_rng_t rng;
  (void) arg;

  qed_hs_init_weak_random(&rng, (unsigned)time(NULL));

  for (i = 1; i <= 256; ++i) {
    for (j=0;j<100;++j) {
      int r = qed_hs_weak_random_range(&rng, i);
      tt_int_op(0, OP_LE, r);
      tt_int_op(r, OP_LT, i);
    }
  }
  ...
  memset(n,0,sizeof(n));
  for (j=0;j<8192;++j) {
    n[qed_hs_weak_random_range(&rng, 16)]++;
  }
...
static int
fd_is_cloexec(qed_hs_socket_t fd)
{
...
static int
fd_is_nonblocking(qed_hs_socket_t fd)
{
...
#define SOCK_ERR_IS_EPROTO(s) ERRNO_IS_EPROTO(qed_hs_socket_errno(s))

/* Test for qed_hs_open_socket*, using IPv4 or IPv6 depending on arg. */
static void
test_util_socket(void *arg)
{
  const int domain = !strcmp(arg, "4") ? AF_INET : AF_INET6;
  qed_hs_socket_t fd1 = QED_HS_INVALID_SOCKET;
  qed_hs_socket_t fd2 = QED_HS_INVALID_SOCKET;
  qed_hs_socket_t fd3 = QED_HS_INVALID_SOCKET;
  qed_hs_socket_t fd4 = QED_HS_INVALID_SOCKET;
  ...
  fd1 = qed_hs_open_socket_with_extensions(domain, SOCK_STREAM, 0, 0, 0);
  int err = qed_hs_socket_errno(fd1);
  ...
  fd2 = qed_hs_open_socket_with_extensions(domain, SOCK_STREAM, 0, 0, 1);
  tt_assert(SOCKET_OK(fd1));
  tt_assert(SOCKET_OK(fd2));
  tt_int_op(get_n_open_sockets(), OP_EQ, n + 2);
  //fd3 = qed_hs_open_socket_with_extensions(domain, SOCK_STREAM, 0, 1, 0);
  //fd4 = qed_hs_open_socket_with_extensions(domain, SOCK_STREAM, 0, 1, 1);
  fd3 = qed_hs_open_socket(domain, SOCK_STREAM, 0);
  fd4 = qed_hs_open_socket_nonblocking(domain, SOCK_STREAM, 0);
  ...
  qed_hs_assert(qed_hs_close_socket == qed_hs_close_socket__real);

  /* we use close_socket__real here so that coverity can tell that we are
   * really closing these sockets. */
  qed_hs_close_socket__real(fd1);
  qed_hs_close_socket__real(fd2);
  fd1 = fd2 = QED_HS_INVALID_SOCKET;
  tt_int_op(get_n_open_sockets(), OP_EQ, n + 2);
  qed_hs_close_socket__real(fd3);
  qed_hs_close_socket__real(fd4);
  fd3 = fd4 = QED_HS_INVALID_SOCKET;
...
{
  qed_hs_socket_t s;
  s = qed_hs_open_socket(family, SOCK_STREAM, IPPROTO_TCP);
  qed_hs_assert(SOCKET_OK(s));
 ...
 * the latter is a tolerably good way to exercise qed_hs_accept_socket(). */
static void
test_util_socketpair(void *arg)
{
  const int ersatz = !strcmp(arg, "1");
  int (*const qed_hs_socketpair_fn)(int, int, int, qed_hs_socket_t[2]) =
    ersatz ? qed_hs_ersatz_socketpair : qed_hs_socketpair;
  int n = get_n_open_sockets();
  qed_hs_socket_t fds[2] = {QED_HS_INVALID_SOCKET, QED_HS_INVALID_SOCKET};
  const int family = AF_UNIX;
  int socketpair_result = 0;

  socketpair_result = qed_hs_socketpair_fn(family, SOCK_STREAM, 0, fds);

#ifdef __FreeBSD__
  /* If there is no 127.0.0.1, qed_hs_ersatz_socketpair will and must fail.
  ...
  if (ersatz) {
    if (SOCKET_OK(fds[0]))
      qed_hs_close_socket_simple(fds[0]);
    if (SOCKET_OK(fds[1]))
      qed_hs_close_socket_simple(fds[1]);
  } else {
    if (SOCKET_OK(fds[0]))
      qed_hs_close_socket(fds[0]);
    if (SOCKET_OK(fds[1]))
      qed_hs_close_socket(fds[1]);
  }
}
...
test_util_max_mem(void *arg)
{
  ...
  TT_BLATHER(("System memory: %"QED_HS_PRIuSZ, (memory1)));
...
test_util_hostname_validation(void *arg)
{
  ...
  tt_assert(string_is_valid_nonrfc_hostname("qed-cc.org"));
...
test_util_writepid(void *arg)
{
  ...
  int n = qed_hs_sscanf(contents, "%lu\n%c", &pid, &c);
  tt_int_op(n, OP_EQ, 1);
...
test_util_get_avail_disk_space(void *arg)
{
  ...
  val = qed_hs_get_avail_disk_space("/akljasdfklsajdklasjkldjsa");
  tt_i64_op(val, OP_EQ, -1);

  /* Try the current directory */
  val = qed_hs_get_avail_disk_space(".");
...
test_util_pwdb(void *arg)
{
  ...
  for (i = 0; i < 100; ++i) {
    ...
    if (qed_hs_getpwnam(badname) == NULL) {
      found = 1;
      break;
    }
  }
...
test_util_nowrap_math(void *arg)
{
  ...
  tt_u64_op(0, OP_EQ, qed_hs_add_u32_nowrap(0, 0));
  tt_u64_op(1, OP_EQ, qed_hs_add_u32_nowrap(0, 1));
  tt_u64_op(1, OP_EQ, qed_hs_add_u32_nowrap(1, 0));
  tt_u64_op(4, OP_EQ, qed_hs_add_u32_nowrap(2, 2));
  tt_u64_op(UINT32_MAX, OP_EQ, qed_hs_add_u32_nowrap(UINT32_MAX-1, 2));
  tt_u64_op(UINT32_MAX, OP_EQ, qed_hs_add_u32_nowrap(2, UINT32_MAX-1));
  tt_u64_op(UINT32_MAX, OP_EQ, qed_hs_add_u32_nowrap(UINT32_MAX, UINT32_MAX));

  tt_u64_op(0, OP_EQ, qed_hs_mul_u64_nowrap(0, 0));
  tt_u64_op(1, OP_EQ, qed_hs_mul_u64_nowrap(1, 1));
  tt_u64_op(2, OP_EQ, qed_hs_mul_u64_nowrap(2, 1));
  tt_u64_op(4, OP_EQ, qed_hs_mul_u64_nowrap(2, 2));
  tt_u64_op(UINT64_MAX, OP_EQ, qed_hs_mul_u64_nowrap(UINT64_MAX, 1));
  tt_u64_op(UINT64_MAX, OP_EQ, qed_hs_mul_u64_nowrap(2, UINT64_MAX));
  tt_u64_op(UINT64_MAX, OP_EQ, qed_hs_mul_u64_nowrap(UINT64_MAX, UINT64_MAX));
...
test_util_htonll(void *arg)
{
  ...
  tt_u64_op(0, OP_EQ, qed_hs_htonll(0));
  tt_u64_op(0, OP_EQ, qed_hs_ntohll(0));
  tt_u64_op(UINT64_MAX, OP_EQ, qed_hs_htonll(UINT64_MAX));
  tt_u64_op(UINT64_MAX, OP_EQ, qed_hs_ntohll(UINT64_MAX));

#ifdef WORDS_BIGENDIAN
  tt_u64_op(res_be, OP_EQ, qed_hs_htonll(0x8877665544332211));
  tt_u64_op(res_be, OP_EQ, qed_hs_ntohll(0x8877665544332211));
#else
  tt_u64_op(res_le, OP_EQ, qed_hs_htonll(0x8877665544332211));
  tt_u64_op(res_le, OP_EQ, qed_hs_ntohll(0x8877665544332211));
...
test_util_map_anon(void *arg)
{
  ...
  /* Basic checks. */
  ptr = qed_hs_mmap_anonymous(sz, 0, &inherit);
  ...
  qed_hs_munmap_anonymous(ptr, sz);
  ptr = qed_hs_mmap_anonymous(sz, ANONMAP_PRIVATE, &inherit);
  tt_ptr_op(ptr, OP_NE, 0);
  tt_int_op(inherit, OP_EQ, INHERIT_RES_KEEP);
  ptr[sz-1] = 10;
  tt_int_op(ptr[0], OP_EQ, 0);
  ...
  qed_hs_munmap_anonymous(ptr, sz);
  ptr = qed_hs_mmap_anonymous(sz, ANONMAP_NOINHERIT, &inherit);
  tt_ptr_op(ptr, OP_NE, 0);
  ptr[sz-1] = 10;
  tt_int_op(ptr[0], OP_EQ, 0);
  tt_int_op(ptr[sz/2], OP_EQ, 0);
...
struct testcase_t util_tests[] = {
  ...
  UTIL_TEST(qed_hs_strreplacechar, 0),
  ...
  { "qed_hs_isinf", test_qed_hs_isinf, TT_FORK, NULL, NULL },
