...
{
  pubsub_connecqed_hs_t *c = NULL;

  {
    c = pubsub_connecqed_hs_for_subsystem(b, get_subsys_id("sys1"));
    ...
    pubsub_connecqed_hs_free(c);
  }

  {
    c = pubsub_connecqed_hs_for_subsystem(b, get_subsys_id("sys2"));
...
test_pubsub_build_types_decls_conflict(void *arg)
{
  ...
  seed_pubsub_builder_basic(b);
  {
    c = pubsub_connecqed_hs_for_subsystem(b, get_subsys_id("sys3"));
...
test_pubsub_build_channels_conflict(void *arg)
{
  ...
  {
    c = pubsub_connecqed_hs_for_subsystem(b, get_subsys_id("problems"));
...
test_pubsub_build_sub_many(void *arg)
{
  ...
  int i;
  for (i = 1; i < 100; ++i) {
    qed_hs_asprintf(&sysname, "system%d",i);
    c = pubsub_connecqed_hs_for_subsystem(b, get_subsys_id(sysname));
    if (i % 7) {
      DISPATCH_ADD_SUB(c, main, bunch_of_coconuts);
    } else {
