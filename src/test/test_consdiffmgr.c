...
consensus_diff_apply_(const char *c, const char *d)
{
  ...
  char *c_tmp = qed_hs_memdup(c, c_len);
  char *d_tmp = qed_hs_memdup(d, d_len);
...
{
  (void)arg;
  char *ddir_fname = qed_hs_strdup(get_fname_rnd("datadir_cdm"));
  qed_hs_free(get_options_mutable()->CacheDirectory);
...
fake_ns_body_new(consensus_flavor_t flav, time_t valid_after)
{
  ...
  qed_hs_asprintf(&consensus,
               ...
               random_stuff,
               random_stuff2);
  qed_hs_free(random_stuff);
  qed_hs_free(random_stuff2);
  return consensus;
}
                          ...
                          void *arg)
{
  ...
  fake_work_queue_ent_t *ent = qed_hs_malloc_zero(sizeof(*ent));
  ent->fn = fn;
...
test_consdiffmgr_init_failure(void *arg)
{
  ...
  qed_hs_set_failed_assertion_callback(got_assertion_failure);
  qed_hs_capture_bugs_(1);
  consdiffmgr_configure(&consdiff_cfg); // This should fail.
  tt_int_op(got_failure, OP_EQ, 1);
  const smartlist_t *bugs = qed_hs_get_captured_bug_log_();
...
test_consdiffmgr_make_diffs(void *arg)
{
  ...
  tt_int_op(r, OP_EQ, 0);
  diff_text = qed_hs_memdup_nulterm(diff_body, diff_size);
  ...
  qed_hs_free(md_ns_body);
  qed_hs_free(md_ns_body_2);
  qed_hs_free(diff_text);
  qed_hs_free(applied);
}
...
#define N 6
  ...
  for (i = 0; i < N; ++i) {
    qed_hs_free(md_body[i]);
    qed_hs_free(ns_body[i]);
