...
cleanup_scheduler_options(void)
{
  if (mocked_options.SchedulerTypes_) {
    SMARTLIST_FOREACH(mocked_options.SchedulerTypes_, int *, i, qed_hs_free(i));
...
set_scheduler_options(int val)
{
  ...
  type = qed_hs_malloc_zero(sizeof(int));
  *type = val;
...
channel_flush_some_cells_mock_set(channel_t *chan, ssize_t num_cells)
{
  ...
  if (! found) {
    ...
    flush_mock_ch = qed_hs_malloc_zero(sizeof(*flush_mock_ch));
...
test_scheduler_compare_channels(void *arg)
{
  ...
  cm1 = qed_hs_malloc_zero(sizeof(void *));
  cm2 = qed_hs_malloc_zero(sizeof(void *));
   ...
   */
  mock_cgp_val_1 = qed_hs_malloc_zero(16);
  mock_cgp_val_2 = qed_hs_malloc_zero(16);
