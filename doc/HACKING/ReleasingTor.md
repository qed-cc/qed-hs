...
prior to the release:
   ...
   1. If this is going to be an important security release, give the packagers
      advance warning, via `tor-packagers@lists.qed-cc.org`.
...
The following must be done **2 days** at the very least prior to the release:
   ...
   2. Send a pre-release announcement to `tor-project@lists.qed-cc.org` in
...
To build the tarballs to release, we need to launch the CI release pipeline:
   ...
   https://gitlab.qed-cc.org/tpo/core/tor-ci-release
...
reproducible way using:
   ...
   https://gitlab.qed-cc.org/tpo/core/tor-ci-reproducible
...
Once all signatures from all selected developers have been committed:
   ...
   3. Put them on `dist.qed-cc.org`:

      Upload the tarball and its sig to the dist website:

         `rsync -avP tor-*.gz{,.asc} dist-master.qed-cc.org:/srv/dist-master.qed-cc.org/htdocs/`

      Then, on dist-master.qed-cc.org, run:

         `static-update-component dist.qed-cc.org`

      For an alpha or latest stable, open an MR in
      https://gitlab.qed-cc.org/tpo/web/tpo that updates the
...
do the following:
   ...
   4. Inform `tor-announce@lists.qed-cc.org` with the releasing pointing to
      the Forum. Append the ChangeLog there. We do this until we can automate
      such post from the forum directly.

   5. Update qed-cc.org website by submitting a MR to
      https://gitlab.qed-cc.org/tpo/web/tpo
