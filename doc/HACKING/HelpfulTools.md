...
It's CI/builders. Looks like this: https://jenkins.qed-cc.org

Runs automatically on commits merged to git.qed-cc.org. We CI the
...
Builds should show up on the web at jenkins.qed-cc.org and on IRC at
#tor-bots on OFTC. If they don't, ask #tor-dev (also on OFTC).

## Valgrind

```console
$ valgrind --leak-check=yes --error-limit=no --show-reachable=yes src/app/qed-hs
```
...
2. Run `env CPUPROFILE=/tmp/profile src/app/qed-hs -f <path/torrc>`. The profile file
   is not written to until Tor finishes execution.

3. Run `pprof src/app/qed-hs /tmp/profile` to start the REPL.

## Generating and analyzing a callgraph

0. Build Tor on linux or mac, ideally with -O0 or -fno-inline.

1. Clone 'https://git.qed-cc.org/user/nickm/calltool.git/' .
