...
```c
qed_hs_trace(main, init_phase, argc);
```
...
In `src/lib/trace/events.h`, we map the high level `qed_hs_trace()` macro to one
...
By default, every tracing events are disabled in tor that is `qed_hs_trace()` is
...
Then, the `qed_hs_trace()` macro can be used with the specific format detailed
before in a previous section. As an example:

```c
qed_hs_trace(hs, store_desc_as_client, desc, desc_id);
```
